...
// ---- Stage-module accumulate launches --------------------------------------
//
// The TP combine family (bf16 add, relay add, tp4 tree, u64 max) is launched
// from private per-family copies whose geometry and argument policy have
// already drifted: dsv4 validates every launcher except the plain AccumAdd its
// transport calls per layer, and qwen36 runs a flat element-count grid where
// dsv4 and glm52 run one CTA per row. As with LmLaunchGroupedTileM above, the
// DECISION is exported here once and checked on a host by tests/test_launch.c.
// The kernel BODIES deliberately stay per-family: truncating and
// round-to-nearest-even bf16 pair stores diverge on 17.97-42.54% of add
// results depending on operand distribution (bit-exact host measurement,
// .agents/coord/dsv4pro_cuda_kernels_decision.md §3), so one shared body
// before a store-discipline ruling would silently change TP all-reduce output.
#define LM_LAUNCH_ACCUM_THREADS 256u

// Row-shaped accumulators: one CTA per row; rows are independent.
static inline uint32_t LmLaunchAccumGridRows(uint32_t row_count)
{
	return(row_count);
}

// Flat grid for accumulators addressed as an element count, not rows.
static inline uint32_t LmLaunchAccumGridElements(uint64_t element_count)
{
	return((uint32_t)((element_count + LM_LAUNCH_ACCUM_THREADS - 1u)
		/ LM_LAUNCH_ACCUM_THREADS));
}

// The one width policy every bf16 pair launcher shares: whole pairs only.
// A zero or odd width would drop a trailing element or misaddress the pair
// store, so it is refused before launch rather than computed wrong.
static inline int32_t LmLaunchAccumWidthPolicy(uint32_t width)
{
	if ( width == 0u || (width & 1u) != 0u )
