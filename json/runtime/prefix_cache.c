 ...
 * block ordinals a driver's table maps positions through. The O(1)
 * tail fast path below preserves that contract; see its comment for the
 * invariant argument and measured effect (profile proposal M1). */
    ...
    const SparkPrefixCacheCoreSequence *sequence)
{
	...
	uint32_t index;
	/* O(1) fast path (profile proposal M1,
	 * paged_kv_lookup_profile_report.md): the append discipline admits at
	 * most ONE private block per sequence and appends it at the tail - a
	 * block is allocated only after the previous one published, and a
	 * filled private block publishes in the same loop iteration that
	 * filled it (SparkPrefixCacheCoreAppendTokens). When the LAST block
	 * is private with room it therefore IS the leftmost such block, and
	 * returning it equals the front-scan result; otherwise the scan
	 * below runs verbatim and stays the source of truth in every state,
	 * including the historical multi-open defensive case noted on
	 * CoreCommittedTokens. Removes the O(depth) rescan that made
	 * single-token appends grow 121->203 ns/token over depths 256->508. */
	if (sequence->block_count != 0u)
	{
