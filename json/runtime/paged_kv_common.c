...
/* Core sequence ids come from the bound geometry callbacks' private
 ...
 * Seam decision (audit DRY note): this is DELIBERATELY engine-local -
 ...
 * scratch), so this is the ONE borrow path every port inherits. */
	...
	uint32_t *block_out)
{
	 ...
	 * in-use scratch as free, so SparkPagedKvFreeBlocks() and the
	 * spec-coverage feasibility check over-reported pool room and
	...
	SparkPagedKvMatch *match_out)
{
	...
	if ( lcp_tokens != 0u )
	{
		...
		{
			/* The donor clamp bites: reuse is only sound at boundaries
	...
	uint32_t lane, uint32_t slot, uint64_t end_position)
{
	 ...
	 * into that open block and binds the donor witness to a mutable
	 * block (audit F4): the witness must sit inside the committed
	 * tokens. */
