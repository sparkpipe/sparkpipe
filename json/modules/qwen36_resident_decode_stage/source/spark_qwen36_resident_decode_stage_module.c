...
#include "spark_qwen36_resident_decode_stage_internal.h" /* former spark_qwen36_stagepack_format.h; mechanics via runtime/spark_stagepack_reader.h */
#include "spark_qwen36_dspark_internal.h" /* former spark_qwen36_dspark_format.h */
...
typedef struct SparkQwen36DsparkWeights
{
	...
	SparkQwen36LinearView selector_pred;
	SparkQwen36LinearView selector_succ;
	SparkQwen36LinearView selector_hidden_proj;
...
/* rANS wire constants mirrored from the .cu decoder - these are exactly the
 * fields the device trusts blindly (shared-table build, id_table index, chunk
 * staging), so the host pins them before any bind. */
#define SPARK_QWEN36_RANS_TABLE_SLOTS 4096u
#define SPARK_QWEN36_RANS_MAX_DIRECT_ENTRIES 65536u
#define SPARK_QWEN36_RANS_CHUNK_HEADER_BYTES 768u /* states[128] + lens[128] */

/* Host-side rANS header validator for BF16_RANS entries. The generic byte
 * accounting below skips this class because payload_bytes IS the stream
 * length; that left every header field unvalidated, and the device decoder
 * derives shared-memory extents from them directly. This reads the stream
 * header from the pack and refuses anything the decoder would trust past its
 * tables: ndirect inside the symbol-space cap with the entry table inside
 * the payload, sum(fs) == 4096 with every C equal to the running prefix (so
 * the device table build provably stays inside its 4096 slots), id_bits in
 * [1,16] with the id table inside the payload, chunk_count equal to the
 * tiled geometry already enforced above, and offsets strictly monotonic
 * from the end of the offsets array with room for each 768-byte chunk
 * header inside payload_bytes. Minted streams satisfy every clause by
 * construction (tools/qwen36_stagepack_rans.c emits exactly this layout);
 * only hostile headers refuse. */
static SparkStatus SparkQwen36ModuleValidateRansHeader(FILE *file,const SparkQwen36StagePackEntry *entry,uint32_t rows,uint32_t columns)
{
	uint64_t cursor,payload_end,prior;
	uint32_t ndirect,id_bits,chunk_count,i,run;
	uint32_t *entries,*offsets;
	SparkStatus status;
	payload_end = entry->payload_bytes;
	if ( payload_end < 40u )
	{
		fprintf(stderr,"%s rans_header_truncated bytes=%llu\n",SPARK_QWEN36_MODULE_TAG,(unsigned long long)payload_end);
		return(SPARK_STATUS_VALIDATION_FAILED);
	}
	if ( SparkStageModulePackRead(SPARK_QWEN36_MODULE_TAG,file,entry->payload_offset,&ndirect,sizeof(ndirect)) != SPARK_STATUS_OK )
		return(SPARK_STATUS_IO_ERROR);
	if ( ndirect >= SPARK_QWEN36_RANS_MAX_DIRECT_ENTRIES || 4u + 12u * ((uint64_t)ndirect + 1u) + 8u > payload_end )
	{
		fprintf(stderr,"%s rans_header_table_invalid ndirect=%u bytes=%llu\n",SPARK_QWEN36_MODULE_TAG,ndirect,(unsigned long long)payload_end);
		return(SPARK_STATUS_VALIDATION_FAILED);
	}
	cursor = 4u + 12u * ((uint64_t)ndirect + 1u);
	entries = (uint32_t *)malloc((size_t)((uint64_t)(ndirect + 1u) * 3u) * sizeof(uint32_t));
	if ( entries == 0 )
		return(SPARK_STATUS_CAPACITY_EXCEEDED);
	status = SparkStageModulePackRead(SPARK_QWEN36_MODULE_TAG,file,entry->payload_offset + 4u,entries,(uint64_t)(ndirect + 1u) * 3u * sizeof(uint32_t));
	if ( status != SPARK_STATUS_OK )
	{
		free(entries);
		return(status);
	...
	run = 0u;
	for ( i = 0u; i <= ndirect; i++ )
	{
		uint32_t fs = entries[3u * i + 1u],cs = entries[3u * i + 2u];
		/* The decoder truncates fs to 16 bits when it builds the table, so a
		 * wider frequency would silently break the 4096 law between here and
		 * the device; cs must be the running prefix or slots alias. */
		if ( cs != run || fs > 0xFFFFu || (uint64_t)run + fs > SPARK_QWEN36_RANS_TABLE_SLOTS )
		{
			fprintf(stderr,"%s rans_header_frequency_invalid entry=%u f=%u c=%u run=%u\n",SPARK_QWEN36_MODULE_TAG,i,fs,cs,run);
			free(entries);
			return(SPARK_STATUS_VALIDATION_FAILED);
		}
		run += fs;
	...
	free(entries);
	if ( run != SPARK_QWEN36_RANS_TABLE_SLOTS )
	{
		fprintf(stderr,"%s rans_header_frequency_sum run=%u\n",SPARK_QWEN36_MODULE_TAG,run);
		return(SPARK_STATUS_VALIDATION_FAILED);
	}
	if ( SparkStageModulePackRead(SPARK_QWEN36_MODULE_TAG,file,entry->payload_offset + cursor,&id_bits,sizeof(id_bits)) != SPARK_STATUS_OK )
		return(SPARK_STATUS_IO_ERROR);
	cursor += 4u;
	/* Escape ids are raw 16-bit stream bytes; id_bits beyond 16 would both
	 * shift-overflow the device's 32-bit table stride and never bound rid. */
	if ( id_bits < 1u || id_bits > 16u || cursor + (((uint64_t)1u << id_bits) * 2u) + 4u > payload_end )
	{
		fprintf(stderr,"%s rans_header_id_bits_invalid bits=%u\n",SPARK_QWEN36_MODULE_TAG,id_bits);
		return(SPARK_STATUS_VALIDATION_FAILED);
	}
	cursor += ((uint64_t)1u << id_bits) * 2u;
	if ( SparkStageModulePackRead(SPARK_QWEN36_MODULE_TAG,file,entry->payload_offset + cursor,&chunk_count,sizeof(chunk_count)) != SPARK_STATUS_OK )
		return(SPARK_STATUS_IO_ERROR);
	cursor += 4u;
	if ( chunk_count != (rows / 64u) * (columns / 128u) || cursor + (uint64_t)chunk_count * 4u > payload_end )
	{
		fprintf(stderr,"%s rans_header_chunk_count_invalid chunks=%u expected=%u bytes=%llu\n",SPARK_QWEN36_MODULE_TAG,chunk_count,(rows / 64u) * (columns / 128u),(unsigned long long)payload_end);
		return(SPARK_STATUS_VALIDATION_FAILED);
	}
	offsets = (uint32_t *)malloc((size_t)chunk_count * sizeof(uint32_t));
	if ( offsets == 0 )
		return(SPARK_STATUS_CAPACITY_EXCEEDED);
	status = SparkStageModulePackRead(SPARK_QWEN36_MODULE_TAG,file,entry->payload_offset + cursor,offsets,(uint64_t)chunk_count * sizeof(uint32_t));
	if ( status != SPARK_STATUS_OK )
	{
		free(offsets);
		return(status);
	}
	prior = cursor + (uint64_t)chunk_count * 4u;
	for ( i = 0u; i < chunk_count; i++ )
	{
		uint64_t off = offsets[i];
		if ( off < prior || off > payload_end - SPARK_QWEN36_RANS_CHUNK_HEADER_BYTES )
		{
			fprintf(stderr,"%s rans_header_offset_invalid chunk=%u offset=%llu floor=%llu bytes=%llu\n",SPARK_QWEN36_MODULE_TAG,i,(unsigned long long)off,(unsigned long long)prior,(unsigned long long)payload_end);
			free(offsets);
			return(SPARK_STATUS_VALIDATION_FAILED);
		}
		prior = off + SPARK_QWEN36_RANS_CHUNK_HEADER_BYTES;
	}
	free(offsets);
	return(SPARK_STATUS_OK);
}

static SparkStatus SparkQwen36ModuleValidateEntry(SparkQwen36ModuleState *state, const SparkQwen36StagePackEntry *entry, uint64_t file_bytes, FILE *file, uint32_t *is_global)
{
	...
	if ( entry->weight_format != SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_BF16_RANS && (entry->payload_bytes != SparkQwen36StagePackPayloadBytes(entry->weight_format,entry->rows,entry->columns) || entry->scale_bytes != SparkQwen36StagePackScaleBytes(entry->weight_format,entry->rows,entry->columns)) )
		return(SPARK_STATUS_VALIDATION_FAILED);
	/* RANS payloads carry their own stream header; validate it here. */
	if ( entry->weight_format == SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_BF16_RANS && SparkQwen36ModuleValidateRansHeader(file,entry,entry->rows,entry->columns) != SPARK_STATUS_OK )
		return(SPARK_STATUS_VALIDATION_FAILED);
...
static SparkStatus SparkQwen36ModuleLoadEntry(SparkQwen36ModuleState *state, FILE *file, const SparkQwen36StagePackEntry *entry, uint64_t file_bytes)
{
	...
	status = SparkQwen36ModuleValidateEntry(state,entry,file_bytes,file,&is_global);
	if ( status != SPARK_STATUS_OK )
	{
...
/* Directory sweep (the glm52 discipline): no two entries' payload/scale
 * spans may overlap and every payload must start past the directory.
 * Declared byte counts were already pinned per entry (RANS entries carry
 * their stream length); this adds the cross-entry aliasing view. */
static SparkStatus SparkQwen36ModuleSweepDirectory(const SparkQwen36StagePackEntry *directory,uint32_t tensor_count,uint64_t minimum_offset)
{
	uint32_t i,j;
	for ( i = 0u; i < tensor_count; i++ )
	{
		const SparkQwen36StagePackEntry *entry = &directory[i];
		if ( entry->payload_offset < minimum_offset )
			return(SPARK_STATUS_VALIDATION_FAILED);
		for ( j = i + 1u; j < tensor_count; j++ )
		{
			const SparkQwen36StagePackEntry *other = &directory[j];
			if ( SparkStagePackRangesOverlap(entry->payload_offset,entry->payload_bytes,other->payload_offset,other->payload_bytes) != 0u )
				return(SPARK_STATUS_VALIDATION_FAILED);
			if ( entry->scale_bytes != 0u && other->payload_bytes != 0u && SparkStagePackRangesOverlap(entry->scale_offset,entry->scale_bytes,other->payload_offset,other->payload_bytes) != 0u )
				return(SPARK_STATUS_VALIDATION_FAILED);
		}
	}
	return(SPARK_STATUS_OK);
	...
	void *scale)
{
	...
	if ( layer >= SPARK_QWEN36_DSPARK_LAYER_COUNT && layer != 0xFFFFFFFFu )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	...
	case SPARK_QWEN36_DSPARK_TENSOR_SELECTOR_PRED: SparkQwen36ModuleFillLinearView(&w->selector_pred,entry,payload,scale); return(SPARK_STATUS_OK);
	case SPARK_QWEN36_DSPARK_TENSOR_SELECTOR_SUCC: SparkQwen36ModuleFillLinearView(&w->selector_succ,entry,payload,scale); return(SPARK_STATUS_OK);
	case SPARK_QWEN36_DSPARK_TENSOR_SELECTOR_HIDDEN_PROJ: SparkQwen36ModuleFillLinearView(&w->selector_hidden_proj,entry,payload,scale); return(SPARK_STATUS_OK);
	...
	case SPARK_QWEN36_DSPARK_TENSOR_CONV_ATTN_BASE: lw->conv_attn_base_bf16 = payload; return(SPARK_STATUS_OK);
	...
	case SPARK_QWEN36_DSPARK_TENSOR_CONV_MLP_BASE: lw->conv_mlp_base_bf16 = payload; return(SPARK_STATUS_OK);
...
static SparkStatus SparkQwen36ModuleLoadDsparkPack(SparkQwen36ModuleState *state, const char *path)
{
	...
	if ( status == SPARK_STATUS_OK )
	{
		...
		if ( status == SPARK_STATUS_OK )
		{
			...
			status = SparkStageModuleCudaStatus(SPARK_QWEN36_MODULE_TAG,d2h,"dspark_selector_d2h");
		}
	}
	if ( status == SPARK_STATUS_OK )
	{
		...
		status = SparkStageModuleDeviceAllocate(&state->ledger,(uint64_t)8192u * 5u * SPARK_QWEN36_MODEL_HIDDEN_DIMENSION * 2u,&state->dflash_taps_history);
		if ( status == SPARK_STATUS_OK )
			status = SparkStageModuleDeviceAllocate(&state->ledger,(uint64_t)2048u * SPARK_QWEN36_MODEL_HIDDEN_DIMENSION * 2u,&state->dflash_fc_out);
		if ( status == SPARK_STATUS_OK )
...
static SparkStatus SparkQwen36ModuleLoadPack(SparkQwen36ModuleState *state, const char *path)
{
	...
	if ( status == SPARK_STATUS_OK )
		status = SparkQwen36ModuleSweepDirectory(directory,header.tensor_count,(uint64_t)sizeof(header) + (uint64_t)header.tensor_count * SPARK_QWEN36_STAGEPACK_ENTRY_BYTES);
	if ( status == SPARK_STATUS_OK )
...
static SparkStatus SparkQwen36ModuleAllocateSlotControl(SparkQwen36ModuleState *state, SparkQwen36ModuleSlot *slot)
{
	...
	if ( status == SPARK_STATUS_OK )
		status = SparkStageModuleDeviceAllocate(&state->ledger,(uint64_t)(1u*SPARK_QWEN36_MODEL_HIDDEN_DIMENSION + 3u*SPARK_QWEN36_DSPARK_BLOCK_SIZE*SPARK_QWEN36_MODEL_HIDDEN_DIMENSION + 2u*SPARK_QWEN36_DSPARK_CONV_PROJ_ROWS*SPARK_QWEN36_DSPARK_BLOCK_SIZE + 2u*(SPARK_QWEN36_DSPARK_BLOCK_SIZE+1u)*SPARK_QWEN36_DSPARK_ATTN_KV_HEADS*SPARK_QWEN36_DSPARK_ATTN_HEAD_DIMENSION + 2u*SPARK_QWEN36_DSPARK_BLOCK_SIZE*SPARK_QWEN36_MODEL_HIDDEN_DIMENSION + 2u*SPARK_QWEN36_DSPARK_BLOCK_SIZE*SPARK_QWEN36_DSPARK_FFN_INTERMEDIATE + SPARK_QWEN36_DSPARK_BLOCK_SIZE*SPARK_QWEN36_MODEL_OUTPUT_VOCAB_COUNT) * SPARK_QWEN36_MODEL_BF16_ELEMENT_BYTES,&slot->dspark_scratch);
	if ( status == SPARK_STATUS_OK )
	...
	if ( status == SPARK_STATUS_OK )
		slot->dspark_hidden_host = (uint16_t *)malloc((size_t)SPARK_QWEN36_DSPARK_BLOCK_SIZE * SPARK_QWEN36_MODEL_HIDDEN_DIMENSION * sizeof(uint16_t));
	...
	uint32_t rows)
{
	...
	const uint32_t K = SPARK_QWEN36_DSPARK_SELECTOR_TOP_K;
	const uint32_t conv_groups = H / SPARK_QWEN36_DSPARK_CONV_GROUP_SIZE;
	const uint32_t conv_side_base = SPARK_QWEN36_DSPARK_CONV_KERNEL_SIZE * H;
	...
	uint16_t *conv_proj_m = conv_proj_a + (uint64_t)B * SPARK_QWEN36_DSPARK_CONV_PROJ_ROWS;
	uint16_t *q = conv_proj_m + (uint64_t)B * SPARK_QWEN36_DSPARK_CONV_PROJ_ROWS;
	 ...
	 * anchor-row-0 single block. */
	{
		...
		if ( multi < 1u )
			multi = 1u;
		if ( multi > SPARK_QWEN36_RESIDENT_DECODE_STAGE_DSPARK_MAX_MULTI_BLOCKS )
			multi = SPARK_QWEN36_RESIDENT_DECODE_STAGE_DSPARK_MAX_MULTI_BLOCKS;
		if ( multi > 1u && row_tokens_host != 0 )
		{
			...
			uint32_t emissions[SPARK_QWEN36_RESIDENT_DECODE_STAGE_MAX_MTP_DRAFT_TOKENS];
			if ( rows > SPARK_QWEN36_RESIDENT_DECODE_STAGE_MAX_MTP_DRAFT_TOKENS )
				rows = SPARK_QWEN36_RESIDENT_DECODE_STAGE_MAX_MTP_DRAFT_TOKENS;
	...
	if ( error == cudaSuccess )
		error = SparkQwen36LaunchEmbeddingGather(stream,slot->output_token_ids + blk,state->token_embedding_bf16,block_hidden,1u);
	if ( error == cudaSuccess )
	...
	if ( status == SPARK_STATUS_OK && blk == 0u && getenv("SPARK_QWEN36_DFLASH2_CTX_DUMP") != 0 )
	{
	...
	for (layer = 0u; status == SPARK_STATUS_OK && layer < SPARK_QWEN36_DSPARK_LAYER_COUNT; layer++)
	{
		...
		if ( error == cudaSuccess )
			error = SparkQwen36LaunchDsparkConv(stream,q,conv_proj_a,(const char *)lw->conv_attn_base_bf16 + (uint64_t)conv_side_base * 2u,conv_h,B,conv_groups,SPARK_QWEN36_DSPARK_CONV_GROUP_SIZE,1u);
		if ( error == cudaSuccess )
		...
		if ( error == cudaSuccess )
			error = SparkQwen36LaunchDsparkConv(stream,norm,conv_proj_m,lw->conv_mlp_base_bf16,conv_h,B,conv_groups,SPARK_QWEN36_DSPARK_CONV_GROUP_SIZE,0u);
		if ( error == cudaSuccess )
			error = SparkQwen36LaunchLinear(stream,&lw->gate,conv_h,ffn,B);
		if ( error == cudaSuccess )
			error = SparkQwen36LaunchLinear(stream,&lw->up,conv_h,up,B);
		if ( error == cudaSuccess )
		...
		if ( error == cudaSuccess )
			error = SparkQwen36LaunchDsparkConv(stream,q,conv_proj_m,(const char *)lw->conv_mlp_base_bf16 + (uint64_t)conv_side_base * 2u,conv_h,B,conv_groups,SPARK_QWEN36_DSPARK_CONV_GROUP_SIZE,1u);
		if ( error == cudaSuccess )
			error = SparkQwen36LaunchResidualAdd(stream,block_hidden,conv_h,B,H);
	...
	if ( status == SPARK_STATUS_OK )
	{
		...
		status = SparkStageModuleCudaStatus(SPARK_QWEN36_MODULE_TAG,error,"dflash2_head");
	}
	}
	}
	return(status);
	...
	uint32_t rows)
{
	...
	return(SparkStageModuleCudaStatus(SPARK_QWEN36_MODULE_TAG,error,"dflash_tap_store"));
}
    ...
    SparkModelDriverFrame *frame)
{
    ...
    if (status == SPARK_STATUS_OK && (context->flags & SPARK_QWEN36_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_DSPARK_DRAFT_AFTER) != 0u)
        status = SparkQwen36ModuleRunDsparkBlockForward(state,slot,context->dspark_draft,
