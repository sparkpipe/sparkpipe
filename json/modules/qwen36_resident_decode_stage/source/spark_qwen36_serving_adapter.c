...
{
	return(SparkAdapterEnvFlagDefaultOn(SPARK_QWEN36_SERVING_PREFIX_CACHE_ENV));
}
...
typedef struct SparkQwen36ServingPending
{
	...
	/* The submission-anchor school: the completion echoes THIS residency,
	 * never a driver frame's. */
 ...
 * (runtime/json.c); member lookups call SparkJsonFindObjectMember directly -
 * this adapter's private byte-equal wrappers are retired, same as glm52's. */
...
#endif
	SPARK_QWEN36_SERVING_SET_UNSIGNED("SPARK_QWEN36_STAGE_MAX_ACTIVE_SEQUENCES",state->common.max_active_sequence_count);
	SPARK_QWEN36_SERVING_SET_UNSIGNED("SPARK_QWEN36_STAGE_PIPELINE_SLOTS",state->common.pipeline_slot_count);
	...
	const SparkModelServingSubmission *submission)
{
	...
	status = SparkAdapterValidateSubmissionOpen(&state->common,
		&SparkQwen36ServingDescriptor,submission);
	if ( status != SPARK_STATUS_OK )
	{
	...
	const SparkModelServingSubmission *submission)
{
	...
	index = SparkAdapterPendingClaim(state->common.pending,
		sizeof(SparkQwen36ServingPending),state->common.pipeline_slot_count);
	if ( index < 0 )
		return(0);
	pending = &state->pending[index];
	memset(pending,0,sizeof(*pending));
	pending->owner = state;
	/* Early active-stamp preserved: qwen36 stamps active first. */
	...
	const SparkModelDriverCompletion *driver_completion)
{
	...
	if ( matches == 0u )
	{
		 ...
		 * synchronous status read fails, and count the orphan in the shared
		 * counter. */
...
/* First-byte-truthy semantics live in SparkAdapterEnvFlagDefaultOffTruthy.
 * The strict nonempty-not-'0' SPECULATE / SPEC_AUDIT readers stay here: they
 * are a fifth semantics ('01' ON, empty OFF) with only two users in-tree.
 */
static uint32_t SparkQwen36ServingKvFlatMode(void)
{
	return(SparkAdapterEnvFlagDefaultOffTruthy("SPARK_QWEN36_SERVING_KV_FLAT"));
}
	...
	uint64_t end_position)
{
	...
	token_count = SparkQwen36ServingGatherLaneTokens(submission,lane,
		committed,state->match_tokens,state->common.max_input_row_count);
	...
	uint32_t *module_slot_out)
{
	...
	count = SparkQwen36ServingGatherLaneTokens(submission,lane,0,tokens,
		state->common.max_input_row_count);
	slot = submission->lanes[lane].resident_sequence_slot;
	...
	const SparkQwen36ServingState *state)
{
	...
	if ( state->stage_attn_layer_count == 0u )
		return(SPARK_STATUS_OK);
	indices_bytes = (uint64_t)state->common.max_active_sequence_count * state->blocks_per_lane * sizeof(uint32_t);
	...
	SparkStatus status)
{
	...
	uint32_t index;
	/* ABI stamp + identity echo from the shared builder; residency comes
	 * from the family's submission anchor, not any driver frame. */
	...
	if ( SparkQwen36ServingOwnsFinalHead(state) != 0u && status == SPARK_STATUS_OK && pending->core.active_sequence_count != 0u )
	{
		...
		if ( pending->spec_active != 0u )
		{
			...
			completion.token_count = pending->core.active_sequence_count * pending->spec_tokens_per_sequence;
	...
	const SparkModelServingSubmission *submission)
{
	...
	if ( submission->work_kind == SPARK_MODEL_SERVING_WORK_KIND_RELEASE )
	{
		...
		for (lane=0u; lane<submission->active_sequence_count; lane++)
			SparkQwen36ServingReleaseLane(state,submission->lanes[lane].resident_sequence_slot);
		pending->core.active_sequence_count = 0u;  /* RELEASE completes with no tokens */
	...
	uint32_t prepare)
{
	...
	if ( adapter_state == 0 || submission == 0 )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	return(((SparkQwen36ServingState *)adapter_state)->common.quiescing != 0u ?
...
/* Progress is the shared stream-ordered stub; quiesce and snapshot are the
 * shared lifecycle bodies over the common state. */
	...
	const SparkModelServingAdapterConfiguration *configuration)
{
	...
	contract.model_id = SPARK_QWEN36_SERVING_DRIVER_MODEL_ID;
	contract.model_revision = QWEN36_MODEL_REVISION;
	contract.stage_name = SPARK_QWEN36_SERVING_STAGE_NAME;
	contract.target = SPARK_QWEN36_SERVING_TARGET;
	contract.description_sha256 = QWEN36_CONTRACT_SHA256;
	contract.required_program_flags = SPARK_QWEN36_SERVING_REQUIRED_PROGRAM_FLAGS;
	...
	SparkQwen36ServingState *state)
{
	...
	if ( state->host_block_indices == 0 || state->free_blocks == 0 || state->match_tokens == 0 )
		return(SPARK_STATUS_CAPACITY_EXCEEDED);
	if ( cudaMalloc(&state->gather_scratch,(size_t)((uint64_t)state->common.max_active_sequence_count * SPARK_QWEN36_MODEL_HIDDEN_BF16_BYTES)) != cudaSuccess )
		return(SPARK_STATUS_CAPACITY_EXCEEDED);
	...
	paged_configuration.lane_count = state->common.max_active_sequence_count;
	...
	void **adapter_state)
{
	...
	*adapter_state = 0;
	...
	if ( status == SPARK_STATUS_OK )
	{
		...
		uint32_t derived_pool = state->common.resident_sequence_capacity * ((max_sequence_positions + SPARK_QWEN36_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS - 1u) / SPARK_QWEN36_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS);
