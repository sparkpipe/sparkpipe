...
/* Slot-table depth matches the descriptor face (16 inflight / 16 rows /
 * 16 resident). The runner completes synchronously, so at most one core
 * is ever active BETWEEN submits - the table exists so the shared slot
 * discipline (claim/capture/available-count) and the shared completion
 * header builder have one honest home, exactly as in the four
 * driver-loading adapters. */
#define SPARK_K3_SERVING_SLOT_COUNT 16u

typedef struct SparkK3ServingState
{
	/* Counters/routes/limits prologue + the quiescing latch: the shared
	 * SparkAdapterCommonState (adapter_common.h). The orphan counter and
	 * driver handle stay zeroed forever - this family has no external
	 * driver .so and publishes completions inline. */
	SparkAdapterCommonState common;
	SparkAdapterPendingCore pending[SPARK_K3_SERVING_SLOT_COUNT];
	...
	const SparkModelServingAdapterConfiguration *configuration)
{
	...
	/* THE CONTEXT AXIS. The MLA cache is paged in K3_KV_PAGE_SLOTS-token
	 * pages, and a page count pinned without regard to context made every
	 * context past 2*64 = 128 tokens unreachable ("kv_pages": 2 was one
	 * 128-token cache - the whole ctx512/ctx2048 bench axis died on it).
	 * Deployments now speak MODEL units: "context_tokens" derives the page
	 * count by ceil over the page size, an explicit "kv_pages" still
	 * overrides for exotic shapes, and with neither key the default covers
	 * a 2048-token context - never again a fixed two pages. */
	{
		uint32_t context_tokens = K3ServingJsonU32(&doc, root,
			"context_tokens", 2048u);
		uint32_t pages = K3ServingJsonU32(&doc, root, "kv_pages", 0u);
		if ( pages == 0u )
			pages = (context_tokens + K3_KV_PAGE_SLOTS - 1u) /
				K3_KV_PAGE_SLOTS;
		if ( pages == 0u )
			pages = 1u;
		state->runner_config.kv_pages_per_sequence = pages;
	}
...
/* Defined beside the interface tables at the foot of the file; the shared
 * open gate in validate/submit validates submissions against it. */
static const SparkModelServingAdapterDescriptor K3ServingDescriptor;

/* F4: one scratch release shared by every failure path after the buffers
 * exist. The state arrives calloc'd, so untouched device pointers are NULL
 * and cudaFree(NULL) is a no-op; pack_path may or may not be set depending
 * on how far configuration loading got. The old per-site frees leaked the
 * five device buffers (and pack_path) whenever the runner said no. */
static void K3ServingFreeScratch(SparkK3ServingState *state)
{
	...
	void **adapter_state)
{
	...
	/* Shared prologue: serving-completion route, wake route, runtime-limits
	 * mirror, slot count, quiescing latch zeroed. The family pending table
	 * is ours; the shared helpers walk it through the core stride. */
	status = SparkAdapterInitializePrologue(&state->common, configuration);
	if ( status != SPARK_STATUS_OK )
		{ free(state); return status; }
	state->common.pending = state->pending;
	state->common.pending_element_bytes = sizeof(state->pending[0]);
	status = K3ServingLoadConfiguration(state, configuration);
	if ( status != SPARK_STATUS_OK )
		{ free(state->pack_path); free(state); return status; }
	...
	if ( state->positions_host == 0 || state->context_host == 0 ||
		state->state_host == 0 )
		{ K3ServingFreeScratch(state); free(state); return SPARK_STATUS_CAPACITY_EXCEEDED; }
	if ( cudaMalloc((void **)&state->positions_device, (uint64_t)state->max_rows * 4u) != cudaSuccess ||
		cudaMalloc((void **)&state->context_device, (uint64_t)state->max_rows * 4u) != cudaSuccess ||
		cudaMalloc((void **)&state->state_device, (uint64_t)state->max_rows * 4u) != cudaSuccess ||
		cudaMalloc((void **)&state->output_tokens, (uint64_t)state->max_rows * 4u) != cudaSuccess ||
		cudaMalloc((void **)&state->output_scores, (uint64_t)state->max_rows * 4u) != cudaSuccess )
		{ K3ServingFreeScratch(state); free(state); return SPARK_STATUS_CAPACITY_EXCEEDED; }
	status = SparkK3StageRunnerInitialize(&state->runner, &state->runner_config);
	if ( status != SPARK_STATUS_OK )
		{ K3ServingFreeScratch(state); free(state); return status; }
	*adapter_state = state;
	return SPARK_STATUS_OK;
}
...
static void K3ServingDestroy(void *adapter_state)
{
	...
	if ( state == 0 )
		return;
	/* Shared destroy guard: defer teardown while any slot is active. With
	 * the synchronous runner this only bites when destroy races a submit
	 * from another thread - free-under-runner becomes a one-shot deferral
	 * instead of a use-after-free. The driver poll half of the guard is a
	 * no-op here: common.driver is zeroed (no external driver). */
	...
	const SparkModelServingSubmission *submission)
{
	...
	if ( state == 0 || submission == 0 )
		return SPARK_STATUS_INVALID_ARGUMENT;
	/* Shared open gate: presence, the quiescing latch (BUSY after quiesce -
	 * audit D8), then ABI/runtime-limits validation of the submission
	 * against the descriptor. Family boundary law follows. */
	status = SparkAdapterValidateSubmissionOpen(&state->common,
		&K3ServingDescriptor, submission);
	if ( status != SPARK_STATUS_OK )
		return status;
	...
	const SparkModelServingSubmission *submission)
{
	...
	if ( state == 0 || submission == 0 )
		return SPARK_STATUS_INVALID_ARGUMENT;
	/* Submit re-runs the shared open gate (quiesce refusal included), the
	 * same discipline as the four driver-loading adapters. */
	status = SparkAdapterValidateSubmissionOpen(&state->common,
		&K3ServingDescriptor, submission);
	if ( status != SPARK_STATUS_OK )
		return status;
	slot = SparkAdapterPendingClaim(state->common.pending,
		state->common.pending_element_bytes,
		state->common.pipeline_slot_count);
	if ( slot < 0 )
		return SPARK_STATUS_BUSY;
	core = &state->pending[slot];
	memset(core, 0, sizeof(*core));
	SparkAdapterPendingCapture(core, submission);
	core->active = 1u;
	...
	/* THE COMPLETION PUBLISHES STREAM-ORDERED TRUTH. Submit ENQUEUES the
	 * step on the execution stream and returns - it does not wait for the
	 * kernels. Reading output_tokens, or telling the orchestrator OK,
	 * before that stream drains publishes tokens nobody has computed yet
	 * (the HIGH thread-safety defect: every rank whose head stage does
	 * not already sync internally raced its own completion). One explicit
	 * synchronization here orders the publication behind every kernel of
	 * the step, on all ranks; a failed sync is a failed step and must not
	 * publish OK either. */
	{
		cudaError_t error = cudaStreamSynchronize(
			(cudaStream_t)state->runner_config.execution_stream);
	...
	/* The stream is drained: build and publish the completion through the
	 * serving ABI from values that exist. */
	if ( state->common.sink.function != 0 )
	{
		SparkModelServingCompletion completion;
		uint32_t *tokens_host = (uint32_t *)malloc((uint64_t)rows * 4u);
		uint32_t copied = 0u;
		/* Shared header builder: zeroed completion, ABI version STAMPED
		 * (ABI LAW - the old echo of submission->abi_version validated only
		 * while submissions carried the same constant), descriptor bytes,
		 * and the FULL nine-field identity echo captured at claim - audit
		 * D7's missing five generation fields ride along now. Residency
		 * keeps this family's submission-anchor school. */
		...
		/* F7: the counts report what the fixed token_ids array CARRIES -
		 * rows can exceed the completion capacity, and an accepted count
		 * above the carried count advertises tokens the caller can never
		 * read back. */
...
{
	SparkK3ServingState *state = (SparkK3ServingState *)adapter_state;
	/* Shared gate + latch semantics: presence/deadline argument check, then
	 * the quiescing latch that SparkAdapterValidateSubmissionOpen enforces
	 * on every later validate/submit (audit D8 - work after quiesce was
	 * accepted before). The driver-poll half of the shared SparkAdapterQuiesce
	 * body does NOT apply: common.driver is zeroed in this runner-based
	 * family, and the synchronous runner cannot hold submissions past its
	 * return - the idle-slot table walk below stands in for the poll. */
	if ( state == 0 || deadline_time_ns == 0u )
		return SPARK_STATUS_INVALID_ARGUMENT;
	state->common.quiescing = 1u;
	if ( SparkAdapterAvailableSubmissionCount(state->common.pending,
			state->common.pending_element_bytes,
			state->common.pipeline_slot_count) !=
			state->common.pipeline_slot_count )
		return SPARK_STATUS_BUSY;
	...
	SparkModelServingAdapterSnapshot *snapshot)
{
	...
	/* Slot-honest availability from the shared counter (audit D9): free
	 * slots among the configured pipeline_slot_count, not a row-count lie.
	 * Between submits the synchronous runner holds nothing, so this reads
	 * full; during one it reads the truth. */
	available = SparkAdapterAvailableSubmissionCount(state->common.pending,
		state->common.pending_element_bytes,
		state->common.pipeline_slot_count);
	snapshot->available_submission_count =
		state->common.quiescing == 0u ? available : 0u;
	snapshot->active_submission_count =
		state->common.pipeline_slot_count - available;
...
static const SparkModelServingAdapterDescriptor K3ServingDescriptor =
{
	...
	/* PREFILL|DECODE are the loader's required capabilities; without them
	 * SparkModelServingAdapterValidateInterface refuses the interface and
	 * every residentd init dies INVALID_ARGUMENT at adapter_load. */
	...
	/* The descriptor CAPS the deployment's runtime limits (ValidateRuntime-
	 * Limits rejects limits.max_input_rows above it): it must cover the
	 * row-width ladder through B1024, not pin the old b16 ceiling. */
	.max_input_row_count = 1024u,
	...
	/* sha256 of the bucketed module identity string (the artifact contract
	 * this descriptor publishes); the validator requires 64 lowercase hex. */
	.artifact_sha256 = "fc126191a2b69ebc51ab64aeed661754c4bd19968a182d1652b3e451cb1ed3db",
...
/* THE RESIDENTD ENTRY POINT: the generic loader dlsym's the family-neutral
 * name (runtime/model_serving_adapter.c); every sibling adapter exports it,
 * and without it the shared object loads but the interface is NOT_FOUND.
 * The K3-named symbol above stays for direct-link tests. */
