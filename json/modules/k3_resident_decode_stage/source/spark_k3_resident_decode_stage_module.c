...
void SparkK3ModuleDestroy(SparkK3ModuleState *state)
{
	...
	memset(state, 0, sizeof(*state));
	/* The memset leaves pack.fd == 0, so a SECOND destroy on the same
	 * state (the runner's unified init-failure teardown after a module
	 * self-clean) would close(0) - stdin. Keep the closed-pack sentinel. */
	state->pack.fd = -1;
}
