	...
	SparkK3BoundLayer *bound)
{
	...
	/* THE canonical kind rule (spark_k3_pool_sizing.h): the 3:1 period AND
	 * the backbone's trailing layer 92 is MLA (the checkpoint's
	 * full_attn_layers includes 93 one-indexed). A private period-only form
	 * here bound layer 92 as GDN and made the module resolve KDA names the
	 * pack does not carry for it - one kind truth, shared, not re-derived. */
	bound->layer_is_gdn = (SparkK3LayerIsMla(layer_index) == 0u);
