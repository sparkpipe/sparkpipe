...
/* Bound-span registry: every resolved tensor's payload span is checked
 * against every previously resolved span (the glm52 sweep discipline,
 * applied lazily because this resolver binds by name). Two tensors sharing
 * bytes would let one bind silently alias another. */
#define SPARK_K3_PACK_SPAN_CAPACITY 8192u

typedef struct SparkK3PackSpan
{
	uint64_t offset;
	uint64_t bytes;
} SparkK3PackSpan;
...
SparkStatus SparkK3PackOpen(const char *path, SparkK3Pack *pack)
{
	...
	pack->private_state->spans = (SparkK3PackSpan *)calloc(
		SPARK_K3_PACK_SPAN_CAPACITY, sizeof(SparkK3PackSpan));
	if ( pack->private_state->spans == 0 )
	{
	...
	SparkK3PackEntry *entry)
{
	...
	if ( pack->payload_base > pack->file_bytes ||
		entry->payload_offset > pack->file_bytes - pack->payload_base ||
		entry->bytes > pack->file_bytes - pack->payload_base - entry->payload_offset )
		return(SPARK_STATUS_VALIDATION_FAILED);
	/* Cross-tensor aliasing refusal (glm52 sweep discipline, lazy form):
	 * this resolver binds by name on demand, so spans register as they are
	 * resolved; any span overlapping an earlier-resolved tensor refuses.
	 * The registry is bounded - a manifest needing more distinct tensors
	 * than SPARK_K3_PACK_SPAN_CAPACITY cannot be swept and is refused. */
	if ( pack->private_state->span_count >= SPARK_K3_PACK_SPAN_CAPACITY )
		return(SPARK_STATUS_VALIDATION_FAILED);
	for ( uint32_t s = 0u; s < pack->private_state->span_count; s++ )
	{
		const SparkK3PackSpan *prior = &pack->private_state->spans[s];
		/* An exact re-resolution of the same tensor is idempotent and
		 * allowed; any other overlap aliases two distinct tensors. */
		if ( prior->offset == entry->payload_offset && prior->bytes == entry->bytes )
			continue;
		if ( SparkStagePackRangesOverlap(entry->payload_offset,entry->bytes,prior->offset,prior->bytes) != 0u )
			return(SPARK_STATUS_VALIDATION_FAILED);
	}
	pack->private_state->spans[pack->private_state->span_count].offset = entry->payload_offset;
	pack->private_state->spans[pack->private_state->span_count].bytes = entry->bytes;
	pack->private_state->span_count++;
	return(SPARK_STATUS_OK);
}
