	...
	SparkK3BoundLayer *bounds, uint32_t layer_count)
{
	...
	for ( uint32_t off = 0u; off < layer_count && status == SPARK_K3_DISPATCH_OK; ++off )
	{
		...
		/* F5: the interleave flag comes from what the pack ACTUALLY
		 * carries for this layer's expert stream, not from the layer kind.
		 * The old layer-kind rule armed interleave on every routed layer,
		 * which is right only while every production pack ships the V2
		 * interleaved MXFP4 cells - a pack whose experts are BF16/F32 would
		 * launch interleaved and read scale bytes as payload. Dense layers
		 * carry no expert tensor at all, so they resolve to plain. */
		w->expert_interleave = 0u;
		w->expert_tile_k = 128u;
		{
			char w1_name[128];
			uint32_t tile_k = 0u;
			SparkK3PackEntry w1_entry;
			snprintf(w1_name, sizeof(w1_name), "model.layers.%u.expert_w1_weight",
				d->first_layer + off);
			if ( SparkK3PackLoadEntry(pack, w1_name, &w1_entry) == SPARK_STATUS_OK )
			{
				w->expert_interleave =
					w1_entry.kind == SPARK_K3_PACK_KIND_MXFP4_WS_INTERLEAVED_V1 ?
					1u : 0u;
				if ( w->expert_interleave != 0u &&
					SparkK3PackLoadInterleaveTileK(pack, w1_name, &tile_k) ==
						SPARK_STATUS_OK &&
