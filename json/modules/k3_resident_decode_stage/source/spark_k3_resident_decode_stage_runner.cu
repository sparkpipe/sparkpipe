	...
	const SparkK3StageRunnerConfiguration *configuration)
{
	...
	K3_CUDA_ALLOC_OR_FAIL(state->route_expert,(uint64_t)routes * 4u);
	K3_CUDA_ALLOC_OR_FAIL(state->route_packed_row,(uint64_t)routes * 4u);
	K3_CUDA_ALLOC_OR_FAIL(state->route_source_token,(uint64_t)routes * 4u);
	K3_CUDA_ALLOC_OR_FAIL(state->route_weight,(uint64_t)routes * 4u);
	K3_CUDA_ALLOC_OR_FAIL(state->group_row_offset,(uint64_t)(K3_EXPERTS + 1u) * 4u);
	K3_CUDA_ALLOC_OR_FAIL(state->group_tile_prefix_w1,(uint64_t)(K3_EXPERTS + 1u) * 4u);
	K3_CUDA_ALLOC_OR_FAIL(state->group_tile_prefix_w2,(uint64_t)(K3_EXPERTS + 1u) * 4u);
