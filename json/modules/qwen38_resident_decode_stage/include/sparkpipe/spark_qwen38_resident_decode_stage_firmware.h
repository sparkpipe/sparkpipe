...
#define SPARK_QWEN38_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_ABI_VERSION 3u
#define SPARK_QWEN38_RESIDENT_DECODE_STAGE_PREFILL_FRAME_VIEW_ABI_VERSION 1u
#define SPARK_QWEN38_RESIDENT_DECODE_STAGE_MTP_DRAFT_VIEW_ABI_VERSION 1u
#define SPARK_QWEN38_RESIDENT_DECODE_STAGE_GDN_SNAPSHOT_VIEW_ABI_VERSION 1u
#define SPARK_QWEN38_RESIDENT_DECODE_STAGE_MAX_MTP_DRAFT_TOKENS 8u
#define SPARK_QWEN38_RESIDENT_DECODE_STAGE_MAX_GDN_SNAPSHOT_SLOTS 8u
#define SPARK_QWEN38_RESIDENT_DECODE_STAGE_GDN_STATE_POOL_ABI_VERSION 1u
...
#define SPARK_QWEN38_RESIDENT_DECODE_STAGE_MAX_STAGE_COUNT 32u
...
#define SPARK_QWEN38_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT 512u
...
#define SPARK_QWEN38_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_FP8_E4M3_F32B128 4u
 ...
 */
typedef struct SparkQwen38AttnLayerWeights
{
...
typedef struct SparkQwen38MtpWeights
{
	...
	SparkQwen38AttnLayerWeights attention;
 ...
 */
typedef struct SparkQwen38GdnStatePool
{
	uint32_t abi_version;
...
} SparkQwen38GdnStatePool;

/*
 * Paged KV cache table for the stage's own full-attention layers. One token
 * costs SPARK_QWEN38_MODEL_ATTN_CACHE_TOKEN_ELEMENTS bf16 elements per
 ...
 */
typedef struct SparkQwen38KvBlockTableView
{
	uint32_t abi_version;
 ...
 */
typedef struct SparkQwen38PipelineSlot
{
	void *cuda_stream;
 ...
 */
typedef struct SparkQwen38ResidentDecodeStageNodeContext
{
	uint32_t abi_version;
	...
	const void *attention_norm_weights_by_layer_bf16[SPARK_QWEN38_RESIDENT_DECODE_STAGE_LAYER_COUNT];
	const void *mlp_norm_weights_by_layer_bf16[SPARK_QWEN38_RESIDENT_DECODE_STAGE_LAYER_COUNT];
	const SparkQwen38GdnLayerWeights *gdn_weights_by_layer;
	const SparkQwen38AttnLayerWeights *attn_weights_by_layer;
	const SparkQwen38MoeWeights *moe_weights_by_layer;
	SparkQwen38GdnStatePool gdn_state_pool;
	void *kv_cache_bf16;
	const SparkQwen38PipelineSlot *pipeline_slots;
	uint64_t estimated_service_time_ns;
} SparkQwen38ResidentDecodeStageNodeContext;
 ...
 */
typedef struct SparkQwen38DecodeBatchView
{
	uint32_t abi_version;
	uint32_t descriptor_bytes;
	uint32_t row_count;
 ...
 */
typedef struct SparkQwen38PrefillFrameView
{
	uint32_t abi_version;
	uint32_t descriptor_bytes;
	uint32_t lane_index;
 ...
 *
 * MTP_DRAFT_AFTER (head stage only, SPARK_QWEN38_STAGE_MTP=1): after the
 ...
 */
typedef struct SparkQwen38MtpDraftView
{
	uint32_t abi_version;
	uint32_t descriptor_bytes;
	uint32_t lane_index;
...
} SparkQwen38MtpDraftView;

typedef struct SparkQwen38GdnSnapshotView
{
	uint32_t abi_version;
	uint32_t descriptor_bytes;
	uint32_t snapshot_index;
	uint32_t reserved0;
} SparkQwen38GdnSnapshotView;

#define SPARK_QWEN38_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_KV_BLOCK_TABLE 0x00000001u
#define SPARK_QWEN38_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_DECODE_BATCH_VIEW 0x00000002u
#define SPARK_QWEN38_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_HIDDEN_INPUT_TRANSPORT 0x00000004u
#define SPARK_QWEN38_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_HIDDEN_OUTPUT_TRANSPORT 0x00000008u
#define SPARK_QWEN38_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_PREFILL_FRAME_VIEW 0x00000010u
#define SPARK_QWEN38_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_MTP_DRAFT_AFTER 0x00000020u
#define SPARK_QWEN38_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_SPECULATIVE_VERIFY 0x00000040u
#define SPARK_QWEN38_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_GDN_RESTORE_FIRST 0x00000080u

typedef SparkStatus (*SparkQwen38HiddenTransportPostReceiveFunction)(SparkHiddenTransportSession *transport_session, SparkHiddenTransportPacket *packet);
typedef SparkStatus (*SparkQwen38HiddenTransportSendFunction)(SparkHiddenTransportSession *transport_session, const SparkHiddenTransportPacket *packet);
 ...
 */
typedef struct SparkQwen38ResidentDecodeStageFrameContext
{
	uint32_t abi_version;
	uint32_t descriptor_bytes;
	uint32_t flags;
	uint32_t reserved0;
	const SparkQwen38KvBlockTableView *kv_block_table;
	const SparkQwen38DecodeBatchView *decode_batch;
	const SparkQwen38PrefillFrameView *prefill_frame;
	const SparkQwen38MtpDraftView *mtp_draft;
	const SparkQwen38GdnSnapshotView *gdn_snapshot;
	SparkHiddenTransportSession *hidden_input_transport_session;
	SparkHiddenTransportSession *hidden_output_transport_session;
	SparkQwen38HiddenTransportPostReceiveFunction hidden_input_post_receive_function;
	SparkQwen38HiddenTransportSendFunction hidden_output_send_function;
	SparkHiddenTransportPacket hidden_input_packet;
	SparkHiddenTransportPacket hidden_output_packet;
} SparkQwen38ResidentDecodeStageFrameContext;

SparkStatus SparkQwen38ResidentDecodeStageInitialize(const SparkFirmwareModuleConfiguration *configuration, const SparkFirmwareModuleHostServices *host_services, void **module_state);
SparkStatus SparkQwen38ResidentDecodeStageExecute(void *module_state, SparkModelDriverFrame *frame);
SparkStatus SparkQwen38ResidentDecodeStageAdmit(void *module_state, const SparkModelDriverAdmissionRequest *request, SparkModelDriverAdmissionDecision *decision);
SparkStatus SparkQwen38ResidentDecodeStageSnapshot(void *module_state, uint32_t program_id, SparkModelDriverRuntimeSnapshot *snapshot);
