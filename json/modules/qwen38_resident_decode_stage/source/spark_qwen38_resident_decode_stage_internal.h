...
/*
 * Stage-pack wire contract, Qwen 3.8 Max dialect - formerly the standalone
 * source/spark_qwen38_stagepack_format.h (cleanup round 2026-08-24). The
 ...
 * to the frozen reference under tests/fixtures/stagepack_parity/qwen38/.
 */
...
/* Layer-class values are the shared reader's
 * (runtime/spark_stagepack_reader.h), under this family's names. */
#define SPARK_QWEN38_STAGEPACK_CLASS_GLOBAL SPARK_STAGE_PACK_LAYER_CLASS_GLOBAL
#define SPARK_QWEN38_STAGEPACK_CLASS_EVERY_LAYER SPARK_STAGE_PACK_LAYER_CLASS_EVERY_LAYER
#define SPARK_QWEN38_STAGEPACK_CLASS_GDN_LAYER SPARK_STAGE_PACK_LAYER_CLASS_GDN_LAYER
#define SPARK_QWEN38_STAGEPACK_CLASS_ATTN_LAYER SPARK_STAGE_PACK_LAYER_CLASS_ATTN_LAYER
...
{
	return(SparkStagePackFullAttentionLayersBelow(SPARK_QWEN38_MODEL_ATTENTION_PERIOD,layer_count));
}
 ...
 * (multi-stage only) a second embedding copy on the last stage. The
 * arithmetic is the shared periodic-slice inventory; this family's part is
 * only its constant row.
 */
#define SPARK_QWEN38_STAGEPACK_INVENTORY_PERIODIC \
	SPARK_QWEN38_MODEL_LAYER_COUNT,SPARK_QWEN38_MODEL_ATTENTION_PERIOD,10u,9u,6u,22u,1u
...
static inline uint32_t SparkQwen38StagePackExpectedTensorCount(uint32_t first_layer_index, uint32_t layer_count)
{
	return(SparkStagePackPeriodicSliceInventory(first_layer_index,layer_count,
		SPARK_QWEN38_STAGEPACK_INVENTORY_PERIODIC));
}
...
/*
 * Byte accounting is the shared reader's parameterized accounting
 * (runtime/spark_stagepack_reader.h); these forwards map the family's wire
 * format enum onto the normalized classes - ids identical in both homes.
 */
...
{
	return(SparkStagePackWeightPayloadBytes(SparkQwen38StagePackWeightClass(weight_format),rows,columns));
}

static inline uint64_t SparkQwen38StagePackScaleBytes(uint32_t weight_format, uint32_t rows, uint32_t columns)
{
	return(SparkStagePackWeightScaleBytes(SparkStagePackWeightClassAccounting(SparkQwen38StagePackWeightClass(weight_format)),rows,columns));
}