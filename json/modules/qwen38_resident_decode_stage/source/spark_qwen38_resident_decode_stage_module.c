...
#include "spark_qwen38_resident_decode_stage_internal.h" /* former spark_qwen38_stagepack_format.h */
...
typedef struct SparkQwen38ModuleState
{
	...
	/* Routed-expert table mode (TP16 pack production): 0 undetermined,
	 * 1 whole tables seen, 2 rank-local slices seen. A pack mixing the two
	 * modes fails validation; every MOE entry must match the latched mode
	 * with its own kind's exact sliced extent. */
	uint32_t moe_table_mode;
...
static SparkStatus SparkQwen38ModuleValidateEntry(SparkQwen38ModuleState *state, const SparkQwen38StagePackEntry *entry, uint64_t file_bytes, uint32_t *is_global)
{
	...
	/* The three routed-expert tables admit a second, TP-gated shape: the
	 * rank-local view produced by tools/qwen38_tp16_stagepack.py (docs/
	 * QWEN38_MAX_BRINGUP.md section 3). Rows shrink to exactly
	 * (EXPERT_COUNT / tp_degree) experts' worth; columns are untouched; the
	 * format may be MXFP4_E2M1 (the capacity mitigation) or the vendor FP8
	 * instead of the natural FP8. Gated on the CONFIGURED tp degree (> 1)
	 * so whole-pack deployments keep their byte-identical behavior; the
	 * header stays canonical and coverage checks are unchanged. */
	uint32_t moe_kind = entry->tensor_kind == SPARK_QWEN38_STAGEPACK_TENSOR_MOE_W1 ||
		entry->tensor_kind == SPARK_QWEN38_STAGEPACK_TENSOR_MOE_W3 ||
		entry->tensor_kind == SPARK_QWEN38_STAGEPACK_TENSOR_MOE_DOWN;
	uint32_t rows_per_expert,experts_per_rank,sliced;
	if ( SparkQwen38StagePackResolvedShape(entry->tensor_kind,global != 0u ? 0u : entry->layer_index,global,&shape) != 0 )
		return(SPARK_STATUS_VALIDATION_FAILED);
	sliced = entry->rows != shape.rows || entry->columns != shape.columns;
	if ( sliced != 0u )
	{
		if ( moe_kind == 0u || state->tp_degree <= 1u ||
			(SPARK_QWEN38_MODEL_ROUTED_EXPERT_COUNT % state->tp_degree) != 0u ||
			(shape.rows % SPARK_QWEN38_MODEL_ROUTED_EXPERT_COUNT) != 0u ||
			entry->columns != shape.columns )
			return(SPARK_STATUS_VALIDATION_FAILED);
		rows_per_expert = shape.rows / SPARK_QWEN38_MODEL_ROUTED_EXPERT_COUNT;
		experts_per_rank = SPARK_QWEN38_MODEL_ROUTED_EXPERT_COUNT / state->tp_degree;
		/* Exactly this rank's expert count - no partial experts, no smaller
		 * divisor masquerading as a slice. */
		if ( entry->rows != experts_per_rank * rows_per_expert )
			return(SPARK_STATUS_VALIDATION_FAILED);
	}
	else if ( entry->rows != shape.rows )
		return(SPARK_STATUS_VALIDATION_FAILED);
	/* Strict natural format, except the three routed-expert tensors may also
	 * arrive BF16 (synthesized test packs) or - in the rank-local view only -
	 * MXFP4_E2M1 (the capacity mitigation); anything else fails. */
	if ( entry->weight_format != shape.natural_format )
	{
		uint32_t bf16_escape = entry->weight_format == SPARK_QWEN38_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_BF16 &&
			(shape.natural_format == SPARK_QWEN38_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_MXFP4_E2M1 || shape.natural_format == SPARK_QWEN38_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_FP8_E4M3_F32B128);
		uint32_t mxfp4_rank_local = sliced != 0u && moe_kind != 0u && state->tp_degree > 1u &&
			entry->weight_format == SPARK_QWEN38_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_MXFP4_E2M1;
		if ( bf16_escape == 0u && mxfp4_rank_local == 0u )
			return(SPARK_STATUS_VALIDATION_FAILED);
	}
	...
	if ( moe_kind != 0u && state->tp_degree > 1u &&
		...
		(shape.rows % SPARK_QWEN38_MODEL_ROUTED_EXPERT_COUNT) == 0u )
	{
		/* Consistency: latch the pack's table mode from the FIRST routed-
		 * expert entry, then hold every later MOE entry to that mode with
		 * its OWN kind's exact extent (W1/W3 slice to experts_per_rank x I,
		 * DOWN to experts_per_rank x H - raw row counts legitimately differ
		 * between the two classes). Mixing modes in one pack would run some
		 * layers on the wrong expert stride. */
		uint32_t rows_per_expert = shape.rows / SPARK_QWEN38_MODEL_ROUTED_EXPERT_COUNT;
		uint32_t expected_sliced_rows = (SPARK_QWEN38_MODEL_ROUTED_EXPERT_COUNT / state->tp_degree) * rows_per_expert;
		uint32_t entry_mode = sliced != 0u ? 2u : 1u;
		if ( entry_mode == 2u && entry->rows != expected_sliced_rows )
			return(SPARK_STATUS_VALIDATION_FAILED);
		if ( state->moe_table_mode == 0u )
			state->moe_table_mode = entry_mode;
		else if ( state->moe_table_mode != entry_mode )
			return(SPARK_STATUS_VALIDATION_FAILED);
	}
	*is_global = global;
	return(SPARK_STATUS_OK);
 ...
 * Declared byte counts were pinned per entry against the computed extents;
 * this adds the cross-entry aliasing view. */
static SparkStatus SparkQwen38ModuleSweepDirectory(const SparkQwen38StagePackEntry *directory,uint32_t tensor_count,uint64_t minimum_offset)
{
	uint32_t i,j;
	for ( i = 0u; i < tensor_count; i++ )
	{
		const SparkQwen38StagePackEntry *entry = &directory[i];
		if ( entry->payload_offset < minimum_offset )
			return(SPARK_STATUS_VALIDATION_FAILED);
		for ( j = i + 1u; j < tensor_count; j++ )
		{
			const SparkQwen38StagePackEntry *other = &directory[j];
...
static SparkStatus SparkQwen38ModuleLoadPack(SparkQwen38ModuleState *state, const char *path)
{
	...
	if ( status == SPARK_STATUS_OK )
		status = SparkQwen38ModuleSweepDirectory(directory,header.tensor_count,(uint64_t)SPARK_QWEN38_STAGEPACK_HEADER_BYTES + (uint64_t)header.tensor_count * SPARK_QWEN38_STAGEPACK_ENTRY_BYTES);
	if ( status == SPARK_STATUS_OK )
...
static SparkStatus SparkQwen38ModuleRunMoe(SparkQwen38ModuleState *state, SparkQwen38ModuleSlot *slot, const void *mlp_norm_bf16, const SparkQwen38MoeWeights *weights, uint32_t rows)
{
	...
	cudaError_t error;
	SparkStatus status;
	if ( state->moe_table_mode == 2u )
	{
		/* Rank-local routed-expert tables LOAD (the TP16 pack production
		 * path, docs/QWEN38_MAX_BRINGUP.md section 3) but the grouped-expert
		 * launchers still assume the whole-table pointer shift - executing
		 * them on a sliced view would stride past the buffer. Fail loudly
		 * until the rank-local launcher increment lands; whole-table packs
		 * keep their byte-identical execution path. */
		fprintf(stderr,"%s rank_local_moe_execution_not_wired tp_degree=%u rows=%u\n",SPARK_QWEN38_MODULE_TAG,state->tp_degree,rows);
		return(SPARK_STATUS_UNSUPPORTED);
	}
