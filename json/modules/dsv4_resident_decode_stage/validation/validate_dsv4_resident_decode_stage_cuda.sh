...
case "${batch_bucket}" in
    1|2|4|6|8|9|11|16|32|64|128|256|512|1024)
        ;;
    *)
...
unset SPARK_DSV4_REFERENCE_INPUT_HIDDEN_PATH
reference_mandated=0
if [[ "${SPARK_DSV4_STAGE_INDEX:-}" == "0" &&
      "${SPARK_DSV4_STAGE_FIRST_LAYER:-}" == "0" &&
      "${SPARK_DSV4_STAGE_LAYER_COUNT:-}" == "3" ]]; then
    reference_mandated=1
fi
if [[ "${reference_mandated}" == "1" || -n "${SPARK_DSV4_REFERENCE_MANIFEST_SHA256:-}" ]]; then
    # The numerical oracle gate is slice-independent: any layer range may
    # compare against its own fixture boundary, addressed by slice. Stage 0
    # layers 0..2 remains the one configuration where the comparison is
    # mandatory.
    if [[ ! "${SPARK_DSV4_STAGE_FIRST_LAYER:-}" =~ ^[0-9]+$ ||
          ! "${SPARK_DSV4_STAGE_LAYER_COUNT:-}" =~ ^[1-9][0-9]*$ ]]; then
        echo "oracle-enabled validation requires numeric SPARK_DSV4_STAGE_FIRST_LAYER/LAYER_COUNT" >&2
        exit 2
    fi
    first_layer="${SPARK_DSV4_STAGE_FIRST_LAYER}"
    layer_count="${SPARK_DSV4_STAGE_LAYER_COUNT}"
    last_layer=$((first_layer + layer_count - 1))
    if [[ ! "${SPARK_DSV4_REFERENCE_MANIFEST_SHA256:-}" =~ ^[0-9a-f]{64}$ ]]; then
        echo "oracle-enabled validation requires its retained reference manifest SHA-256" >&2
        exit 2
    fi
    python3 "${reference_verifier}" \
        "${reference_fixture_directory}" \
        "${SPARK_DSV4_REFERENCE_MANIFEST_SHA256}" \
        --expect-layer-first "${first_layer}" \
        --expect-layer-count "${layer_count}"
    export SPARK_DSV4_REFERENCE_TOKEN_PATH="${reference_fixture_directory}/prompt_tokens.u32le"
    export SPARK_DSV4_REFERENCE_OUTPUT_PATH="${reference_fixture_directory}/after_layer_${last_layer}.bf16le"
    if (( first_layer > 0 )); then
        export SPARK_DSV4_REFERENCE_INPUT_HIDDEN_PATH="${reference_fixture_directory}/after_layer_$((first_layer - 1)).bf16le"
    fi
fi
