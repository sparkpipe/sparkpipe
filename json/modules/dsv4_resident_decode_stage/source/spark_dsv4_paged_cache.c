	...
	const SparkDsv4PagedCacheConfiguration *configuration)
{
	if ( configuration == 0 || configuration->layer_count == 0u ||
		...
		return(0u);
	/* Page-capacity sanity (nonzero, logical covering the physical pool
	 * and every lane's widest row) is the shared paged-KV predicate. */
	return(SparkPagedKvPoolGeometryIsValid(
		configuration->logical_page_capacity,
		configuration->physical_page_capacity,
		configuration->maximum_sequence_positions,
...
{
	cache->blocks = (SparkKvCacheBlock *)SparkPagedKvCheckedCalloc(
		cache->logical_page_capacity,sizeof(cache->blocks[0]));
	cache->entries = (SparkKvPageCacheEntry *)SparkPagedKvCheckedCalloc(
		cache->logical_page_capacity,sizeof(cache->entries[0]));
	cache->sequences = (SparkKvPageCacheSequence *)SparkPagedKvCheckedCalloc(
		sequence_capacity,sizeof(cache->sequences[0]));
	cache->resident_page_owners = (uint32_t *)SparkPagedKvCheckedCalloc(
		cache->physical_page_capacity,sizeof(cache->resident_page_owners[0]));
	cache->hash_bucket_heads = (uint32_t *)SparkPagedKvCheckedCalloc(
		cache->logical_page_capacity,sizeof(cache->hash_bucket_heads[0]));
	cache->entry_indices_by_logical_page =
		(uint32_t *)SparkPagedKvCheckedCalloc(cache->logical_page_capacity,
			sizeof(cache->entry_indices_by_logical_page[0]));
	cache->physical_page_generations = (uint64_t *)SparkPagedKvCheckedCalloc(
		cache->physical_page_capacity,sizeof(cache->physical_page_generations[0]));
	cache->physical_page_content_logical_pages =
		(uint32_t *)SparkPagedKvCheckedCalloc(cache->physical_page_capacity,
			sizeof(cache->physical_page_content_logical_pages[0]));
	cache->host_device_page_table = (uint32_t *)SparkPagedKvCheckedCalloc(
	...
	SparkStageModuleLedger *ledger)
{
	...
	cache->lane_page_capacity =
		SparkPagedKvBlocksForPositions(
			configuration->maximum_sequence_positions,
			SPARK_DSV4_PAGED_POOL_BLOCK_TOKENS);
