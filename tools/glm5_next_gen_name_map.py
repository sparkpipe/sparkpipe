...
def main() -> int:
    ...
    e(A + "f_a_proj.weight", "kda_decay_gate_down_weight", "k3", "kda_layers",
      ...
      transform="fuse rows decay_down|gate_down from f_a_proj+g_a_proj "
                "[128+128, 4096]",
      delta="glm53 gate is the LOW-RANK two-stage g_a->g_b form (the k3 "
            "pack's historical fusion vocabulary); the released K3 "
            "checkpoint is full-rank g_proj - the module must read the "
            "fused down + gate_up pair")
