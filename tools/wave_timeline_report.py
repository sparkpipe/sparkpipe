...
LINE = re.compile(r"G5N-WAVE-TIMING rank=(?P<rank>\d+) waves=(?P<waves>\d+) rows=(?P<rows>\d+) prefill=(?P<prefill>\d+) graph=(?P<graph>\d+) "
                  r"eager=(?P<eager>\d+) (?:linear=(?P<linear>\d+) )?graph_path=(?P<path>\d+) retries=(?P<retries>\d+) "
                  ...
                  r"graph_run_ms=(?P<graph_run>\d+) eager_run_ms=(?P<eager_run>\d+) (?:linear_run_ms=(?P<linear_run>\d+) )?decode_wait_ms=(?P<decode_wait>\d+) "
...
def total(rows, field):
    return sum(int(row.group(field) or 0) for row in rows)
...
def rank_line(rank, rows):
    waves, graph, linear, prefill = total(rows, "waves"), total(rows, "graph"), total(rows, "linear"), total(rows, "prefill")
    ...
    return (f"{rank:>4} {len(rows):>7} {waves:>6} {ratio(total(rows, 'rows'), waves):>9.1f} {100.0 * ratio(prefill, waves):>8.1f} {100.0 * ratio(graph, waves):>6.1f} {100.0 * ratio(linear, waves):>7.1f} "
            ...
            f"{ratio(total(rows, 'eager_run') - total(rows, 'linear_run'), waves - graph - linear):9.1f} {ratio(total(rows, 'linear_run'), linear):10.1f} "
            f"{ratio(total(rows, 'decode_wait'), waves - prefill):11.1f} {total(rows, 'captures'):>8} "
...
def main():
    ...
    print("graph, eager and linear runs are means per wave of that kind; eager counts only waves that ran the chain state machine")
    print(f"{'rank':>4} {'windows':>7} {'waves':>6} {'rows/wave':>9} {'prefill%':>8} {'graph%':>6} {'linear%':>7} {'path':>8} {'retries/wave':>12} {'busy':>24} | "
          + " ".join(f"{part:>6}" for part in INTERVALS) + " | " + " ".join(f"{part:>6}" for part in COLLECTIVE + ("other",))
          + f" | {'graph_run':>9} {'eager_run':>9} {'linear_run':>10} {'decode_wait':>11} {'captures':>8} {'capture_ms':>10} | {'wait99_us':>9} {'run99_us':>9}")
