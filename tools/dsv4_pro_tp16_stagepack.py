...
``dsv4_stagepack.py --first-layer 0 --layer-count 61``. This utility only
...
def layer_slice(pp_stages: int, pp_stage: int) -> Tuple[int, int]:
    ...
    base, remainder = divmod(61, pp_stages)
    if base == 0:
...
def shard_pack(input_path: Path, output_path: Path, rank: int,
    ...
    with input_path.open("rb") as source:
        ...
        if header[9] != 0 or header[10] != 61:
            raise PackFailure("input must cover the complete 61-layer model")
