...
esac
{
  ...
  for i in $(seq 0 15); do
    hex=$(printf '%x' "$i")
    host="spark$hex"
    # THE STAGE LAW (runtime/model_resident_deployment.c ValidateStructure):
    # stage_index must be a PERMUTATION of 0..N-1 - one pipeline slot per
    # node, duplicated stages are schema_error. The TP placement lives in
    # the adapter config; the runner derives its PP stage from world/tp.
    stage=$i
    comma=","
    [ "$i" = "15" ] && comma=""
    echo '    {'
