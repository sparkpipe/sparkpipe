#!/bin/bash
# dsv4flash TP16 hill-climb cell3: weightd residency — warm-attach instead of
# cold 22G/rank pack reads. weightdctl load per node first (file-derived
# identity), residentd launches with the weightd socket in the deployment,
# O128 x3 with the exactness gate BEFORE timing.
# NOTE: identity strings (model/revision) must match what the dsv4 module
# derives from its pack; both sides derive from the same rank file bytes.
set -uo pipefail
BASE=/home/spark5/lane-dsv4flash-m1
CFG=$BASE/tp16-configs-weightd
WCTL=$BASE/src2/build/weightdctl
...
[ -x $WCTL ] || WCTL=$BASE/src2/build/sparkpipe_weightdctl
[ -x $WCTL ] || { echo "NO-WEIGHTDCTL (build it in src2 first)"; exit 1; }
[ -f $BASE/driver-tp16/model_driver.so ] || { echo "NO-CELL1-RUNTIME (cell1 must run first)"; exit 1; }

echo "== [1/5] stage weightd configs"
for h in $RANK_HOST; do
  rt=$(rt_of $h)
  if [ "$h" != "spark5" ]; then
...
echo "== [2/5] weightdctl load per node (warm arenas)"
for h in $RANK_HOST; do
  r=$(rank_of $h)
  ssh -o BatchMode=yes $h "SPARK_WEIGHTD_SOCKET=/tmp/spark_weightd.sock $WCTL load /home/$h/sparkdata/dsv4flash.tp16/packs/dsv4flash.tp16.rank$r.spstage dsv4_flash 7872f01b1d1fe23eabc4c98b48bffcef5a386062 || echo LOAD-FAIL-$h" &
done
wait

echo "== [3/5] TERM stragglers + drop caches + launch"
term_all
sleep 2
for h in $RANK_HOST; do
...
echo "== [4/5] ready-wait (max 30 min)"
FAIL=0
for h in $RANK_HOST; do
  rt=$(rt_of $h); ok=""
...
echo "== [5/5] O128 x3, exactness before timing"
trap term_all EXIT
for i in 1 2 3; do
  ssh -o BatchMode=yes spark0 "cd /home/spark0/sparkdata/dsv4flash.tp16/rt && ./bin/sparkpipe_model_batch --deployment config/model_resident.json --runtime-root /home/spark0/sparkdata/dsv4flash.tp16/rt --batch /home/spark5/lane-dsv4flash-m1/staging/devcycle-o128-batch.json" 2> $BASE/results/tp16_cell3_run$i.stderr | python3 -u -c "
import sys, time
for line in sys.stdin:
    sys.stdout.write(f'{time.time():.6f} {line}')
    sys.stdout.flush()" > $BASE/results/tp16_cell3_run$i.jsonl
done
python3 << 'PY'
import json
base = "/home/spark5/lane-dsv4flash-m1/results/tp16_cell3_run"
exp = json.load(open("/home/spark5/lane-dsv4flash-m1/tp16-configs-weightd/expected_o128_tokens.json"))
for i in (1, 2, 3):
    ev = []
