...
extern SparkStatus SparkQwen4FlashResidentDecodeStageInitialize(
	const SparkFirmwareModuleConfiguration *configuration,
	const SparkFirmwareModuleHostServices *host_services,
	void **module_state);
extern void SparkQwen4FlashResidentDecodeStageDestroy(void *module_state);
extern SparkStatus SparkQwen4FlashResidentDecodeStageExecute(void *module_state, SparkModelDriverFrame *frame);

#define T1_Q3F_PROMPT_CAPACITY 64u

static uint32_t t1_parse_prompt(const char *text, uint32_t *prompt)
{
	const char *scan = text;
	uint32_t count = 0u;
	while ( *scan != '\0' && count < T1_Q3F_PROMPT_CAPACITY )
	{
		char *end;
		unsigned long value;
		errno = 0;
		...
		{
			fprintf(stderr,"t1_q3f_harness bad prompt token near '%s'\n",scan);
			exit(2);
		}
	...
	if ( count == 0u )
	{
		fprintf(stderr,"t1_q3f_harness empty prompt\n");
		exit(2);
	}
	return(count);
}
...
int main(int argc, char **argv)
{
	...
	uint32_t prompt[T1_Q3F_PROMPT_CAPACITY];
	uint32_t prompt_count,new_tokens,step,status,generated;
	uint32_t input_token,output_token;
	void *state;
	if ( argc != 3 )
	{
		fprintf(stderr,"usage: t1_q3f_harness PROMPT_CSV NEW_TOKENS\n");
		return(2);
	}
	...
	state = 0;
	status = SparkQwen4FlashResidentDecodeStageInitialize(&configuration,&services,&state);
	if ( status != SPARK_STATUS_OK )
	{
		fprintf(stderr,"t1_q3f_harness initialize status=%d\n",(int)status);
		return(1);
	}
	generated = 0u;
	{
		...
		uint32_t timing = getenv("T1_Q3F_TIMING") != 0 ? 1u : 0u;
		...
		for ( step = 0u; step < prompt_count + new_tokens; step++ )
		{
			...
			status = SparkQwen4FlashResidentDecodeStageExecute(state,&frame);
			if ( status != SPARK_STATUS_OK )
			{
				fprintf(stderr,"t1_q3f_harness execute step=%u status=%d\n",step,(int)status);
	...
	SparkQwen4FlashResidentDecodeStageDestroy(state);
	fprintf(stderr,"t1_q3f_harness done steps=%u generated=%u\n",prompt_count + new_tokens,generated);
	return(0);
}
