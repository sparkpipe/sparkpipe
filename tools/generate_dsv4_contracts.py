...
PRO_DESCRIPTION_PATH = ROOT / "examples" / "model_descriptions" / "dsv4_pro_resident_decode_stage_firmware.json"
PRO_B1_DESCRIPTION_PATH = (
        ROOT / "examples" / "model_descriptions" /
        "dsv4_pro_resident_decode_stage_firmware_b1.json")
PRO_SOURCE_MODEL_REVISION = "GA release (HF, 2026-08-13)"
PRO_DRIVER_MODEL_ID = "deepseek.v4.pro.resident-decode-stage-firmware"
PRO_DRIVER_REVISION = "h7168-l61-dsa-e384k6-hash3-v129280-ga0813-v1"
PRO_MODULE_ID_PREFIX = (
        "spark.dsv4.pro.resident_decode_stage.linear_fp8.expert_mxfp4."
        "kv_bf16.h7168.l61.e384.k6.ga0813")
PRO_MODULE_ID_SUFFIX = "v1"
PRO_MODULE_ID = f"{PRO_MODULE_ID_PREFIX}.{PRO_MODULE_ID_SUFFIX}"
PRO_MODULE_TARGET = (
        "cuda.sm121.dsv4.pro.resident_decode_stage.linear_fp8."
        "expert_mxfp4.kv_bf16")
ALIASES_HEADER_PATH = (
        ROOT / "model-families" / "dsv4" / "include" / "sparkpipe" /
        "spark_dsv4_pro_model_aliases.h")
CONTRACTS = {
    "flash": (
...
def pro_module_id(batch_bucket: int) -> str:
    if batch_bucket == 1024:
        return PRO_MODULE_ID
    if batch_bucket not in (1, 2, 4, 6, 8, 9, 11, 16, 32, 64, 128, 256, 512):
        raise ValueError(f"invalid DSV4 batch bucket: {batch_bucket}")
    return f"{PRO_MODULE_ID_PREFIX}.b{batch_bucket}.{PRO_MODULE_ID_SUFFIX}"


def _render_model_description(
        contract: dict[str, Any], batch_bucket: int, variant: str) -> str:
    if variant == "pro":
        driver_model_id = PRO_DRIVER_MODEL_ID
        driver_revision = PRO_DRIVER_REVISION
        source_revision = PRO_SOURCE_MODEL_REVISION
        module_id = pro_module_id(batch_bucket)
        module_target = PRO_MODULE_TARGET
        purpose = (
            "DeepSeek V4 Pro resident firmware with checkpoint FP8 "
            "non-expert weights, MXFP4 routed-expert weights, BF16 KV "
            "cache and FP32 accumulation"
        )
        # Speculation is not qualified for Pro yet and the packer carries a
        # single MTP layer; both literals mirror the committed artifacts.
        speculative_decoding = "disabled"
        speculative_decoding_reason = (
            "checkpoint MTP layer packed but speculation not yet qualified "
            "for Pro")
        checkpoint_dspark_layer_count = 1
        packed_dspark_layer_count = 1
    else:
        driver_model_id = FLASH_DRIVER_MODEL_ID
        driver_revision = FLASH_DRIVER_REVISION
        source_revision = contract["source_revision"]
        module_id = flash_module_id(batch_bucket)
        module_target = FLASH_MODULE_TARGET
        purpose = (
        ...
        speculative_decoding_reason = (
            contract["runtime"]["speculative_decoding_reason"])
        checkpoint_dspark_layer_count = contract["dspark"]["layer_count"]
        packed_dspark_layer_count = (
            contract["runtime"]["packed_mtp_layer_count"])
    model = contract["model"]
    attention = contract["attention"]
    moe = contract["moe"]
    precision = contract["precision"]
    description = {
        ...
        "metadata": {
            ...
            "module_geometry": {
                ...
                "expert_intermediate_dimension": moe["expert_intermediate_dimension"],
                "vocab_count": model["vocabulary_size"],
                "checkpoint_dspark_layer_count": checkpoint_dspark_layer_count,
                "packed_dspark_layer_count": packed_dspark_layer_count,
            },
            "runtime_contract": {
                ...
                "speculative_decoding_reason": speculative_decoding_reason,
            },
        },
        "stages": [{
    ...
    # Flash artifacts carry a trailing newline; the committed Pro
    # descriptions predate it and their pins stay authoritative.
    rendered = json.dumps(description, indent=2, sort_keys=True)
    if variant == "flash":
        rendered += "\n"
    return rendered
...
def render_flash_model_description(
    ...
    return _render_model_description(contract, batch_bucket, "flash")


def render_pro_model_description(
        contract: dict[str, Any], batch_bucket: int = 1024) -> str:
    return _render_model_description(contract, batch_bucket, "pro")
...
def main() -> int:
    ...
    stale: list[str] = []
    bucket_ladder = (6, 8, 9, 11, 16, 32, 64)
    for variant, (source_path, header_path, normalized_path) in CONTRACTS.items():
        ...
        if variant == "flash":
            render_description = render_flash_model_description
            description_prefix = "dsv4_resident_decode_stage_firmware"
        else:
            render_description = render_pro_model_description
            description_prefix = "dsv4_pro_resident_decode_stage_firmware"
        description = render_description(contract)
        b1_description = render_description(contract, 1)
        description_sha256 = hashlib.sha256(description.encode("utf-8")).hexdigest()
        b1_description_sha256 = hashlib.sha256(
            b1_description.encode("utf-8")).hexdigest()
        bucket_shas: dict[int, str] = {}
        bucket_outputs: dict[Path, str] = {}
        for bucket in bucket_ladder:
            bucket_description = render_description(contract, bucket)
            bucket_shas[bucket] = hashlib.sha256(
                bucket_description.encode("utf-8")).hexdigest()
            bucket_outputs[
                ROOT / "examples" / "model_descriptions" /
                f"{description_prefix}_b{bucket}.json"] = bucket_description
        outputs = {
        ...
        outputs.update(bucket_outputs)
        if variant == "flash":
            outputs[FLASH_DESCRIPTION_PATH] = description
            outputs[FLASH_B1_DESCRIPTION_PATH] = b1_description
        else:
            outputs[PRO_DESCRIPTION_PATH] = description
            outputs[PRO_B1_DESCRIPTION_PATH] = b1_description
            # The alias header is the compile-time pin board the serving
            # adapter ladder reads under SPARK_DSV4_PRO_BUILD; a drifted or
            # missing bucket pin is exactly the P0 this guards.
            aliases = ALIASES_HEADER_PATH.read_text(encoding="utf-8")
            for bucket in bucket_ladder:
                pin = (
                    f"#define SPARK_DSV4_MODEL_DESCRIPTION_SHA256_B{bucket} "
                    f'"{bucket_shas[bucket]}"')
                if pin not in aliases:
                    stale.append(f"alias header pin B{bucket}")
