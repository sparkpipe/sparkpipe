...
== ASSUMED CONSTANTS - measured nowhere in this tree, re-run with receipts ====
  ...
  GB10 SM specs (occupancy sanity check ONLY):
    ...
    SMEM_PER_SM       131072 B    NOT assumed and no longer local: sourced from
                                  tools/hardware_topology.py (the Python mirror of
                                  include/sparkpipe/spark_hardware_topology.h;
                                  cuda.sm121.gb10 pins it at 131072), the same
                                  profile inference/kernels/layout.cuh's
                                  LM_SMEM_SM_TOTAL now draws from, matching the
                                  calibration doc's "128 KB L1/shared per SM" (:23).
...
from hardware_topology import TARGET_CUDA_SM121_GB10, select_target  # noqa: E402
from nvme_kv_estimate import (  # noqa: E402
    GB, MODELS_BY_NAME, BATCHES, coverage, step_gb)

# -- target topology ----------------------------------------------------------
# Single source with the C side (include/sparkpipe/spark_hardware_topology.h).
# Pinned explicitly to cuda.sm121.gb10 because this IS the GB10 cost model; an
# unpinned field would be a defect here, not an estimate.
SPARK_HW = select_target(TARGET_CUDA_SM121_GB10)
...
SMEM_PER_SM = SPARK_HW.shared_per_sm_bytes          # 131072 B on cuda.sm121.gb10
GEMM_BLOCK_THREADS = 256

# -- derived constants ---------------------------------------------------------
ETA_BW = 0.80
NODE_BW_GBPS = SPARK_HW.memory_bandwidth_gbps       # GB10 LPDDR5X (roadmap:29)
