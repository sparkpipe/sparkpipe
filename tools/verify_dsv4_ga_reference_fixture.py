...
def vector_path_for(layer_first: int, layer_count: int) -> str:
	"""Boundary dump name for a layer range (after the last consumed layer)."""
	return f"after_layer_{layer_first + layer_count - 1}.bf16le"
...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument(
		"--expect-layer-first",
		type=int,
		help="require the fixture to start at this layer index",
	)
	parser.add_argument(
		"--expect-layer-count",
		type=int,
		help="require the fixture to cover exactly this many layers",
	)
...
def verify_vector(value: Any, fixture: Path, vector_path: str) -> None:
...
def verify_fixture(
	...
	expect_layer_first: int | None = None,
	expect_layer_count: int | None = None,
) -> None:
	...
	require(
		not isinstance(layer_range["first"], bool)
		and isinstance(layer_range["first"], int)
		and layer_range["first"] >= 0,
		"layer_range.first is invalid",
	)
	require(
		not isinstance(layer_range["count"], bool)
		and isinstance(layer_range["count"], int)
		and layer_range["count"] >= 1,
		"layer_range.count is invalid",
	)
	if expect_layer_first is not None:
		require_integer(layer_range["first"], expect_layer_first, "layer_range.first")
	if expect_layer_count is not None:
		require_integer(layer_range["count"], expect_layer_count, "layer_range.count")
	vector_path = vector_path_for(layer_range["first"], layer_range["count"])
	...
	verify_vector(manifest["vectors"][0], fixture, vector_path)
...
def main() -> int:
	...
	try:
		verify_fixture(
			options.fixture_directory.absolute(),
			options.expected_manifest_sha256,
			expect_layer_first=options.expect_layer_first,
			expect_layer_count=options.expect_layer_count,
		)
