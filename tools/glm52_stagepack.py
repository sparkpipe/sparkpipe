...
def size_records(records: list[Record], codec_name: str) -> int:
    ...
    cursor = align_up(align_up(HEADER_BYTES, ALIGNMENT) + len(records) * ENTRY_BYTES, ALIGNMENT)
    for record in records:
        ...
        cursor = align_up(cursor + record.payload_bytes, ALIGNMENT)
        if record.payload_type == PAYLOAD_PACKED_WEIGHT:
            ...
            cursor = align_up(cursor + record.scale_bytes, ALIGNMENT)
    return cursor
...
def pack_header(
    ...
    return HEADER.pack(
        ...
        align_up(HEADER_BYTES, ALIGNMENT),file_bytes,revision,contract_sha,config_sha,recipe_sha,
    )
...
def build(args: argparse.Namespace) -> dict[str, object]:
    ...
    with partial.open("w+b") as file:
        ...
        file.seek(align_up(HEADER_BYTES, ALIGNMENT))
