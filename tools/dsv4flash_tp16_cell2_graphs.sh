#!/bin/bash
# dsv4flash TP16 hill-climb cell2: CUDA graphs 0->130 (the TP4-cell winner).
# Reuses the cell1 runtime binaries + published module (no rebuild). Relaunches
# the 16 residentd with the graphs-130 stage configs, runs O128 x3 with the
# exactness gate BEFORE timing, TERMs everything.
set -uo pipefail
BASE=/home/spark5/lane-dsv4flash-m1
CFG=$BASE/tp16-configs-graphs
...
echo "== [2/4] TERM stragglers + drop caches + launch"
term_all
sleep 2
for h in $RANK_HOST; do
...
echo "== [3/4] ready-wait (max 30 min; graph capture adds load time)"
FAIL=0
for h in $RANK_HOST; do
  rt=$(rt_of $h); ok=""
...
echo "== [4/4] O128 x3, exactness before timing"
trap term_all EXIT
for i in 1 2 3; do
  ssh -o BatchMode=yes spark0 "cd /home/spark0/sparkdata/dsv4flash.tp16/rt && ./bin/sparkpipe_model_batch --deployment config/model_resident.json --runtime-root /home/spark0/sparkdata/dsv4flash.tp16/rt --batch /home/spark5/lane-dsv4flash-m1/staging/devcycle-o128-batch.json" 2> $BASE/results/tp16_cell2_run$i.stderr | python3 -u -c "
import sys, time
for line in sys.stdin:
    sys.stdout.write(f'{time.time():.6f} {line}')
    sys.stdout.flush()" > $BASE/results/tp16_cell2_run$i.jsonl
done
python3 << 'PY'
import json
base = "/home/spark5/lane-dsv4flash-m1/results/tp16_cell2_run"
exp = json.load(open("/home/spark5/lane-dsv4flash-m1/tp16-configs-graphs/expected_o128_tokens.json"))
for i in (1, 2, 3):
    ev = []
