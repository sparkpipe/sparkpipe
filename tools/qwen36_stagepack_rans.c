...
static void rans_build_table(RansEnc *e, const uint16_t *values, uint64_t n)
{
	...
	if (e->id_bits > 16)
		die("id_bits > 16");
	e->ndirect = ndirect;
	e->entries = malloc((size_t)(ndirect + 1) * 3u * sizeof(uint32_t));
	e->id_table = calloc((size_t)1 << e->id_bits, sizeof(uint16_t));
	e->entry_of = malloc(65536u * sizeof(uint32_t));
	e->rid_of = malloc(65536u * sizeof(uint32_t));
...
static Buffer rans_encode_tensor(RansEnc *e, const uint16_t *tiled, uint32_t chunk_count)
{
	...
	if (!b.data)
		die("oom stream");
	out_len = 0;
	memcpy(b.data + out_len, &e->ndirect, sizeof(e->ndirect));
	out_len += sizeof(e->ndirect);
	memcpy(b.data + out_len, e->entries,
		(size_t)(e->ndirect + 1) * 3u * sizeof(uint32_t));
	out_len += (uint64_t)(e->ndirect + 1) * 3u * sizeof(uint32_t);
	memcpy(b.data + out_len, &e->id_bits, sizeof(e->id_bits));
	out_len += sizeof(e->id_bits);
	memcpy(b.data + out_len, e->id_table,
		(size_t)(1u << e->id_bits) * sizeof(*e->id_table));
	out_len += (uint64_t)(1u << e->id_bits) * sizeof(*e->id_table);
	memcpy(b.data + out_len, &chunk_count, sizeof(chunk_count));
	out_len += sizeof(chunk_count);
	offsets = (uint32_t *)(b.data + out_len);
	out_len += (uint64_t)chunk_count * sizeof(uint32_t);
	for (t = 0; t < chunk_count; t++)
	{
		...
		memcpy(chunk + SUBSTREAMS * sizeof(uint32_t), lens, sizeof(lens));
		while (data_start & 3u)
			chunk[data_start++] = 0;
		out_len += data_start;
	}
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < tensor_count; i++)
	{
		    ...
		    (en->rows % 64u) == 0u && (en->columns % 128u) == 0u && en->scale_bytes == 0u)
		{
			...
			uint16_t *tiled = malloc((size_t)nvals * sizeof(*tiled));
			RansEnc e;
			Buffer comp;
			uint32_t r, c;
	...
	if (fwrite(&out_size, sizeof(out_size), 1, of) != 1)
