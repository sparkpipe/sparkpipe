...
Follows tools/glm5_next_gen_deployment.py; adapted to the qwen38_max
...
The weightd block is load-bearing for the operator's lazy-expert
directive: residentd ensures the per-node weightd daemon and exports
SPARK_WEIGHTD_SOCKET, and the module's region loader then serves every
256-aligned pack region as a zero-copy consumer-map slice - expert data
...
QWEN38MAX_CONTROL_BASE, QWEN38MAX_TRANSPORT_BASE.
"""
import json
import os
import re
...
def resident_deployment():
    ...
    return {
        ...
        # Lazy expert data: residentd ensures the per-node weightd daemon
        # and exports SPARK_WEIGHTD_SOCKET; the module region loader then
        # serves pack regions as zero-copy consumer-map slices - expert
        # pages materialize on demand (only routed experts are touched),
        # multiple models' drivers coexist, and code-only redeploys skip
        # the bulk re-read. Eager H2D remains the documented fallback.
        "weightd": {
