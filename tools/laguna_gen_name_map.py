#!/usr/bin/env python3
"""Generate the laguna checkpoint->pack name mapping (M2).

Consumes the frozen contract (model_contracts/laguna_authoritative.json)
...
glm53_tensor_manifest.tsv, produced by tools/glm53_contract_freeze.py) to emit:
  ...
  model-families/laguna/name_map.json        - the mapping table
  model-families/laguna/tensor_patterns.json - every checkpoint tensor
...
OUT_MAP = ROOT / "model-families" / "laguna" / "name_map.json"
OUT_PATTERNS = ROOT / "model-families" / "laguna" / "tensor_patterns.json"
...
def main() -> int:
    ...
    mapping = {
        ...
        "reference_semantics": {
            "source": "transformers main: models/laguna/modeling_laguna.py",
