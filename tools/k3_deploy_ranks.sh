#!/usr/bin/env bash
# k3_deploy_ranks.sh — copy the 16 TP4 rank packs to their rank hosts in
# PARALLEL and sha256-verify each. Stage s ranks t live on spark[s*4+t]:
# stage0->spark0-3, stage1->spark4-7, stage2->spark8-b, stage3->sparkc-f.
set -u
...
echo "k3 rank deployment done (failed=$failed)"
