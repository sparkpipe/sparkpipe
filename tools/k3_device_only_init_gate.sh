#!/usr/bin/env bash
# K3 device-only init gate (F1): builds the init-contract test against a real
# rank pack and runs it on the local spark. Proves sharded init admits an
# NCCL-only device tier, still rejects no-tier and hidden-only shapes, and
# keeps honouring a supplied host tier.
# Usage: bash tools/k3_device_only_init_gate.sh RANK_PACK [NVCC]
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PACK="${1:?usage: k3_device_only_init_gate.sh RANK_PACK [NVCC]}"
NVCC="${2:-nvcc}"
cd "$ROOT"
...
"$NVCC" -std=c++17 -O3 --expt-relaxed-constexpr -lineinfo \
    ...
    tests/test_k3_device_only_init.cu \
    ...
    -Xcompiler -pthread -ldl -lcuda -lcudart -o /tmp/k3_devinit
/tmp/k3_devinit "$PACK"
