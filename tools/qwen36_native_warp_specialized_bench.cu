 ...
 * CTAs, and the target dynamic-shared ceiling (SPARK_HW_MAX_DYNAMIC_SHARED_BYTES,
 * 101376 B on cuda.sm121.gb10) caps in-flight x occupancy - every
...
int main(int argc, char **argv)
{
	...
	CHECK(cudaMalloc(&in, (uint64_t)M * K * sizeof(uint16_t)));
	CHECK(cudaMalloc(&out, (uint64_t)M * N * sizeof(uint16_t)));
	CHECK(cudaMalloc(&ref, (uint64_t)M * N * sizeof(uint16_t)));
	/* RANDOM data so the comparison is meaningful */
	{
		...
		free(h);
		h = (uint8_t *)malloc((size_t)M * K * sizeof(uint16_t));
		{
		...
		CHECK(cudaMemcpy(in, h, (size_t)M * K * sizeof(uint16_t), cudaMemcpyHostToDevice));
		free(h);
	}
	cudaStream_t stream;
	 ...
	 * with kill-switch SPARK_QWEN38_27B_WS_PLAIN=0. */
	{
		...
		uint16_t *h_ref = (uint16_t *)malloc((size_t)M * N * sizeof(uint16_t));
		uint16_t *h_out = (uint16_t *)malloc((size_t)M * N * sizeof(uint16_t));
		CHECK(cudaMemcpy(h_ref, ref, (size_t)M * N * sizeof(uint16_t), cudaMemcpyDeviceToHost));
		CHECK(cudaMemcpy(h_out, out, (size_t)M * N * sizeof(uint16_t), cudaMemcpyDeviceToHost));
		int exact = 0;
