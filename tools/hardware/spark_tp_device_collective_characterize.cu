...
static SparkStatus SparkTpCharacterizeRun(
	...
	void *output,float local_value,uint32_t payload_bytes,uint32_t credit_count,
	...
	SparkTpCharacterizeCompletion *completion)
{
	...
	for (ordinal=first_ordinal; ordinal<first_ordinal + iterations; ordinal++)
	{
		...
		SparkTpCharacterizeFill<<<
			...
			collective->local_hidden_dimension,local_value);
...
int main(int argc,char **argv)
{
	...
	{
		fprintf(stderr,"usage: %s MODULE RANK DIRECT_CSV SWITCH_CSV PAYLOAD_BYTES ITERATIONS CONTROL_PORT_BASE CREDIT_COUNT SPLIT_RING_MIN_PAYLOAD_BYTES INFLIGHT_COUNT\n",argv[0]);
		return(2);
	}
	...
	status = SparkTpCharacterizeRun(&collective,stream,local,output,
		(float)(options.rank + 1u),options.payload_bytes,options.credit_count,
	...
	if ( status == SPARK_STATUS_OK && cudaMemcpy(&result,result_output,
			sizeof(result),
			cudaMemcpyDeviceToHost) != cudaSuccess )
		status = SPARK_STATUS_DRIVER_LOAD_ERROR;
	printf("{\"rank\":%u,\"payload_bytes\":%u,\"iterations\":%u,\"credits\":%u,\"inflight\":%u,\"split_ring_min_payload_bytes\":%u,\"memory_mode\":%u,\"status\":%u,\"result\":%.1f,\"average_us\":%.3f}\n",
		options.rank,options.payload_bytes,options.iterations,
		options.credit_count,options.inflight_count,
		options.split_ring_min_payload_bytes,memory_mode,
		(uint32_t)status,
		__bfloat162float(result),status == SPARK_STATUS_OK ?
	...
	return(status == SPARK_STATUS_OK && __bfloat162float(result) == 10.0f ? 0 : 5);
}
