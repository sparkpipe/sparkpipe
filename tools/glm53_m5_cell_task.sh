...
for n in $(seq 1 "$RUNS"); do
  echo "== run $n/$RUNS =="
  ( for delay in 10 30; do sleep "$delay"; \
  ...
  probe_pid=$!
  ( cd "$rr" && export LD_LIBRARY_PATH="$rr/lib:$LD_LIBRARY_PATH" && \
    /usr/bin/time -f "WALL %e" bin/sparkpipe_model_batch \
      --deployment model_resident.json --runtime-root "$rr" \
      --batch /tmp/glm53-m5-exact32k-b1.json ) > "/tmp/glm53-m5-run$n.out" 2> "/tmp/glm53-m5-run$n.err"
  wait "$probe_pid"
  python3 - "/tmp/glm53-m5-run$n.out" <<'EOF'
import json,hashlib,sys
toks=[json.loads(l)["token_id"] for l in open(sys.argv[1]) if '"event":"token"' in l]
print(f"run$n tokens={len(toks)} sha256={hashlib.sha256(str(toks).encode()).hexdigest()[:16]}")
EOF
  tail -1 "/tmp/glm53-m5-run$n.err"
done
