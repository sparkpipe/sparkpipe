...
for host in "${HOSTS[@]}"; do
    ...
    if [[ "$driver" == "$EXPECTED_SHA" && "$config" == "yes" && "$adapter" == "yes" && "$transport" == "yes" && "$kv" == "yes" && "$pack" -gt 50000000000 ]]; then
        echo "$host OK driver=$driver pack=$pack adapter=$adapter transport=$transport kv=$kv"
        pass=$((pass + 1))
    else
        echo "$host INCOMPLETE driver=$driver pack=$pack config=$config adapter=$adapter transport=$transport kv=$kv"
        fail=$((fail + 1))
    fi
done
