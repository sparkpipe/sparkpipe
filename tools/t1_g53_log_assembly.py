...
G5N-T1 dump lines (SPARK_GLM5_NEXT_T1=1 module build).

Line grammar (one line per row):
  G5N-T1 wave seq<r> rows=<n> pos<p>=<token>...
  G5N-T1 stream L<layer> pos<p> <16384 hex u16 words>
...
after the last prompt position of that sequence).
"""
import argparse
import json
import sys
...
def assemble(log_path, prompt_length):
    sequences = {}
    streams = {}
    routes = {}
    heads = {}
    with open(log_path, "r", errors="replace") as handle:
        for line in handle:
            if line.startswith("G5N-T1 wave "):
                fields = line.split()
                seq = int(fields[3][3:])
                rows = int(fields[4][5:])
                pairs = {}
                ...
                routes[(position, layer)] = (
                    ...
                    np.frombuffer(np.array(weights, np.uint32).tobytes(), np.float32).copy())
            elif line.startswith("G5N-T1 head "):
                fields = line.split()
                ...
                heads[position] = (
                    ...
                    np.frombuffer(np.array([score_bits], np.uint32).tobytes(), np.float32).copy())
    return sequences, streams, routes, heads


def build_arrays(sequences, streams, routes, heads, prompt_length):
    arrays = {}
...
def main():
    ...
    sequences, streams, routes, heads = assemble(arguments.log,
