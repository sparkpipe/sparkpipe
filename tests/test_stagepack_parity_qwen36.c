 ...
 * frozen reference revision of the qwen36 stage-pack contract (now the
 * family internal header) and once
 ...
 *
 * The validation replica below walks SparkQwen36ModuleValidateEntry's exact
...
#include "spark_qwen36_resident_decode_stage_internal.h"
...
static uint32_t required_group_size(uint32_t weight_format)
{
	if ( weight_format == SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_MXFP4_E2M1 )
		return(32u);
	if ( weight_format == SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_FP8_E4M3_F32B128 || weight_format == SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_FP8_E4M3_E8M0B128 )
		return(128u);
	return(0u);
}
	...
	uint64_t payload_offset,uint64_t scale_offset,uint64_t file_bytes,uint32_t tp_degree)
{
	...
	uint32_t global = layer_index == SPARK_QWEN36_STAGEPACK_GLOBAL_LAYER ? 1u : 0u;
	memset(&shape,0,sizeof(shape));
	if ( SparkQwen36StagePackResolvedShape(tensor_kind,global != 0u ? 0u : layer_index,global,tp_degree,&shape) != 0 || rows != shape.rows || columns != shape.columns )
		return(1);
	if ( weight_format == SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_BF16_RANS )
	{
	...
	{
		if ( weight_format != SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_BF16 && weight_format != SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_MXFP4_E2M1 && weight_format != SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_FP8_E4M3_F32B128 && weight_format != SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_FP8_E4M3_E8M0B128 )
			return(1);
	}
	else if ( weight_format != shape.natural_format )
		return(1);
	if ( weight_format == SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_MXFP4_E2M1 ? scale_group_size != 32u : ((weight_format == SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_FP8_E4M3_F32B128 || weight_format == SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_FP8_E4M3_E8M0B128) ? scale_group_size != 128u : scale_group_size != 0u) )
		return(1);
	if ( weight_format != SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_BF16_RANS && (payload_bytes != SparkQwen36StagePackPayloadBytes(weight_format,rows,columns) || scale_bytes != SparkQwen36StagePackScaleBytes(weight_format,rows,columns)) )
		return(1);
	...
	if ( scale_bytes != 0u && (scale_offset > file_bytes || scale_bytes > file_bytes - scale_offset) )
		return(1);
	if ( layer_index == SPARK_QWEN36_STAGEPACK_MTP_LAYER || (global != 0u && (tensor_kind >= SPARK_QWEN36_STAGEPACK_TENSOR_MTP_FC && tensor_kind <= SPARK_QWEN36_STAGEPACK_TENSOR_MTP_FINAL_NORM)) )
	{
		if ( 0u == 1u ) /* owns_final_head pinned true */
			return(1);
	}
	else if ( global == 0u && (layer_index < 0u || layer_index >= 0u + SPARK_QWEN36_MODEL_LAYER_COUNT) )
		return(1);
	return(0);
}
...
int main(void)
{
	static const uint32_t layers[] = {SPARK_QWEN36_STAGEPACK_GLOBAL_LAYER,SPARK_QWEN36_STAGEPACK_MTP_LAYER,0u,1u,2u,3u,7u,62u,63u,64u,100u,UINT32_MAX};
	static const uint32_t formats[] = {SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_BF16,SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_BF16_RANS,SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_MXFP4_E2M1,SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_FP8_E4M3_F32B128,SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_FP8_E4M3_E8M0B128,SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_F32,SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_U32,6u,7u,UINT32_MAX};
	...
	for (kind = 0; kind < SPARK_QWEN36_STAGEPACK_TENSOR_KIND_COUNT + 2u; kind++)
		for (i = 0; i < (uint32_t)(sizeof(layers)/sizeof(layers[0])); i++)
			for (tp = 1; tp <= 2; tp++)
			{
				SparkQwen36StagePackTensorShape shape;
				uint32_t global = layers[i] == SPARK_QWEN36_STAGEPACK_GLOBAL_LAYER ? 1u : 0u;
				int32_t code;
				memset(&shape,0,sizeof(shape));
				code = SparkQwen36StagePackResolvedShape(kind,global != 0u ? 0u : layers[i],global,tp,&shape);
	...
	for (kind = 0; kind < SPARK_QWEN36_STAGEPACK_TENSOR_KIND_COUNT + 2u; kind++)
		for (i = 0; i < (uint32_t)(sizeof(layers)/sizeof(layers[0])); i++)
			for (f = 0; f < fmt_count; f++)
			{
				...
				uint64_t pb,sb;
				memset(&shape,0,sizeof(shape));
				if ( SparkQwen36StagePackResolvedShape(kind,global != 0u ? 0u : layers[i],global,1u,&shape) != 0 )
					continue;
				pb = SparkQwen36StagePackPayloadBytes(formats[f],shape.rows,shape.columns);
				sb = SparkQwen36StagePackScaleBytes(formats[f],shape.rows,shape.columns);
	...
	for (f = 0; f < fmt_count; f++)
		for (r = 0; r < dim_count; r++)
			for (c = 0; c < dim_count; c++)
			{
				...
				emit(tag,(long)SparkQwen36StagePackPayloadBytes(formats[f],dims[r],dims[c]));
				snprintf(tag,sizeof(tag),"C fmt=%u r=%u c=%u scale",f,dims[r],dims[c]);
				emit(tag,(long)SparkQwen36StagePackScaleBytes(formats[f],dims[r],dims[c]));
			}
	...
	/* D: computed slice inventories across the split space. */
	{
		...
		for (i = 0; i < (uint32_t)(sizeof(firsts)/sizeof(firsts[0])); i++)
			for (count = 0; count <= SPARK_QWEN36_MODEL_LAYER_COUNT + 1u; count++)
			{
				first = firsts[i];
				if ( first + count > SPARK_QWEN36_MODEL_LAYER_COUNT + 1u )
					continue;
				snprintf(tag,sizeof(tag),"D first=%u count=%u",first,count);
				emit(tag,(long)SparkQwen36StagePackExpectedTensorCount(first,count));
			}
	}

	/* E: geometry-compare codes and names under single-field hostility. */
	{
		SparkQwen36StagePackHeader expected,file_header;
		uint32_t field,u32_count = (uint32_t)SPARK_QWEN36_STAGEPACK_COMPARE_U32_FIELDS;
		static const uint32_t hostiles[] = {0u,1u,2u,UINT32_MAX};
		memset(&expected,0,sizeof(expected));
		SparkQwen36StagePackExpectedGeometry(&expected,0u,SPARK_QWEN36_MODEL_LAYER_COUNT);
		expected.directory_offset = SPARK_QWEN36_STAGEPACK_HEADER_BYTES;
		...
		for (field = 0; field < u32_count; field++)
		{
			...
			for (h = 0; h < (uint32_t)(sizeof(hostiles)/sizeof(hostiles[0])); h++)
			{
				...
				code = SparkQwen36StagePackCompareGeometry(&file_header,&expected);
				snprintf(tag,sizeof(tag),"E field=%u hostile=%u",field,hostiles[h]);
				emit(tag,(long)code);
				printf("%s name=%s\n",tag,SparkQwen36StagePackGeometryFieldName(code));
			}
		}
		emit("E clean",(long)SparkQwen36StagePackCompareGeometry(&expected,&expected));
	}
	...
	/* F: the TP shard policy over every kind and degree. */
	{
		...
		SparkQwen36StagePackTensorShape shape;
		for (kind = 0; kind < SPARK_QWEN36_STAGEPACK_TENSOR_KIND_COUNT; kind++)
			for (i = 0; i < (uint32_t)(sizeof(degrees)/sizeof(degrees[0])); i++)
			{
				uint32_t d;
				memset(&shape,0,sizeof(shape));
				if ( SparkQwen36StagePackTensorShapeOf(kind,&shape) != 0 )
					continue;
				for (d = 0; d < 3u; d++)
				{
					shape.rows = 4096u >> d;
					shape.columns = 1024u << d;
					SparkQwen36StagePackApplyTpShard(kind,degrees[i],&shape);
	...
	for (i = 0; i <= 70u; i++)
	{
		...
		emit(tag,(long)SparkHybridStagePackFullAttentionLayersBelow(SPARK_QWEN36_MODEL_ATTENTION_PERIOD,i));
	}
