...
def main() -> None:
	...
	if contract["precision"]["non_expert_activation_format"] != "bf16":
		raise SystemExit("DSV4 Flash package does not preserve BF16 spine activations")
	if (contract["precision"]["routed_expert_activation_format"], contract["precision"]["scale_format"]) != ("fp8_e4m3", "ue8m0"):
		raise SystemExit("DSV4 Flash package does not declare its expert activation codec")
	...
	stagepack = read("modules/dsv4_resident_decode_stage/source/spark_dsv4_resident_decode_stage_internal.h")
	adapter = read("modules/dsv4_resident_decode_stage/source/spark_dsv4_serving_adapter.c")
	adapter_common = read("runtime/adapter_common.c")
	...
	# The adapter audit moved generic driver-create request assembly into the
	# shared adapter layer; the budget-forwarding pins follow it there.
	require(adapter_common, "request.kv_logical_page_capacity =", "generic logical-page budget forwarding")
	require(adapter_common, "request.kv_physical_page_capacity =", "generic physical-page budget forwarding")
	...
	# The slot-alloc refactor consolidated the six pinned staging writes into
	# SparkDsv4ModuleStageRowSlotWrites; the staging pin follows it there.
	require(module, "slot->host_input_token_ids[row] = token_id;", "all-stage token staging")
	...
	require(moe_shared, "SparkDsv4LaunchExpertLinear(stream,&moe->shared_w2", "column-parallel shared-W2 full-hidden partial")
	...
	# The all-layer-count oracle gate addresses fixtures by slice; the output
	# export follows the last layer of the validated range.
	require(validator_script, 'export SPARK_DSV4_REFERENCE_OUTPUT_PATH="${reference_fixture_directory}/after_layer_${last_layer}.bf16le"', "verified reference output export")
	require(validator_script, "SPARK_DSV4_REFERENCE_MANIFEST_SHA256", "pinned reference manifest input")
	# The all-layer-count oracle gate made the exact-slice rule explicit:
	# compute the mandated slice, then reject reference-less runs of it.
	require(validator, "exact_reference_slice = node_context->stage_index == 0u && node_context->first_layer_index == 0u && node_context->layer_count == 3u", "C-side exact stage-0 slice identification")
	require(validator, "exact_reference_slice != 0 && mode->use_reference == 0", "C-side exact stage-0 reference enforcement")
