...
from host_cuda_compiler import host_cuda_cxx, host_cuda_include_flags
...
def main():
    ...
    for codec in CODECS:
        build = subprocess.run(
            [host_cuda_cxx(), *host_cuda_include_flags(), "-std=c++17", "-O1",
    ...
    for row in range(ROWS):
        ...
        normed_rows.append(normed3)
        full_scores.append([sum(n * w for n, w in zip(normed3,
                            head_weight[t * HIDDEN:(t + 1) * HIDDEN]))
                            for t in range(HEAD_VOCAB)])
        want_token = max(range(HEAD_VOCAB), key=lambda t: full_scores[row][t])
        got_token = int(series["token"][row])
        if got_token != want_token:
            print(f"  FAIL head token row {row}: {got_token} != {want_token}")
            failures += 1
    failures = compare("head normed", [x for r in normed_rows for x in r],
                       series["normed3"], failures, 2e-2)

    # The RESTRICTED walk: config.h's constrained-decoding capability executed.
    # The harness kept row 0's winner in the table and dropped row 1's; the
    # restricted argmax must reproduce an in-set winner exactly (same score,
    # same token) and clip an out-of-set winner to the best LEGAL token - never
    # the dropped one. This is the exact call shape Glm52HeadRestricted drives.
    if scalars.get("status restricted") != "0":
        print(f"  FAIL restricted returned {scalars.get('status restricted')}")
        failures += 1
    winners = [int(series["token"][row]) for row in range(ROWS)]
    restricted = [t for t in range(HEAD_VOCAB) if t != winners[1]]
    failures = exact("restricted set size",
                     [HEAD_VOCAB - 1],
                     [int(series["restrictedcount"][0])], failures)
    for row in range(ROWS):
        want_token = max(restricted, key=lambda t: full_scores[row][t])
        got_token = int(series["rtoken"][row])
        if got_token != want_token:
            print(f"  FAIL restricted token row {row}: "
                  f"{got_token} != {want_token}")
            failures += 1
        if got_token == winners[1]:
            print(f"  FAIL restricted row {row} served a dropped token")
            failures += 1
    # Row 0's winner is in-set: restriction must be invisible to it.
    if int(series["rtoken"][0]) != winners[0]:
        print(f"  FAIL restriction changed an in-set winner: "
              f"{series['rtoken'][0]} != {winners[0]}")
        failures += 1
    failures = compare("restricted row0 score", series["score"][:1],
                       series["rscore"][:1], failures, 1e-6)
