...
int main(void)
{
	...
	uint8_t *hidden_input,*hidden_output;
	...
	hidden_input = (uint8_t *)calloc(1u,(size_t)hidden_bytes);
	hidden_output = (uint8_t *)calloc(1u,(size_t)hidden_bytes);
	...
	for (byte=0u; byte<hidden_bytes; byte++)
		hidden_input[byte] = (uint8_t)(byte * 131u + 7u);
	submission.hidden_input_address = hidden_input;
	...
	memset(hidden_output,0,(size_t)hidden_bytes);
	...
	assert(memcmp(hidden_input,hidden_output,(size_t)(4u * SPARK_QWEN36_MODEL_HIDDEN_BF16_BYTES)) == 0);
	submission.work_kind = SPARK_MODEL_SERVING_WORK_KIND_DECODE;
	submission.new_token_count = 2u;
	submission.row_count = 2u;
	submission.token_count = 2u;
	...
	assert(memcmp(hidden_input,hidden_output,(size_t)(2u * SPARK_QWEN36_MODEL_HIDDEN_BF16_BYTES)) == 0);
	...
	for (byte=0u; byte<2u * SPARK_QWEN36_MODEL_HIDDEN_BF16_BYTES; byte++)
		assert(hidden_output[byte] == 0x5au);
