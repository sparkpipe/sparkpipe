...
int main(void)
{
    ...
    // RESTRICTED WALK: the config.h capability - a grammar or tool schema pays
    // a token SUBSET instead of the vocabulary. The table keeps row 0's full-
    // vocab winner (the restriction must not change a legal argmax) and drops
    // row 1's (the restriction must clip, not serve an out-of-set token). The
    // same Glm52Head the wrapper in unity.cu drives; only the candidate gather
    // sees the ids.
    {
        static uint32_t restricted[HEAD_VOCAB];
        uint32_t restricted_count = 0u;
        for (index = 0u; index < HEAD_VOCAB; ++index)
            if (index != output_token[1])
                restricted[restricted_count++] = index;
        status = Glm52Head(&buffers, head_norm_w, head_weight, restricted,
            restricted_count, ROWS, 0);
        printf("status restricted %d\n", (int)status);
        printf("restrictedcount %u\n", restricted_count);
        EmitU32("rtoken", output_token, ROWS);
        EmitF32("rscore", output_score, ROWS);
    }
