...
def main() -> int:
    ...
    mtp_extras = [record for record in final_stage if record.kind in (
                               pack.KIND_MTP_MAIN_PROJ, pack.KIND_MTP_MAIN_NORM,
    ...
    assert len(mtp_extras) == 9
    # The nine draft extras are stage-level globals bound once through
    # BindGlobal - never per-draft-layer records.
    assert all(record.layer == pack.GLOBAL_LAYER for record in mtp_extras)
