...
TOPOLOGY = ROOT / "include/sparkpipe/spark_hardware_topology.h"
...
def main() -> int:
    ...
    # The runtime device gate is parameterized through the shared topology
    # header so every driver refuses the same unqualified device; the
    # sm_121 branch of that header stays pinned to the qualified part.
    require(dsv4, "properties.major != SPARK_HW_COMPUTE_CAPABILITY_MAJOR",
            "runtime major check")
    require(dsv4, "properties.minor != SPARK_HW_COMPUTE_CAPABILITY_MINOR",
            "runtime minor check")
    require(topology, "#define SPARK_HW_COMPUTE_CAPABILITY_MAJOR 12u",
            "topology pins SM major 12")
    require(topology, "#define SPARK_HW_COMPUTE_CAPABILITY_MINOR 1u",
            "topology pins SM minor 1")
    ...
    # The certified B1 exact kernel now serves EVERY qualified bucket: the
    # native tiled kernels' per-tile scales diverged for rows>1 batches.
    # Both launchers keep the exact decode-bucket gate and the measured
    # per-SM block counts; W13 splits tile N at one row (cooperative 1-row
    # route build uses N32, the batched build N128).
    require(expert_host, "SparkLmSm121NativeDecodeShape(rows) == 0u",
            "routed W13 exact decode bucket gate")
    require(w2_host, "SparkLmSm121NativeDecodeShape(rows) == 0u",
            "routed W2 exact decode bucket gate")
    require(expert_host, "if ( rows == 1u )",
            "routed W13 tile-N split at one row")
    require(expert_host,
            "SparkLmSm121B1ExpertW13Kernel<SPARK_LM_SM121_B1_EXPERT_W13_TILE_N>",
            "true-B1 routed W13 exact kernel")
    require(expert_host,
            "SparkLmSm121B1ExpertW13Kernel<SPARK_LM_SM121_NATIVE_TILE_N>",
            "batched routed W13 exact kernel")
    require(w2_host,
            "SparkLmSm121B1ExpertW2Kernel<SPARK_LM_SM121_B1_EXPERT_W2_TILE_N>",
            "routed W2 exact kernel for every bucket")
    ...
    # Multi-row runs the certified 1-row GEMV once per row (base pointers
    # advanced by the row strides): bit-identical to B1, where the native
    # tiled kernel's per-TILE activation scales would diverge.
    require(dense_w13_dispatch,
            "SparkLmSm121FusedDenseW13GemvKernel<WEIGHT_BITS>",
            "shared W13 exact per-row GEMV")
    require(dense_w13_dispatch, "(uint64_t)row * input_dimension * 2u",
            "shared W13 per-row input stride")
    require(dense_w13_dispatch, "(uint64_t)row * output_dimension * 2u",
            "shared W13 per-row output stride")
    forbid(dense_w13_dispatch, "SparkLmSm121FusedDenseW13Kernel",
           "diverging per-tile-scale shared W13 route")
    require(dense_w13_dispatch,
            "SPARK_LM_SM121_B1_DENSE_W13_CTA_THREADS",
            "shared W13 measured launch geometry")
    require(dense_w13_dispatch,
            "SPARK_LM_SM121_B1_DENSE_W13_CTA_WARPS",
            "shared W13 measured grid geometry")
    ...
    require(dense_w13_gemv, "SPARK_LM_SM121_B1_DENSE_W13_CTA_WARPS",
            "shared W13 measured neuron geometry")
    ...
    # rows==1 takes the strided GEMV directly; multi-row relaunches the same
    # certified kernel once per row (the native MXFP8 batched route
    # quantizes per-K-block and diverges from the 1-row math).
    require(strided_dispatch, "if ( row_count == 1u )",
            "true-B1 strided dispatch")
    require(strided_dispatch, "SparkLmStridedLinearKernel",
            "B1 strided GEMV route")
    require(strided_dispatch,
            "(uint64_t)row * input_row_stride * 2u",
            "multi-row exact strided input stride")
    require(strided_dispatch,
            "(uint64_t)row * output_row_stride * 2u",
            "multi-row exact strided output stride")
    forbid(strided_dispatch, "SparkLmHostLaunchSm121NativeLinear",
           "diverging per-K-block native strided route")
    require(strided_dispatch,
    ...
    # Every row takes the certified 1-row head: the screened-argmax route
    # diverges from the certified exact rescore on near-tie argmaxes, so
    # production relaunches the sharded certified kernel once per row.
    require(project_head,
            "for (row = 0u; row < rows && error == cudaSuccess; row++)",
            "certified 1-row head launched per row")
    require(project_head,
            "(uint64_t)row * SPARK_DSV4_MODEL_HIDDEN_DIMENSION *",
            "per-row hidden stride into the certified head")
    ...
    require(project_head, "state->head_certified_fp8_norm_f32,",
            "dedicated certified norm argument")
    forbid(project_head, "SparkDsv4LaunchHeadScreenedArgmax(",
           "diverging screened production head")
