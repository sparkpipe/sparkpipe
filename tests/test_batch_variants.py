...
per-bucket constant lives in the family's spark_<model>_batch_tuning.h.
The ladder is the power-of-two rungs plus each family's extra rungs (dsv4
carries the three DSpark spec-step buckets b6/b9/b11). A
...
POWER_OF_TWO_BUCKETS = (1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024)
# The DSpark spec-step buckets: SPEC_STEP+1 for k=5/8/10 -> b6/b9/b11.
DSV4_EXTRA_BUCKETS = (6, 9, 11)
# Each family publishes its own rung set; dsv4 is the only family whose
# ladder carries the spec-step buckets.
FAMILY_BUCKETS = {
    "glm52": POWER_OF_TWO_BUCKETS,
    "k3": POWER_OF_TWO_BUCKETS,
    "dsv4": tuple(sorted(POWER_OF_TWO_BUCKETS + DSV4_EXTRA_BUCKETS)),
}
ALL_BUCKETS = tuple(sorted(
    {bucket for buckets in FAMILY_BUCKETS.values() for bucket in buckets}))
...
EXPECTED_TILE_M = {bucket: 16 for bucket in ALL_BUCKETS}
...
def check_family_makefile(path, family):
    ...
    expected_list = " ".join(
        str(bucket) for bucket in FAMILY_BUCKETS[family])
...
def check_tuning_header(path, family, id_prefix, id_suffix):
    ...
    text = open(path).read()
    upper = family.upper()
    family_buckets = FAMILY_BUCKETS[family]
    ...
    if guard is None or any(
        ...
        report("bucket guard", rel,
               f"the #error guard must name all {len(family_buckets)} buckets")
    # Spelled once each; whether the string rides a continuation line or the
    # definition line itself is house style, not contract.
    prefix_defs = re.findall(
        rf"^#define SPARK_{upper}_BATCH_VARIANT_MODULE_ID_PREFIX\b",
        text, re.M)
    suffix_defs = re.findall(
        rf"^#define SPARK_{upper}_BATCH_VARIANT_MODULE_ID_SUFFIX\b",
        text, re.M)
...
// The family's model/firmware headers first: each tuning header picks up
// its geometry by the module's model-header-first pattern.
{FAMILY_INCLUDES}
                          ...
                          const char *prefix, const char *suffix)
{
    ...
    // rung in the ladder silently serves a microbatch under a ceiling above
    // its size - the pool waste the bucket ladder exists to remove.
    ...
    {
        const uint32_t buckets[] = {BUILT_BUCKETS};
        for (index = 0u; index < sizeof(buckets) / sizeof(buckets[0]);
             ++index)
        {
...
# Per-family probe wiring: which headers to include (model-header-first),
# what the compiled bucket must size, and which ceiling/ID pair to drive.
FAMILY_SPECS = {
    "glm52": {
        "includes": (
        ...
        "main_body": """\
    // The firmware capacity ceiling IS the compiled bucket: this is what
    ...
    "k3": {
        "includes": '#include "sparkpipe/spark_k3_batch_tuning.h"',
        "main_body": """\
...
def probe_source(bucket, family):
    source = PROBE_TEMPLATE
    source = source.replace("{FAMILY_INCLUDES}",
                            FAMILY_SPECS[family]["includes"])
    source = source.replace("{FAMILY_MAIN_BODY}",
                            FAMILY_SPECS[family]["main_body"])
    source = source.replace("BUILT_BUCKETS",
                            ", ".join(f"{rung}u" for rung
                                      in FAMILY_BUCKETS[family]))
...
def check_selection_contract():
    ...
    with tempfile.TemporaryDirectory() as scratch:
        for family in ("glm52", "k3", "dsv4"):
            for bucket in FAMILY_BUCKETS[family]:
                source_path = os.path.join(
                    scratch, f"probe_{family}_b{bucket}.c")
                binary_path = os.path.join(
                    scratch, f"probe_{family}_b{bucket}")
                with open(source_path, "w") as handle:
                    handle.write(probe_source(bucket, family))
                compile_command = [
                ...
                built = subprocess.run(
                    compile_command, cwd=ROOT, capture_output=True,
                    text=True)
                if built.returncode != 0:
                    report("probe compile", f"{family} b{bucket}",
                           built.stderr.strip().splitlines()[-1]
                           if built.stderr else "")
                    continue
                run = subprocess.run([binary_path], capture_output=True)
                if run.returncode != 0:
                    report("probe run", f"{family} b{bucket}",
        ...
        with open(rogue_source, "w") as handle:
            handle.write(probe_source(8, "k3"))
        built = subprocess.run(
...
def main():
    ...
    print("glm52 + dsv4 + k3 batch variants: per-family ladders; dsv4 adds "
          "the DSpark spec-step rungs b6/b9/b11")
    if FAILURES:
