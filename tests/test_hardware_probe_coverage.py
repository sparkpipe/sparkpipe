...
def validate_single_registry() -> None:
    ...
    for path in ROOT.rglob("*.json"):
        # Only the maintained tree may carry a registry: skip build
        # output, caches, session scratch (tmp/), and hidden worktree
        # copies (.agents/, .git/) wholesale.
        if (any(part in {"build", "__pycache__", "tmp"} for part in path.parts)
