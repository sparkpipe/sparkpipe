...
int main(void)
{
	...
	/* The topology-parameterization wave admitted the TP16/TP32/TP64
	 * ladders alongside the original single-rank, TP8 and saturated
	 * 1024 widths; everything else stays refused. */
	...
	assert(SparkDsv4ResidentDecodeStageNativeTpWidthSupported(3u) == 0u);
	assert(SparkDsv4ResidentDecodeStageNativeTpWidthSupported(12u) == 0u);
	assert(SparkDsv4ResidentDecodeStageNativeTpWidthSupported(128u) == 0u);
	return(0);
}
