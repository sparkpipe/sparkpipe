...
WAVE = "G5N-WAVE-TIMING rank=3 waves=3 rows=24 prefill=1 graph=2 eager=1 linear=1 graph_path=1 retries=2 busy=1/1/0/0/0 captures=1 capture_ms=250 idle_us=8388608/8388608 wait_us=128/4096 key_us=512/512 setup_us=1024/262144 run_us=65536/131072 post_us=512/512 idle_ms=10593 wait_ms=3 key_ms=0 setup_ms=252 run_ms=210 post_ms=1 graph_run_ms=150 eager_run_ms=60 linear_run_ms=60 decode_wait_ms=3 source_wait_ms=3 peer_wait_ms=60 copy_ms=6 combine_ms=9 worst_ms=311 worst_request=7 worst_epochs=11/12 worst_us=0/100/300/251000/60000/500"
WAVE_BEFORE_LINEAR = WAVE.replace("rank=3", "rank=4").replace(" linear=1", "").replace(" linear_run_ms=60", "")
...
def main():
    ...
    with tempfile.TemporaryDirectory() as directory:
        mesh0, mesh1, wave, older = Path(directory) / "w0.log", Path(directory) / "w1.log", Path(directory) / "r3.log", Path(directory) / "r4.log"
        ...
        output = run("wave_timeline_report.py", f"3={wave}", f"4={older}")
        lines = [line.split() for line in output.splitlines()]
        check("3 1 3 8.0 33.3 66.7 33.3 on 0.7 1/1/0/0/0 | 3531.0 1.0 0.0 84.0 70.0 0.3 | 20.0 1.0 2.0 3.0 44.0 | 75.0 0.0 60.0 1.5 1 250 | 4096 131072".split() in lines,
              "the per-wave budget splits a wave into contiguous intervals, graph, eager and linear runs, and busy reasons", output)
        check("4 1 3 8.0 33.3 66.7 0.0 on 0.7 1/1/0/0/0 | 3531.0 1.0 0.0 84.0 70.0 0.3 | 20.0 1.0 2.0 3.0 44.0 | 75.0 60.0 0.0 1.5 1 250 | 4096 131072".split() in lines,
