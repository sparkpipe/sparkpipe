	...
	SparkModelDriverFrame *frame)
{
	...
	if ( (context->flags & SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_SPECULATIVE_VERIFY) != 0u )
	{
		const uint32_t *verify_in = (const uint32_t *)frame->buffers[0].address;
		uint32_t out_count = (uint32_t)(frame->buffers[1].bytes / sizeof(uint32_t));
		const char *capture = getenv("SPARK_QWEN38_27B_TEST_VERIFY_CAPTURE");
		if ( capture != 0 )
		{
			FILE *file = fopen(capture,"a");
			if ( file != 0 )
			{
				fprintf(file,"%u",rows);
				for (row=0u; row<rows; row++)
					fprintf(file," %u",verify_in[row]);
				fprintf(file,"\n");
				fclose(file);
			}
		}
		if ( driver->verify_count == 0u )
		{
			tokens[0] = verify_in[1];
			for (row=1u; row<out_count; row++)
				tokens[row] = 9998u + (row - 1u);
		}
		else
		{
			for (row=0u; row + 1u < out_count; row++)
				tokens[row] = verify_in[row + 1u];
			tokens[out_count - 1u] = 9999u;
		}
		driver->verify_count++;
	}
	else if ( (context->flags & SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_GDN_RESTORE_FIRST) != 0u )
		tokens[0] = 8888u;
	else if ( prefill != 0u )
		tokens[0] = 4242u;
	else
		for (row=0u; row<rows; row++)
			tokens[row] = 5000u + row;
