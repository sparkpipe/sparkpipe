...
#define TEST_GEMMA4_DRIVER_PROGRAM_ID 1u

typedef struct TestGemma4ServingDriver
{
...
} TestGemma4ServingDriver;

static SparkStatus TestGemma4ServingDriverSubmit(
	void *driver_instance,
	SparkModelDriverFrame *frame);

static const SparkModelDriverProgramProfile TestGemma4ServingDriverProfile =
{
	...
	.max_inflight = SPARK_GEMMA4_RESIDENT_DECODE_STAGE_MAX_PIPELINE_SLOT_COUNT,
	.max_active_slots = SPARK_GEMMA4_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT,
	.max_new_tokens = SPARK_GEMMA4_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT,
	.max_resident_sequences = SPARK_GEMMA4_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT,
	.max_sequence_tokens = SPARK_GEMMA4_MODEL_MAXIMUM_CONTEXT_TOKENS
};

static const SparkModelDriverProgramDescriptor TestGemma4ServingDriverProgram =
{
	.program_id = TEST_GEMMA4_DRIVER_PROGRAM_ID,
	...
	.profile = &TestGemma4ServingDriverProfile,
	.submit = TestGemma4ServingDriverSubmit
};

static const SparkModelDriverDescriptor TestGemma4ServingDriverDescriptor =
{
	.abi_version = SPARK_MODEL_DRIVER_ABI_VERSION,
	.descriptor_bytes = sizeof(SparkModelDriverDescriptor),
	.model_id = SPARK_GEMMA4_MODEL_DRIVER_MODEL_ID,
	...
	.programs = &TestGemma4ServingDriverProgram
};

static uint32_t TestGemma4ServingDriverEnvironmentUnsigned(
	const char *name,
	uint32_t *value)
{
	const char *text;
...
static SparkStatus TestGemma4ServingDriverCreate(
	const SparkModelDriverCreateRequest *request,
	void **driver_instance)
{
	TestGemma4ServingDriver *driver;
	...
	if ( TestGemma4ServingDriverEnvironmentUnsigned("SPARK_GEMMA4_STAGE_COUNT",&stage_count) == 0u || stage_count == 0u || TestGemma4ServingDriverEnvironmentUnsigned("SPARK_GEMMA4_STAGE_INDEX",&stage_index) == 0u || stage_index >= stage_count || TestGemma4ServingDriverEnvironmentUnsigned("SPARK_GEMMA4_STAGE_KV_BLOCKS",&kv_blocks) == 0u || kv_blocks == 0u || TestGemma4ServingDriverEnvironmentUnsigned("SPARK_GEMMA4_STAGE_PIPELINE_SLOTS",&pipeline_slots) == 0u || pipeline_slots == 0u )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	driver = (TestGemma4ServingDriver *)calloc(1u,sizeof(*driver));
	if ( driver == 0 )
...
static void TestGemma4ServingDriverDestroy(void *driver_instance)
{
	TestGemma4ServingDriver *driver;
	driver = (TestGemma4ServingDriver *)driver_instance;
	if ( driver == 0 )
		return;
	free(driver);
}

static SparkStatus TestGemma4ServingDriverAdmit(
	void *driver_instance,
	...
	SparkModelDriverAdmissionDecision *decision)
{
	...
	decision->available_dispatch_slot_count = SPARK_GEMMA4_RESIDENT_DECODE_STAGE_MAX_PIPELINE_SLOT_COUNT;
	return(SPARK_STATUS_OK);
}
	...
	SparkModelDriverFrame *frame)
{
	...
	if ( context->abi_version != SPARK_GEMMA4_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_ABI_VERSION || context->descriptor_bytes < (uint32_t)sizeof(*context) )
		return(SPARK_STATUS_ABI_MISMATCH);
	prefill = (frame->flags & SPARK_MODEL_DRIVER_FRAME_FLAG_PREFILL) != 0u ? 1u : 0u;
	if ( prefill != 0u )
	{
		if ( (context->flags & SPARK_GEMMA4_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_PREFILL_FRAME_VIEW) == 0u || context->prefill_frame == 0 || context->decode_batch != 0 || frame->active_slot_count != 1u )
			return(SPARK_STATUS_INVALID_ARGUMENT);
		rows = context->prefill_frame->token_count;
	}
	else
	{
		if ( (context->flags & SPARK_GEMMA4_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_DECODE_BATCH_VIEW) == 0u || context->decode_batch == 0 || context->prefill_frame != 0 || frame->active_slot_count != frame->new_token_count )
			return(SPARK_STATUS_INVALID_ARGUMENT);
		rows = context->decode_batch->row_count;
	}
	if ( rows == 0u || rows > TEST_GEMMA4_DRIVER_CAPTURE_ROWS || rows != frame->new_token_count )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	if ( (context->flags & SPARK_GEMMA4_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_KV_BLOCK_TABLE) == 0u || context->kv_block_table == 0 || context->kv_block_table->physical_block_indices == 0 || context->kv_block_table->host_physical_block_indices == 0 || context->kv_block_table->host_lane_physical_block_counts == 0 )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	if ( (context->flags & (SPARK_GEMMA4_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_HIDDEN_INPUT_TRANSPORT | SPARK_GEMMA4_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_HIDDEN_OUTPUT_TRANSPORT)) != 0u || context->hidden_input_post_receive_function != 0 || context->hidden_output_send_function != 0 )
		return(SPARK_STATUS_INVALID_ARGUMENT);
...
static SparkStatus TestGemma4ServingDriverSnapshot(
	void *driver_instance,
	uint32_t program_id,
	SparkModelDriverRuntimeSnapshot *snapshot)
{
	TestGemma4ServingDriver *driver;
	if ( driver_instance == 0 || program_id != TEST_GEMMA4_DRIVER_PROGRAM_ID || snapshot == 0 )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	driver = (TestGemma4ServingDriver *)driver_instance;
	memset(snapshot,0,sizeof(*snapshot));
	snapshot->descriptor_bytes = sizeof(*snapshot);
	snapshot->program_id = program_id;
	snapshot->available_dispatch_slot_count = SPARK_GEMMA4_RESIDENT_DECODE_STAGE_MAX_PIPELINE_SLOT_COUNT;
	snapshot->submitted_count = driver->submitted_count;
	snapshot->completed_count = driver->completed_count;
	snapshot->kv_token_capacity = (uint64_t)driver->kv_block_count * SPARK_GEMMA4_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS;
	return(SPARK_STATUS_OK);
}

static const SparkModelDriverInterface TestGemma4ServingDriverInterface =
{
	.abi_version = SPARK_MODEL_DRIVER_ABI_VERSION,
	.interface_bytes = sizeof(SparkModelDriverInterface),
	.descriptor = &TestGemma4ServingDriverDescriptor,
	.create = TestGemma4ServingDriverCreate,
	.destroy = TestGemma4ServingDriverDestroy,
	.admit = TestGemma4ServingDriverAdmit,
	.snapshot = TestGemma4ServingDriverSnapshot
};

__attribute__((visibility("default")))
const SparkModelDriverInterface *SparkModelDriverGetInterface(void)
{
	return(&TestGemma4ServingDriverInterface);
}
