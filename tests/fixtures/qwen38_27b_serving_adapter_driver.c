 ...
 * (SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS = 64 rows), and
 ...
 * byte-identically. */
#define TEST_QWEN38_27B_DRIVER_CAPTURE_ROWS 64u
#define TEST_QWEN38_27B_DRIVER_RECORD_CAPACITY 8192u

/* One submitted frame, as the prefix gate observes it. */
typedef struct TestQwen38_27bDriverFrameRecord
{
	uint32_t prefill;
	uint32_t rows;
	...
	uint32_t token_ids[TEST_QWEN38_27B_DRIVER_CAPTURE_ROWS];
	uint32_t blocks[TEST_QWEN38_27B_DRIVER_CAPTURE_ROWS];
}
TestQwen38_27bDriverFrameRecord;

static TestQwen38_27bDriverFrameRecord TestQwen38_27bServingDriverRecords[TEST_QWEN38_27B_DRIVER_RECORD_CAPACITY];
static uint32_t TestQwen38_27bServingDriverRecordHead;
/* The KV pool size this driver instance was created with (the adapter's
 * SPARK_QWEN38_27B_STAGE_KV_BLOCKS), for the deployment-wiring assertions. */
static uint32_t TestQwen38_27bServingDriverKvBlocks;

__attribute__((visibility("default")))
uint32_t TestQwen38_27bServingDriverKvBlockCount(void)
{
	return(TestQwen38_27bServingDriverKvBlocks);
}

__attribute__((visibility("default")))
uint32_t TestQwen38_27bServingDriverRecordCount(void)
{
	return(TestQwen38_27bServingDriverRecordHead);
}

__attribute__((visibility("default")))
const TestQwen38_27bDriverFrameRecord *TestQwen38_27bServingDriverRecord(
	uint32_t index)
{
	if ( index >= TestQwen38_27bServingDriverRecordHead )
		return(0);
	return(&TestQwen38_27bServingDriverRecords[index]);
}

__attribute__((visibility("default")))
void TestQwen38_27bServingDriverResetRecords(void)
{
	TestQwen38_27bServingDriverRecordHead = 0u;
	memset(TestQwen38_27bServingDriverRecords,0,
		sizeof(TestQwen38_27bServingDriverRecords));
}
	...
	void **driver_instance)
{
	...
	TestQwen38_27bServingDriverKvBlocks = kv_blocks;
	...
	SparkModelDriverFrame *frame)
{
	...
	else
	{
		...
		if ( (context->flags & SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_DECODE_BATCH_VIEW) == 0u || context->decode_batch == 0 || context->prefill_frame != 0 )
			return(SPARK_STATUS_INVALID_ARGUMENT);
	...
	if ( TestQwen38_27bServingDriverRecordHead < TEST_QWEN38_27B_DRIVER_RECORD_CAPACITY )
	{
		TestQwen38_27bDriverFrameRecord *record =
			&TestQwen38_27bServingDriverRecords[TestQwen38_27bServingDriverRecordHead++];
		const uint32_t *input_ids = (const uint32_t *)frame->buffers[0].address;
		const SparkQwen38_27bKvBlockTableView *table = context->kv_block_table;
		...
		for (row=0u; row<rows && row<TEST_QWEN38_27B_DRIVER_CAPTURE_ROWS; row++)
			...
			lane_index < table->lane_count )
		{
			...
			for (ordinal=0u; ordinal<record->block_count &&
				ordinal<TEST_QWEN38_27B_DRIVER_CAPTURE_ROWS; ordinal++)
