...
#define SPARK_QWEN38_27B_STAGEPACK_CLASS_GLOBAL SPARK_HYBRID_STAGEPACK_CLASS_GLOBAL
#define SPARK_QWEN38_27B_STAGEPACK_CLASS_EVERY_LAYER SPARK_HYBRID_STAGEPACK_CLASS_EVERY_LAYER
#define SPARK_QWEN38_27B_STAGEPACK_CLASS_GDN_LAYER SPARK_HYBRID_STAGEPACK_CLASS_GDN_LAYER
#define SPARK_QWEN38_27B_STAGEPACK_CLASS_ATTN_LAYER SPARK_HYBRID_STAGEPACK_CLASS_ATTN_LAYER
...
_Static_assert(sizeof(SparkQwen38_27bStagePackHeader) == SPARK_QWEN38_27B_STAGEPACK_HEADER_BYTES,"qwen36 stage pack header must be 120 wire bytes");
_Static_assert(sizeof(SparkQwen38_27bStagePackEntry) == SPARK_QWEN38_27B_STAGEPACK_ENTRY_BYTES,"qwen36 stage pack directory entry must be 56 wire bytes");
...
_Static_assert(SPARK_QWEN38_27B_MODEL_GDN_LAYER_COUNT + SPARK_QWEN38_27B_MODEL_FULL_ATTENTION_LAYER_COUNT == SPARK_QWEN38_27B_MODEL_LAYER_COUNT,"qwen36 layer split must cover the stack");
_Static_assert((SPARK_QWEN38_27B_MODEL_LAYER_COUNT % SPARK_QWEN38_27B_MODEL_ATTENTION_PERIOD) == 0u,"qwen36 layer count must be whole periods");
_Static_assert(SPARK_QWEN38_27B_MODEL_GDN_LAYER_COUNT == (SPARK_QWEN38_27B_MODEL_LAYER_COUNT / SPARK_QWEN38_27B_MODEL_ATTENTION_PERIOD) * (SPARK_QWEN38_27B_MODEL_ATTENTION_PERIOD - 1u),"qwen36 gdn count must match the 3:1 period");
_Static_assert((SPARK_QWEN38_27B_MODEL_GDN_VALUE_HEAD_COUNT % SPARK_QWEN38_27B_MODEL_GDN_KEY_HEAD_COUNT) == 0u,"qwen36 value heads must group evenly onto key heads");
_Static_assert(SPARK_QWEN38_27B_MODEL_GDN_VALUE_HEADS_PER_KEY_HEAD == 3u,"qwen36 grouped-value ratio is three per config");
_Static_assert((SPARK_QWEN38_27B_MODEL_ATTN_QUERY_HEAD_COUNT % SPARK_QWEN38_27B_MODEL_ATTN_KV_HEAD_COUNT) == 0u,"qwen36 query heads must group evenly onto kv heads");
_Static_assert(SPARK_QWEN38_27B_MODEL_ATTN_ROPE_DIMENSION == SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION / 4u,"qwen36 rope covers a quarter of the head");
_Static_assert((SPARK_QWEN38_27B_MODEL_ATTN_ROPE_DIMENSION % 2u) == 0u,"qwen36 rope dimension must pair");
_Static_assert(SPARK_QWEN38_27B_MODEL_GDN_CONV_CHANNELS == 10240u,"qwen36 conv width is q+k+v concatenated");
_Static_assert(SPARK_QWEN38_27B_MODEL_GDN_QK_DIMENSION == 2048u && SPARK_QWEN38_27B_MODEL_GDN_VALUE_DIMENSION == 6144u,"qwen36 gdn projection widths per config");
_Static_assert(SPARK_QWEN38_27B_MODEL_ATTN_QUERY_DIMENSION == 6144u && SPARK_QWEN38_27B_MODEL_ATTN_KV_DIMENSION == 1024u,"qwen36 attention projection widths per config");
_Static_assert((SPARK_QWEN38_27B_MODEL_GDN_CHUNK_TOKENS % 16u) == 0u,"qwen36 chunk must tile for wmma");
...
{
	return(SparkHybridStagePackFullAttentionLayersBelow(SPARK_QWEN38_27B_MODEL_ATTENTION_PERIOD,layer_count));
}
...
#define SPARK_QWEN38_27B_STAGEPACK_COMPARE_U32_FIELDS \
	((sizeof(SparkQwen38_27bStagePackHeader) - 2u * sizeof(uint64_t)) / sizeof(uint32_t))
...
{
	return(SparkHybridStagePackHeaderFieldsMatch(file_header,expected,(uint32_t)SPARK_QWEN38_27B_STAGEPACK_COMPARE_U32_FIELDS));
}
...
static inline int32_t SparkQwen38_27bStagePackResolvedShape(uint32_t tensor_kind, uint32_t layer_index, uint32_t is_global, uint32_t tp_degree, SparkQwen38_27bStagePackTensorShape *shape)
{
	...
	return(SparkHybridStagePackResolveLayerClass(shape->layer_class,is_global,
		layer_index == SPARK_QWEN38_27B_STAGEPACK_MTP_LAYER ? 1u : 0u,
		layer_index,SPARK_QWEN38_27B_MODEL_LAYER_COUNT,
		SPARK_QWEN38_27B_MODEL_LAYER_IS_GDN(layer_index)));
}
...
static inline uint32_t SparkQwen38_27bStagePackWeightClass(uint32_t weight_format)
{
	...
	if ( weight_format == SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_F32 )
		return(SPARK_HYBRID_STAGEPACK_WEIGHT_F32);
	if ( weight_format == SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_U32 )
...
{
	return(SparkHybridStagePackPayloadBytes(SparkQwen38_27bStagePackWeightClass(weight_format),rows,columns));
}

static inline uint64_t SparkQwen38_27bStagePackScaleBytes(uint32_t weight_format, uint32_t rows, uint32_t columns)
{
	return(SparkHybridStagePackScaleBytes(SparkQwen38_27bStagePackWeightClass(weight_format),rows,columns));
}
