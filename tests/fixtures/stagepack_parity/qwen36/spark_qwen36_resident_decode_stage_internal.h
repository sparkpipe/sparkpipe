/*
 * Frozen reference revision, qwen36 stage-pack contract (cleanup round
 * 2026-08-24): the last blessed state of the standalone
 * spark_qwen36_stagepack_format.h, renamed under the single include name the
 * parity driver now uses after the fold into the family internal header.
 * Any drift in accepted/received/rejected streams trips
 * tests/test_stagepack_parity.py.
 */
#pragma once
