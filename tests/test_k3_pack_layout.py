...
def mini_checkpoint(root, latent=None, poison_scale=False, layers=None,
                    layer_types=None, dense_layers=()):
    """The synthetic checkpoint the packer consumes. `layers`/`layer_types`
    parameterize the depth and the per-layer kind map (the K-F2 bind
    fixture drives a 93-layer backbone with the canonical rule: MLA at
    period positions 3,7,... AND the trailing layer 92) - defaults keep the
    original 3-layer mixed-kind fixture byte-for-byte. `dense_layers`
    names layer indices that carry the dense mlp.{gate,up,down}_proj MLP
    instead of block_sparse_moe - the packer picks dense vs routed by name
    presence (tools/k3_pack.py), and the K-F2 bind fixture needs a true
    dense layer 0."""
    g = dict(MINI)
    ...
    config = {"hidden_size": g["hidden"], "num_hidden_layers": layers,
    ...
    for layer, kind in enumerate(config["layer_types"]):
        ...
        if layer in dense_layers:
            # the dense replacement: one plain MLP under "mlp.", no router,
            # no experts - exactly the naming tools/k3_pack.py switches on
            t[p + "mlp.gate_proj.weight"] = (
                "BF16", (g["inter"], hidden),
                rand_bf16(g["inter"] * hidden))
            t[p + "mlp.up_proj.weight"] = (
                "BF16", (g["inter"], hidden),
                rand_bf16(g["inter"] * hidden))
            t[p + "mlp.down_proj.weight"] = (
                "BF16", (hidden, g["inter"]),
                rand_bf16(hidden * g["inter"]))
            continue
