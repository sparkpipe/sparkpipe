...
#ifndef TEST_LING_SERVING_ADAPTER_PATH
#define TEST_LING_SERVING_ADAPTER_PATH ""
#endif
...
#endif
#ifndef TEST_LING_MODEL_REVISION
#define TEST_LING_MODEL_REVISION ""
#endif

#define TEST_LING_MANDATORY_MEMBER_COUNT 10u

typedef struct TestLingServingState
{
	uint32_t completion_count;
	void *execution_stream;
	SparkModelServingCompletion completion;
} TestLingServingState;

static void TestLingServingCompletion(
	void *completion_context,
	const SparkModelServingCompletion *completion)
{
	TestLingServingState *state;
	state = (TestLingServingState *)completion_context;
	assert(state != 0);
	assert(completion != 0);
...
static void TestLingServingInterfaceCompleteness(
	const SparkModelServingAdapterInterface *adapter_interface)
{
	...
	assert(member_count == TEST_LING_MANDATORY_MEMBER_COUNT);
...
static void TestLingServingConfiguration(
	...
	TestLingServingState *test_state)
{
	...
	configuration->node_target = "cuda.sm121.ling.resident_decode_stage.bf16.expert_bf16";
...
int main(void)
{
	...
	assert(library.adapter_interface.descriptor->cache_block_token_count ==
		SPARK_LING_KV_BLOCK_TOKEN_COUNT);
	...
	TestLingServingDecodeSubmission(&submission,lanes,token_ids,
