...
#ifndef TEST_GEMMA4_SERVING_ADAPTER_PATH
#define TEST_GEMMA4_SERVING_ADAPTER_PATH ""
#endif
#ifndef TEST_GEMMA4_MOE_SERVING_ADAPTER_PATH
#define TEST_GEMMA4_MOE_SERVING_ADAPTER_PATH ""
#endif
#ifndef TEST_GEMMA4_SERVING_DRIVER_PATH
#define TEST_GEMMA4_SERVING_DRIVER_PATH ""
#endif
#ifndef TEST_GEMMA4_SERVING_CONFIG_PATH
#define TEST_GEMMA4_SERVING_CONFIG_PATH ""
#endif
#ifndef TEST_GEMMA4_MODEL_REVISION
#define TEST_GEMMA4_MODEL_REVISION ""
#endif
...
typedef struct TestGemma4ServingState
{
	uint32_t completion_count;
	void *execution_stream;
	SparkModelServingCompletion completion;
} TestGemma4ServingState;

static void TestGemma4ServingCompletion(
	void *completion_context,
	const SparkModelServingCompletion *completion)
{
	TestGemma4ServingState *state;
	state = (TestGemma4ServingState *)completion_context;
	assert(state != 0);
	assert(completion != 0);
...
static void TestGemma4ServingInterfaceCompleteness(
	const SparkModelServingAdapterInterface *adapter_interface)
{
	...
	assert(member_count == TEST_GEMMA4_MANDATORY_MEMBER_COUNT);
...
static void TestGemma4ServingMoeReceipt(
	const SparkModelServingAdapterInterface *adapter_interface)
{
	assert(strcmp(adapter_interface->descriptor->adapter_id,
		"spark.gemma4.serving-adapter.tp4pp4.v1") == 0);
	assert(strcmp(adapter_interface->descriptor->model_id,
		"google/gemma-4-26B-A4B-it") == 0);
	assert(strcmp(adapter_interface->descriptor->model_revision,
		TEST_GEMMA4_MOE_MODEL_REVISION) == 0);
	assert(adapter_interface->descriptor->capability_flags ==
		(SPARK_MODEL_SERVING_ADAPTER_CAPABILITY_HIDDEN_TRANSPORT |
		 SPARK_MODEL_SERVING_ADAPTER_CAPABILITY_PARALLEL_FANOUT |
		 SPARK_MODEL_SERVING_ADAPTER_CAPABILITY_HYBRID_TP_PP));
	assert(adapter_interface->descriptor->layer_count == 30u);
	assert(adapter_interface->descriptor->stage_count == 16u);
	assert(adapter_interface->descriptor->parallel_group_size == 4u);
	assert(adapter_interface->descriptor->cache_block_token_count ==
		SPARK_GEMMA4_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS);
	assert(adapter_interface->descriptor->stage_layer_counts[0] == 8u);
	assert(adapter_interface->descriptor->stage_layer_counts[4] == 8u);
	assert(adapter_interface->descriptor->stage_layer_counts[7] == 8u);
	assert(adapter_interface->descriptor->stage_layer_counts[8] == 7u);
	assert(adapter_interface->descriptor->stage_layer_counts[12] == 7u);
	assert(adapter_interface->descriptor->stage_layer_counts[15] == 7u);
	TestGemma4ServingInterfaceCompleteness(adapter_interface);
}

static void TestGemma4ServingConfiguration(
	SparkModelServingAdapterConfiguration *configuration,
	uint32_t stage_index,
	const char *config_path,
	const char *runtime_root,
	const char *driver_path,
	TestGemma4ServingState *test_state)
{
	...
	configuration->node_target = SPARK_GEMMA4_MODEL_MODULE_TARGET;
	...
	configuration->completion_function = TestGemma4ServingCompletion;
	configuration->completion_context = test_state;
}

static void TestGemma4ServingDecodeSubmission(
...
int main(void)
{
	SparkModelServingAdapterDynamicLibrary moe_library;
	...
	assert(SparkModelServingAdapterLoadInterfaceFromSharedObject(
		TEST_GEMMA4_MOE_SERVING_ADAPTER_PATH,
		SPARK_MODEL_SERVING_ADAPTER_CAPABILITY_HIDDEN_TRANSPORT,
		&moe_library) == SPARK_STATUS_OK);
	TestGemma4ServingMoeReceipt(&moe_library.adapter_interface);
	SparkModelServingAdapterUnloadInterface(&moe_library);
	assert(SparkModelServingAdapterLoadInterfaceFromSharedObject(
		TEST_GEMMA4_SERVING_ADAPTER_PATH,
		SPARK_MODEL_SERVING_ADAPTER_CAPABILITY_HIDDEN_TRANSPORT,
	...
	TestGemma4ServingConfiguration(&configuration,0u,
		TEST_GEMMA4_SERVING_CONFIG_PATH,runtime_root,
		TEST_GEMMA4_SERVING_DRIVER_PATH,&test_state);
	adapter_state = 0;
	...
	TestGemma4ServingDecodeSubmission(&submission,lanes,token_ids,
