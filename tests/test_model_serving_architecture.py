...
def main() -> int:
    ...
    # The adapter audit moved generic per-row capture and driver-create
    # request assembly into the shared adapter layer; pins follow them.
    adapter_common = (
        ROOT / "runtime/adapter_common.c"
    ).read_text(encoding="utf-8")
    model_header = (
        ROOT
    ...
    require(
        "SparkAdapterCaptureResidentSlotsPerRow(submission," in adapter
        and "submission->lanes[lane].resident_sequence_slot" in adapter_common,
        "DSV4 adapter does not translate batch lanes to persistent KV slots",
    )
    require(
        "request.kv_logical_page_capacity =" in adapter_common
        and "request.kv_physical_page_capacity =" in adapter_common
    ...
    require(
        ...
        and "common->quiescing = 1u" in adapter_common
        and "snapshot.active_submission_count == 0u" in adapter_common,
