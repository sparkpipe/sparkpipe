...
int main(void)
{
	...
	printf("\nshared memory is dynamic, so it may exceed the static cap\n");
	...
	expect(plan.shared_bytes > LM_SMEM_STATIC_LIMIT, "and needs more than the static limit allows");
	...
	/* Stage-module accumulate launches: the exported geometry and argument
	 * policy every family's TP combine launchers route through. The values
	 * pin what the private kernels already assumed; a change here is a
	 * cross-family launch-shape change and needs the decision memo. */
	printf("\nstage-module accumulate geometry\n");
	expect(LmLaunchAccumGridRows(1u) == 1u &&
		LmLaunchAccumGridRows(1024u) == 1024u,
		"row grid is one CTA per row");
	expect(LmLaunchAccumGridElements(0u) == 0u &&
		LmLaunchAccumGridElements(1u) == 1u &&
		LmLaunchAccumGridElements(257u) == 2u,
		"element grid covers the partial tail block");
	expect(LmLaunchAccumGridElements(7168ull * 1024ull) ==
		(uint32_t)(7168ull * 1024ull + 255u) / 256u,
		"b1024 hidden-7168 element grid matches the literal ceil-div");
	expect(LmLaunchAccumWidthPolicy(0u) != LM_LAUNCH_OK &&
		LmLaunchAccumWidthPolicy(4095u) != LM_LAUNCH_OK,
		"width policy refuses zero and odd widths");
	expect(LmLaunchAccumWidthPolicy(4096u) == LM_LAUNCH_OK &&
		LmLaunchAccumWidthPolicy(7168u) == LM_LAUNCH_OK,
		"width policy admits the even hidden dimensions in service");
