 ...
 * and the outputs must be byte-identical. Walks SparkQwen38MaxModuleValidateEntry's
	...
	uint64_t payload_offset,uint64_t scale_offset,uint64_t file_bytes)
{
	...
	uint32_t global = layer_index == SPARK_QWEN38_MAX_STAGEPACK_GLOBAL_LAYER ? 1u : 0u;
	memset(&shape,0,sizeof(shape));
	if ( SparkQwen38MaxStagePackResolvedShape(tensor_kind,global != 0u ? 0u : layer_index,global,&shape) != 0 || rows != shape.rows || columns != shape.columns )
		return(1);
	/* Strict natural format, except the three routed-expert tensors may also arrive BF16. */
	if ( weight_format != shape.natural_format )
	{
		if ( (shape.natural_format != SPARK_QWEN38_MAX_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_MXFP4_E2M1 && shape.natural_format != SPARK_QWEN38_MAX_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_FP8_E4M3_F32B128) || weight_format != SPARK_QWEN38_MAX_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_BF16 )
			return(1);
	}
	if ( SparkHybridStagePackScaleGroupSizeOk(SparkQwen38MaxStagePackWeightClass(weight_format),scale_group_size) == 0u )
		return(1);
	if ( payload_bytes != SparkQwen38MaxStagePackPayloadBytes(weight_format,rows,columns) || scale_bytes != SparkQwen38MaxStagePackScaleBytes(weight_format,rows,columns) )
		return(1);
	...
	if ( scale_bytes != 0u && (scale_offset > file_bytes || scale_bytes > file_bytes - scale_offset) )
		return(1);
	if ( layer_index == SPARK_QWEN38_MAX_STAGEPACK_MTP_LAYER || (global != 0u && (tensor_kind >= SPARK_QWEN38_MAX_STAGEPACK_TENSOR_MTP_FC && tensor_kind <= SPARK_QWEN38_MAX_STAGEPACK_TENSOR_MTP_FINAL_NORM)) )
	{
		if ( 0u == 1u ) /* owns_final_head pinned true */
			return(1);
	}
	else if ( global == 0u && (layer_index < 0u || layer_index >= 0u + SPARK_QWEN38_MAX_MODEL_LAYER_COUNT) )
		return(1);
	return(0);
}
...
int main(void)
{
	static const uint32_t layers[] = {SPARK_QWEN38_MAX_STAGEPACK_GLOBAL_LAYER,SPARK_QWEN38_MAX_STAGEPACK_MTP_LAYER,0u,1u,2u,3u,7u,30u,31u,32u,100u,UINT32_MAX};
	static const uint32_t formats[] = {SPARK_QWEN38_MAX_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_BF16,SPARK_QWEN38_MAX_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_MXFP4_E2M1,SPARK_QWEN38_MAX_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_FP8_E4M3_F32B128,SPARK_QWEN38_MAX_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_F32,SPARK_QWEN38_MAX_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_U32,5u,6u,UINT32_MAX};
	...
	for (kind = 0; kind < SPARK_QWEN38_MAX_STAGEPACK_TENSOR_KIND_COUNT + 2u; kind++)
		for (i = 0; i < (uint32_t)(sizeof(layers)/sizeof(layers[0])); i++)
		{
			SparkQwen38MaxStagePackTensorShape shape;
			uint32_t global = layers[i] == SPARK_QWEN38_MAX_STAGEPACK_GLOBAL_LAYER ? 1u : 0u;
			int32_t code;
			memset(&shape,0,sizeof(shape));
			code = SparkQwen38MaxStagePackResolvedShape(kind,global != 0u ? 0u : layers[i],global,&shape);
	...
	for (kind = 0; kind < SPARK_QWEN38_MAX_STAGEPACK_TENSOR_KIND_COUNT + 2u; kind++)
		for (i = 0; i < (uint32_t)(sizeof(layers)/sizeof(layers[0])); i++)
			for (f = 0; f < fmt_count; f++)
			{
				...
				uint64_t pb,sb;
				memset(&shape,0,sizeof(shape));
				if ( SparkQwen38MaxStagePackResolvedShape(kind,global != 0u ? 0u : layers[i],global,&shape) != 0 )
					continue;
				pb = SparkQwen38MaxStagePackPayloadBytes(formats[f],shape.rows,shape.columns);
				sb = SparkQwen38MaxStagePackScaleBytes(formats[f],shape.rows,shape.columns);
	...
	for (f = 0; f < fmt_count; f++)
		for (r = 0; r < dim_count; r++)
			for (c = 0; c < dim_count; c++)
			{
				...
				emit(tag,(long)SparkQwen38MaxStagePackPayloadBytes(formats[f],dims[r],dims[c]));
				snprintf(tag,sizeof(tag),"C fmt=%u r=%u c=%u scale",f,dims[r],dims[c]);
				emit(tag,(long)SparkQwen38MaxStagePackScaleBytes(formats[f],dims[r],dims[c]));
			}
	...
	/* D: computed slice inventories across the split space. */
	{
		...
		for (i = 0; i < (uint32_t)(sizeof(firsts)/sizeof(firsts[0])); i++)
			for (count = 0; count <= SPARK_QWEN38_MAX_MODEL_LAYER_COUNT + 1u; count++)
			{
				first = firsts[i];
				if ( first > 60u )
					continue;
				snprintf(tag,sizeof(tag),"D first=%u count=%u",first,count);
				emit(tag,(long)SparkQwen38MaxStagePackExpectedTensorCount(first,count));
			}
	}

	/* E: header-compare codes and names under single-field hostility. */
	{
		SparkQwen38MaxStagePackHeader expected,file_header;
		uint32_t field,u32_count = (uint32_t)SPARK_QWEN38_MAX_STAGEPACK_COMPARE_U32_FIELDS;
		static const uint32_t hostiles[] = {0u,1u,2u,UINT32_MAX};
		memset(&expected,0,sizeof(expected));
		SparkQwen38MaxStagePackExpectedGeometry(&expected,0u,SPARK_QWEN38_MAX_MODEL_LAYER_COUNT);
		expected.directory_offset = SPARK_QWEN38_MAX_STAGEPACK_HEADER_BYTES;
		...
		for (field = 0; field < u32_count; field++)
		{
			...
			for (h = 0; h < (uint32_t)(sizeof(hostiles)/sizeof(hostiles[0])); h++)
			{
				...
				code = SparkQwen38MaxStagePackHeaderMatches(&file_header,&expected);
				snprintf(tag,sizeof(tag),"E field=%u hostile=%u",field,hostiles[h]);
				emit(tag,(long)code);
			}
		}
		emit("E clean",(long)SparkQwen38MaxStagePackHeaderMatches(&expected,&expected));
	}
	...
	for (i = 0; i <= 70u; i++)
	{
		...
		emit(tag,(long)SparkHybridStagePackFullAttentionLayersBelow(SPARK_QWEN38_MAX_MODEL_ATTENTION_PERIOD,i));
	}
