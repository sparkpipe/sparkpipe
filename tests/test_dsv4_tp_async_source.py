...
# The speculative-wave full-width gate lives in exactly one place now:
# SparkDsv4ModuleBucketShapeAccepted. Frame validation and admission must
# both delegate to it (the pre-fix duplicated inline copies had already
# diverged on the spec-step arm), and the helper itself keeps the equality
# form plus the bucket-8 DSpark lane arm with TP1 exempt.
bucket_gate = body("static uint32_t SparkDsv4ModuleBucketShapeAccepted")
assert "active_slot_count == SPARK_BATCH_BUCKET &&" in bucket_gate
assert "new_token_count == SPARK_BATCH_BUCKET )" in bucket_gate
assert "SPARK_DSV4_MODEL_DSPARK_SPEC_STEP + 1u" in bucket_gate
assert "state->dspark_enabled != 0u" in bucket_gate
assert "state->tp_degree <= 1u" in bucket_gate
for shape in (validate_shape, admission_shape):
    assert "SparkDsv4ModuleBucketShapeAccepted(state," in shape
