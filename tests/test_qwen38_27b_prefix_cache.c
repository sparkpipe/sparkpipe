...
/*
 * Prefix-cache correctness gate for the qwen38_27b serving adapter, run
 ...
 *      spark_qwen38_27b_paged_kv.c directly (the audit-probe pattern),
	...
	uint32_t logical_pages, uint32_t physical_pages)
{
	...
	configuration->driver_shared_object_path = TEST_QWEN38_27B_SERVING_DRIVER_PATH;
	...
	uint32_t length, uint64_t submission_id, int publish)
{
	...
	if ( publish != 0 )
	{
		boundary = length - (length % SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS);
		if ( boundary == 0u )
			boundary = length;
	...
	const GateFrameRecord *donor_tail)
{
	...
	for (index=from; index<to; index++)
	{
		...
		if ( (record->context_flags & SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_PREFIX_RESUME) != 0u )
		{
			resume_count++;
			assert(record->base_position == walked_cursor);
			assert(record->snapshot_index_present != 0u);
			assert(record->snapshot_index < SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_MAX_GDN_SNAPSHOT_SLOTS);
		}
		else
			assert(record->snapshot_index_present == 0u ||
				(record->context_flags & SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_GDN_CHECKPOINT) != 0u);
		assert(record->base_position == walked_cursor);
		...
		if ( (record->context_flags & SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_GDN_CHECKPOINT) != 0u )
			assert((record->base_position + record->rows) % SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS == 0u);
 ...
 * Direct host-level driver of spark_qwen38_27b_paged_kv.c (the audit-probe
...
static uint32_t GateFreeListConservation(const SparkQwen38_27bPagedKv *cache,
	...
	int8_t *in_free,int8_t *in_use,uint32_t pool)
{
	...
	for (i=0u; i<cache->configuration.lane_count; i++)
			...
			ordinal++)
		{
			...
			{
				if ( block != SPARK_QWEN38_27B_PAGED_KV_NO_BLOCK )
				{
			...
			assert(block != SPARK_QWEN38_27B_PAGED_KV_NO_BLOCK &&
				block < pool);
			in_use[block] = 1;
		}
...
static uint32_t F1ConservationCheck(const SparkQwen38_27bPagedKv *cache,
...
static void GatePagedKvFixture(SparkQwen38_27bPagedKvConfiguration *configuration,
...
static int F1ScenarioReuseOn(void)
{
	...
	assert(SparkQwen38_27bPagedKvInitialize(&cache,&configuration,f1_table,
		f1_counts) == SPARK_STATUS_OK);
	F1Fill(prompt,10000u,130u); /* 130 tokens: 2 full blocks + 1 open */
	status = SparkQwen38_27bPagedKvAdmit(&cache,0u,prompt,130u,&match);
	...
	for (i=0u; i<F1_ROUNDS; i++)
	{
		...
		status = SparkQwen38_27bPagedKvCover(&cache,0u,pos + 1u,0,0);
		assert(status == SPARK_STATUS_OK);
		assert(F1ConservationCheck(&cache,"A/pre",i) == 0u);
		status = SparkQwen38_27bPagedKvCover(&cache,0u,pos + 6u,0,0);
		assert(status == SPARK_STATUS_OK);
	...
	printf("f1_conservation reuse-on: %u rounds, max attached=%u, final free=%u\n",
		F1_ROUNDS,worst_counts,SparkQwen38_27bPagedKvFreeBlocks(&cache));
	/* Teardown must return every outstanding scratch block. */
	SparkQwen38_27bPagedKvLaneReset(&cache,0u);
	assert(F1ConservationCheck(&cache,"A/reset",F1_ROUNDS) == 0u);
	SparkQwen38_27bPagedKvDestroy(&cache);
	return(0);
}
...
static int F1ScenarioReuseOff(void)
{
	...
	status = SparkQwen38_27bPagedKvAdmit(&cache,0u,prompt,70u,0);
	assert(status == SPARK_STATUS_OK);
	worst_counts = f1_counts[0];
	for (i=0u; i<F1_ROUNDS; i++)
	{
 ...
 * state = BLOCK_FREE, so SparkQwen38_27bPagedKvFreeBlocks() counted in-use
...
static int F2ScenarioFeasibility(void)
{
	...
	assert(SparkQwen38_27bPagedKvInitialize(&cache,&configuration,f2_table,
		f2_counts) == SPARK_STATUS_OK);
	for (i=0u; i<128u; i++)
		prompt[i] = 40000u + i * 3u;
	status = SparkQwen38_27bPagedKvAdmit(&cache,0u,prompt,64u,0);
	assert(status == SPARK_STATUS_OK);
	/* Phase 1: drain the pool down to 2 free blocks through lane 0. */
	for (i=1u; i<=10u; i++)
	{
		status = SparkQwen38_27bPagedKvCover(&cache,0u,
		...
		free_now = SparkQwen38_27bPagedKvFreeBlocks(&cache);
	...
	{
		status = SparkQwen38_27bPagedKvCover(&cache,1u,
	...
	SparkQwen38_27bPagedKvLaneReset(&cache,0u);
	SparkQwen38_27bPagedKvLaneReset(&cache,1u);
	free_now = SparkQwen38_27bPagedKvFreeBlocks(&cache);
	if ( free_now != F2_POOL )
	{
 ...
 * size is SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT
 * = 512 (modules/qwen38_27b_resident_decode_stage/include/sparkpipe/
 * spark_qwen38_27b_resident_decode_stage_firmware.h; deployment description
 * examples/model_descriptions/qwen38_27b_resident_decode_stage_firmware.json
...
#define S_LANES SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT
...
static uint32_t S512ConservationCheck(const SparkQwen38_27bPagedKv *cache,
	const char *where,uint32_t step)
{
...
static int S512ScaleScenario(void)
{
	...
	assert(SparkQwen38_27bPagedKvInitialize(&cache,&configuration,s_table,
	...
	for (batch=0u; batch<S_LANES/S_BATCH_LANES; batch++)
	{
		for (group=0u; group<S_BATCH_LANES/S_GROUP_SIZE; group++)
		{
			...
			status = SparkQwen38_27bPagedKvAdmit(&cache,lanes[0],
				donor_tokens,130u,&match);
			assert(status == SPARK_STATUS_OK);
			assert(match.block_count == 0u);
			assert(SparkQwen38_27bPagedKvCommittedTokens(&cache,
				lanes[0]) == 130u);
			if ( SparkQwen38_27bPagedKvCheckpointOffer(&cache,lanes[0],
				64u,&slot) != 0u )
				SparkQwen38_27bPagedKvCheckpointCommit(&cache,lanes[0],
					slot,64u);
			if ( SparkQwen38_27bPagedKvCheckpointOffer(&cache,lanes[0],
				128u,&slot) != 0u )
				SparkQwen38_27bPagedKvCheckpointCommit(&cache,lanes[0],
			...
			{
				uint64_t frontier = SparkQwen38_27bPagedKvCommittedTokens(
					&cache,lanes[index]);
				for (round=0u; round<S_ROUNDS_PER_LANE; round++)
				{
					uint64_t pos = frontier + 1ull +
						(uint64_t)round;
					status = SparkQwen38_27bPagedKvCover(&cache,
				...
				SparkQwen38_27bPagedKvLaneReset(&cache,lanes[index]);
	...
	printf("s512_scale lanes=%u rounds_per_lane=%u matched_blocks=%llu appended_tokens=%llu evicted_blocks=%llu free=%u\n",
		...
		SparkQwen38_27bPagedKvFreeBlocks(&cache));
	SparkQwen38_27bPagedKvDestroy(&cache);
	return(0);
}
int main(void)
{
	...
	driver_library = dlopen(TEST_QWEN38_27B_SERVING_DRIVER_PATH,RTLD_NOW);
	assert(driver_library != 0);
	gate_record_count = (GateRecordCountFunction)dlsym(driver_library,"TestQwen38_27bServingDriverRecordCount");
	gate_record_get = (GateRecordGetFunction)dlsym(driver_library,"TestQwen38_27bServingDriverRecord");
	gate_record_reset = (GateRecordResetFunction)dlsym(driver_library,"TestQwen38_27bServingDriverResetRecords");
	gate_kv_blocks = (GateKvBlockCountFunction)dlsym(driver_library,"TestQwen38_27bServingDriverKvBlockCount");
...

	{
		...
		setenv("SPARK_QWEN38_27B_SERVING_PREFIX_CACHE","1",1);
		GateConfiguration(&configuration,TEST_QWEN38_27B_SERVING_CONFIG_PATH,runtime_root,&test_state,8u,8u,1024u,512u);
		adapter_state = 0;
		...
		/* ---------- CASE 3: ON vs OFF ------------------------------------ */
		setenv("SPARK_QWEN38_27B_SERVING_PREFIX_CACHE","0",1);
		GateConfiguration(&configuration,TEST_QWEN38_27B_SERVING_CONFIG_PATH,runtime_root,&test_state,8u,8u,1024u,512u);
		adapter_state = 0;
		assert(library.adapter_interface.initialize(&configuration,&adapter_state) == SPARK_STATUS_OK);
		gate_record_reset();
		...
		assert(GatePrefill(adapter_state,&test_state,3u,2002u,prompt_a,prompt_length,801u) == SPARK_STATUS_OK);
		{
			...
			{
				const GateFrameRecord *record = gate_record_get(index);
				assert((record->context_flags & (SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_PREFIX_RESUME | SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_GDN_CHECKPOINT)) == 0u);
				total_rows += record->rows;
			}
			assert(total_rows == 2u * prompt_length);
		}
		library.adapter_interface.destroy(adapter_state);
		unsetenv("SPARK_QWEN38_27B_SERVING_PREFIX_CACHE");
		...
		GateConfiguration(&configuration,TEST_QWEN38_27B_SERVING_CONFIG_PATH,runtime_root,&test_state,25u,32u,512u,256u);
		adapter_state = 0;
		assert(library.adapter_interface.initialize(&configuration,&adapter_state) == SPARK_STATUS_OK);
			...
			for (root=0u; root<25u; root++)
			{
				...
				if ( gate_status == SPARK_STATUS_OK )
				{
					...
					for (index=records_before; index<gate_record_count(); index++)
					{
						...
						{
							resume_seen++;
							assert(record->base_position % SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS == 0u);
							assert(record->snapshot_index_present != 0u);
						}
		...
		GateConfiguration(&configuration,TEST_QWEN38_27B_SERVING_CONFIG_PATH,runtime_root,&test_state,8u,8u,128u,512u);
		adapter_state = 0;
		...
		GateConfiguration(&configuration,TEST_QWEN38_27B_SERVING_CONFIG_PATH,runtime_root,&test_state,8u,8u,1024u,600u);
		adapter_state = 0;
		 ...
		 * declared batch size (SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_MAX_
		 * ACTIVE_SEQUENCE_COUNT = 512, spark_qwen38_27b_resident_decode_
		...
		GateConfiguration(&configuration,TEST_QWEN38_27B_SERVING_CONFIG_PATH,runtime_root,&test_state,
		...
		gate_record_reset();
		{
			...
			statuses_ok = 0u;
			statuses_refused = 0u;
			for (root=0u; root<SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT; root++)
			{
				length = 70u + ((root * 37u) % 300u);
				for (row=0u; row<length && row<GATE_MAX_ROWS; row++)
				...
				records_before = gate_record_count();
				{
					SparkStatus gate_status = GatePrefill(adapter_state,&test_state,
						root % SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT,
		...
		setenv("SPARK_QWEN38_27B_SERVING_PREFIX_CACHE","1",1);
		setenv("SPARK_QWEN38_27B_SERVING_SPECULATE","1",1);
		GateConfiguration(&configuration,TEST_QWEN38_27B_SERVING_CONFIG_PATH,runtime_root,&test_state,4u,4u,128u,128u);
		adapter_state = 0;
		assert(library.adapter_interface.initialize(&configuration,&adapter_state) == SPARK_STATUS_OK);
		...
		assert(library.adapter_interface.quiesce(adapter_state,UINT64_MAX) == SPARK_STATUS_OK);
		library.adapter_interface.destroy(adapter_state);
		unsetenv("SPARK_QWEN38_27B_SERVING_SPECULATE");
		 ...
		 * qwen38_27b_prefix_reuse despite publications). The law: cycle N+1 on
		 * the SAME resident slot must match cycle N's publications and
		 * resume at their witnessed boundary.
		 * ARMING: SPARK_QWEN38_27B_CROSS_CYCLE_GATE=1. Unarmed the cell SKIPs
		...
		if ( getenv("SPARK_QWEN38_27B_CROSS_CYCLE_GATE") == 0 )
	...
	printf("qwen38_27b prefix-cache gate PASS\n");
	return(0);
}
