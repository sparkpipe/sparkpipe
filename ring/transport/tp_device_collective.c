    ...
    uint32_t operation_kind)
{
    ...
    slot = implementation->gpu_base + implementation->band_base +
    ...
    __sync_synchronize();
    {
        ...
        if ( cudaMemcpyAsync(slot,submission->local_device,(size_t)bytes,
                cudaMemcpyDeviceToDevice,submission->cuda_stream) != 0 )
    ...
    for ( peer = 0u; peer < implementation->tp_degree - 1u; peer++ )
    {
        ...
        volatile uint64_t *landed = (volatile uint64_t *)
            (implementation->doorbell +
            SPARK_WEIGHTD_MESH_DOORBELL_LANDED(band_index,peer_rank));
        while ( *landed < ordinal + 1u )
        {
    ...
    for ( peer = 0u; peer < implementation->tp_degree - 1u; peer++ )
    {
        uint8_t *source = implementation->gpu_base +
    ...
    uint32_t operation_kind)
{
    ...
    if ( implementation->doorbell == 0 || implementation->gpu_base == 0 )
        SPARK_FAIL(SPARK_STATUS_UNSUPPORTED);
    round = *submission;
    ...
    void *cuda_stream)
{
    ...
    if ( cudaHostRegister(receive_device,
            SPARK_WEIGHTD_MESH_DOORBELL_BYTES,0u) != 0 )
        SPARK_FAIL(SPARK_STATUS_IO_ERROR);
    implementation->doorbell = receive_device;
    return SPARK_STATUS_OK;
}
