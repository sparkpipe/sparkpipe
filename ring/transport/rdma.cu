    ...
    const SparkHiddenTransportPacket *packet)
{
    ...
    if (remote_receive_index >=
            SPARK_HIDDEN_SPARK_HOST_RDMA_MAX_REMOTE_RECEIVE_COUNT)
        return SPARK_STATUS_INTERNAL_ERROR;
	/* Mapped-host packets are fenced by the gather before this call.  Do not
	 * enqueue a second CUDA event/host callback for each recursive-doubling
	 * exchange; device-direct packets still require the transport fence. */
	if (state->memory_mode !=
		SPARK_HIDDEN_SPARK_HOST_RDMA_MEMORY_MODE_MAPPED_HOST)
	{
