...
# usage: mb_run.sh <mode:0 async|1 sync> <iters> <rows> [credits] [d2a_max_bytes] [degree:4|16]
set -euo pipefail
...
DEGREE="${6:-16}"
case "$DEGREE" in
    4) NODES=spark0,spark1,spark2,spark3 ;;
    16) NODES=spark0,spark1,spark2,spark3,spark4,spark5,spark6,spark7,spark8,spark9,sparka,sparkb,sparkc,sparkd,sparke,sparkf ;;
    *) echo "degree must be 4 or 16" >&2; exit 2 ;;
esac
