...
/* Rolling-acceptance gate normalization: window clamps to the ring
 * capacity, thresholds are milli-ratios capped at one (1000). */
static uint32_t SparkSpeculationPolicyNormalizeAcceptanceWindow(
    uint32_t window_rounds)
{
    if (window_rounds > SPARK_DSPARK_ACCEPTANCE_WINDOW_MAX)
    {
        return SPARK_DSPARK_ACCEPTANCE_WINDOW_MAX;
    }
    return window_rounds;
}

static uint32_t SparkSpeculationPolicyNormalizeMilliRatio(uint32_t milli)
{
    if (milli > 1000u)
    {
        return 1000u;
    ...
    const SparkSpeculationConfiguration *configuration)
{
    ...
    /* The gate normalizers clamp rather than reject, so any raw value is
     * admissible here; nothing further to validate for the gate. */
    ...
    const SparkSpeculationConfiguration *configuration)
{
    ...
    speculator->acceptance_window_rounds =
        SparkSpeculationPolicyNormalizeAcceptanceWindow(
            configuration->acceptance_window_rounds);
    speculator->acceptance_disable_threshold_milli =
        SparkSpeculationPolicyNormalizeMilliRatio(
            configuration->acceptance_disable_threshold_milli);
    speculator->acceptance_resume_margin_milli =
        SparkSpeculationPolicyNormalizeMilliRatio(
            configuration->acceptance_resume_margin_milli);
...
/* Feed one completed verify round into the rolling-acceptance ring and
 * re-evaluate the gate with hysteresis: drafting suspends when the window
 * is full and the accepted/proposed milli-ratio falls below the disable
 * threshold; it resumes only at threshold + margin. While suspended no
 * regular rounds occur, so EnsureDraft's periodic probe keeps the window
 * turning (without it the gate could never lift). */
static void SparkSpeculationPolicyGateObserveRound(
    SparkSpeculationSpeculator *speculator,
    ...
    uint32_t accepted_draft_token_count)
{
    ...
    if (speculator->acceptance_window_rounds == 0u)
    {
        return;
    }
    speculator->acceptance_ring_proposed[speculator->acceptance_ring_index] =
        proposed_token_count;
    speculator->acceptance_ring_accepted[speculator->acceptance_ring_index] =
        accepted_draft_token_count;
    speculator->acceptance_ring_index += 1u;
    if (speculator->acceptance_ring_index ==
        speculator->acceptance_window_rounds)
    {
        speculator->acceptance_ring_index = 0u;
    }
    if (speculator->acceptance_round_count <
        speculator->acceptance_window_rounds)
    {
        speculator->acceptance_round_count += 1u;
    }

    for (sample_index = 0u; sample_index < speculator->acceptance_round_count;
         ++sample_index)
    {
        proposed_total += speculator->acceptance_ring_proposed[sample_index];
        accepted_total += speculator->acceptance_ring_accepted[sample_index];
    }
    /* proposed_total > 0: verify validation rejects a zero proposal. */
    rate_milli = (uint32_t)(accepted_total * 1000ull / proposed_total);

    if (speculator->acceptance_suspended == 0u)
    {
        if (speculator->acceptance_round_count ==
                speculator->acceptance_window_rounds &&
            rate_milli < speculator->acceptance_disable_threshold_milli)
        {
            speculator->acceptance_suspended = 1u;
            speculator->gate_suspend_count += 1u;
            speculator->acceptance_probe_skip_count = 0u;
        }
    }
    else if (rate_milli >=
             speculator->acceptance_disable_threshold_milli +
                 speculator->acceptance_resume_margin_milli)
    {
        speculator->acceptance_suspended = 0u;
        speculator->acceptance_probe_skip_count = 0u;
    }
}
    ...
    const SparkSpeculationDraftRequest *request)
{
    ...
    /* Rolling-acceptance gate: while suspended, refuse to draft (NOT_FOUND,
 * the same signal the confidence gate uses - callers fall back to a plain
 * decode step). A probe round goes through once per window of skipped
 * requests so completed verifies keep feeding samples; without it a lifted
 * acceptance rate could never reopen the gate. An already-prepared draft
 * on this sequence still serves its round above. */
    if (speculator->acceptance_window_rounds != 0u &&
        speculator->acceptance_suspended != 0u)
    {
        if (speculator->acceptance_probe_skip_count <
            speculator->acceptance_window_rounds)
        {
            speculator->acceptance_probe_skip_count += 1u;
            speculator->gate_skipped_round_count += 1u;
            return SPARK_STATUS_NOT_FOUND;
        }
        speculator->acceptance_probe_skip_count = 0u;
    }
    ...
    const SparkSpeculationVerifyResult *verify_result)
{
    ...
    SparkSpeculationPolicyGateObserveRound(
        speculator, verify_result->proposed_token_count,
        verify_result->accepted_draft_token_count);
...
uint32_t SparkSpeculationPolicyAcceptanceSuspended(
    const SparkSpeculationSpeculator *speculator)
{
    if (speculator == 0 || speculator->abi_version != SPARK_DSPARK_ABI_VERSION ||
        speculator->descriptor_bytes != SPARK_DSPARK_DESCRIPTOR_BYTES)
    {
        return 0u;
    }
    return speculator->acceptance_suspended;
}