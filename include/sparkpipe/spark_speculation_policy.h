 ...
 * header. */
/* Rolling-acceptance gate ring capacity (rounds of history). The gate
 * itself is configured by the three acceptance_* fields appended to the
 * configuration/speculator structs below; a zero window disables it. */
#define SPARK_DSPARK_ACCEPTANCE_WINDOW_MAX 256u
...
typedef struct SparkSpeculationConfiguration
{
    ...
    /* Rolling-acceptance speculation gate. Appended fields: zero-initialize
     * keeps every pre-gate initializer behaving exactly as before (window
     * 0 disables the gate). Window clamps to ACCEPTANCE_WINDOW_MAX; both
     * thresholds are milli-ratios of accepted/proposed tokens, <= 1000. */
    uint32_t acceptance_window_rounds;
    uint32_t acceptance_disable_threshold_milli;
    uint32_t acceptance_resume_margin_milli;
...
typedef struct SparkSpeculationSpeculator
{
    ...
    SparkSpeculationDraftFunction draft_function;
    void *draft_context;
    /* Rolling-acceptance gate state (mirrors the configuration fields plus
     * the sample ring; zero-init compatibility note as above). */
    uint32_t acceptance_window_rounds;
    uint32_t acceptance_disable_threshold_milli;
    uint32_t acceptance_resume_margin_milli;
    uint64_t acceptance_ring_proposed[SPARK_DSPARK_ACCEPTANCE_WINDOW_MAX];
    uint64_t acceptance_ring_accepted[SPARK_DSPARK_ACCEPTANCE_WINDOW_MAX];
    uint32_t acceptance_ring_index;
    uint32_t acceptance_round_count;
    uint32_t acceptance_suspended;
    uint32_t acceptance_probe_skip_count;
    uint64_t gate_suspend_count;
    uint64_t gate_skipped_round_count;
...
/* Nonzero while the rolling-acceptance gate has suspended speculative
 * drafting (EnsureDraft refuses with NOT_FOUND except for its periodic
 * probe rounds). Always 0 when the gate is disabled (zero window). */
