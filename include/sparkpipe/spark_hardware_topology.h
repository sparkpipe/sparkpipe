 ...
 * Exception: the compute-capability pair is pinned to the public CDNA4
 * specification and to what the in-tree target guard already requires -
 * see the capability block below.
 */
...
/* Capability pins: gfx950 (CDNA4) reports compute capability 9.5, and the
 * in-tree fail-closed guard has required exactly that since its landing
 * (spark_hw_rocm_require_gfx950: gcnArchName "gfx950" AND capability (9,5)).
 * Spec-pinned here so the whole tree expects one number; per plan section
 * 3.3 the first on-hardware receipt must confirm the pin, and the runtime
 * descriptor (SparkHwTarget.capability_*) stays the measured authority the
 * guard compares against. */
#define SPARK_HW_COMPUTE_CAPABILITY_MAJOR 9u
#define SPARK_HW_COMPUTE_CAPABILITY_MINOR 5u
