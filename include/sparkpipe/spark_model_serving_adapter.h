...
#define SPARK_MODEL_SERVING_ADAPTER_CAPABILITY_RESIDENT_DECODE_CHAIN \
	UINT32_C(0x00002000)
/*
 * Completions are delivered synchronously before submit resolves, on the
 * thread that invoked it. See the THREADING CONTRACT below (T2).
 * Mutually exclusive with CAPABILITY_ASYNC_COMPLETION.
 */
#define SPARK_MODEL_SERVING_ADAPTER_CAPABILITY_SYNC_DELIVERY \
	UINT32_C(0x00004000)
#define SPARK_MODEL_SERVING_ADAPTER_KNOWN_CAPABILITIES \
	(SPARK_MODEL_SERVING_ADAPTER_CAPABILITY_PREFILL | \
	 ...
	 SPARK_MODEL_SERVING_ADAPTER_CAPABILITY_SYNC_DELIVERY)

/*
 * THREADING CONTRACT (normative; additive revision of ABI 20u -- no struct
 * layout changes, therefore no ABI bump).
 *
 * T1 SINGLE SUBMITTER. For one adapter instance, the resident invokes the
 * submission-side entries -- validate_submission, prefetch,
 * resolve_prefetch, submit -- and the control entries quiesce, snapshot,
 * reset, progress from at most one thread at a time, serializing them
 * externally. Adapters MAY keep these paths lock-free: pending-slot
 * claiming, admission counters, and latch reads are single-writer by this
 * rule. Two concurrent entry-point calls on one instance are a RESIDENT
 * bug, not an adapter defect.
 *
 * T2 COMPLETION DELIVERY CONTEXT. Exactly one delivery school is
 * advertised per instance via capability_flags:
 *   - CAPABILITY_SYNC_DELIVERY: every serving completion is delivered
 *     before submit resolves, on the calling thread. progress is the
 *     stream-ordered stub; wake_function is advisory. A completion
 *     callback MUST NOT re-enter any entry point of the same adapter
 *     instance.
 *   - CAPABILITY_ASYNC_COMPLETION: completions may arrive after submit
 *     returns, on driver-owned threads, possibly concurrent with a later
 *     submission-side call. Safety rests on PENDING-SLOT EXCLUSIVITY:
 *     between claim and release a pending record is owned solely by its
 *     own completion path, so cross-thread contact stays confined to
 *     uncontended slot state and the orphan counter.
 * Declaring neither bit for an adapter that delivers completions, or
 * declaring both, is a conformance defect caught by
 * SparkModelServingAdapterValidateInterface policy checks.
 *
 * T3 QUIESCE LATCH. quiesce latches the instance BEFORE any availability
 * check: once the latch is set, every validate_submission, prefetch, and
 * submit returns BUSY without starting model work, even if slots are
 * idle. The latch decision shares the T1 serialization domain, so a call
 * already in flight when quiesce begins completes normally first. A
 * quiesce that returns BUSY/PENDING leaves the latch SET -- admission
 * stays closed until a retry drains the instance and returns OK. With
 * CAPABILITY_ASYNC_COMPLETION, OK additionally requires zero in-flight
 * driver submissions, so no completion can reference adapter state after
 * OK. Completion callbacks MUST NOT call quiesce, submit, or
 * validate_submission on their own instance. destroy keeps its separate
 * idle-instance guard and requires a prior OK (or a never-submitted
 * instance).
 */
 ...
 *
 * Threading: submit runs under the single-submitter rule and delivers
 * completions per the advertised delivery school; see T1/T2 of the
 ...
 *
 * Threading: the admission latch is set on entry and STAYS set across
 * BUSY/PENDING retries; see T3 of the THREADING CONTRACT above.
 */
