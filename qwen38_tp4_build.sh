#!/usr/bin/env bash
# qwen38_27b TP4 whole-stack build: module archive + TP4 GPU validation + driver
set -euo pipefail
...
mkdir -p "$OUT"
echo "=== module archive ==="
make -C modules/qwen38_27b_resident_decode_stage -j20 archive \
...
make -C modules/qwen38_27b_resident_decode_stage publish \
    NVCC=/usr/local/cuda/bin/nvcc CUDA_ARCH=sm_121a \
    STAGE_PACK_PATH="$PACKS/tp4-rank0.qwen38_27bsp" \
...
build/sparkpipe_model_compile \
    ...
    --stage qwen38_27b_resident_decode_stage \
...
cp build/libqwen38_27b_serving_adapter.so "$OUT/model_serving_adapter.so"
...
python3 tools/generate_model_resident_deployment.py \
    --specification examples/deployments/qwen38_27b_tp4_host_rdma.spec.json \
    --output "$OUT/model_resident.json"
cp examples/deployments/qwen38_27b_tp4_rank0.json "$OUT/qwen38_27b_tp4_rank0.json"
cp examples/deployments/qwen38_27b_tp4_rank1.json "$OUT/qwen38_27b_tp4_rank1.json"
cp examples/deployments/qwen38_27b_tp4_rank2.json "$OUT/qwen38_27b_tp4_rank2.json"
cp examples/deployments/qwen38_27b_tp4_rank3.json "$OUT/qwen38_27b_tp4_rank3.json"
echo "=== build done ==="
