...
{
    cudaError_t error;
    if (device_physical_block_indices == 0 || device_lane_block_counts == 0 ||
        host_physical_block_indices == 0 || host_lane_block_counts == 0 ||
        lane_count == 0u || lane_stride < lane_count)
    {
        return SPARK_STATUS_INVALID_ARGUMENT;
    }
    error = cudaMemcpyAsync(
        device_physical_block_indices,
        host_physical_block_indices,
        (size_t)lane_stride * lane_count * sizeof(uint32_t),
        cudaMemcpyHostToDevice,
        stream);
    if (error != cudaSuccess)
    {
        return SparkStageModuleCudaStatus(
            module_tag, error, "block_table_indices_upload");
    }
    error = cudaMemcpyAsync(
        device_lane_block_counts,
        host_lane_block_counts,
    ...
    {
        return SparkStageModuleCudaStatus(
            module_tag, error, "block_table_counts_upload");
    }
    return SPARK_STATUS_OK;
}
