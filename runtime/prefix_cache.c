...
static uint32_t CoreIdHash(uint64_t sequence_id, uint32_t mask)
{
	uint64_t x = sequence_id * UINT64_C(0x9E3779B97F4A7C15);
	x ^= x >> 32u;
	return (uint32_t)x & mask;
}

static void CoreIndexIdPut(SparkPrefixCacheCore *core,
	uint64_t sequence_id, uint32_t slot)
{
	uint32_t mask = core->id_bucket_mask;
	uint32_t i = CoreIdHash(sequence_id, mask);
	while (core->id_slots[i] != 0u)
	{
		i = (i + 1u) & mask;
	}
	core->id_slots[i] = slot + 1u;
	core->id_keys[i] = sequence_id;
}

static void CoreIndexIdRemove(SparkPrefixCacheCore *core,
	uint64_t sequence_id)
{
	...
	while (core->id_slots[i] != 0u)
	{
		if (core->id_keys[i] == sequence_id)
		{
			...
			core->id_slots[j] = 0u;
			k = (j + 1u) & mask;
			while (core->id_slots[k] != 0u)
			{
				uint32_t home = CoreIdHash(core->id_keys[k],
					mask);
				int in_segment = (j <= k)
					? (home > j && home <= k)
					: (home > j || home <= k);
				if (!in_segment)
				{
					core->id_slots[j] = core->id_slots[k];
					core->id_keys[j] = core->id_keys[k];
					core->id_slots[k] = 0u;
					j = k;
				}
				k = (k + 1u) & mask;
...
static SparkStatus CoreFindSequence(
	...
	uint32_t *index_out)
{
	uint32_t i = CoreIdHash(sequence_id, core->id_bucket_mask);
	while (core->id_slots[i] != 0u)
	{
		uint32_t slot = core->id_slots[i] - 1u;
		if (core->id_keys[i] == sequence_id &&
		    core->sequences[slot].used != 0u &&
		    core->sequences[slot].sequence_id == sequence_id)
		{
			*index_out = slot;
			return SPARK_STATUS_OK;
		}
		i = (i + 1u) & core->id_bucket_mask;
	}
	...
	const SparkPrefixCacheCoreConfiguration *configuration)
{
	...
	core->id_bucket_mask = 15u;
	while (core->id_bucket_mask + 1u <
	           configuration->max_sequence_count * 2u)
	{
		core->id_bucket_mask = (core->id_bucket_mask << 1u) | 1u;
	}
	core->id_slots = (uint32_t *)calloc(
	    (size_t)core->id_bucket_mask + 1u, sizeof(core->id_slots[0]));
	core->id_keys = (uint64_t *)calloc(
	    (size_t)core->id_bucket_mask + 1u, sizeof(core->id_keys[0]));
	if (core->blocks == 0 || core->block_tokens == 0 ||
	    core->sequences == 0 || core->sequence_blocks == 0 ||
	    core->hash_buckets == 0 || core->id_slots == 0 ||
	    core->id_keys == 0)
	{
	...
	uint32_t *matched_token_count_out)
{
	...
	CoreIndexIdPut(core, sequence_id,
	    (uint32_t)(sequence - core->sequences));
    ...
    const SparkPrefixCacheCoreSequence *sequence)
{
	...
	if (sequence->block_count != 0u)
	{
		candidate = &core->blocks[
		    sequence->blocks[sequence->block_count - 1u]];
	...
	uint64_t sequence_id)
{
	...
	CoreIndexIdRemove(core, sequence_id);
	sequence->used = 0u;
