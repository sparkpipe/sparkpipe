	...
	const SparkModelDriverCompletion *driver_completion)
{
	...
	if ( common != 0 )
		...
		/* F-A2: driver completion threads bump concurrently with the
		 * control thread's snapshot reads; relaxed fetch_add keeps the
		 * count exact without ordering it against anything. */
		atomic_fetch_add_explicit(&common->orphan_completion_count,1u,
	...
	SparkModelServingAdapterSnapshot *snapshot)
{
	...
	/* F-A2: atomic load - the orphan counter moves on driver completion
	 * threads while this control-side read runs. */
	snapshot->rejected_count = driver_snapshot.rejected_count +
		atomic_load_explicit(&common->orphan_completion_count,
			memory_order_relaxed);
...
{
	SparkModelDriverRuntimeSnapshot snapshot;
	uint64_t driver_submitted_count;
	driver_submitted_count = 0ull;
		...
		common->driver_instance != 0 && common->program != 0 )
	{
		...
		if ( common->driver.interface->snapshot(common->driver_instance,
			...
			return(0u);
		driver_submitted_count = snapshot.submitted_count;
	}
	/* F-A3 (T3): ready only after quiesce has latched admission shut -
	 * otherwise a submission admitted past this check races the teardown.
	 * Escape: the never-submitted instance (zero lifetime driver
	 * submissions) has nothing in flight and no admission window left
	 * open, so it tears down without a quiesce round-trip. */
	if ( common->quiescing == 0u && driver_submitted_count != 0ull )
		return(0u);
	if ( SparkAdapterAvailableSubmissionCount(common->pending,
		common->pending_element_bytes,
		common->pipeline_slot_count) != common->pipeline_slot_count )
