	...
	SparkKvBacking *backing)
{
	...
	/* Random-access read-ahead hint (Linux/glibc). Darwin has neither
	 * posix_fadvise nor the POSIX_FADV_* constants and no equivalent
	 * advisory knob, so the hint simply compiles out there. */
