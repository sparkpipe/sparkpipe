...
LOG=/tmp/fresh_driver_deploy.log
exec >> "$LOG" 2>&1
echo "=== FRESH-DRIVER $(date -u +%H:%M:%S) ==="
...
cp -f lib/model_driver.so lib/model_driver.so.hiddenflip-v1 2>/dev/null
...
echo "DEPLOY updated=$ok unreachable:$miss unit=$UNIT"
n=0; bad=""
for h in $ALL; do
    case " $miss " in *" $h "*) [ "$h" != "spark5" ] && continue;; esac
    s=$(timeout 15 ssh -o BatchMode=yes -o ConnectTimeout=6 "$h" "sha256sum < \$HOME/sparkdata/glm5_next.tp16/lib/model_driver.so 2>/dev/null; sha256sum < \$HOME/sparkdata/glm5_next.tp16/lib/hidden_transport.so 2>/dev/null | cut -d' ' -f1" 2>/dev/null)
    ds=$(echo "$s" | head -1 | cut -d' ' -f1)
    ts=$(echo "$s" | tail -1)
    tr=$(sha256sum < "$HOME/mb_transport/hidden_transport.so" | cut -d' ' -f1)
    if [ "$ds" = "$DRV" ] && [ "$ts" = "$tr" ]; then n=$((n+1)); else bad="$bad $h"; fi
done
echo "VERIFY $n/16 driver+transport bad:$bad"
[ "$n" -ge 16 ] && echo FRESH-DRIVER-OK || echo FRESH-DRIVER-PARTIAL
