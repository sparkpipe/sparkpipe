...
{
	void *initial_bf16,*control_bf16,*candidate_bf16,*gain_bf16;
	float *freqs_f32;
...
static __global__ void ProbeFillBf16Kernel(void *destination,uint64_t count)
{
	...
	if ( index < count )
		((__nv_bfloat16 *)destination)[index] = __float2bfloat16(
			(float)((int32_t)((index * 37u + 17u) % 131u) - 65) / 56.0f);
}
...
static __global__ void ProbeFillFreqsKernel(float *freqs,uint32_t count)
{
	...
	if ( index < count )
		freqs[index] = 0.000013f * (float)(index + 1u);
}

static __global__ void ProbeKvPostKernel(
	void *data_bf16,const void *gain_bf16,const float *freqs_f32,
	const uint64_t *row_positions,uint32_t head_dim,uint32_t rope_dim,
	uint32_t quant_width,uint32_t quant_block,float epsilon)
{
	...
	SparkLmRmsNormRow(data_bf16,gain_bf16,data_bf16,0u,head_dim,epsilon,
		staged_input,reduce_scratch);
	__syncthreads();
	if ( warp * quant_block < quant_width )
		SparkDsv4QuantSimGroup(data_bf16,0u,warp,lane,head_dim,quant_width,
			quant_block,448.0f,0u);
	else if ( warp == SPARK_LM_CTA_WARPS - 1u && pair < rope_dim / 2u )
	{
		base = head_dim - rope_dim + 2u * pair;
		angle = (float)row_positions[0] * freqs_f32[pair];
		cosine = cosf(angle);
		sine = sinf(angle);
...
{
	uint64_t layer_elements = SPARK_DSV4_MODEL_ATTN_HEAD_DIMENSION;
	uint64_t elements = PROBE_LAYER_COUNT * layer_elements;
	uint32_t index;
	...
	PROBE_CUDA(cudaMalloc(&buffers->gain_bf16,elements*sizeof(uint16_t)));
	...
	ProbeFillBf16Kernel<<<(elements+255u)/256u,256u,0,buffers->stream>>>(buffers->gain_bf16,elements);
	...
	{
		uint64_t position = 8192u + index * 89u;
...
static int32_t ProbeTimed(ProbeBuffers *buffers,uint32_t fused,float *elapsed)
{
	...
	for (layer=0u; layer<PROBE_LAYER_COUNT; layer++)
	{
		...
		const void *gain = (const uint8_t *)buffers->gain_bf16 + layer * layer_elements * sizeof(uint16_t);
		if ( fused != 0u )
			ProbeKvPostKernel<<<1u,SPARK_LM_CTA_THREADS,SPARK_DSV4_MODEL_ATTN_HEAD_DIMENSION*sizeof(float),buffers->stream>>>(layer_data,gain,buffers->freqs_f32,buffers->positions_u64+layer,SPARK_DSV4_MODEL_ATTN_HEAD_DIMENSION,SPARK_DSV4_MODEL_ATTN_ROPE_DIMENSION,SPARK_DSV4_MODEL_ATTN_HEAD_DIMENSION-SPARK_DSV4_MODEL_ATTN_ROPE_DIMENSION,SPARK_DSV4_MODEL_KV_QUANT_BLOCK,SPARK_DSV4_MODEL_RMS_NORM_EPSILON);
		else
		{
			PROBE_CUDA(SparkDsv4LaunchRmsNorm(buffers->stream,layer_data,gain,layer_data,1u,SPARK_DSV4_MODEL_ATTN_HEAD_DIMENSION,SPARK_DSV4_MODEL_RMS_NORM_EPSILON));
			PROBE_CUDA(SparkDsv4LaunchRope(buffers->stream,layer_data,buffers->freqs_f32,buffers->positions_u64+layer,1u,1u,SPARK_DSV4_MODEL_ATTN_HEAD_DIMENSION,SPARK_DSV4_MODEL_ATTN_ROPE_DIMENSION,0u));
			PROBE_CUDA(SparkDsv4LaunchQuantSim(buffers->stream,layer_data,1u,SPARK_DSV4_MODEL_ATTN_HEAD_DIMENSION,SPARK_DSV4_MODEL_ATTN_HEAD_DIMENSION-SPARK_DSV4_MODEL_ATTN_ROPE_DIMENSION,SPARK_DSV4_MODEL_KV_QUANT_BLOCK,0u));
		}
	}
...
{
	uint64_t bytes = (uint64_t)PROBE_LAYER_COUNT * SPARK_DSV4_MODEL_ATTN_HEAD_DIMENSION * sizeof(uint16_t);
...
int main(void)
{
	...
	printf("summary control_median_ms=%.6f candidate_median_ms=%.6f gain_percent=%.6f exact=1 layers=%u head_dim=%u\n",control_median,candidate_median,100.0f*(control_median-candidate_median)/control_median,PROBE_LAYER_COUNT,SPARK_DSV4_MODEL_ATTN_HEAD_DIMENSION);
