...
2. Add checkpoint-derived CPU/reference checks for the real layer-0 BF16 q/kv/o, attention output projection, and dense MLP path instead of only checking nonzero progression plus final restricted-logit consistency.
3. Extend the checkpoint-derived prefill/KV fixture beyond the checked B1 context-4 window and compare its post-attention and layer-output activations against an external reference.
