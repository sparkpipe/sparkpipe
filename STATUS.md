...
- `sparkpipe_glm52_artifact_check` fails closed if a live GLM `config.json`, the model-description JSON artifact geometry, the resident decode-stage firmware constants, and the model-description route disagree on the production GLM 5.2 geometry.
...
- Live GLM run setup should call `sparkpipe_glm52_artifact_check --model-dir <GLM-5.2 model dir>` before compiling or launching a resident decode-stage package. The model-description JSON owns the expected HF artifact geometry; the firmware header owns the compiled CUDA module geometry. A stale model description, wrong checkpoint family, or wrong compiled firmware geometry is a hard failure.
