...
2. Add chained hidden-state progression through dense layers 0->1->2, including per-layer KV cache ownership for the B1 context-4 fixture.
3. Add an external checkpoint-derived activation artifact and compare full post-attention and layer-output activations against that reference, not only validator-local CPU math.
4. Add real router/top-k and checkpoint expert weights for sparse layer 3.
