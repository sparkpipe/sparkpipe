...
- Decode-stage CUDA also has an explicit dense BF16 MLP layer-progression mode for GLM 5.2's first dense layers. The live validator can load layer-0 `post_attention_layernorm`, `gate_proj`, `up_proj`, and `down_proj` BF16 tensors from the NVFP4 checkpoint and run them through the resident CUDA stage before final restricted logits.
...
2. Replace the remaining smoke attention tensors with checkpoint-derived deterministic GLM tensor fixtures for q/kv/o projection, cached attention, and MTP verify/commit; restricted logits now have a live `lm_head.weight` row validator and layer-0 dense MLP now has a live BF16 checkpoint gate.
...
4. Split GLM driver programs explicitly into broad decode, final restricted decode, and classifier decode profiles so restricted logits stay in the model-driver final-head path rather than becoming a SparkPipe-wide mask.
5. Add graph capture, transport handoff, and checkpoint-derived multi-layer fixtures inside one or a few model-specific GLM firmware archives.
6. Publish only exact archives that pass numerical and model-stage performance qualification, then compile the GLM model JSON into direct-call drivers.
7. Measure end-to-end stage latency and throughput before deciding whether another module split or fusion is profitable.
8. Add the remote node agent and fixed submission/completion wire path without changing the driver ABI.
