...
- `sparkpipe_glm52_artifact_check` fails closed if a live GLM `config.json`, the model-description JSON artifact geometry, the raw safetensors tensor contract, the resident decode-stage firmware constants, and the model-description route disagree on the production GLM 5.2 geometry.
...
- Live GLM run setup should call `sparkpipe_glm52_artifact_check --model-dir <GLM-5.2 model dir>` before compiling or launching a resident decode-stage package. The model-description JSON owns the expected HF artifact geometry and first raw tensor contract; the firmware header owns the compiled CUDA module geometry. A stale model description, wrong checkpoint family, wrong raw tensor layout, or wrong compiled firmware geometry is a hard failure.
...
- The first raw tensor contract passes on the NVFP4 artifacts whose attention and lm_head tensors are BF16. It intentionally rejects the zAI FP8 artifact for this BF16 resident module because attention projection weights are `F8_E4M3` with `weight_scale_inv`; FP8 needs a separate module or explicit lowering into the BF16 resident ABI.
