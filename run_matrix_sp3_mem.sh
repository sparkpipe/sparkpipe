...
SUMMARY=/tmp/sp2_cells/pass2_summary.tsv
cd "$ROOT" || exit 1
mkdir -p "$OUTDIR" "$MEMDIR"
printf 'cell\trc\twall_ms\tsubmitted\tadmitted\trejected\tstatus_line\trd_peak_rss_mb\trd_hwm_mb\tbat_peak_rss_mb\tbat_hwm_mb\trd_gpu_mib\tbat_gpu_mib\tguard_waits\n' > "$SUMMARY"
...
for cell in $CELLS_LIST; do
  ...
  echo "=== CELL $cell done rc=$rc wall_ms=${wall_ms}ms rdHWM=${rd_hwm}MB batHWM=${bat_hwm}MB $(date -Is) ==="
done
echo MATRIX_DONE
echo "--- PASS2 SUMMARY ---"
cat "$SUMMARY"
