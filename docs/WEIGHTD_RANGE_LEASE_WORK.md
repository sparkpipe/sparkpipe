...
The legacy test_weightd_expert fixture is migrated to v2 48-byte range records and explicit acquire/read/release leases. It preserves missing/corrupt and warm-reattach manifest rejection, cross-model byte equality, pressure/reload, checksum drift, pack size drift, and unknown-key/generation checks, and asserts obsolete Ensure is unsupported. Host CUDA-stub execution passes. Its daemon-VA byte oracle is same-process only; test_weightd_working_set separately covers imported consumer VA. This does not migrate test_weightd_map or establish real GPU qualification.

MapAcquire now uses one overflow-checked monotonic deadline for the initial acquisition, descriptor batches, and checks between CUDA imports. Expiry returns BUSY with any acquired identifier retained for explicit cleanup rather than starting another full timeout. CUDA calls already underway cannot be interrupted. A delayed-import host stub test verifies expiry retains the lease, Destroy refuses it, and subsequent Release cleans it successfully. The default timeout constant is shared with the IPC client.
