...
### 22.6 Resolution of the 22.5 regressions (2026-08-25, HEAD f2281b0 lineage)

1. **stagepack_parity RED - RESOLVED** (concurrent fold completion, verified):
   glm52/default 50,422 verdicts byte-identical, v4/mechanics 5,828 vs frozen
   golden, all dialects green at close.
2. **code-size breach - RESOLVED**: the committed conflict residue in
   tests/test_code_size.py (unparsed markers, two competing CEILINGs) is gone;
   '.tmp-build' work-tree copies are excluded like tmp/ per the 247004b
   precedent; ceiling ratcheted to the exact recount (210,391) with the wave
   ledger naming the AMD island, adapter_common migration (-455),
   node/model_api.c and this repair wave. Gate PASS.
3. **unwired gates - WIRED where they can run, DOCUMENTED where they cannot**:
   restored to TEST_BINARIES + rules + all green: test_glm52_pp7_configure,
   test_glm52_round_major_contract, test_glm52_spec_verify_contract,
   test_glm52_dspark_pack_ingest; stagepack rule deps corrected to the folded
   internal header + shared reader (the format.h dependency pointed at a file
   that no longer exists). Added to PYTHON_TESTS + green:
   test_glm52_dspark_backend_host_syntax,
   test_glm52_cuda_validator_tier2_oracle,
   test_glm52_resident_stage_host_syntax, tests/test_stagepack_parity.py.
   DELIBERATELY not wired, with an owning comment in the Makefile:
   pp7_tap_transport/pp7_stage_role are written against the PP7 [12,11x6]
   speculation-topology API no landed module/adapter carries yet
   (dspark_drafter_role / resident_sequence_capacity drift), and
   dspark_pack_roundtrip is argv-driven by design - its build rule exists;
   its driver leg belongs to the dspark-pack round.

Also absorbed en route: r3_01..03 content survived every consolidation and is
now committed; r3_04's include widening was superseded by the pack reader
becoming self-contained (obsolete, dropped).

## 23. DFlash2 speculative decoding wired end-to-end via the adapter env bridge (2026-08-25)

Audit finding: every stage of the glm52 DFlash2 chain already existed - the
drafter backend archive, the module's env-gated speculator
(SparkGlm52ModuleSpeculatorConfigure/Initialize + full draft/verify execute),
and the adapter's speculation hooks (config stanza, arm, frame building,
verify resolution, crediting). The ONE missing link: nothing translated the
adapter's `speculation_enabled`/`dspark_pack_path` config into the module's
environment contract, which the module reads exactly once at Configure -
inside SparkAdapterLoadDriver, BEFORE ArmSpeculation ever ran. A generated
TP1 deployment therefore armed an adapter whose first speculative frame died
in SCHEMA_ERROR against an unarmed module.

Wiring landed:

1. **SparkGlm52ServingStageSpeculatorEnvironment** (serving adapter): called
   in Initialize between config validation and SparkGlm52ServingLoadDriver;
   stages SPARK_GLM52_STAGE_SPECULATOR=1 and
   SPARK_GLM52_DSPARK_{MANIFEST,CONFIG,SAFETENSORS} as
   <dspark_pack_path>/{manifest.json,config.json,model.safetensors} through
   SparkAdapterSetEnvironmentText (the same-process setenv seam of
   adapter_common). Single-rank-only by doctrine; kill switch
   SPARK_GLM52_SERVING_SPECULATE=0 wins over the config; enabled-without-
   pack-path refuses loudly instead of staging garbage paths.
2. **tests/test_glm52_spec_env_bridge.c** (+ Makefile rule, in TEST_BINARIES):
   white-box over the adapter TU pinning all four outcomes - disabled no-op,
   kill-switch override, missing-pack refusal, happy path staging the three
   joined artifact paths. GREEN.
3. Generator unchanged: tp1_config() already emits the stanza; TP8 stays
   speculation-free because the bridge stages nothing off the single-rank
   shape.

Verified: both adapter flavors build; spec_env_bridge green; C gate sweep
(stagepack/pp7_configure/round_major/spec_verify/pack_ingest) green; parity +
layer_host green; code-size ratcheted for the wiring (+519 ledger entry,
then +16/+519-follow-up recounts during verification on the live tree).

## 24. GLM52 speculation thread-safety MED defect fixed (2026-08-25)

Defect: the CUDA completion callback (SparkGlm52ServingDriverCompletion ->
SparkGlm52ServingCompleteSpeculative) mutates the per-lane speculation
records and the shared neutral-policy machine concurrently with the control
thread's frame builder / draft retrieval / release path - torn draft reads,
lost have_drafts updates, racing pending_stamp/last_accepted_depth, and
unsynchronized SparkGlm52Dspark* policy entry points.

Fix: per-lane spinlock (`_Atomic uint32_t lock` inside
SparkGlm52ServingLaneSpec; zero-init = unlocked) plus one speculator-level
spinlock, with a strict **speculator-before-lane** acquisition order (the
policy draft callback fires from inside EnsureDraft and takes lane locks as
the inner lock - the reverse order can never occur). The four mutation
points:

1. CompleteSpeculative verify loop: accept-depth + stamp writes under the
   lane lock; CompleteVerify under the speculator lock (held across the
   whole round).
2. CompleteSpeculative refresh loop: sequence/draft/have_drafts +
   MarkVerifierTapsReady under both locks.
3. Control thread: PrepareVerifyDrafts (speculator lock spans the sweep;
   per-lane peek/write groups locked), BuildFrame availability precheck /
   draft-id expansion / stamp scratch / clear-all drops (per-lane locks),
   PolicyDraftFunction scan (lane locks, leak-free on every path).
4. ForgetAllSpeculation: CancelSequence under the speculator lock, then
   explicit field clears under the lane lock (a memset would zero the held
   lock itself).

Verified: single-rank + parallel adapter builds clean (-Werror), env-bridge
gate green, pp7_configure/round_major green, code-size gate re-stabilized at
212,727 on a quiescent tree (json/ tree-mirror exclusion adopted; its owner
must consolidate or relocate that 106k-line duplicate).

## 25. Improvement pass (queue empty): first shared-layer consolidation + queue hygiene (2026-08-25)

Queue check (TEAM_TODO.md): three glm52 rows were stale and are now struck
with receipts (validator restore CLOSED - landed §22.5; stagepack collapse
CLOSED - commit 08d2a96, parity 127k+ verdicts; DFlash2 adoption WIRED -
§23, ~100 tok/s class awaits on-device acceptance measurement). The open
item "glm52-named general code -> shared layers" became this pass:

- **Speculation spinlocks moved to the shared adapter skeleton**:
  SparkGlm52ServingSpecLock{Acquire,Release} -> SparkAdapterSpinLock{Acquire,
  Release} in runtime/adapter_common.{c,h} (<stdatomic.h> added there), with
  the header note that callers own any nesting ORDER - the layer guarantees
  only the primitive. 36 glm52 call sites migrated; the glm52-private
  helpers deleted. qwen36's serving adapter - a consumer of the same header -
  rebuilt green against it.
- **Duplication scan result**: pack-load/validate chain is thin dialect
  wrappers over runtime/spark_stagepack_reader.h; tp-collective loading
  delegates to SparkAdapterLoadTpCollective; page-KV wraps SparkPagedKv*;
  byte allocation wraps SparkStageModuleDeviceAllocate. No further
  glm52-private copies of shared-layer code flagged.
- Verified: both adapter flavors, spec_env_bridge, stagepack, pp7_configure,
  round_major_contract, spec_verify_contract, pack_ingest, parity,
  layer_host - all green; code-size gate re-stabilized (213,045) on the
  quiescent tree after the concurrent AMD/bench landings settled.

## 26. Validator segfault FIXED on hardware; publication now blocked by the next honest gate failure (2026-08-25, sparke)

**Root cause of the FixtureSetup SIGSEGV**: `fixture->index_norm_bias` was
never allocated - missing from the AllocMatrix chain - and line 831 then
`memset` the zeroed struct's NULL host pointer. Guaranteed SIGSEGV
(`__memset_zva64 <- main`) on every run, exactly as reported from
spark1/8/9/a/b/d/e/f. A mechanical use-before-alloc sweep over ALL fixture
matrix fields confirms this was the only instance.

Fix + sync: AllocMatrix added to the chain; merged with sparke tree's two
extra weight wirings (attn_output_bf16 / post_attn_norm_bf16 - that build's
layer contract consumes them); pushed so local and sparke trees carry an
identical validation source (md5 c7c5866b...).

Hardware verification on sparke (CUDA 13.0, sm_121a):
- validate target now runs past fixture setup: **tier=dense PASS**
  (hidden rel_l2 6.0e-3, residual 3.6e-3, determinism bit-exact x12288).
- tier 2a router selection: top-8 SETS exact, weights within 6.2e-5.
- **NEW BLOCKER (next gate doing its job)**: check=routed_expert_forward
  relative_l2=0.336 cosine=0.943 max_abs=882,688 -> Error 1. Evidence points
  at an expert SCALE/PAYLOAD layout contract mismatch between the validator
  fixture's separate payload/scale buffers and this tree's weight-codec
  scale-tensor layout (scales grouped at the front of data). Owning round:
  reconcile fixture shaping vs kernels/weight_codec.cuh on that tree.

Publication/deploy/benchmark: BLOCKED BY DESIGN - publish_variants refuses
without a passing validator and bypassing is prohibited; no model_driver.so
emitted; fleet left in the clean stopped state. compute-sanitizer available
at /usr/local/cuda-13.0/bin/compute-sanitizer on sparke for the next leg.
Host-side oracle selftest PASS after the edit.
