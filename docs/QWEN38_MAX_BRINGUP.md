...
- **TP16 rank-local production packs (R1 mitigation, LANDED as tooling):**
  `tools/qwen38_tp16_pack_production.sh` → `tools/qwen38_tp16_stagepack.py` produce the
  sixteen `qwen38.tp16.<precision>.rankNN.stagepack.<sha256>` packs per
  DATAFILE_NAMING: routed experts sliced 512/16 = 32 consecutive experts per rank
  (`--expert-codec mxfp4_e2m1`, the capacity choice, or `fp8_e4m3` vendor passthrough),
  non-expert tensors verbatim-replicated, MTP packed-not-served, receipts correctly
  labeled, placement manifest binds all ranks. The MXFP4 codec is the vectorized twin of
  the family scalar `quantize_mxfp4_e2m1` and is byte-equality-gated against it
  (`--self-test`, plus per-expert spot checks at pack time); validated end-to-end on a
  synthetic mini-checkpoint (inventory math, header/entry bytes, alignment,
  content-addressed naming, manifest partitioning). Footprint arithmetic per rank:
  ~74 GiB expert share + ~91 GiB replicated dense spine ≈ **165 GiB/rank today**,
  closing to ≈99 GiB/rank when attention/GDN head-slicing lands (TP4xPP4 variant would
  be ≈80 GiB). Projections, not measurements.
- **Loader dependency for those packs (LANDED + host-tested):** the
  family-local delta accepts, for KIND_MOE_W1/W3/DOWN only, rows ==
  (EXPERT_COUNT/tp_degree)·per_expert_rows with formats MXFP4_E2M1|FP8 when
  SPARK_QWEN38_STAGE_TP_DEGREE/_TP_RANK are set; header geometry stays canonical and
  coverage checks are unchanged. A mode latch refuses mixed whole/sliced packs.
  Verified host-side against a fabricated full-geometry sliced-MXFP4 pack
  (`tools/qwen38_tp16_stagepack.py --fabricate-rank`, zero-filled sparse payloads):
  refused without TP env, accepted with it, tampered rows refused. The launcher
  increment has since LANDED: both grouped launchers take the table's per-expert width,
  suppress the payload shift for rank-local views, and keep global route-offset
  windowing — host harness runs a full decode step status=0 on an FP8-sliced pack (3
  grouped-scalar launches at rpe 2048/8192), while MXFP4 still fails closed at
  `mxfp4_moe_execution_not_wired` pending the sm121 tensor-core wiring. Two latent traps the new tool avoids: the wire code
  for MXFP4 is **3** per firmware.h (qwen38_stagepack.py's constant says 7), and its
  `copy_mxfp4_tensor` is called but never defined.
...
# smoke binaries now have root-make rules (archive must exist first):
make build/test_qwen38_pack_load build/test_qwen38_execute
# mechanical S1/S2 against a fabricated zero-weight pack (GPU spark):
make smoke-qwen38-executes
# host-side loader-delta suite (runs ANYWHERE, no CUDA needed — stubbed
# launchers, real module code): admission gate, sliced-FP8 execution,
# MXFP4 fail-closed:
make test-qwen38-loader-delta
# manual link fallback, identical to the make rules:
cc -o build/test_qwen38_pack_load tests/test_qwen38_pack_load.c \
   build/modules/qwen38_resident_decode_stage/libqwen38_resident_decode_stage.a \
   -lcudart -lcuda -lstdc++ -lpthread -lm
cc -o build/test_qwen38_execute tests/test_qwen38_execute.c \
   build/modules/qwen38_resident_decode_stage/libqwen38_resident_decode_stage.a \
   -lcudart -lcuda -lstdc++ -lpthread -lm
```
...
| `max_active_sequences` | 1024 | 1 (b1) | module refuses > 409 (`config_batch_too_wide`, MoE tile guard); the adapter now CLAMPS active/input widths to 400 with a log line, but b1024 still cannot start - its `resident_sequence_capacity` 4096 blows the KV pool arithmetic (next row) |
...
ProbeMemoryMode all pass (rank log shows `qwen38_stage tp_create_failed status=…`) —
see QWEN38_MAX_TP16.md for the three suspects (backend .so
...
Success is SILENT by policy (silent-on-success sweep): module Initialize prints
nothing when it passes — failures print `qwen38_stage initialize_failed status=N`
plus a cause line (`config_slice_invalid`, `config_position_mismatch`, `pack_*`,
`tp_apply_topology_failed`, `tp_probe_memory_mode_failed`, `tp_create_failed`).
Pass per rank = a clean log with no such lines AND identical emitted tokens
across the four ranks for the same submission (tools/qwen38_tp4_e2e.c pattern).
The old `tp_collective_open degree=4 …` boot line was removed in the same sweep,
so TP engagement can no longer be read from logs: verify group-B env on every
rank directly, e.g. `tr '\0' '\n' </proc/<pid>/environ | grep SPARK_QWEN38_STAGE_TP_DEGREE`.
A rank without it is running tp=1 — stop, fix env (risk R3).
...
- **R1 — Capacity wall (gates S5+):** ~148 GB/rank vendor-FP8 residency > ~107 GB usable.
  ...
  Decide BEFORE burning a fleet window. The MXFP4 option is now PRODUCIBLE:
  `tools/qwen38_tp16_pack_production.sh` (see §3) emits the TP16 rank-local MXFP4 packs;
  the remaining gates are the ValidateEntry loader delta and, for the full fit, the
  dense head-slicing increment.
- **R2 — Rank-local packs don't exist:** packer emits whole-stage slices; the TP-rank
  ...
  blocked on both halves landing together. Partially unblocked: the TP16 rank-local
  PACKS are now producible (§3); what remains is the module's rank-local-view dispatch
  (ValidateEntry delta) plus the adapter's TP-rank frame mapping.
- **R3 — Silent tp=1 fallback:** the module takes its TP geometry ONLY from the group-B
  ...
  launcher AND verify each rank's environment directly
  (`tr '\0' '\n' </proc/<pid>/environ | grep SPARK_QWEN38_STAGE_TP_DEGREE`) —
  the boot line that used to expose tp engagement (`tp_collective_open …`)
  was removed by the silent-on-success sweep, so logs no longer reveal the
  fallback. FIXED in the adapter (this pass): TP degree/rank are now DERIVED
  from the stage json into the module environment (fill-if-absent — launcher
  exports still win), a launcher export that CONTRADICTS the json is refused
  before driver load, and degree>1 with missing backend/hosts exports fails
  loudly at Configure instead of running replicated tp=1. Keep exporting the
  full group-B set in launchers; the derive line
  (`derive TP geometry from stage config: …`) is the new boot marker.
- **R4 — TP collective CAPACITY_EXCEEDED:** unresolved backend contract detail (S3).
- **R5 — Checked-in runtime_limits exceed module caps (PARTIALLY FIXED):** the adapter
  now clamps max_active/max_input to 400 (P0: a 512 deployment used to die at init with
  `config_batch_too_wide`; it now comes up clamped with a loud log line). The b1024 spec
  STILL cannot start — its `resident_sequence_capacity` 4096 × `blocks_per_lane` explodes
  the KV pool allocation, and inflight 4 exceeds the single-slot adapter. Use b1-shaped
  limits; the real width fix is the launch-planner GEMM move (audit §1.6).
