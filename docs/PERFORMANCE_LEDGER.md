...
| GLM 5.3 Flash | — | FIRST REAL TOKENS LIVE: non-zero plausible ids from the real distribution (deterministic per-prompt; repetition pattern suggests decode-state refresh nuances pending the KDA agent's verification). M5 + COMPSEC-17 next. 36h from cold architecture read to serving | — | — | fleet live | 2026-08-30 |
