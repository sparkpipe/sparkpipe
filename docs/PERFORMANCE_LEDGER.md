...
| K3 | 18.0 | 16-RANK CONTRACT PROVEN (4-rank slice impossible: 3 code locks + transport receipt; rank12 needs rank8). All module fixes verified; ranks 12-15 staged 4/4 digest-verified; ASAN smoke CLEAN (destroy-path item CLOSED not-reproducible); ~96GiB/rank fits 110 alone (exclusive window vs glm5_next). Critical path: stage-0/2 packs (build re-armed on sparke) → all-16 wave → B1 (PR #733) | — | not measured | main + PR733 | 2026-08-28 |
| Qwen 3.8 Flash | — | PACKS 16/16 DONE + HELD at operator pause: bf16 repackage-only (policy-clean), byte-trace verified, serving stack staged on all 16 (wave = one command); bf16 expert kernels bit-exact; official FP8 fetched+verified as fast-follow. On resume: wave → B1 → 32K cell → COMPSEC-17 (a602037 merged) | — | — | main cdeb3dd | 2026-08-29 |
| GLM 5.3 Flash | — | REAL TOKENS + TWO BUGS ROOT-CAUSED: (1) HC-post double-reduce exploded streams to 1e18@L17 (fixed dace693); (2) packer sliced fused q|k|v|beta rows contiguously — context-free degeneration, the repetition's cause (fixed 1dac68b). COMPSEC-17 RUN by coordinator on the pre-fix pack: all 17 degenerate (the gate WORKED — correctly refusing to bless a broken model). Fixed pack building (rank 0 on spark0, ceph-bound); then 16-rank rebuild + COMPSEC + M5. O_norm/conv/attn byte-exact | — | — | main 913b648 | 2026-08-30 |
