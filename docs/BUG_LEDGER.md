...
| Finding | State |
|---|---|
| R1 screened head — FLASH (glm5_next) | WIRED + HARDWARE-QUALIFIED (spark4 GB10, 2026-08-30): certified-vs-full token EXACT on all 6 runs (shard 9680 + full 154880, 3 seeds; scores to bf16 rounding). Speed: 5.1-5.3x at the deployment shard (1.73→0.33 ms), 2.7-3.0x full-vocab. Cell + receipts: modules/glm5_next_resident_decode_stage/validation/flash_decode_cell*. glm52/k3 remain unwired. |
| R3 flash-decode split-K — engagement | ENGAGED + HARDWARE-QUALIFIED (spark4 GB10): below-threshold byte-identical, deterministic, split==base (max_rel 0.0) at 8K and 32K positions at flash geometry (Glm5NextKv paged pool, 4 heads/rank). Speed: 12.3x @8K (0.46→0.037 ms), 14.6x @32K (1.83→0.125 ms) per layer-class. Receipts in validation/flash_decode_cell*. |
