...
Cleanup retries now remember successful completion recording and clear the wave address immediately at that boundary. A release retry therefore never re-records an already recorded/retiring lease or exposes the address for further kernels. The worker retries IO/BUSY cleanup once; persistent failure still retains ownership and needs recovery. The dispatch test covers a failed release followed by successful retry without a second completion record.

## Consumer/daemon manifest agreement

Lazy attachment now returns SHA-256 over the daemon loaded manifest in canonical group order, with explicitly little-endian layer/expert/kind/version/offset/length fields and all range checksums. The composed consumer compares this with its own parsed manifest before creating the map or worker. The fingerprint describes the in-memory range table, not a later pathname read. A test changes a sidecar checksum after consumer parsing but before daemon attach and confirms HASH_MISMATCH with no published pack. Normal working-set and expert lease regressions pass. Both sides must be rebuilt together for the changed draft lazy-result frame. This closes manifest disagreement at attachment, not the separate remaining pack publication and full numerical qualification gates.
