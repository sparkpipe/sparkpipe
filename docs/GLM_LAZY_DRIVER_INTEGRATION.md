...
Keep the request slot claimed until acquisition, GPU work and lease cleanup have finished. Partial expert launches still need completion recording and draining. A persistent cleanup or CUDA drain failure retains the chain, slot and pins with a diagnostic; recovery beyond the immediate retry remains unfinished. Socket disconnection alone does not release daemon pins because it does not prove GPU completion.
