...
GLM destruction waits for slots, then observes worker idleness with a monotonic timeout before releasing resources. This covers the interval between final slot release and worker-task return. The idle observation requires submissions to have stopped and rejects calls from the worker itself. Failed stream drains retain resources. The idle worker must be joined before destroying maps and spine. `LazyPackDestroy` preserves partial cleanup state on failure and forbids new slices once teardown starts. A nonnull failed Create result is cleanup-only. GLM startup currently retains resources until process exit if cleanup itself fails; recoverable ownership reporting and other preexisting submission-error drain paths still need work.
...
Build `build/sparkpipe_weightd` and `build/weightd_lazy_consumer`, then run `python3 tools/weightd_lazy_pair.py --daemon build/sparkpipe_weightd --probe build/weightd_lazy_consumer`. The controller owns a fresh temporary fixture and daemon, starts two separate consumers with sets {0,1} and {1,2}, waits for both to hold their leases, then permits completion/release. The pool holds three of the four fixture chunks. While both holders remain at the barrier, a separate pressure consumer must receive CAPACITY_EXCEEDED for the fourth chunk. After their release, another consumer reads experts {2,3}, requiring eviction of unpinned storage within the same pool. Both consumers read payload and scale bytes through consumer-local maps and compare their contents. Child processes have bounded waits and are terminated on failure. The fixture writer is for this test only, not production model publication.
