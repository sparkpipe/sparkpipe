...
|----------|--------|--------|--------------|-----------|
| model-families/qwen36 | 5120 | 64 | Qwen 3.8-27B | qwen38_27b |
| model-families/qwen38 | 8192 | 92 | Qwen 3.8 Max | qwen38_max |
| modules/qwen36_resident_decode_stage | | | serves Qwen 3.8-27B | qwen38_27b_resident_decode_stage |
...
was copy-pasted from qwen36 without updating the model name.
...
Phase 1: Rename directories (breaks builds, fix immediately)
  git mv model-families/qwen36 model-families/qwen38_27b
  git mv model-families/qwen38 model-families/qwen38_max
  git mv modules/qwen36_resident_decode_stage modules/qwen38_27b_resident_decode_stage
...
Phase 2: Rename files
  ...
  for f in spark_qwen36_*; do git mv $f ${f/qwen36/qwen38_27b}; done
  (same for qwen38_max, renaming qwen38 → qwen38_max)

Phase 3: Update all content (sed pass)
  find . -type f \( -name "*.c" -o -name "*.h" -o -name "*.cu" -o -name "*.cuh" -o -name "*.py" -o -name "*.sh" -o -name "Makefile" \) -exec sed -i 's/qwen36/qwen38_27b/g; s/QWEN36/QWEN38_27B/g; s/Qwen36/Qwen38_27b/g' {} +
...
- The 27B rename (qwen36→qwen38_27b) is the priority (169 files)
