...
qwen38_27b, the TP4 band (spark0-3), and the perf targets (the qwen38_27b TP4
...
**What exists.** A fully wired, frozen 27B driver under the *qwen38_27b* name —
`model-families/qwen38_27b/`, `modules/qwen38_27b_resident_decode_stage/` (~9k LOC:
module 2307 + cuda 2208 + serving adapter 1724 + tp 403 + validation 1209 +
reference 567), `inference/llms/qwen_3_6/`, `tools/qwen38_27b_stagepack.py` (692).
Geometry pinned: hidden 5120, 64 layers, 16×(3 GDN→1 attn), GDN 16/48 heads,
attn 24/4, dense FFN 17408, MTP 1 (`spark_qwen38_27b_model.h:7-25`;
`qwen38_27b_authoritative.json:15-44`). The TP4 band deployment artifacts are
already renamed to "qwen38" on the *runtime* side while still building the
*qwen38_27b* module: `qwen38_tp4_build.sh:11,22` (`make -C modules/
qwen38_27b_resident_decode_stage`), `qwen38_tp4_deploy.sh:4` (runtime root
`qwen38.bf16.tp4`), deploy env `SPARK_QWEN38_27B_STAGE_MTP=1 …
...
(`QWEN38_27B_TP4_PERF.md:6,19-28`). Entropy of the real weights: 10.52 bits
order-0 = 1.52× lossless headroom (`QWEN38_27B_TP4_PERF.md:35-36`). These are
**Qwen 3.6 27B** weights (54.5 GB BF16, 27.27B params;
`QWEN38_27B_BF16_SPEED.md:6-8`). There is **no Qwen row in PERFORMANCE_STATUS.md**
...
`qwen38_27b_resident_decode_stage` (`spark_qwen38_27b_serving_adapter.c:55-62`).
(3) `must_work_targets.json:59-76` still pins `qwen38_27b_27b_bf16`.
...
- **What.** New `model_contracts/qwen38_27b_authoritative.json` +
  ...
  the `qwen38_27b_27b_bf16` must-work target (`must_work_targets.json:59-76`) in the
...
### 2. Finish the rename qwen38_27b → qwen38_27b; fix the 4-way identity drift + fleet registry
- **What.** A real `qwen38_27b` family/module/adapter/stagepack identity; collapse
  the conflicting ids at `spark_qwen38_27b_serving_adapter.c:55-62`; fix
...
- **Why.** DRY/structural: one model should have one name; today the same
  artifact is `qwen38_27b`/`qwen3.6-27b`/`Qwen3.8-27B` in four adjacent lines,
...
- **What.** The new qwen38-27b module fills `SparkAdmissionPolicyTable` and calls
  `SparkAdmissionEvaluateShape` instead of cloning the qwen38_27b admit ladder
  (`spark_qwen38_27b_resident_decode_stage_module.c:2031-2116`, serving
  `spark_qwen38_27b_serving_adapter.c:981-1010`). The proposal explicitly orders
...
- **Delta.** −~85 lines vs carrying the qwen38_27b ladder (table ≈ 15 lines).
...
- **What.** Fill `SparkKvModelTable` for qwen38-27b's **16** full-attention
  ...
  token/layer, 48 GDN layers carry no KV. Re-base qwen38_27b's
  `SparkQwen38_27bModuleOpenKvTier` as the reference (`PROPOSAL_KV_SEAM.md:464-478`).
...
- **What.** Replace the hand-rolled qwen38_27b MTP accept loop
  (`spark_qwen38_27b_serving_adapter.c:1280`) and the 3-frame decode-draft/verify/
...
- **Why.** DRY win: qwen38_27b's MTP speculation is 0-match against the shared
...
- **What.** Add a qwen38-27b geometry to `tools/perf_estimate.py:212` (PERF) and
  ...
  (`QWEN38_27B_TP4_PERF.md:53-66`) onto the 3.8 27B weights; record a
...
- **Why.** Makes the level visible and the "N× single-spark" accounting honest
  (`QWEN38_27B_TP4_PERF.md:8-13`). The current 13.2 tok/s and 1.52× entropy are
...
- **What.** ANS codec over the BF16 weight stream (measured 1.52× headroom;
  `QWEN38_27B_TP4_PERF.md:35-51`), decoded in the small-batch GEMM tile staging so
...
- **Why.** **Perf level 90% SOTA → match SOTA** on single-stream decode at
  ...
  (`QWEN38_27B_TP4_PERF.md:27`); shrinking the bytes is the only way past it
...
- **What.** B64 collective sends 1280 of 5120 changed hidden columns instead of
  the full 655,360 bytes; B64 step ~404→~150 ms (`QWEN38_27B_TP4_PERF.md:56-57`).
...
- **What.** Screen the B1 full-vocab head read 2.8→~1.5 ms and fuse the GDN
  ...
  `QWEN38_27B_TP4_PERF.md:25-28,59-60`.
...
- **Why.** **Perf level on B64+ (state-bound knee)**: GDN state is 310 MB/seq/
  token fp32 and overtakes weights past ~B95 (`QWEN38_27B_BF16_SPEED.md:11-13,45-51`);
  bf16 halves the stream and is "the biggest single lever the model has at
  B64+" (`QWEN38_27B_BF16_SPEED.md:20-23`). It is a numerics question on a
...
- **Coordinator:** land the #1 contract PR (gate-breaking); fix the
  ...
  qwen38_27b/qwen38/qwen38-27b don't fork three ways (`DRY_CONSOLIDATION_PLAN.md:14-17`).
...
grep/read of COORDINATION.md, the qwen38_27b family/module/adapter/stagepack, the
...
KERNEL_CONTRACT_CARDS), the perf estimators, QWEN38_27B_TP4_PERF.md and
QWEN38_27B_BF16_SPEED.md, fleet_registry.json, must_work_targets.json, and
