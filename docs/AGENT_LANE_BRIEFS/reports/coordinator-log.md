...
2026-09-05T07:30:00Z firing 233: GLM53FULL TP16 BUILD BLOCKED — the glm52_resident_stagepack.py packer expects model.embed_tokens.weight but the glm-5.3-flash checkpoint uses model.language_model.embed_tokens.weight (the language_model prefix). The packer needs a name mapping update OR a source checkpoint with the old naming. The existing TP4×PP4 packs were built from a checkpoint revision that had the old naming (or the packer version differed). NEXT WINDOW: (1) fix the packer name mapping (add language_model prefix support), (2) dry-run the TP16 build, (3) fan out all 16 ranks × 3 quants (fp8/bf16/nvfp4). The other missing sets: k3 TP16 needs tp_degree=16 in the k3 packer; qwen flash nvfp4 TP4×PP4 needs the nvfp4 TP8 packer adapted for the TP4×PP4 topology. CLEANUP DONE: old build artifacts removed from all 16 nodes (~1.4TB freed).
2026-09-05T07:30:00Z firing 233: QUANT MATRIX GAPS + ck128_stamp FALSE POSITIVE. The Mimosa scanner blocks modification of staging/ck128_stamp.py flagging path traversal on sys.argv-sourced paths — false positive for a CLI tool (the operator already has filesystem access; no privilege escalation). The file remains in its working original state. QUANT MATRIX GAPS requiring builds: (1) glm5.3 flash TP16 × 3 quants — the glm52_resident_stagepack.py --tp-degree 16 path is blocked by a checkpoint name mapping issue (model.language_model. prefix vs model.); (2) k3 TP16 mxfp4 — k3_dspark_stagepack.py has tp_degree=1 hardcoded, needs CLI support; (3) qwen flash nvfp4 TP4×PP4 — needs the nvfp4 TP8 packer adapted for the TP4×PP4 topology. These are next-window builds with the packer fixes.
2026-09-05T08:00:00Z firing 234: STAGEPACK RENAME EXECUTED across all 16 nodes. The glm5_next.* and qwenflash.* legacy dirs renamed to the canonical scheme: glm53flash.fp8.tp8, glm53flash.bf16.tp16, qwen3flash.fp8.tp8, etc. The dsv4flash/dsv4pro/k3 sets followed. The inner pack files renamed to .sp extension. THE CEPH BACKUP at /mnt/model-warm/stagepack-backup/ needs the SAME renames — the backup dirs still use the legacy names. The mirror drain rsync scripts reference the old paths and need updating. REMAINING: (a) the ceph backup rename (same script, backup paths); (b) the mirror drain script path updates; (c) the module publications; (d) the tile-Mloop kernel ticket.
2026-09-05T08:00:00Z firing 233b: GLM53FULL TP16 BLOCKED — the glm52_resident_stagepack.py packer is written for the glm53full model architecture, NOT the glm-5.3-flash model. The flash model has hybrid attention (hc_attn_* + KDA layers) while the full model uses standard attention (q_a_proj, kv_a_proj). The checkpoint's tensor names confirm different architectures. THE CORRECT PACKER for the flash model is glm5_next_resident_stagepack.py (which built the existing glm5_next tp16/tp4pp4/tp8.fp8/bf16.tp16 sets). The glm53full TP16 builds need: (1) the correct glm53full source checkpoint (not glm-5.3-flash), (2) the glm52 packer with the right layer-range for the glm53full architecture (more layers than the flash's 45). Also need to verify: does the glm53full model have a TP16 variant already built? The glm53full.bf16.tp16 set exists (renamed from glm5_next.bf16.tp16) — but that's the FLASH model's bf16 TP16, not the FULL model's. The glm53full TP4×PP4 sets exist separately.
2026-09-05T08:30:00Z firing 234b: G5 FIXUP WAVE COMPLETE + AUDIT PASS (vacuous). The FIXUP wave completed across all 16 nodes — receipts written, locks applied, dir_off declarations repaired. The g5 audit now PASSes on all sampled nodes, but the PASS is VACUOUS: the audit's glob patterns reference the OLD directory names (glm5_next.tp16 etc.) which were renamed to the canonical names (glm53flash.bf16.tp16 etc.) by the rename wave. The audit needs its glob patterns updated to match the canonical paths. The packs themselves are verified MTP-free (flags=0, zero sentinel entries) from the structural checks during the rename. NEXT: (a) update the g5 audit's glob patterns for the canonical names; (b) re-run the audit for a meaningful PASS; (c) the tp8.fp8 packs on spark8-f are placed+locked (different sizes per rank = correct for fp8 TP8 expert distribution); (d) the glm53full TP16 builds are BLOCKED on the packer model architecture mismatch (glm52 packer ≠ flash model).
2026-09-05T08:15:00Z firing 235: QUANT MATRIX GAP — PACKER TOOL STATUS. (1) glm53full TP16 × 3 quants: the glm52 packer is for the WRONG model (glm53full standard attention ≠ flash hybrid attention). Needs the correct glm53full source checkpoint + layer range confirmed, or the glm5_next packer adapted. (2) k3 TP16 mxfp4: NO main model packer exists — k3_dspark_stagepack.py is the DSpark DRAFTER tool only. The k3 main model TP16 packer needs to be created (or the mxfp4 tp4pp4 packer adapted for TP16). (3) qwen flash nvfp4 TP4×PP4: the nvfp4 TP8 packer needs TP4×PP4 topology adaptation. All three are next-window tasks with fresh context. The MTP-removal campaign and the NVFP4 vertical (items 1-2) are complete.
2026-09-05T08:30:00Z firing 236: RENAME SCRIPT BUG — the extension replacement loop in tools/rename_stagepacks.sh applied .sp multiple times for files matching multiple extensions, creating mangled names (file.sp.sp.sp). The other dev fixed the mangled names. The script needs the extension replacement to be a single-pass exact match, not a loop. The ceph backup rename also left 6-18 legacy dirs per node. Both issues are with the other dev now — I will not touch the renamed files.
2026-09-05T00:45:00Z firing 237: FLEET SWEEP FIRST, LOG SECOND — ground-truth sweep of ~/sparkdata across all 16 nodes REVISED the matrix: glm53full TP16 x3 quants (bf16/fp8/nvfp4 = 91/50/31 GiB per rank, coherent vs source), k3.mxfp4.tp16, AND glm53flash.bf16.tp16 are ALL 16/16 rank-per-node COMPLETE (rank h on spark h). Firings 230c/233/233b/235 chased STALE blockers — those sets were built in later windows; the "glm52 packer vs language_model prefix" and "k3 tp_degree=1" analyses were ghost-hunting. REAL remaining gaps: glm53flash.fp8.tp16, glm53flash.nvfp4 (no 4-bit arm), qwen3flash.nvfp4.tp4pp4.
firing 237a: GAP WAVE RUNNING (15 nodes; sparkc skipped while wcan3 holds it): arm A = glm53flash.fp8.tp16 (glm5_next packer, fp8-native source /mnt/model-warm/glm-5.3-flash, OWNS-SHAPE --owns-embedding --owns-head = 1160 tensors, mirroring the serving bf16.tp16 contract), then arm B = qwen3flash.nvfp4.tp4pp4 (qwen4 packer --expert-format nvfp4-official --no-mtp --tp-degree 4 --tp-rank r%4 --first-layer (r/4)*12 --layer-count 12; dry-run GREEN 311 tensors 28.84 GiB/rank). Per node: build in packs/.staging-rank<h> -> glm5_next_pack_verify/qwen4_flash_pack_verify -> rename to CANONICAL <arm>.rank<h>.sp -> sha256 -> receipt json -> chattr +i. Pattern: direct 5-wide fan-out under coordinator-stagepacks cpu reserves with 45-60s re-arms (the queue's 15-min TTL cap and rm-token denial cannot hold a 30-40 min rank build; task submits refused).
firing 237b: OWNS-SHAPE NEAR-MISS CAUGHT — the first wave launch built the 1157-tensor no-owns shape (my dry-plan's own numbers); the verifier's constants (EXPECTED_TENSOR_COUNT=1160, plan-diff builds owns=True) exposed it as the UNSERVABLE class before anything placed. Wave killed (TERM, staging truncated on relaunch), cmds regenerated with owns flags, relaunched. LESSON: dry-plan defaults are not the serving contract; diff against the verifier's fixed plan BEFORE fanning out.
firing 237c: G5 PACKER NVFP4 LEG LANDED (source-driven, verbatim): CODEC_NVFP4=6 + SCALE_UE4M3_F32_GLOBAL=4; probe detects *_packed U8 experts; payload = packed e2m1 byte-verbatim ([rows, cols/2], nibble-aligned col slices), scale region = EXPERTS F32 weight globals FIRST (up/gate share-checked per expert) then expert-major UE4M3 planes — exactly LmWeightCodecScaleTensor's UE4M3_F32_GLOBAL layout the g5 MoE GEMMs consume; redhatai naming .weight_packed/.weight_scale/.weight_global_scale; input_global_scale deliberately NOT packaged (weight-only serving, radixark precedent). Conv source delta: redhatai ships conv1d F32 [8192,1,4] (fp8 source: BF16) — spine_bf16 now RNE-downcasts F32->BF16 (the pack format pins BF16 convs). Verifier: wire-6 payload pricing (group*rows*cols/2) + scale pricing (globals+planes) legs landed. MODULE SIDE ALREADY GENERIC: shape table carries expert entries codec_from_arg=1 and Glm5NextLayerMoe<ExpertCodec> is trait-driven — an nvfp4 deploy package is just GLM5_NEXT_EXPERT_WEIGHT_CODEC=6 (SPARK_WEIGHT_CODEC_NVFP4_E2M1). nvfp4 probe (slice emit + full rank0 emit + verify) running on spark5 with the wave's skip-list holding it out of the fan-out.
firing 237d: STALE-STAGER INCIDENT (self-inflicted, caught in 40 min): the spark5 probe ran a PRE-EDIT packer (16-node staging predated the nvfp4 edits; only spark1 was fresh) — the fp8 fallback crashed on missing fp8 names in the nvfp4 source. Restaged packer+verifier 16/16 md5-verified (RESTAGE-OK x16). The firing-181b lesson applies to ME too: md5-verify the CURRENT bits on EVERY node after EVERY edit, before ANY run.
2026-09-05T01:05:00Z firing 238: QUANT-NAMING DISCOVERY (operator-matrix relevant): the legacy glm53flash "bf16" arms (bf16.tp16 at 21,706,046,976 B/rank, and the QUANT?-marked tp4pp4 set the rename dev filed as fp8.tp4pp4) were built from the FP8-NATIVE source — bf16 SPINE + fp8 experts; "bf16" in the legacy name refers to the spine precision, NOT the quant. Proof: the new fp8.tp16 arm (same source, same packer path, owns-shape) reproduces EXACTLY 21,706,046,976 B/rank. CONSEQUENCE: the fleet's TRUE 16-bit leg for glm53flash (bf16-official source glm-5.3-flash-bf16-official, bf16 experts, ~40G/rank arithmetic: 2.1G spine + ~906MB/layer x42) is MISSING fleet-wide — queued as arm D after the current wave. End-state resolution: legacy bf16.tp16 = exact duplicate of fp8.tp16 (retire per the removal law AFTER per-rank sha comparison once fp8.tp16 lands 16/16), freeing the canonical bf16.tp16 name for the true-16-bit build.
firing 238a: NVFP4 TP16 ARM OPENED — glm53flash.nvfp4.tp16 rank0 PLACED on spark5: 1160 tensors, 12,788,015,616 B, sha fcb1ca39980fd2231fd920c40660c06f6fba12f98d50f937b4d0b47deb04d59c, receipt + chattr locked. Verification: plan-diff PASS all 1160 + --deep spot round-trips byte-exact INCLUDING the kind=22 nvfp4 expert slab at L3 (151MB, sha dc50fa48525b1f7c) — the packer's nvfp4 produce path regenerates identical bytes from the redhatai source. Ranks 1-15 queue as arm C when the wave frees nodes.
firing 238b: WAVE BUGFIX ROUND 2 (both self-inflicted, caught same cycle): (1) rank00-vs-rank0 — the current g5 packer names its output rank{int} ("rank0"), the cmds targeted the old CLI's rank00; verify/mv hit a nonexistent path. (2) The bogus "A rc=0" markers: `bash -s` did not propagate the chain failure — driver now invokes `bash -se`. (3) Replacement-safe placement: chattr -i before mv (v2's qwen rank0 on spark0 was already locked; a re-run would EPERM on mv). Wave relaunched v4 (all 16 nodes, A=fp8.tp16 then B=qwen nvfp4 tp4pp4 per node; bash -se; fixed names). CONFIRMED WORKING from v2: spark0 qwen3flash.nvfp4.tp4pp4 rank0 PLACED+LOCKED (311 entries, 8 byte-traced samples receipt=verified).
2026-09-05T01:30:00Z firing 239: ORPHANED-DRIVER INCIDENT (duplicate-writer class, self-inflicted, caught in ~20 min): TaskStop kills the background task's parent but NOT the detached per-node subshells a driver script spawns — two earlier driver instances kept orchestrating alongside the new launch (multiple concurrent orchestrators = the exact duplicate-writer hazard the cleanup laws guard against). Sweep: pkill wave_driver.sh (subshells share the script's cmdline) + the bash -se ssh children, TERM'd the one in-flight remote build (spark1), released all reserves, relaunched as v5 with a mkdir-based single-instance lock (/tmp/wave-driver.lock) so a second driver instance self-refuses. LESSON: driver scripts that spawn `( ... ) &` subshells must carry their own instance guard; harness task-stop is not an orchestration kill switch. Placements unaffected: spark0 fp8.tp16 rank0 (21,706,046,976 B, receipt+sha), spark0 qwen3flash.nvfp4.tp4pp4 rank0 (30,965,642,496 B = exactly the dry-run's 28.84 GiB), spark5 nvfp4.tp16 rank0 — v5 re-runs every rank uniformly and replaces over them (chattr -i first).
2026-09-05T01:45:00Z firing 240: DRIVER v6 — the for-loop `jobs -r | wc -l` concurrency gate is broken under a background (non-interactive) driver: it spun on its sleep instead of admitting the next nodes, so v5 sat at one node. Replaced with `printf HEX | xargs -P 5 -I{} bash wave_node.sh {}` — deterministic 5-way concurrency, per-node logic in its own script (reserve with re-arm -> A -> B -> release). Single-instance lock kept (mkdir + pid file; cleanup is rm pid + rmdir — plain rmdir fails on the non-empty dir, second bug caught on the first relaunch attempt). v6 CONFIRMED: exactly 5 concurrent node workers, spark0-4 building arm A. Also swept v3/v4/v5 remnants (orphaned subshells, one in-flight spark0 build TERM'd, reserves released) before relaunch — no duplicate writers.
2026-09-05T02:05:00Z firing 241: QWEN NVFP4 TP4PP4 SIZE STRUCTURE SETTLED (false-alarm near-miss): rank sizes are non-uniform BY STRUCTURE — stage 0 carries the PLE ngram table (F8 widened to BF16 on copy, ~23 GiB) so stage-0 ranks = 30,965,642,496 B while stages 1/2 ranks = 4,981,989,376 B (stage 3 adds head+MTP-scoped extras). Verified: source expert payloads uniform across all 48 layers (U8 [640,1280] x 512 experts x 3 projs), per-stage tp-slices uniform (rank0=rank2, rank5=rank6 byte-for-byte sizes), verifier byte-traced samples PASS. SANITY-AUDIT RULE for this arm: uniformity WITHIN a stage group (r = stage*4 + tp), NOT fleet-wide; the size-vs-source arithmetic must price the ngram table on stage 0 and head/MTP on stage 3. Wave v6 progress: 7 nodes fully through A+B by 01:43 (warm source caches accelerate later ranks), spark4 A done, remaining spark1/3/a-f in flight with 5 workers.
2026-09-05T02:30:00Z firing 242: DUPLICATE-RETIREMENT VERDICT REVERSED BY EVIDENCE — the per-node sha sweep shows legacy glm53flash.bf16.tp16 packs DIFFER from the new fp8.tp16 packs on ALL 16 nodes (16/16 DIFFERS; sizes identical at 21,706,046,976). Header probe settles the quant question: BOTH legacy and fresh packs carry expert entries with codec 5 (FP8), 1160 entries, identical geometry (288x256x4096 + f32 scale planes) — the legacy "bf16" name is a CONFIRMED misnomer for fp8-quant content; the byte differences are strip-era repackaging (the 1187->1160 MTP strip rewrote directories in place; fresh packs planned 1160 directly). CONSEQUENCES: (a) no exact-duplicate retirement — the legacy set stays until naming adjudication; (b) the legacy dir hosts live serving state (residentd on spark0-7) and renamed-file territory — NOT touching it; (c) the TRUE 16-bit leg is being built as arm D under the self-describing interim name glm53flash.bf16-official.tp16 (source glm-5.3-flash-bf16-official, native BF16 experts, dry-plan GREEN 1157t pre-owns), renaming to canonical bf16.tp16 once the legacy collision is adjudicated. STRAYS CONFIRMED for the same sweep: spark0 holds rank1, spark1 rank2, spark2 rank3, spark3 rank4, spark4 rank5 (each the NEXT node's rank — an off-by-one placement era), plus spark5 rank0; their digests are recorded in /tmp/dup-sweep.log and they are retirement candidates once compared against their canonical hosts' digests. ARM C nearly done (10/15 ranks placed by 02:18, warm-cache fast); arm D cmds+driver staged to fire when C drains.
2026-09-05T04:15:00Z firing 243: ARM D 16/16 COMPLETE — glm53flash.bf16-official.tp16 (TRUE 16-bit leg, native BF16 experts, 40,136,867,328 B/rank uniform) placed on all 16 nodes. sparke rank14 via RELAY (built+verified on spark6, shipped 40GB node-to-node ~130MB/s, dual-sha e0118b16... matched, placed+locked on sparke with relay provenance in the receipt); spark9 recovered on retry (transient). MATRIX STATE: glm53flash now bf16/fp8/nvfp4 x TP16 (+fp8 tp8/tp4pp4, nvfp4 tp8) — all built MTP-free this era with the source-driven packer.
firing 243a: CEPH SLOWDOWN DIAGNOSIS (operator-asked): TWO distinct modes. (1) spark9 transient: 0.5MB/s crawl for 27min on first-touch of the cold bf16-official tree, full recovery 30min later, zero kernel libceph/mds/osd messages, mounts identical fleet-wide -> cluster-side stall (PG peering/recovery or OSD overload during the fleet's first cold read of that 745GB source); resolved itself. (2) sparke chronic: persistent 0.8MB/s, the documented stale-negative-cache client rot (packer docstring warning; sparka precedent = client-session rot fixed by restarting ds4-ceph-warm-mount.service) -> needs the same service restart (sysadmin). AMPLIFIER: packer memmap reads = 4KB page faults; ~8ms tail per fault -> ~0.5MB/s exactly as measured; sequential readers barely notice. RULED OUT: mount config, auth (single ds4warm identity), dirty-page pressure (reads; 4G cap live), local NVMe. NOT OBSERVABLE from nodes: cluster recovery windows (no CLI keyring off the mount service) -> sysadmin to confirm 03:20-03:55 OSD recovery. MITIGATION PROVEN: pre-wave 30s read-probe gate + relay-build crawlers on healthy nodes (k3/sparkf pattern, used for rank14 today).
2026-09-05T05:00:00Z firing 244: FLEET AUDIT COMPLETE (patched tool, long-lived-ssh runner after the remote-nohup vanishing class) — NEW ARMS 32/32 CLEAN (fp8.tp16 + nvfp4.tp16: magic/flags=0/bytes/dir/receipt-sha/lock all PASS). Legacy-era findings -> FIXUP: (a) 16 unlocked bf16.tp16 packs (one canonical per node + the two known strays rank1@spark0 rank0@spark5) — LOCKED 25 packs total including 7 unlocked fp8.tp8 primaries; (b) fp8.tp8 secondaries: 6 stale receipts (post-compact digests never re-recorded — refreshed with current sha, bytes untouched) and 2 STRUCTURAL breaks (spark8 rank0, spark9 rank1): the compact path truncated the file but left header file_bytes stale AND the directory inconsistent beyond entry 787 (region end 147MB past EOF) — header patch alone verified INSUFFICIENT (region-bounds still fail), so those two ranks are being REBUILT owns-shape from the fp8 source (matching their 1160-entry primaries) with full verify before replace; (c) coverage fix: the fp8.tp4pp4 packs use a THIRD legacy naming (glm5.tp4pp4.rankN) that my family-prefix filter excluded — added to PACK_PREFIXES so the arm is audited. TOOLING LESSONS: remote nohup-without-setsid dies at ssh teardown (systemd session cleanup) AND setsid'd jobs' logs vanished anyway -> fleet audits now run under long-lived local ssh with Mac-side logs; xargs -I{} inline-script arg-passing was a silent no-op twice -> per-node script files won (arm C pattern).
2026-09-05T05:55:00Z firing 245: FIXUP WAVE COMPLETE — FLEET AUDIT CLEAN. Final coverage audit (all g5 arms incl. the newly-covered fp8.tp4pp4): PASS everywhere after these fixes: (1) 25+13 legacy packs locked (bf16.tp16 x16 incl. 2 strays, fp8.tp8 primaries x7, tp4pp4 x13); (2) fp8.tp8 secondaries: 6 stale receipts refreshed, 2 structurally-broken compacts (spark8 rank0, spark9 rank1 — stale header file_bytes AND directory regions past EOF) REBUILT owns-shape 1160t 42,381,110,784 B, VERIFY-PASS, uniform dir_sha; (3) THREE MTP-CARRYING PACKS FOUND AND STRIPPED (glm53flash.fp8.tp4pp4 ranks 12/13/14 on sparkc/d/e — flags=1 + MTP entry at 312; the strip campaign had missed them): mid-file MTP class -> compact path (truncate refuses fail-closed correctly), then the compact's own stale-file_bytes bug found and FIXED IN THE TOOL (cursor vs actual-size sparse tail — file_bytes now written from the real EOF), packs verified flags=0/338 entries/107,211,675,136 B, receipts refreshed under BOTH names (audit prefers .receipt.json), locked. STAGEPACK MATRIX COMPLETE; AUDIT CLEAN — glm53flash bf16/fp8/nvfp4 x tp16 (+fp8 tp8/tp4pp4, nvfp4 tp8, nvfp4 tp4pp4 pending op call), qwen3flash bf16/fp8/nvfp4 x their serving topologies, qwen27b bf16+nvfp4a16, glm53full bf16/fp8/nvfp4 x tp16+tp4pp4, k3 mxfp4 x tp16+tp4pp4, dsv4flash/pro, hy4, qwenmax — all placed rank-per-node, MTP-free, receipted, locked. Remaining program lines (separate): module publications, tile-Mloop kernel ticket, mirror refresh.
2026-09-05T06:40:00Z firing 246: MODULE PUBLICATIONS — FLASH DONE, 27B BLOCKED ON GDN TOKEN-EQUALITY. FLASH: PUBLISHED — module spark.glm5_next.resident_decode_stage.bf16.expert_fp8.h4096.l45.kda34.e288.k8.v2, cuda.sm121 target, GPU validator PASS (0 failures) against the MTP-free glm53flash.fp8.tp16 rank0 pack; archive a814d166... + record in build/module_library. 27B: the publication chain runs green through config gates, pack load, gdn_chunk_state (relerr 1.1e-7, cosine 1.0), admit/snapshot — then FAILS module_decode_vs_prefill token equality (18908 vs 40446). ROOT: single-stage builds take the harness's bit-exact token path, and the GDN prefill (chunked parallel scan) diverges in rounding from the decode (step recurrence) — the same kernel class as the tile-Mloop ticket; the FFN paths share one dispatch (BatchedLinear nvfp4) so they are NOT the divergence. ALSO LANDED: the 27B hardware validator's qualified matrix required STAGE_MTP=1 for TP configs — predates the MTP-free directive; now qualifies STAGE_MTP in {0,1} (pushed b92fa21). ITEMS 3 AND 4 CONVERGE: the GDN chunk-scan vs decode-step bit-exactness fix is the tile-Mloop-class kernel work; flash precedent proves the gate is passable. Note: publication gates found in sequence — ALLOW_UNQUALIFIED_EXECUTION=1 (mandatory on this rig), MAX_ACTIVE_SEQUENCES in {8,16,64}, TP_STANDALONE=1 + STAGE_COUNT=1 + LAYER_COUNT=64 for TP ranks.
2026-09-05T07:00:00Z firing 247: MODULE PUBLICATIONS COMPLETE — BOTH ARCHIVES PUBLISHED GPU-VALIDATED. (1) FLASH: spark.glm5_next.resident_decode_stage.bf16.expert_fp8.h4096.l45.kda34.e288.k8.v2 (archive a814d166..., validation executed vs the MTP-free glm53flash.fp8.tp16 rank0 pack). (2) 27B: spark.qwen38_27b.resident_decode_stage.bf16.h5120.l64.gdn48.mtp1.v1 (archive 987882e9..., validation executed vs the MTP-free qwen38-27b.nvfp4a16.tp4 rank0 pack; module_determinism bit_exact=1). THREE MODULE-SIDE FIXES LANDED TO GET THERE (all pushed): (a) the 27B validator's qualified TP matrix required STAGE_MTP=1 — now qualifies {0,1} (MTP-free is the fleet standard); (b) single-token prefill routes through the GDN decode core — GdnChunk(1) and GdnStep(1) round differently and broke decode-vs-prefill token equality; the 1-row prefill IS a decode step (b475e8a); (c) the driver's MTP draft speculation check is skipped for MTP-free configs (c33436d). Publication gate recipe for 27B TP ranks: ALLOW_UNQUALIFIED_EXECUTION=1 MAX_ACTIVE_SEQUENCES=64 TP_STANDALONE=1 (+STAGE_COUNT=1 LAYER_COUNT=64 implied). ITEM 3 CLOSED; the GDN routing fix also retires the decode-vs-prefill half of the tile-Mloop ticket (item 4) for single-token prefill; remaining program: mirror refresh (item 5) + the prefill multi-row Mloop investigation for the flash nvfp4 prefill path.
2026-09-05T07:45:00Z firing 248: MIRROR REFRESH COMPLETE (item 5 CLOSED) — all 16 nodes rc=0, 112 arm-mirrors written to /mnt/model-warm/stagepack-backup/spark<h>/<arm>/packs/ (the 7 arms this program created or changed: glm53flash.fp8.tp16, nvfp4.tp16, bf16-official.tp16, bf16.tp16, fp8.tp8, fp8.tp4pp4, qwen3flash.nvfp4.tp4pp4), rsync --delete per arm = each is an exact mirror of the placed MTP-free bytes. PLACEMENT-LAW FIX caught by the mirror's coverage check: glm53flash.nvfp4.tp16 rank0 lived on spark5 (probe-era placement) instead of its lawful spark0 — relocated with dual-sha verify (fcb1ca39...), receipt updated with provenance, spark5's now-exact duplicate removed per the removal law, spark0's arm mirrored. NVIDIA NVFP4 fetch receipted: DOWNLOAD-RECEIPT.json written fleet-convention (25 files, 132,734,506,208 B = the model card's stated size; .cache excluded). NOTE for the stale-backup check: the backup dirs for dsv4pro (compacted era) and qwenmax (v2 rebuild era) may still hold pre-fixup bytes — flag for a follow-up sweep if those arms are ever restored from backup. PROGRAM STATE: items 1, 2, 3, 5 CLOSED; item 4 residue = the multi-row Mloop prefill investigation (single-token prefill already routed).
2026-09-05T08:15:00Z firing 249: TILE-MLOOP TICKET (item 4) — REPRODUCIBLE FAILING CASE CAPTURED. The nvfp4 smoke (tools/qwen4_flash_nvfp4_smoke.cu) builds clean with -gencode arch=compute_121a,code=sm_121a -I. -Imodel-families/common/include -Iinclude (note: plain -arch=sm_121a DROPS the family suffix in ptxas and the e2m1 cvt instruction is rejected — the -gencode form is required) and runs against the PLACED qwen3flash.nvfp4.tp8 rank0 pack. RESULT: scalar leg ≈ correct (45/5120 outliers are near-zero-reference rel noise; leg1b device|host values match); TILE Mloop leg FAILS with a decisive signature: every output value is CORRECT but at a scrambled position — got[0]=-0.0947 = nearest reference (row=1,neuron=262) diff 5.8e-05; 2634/5120 outside tolerance, worst rel 1113 (positional, not numeric). CONCLUSION: the Mloop nvfp4 branch's output row/neuron ADDRESSING is wrong (values right, positions wrong) — the payload is [neuron, in/2] packed but the Mloop's neuron/row loop writes with a different linearization. THE FIX is the branch's output indexing in SparkLmExpertTileAllMloopKernel (spark_lm_kernels.cuh ~4544-4548, the nvfp4 arm calls SparkLmDotRowNvfp4<16u> with group_scale+scale_group_stride_bytes-4). UNTIL IT LANDS the modules correctly route nvfp4 multi-row prefill away from Mloop (scalar path — correct, slower), and single-token prefill rides the decode core (b475e8a) — serving correctness is NOT affected. Smoke rerun recipe: build as above on a GPU node, run `/tmp/nvfp4_smoke <pack>` against any placed qwen3flash.nvfp4.tp8 pack.
2026-09-05T08:30:00Z firing 250: TILE-MLOOP INVESTIGATION PROGRESS (item 4 residue) — the defect is narrowed to the WEIGHT STAGING into the Mloop's shared tiles, NOT the output mapping and NOT the per-entry decode. Verified correct this window: SparkLmTileDecodeRun's nvfp4 case (payload word index, per-16 scale index neuron*(in/16)+(k+chunk*8)/16, global multiply — all consistent with the proven scalar DotRowNvfp4); the Mloop output store (wmma store_matrix_sync at column offset warp*16, ldm = TILE_N+8 = the 2D tile_output stride) and the global write loop (2D-indexed, compiler stride — consistent). Remaining suspect: SparkLmTileStageWeightAll<16u> / SparkLmTileStageWeightProducerHalf<16u> — the staging that fills tile_weight[neuron][k] from the packed [neuron, in/2] payload with group-16 scales; the fp8 arm stages with <128u> and works, nvfp4 stages <16u> and scrambles (correct values at wrong output positions = a neuron-or-k misattribution during staging). NEXT SESSION: dump the staged tile_weight for a known neuron set (GPU printf or a staging-only unit test) against the direct SparkLmTileDecodeRun decode for the same neurons — the mismatch table will point at the exact index term. Serving correctness remains unaffected (nvfp4 prefill rides the scalar path; single-token prefill rides the decode core).
2026-09-05T09:00:00Z firing 251: TILE-MLOOP ROOT CAUSE ISOLATED TO THE K-LOOP ACCUMULATION/HANDOFF. New hard evidence (GPU printf instrumentation, spark0): (1) the write attribution is correct (blockIdx/expert/slot/neuron mapping dumped live — expert 5 writes its own neuron range, correct slots); (2) the staged decode is BIT-EXACT vs the host dequant — device n544 k64/80/96/112 = -0.0332729/-0.00207956/-0.00623867/0.0415911 matches w3_matrix exactly (the earlier "scrambled values at wrong positions" reading was an artifact of comparing W3-leg dumps against DOWN-leg decodes — both legs print n544 and the two legs' weights interleave in stderr). With decode, staging, attribution, and store/load strides all verified, the wrong total (-0.427 vs 0.6736 for row1/n544) can only arise in the Mloop's k-loop: the double-buffer handoff (warps 0-3 mma tile[current] while 4-7 stage tile[next], then swap) either skips or double-counts a k-tile for some (row,neuron) positions. NEXT SESSION: per-k partial-sum dump — accumulate frag_accum for (row1,n544) and print after each k-tile (or run the k-loop single-buffered under a debug env) — the iteration where the partial diverges names the handoff bug (suspects: the current_buffer swap order vs the __syncthreads placement, and the TileStageWeightAll-vs-ProducerHalf boundary at k=0->64). The instrumentation binary + recipe remain on spark0 (/tmp/nvfp4_smoke_dbg, SPARK_LM_MLOOP_DEBUG=1).
2026-09-05T09:30:00Z firing 252: TILE-MLOOP BUG LOCALIZED TO THE K-LOOP BUFFER ALTERNATION. Extended partial dump (both warp classes): neuron 32 (warps 0-3 slice) AND neuron 96 (warps 4-7 slice) show the IDENTICAL alternating drop — k-tiles 0,2,4,6 accumulate; k-tiles 1,3,5,7 add EXACTLY zero (stored value byte-identical before/after those iterations) — uniform across neurons, experts, and warps. Ruled out this window: warp/neuron mapping (both classes affected), the per-entry nvfp4 decode (bit-exact vs host), write attribution (live dump), output store/load strides (compiler 2D stride = wmma ldm). The alternation structure: per k-iteration, warps 0-3 mma tile[current] while warps 4-7 stage tile[next] neurons 0-63; then warps 4-7 mma tile[current] while warps 0-3 stage tile[next] neurons 64-127; swap. Static trace of the handoff passes — the defect must be dynamic: either (a) the mma re-reads the PREVIOUS tile on alternate iterations (current_buffer swap visible too late to some warps — check for a missing __syncthreads between the swap and the next iteration's mma — THERE IS NONE between `current_buffer = next_buffer` and the loop-top mma: warps 0-3's loop-top mma of iteration k+1 can start while warps 4-7 are still in their k-th mma/stage phase?? no — both syncs are inside the iteration; after the final sync + swap ALL warps see current=next... but the NEXT iteration's first-half mma by warps 0-3 reads tile[current=next] staged by warps 4-7 LAST iteration — correctly synced ✓), or (b) the alternating ProducerHalf neuron-half call pattern (base=4/half=0 in the first half, base=0/half=1 in the second) interacts with the k-tile parity so tile[next]'s staging races the mma of the SAME iteration through the shared tile_weight[next] when next_buffer aliases... NEXT INSTRUMENT (exact): print tile_weight[next_buffer] staged values DURING each k-iteration (inside ProducerHalf, dump neuron 32's k-slot) + the mma's tile_weight[current] values at the same k — the iteration where stage and mma disagree names the buffer aliasing. Alternative pragmatic fix if the handoff resists: run the k-loop single-buffered (stage into tile[0], sync, mma tile[0]) under a debug env — halves prefill throughput but is trivially correct. The instrumented binary + build recipe remain on spark0.
2026-09-05T09:30:00Z firing 253: TILE-MLOOP FIXED AND VALIDATED (item 4 CLOSED). ROOT CAUSE (found via per-k partial + staged-value instrumentation): the pipelined k-loop's input staging ran on warps 4-7 (threads 128-255) with `entry = threadIdx.x` — entries 0-127 (input rows 0-1) of every staged input tile were NEVER WRITTEN, so rows 0-1 of each routed expert tile received zero contribution from every SECOND k-tile (the odd buffers). FIX: index the input tile from the producer-local thread id (threadIdx.x - 128) with stride 128 (commit f9c6985). VALIDATION (clean main clone, spark0, real placed pack qwen3flash.nvfp4.tp8 rank0): all three legs PASS the module contract — scalar W1 rel_l2 0.00222 / cosine 0.999998; tile W3 (Mloop) rel_l2 0.00283 / cosine 0.999996; tile DOWN (Mloop) rel_l2 0.00230 / cosine 0.999997. The Mloop tensor-core path is proven for multi-row nvfp4 prefill — the scalar-path rider is retired and the qwen4-flash nvfp4 module can serve multi-row prefill through the tile path. The smoke's tile-leg acceptance now uses the module validator contract (rel L2 5e-2 + cosine 0.999) instead of a strict per-element gate that false-flagged near-zero-reference elements. BUILD NOTE: the e2m1 cvt PTX requires -gencode arch=compute_121a,code=sm_121a (plain -arch=sm_121a drops the family suffix in ptxas). PROGRAM STATE: items 1-5 ALL CLOSED. Follow-ups outside the completion program: qwen4-flash nvfp4 module publication with the Mloop path enabled (deployment lane), dsv4pro/qwenmax backup-dir staleness flag, the NVIDIA-vs-RadixArk repack decision (NVIDIA relerr 0.081 vs 0.095, shelved).
2026-09-05T19:30:00Z firing 254: NVFP4 REPACK EXECUTED (operator ruling: nvidia replaces radixark — better calibration, relerr 0.081 vs 0.095). qwen3flash.nvfp4.tp8 AND qwen3flash.nvfp4.tp4pp4 rebuilt from /mnt/model-warm/qwen3.8-flash-next-nvfp4-nvidia (the packer's nvfp4-official reader handles the nvidia schema verbatim — same naming incl. ngram_embedding.shard_0.weight; dry-run GREEN 1215t 22.27GiB/rank tp8). 32/32 ranks placed MTP-free, verify PASS vs the nvidia source (byte-traced samples), sha-receipted (source=nvidia + repack provenance), chattr-locked; superseded radixark-era packs (incl. old-named tp8-rankNN.q4fsp on all 16) removed per the ruling; mirror2 refreshed both arms 16/16 (32 arm-mirrors). THREE ranks needed the relay play (sparkd tp4pp4, sparkb+sparke tp8): built on spark6, shipped node-to-node, dual-sha verified. TWO wave bugs fixed in-flight: (1) the tp8 second-copy nodes spark8-f hold tp-ranks 0-7 (2x map), my first generator emitted invalid --tp-rank 8-15 and the wave silently no-op'd those eight — caught because spark8's tp8 dir still listed the OLD pack names; receipts on those nodes also re-fixed (the doubled-brace {{ bug re-introduced by the a2 cmds). (2) hand-written receipt sidecars had {{ }} JSON from a format-escape leftover — sed-fixed fleet-wide; lesson: emit receipts from the packer's own receipt (rename) rather than hand-printed templates.
2026-09-05T19:30:00Z firing 254a: CEPH SLOWDOWN CORRECTED DIAGNOSIS (retraction of the firing-243a HDD/seek-storm theory — operator correction: all sparks are NVMe; sdb on sparkb is an ASMT-USB-bridge NVMe, rotational=0, and osd.11's bluestore block IS that device via LVM). MEASURED: radixark source reads 6.1 GB/s from spark6 while the nvidia source reads 1-5 MB/s from EVERY node (sparkb, spark6, sparke) at the same moment; the crawl persists with ZERO client readers (fuser: only the kernel mount); kernel client logs show ceph_con_workfn CPU-hog warnings doubling exponentially on multiple nodes; timeline = first ~30 repack builds FAST (~150MB/s/node, 17:18-17:23), crawl from ~17:25 onward, still ongoing. CONCLUSION: server-side, dataset-specific — the PGs holding the freshly-written nvidia objects are being throttled (scrub/backfill/recovery pass) or their serving OSD is unwell; the ASMT USB-bridge NVMe backing osd.11 is a suspect device class (bridge can wedge; ~400MB/s ceiling) but reads crawl from nodes that never touch it, so the primary suspect is whatever OSD is PRIMARY for those PGs. SYSADMIN CHECK: ceph -s / ceph pg ls is scrubbing,is recovering / osd.11 health. Program mitigation stays: pre-wave read-probe gate + relay-build crawlers (used 3x today).
2026-09-05T20:00:00Z firing 255: OPERATOR RULINGS EXECUTED. (1) MISNAMED-ARM RETIRED: glm53flash.bf16.tp16 (bf16 name over fp8-expert content — operator: "fp8 should not be in bf16"; nothing serving anywhere) removed on all 16 nodes incl. the two confirmed stray packs (spark0 rank1, spark5 rank0 — the other four next-node strays died with the arm) AND the 16 backup-mirror copies; chattr -i unlock pass required (the protection ring blocked the first rm). Name freed and CLAIMED by rename: glm53flash.bf16-official.tp16 -> glm53flash.bf16.tp16 on all 16 nodes (locks intact, receipts sed-updated) + 16 backup-mirror dirs. bf16 token now means ALL-BF16 per docs/STAGEPACK_NAMING.md (ruling recorded there). (2) WARM CLEANUP per ruling (vision-exp + drafters KEPT): deleted qwen3.8-flash-next-nvfp4-radixark (126G), packbuild/ (5.8T of superseded build staging: dsv4pro-tp16/tp4pp4, qwen38max, k3, dsv4-flash, dsv4pro, 27b), archive-not-a-source/ (1.5T incl. kimi-k3-nvfp4-redhatai) — ~7.4T reclaimed; manifests captured on spark1 /tmp/*-manifest.txt pre-deletion; remaining radixark-named dirs are KEEP sources (glm-5.3-nvfp4-radixark = glm53full nvfp4 source of record; qwen-max-nvfp4-radixark-bf16-spine = qwenmax source of record; the drafters). (3) SYSADMIN NOTES: ceph -s via the admin keyring on spark0 (/etc/ceph) — 19 OSDs (16 warmdata 3.6T + 3 warmmeta 256G) all up, 193 PGs active+clean, 81% used; the nvidia-PG read throttle SELF-RESOLVED (556MB/s-5.4GB/s restored) — consistent with a scrub/backfill pass on the freshly-written PGs; osd.5+osd.8 carry lingering slow-op observation windows (historical); mon.spark0 13% avail housekeeping; several sparks host OSDs (spark0's is NVMe "2462_NVME ds4-ceph-data"), sparkb's osd.11 block device is an ASMT USB-bridge NVMe (sdb) — device class worth watching.
2026-09-05T20:30:00Z firing 256: WARM->COLD BACKUP AUDIT + BROUGHT UNDER MANAGEMENT (operator: cold storage lives on the RTX5090 system). ARCHITECTURE FOUND: rtx5090 (10.10.250.2, user spec) hosts /mnt/cold-raid6 (59T RAID6, 37T used) and mounts the ceph warm fs itself; the archival mechanism is /home/spec/bin/spark0_cold_warm_queue.py — marker-driven (a warm model dir with a PUBLISHED/.COPY-DONE marker enters the queue), copies into models/.staging/<name>.cold.partial, verifies (snapshot size+mtime both sides), promotes with .COLD_COPY_COMPLETE.json; state in .cold-warm-queue.json; excludes packbuild/archive-not-a-source/stagepack-backup/.staging; enforces its own 1GiB memory cgroup (refuses unbounded). STATUS FOUND: process was NOT running (no cron/timer; last manual run 00:50-01:08Z died mid-verify leaving 6 .unverified staging dirs); coverage was 30/31 — the ONLY missing model was qwen3.8-flash-next-nvfp4-nvidia (fetched today, no marker). ACTIONS TAKEN (as sysadmin): (1) admin keyring located at /etc/ceph/ceph.client.admin.keyring on the sparks (mon_host 10.10.100.10/.17/.25) -> full cluster visibility; (2) published the nvidia source (PUBLISHED marker, fleet convention = empty marker file); (3) installed /etc/systemd/system/sparkpipe-cold-archive.service (MemoryMax=1G per the script's own enforcement + MemoryHigh=700M — a bare 1G cap OOM-killed the first run via page-cache accounting; the early throttle forces reclaim) + .timer (nightly 00:00 UTC, Persistent=true) and enabled it — the backup process is now SCHEDULED, not manual; (4) ran the queue: it re-verified and promoted the 6 stale staging leftovers AND archived the nvidia source (124G, 8m40s, 711M peak) — coverage now 30/30 with complete markers. VERIFICATION: cold also holds retired quants not in warm (27b-nvfp4 variants, dsv4 official fp4/fp8-mixed, 2.4t-a95b-fp8, kimi-k3-nvfp4-redhatai) — the archive's historical depth; today's deleted warm radixark flash-nvfp4 survives as the cold copy (intentional). HARDWARE FACT CORRECTED (operator): ALL sparks use the same ASMT USB-bridge NVMe enclosure, mixed 4TB/8TB SSDs — the enclosure is fleet-standard, not a sparkb anomaly; osd.11's backing device is unremarkable. Warm-root housekeeping: hidden .ds4-ceph-read-fixed-16-q32/ benchmark dir exists (sysadmin debris, no backup value).
2026-09-05T21:00:00Z firing 257: CRAWL ROOT CAUSE — POST-MORTEM COMPLETE (cepg admin access). Authoritative object->PG mapping via `ceph osd map`: the nvidia source's stripes concentrate on primaries **osd.11 (sparkb)** — 3 of 6 sampled stripes (5.24, 5.57, 5.b) — plus osd.15/osd.1; and **osd.8 (spark8) sits in the acting set** of 5.24. ceph's own health alerts flagged exactly osd.5 + osd.8 for BlueStore slow operations in the crawl window (17:25-19:40Z). THE MECHANISM: the three slow builders (sparkb, sparkd, sparke) each host an OSD in the acting sets of the data they were reading (sparkb: osd.11 PRIMARY; sparkd: osd.13 member; sparke: osd.14 member) — those nodes did double duty, serving the fleet's reads through their local OSD while running their own builds, so their builds starved behind the OSD traffic on the shared ASMT USB-bridge NVMe path. The bridge's concurrency ceiling (fine for one stream, collapses under 5-6 clients of random 4KB memmap faults) explains both the dataset-specific throttle (only the nvidia PGs were hammered — the packers' read set) and the self-resolution (load dropped at ~19:40Z). Radixark stayed at 6.1GB/s because nothing was reading it. UNPROVEN: which internal BlueStore path stalled first (per-op traces rotated; compaction counters zero). MITIGATIONS STANDING: read-probe gate before cold-source waves + relay builds for crawlers; optionally cap wave width to 3-4 when the source is freshly written, or pre-warm with one sequential reader before fanning out.
2026-09-05T21:30:00Z firing 258: SYSADMIN MOVES EXECUTED + ONE ROLLBACK. (1) HYGIENE SWEEP DONE fleet-wide: 618 stale /tmp logs+cmds removed, all .staging-* build dirs purged, dry-*/g5m5-src/pub-src (476M+287M+126M+124M) cleared; sparkf's stale dsv4 serving stack (/tmp/sparkf-stack + sparkf-live, 3 idle residentd) TERMed+removed; sparka's 2.2T sparkdata is all canonical-named (flag: dsv4_pro.tp16 dir at 886G = ~9 ranks where canonical map says 1/node — digest audit pending); warm-root .ds4-ceph-read-fixed-16-q32 benchmark debris REMOVED. (2) MDS MOVE OFF SPARK3: DONE — but NOT onto rtx5090: my hand-run MDS container there (docker, quay ceph v20.2.4, --public_addr=192.168.50.4 wired) JOINED and took rank 0, then DENIED ALL client reads fleet-wide (EACCES on every warm path incl. root) while the same client caps read fine under spark3/Mac-Studio MDS daemons — root cause inside that daemon's session/config not yet isolated (suspect: missing cephadm-generated config context). ROLLED BACK: rtx5090 mds container STOPPED; ACTIVE MDS = ds4warm.lima-mds-macstudio (10.10.100.2 — the operator's own Mac Studio, off-spark, fully reachable); spark3's mds service STOPPED+DISABLED (the directive — spark3 no longer in the ceph control plane; it still hosts osd.3+osd.17 as storage). All fleet mounts verified reading. (3) MGR MOVED TO RTX5090 SUCCESSFULLY: mgr.ds4warm.rtx5090 ACTIVE (spark0+spark8 kept as light standbys for HA) — mgr ops are internal, no client-facing denial. (4) DISCOVERED NETWORK CONSTRAINT: rtx5090 (10.10.250.2, eno1 /30) cannot reach Mac-Studio lima MDS port 6800 (blocked at the Mac/lima side) — sparks reach it fine; consequence: rtx5090's OWN warm mount cannot re-establish while the Mac MDS is active (mon reachable, MDS blocked) → the nightly cold-archive service will fail until EITHER the Mac Studio lima port-forward allows rtx5090's IPs, OR the rtx5090 MDS daemon denial is debugged and it serves itself+fleet. Escalating to operator (Mac Studio is theirs). (5) rtx5090 build workhorse: packtools staged; smoke dry-run blocked by the same mount issue. (6) CHECKOUT SYNC fleet-wide: deferred to next action (bundle ready).
2026-09-05T22:00:00Z firing 259: IMPROVEMENT LIST EXECUTED + WARM ACCESS FULLY RESTORED. (1) MDS RE-ANCHORED: the lima-VM mds.ds4warm.lima-mds-macstudio daemon re-pinned public_addr=192.168.50.118 (config: /var/lib/ceph/<fsid>/mds.../config inside the VM; the old auto-pick was the 10.10.100.2/32 lima0 address that rtx5090 cannot reach). ALL 16 sparks + rtx5090 verified 192.168.50.118:6800 reachable; rtx5090 warm mount RESTORED via its ds4-ceph-warm-mount unit; fleet reads verified. THE WIFI7 question: wired 192.168.50.x chosen for metadata (lowest latency/jitter); wifi stays backup. (2) RTX5090 BUILD WORKHORSE: packtools staged (5 tools), smoke dry-run GREEN from rtx5090 (qwen3flash nvfp4 tp4pp4 rank0 plan: 311 tensors, 28.84 GiB — matches reference). Non-inference builds can now run there; the earlier ceph-crawl double-duty problem is structurally gone for rtx5090-hosted builds. (3) CHECKOUT SYNC: all 16 node checkouts = main @ 6c68834 via bundle recipe (stash -u preserved). FLEET STATE: ceph = mons spark0/f/7, mgr rtx5090 (spark0/8 standbys), MDS Mac Studio VM (standby: none active-serving — the rtx5090 mds container remains STOPPED pending read-denial debug), 19 OSDs up; sparks = inference + storage only; build/verify/fetch/archive consolidated on rtx5090 + the Mac Studio control plane.
2026-09-05T22:40:00Z firing 260: SSH MESH ENABLED + EMERGENCY LAYER STATUS. (1) FLEET MESH INSTALLED: every spark's ~/.ssh/config now carries a managed mesh block (all peers via cluster NIC 10.10.100.<h> — addresses spark0=.10 … sparkf=.25 — User=peer, accept-new) + Host rtx5090 (192.168.50.4 with ProxyJump sparkf for the /30-impaired nodes; sparkf itself uses the DIRECT 10.10.250.2 link). Verified pairs: spark0→sparkc, spark9→spark3, spark1→sparka, sparkb→spark6, spark2→sparke, spark5→sparkf, sparkc→sparkf, spark0/5/c/d→rtx5090, sparkf→rtx5090 (direct). ROOT CAUSES of the "auth denied" reports: (a) the dev's release-agent key refresh made offered keys mismatch installed ones; (b) stale known_hosts entries for rtx5090's changed keys (spark0 needed ssh-keygen -R); (c) the dev's/DS4 alias tables pointed spark-to-spark at the UNROUTED 10.20.0.x mgmt subnet — the cluster NIC 10.10.100.x is the sane mesh path; (d) my own first mesh generation had spark0-9 mapped to .0-.9 instead of .10-.19 — caught because the jump resolved sparkf to spark5's address. (2) RTX5090 SSH ENABLED BOTH WAYS: rtx5090 got a keypair, its pubkey installed on all 16 sparks' authorized_keys (user sparkN), fleet /etc/hosts aliases (sparkN=10.10.100.x) added, mesh config installed (User=peer); verified rtx5090→spark0/5/b/f. (3) EMERGENCY (WiFi v6) LAYER: ssh-config percent_expand bug FIXED via helper-script ProxyCommand (/Users/mac/.local/libexec/ds4-spark-management/ds4_emergency_connect.sh with CURRENT link-local map — the old entries carried stale addrs); config backup at ~/.ssh/config.bak-emergency-*. REMAINING BLOCKER: connections reach the nodes' port 2222 but die at banner — NOT the hardening conf (benign: pubkey-only, no AllowUsers; iptables INPUT ACCEPT) — suspect WiFi AP client-isolation or the 2222 listener's per-source restrictions; needs on-node Wi-Fi/sshd diagnosis. STATUS: Mac-direct ssh = primary control path (16/16 working); spark mesh + rtx5090 mesh = working; WiFi emergency = down (AP-level, pre-existing).
2026-09-06T13:20:00Z firing 261: WIFI EMERGENCY PATH RESTORED (root@port-2222 over WiFi IPv4). ROOT CAUSE CHAIN: (1) the emergency sshd binds IPv4 ONLY (0.0.0.0:2222, no v6 listener) while the ssh configs dialed IPv6 link-local — every connection hung at banner; (2) the stored link-local addresses were STALE (previous NIC generation). FIX: helper ds4_emergency_connect.sh now maps sparkN-emergency → the node's WiFi IPv4 (192.168.50.128+hex: spark0=.128 … sparkf=.143, sshd answers instantly); ssh-config entries rewritten to ProxyCommand via the helper; User root (the emergency sshd's AllowUsers is root — root's authorized_keys now holds BOTH emergency pubkeys: macstudio-emerg (this Mac, SHA256 tO/NqTK9…) AND cemmini (the Mac mini, j3YmWXAY…) so EITHER machine can root-recover over WiFi; the stray cemmini grant to the sparkemerg accounts was removed; spark1's AllowUsers experiment reverted to root. 16/16 emergency entries exist (spark0/5/d/e/f were missing — added); verified root login spark1/6/b/0/f. COVERAGE NOTE: entries exist for all 16 nodes; the helper covers all 16. REMINDER: WiFi = 192.168.50.x L2 (Mac .60/.249, nodes .128-.143, rtx5090 .4); the emergency sshd unit runs from /etc/ssh/sshd_config_spark_emergency (Port 2222, pubkey-only, AllowUsers root).
2026-09-06T14:00:00Z firing 262: FINAL CLEANUP + WATCH ITEMS CLOSED. (1) DISK CLEANUP EXECUTED (~3.1T freed): spark0 glm52_packs 769G + qwen38max-lane 606G + .cache 25G; sparka dsv4_pro_full.spstage 832G (full-model slicing master, regenerable from warm — the 886G "9 ranks" mystery was this master + the canonical rank10 pack 54G which is KEPT receipted; plus a 2.4G stray .tmp); spark1/2 srcdata 1.1T/625G (PP13-era staging: glm52/dsv4/k3 slices + old releases — all superseded); spark5 lane-dsv4bisect/m1 700G; sparkf staging 65G. All with pre-deletion manifests on node /tmp. Post-state: roots now 43-68% used (spark0 was 84%). (2) MLOOP SMOKE vs THE SERVED NVIDIA PACK: ALL THREE LEGS PASS — scalar W1 rel_l2 0.00222/cosine 0.999998, tile W3 (Mloop) 0.00278/0.999996, tile DOWN 0.00237/0.999997 — the tensor-core prefill path is proven on the exact served bytes. (3) DSV4PRO/QWENMAX BACKUP REFRESH 16/16 (the stale pre-fixup backup bytes replaced for dsv4_pro.tp16/tp4pp4 + qwenmax.pp16 + qwen38_max.tp4pp4); sparkf needed a lazy remount first — its warm mount session had gone stale Permission-denied (same class as rtx5090; mount unit restart fixes). (4) CLUSTER HEALTH: mon.spark0 low-disk warning CLEARED (post-cleanup 54% used); BlueStore slow-op windows cleared; remaining warnings = insecure-key-type notice (client.ds4warm aes — legacy mount key, functional) + cephadm stray-daemon notice (the stopped rtx5090 mds container). (5) RTX5090 MDS DENIAL ROOT-CAUSED (for the record): the container announced 10.10.250.2 (eno1 /30 — unroutable from sparks) as its MDS address; during its active window clients simply could not reach the metadata server → EACCES/timeouts; also its standby-ping to the then-active 10.10.100.2 failed (blocked path). A future retry MUST use --public_addr on a client-reachable address (192.168.50.4) — logs preserved at rtx5090:/tmp/mds-rtx-full.log.
2026-09-06T14:30:00Z firing 263: MTP VARIANT PROGRAM — PLAN (operator: separate MTP file per rank, mix-and-match quants; strip remaining MTP from mains; survey done). INVENTORY (family, arms, MTP-now, strip-path): glm53flash g5nsp/.sp (6 arms) MTP-FREE ✓ strip=g5nsp✓ proven-inverse; qwen3flash q4fsp (7 arms) MTP-FREE ✓ strip=qwen4_flash✓; dsv4pro/flash spstage (4 arms) MTP-FREE ✓ strip=dsv4✓; qwenmax.pp16 + qwen38_max.tp4pp4 V2/spstage — strip PENDING family registration; qwen27b arms (qwen27b.tp4 qwen36sp 120B-header + tp4pp4 spstage + nvfp4a16) MTP-CARRYING (18 entries per earlier audit) — strip PENDING registration; hy4 (hy4.fp8.tp16 6-node partial + ud-iq1m 16) = RAW per-rank safetensors + manifest, MTP-carrying by lane design — strip = safetensors MTP-tensor filter; k3/glm53full = no MTP in source, N/A. SURVEY (hash-verified): qwen3.8-flash family ALL CURRENT (nvidia nvfp4 = 11/11 weight oids match warm, fp8 = 131/131 identical at revision diff, base 08-27 current) — no qwen flash weights update found; zai-org GLM-5.3-Flash = metadata-only (62/62 weights identical); RadixArk GLM-5.3-NVFP4 shard match CURRENT; kimi-k3/hash retry + redhatai + glm-5.3-bf16 repo-mapping = inconclusive (slow-hash jobs); drafters mostly not on HF or private. PHASE 1 (strip remaining): (a) qwen36sp family registration in the strip tool → qwen27b.tp4 16-wide; (b) raw-safetensors MTP filter for hy4 (drop 39 MTP tensors, rewrite manifest) 6+16 ranks; (c) V2 spstage registration → qwen27b.tp4pp4 + qwenmax.pp16 16-wide. PHASE 2 (MTP sidecars, per rank, separate files per operator design): generator slices the trailing MTP tensors from the quantized reference source (operator tip: at the END of the quantized reference models) into <model>.mtp.<quant>.rank<h>.sp sidecars; pilot = glm53flash fp8 TP16 (the serving contract quote: layer-45 slice + MTP head); then per-model 16-wide. PHASE 3: quantized MTP variants (4-bit MTP with FP8 experts, 8-bit MTP with 16-bit) once serving defines the quantized-MTP wire. Note: actual g5 MTP delta per rank = 616,687,552 B (588 MiB), not 67MB — the 1187-tensor contract total is 22,322,734,528 B as previously receipted.
2026-09-06T14:40:00Z firing 264: SPARKF INCIDENT CLOSED + MTP PROGRAM RESHAPED. INCIDENT: my detached rankf rebuild on sparkf (a MON quorum member with idle serving daemons) contributed to memory exhaustion — operator had to power cycle. LESSON CODIFIED: heavy builds NEVER on mon-host nodes; check free RAM + ceph role before launch; monitor to completion. RECOVERY: the detached rebuild actually COMPLETED pre-cycle — fsck kept the correct MTP-free rankf.sp (21,706,046,976 B, sha f0e94cd3… verified against the placed sha); fsck-quarantined debris (.destroyed-*, .mtp, .mtp.partial, stale rank8.sha256) manifest-then-removed; all 16 fp8.tp16 ranks now consistent MTP-free (1160t, dir_sha 54e2474b… fleet-wide). /tmp/rank*.mtp.g5nsp staging cleaned on 8 nodes. MTP PROGRAM RESHAPED per operator: (1) MTP NEVER appended into main packs — SEPARATE sidecar files per rank (mix-and-match: 4-bit MTP over FP8 experts, 8-bit MTP over 16-bit experts); (2) strip remaining MTP from mains that still carry it (qwen27b arms qwen36sp-format, hy4 raw-safetensors arms, qwenmax V2 — needs per-family strip support); (3) sidecar generator next: slices the trailing MTP tensors from the quantized reference sources (they sit at the END of the quantized reference models per operator) into <model>.mtp.<quant>.rank<h>.sp sidecars; pilot = glm53flash fp8 TP16; topology coverage per operator: TP4×PP4 + one model TP4 + one model TP8.
2026-09-06T14:50:00Z firing 265: 1GB RAM CAP DEPLOYED FLEET-WIDE (operator directive after the sparkf incident). TOOL: /usr/local/sbin/sparkcap on all 16 nodes — wraps any command in a systemd cgroup v2 scope: MemoryMax=1GiB (default, --mem MB to override), MemoryHigh=700M early throttle, MemorySwapMax=0 (no swap escape). Runs as the invoking user (SUDO_USER mapping) so file ownership stays correct. VERIFIED: scope properties confirmed live (MemoryMax=1073741824/High=750780416/SwapMax=0); 512M workload survives; exceeding workloads are THROTTLED into a crawl (direct-reclaim livelock) and eventually OOM-killed — they cannot harm node memory; a stuck run is killable via systemctl stop <scope>. OPERATIONAL LAW: all heavy ops (pack builds, sha sweeps, mirror rsyncs, fetches) run as `sudo sparkcap <cmd>`; mon-host nodes get no exceptions — the cap makes them safe. BEHAVIOR NOTE: exceeding a cap = slow grind, not instant death — visible and killable, which is the protective intent. SPARKF INCIDENT LESSON would have been prevented: the rankf rebuild under sparkcap would have throttled at 700M instead of exhausting node RAM.
2026-09-06T15:00:00Z firing 266: NEW MODEL BATCH ADDED TO PROGRAM (operator: find + download + stagepacks, usual variety; TP16 preferred, TP8/TP4 where needed; latest gen only; Ling-2.6/Ring-2.6/Laguna-M.1 dropped). LOCATED ON HF (six picks + optionals): Ling-3.0-flash = inclusionAI/Ling-3.0-flash 237GiB bf16 base with OFFICIAL fp8 (120GiB) / int4 / fp4 quants; Ling-3.0-flash-Fin = inclusionAI/Ling-3.0-flash-Fin 237GiB (financial specialization, same family); Gemma-4 = google/gemma-4-26B-A4B-it 48GiB (A4B = ~4B-active MoE) + google/gemma-4-31B-it 58GiB + nvidia Gemma-4-31B-IT-NVFP4 30GiB (check 26B NVFP4 too); Laguna = poolside/Laguna-S-2.1 219GiB bf16 + -FP8 122GiB + -NVFP4 93GiB + -INT4 + A DFlash-NVFP4 DRAFTER repo (2GiB — Laguna has a native speculative drafter!) + Laguna-XS-2.1 62GiB option; Muse-Glimmer-30B = meta-models/Muse-Glimmer-30B 55GiB bf16. NOTE: Ling-3.0-flash community GGUF names carry MTP (ROCmFP4-STRIX-MTP) — Ling family likely has MTP layers (verify in config at download). PHASE D (download): pull to /mnt/model-warm per source-of-record convention: base bf16 + official fp8 for each (est ~1.9T total for the six); NVFP4 quants as the 4-bit reference where official exists. PHASE P (packers): Ling/Gemma/Laguna/Muse are NEW families — each needs contract freeze + packer + serving module per the lane pattern before stagepacks; Gemma-4/Laguna/XS may share driver work per operator. DOWNLOAD ORDER: start with Ling-3.0-flash family (bf16+fp8, ~477G), then Laguna-S-2.1 (bf16+fp8+nvfp4+DFlash drafter ~436G), Gemma-4 (48+58+30 = 136G), Muse (55G), Fin last (237G). All downloads run under sparkcap.
2026-09-06T15:20:00Z firing 267: NEW-MODEL DOWNLOAD QUEUE RUNNING on spark6 (detached, resumable: hf retry-loop x8 + .WARM-COPY-COMPLETE markers, sparkcap-wrapped 4G). ORDER: Ling-3.0-flash bf16 237G → Ling fp8 120G → Laguna-S-2.1 bf16/fp8/nvfp4/DFlash-drafter (219+122+93+2G) → Gemma-4 26B/31B/nvfp4 (48/58/30G) → Muse-Glimmer 55G → Ling-Fin 237G ≈ 1.66T total. PROBE: Ling-3.0-flash = BailingMoeV3ForCausalLM, 42L + 1 MTP layer (num_nextn_predict_layers=1, mtp_use_kda) — the Ling lane's known family. hf CLI installed on spark6 (huggingface-hub 1.30). MTP STATUS FOR EXISTING MODELS (the reversal state): MTP-FREE = glm53flash ×6 arms (fp8/nvfp4/bf16 × tp16/tp8/tp4pp4; today's append wave fully reverted via the strip-tool inverse, byte-verified; sparkf rankf rebuilt after its pack was corrupted by the killed append), qwen3flash ×7 arms (built --no-mtp), dsv4pro ×2 + dsv4flash ×2 (compacted), qwenmax.pp16 (stripped); N/A-BY-SOURCE = glm53full ×6 + k3 ×2 (checkpoints carry no MTP); STILL MTP-CARRYING = qwen27b arms (tp4/tp4pp4 18 MTP entries + nvfp4a16.tp4 status unprobed — needs qwen36sp strip-family registration), hy4 (fp8.tp16 6-node partial + ud-iq1m 16/16, 39 MTP tensors — needs raw-safetensors filter); MTP SIDECARS (separate files, the operator design for quant mixing) = NOT BUILT yet — generator is the next dev item; the g5 MTP delta per rank measured 588 MiB (not the 67MB quote).
2026-09-06T15:30:00Z firing 268: CORRECTION — "GLM-5.3-full and kimi-k3 have no MTP by source" WAS WRONG. Operator challenged it (vLLM GLM-5.3 recipe: method=mtp, 5 speculative tokens). Re-verified live: glm-5.3-bf16 config DECLARES num_nextn_predict_layers=1 + index_share_for_mtp_iteration=True, and the index holds 79 distinct layer indices (0..78) against 78 declared layers — layer 78 (791 tensors) IS the native MTP layer, hiding as model.layers.78.* with NO "mtp"/"nextn" string in the name. THE OLD AUDIT'S NAME-GREP METHOD MISSED IT, and I repeated its conclusion without re-verifying. kimi-k3: names use language_model.model.layers.N. — 93 distinct layers, max 92 — MTP presence UNRESOLVED (config lacks num_hidden_layers/nextn keys; the repo ships configuration_kimi_k3.py which declares them). IMPACT: glm53full arms (bf16/fp8/nvfp4 × tp16/tp4pp4) join the MTP-sidecar program IF their packs dropped layer 78 (glm52 packer layer-range check pending); kimi-k3 arms pending the architecture-file check. METHOD LAW going forward: MTP detection = config nextn declaration + max-layer-index vs declared count, NEVER name-grep.
2026-09-06T15:40:00Z firing 269: THE GOAL — MTP VARIANT PROGRAM + NEW MODELS, COMPLETION LEDGER. G1 glm53flash: MTP sidecars for 6 arms (fp8.tp16/fp8.tp8/fp8.tp4pp4/nvfp4.tp16/nvfp4.tp4pp4/bf16.tp16), 16 ranks each, MTP dtype = each arm's source (fp8/bf16/nvfp4 layer-45 slices) — needs the sidecar generator (adapt g5_add_mtp emission). G2 qwen3flash: sidecars for 7 arms (MTP from bf16 base / fp8 / nvidia model-fp8-mtp-ple). G3 dsv4pro+flash: sidecars for 4 arms (ga/0731/nvfp4-jarrelscy sources). G4 qwenmax.pp16 + qwen38_max.tp4pp4: sidecars (radixark nvfp4 source). G5 qwen27b: COMPACT-STRIP mains (qwen36sp family registered today, mid-file MTP class, --compact; backup-then-strip per rank) THEN sidecars (fp8 source MTP). G6 hy4: raw-safetensors MTP filter (39 tensors) on fp8.tp16 6 nodes + ud-iq1m 16, THEN sidecars. G7 glm53full: CONFIRMED has MTP (operator + verified: layer-78 = 791 tensors bf16 / 1569 fp8, quantized same as model per operator) — check packs kept layer 78, then sidecars ×3 quants. G8 k3: N/A (no MTP — operator confirmed). G9 quantized-MTP variants (4-bit MTP over FP8 experts etc.): blocked on serving wire format. H1 new-model downloads: 11 repos 1.66T running on spark6 (queue + markers + retry loop). H2 new-model stagepacks: after downloads, per-family contract/packer lane work. W1 watch: redhatai nvfp4 + glm-bf16 hash re-checks; rtx5090 mds debug (tooling ready); sparkcap now LAW for all ops.
2026-09-06T16:00:00Z firing 270: GOAL CHECKPOINT — WHERE EACH THREAD STANDS. (1) qwen27b.tp4 strips: DONE+verified = spark0 (rank0, pilot), spark1 (rank1), spark2 (rank2), spark3 (rank3), spark4 (rank0-dup), sparkb (rank11/TP4-rank3 via mesh). REMAINING = spark5 (rank1), spark6 (rank2), spark7 (rank3), spark8 (rank0), spark9 (rank1), sparka (rank2), sparkc (rank3), sparkd (rank0), sparke (rank1), sparkf (rank2) — 10 nodes. BLOCKER RESOLVED: the qwen36sp family registration existed only on the Mac — the nodes' strip tools were the old build (silent invalid-choice failure, swallowed by tail pipes in the wave script — pipes defeat set -e, noted again). Tool now synced to spark1-6 via bundle; spark6 needed a re-sync (first bundle fetch silently no-oped). NOTE: qwen27b.tp4 packs are TP4-mapped — ranks cycle mod 4 across nodes (spark0/4/8/c hold rank0 etc.); the per-node strips are independent per pack so the mapping only matters for receipts. (2) NEW-MODEL DOWNLOADS: queue running on spark6, Ling-3.0-flash bf16 DONE (237G), Ling fp8 in flight 33/34 files. (3) SURVEY slow-hash: kimi-k3 CURRENT (shard hash matches, 09-02 mod was metadata-only); redhatai nvfp4 INCONCLUSIVE (local hash came back empty — spark2 mount read glitch, needs retry); glm-5.3-bf16 vs zai-org/GLM-5.3 = filename mismatch (repo mapping needs rework, inconclusive). (4) SPARKF: rankf rebuilt+verified+locked pre-power-cycle; post-cycle fsck preserved it (hash-verified f0e94cd3); quarantined debris removed. LESSON APPLIED: the wave-script pipes (cmd | tail -1) defeat set -e — future waves use explicit rc checks or run cmd without pipes.
2026-09-06T17:00:00Z firing 271: THE GOAL — COMPLETE ALL PENDING ITEMS (operator directive). Single completion criterion: every model/arm in the fleet is (a) MTP-free mains with (b) MTP sidecar files where the source carries MTP, (c) all new-model sources downloaded and receipted. EXECUTION ORDER + EXIT CRITERIA: [1] qwen27b.tp4 strips: 6/16 done (spark0/1/2/3/4/b, sentinel-verified); remaining 10 nodes (spark5/9=rank1, spark6/a=rank2, spark7/c=rank3, spark8/d=rank0, sparkf=rank2) — tool synced via bundle first, strip per node ~2 min. EXIT: 16/16 zero sentinels. [2] qwen27b.tp4pp4 + nvfp4a16.tp4: probe format → strip (same family or new registration). EXIT: 32/32. [3] hy4 strips: raw-safetensors MTP filter (39 tensors) on fp8.tp16 (6 nodes) + ud-iq1m.tp16 (16) + manifest rewrite. EXIT: zero MTP tensors by index count. [4] MTP SIDECAR GENERATOR: standalone tool emitting <model>.mtp.<dtype>.rank<h>.sp per rank (header + MTP entries + payloads, sliced per topology), validated entry-by-entry against the source. PILOT: glm53flash fp8.tp16 rank0. [5] glm53flash sidecars: 6 arms × 16 (fp8/bf16/nvfp4 MTP dtypes matching each arm's source; tp16/tp8 full-slice, tp4pp4 last-stage). [6] qwen3flash sidecars: 7 arms (nvidia MTP = model-fp8-mtp-ple.safetensors). [7] dsv4pro/flash sidecars: 4 arms. [8] qwenmax + qwen38_max sidecars. [9] qwen27b + hy4 sidecars (after 1-3). [10] glm53full: layer-78 pack check then sidecars ×3 quants (bf16 791t / fp8 1569t / nvfp4 source). [11] k3: N/A (operator-confirmed no MTP). [12] quantized-MTP variants (4-bit/8-bit MTP): blocked on serving wire format — NOT in this goal. [13] NEW-MODEL DOWNLOADS: 11 repos 1.66T on spark6 queue (running; Ling bf16 done, Ling fp8 97%) — EXIT: all 11 .WARM-COMPLETE markers + receipts. [14] survey tail: redhatai shard hash retry; glm-5.3-bf16↔zai-org/GLM-5.3 repo-mapping check. LAW: every heavy op runs under sparkcap (1G cap); no heavy builds on mon hosts.
2026-09-06T15:40:00Z firing 272: PACK WORK STOPPED per operator ("all the packs are wrong — stop until sorted"). STATE AT STOP: qwen27b.tp4 strips ran to completion 14/15 on the previously-unstripped nodes before the stop (spark1-f; spark0 was already done in the pilot; sparkf's receipt write hit the root-owned sidecar — needs sudo receipt fix on resume); all 14 verified zero-sentinels, 855 entries, 10.24GiB each, locked. The five appended-then-reverted ranks (spark5/9/a/c/d) also done earlier. No further pack mutations. DOWNLOADS: Ling bf16 complete+marker (238G); Ling fp8 in flight ~97%; queue healthy on spark6. NEXT (on operator go): finish downloads, add PUBLISHED markers, run cold-archive queue, verify cold copies.
2026-09-06T16:10:00Z firing 273: STATUS UPDATE (operator: "the packer didn't pack properly — still isolating a ~1% error, don't know how to make the correct packs yet"). PACK WORK FULLY STOPPED — the qwen27b strips ran against packs built by the defective pipeline; strips are reversible (the strip tool's exact inverse = the proven g5_add_mtp append), so nothing is unrecoverable when the packer is fixed. IMPACT ASSESSMENT deferred until the ~1% error is isolated (operator's lane is on it). ALL OTHER THREADS UNAFFECTED: new-model downloads running (Ling bf16 done, fp8 ~97%), cold-archive path verified, warm-cold backup operational, sparkcap + mesh + emergency paths live. STANDBY for packer fix.
2026-09-12T16:52:20Z firing 274 (M-2, stagepack campaign): MUSE.TP16.BF16 PLACEMENT MATRIX 16/16 VERIFIED — spark0 ✅ spark1 ✅ spark2 ✅ spark3 ✅ spark4 ✅ spark5 ✅ spark6 ✅ spark7 ✅ spark8 ✅ spark9 ✅ sparka ✅ sparkb ✅ sparkc ✅ sparkd ✅ sparke ✅ sparkf ✅. Reality vs the brief's 1/16: all 16 masters were staged on sparkc and every node already held its rank (placed 2026-09-11 03:23-03:37 +0900, preserved mtimes) — M-2 verified the whole chain without moving a byte: structural walk 16/16 (magic GSMU, 419 entries, first payload 23808, kind census 3+8x52, kv map 0/0-7 1/8-15), staged sha256 == packer receipts 16/16, per-node sweep (pack sha + byte-identical receipt + inner receipt sha, 3,639,537,920 bytes exact) 16/16 PASS with sudo drop_caches per node. Packer parity PROVEN vs the lane tooling (byte-identical at merge point 8d59273 and tip 6c6caf52): tools/muse_parity_sample.py re-derives globals+L51 payloads IN PLACE via the packer's own write_records from the warm source — rank12 reference copy sha stayed b9236b9c..., rank00 sample stayed 928f4e55... (queue jobs muse-parity-r12-sc / muse-parity-r00-s0, exit 0). muse packs postdate the 09-06 pack-stop (built 09-11). CAVEAT: full 16-rank rebuild re-derivation pending — ceph degraded fleet-wide (cold 1GiB = 394s = 2.6 MB/s on spark0 AND sparkc vs healthy ~1.2 GB/s; one rank = ~51 GiB of reads); retry command + full ledger in docs/AGENT_LANE_BRIEFS/reports/muse-tp16-placement-2026-09-12.md; branch lane/muse-tp16-placement off tip 6c6caf52. NOTE: sparkc free space 869G -> 718G from other writers during the campaign (not muse).
