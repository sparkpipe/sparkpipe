...
- **T1.5 — any one returned spark, still offline:** close the F1 DISPOSITION
  owed-evidence before spending the window:
  `bash tools/k3_tp16_tuesday_boot.sh gate` stages the proof tree onto
  GATE_HOST and runs `tools/k3_nccl_multirank_gate.sh` — WORLD real OS
  processes bootstrap ncclCommInitRank through SparkTpDeviceCollectiveCreate
  and prove the two device-tier exchanges live (bf16 hidden sums bit-exact at
  width 7168 + narrowed-submit tail integrity; head u64-max argmax over
  engineered cross-rank cases vs the packed-key oracle with host-rule parity
  on finite rows; back-to-back ordinal chain) at worlds {2,4,8,16}.
  `tools/k3_tp16_tuesday_boot.sh wait,recover,gate,configs,stage,deploy,`
  `registry` chains every offline step of this sheet (playbook §5 recovery
  included); append `window boot` with `WINDOW_REQUEST=1` for Phases
  3-4. RED = STOP.
