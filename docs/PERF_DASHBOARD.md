...
Last update: 2026-08-19 ~08:40 UTC (ON-BOX CONFIRMATION: the deployed build's DFlash2 block forward FIRES on a live request - boot log shows 'dspark_pack_loaded tensors=81' + 'dspark_block_forward entry armed=1 count=7 rows=1', and a fresh /tmp/dspark_drafts.bin (28 B = 7 ids) proves the selector ran on the armed instance. Parity holds on the new input too: c0=270/base_pos=129 -> [270x7] == reference (constant-draft walk for that input; the varied [1074,8422,...] case still matches). Sysadmin agent stood down; 27B lane stays silenced; box untouched (pid 637688, preflight PASS). The trajectory losslessness fix (GLM-credited) is STILL the only gate - the drafter firing does not make the verify/replay lossless. Kernel lane on it. DSV4-FLASH PAUSED; no HWM change since 8.03)
