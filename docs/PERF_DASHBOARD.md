...
Last update: 2026-08-19 ~10:35 UTC (NO-SPEC WINDOW VERDICT (kernel-lane recipe, coordinator-run 08:34-08:36 UTC): SPECULATE=0 + dspark vars in place on the same binary -> no-spec branch WORKS clean (40/40 tokens, 167 submissions, status=0) - the old IO_ERROR was the stripped-env init failure, confirmed. ALSO SOLVED the old status=1 mystery: follow-up requests failing status=1 were the TEST HARNESS reusing fixed request_ids (residentd enforces submission-id monotonicity) - with fresh ids the second request is clean; the k-sweep tooling MUST mint fresh ids per run. Kernel-lane per-boot classification: the cliff is DETERMINISTIC (every max-accepted=6 segment has exactly one refusal; below-cliff segments run 37-556 rounds refusal-free) - the decay was cliff + lane loss, not drafter quality. Non-destructive-refusal patch (validation refusals must not ReleaseLane) commissioned; fresh-tap verdict on the cliff-fixed dumps still owed and decides the k-sweep. Box armed + immutable. DSV4-FLASH PAUSED; no HWM change since 8.03)
