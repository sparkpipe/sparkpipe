...
Last update: 2026-08-19 ~14:00 UTC (THE ACCEPTANCE COLLAPSE AND THE DIVERGENCE ARE THE SAME BUG - MEASURED. The acceptance profile tool (dc0e701, kernel lane) proves: (1) the oracle drafts earn the SAME 0.853 accepted/round as the module's - the 8/34 forward flips cost ZERO acceptance, forward bit-exactness is not the lever; (2) the post-fix GLM capture shows the zero-acceptance suffix STILL UNBROKEN from position 231 (9/34 rounds) - the conv-delta fix did not cure it; the context is still being poisoned. The suffix's onset (231) matches exactly the position-235 state divergence the decode-state dumps found, so the acceptance collapse and the stream divergence share ONE mechanism (the D=7 verify/replay context poisoning that D=2 never exercises). The healthy prefix still earns only 1.16 accepted/round on the GLM prompt - the bar 6.00 needs the collapse fix + the MT-Bench prompt set (L=3.74-4.38 measured = published range) + serving-side tuning. Kernel lane: trace the replay frame's attention KV reads at the onset position. Box armed + immutable; /tmp cleaned. DSV4-FLASH PAUSED; no HWM change since 8.03)
