...
Last update: 2026-08-19 ~12:00 UTC (COMPARABLE-PROMPT SWEEP - the big news and a NEW blocker. ACCEPTANCE ON MT-BENCH-CLASS PROMPTS LANDS IN THE PUBLISHED RANGE: writing L=4.10, roleplay 3.74, reasoning 3.97, math 4.38, coding 3.85 (published: MT-Bench 4.10, HumanEval 4.39, MBPP 4.79) - the devcycle prompt was indeed the wrong yardstick. BUT 2/5 PROMPTS DIVERGE FROM THE GOLDEN: roleplay 49 diffs from token 75, coding 110 diffs from token 16 - DETERMINISTIC across two sweeps, SILENT (zero refusals/repairs/clamps in the windows), prompt-specific. The committed stream carries non-golden tokens -> the TARGET's verify walk emitted wrong ids -> a remaining verify/replay state corruption that specific acceptance patterns trigger (GLM-class hole, new dress). Kernel lane redirected to bisect it with the sweep2/golden artifacts; the eb72bfc round instrumentation goes into the next deploy to name each round vs the golden. The 7/37 forward-numerics fix is now SECOND. Race bar: acceptance >= 6 AND lossless everywhere - acceptance range is nearly there, correctness is not. Box armed + immutable. DSV4-FLASH PAUSED; no HWM change since 8.03)
