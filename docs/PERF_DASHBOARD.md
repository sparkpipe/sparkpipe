...
Last update: 2026-08-19 ~14:30 UTC (THE RACE'S HARD BLOCKER IS FOUND AND FIXED - the credited acceptance ceiling was FIVE. The adapter's DSpark depth was 7 while the kernels walk 8 rows, so the seventh draft was discarded, the accept loop capped at 6, and the cliff clamp then capped the CREDITED acceptance at 5: average acceptance >= 6 was ARITHMETICALLY IMPOSSIBLE on that build regardless of the drafter. LANDED 7ffac6c: depth 7->8, MAX_MTP_DRAFT_TOKENS 8->10, driver cap 8->10, credited ceiling now 7, printed with every round. VERIFIED LIVE: the spec audit's round_ids show credited=7 rounds now occurring. Also the divergence audit (SPARK_QWEN36_SPEC_AUDIT) answered the correctness question: ZERO replay_row_mismatch lines = the replay's shared rows agree with the verify's; the divergence is the replay's FINAL EMISSION - the one committed token checked against nothing - exactly the lane's predicted mechanism; the lane writes the shadow-decode instrument next. The 16-prompt benchmark pack (GSM8K/MATH-500/HumanEval/MBPP) is measured: 14/16 LOSSLESS, spec slower than no-spec on all 16 (fusion still pending). Next: the final-emission fix + the per-prompt acceptance profile with the ceiling at 7. Box armed + immutable. DSV4-FLASH PAUSED; no HWM change since 8.03)
