...
Last update: 2026-08-19 ~08:00 UTC (spark3 deploy now the best state ever: unit synced to the repo template (dropped the unexpanded Environment=LD_LIBRARY_PATH line the old deployed copy had), residentd.env carries all 8 vars incl. expanded LD_LIBRARY_PATH, preflight all-10-PASS (pid 637037). File-backed drafter diagnostics landed (53cb8b6, kernel lane patch w/ corrected rationale: unit stderr goes to /tmp/fleet-swap-residentd.log, shared + journalctl-invisible, hence SPARK_QWEN36_DSPARK_DIAG_PATH -> /tmp/dspark_diag.log per instance). No fleet_swap cycler exists - the 16:10-16:13 unit cycling was concurrent agent deploy activity, now stopped; 27B lane interrupted + parked (was about to run an invalid k-sweep before the trajectory fix). Trajectory losslessness fix (GLM-credited) remains the ONLY gate; kernel lane has the precise stale-KV hypothesis (P+3+accepted..P+7 surviving the replay, next-cycle first decode attends to its own row). DSV4-FLASH PAUSED; CI green; no HWM change since 8.03)
