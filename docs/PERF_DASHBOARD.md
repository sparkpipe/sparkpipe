...
- 27B: DSpark rung-3 weights packed (1.36B); 7-vs-7 diff = ONE decision: draft[1] 16 (ours) vs 17 (numpy ref) - rest is cascade; canonical config MINED from Doopeworld README (GPTQ-Int4 target + kv-cache-dtype fp8 + method:dspark; nearest local = FP8 target) - oracle re-armed (restart residentd w/ smaller KV, ~40GiB free); oracle = structural confirmation only (FP8 rounding != our packer); tie arbitration stays zero-cost: margin + tap diff + 3-way convention diff (still owed); then 20-58 DSpark.
...
Last update: 2026-08-18 21:55 UTC (CI green; Flash: frame-149 pad-row check in flight; 27B: canonical README config mined, oracle re-armed as structural-only, zero-cost tie analyses required; no new HWM since 06:45)
