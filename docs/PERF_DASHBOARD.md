...
Last update: 2026-08-19 ~13:40 UTC (TWO REAL RACE FIXES LANDED, THE DIVERGENCE PERSISTS: the GDN-step and conv-update recurrence races (parallel row blocks racing the state/tail read-modify-write) are REAL and are now serialized (06b5b2e + 4836af5, grid.y=1 + sequential row loops) - but the roleplay divergence survives with the IDENTICAL signature (49 diffs at token 75). The invariants still hold: 232 walk clean, restore perfect, replay outputs golden, replay state 100% wrong, MTP D=2 lossless. The remaining space: the D=7 verify/replay frames' KV interaction that D=2 never exercises - the kernel lane must trace the replay frame's attention KV reads at rows 233/234 against the verify's speculative rows (the one input that differs between the replay and the no-spec decodes). The lattice stage verdict also landed (e60dcd9: the 8/34 draft flips are sub-ULP noise-band samples, NOT a defect - acceptance comes from the serving side). Box armed + immutable; /tmp cleaned. DSV4-FLASH PAUSED; no HWM change since 8.03)
