...
| Qwen 3.8 27B | bit-exact GPU validation + genuine prompt; O128 pins: BF16 7b58db3a..., mixed/FP8 80e8fb9d... (a9385d0b is the DSV4 golden, NOT qwen38_27b) | DSpark draft parity BIT-EXACT vs fixed reference [220,16,92,198,12,328,82] (draft[1] = genuine 19.625 tie, first-max -> 16); spec speed not yet measured | TP1 |
...
Last update: 2026-08-22 16:40 UTC (CONSOLIDATION MERGE IN FLIGHT. The glm dev reached a good stopping point on qwen27B DFlash2 - very close to SOTA - and merged their line to main: WS kernel staging +10% (PR #678), the ops runbook (PR #679, docs/QWEN38_DFLASH2_RUNBOOK.md - self-contained: artifact map, build/deploy/launch, embedded O512 benchmark generator + verification stream hashes, troubleshooting), plus incremental context-KV machinery (precompute_and_store_context_kv) and a PREFIX_RESUME flag. Our unified carries the complementary half: credited-ceiling fix, verify-step routing, full-sequence N+8-key context, prefix caching core, k3/glm52/max round-2 waves. A merge consolidator agent (70ca7228) is resolving the 71 conflict hunks across 8 files under keep-BOTH-feature-set rules; gates + my review before it lands. Benchmarks vs the glm serving (spark3:8124 vLLM) were launched; qwen27B model agent spawned (3798bef7). NO HWM MOVES this tick.
