...
Last update: 2026-08-19 ~09:50 UTC (HONEST CORRECTION - the lossless gate is PROMPT-LIMITED, not closed: on the 1-token prompt [0] the spec stream == no-spec golden bit-exact (128/128, hash 84c1035f, measured 8.028 no-spec / 5.93 spec tok/s @ L=0.55). But the REAL O128 (128-token prompt, /tmp/o128_batch.json - the GLM 270/271 shape) STILL dies at submission 129 with status=4 on the deployed echo-fixed adapter. Coordinator residency diag (e998d25) proves the ADAPTER echo is perfect (sid==pending_sid==origin, hex==sid for all 129 completions) - so the route's expected residency differs from what the runtime sent the adapter: a runtime/wire-side minting or continuation-lease reservation mismatch that correlates with prefill size (1-token prompt clean, 128-token prompt fails at the final submission). Kernel lane has the evidence + the runtime bisect (model_batch_engine.c minting + node/model_residentd.c:1880-1905 continuation reservation + ipc wire). Rollback repair (e273951) + residency echo (a7acd79) remain landed and necessary. Acceptance gap (L=0.55 vs authors' 3.39) + tap-capture interrogation still queued behind this seam. Box armed + immutable. DSV4-FLASH PAUSED; no HWM change since 8.03)
