...
Last update: 2026-08-19 07:05 UTC (Draft parity GREEN on live serving; GLM (spark2) found the ROOT BUG in the shared verify/replay - spec stream diverges from the no-spec golden at token 15 (extra 270 vs 271; MTP D=2 control reproduces) = the acceptance-decay + DSpark port-gap root (credited on PR #675); 27B lane redirected to fix the trajectory losslessness (KV row positions / GDN snapshot-restore) - k-sweep is INVALID until the stream is lossless; DSV4-FLASH PAUSED; CI green; no HWM change since 8.03)
