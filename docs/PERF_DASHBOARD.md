...
Last update: 2026-08-19 ~10:15 UTC (GLM GATE FULLY GREEN - the trajectory losslessness bug is DEAD on both prompts. ROOT CAUSE OF THE REMAINING FAILURE = THE ACCEPTANCE CLIFF (kernel lane, 6-for-6 log correlation): a full acceptance (min_accepted=6) made the round emit 9 tokens under the 8-token driver contract cap -> submission REFUSED -> dropped -> every lane it touched cooled -> next round accepts 0 AND the dropped round's tokens re-derived from a cooled lane (the 270-vs-271 divergence source). FIX LANDED 45a9f45 (clamp credited depth to MAX-3, derive from the contract constant; replay re-walks exactly the credited prefix - stream-exact). VERIFIED ON THE GLM PROMPT (128-token prefill O128): spec stream == no-spec golden BIT-EXACT 128/128 (hash b7543553), no refusal; spec 6.35 tok/s vs no-spec 6.35 (parity at accepted~0). Prompt-[0] lossless still holds (84c1035f). The earlier degenerate repeat-270 drafts were the CONTAMINATED trajectory - the fixed binary now drafts varied ids ([198,27,220,24]...) - so the acceptance climb (L=0.55 -> authors' 3.39) may already be unlocked; kernel lane re-examining the fresh taps. Box armed + immutable. DSV4-FLASH PAUSED; no HWM change since 8.03)
