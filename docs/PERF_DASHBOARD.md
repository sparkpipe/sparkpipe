...
Last update: 2026-08-19 ~07:40 UTC (spark3 serving deploy FINALLY VERIFIED: preflight all-10-PASS against the armed FP8-root process (pid 627705, pack 81 tensors, spec_method=dspark). Two ghost-chase root causes closed: (1) preflight's marker list named the pre-fail-loud revision's literals + a pipefail/SIGPIPE trap made every marker check FAIL on correct drivers - fixed a6d6efc; (2) the unit env file lacked the drafter vars so systemd restarts booted stripped - now persisted in /etc/sparkpipe/residentd.env. One real SEGV 16:13:45 flagged to the kernel lane. Trajectory losslessness fix (GLM-credited) remains the ONLY gate before the k-sweep; kernel lane re-baselined on the armed process. DSV4-FLASH PAUSED; CI green; no HWM change since 8.03)
