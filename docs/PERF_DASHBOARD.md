...
Last update: 2026-08-19 ~11:00 UTC (SEPARATION VERDICT: COMMIT/ACCOUNTING SEAM - already fixed. The kernel lane's 34-round live trace proves the round-1 repeat-270 drafts were CORRECT (the target's own verify emitted 270 everywhere - a genuinely repetitive start-of-generation phase the drafter mirrored faithfully; rounds 2-7 = the target oscillating 270/271, then 26 rounds of varied ids) and the ONLY stream-killer was the full-acceptance refusal (pre-clamp: 8 segments hit accepted=6, each followed by a refusal and segment end; post-clamp: accepted=6 -> accept_clamped -> 34 clean rounds). The cliff fix (45a9f45) is the verdict's fix and is hardware-verified (b7543553 lossless). KNOWN: the k-sweep {4,7,8,12} knob is IGNORED for dspark (draft count fixed at block size 8) - the real acceptance lever per the bench plan is MT-Bench-class prompts (authors' L=3.39/4.80 context), not the devcycle prompt (L~0.5). Non-destructive-refusal patch still owed; fresh-id bench tooling assigned. Box armed + immutable. DSV4-FLASH PAUSED; no HWM change since 8.03)
