...
Last update: 2026-08-19 ~08:55 UTC (TRAJECTORY AUDIT HALF-DONE, REAL DEFECT LANDED: kernel lane's verify/replay audit proved (1) KV row positions IDENTICAL to the no-spec convention (self-healing; suspect 1 CLOSED) and (2) a real rollback hole: when any lane's chain is dead (strict policy) or a replay frame fails, healthy lanes never restored their GDN snapshot - contaminated recurrence (permanent divergence, destructive) + lane_next_positions parked at base+draft_count (the live status=1/4 frame refusals). FIX LANDED e273951: one-row GDN_RESTORE_FIRST repair over C0 for every verify-walked, never-replayed lane. Honest boundary: under DEFAULT policy with no failures that edge is not hit, so the default-policy divergence points at the commit/accounting seam; the lane is now running the 20-step separation experiment (taps+c0+basepos every draft frame vs golden stream, offline on spark0): taps-degenerate-first -> remaining state hole; token-first -> accounting. Box untouched (pid 637688, armed). No HWM change since 8.03)
