...
Last update: 2026-08-19 ~13:00 UTC (DIVERGENCE CORNERED TO ONE KERNEL: the coordinator's full evidence chain (aligned post-walk dumps f00f170, prefill dumps 046efb7, pointer inventory 883e561, stream-synced restore/replaypost dumps with tails 25e1979) proves: the 232 decode walk is CLEAN on both sides, the snapshot captures the clean state, the RESTORE is PERFECT (state AND tail byte-identical to the no-spec's 232 post-walk), and the REPLAY'S 2-ROW WALK diverges - the only structural difference from two sequential no-spec decodes is the CHUNK CONV kernel (multi-row prefill) vs the per-row UPDATE kernel. The replay's outputs stay golden (the credited tokens matched) while its STATE/tail diverge - exactly a chunk-conv tail-handling bias (read-once vs slide-per-row, or wrong write-back columns). The kernel lane audits SparkQwen36ChunkConvKernel's tail for the multi-row case. Also landed en route: the conv-delta host-malloc overflow fix (ddfefb4 - a real 20,480-byte-per-call heap overflow via UVA, spec-runs-only). MTP D=2 is LOSSLESS on the same prompt (D=7 residue is the trigger). Box armed + immutable; /tmp cleanup done (dump dirs had filled the root fs). DSV4-FLASH PAUSED; no HWM change since 8.03)
