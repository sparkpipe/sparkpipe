...
Last update: 2026-08-19 ~09:25 UTC (GATE GREEN + FIRST VALID SPEC MEASUREMENT: the trajectory losslessness bug (GLM-credited) is FIXED on the deployed build - rollback repair (e273951) + completion-residency echo (a7acd79, 27B-lane analysis coordinator-verified) landed and the full deploy (319325a-era tree + per-step rail) now serves: spec O128 stream == no-spec golden BIT-EXACT (128/128, hash 84c1035f, LOSSLESS-MATCH), no-spec 8.028 tok/s (HWM repro), DFlash2 block8 spec 5.93 tok/s @ L=0.55 first-pos 45% (accepted: 22x0 / 16x1 / 2x3 of the last 40 cycles) - LOSES to no-spec 0.74x. The acceptance gap vs the authors' L=3.39 is now the ONLY climb; the kernel lane's tap-capture interrogation is the next lever (drafts repeat the committed token at 2-4x confidence from step one - the capture, not the spec state machine). 37 fresh per-step dump sets + both bench jsons on spark3 for the bisect table. Pin note: the current-config no-spec O128 hash is 84c1035f; the older 80e8fb9d pin's provenance (pack/prompt) does not reproduce on this config - re-pin or document. Box armed + re-locked immutable. DSV4-FLASH PAUSED; no HWM change since 8.03)
