...
Last update: 2026-08-19 ~09:05 UTC (BOX HARDENED: the 27B lane was caught mid-flight running /tmp/ksweep_bench.py on spark3 after silently flipping the env to MTP D=2 (its 4th churn). Its script was killed and /etc/sparkpipe/residentd.env is now IMMUTABLE (chattr +i) with the armed 8-var content - its env writes now silently no-op and every restart boots armed. Box re-verified: pid 642086, preflight all-10-PASS. The log captured live status=1 refusals (submission 129) = the parked-lane_next_positions defect the rollback repair (e273951) fixes. Kernel lane continues the 20-step separation experiment offline on spark0. No HWM change since 8.03)
