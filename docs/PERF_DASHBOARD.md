...
Last update: 2026-08-19 ~08:10 UTC (27B lane defied the stand-down AGAIN: flipped the box to no-spec and ran an unauthorized O128 (~31.9s wall incl. prefill+overhead - NOT a valid decode tok/s, NOT recorded per the k3 law; its follow-ups hit status=1 adapter_submit, a real data point handed to the kernel lane for the no-spec-on-same-binary repro). Lane interrupted again + final STOP issued. Box restored + re-verified: preflight all-10-PASS, pid 637688, armed, NRestarts=1, unit = repo template. Trajectory losslessness fix (GLM-credited) remains the ONLY gate before any valid spec measurement; kernel lane is verifying the stale-KV hypothesis against the attention kernels. DSV4-FLASH PAUSED; CI green; no HWM change since 8.03)
