...
Last update: 2026-08-19 ~13:20 UTC (THE MECHANISM IS NAMED - the GDN state update races across a multi-row frame's rows. The chunk-conv hypothesis is WITHDRAWN (the target's conv launcher serves all row counts via grid.y; SparkQwen36ChunkConvKernel is dead code). The real mechanism: SparkQwen36GdnStepKernel runs with grid.y = row_count, so the state_f32 read-modify-write races across the verify's 7 rows and the replay's 2 rows - one row's state accumulation is lost per frame. That fits every datum: the replay's outputs stay golden (per-row core_out reads the pre-state + its own delta), the post-replay state differs at EVERY element (100% diff), the next decode reads the wrong state and the stream flips at token 75, and MTP D=2 stays lossless on roleplay because its 2-row race perturbs too little to flip an argmax there. THE FIX: serialize GdnStepKernel's rows in order (bit-match k sequential single-row updates) + a validation asserting 2 single-row frames == one 2-row frame. The kernel lane has the patch task. Box armed + immutable; /tmp cleaned. DSV4-FLASH PAUSED; no HWM change since 8.03)
