...
Last update: 2026-08-19 ~12:50 UTC (DIVERGENCE BISECT COMPLETE - THE CORRUPTION IS NAMED AT THE DATA LEVEL. Three dump cycles (decode-state 892dad8, GDN-entry f0dfae9, snapshot-slot d0f98fe) on the roleplay repro: the spec state first diverges from the no-spec state at decode position 235 (all five tap layers + 44% of the final hidden), the GDN recurrence differs ~97MB/97MB (whole-state corruption), and the round-1 verify SNAPSHOT is ALREADY corrupted - so the corruption PREDATES the verify. The only machinery between the decode walk and the snapshot is the DRAFTER'S BLOCK FORWARD: one of its kernels (attention/conv/selector) writes the TARGET's gdn_pool.state_f32/conv_tail via an overlap or an out-of-bounds write. The stream stays golden until token 75 only because the corrupted recurrence preserves the argmax that long. The kernel lane pinpoints the exact kernel/buffer next - that is the race blocker (losslessness on 2/5 comparable prompts). Box armed + immutable. DSV4-FLASH PAUSED; no HWM change since 8.03)
