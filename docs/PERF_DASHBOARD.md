...
Last update: 2026-08-19 ~15:00 UTC (THE SILENT DIVERGENCE IS FIXED - hardware-measured mechanism, hardware-verified fix. The module has TWO recurrence implementations: the chunk path (every prefill-shaped frame - prompt prefill, verify, REPLAY) and the step path (plain decodes); a GPU validator proved 78% of the GDN state differs between them at fp32-rounding magnitude, and a recurrence REMEMBERS the accumulated difference. Every spec round's replay substituted a chunk-built state for the step-built state the golden trajectory holds - the acceptance decay, the taps diverging from 235, and the non-golden C0 at the first thin margin (why 3/5 prompts stayed lossless). FIX LANDED 2bd2673: GDN_RESTORE_FIRST frames walk the STEP path (bit-identical to the decodes they replace). VERIFIED: roleplay is now LOSSLESS (0 diffs, was 49@75 across 15+ runs); coding improved 110@16 -> 20@108 (a residual remains - the lane chases it with the fingerprint). Combined with the ceiling fix (7ffac6c: credited ceiling 5 -> 7, live-verified credited=7 rounds), the >= 6 acceptance bar is now arithmetically OPEN and the per-prompt benchmark acceptance profile is the next measurement. Box armed + immutable. DSV4-FLASH PAUSED; no HWM change since 8.03)
