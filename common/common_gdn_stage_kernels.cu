...
#define SPARK_GDN_STAGE_CUDA_DK SPARK_LLM_GDN_HEAD_KEY_DIMENSION
#define SPARK_GDN_STAGE_CUDA_DV SPARK_LLM_GDN_HEAD_VALUE_DIMENSION
#define SPARK_GDN_STAGE_CUDA_GVA_GROUP \
	(SPARK_LLM_GDN_VALUE_HEAD_COUNT / SPARK_LLM_GDN_KEY_HEAD_COUNT)
#define SPARK_GDN_STAGE_CUDA_ATTN_GROUP \
	(SPARK_LLM_ATTN_QUERY_HEAD_COUNT / SPARK_LLM_ATTN_KV_HEAD_COUNT)
#define SPARK_GDN_STAGE_CUDA_ATTN_VALUE_PAIRS_PER_LANE \
	(SPARK_LLM_ATTN_HEAD_DIMENSION / (2u * SPARK_LM_WARP_LANES))
#define SPARK_GDN_STAGE_CUDA_GDN_STATE_ELEMENTS \
	(SPARK_GDN_STAGE_CUDA_DK * SPARK_GDN_STAGE_CUDA_DV)
#define SPARK_GDN_STAGE_CUDA_GDN_DECODE_SHARED_BYTES \
	(SPARK_GDN_STAGE_CUDA_GDN_STATE_ELEMENTS * sizeof(float))
#define SPARK_GDN_STAGE_CUDA_CHUNK SPARK_LLM_GDN_CHUNK_TOKENS
#define SPARK_GDN_STAGE_CUDA_GDN_QK_SHARED_BYTES \
	(2u * SPARK_GDN_STAGE_CUDA_CHUNK * SPARK_GDN_STAGE_CUDA_DK * sizeof(float))
#define SPARK_GDN_STAGE_CUDA_GDN_CHUNK_SHARED_BYTES \
	((SPARK_GDN_STAGE_CUDA_GDN_STATE_ELEMENTS + \
	  (SPARK_GDN_STAGE_CUDA_CHUNK * SPARK_GDN_STAGE_CUDA_DV) + \
	  (2u * SPARK_GDN_STAGE_CUDA_CHUNK)) * sizeof(float))

static __device__ __forceinline__ float LmGdnStageRopeFrequency(uint32_t pair)
{
...
static __global__ void LmGdnStageGdnStepKernel(const void *conv_out_bf16, const float *log_decay_f32, const float *beta_f32, float *state_f32, void *core_out_bf16, const uint32_t *row_lane_indices, const uint32_t *state_cold_by_row, uint32_t row_count, uint32_t gdn_layer_ordinal, uint64_t state_lane_stride, uint64_t state_layer_stride, uint32_t tp_degree)
{
    ...
    __shared__ float kn[SPARK_GDN_STAGE_CUDA_DK];
    ...
    head = blockIdx.x;
    column = threadIdx.x;
    key_head = head / SPARK_GDN_STAGE_CUDA_GVA_GROUP;
    if (row >= row_count)
    {
        return;
    }
    ...
    qn[column] = value * rsqrtf(q_norm + 1.0e-6f) *
        rsqrtf((float)SPARK_GDN_STAGE_CUDA_DK);
    ...
    state_base =
        ...
        ((uint64_t)head * SPARK_GDN_STAGE_CUDA_GDN_STATE_ELEMENTS);
    ...
    {
        state_index = (element * SPARK_GDN_STAGE_CUDA_DV) + column;
...
static __global__ void LmGdnStageAttnPrepareKernel(
    void *q_fused_bf16,
    const void *k_bf16,
    const void *v_bf16,
    ...
    uint32_t tp_rank)
{
    const uint32_t local_heads =
        SPARK_LLM_ATTN_QUERY_HEAD_COUNT / tp_degree;
    const uint32_t local_kv_heads =
        SPARK_LLM_ATTN_LOCAL_KV_HEAD_COUNT(tp_degree);
    const uint32_t rank_kv_head_base =
        SPARK_LLM_ATTN_RANK_KV_HEAD_BASE(tp_degree, tp_rank);
    const uint32_t local_token_elements =
        2u * local_kv_heads * SPARK_LLM_ATTN_HEAD_DIMENSION;
    __shared__ float reduce_scratch[SPARK_LM_CTA_WARPS];
    __shared__ float query_shared[SPARK_LLM_ATTN_HEAD_DIMENSION];
    __shared__ float key_shared[SPARK_LLM_ATTN_HEAD_DIMENSION];
    __shared__ float rope_cosine[
        SPARK_LLM_ROPE_DIMENSION / 2u];
    ...
    column = threadIdx.x;
    kv_head = head / SPARK_GDN_STAGE_CUDA_ATTN_GROUP;
    if (row >= row_count ||
        blockIdx.x >= local_heads ||
        column >= SPARK_LLM_ATTN_HEAD_DIMENSION)
    {
        return;
    }
    ...
    query_base =
        ...
        ((uint64_t)blockIdx.x * 2u * SPARK_LLM_ATTN_HEAD_DIMENSION);
    value = SparkLmBf16ToFloat(q_fused_bf16, query_base + column);
    sum_squares = SparkLmBlockReduceSum(
        value * value,
        reduce_scratch);
    inverse_rms = rsqrtf(
        sum_squares / (float)SPARK_LLM_ATTN_HEAD_DIMENSION +
        epsilon);
    query_shared[column] =
        value * inverse_rms *
        SparkLmBf16ToFloat(q_norm_weight_bf16, column);
    if (column < SPARK_LLM_ROPE_DIMENSION / 2u)
    {
        float angle;

        pair = column;
        angle =
            (float)row_positions[row] * LmGdnStageRopeFrequency(pair);
    ...
    if (column < SPARK_LLM_ROPE_DIMENSION / 2u)
    {
        ...
        imaginary = query_shared[
            pair + SPARK_LLM_ROPE_DIMENSION / 2u];
        query_shared[pair] =
    ...
    if ((blockIdx.x % SPARK_GDN_STAGE_CUDA_ATTN_GROUP) != 0u)
    {
        return;
    }
    ...
    block = slot / SPARK_LLM_KV_BLOCK_TOKENS;
    offset = slot % SPARK_LLM_KV_BLOCK_TOKENS;
    cache_base =
    ...
    SparkLmFloatToBf16(
        ...
        cache_base + ((uint64_t)local_kv_heads *
            SPARK_LLM_ATTN_HEAD_DIMENSION) + column,
        SparkLmBf16ToFloat(v_bf16, key_base + column));
}

static __device__ __forceinline__ uint64_t LmGdnStageAttnTokenBase(const uint32_t *block_indices, uint64_t lane_base, uint32_t token, uint32_t attn_layer_ordinal, uint64_t cache_layer_stride, uint64_t cache_block_stride, uint32_t local_kv_head, uint32_t local_token_elements)
{
	uint32_t block = __ldg(block_indices + lane_base + (token / SPARK_LLM_KV_BLOCK_TOKENS));
	return(((uint64_t)block * cache_block_stride) + ((uint64_t)attn_layer_ordinal * cache_layer_stride) + ((uint64_t)(token % SPARK_LLM_KV_BLOCK_TOKENS) * local_token_elements) + ((uint64_t)local_kv_head * SPARK_LLM_ATTN_HEAD_DIMENSION));
}

static __global__ void LmGdnStageAttnDecodeKernel(const void *q_fused_bf16, const void *kv_cache_bf16, const uint32_t *block_indices, const uint32_t *block_counts, const uint32_t *row_lane_indices, const uint32_t *context_lengths, void *head_out_bf16, uint32_t row_count, uint32_t lane_stride, uint32_t attn_layer_ordinal, uint64_t cache_layer_stride, uint64_t cache_block_stride, uint32_t tp_degree, uint32_t tp_rank)
{
    const uint32_t local_heads =
        SPARK_LLM_ATTN_QUERY_HEAD_COUNT / tp_degree;
    const uint32_t local_kv_heads =
    ...
    const uint32_t local_token_elements =
        2u * local_kv_heads * SPARK_LLM_ATTN_HEAD_DIMENSION;
    const uint32_t heads_per_cta = SPARK_LLM_ATTN_HEADS_PER_CTA;
    __shared__ float q_shared[
        SPARK_LLM_ATTN_HEADS_PER_CTA *
        SPARK_LLM_ATTN_HEAD_DIMENSION];
    __shared__ float merge_max[
        SPARK_LLM_ATTN_HEADS_PER_CTA * SPARK_LM_CTA_WARPS];
    ...
    uint64_t q_base;
    uint64_t out_base;
    float running_max[SPARK_LLM_ATTN_HEADS_PER_CTA];
    float running_den[SPARK_LLM_ATTN_HEADS_PER_CTA];
    float local_logit[SPARK_LLM_ATTN_HEADS_PER_CTA];
    float rescale[SPARK_LLM_ATTN_HEADS_PER_CTA];
    float weight[SPARK_LLM_ATTN_HEADS_PER_CTA];
    float2 accumulator[
        SPARK_LLM_ATTN_HEADS_PER_CTA]
        [SPARK_GDN_STAGE_CUDA_ATTN_VALUE_PAIRS_PER_LANE];
    float2 key_pair;
    float2 value_pair;
    float head_max;
    float denominator;
    ...
    required_block_count =
        (context + SPARK_LLM_KV_BLOCK_TOKENS - 1u) /
        ...
        required_block_count > available_block_count)
    {
        ...
        for (element = threadIdx.x;
             element < heads_per_cta * SPARK_LLM_ATTN_HEAD_DIMENSION;
             element += blockDim.x)
        {
            local_head = element / SPARK_LLM_ATTN_HEAD_DIMENSION;
            partial = element % SPARK_LLM_ATTN_HEAD_DIMENSION;
            out_base =
    ...
    for (local_head = 0u; local_head < heads_per_cta; ++local_head)
    {
        ...
        q_base =
            ((uint64_t)row * 2u * (uint64_t)local_heads *
                SPARK_LLM_ATTN_HEAD_DIMENSION) +
            ((uint64_t)((blockIdx.x * heads_per_cta) + local_head) *
                2u * SPARK_LLM_ATTN_HEAD_DIMENSION);
        for (element = threadIdx.x;
             element < SPARK_LLM_ATTN_HEAD_DIMENSION;
             element += blockDim.x)
        {
            q_shared[
                (local_head * SPARK_LLM_ATTN_HEAD_DIMENSION) +
        ...
        #pragma unroll
        for (pair = 0u;
             pair < SPARK_GDN_STAGE_CUDA_ATTN_VALUE_PAIRS_PER_LANE;
             ++pair)
        {
    ...
    for (element = threadIdx.x;
         element < SPARK_LLM_ATTN_HEADS_PER_CTA *
             SPARK_LM_CTA_WARPS *
             SPARK_LLM_ATTN_HEAD_DIMENSION;
         element += blockDim.x)
    {
    ...
    {
        token_base = LmGdnStageAttnTokenBase(
            block_indices,
            lane_base,
            token,
            attn_layer_ordinal,
             ...
             ++pair)
        {
            value_pair = SparkLmLoadBf16Pair(
                ...
                ((token_base + ((uint64_t)local_kv_heads *
                    SPARK_LLM_ATTN_HEAD_DIMENSION)) >> 1u) +
                    (pair * SPARK_LM_WARP_LANES) + lane);
            #pragma unroll
            for (local_head = 0u;
                 local_head < heads_per_cta;
    ...
    for (local_head = 0u; local_head < heads_per_cta; ++local_head)
    {
             ...
             element += blockDim.x)
        {
            ...
            gate = SparkLmSigmoid(SparkLmBf16ToFloat(
                q_fused_bf16,
                q_base + SPARK_LLM_ATTN_HEAD_DIMENSION + element));
            SparkLmFloatToBf16(
                head_out_bf16,
                out_base + element,
...
static __global__ void LmGdnStageEmbeddingGatherKernel(const uint32_t *token_ids, const void *embedding_bf16, void *hidden_bf16, uint32_t row_count)
{
	uint64_t index = ((uint64_t)blockIdx.x * blockDim.x) + threadIdx.x;
	uint32_t row = (uint32_t)(index / SPARK_LLM_HIDDEN_DIMENSION),element = (uint32_t)(index % SPARK_LLM_HIDDEN_DIMENSION);
	if ( row >= row_count )
		return;
	SparkLmFloatToBf16(hidden_bf16,index,SparkLmBf16ToFloat(embedding_bf16,((uint64_t)token_ids[row] * SPARK_LLM_HIDDEN_DIMENSION) + element));
}

static_assert(
    SPARK_GDN_STAGE_CUDA_GDN_CHUNK_SHARED_BYTES == 98816u,
    "GDN chunk shared layout must fit the SM 12.x 99-KB block limit");

typedef struct LmGdnStageChunkWorkspaceView
{
	float *qn;
	float *kn;
	float *cum_g;
	...
	float *w;
	float *kg;
} LmGdnStageChunkWorkspaceView;

static __device__ __forceinline__ uint64_t LmGdnStageChunkHeadOffset(uint32_t head, uint32_t per_head_elements)
{
...
static __global__ void LmGdnStageChunkPrepareKernel(const void *conv_out_bf16, const float *log_decay_f32, const float *beta_f32, LmGdnStageChunkWorkspaceView views, uint32_t token_count, uint32_t tp_degree)
{
	...
	{
		value = SparkLmBf16ToFloat(conv_out_bf16,conv_row + ((uint64_t)key_head * SPARK_GDN_STAGE_CUDA_DK) + element);
		total += (value * value);
	}
	total = rsqrtf(total + 1e-6f) * rsqrtf((float)SPARK_GDN_STAGE_CUDA_DK);
	for (element = 0; element < SPARK_GDN_STAGE_CUDA_DK; element++)
		views.qn[qk_base + ((uint64_t)row * SPARK_GDN_STAGE_CUDA_DK) + element] = SparkLmBf16ToFloat(conv_out_bf16,conv_row + ((uint64_t)key_head * SPARK_GDN_STAGE_CUDA_DK) + element) * total;
	total = 0.0f;
	for (element = 0; element < SPARK_GDN_STAGE_CUDA_DK; element++)
	{
	...
	{
		value = column <= row ? __expf(views.cum_g[LmGdnStageChunkHeadOffset(head,SPARK_GDN_STAGE_CUDA_CHUNK) + row] - views.cum_g[LmGdnStageChunkHeadOffset(head,SPARK_GDN_STAGE_CUDA_CHUNK) + column]) : 0.0f;
...
static __global__ void LmGdnStageChunkSolveKernel(LmGdnStageChunkWorkspaceView views, uint32_t token_count)
{
	__shared__ float solve_row[SPARK_GDN_STAGE_CUDA_CHUNK];
	...
	{
		if ( column < row )
			solve_row[column] = views.attn[mat_base + ((uint64_t)row * SPARK_GDN_STAGE_CUDA_CHUNK) + column];
		__syncthreads();
		if ( column < row )
		{
			accumulator = solve_row[column];
			for (element = 0; element < row; element++)
				accumulator += (solve_row[element] * views.attn[mat_base + ((uint64_t)element * SPARK_GDN_STAGE_CUDA_CHUNK) + column]);
			views.attn[mat_base + ((uint64_t)row * SPARK_GDN_STAGE_CUDA_CHUNK) + column] = accumulator;
		}
		__syncthreads();
	}
...
static __global__ void LmGdnStageChunkTransformKernel(const void *conv_out_bf16, const float *beta_f32, LmGdnStageChunkWorkspaceView views, uint32_t token_count, uint32_t tp_degree)
{
	...
	uint64_t mat_base = LmGdnStageChunkHeadOffset(head,SPARK_GDN_STAGE_CUDA_CHUNK * SPARK_GDN_STAGE_CUDA_CHUNK);
	uint64_t vec_base = LmGdnStageChunkHeadOffset(head,SPARK_GDN_STAGE_CUDA_CHUNK * SPARK_GDN_STAGE_CUDA_DK);
...
static __global__ void LmGdnStageChunkQkDecayKernel(LmGdnStageChunkWorkspaceView views, uint32_t token_count)
{
    ...
    qn_shared = qk_shared;
    kn_shared = qn_shared +
        (SPARK_GDN_STAGE_CUDA_CHUNK * SPARK_GDN_STAGE_CUDA_DK);
    vector_base = LmGdnStageChunkHeadOffset(
        head,
        SPARK_GDN_STAGE_CUDA_CHUNK * SPARK_GDN_STAGE_CUDA_DK);
    matrix_base = LmGdnStageChunkHeadOffset(
        head,
        SPARK_GDN_STAGE_CUDA_CHUNK * SPARK_GDN_STAGE_CUDA_CHUNK);

    for (vector_element = threadIdx.x;
         vector_element < token_count * SPARK_GDN_STAGE_CUDA_DK;
         ...
         matrix_element += blockDim.x)
    {
        ...
        if (column <= row)
        {
            ...
            {
                dot = fmaf(
                    qn_shared[(row * SPARK_GDN_STAGE_CUDA_DK) + element],
                    kn_shared[(column * SPARK_GDN_STAGE_CUDA_DK) + element],
                    dot);
            }
            views.decay[
                matrix_base +
                ((uint64_t)row * SPARK_GDN_STAGE_CUDA_CHUNK) + column] *= dot;
        }
        else
        {
            views.decay[
                matrix_base +
                ((uint64_t)row * SPARK_GDN_STAGE_CUDA_CHUNK) + column] = 0.0f;
        }
    }
}
...
static __global__ void LmGdnStageChunkStepKernel(const float *log_decay_f32, LmGdnStageChunkWorkspaceView views, float *state_f32, void *core_out_bf16, uint32_t lane_index, uint32_t token_count, uint32_t gdn_layer_ordinal, uint64_t state_lane_stride, uint64_t state_layer_stride, uint32_t tp_degree)
{
    ...
    v_new_shared = state_shared + SPARK_GDN_STAGE_CUDA_GDN_STATE_ELEMENTS;
    exp_cum_g_shared =
        v_new_shared + (SPARK_GDN_STAGE_CUDA_CHUNK * SPARK_GDN_STAGE_CUDA_DV);
    carry_decay_shared =
        exp_cum_g_shared + SPARK_GDN_STAGE_CUDA_CHUNK;
    vector_base = LmGdnStageChunkHeadOffset(
        head,
        SPARK_GDN_STAGE_CUDA_CHUNK * SPARK_GDN_STAGE_CUDA_DK);
    g_base = LmGdnStageChunkHeadOffset(
        head,
        SPARK_GDN_STAGE_CUDA_CHUNK);
    matrix_base = LmGdnStageChunkHeadOffset(
        head,
    ...
    for (row = 0u; row < token_count; ++row)
    {
        ...
        for (element = 0u; element < SPARK_GDN_STAGE_CUDA_DK; ++element)
        {
            accumulator = fmaf(
                views.kg[
                    vector_base +
                    ((uint64_t)row * SPARK_GDN_STAGE_CUDA_DK) + element],
                state_shared[(element * SPARK_GDN_STAGE_CUDA_DV) + column],
                accumulator);
        }
        v_new_shared[(row * SPARK_GDN_STAGE_CUDA_DV) + column] =
            views.w[
                vector_base +
                ((uint64_t)row * SPARK_GDN_STAGE_CUDA_DV) + column] - accumulator;
    }
    ...
    for (row = 0u; row < token_count; ++row)
    {
        ...
        for (element = 0u; element <= row; ++element)
        {
            accumulator = fmaf(
                views.decay[
                    matrix_base +
                    ((uint64_t)row * SPARK_GDN_STAGE_CUDA_CHUNK) + element],
                v_new_shared[(element * SPARK_GDN_STAGE_CUDA_DV) + column],
                accumulator);
        }
        SparkLmFloatToBf16(
            core_out_bf16,
...
static __global__ void LmGdnStageResidualAddKernel(void *hidden_bf16, const void *delta_bf16, uint32_t row_count, uint32_t dimension)
{
...
static int LmGdnStageAttnDegreeOk(uint32_t tp_degree)
{
	uint32_t group = SPARK_LLM_ATTN_QUERY_HEAD_COUNT
		/ SPARK_LLM_ATTN_KV_HEAD_COUNT;
	if ( (SPARK_LLM_ATTN_KV_HEAD_COUNT % tp_degree) == 0u )
		return 1;
	if ( (tp_degree % SPARK_LLM_ATTN_KV_HEAD_COUNT) != 0u )
		return 0;
...
static __global__ void LmGdnStageGateSelectKernel(
    ...
    float *weights_f32)
{
    __shared__ uint64_t ordered_keys[SPARK_LLM_ROUTER_SORT_CAPACITY];
    ...
    static_assert(
        SPARK_LLM_ROUTED_EXPERT_COUNT <=
            SPARK_LLM_ROUTER_SORT_CAPACITY,
        "expert count exceeds router sort capacity");
    static_assert(
        SPARK_LM_MOE_MAX_TOPK <= SPARK_LM_WARP_LANES,
        "router normalization requires one warp");
    row = blockIdx.x;
    if ( row >= row_count || expert_count == 0u ||
        expert_count > SPARK_LLM_ROUTED_EXPERT_COUNT ||
        topk == 0u || topk > SPARK_LM_MOE_MAX_TOPK || topk > expert_count )
        return;
    row_scores = scores_f32 + ((uint64_t)row * expert_count);
    rank = threadIdx.x;
    for (expert = threadIdx.x; expert < SPARK_LLM_ROUTER_SORT_CAPACITY;
        expert += blockDim.x)
    {
        float choice_score;
    ...
    __syncthreads();
    SparkLmBitonicSortKeysAscending<SPARK_LLM_ROUTER_SORT_CAPACITY>(ordered_keys);
    selected_key = rank < topk
        ? ordered_keys[SPARK_LLM_ROUTER_SORT_CAPACITY - 1u - rank]
        : 0u;
    ...
    {
        float max_score = LmGdnStageWarpReduceMax(selected_score);
        float exp_score;
...
static __global__ void LmGdnStageSwiGluKernel(const void *gate_bf16, void *up_bf16, uint32_t row_count, uint32_t dimension)
{
	uint64_t pair = ((uint64_t)blockIdx.x * blockDim.x) + threadIdx.x,pair_count = ((uint64_t)row_count * dimension) >> 1u;
...
static __global__ void LmGdnStageSharedGateKernel(void *accum_bf16, const void *gate_weight_bf16, const void *gate_input_bf16, uint32_t row_count, uint32_t dimension)
{
	__shared__ float reduce_scratch[SPARK_LM_CTA_WARPS];
	uint32_t row = blockIdx.x;
...
static __global__ void LmGdnStageGateScoresKernel(const void *weight_bf16, const void *input_bf16, float *scores_f32, uint32_t row_count, uint32_t input_dimension, uint32_t expert_count)
{
...
{
	cudaError_t status;
	status = cudaFuncSetAttribute(LmGdnStageGdnStepKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)SPARK_GDN_STAGE_CUDA_GDN_DECODE_SHARED_BYTES);
	if ( status != cudaSuccess )
	{
		fprintf(stderr,"gdn_stage configure gdn_step failed %d shared=%d\n",(int)status,(int)SPARK_GDN_STAGE_CUDA_GDN_DECODE_SHARED_BYTES);
		return(status);
	}
	status = cudaFuncSetAttribute(LmGdnStageChunkQkDecayKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)SPARK_GDN_STAGE_CUDA_GDN_QK_SHARED_BYTES);
	if ( status != cudaSuccess )
		return(status);
	status = cudaFuncSetAttribute(LmGdnStageChunkStepKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)SPARK_GDN_STAGE_CUDA_GDN_CHUNK_SHARED_BYTES);
	if ( status != cudaSuccess )
		return(status);
	return(cudaFuncSetAttribute((const void *)SparkLmLinearKernel<32u,SPARK_ACTIVATION_CODEC_NONE,SPARK_LM_CTA_WARPS>, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)(SPARK_LLM_ATTN_QUERY_DIMENSION * sizeof(float))));
}
...
{
	if ( view->weight_format == SPARK_LLM_WEIGHT_FORMAT_BF16 && row_count >= 2u * SPARK_LM_TILE && view->input_dimension > SPARK_LM_TILE_K && view->output_dimension != 0u )
...
cudaError_t LmGdnStageLaunchGdnStep(cudaStream_t stream, const void *conv_out_bf16, const float *log_decay_f32, const float *beta_f32, float *state_f32, void *core_out_bf16, const uint32_t *row_lane_indices, const uint32_t *state_cold_by_row, uint32_t row_count, uint32_t gdn_layer_ordinal, uint64_t state_lane_stride, uint64_t state_layer_stride, uint32_t tp_degree)
{
	...
	LmGdnStageGdnStepKernel<<<
		...
		SPARK_GDN_STAGE_CUDA_GDN_DECODE_SHARED_BYTES,
		stream>>>(
			conv_out_bf16,
			log_decay_f32,
			beta_f32,
...
{
	if ( tp_degree == 0u || tp_degree > SPARK_LLM_ATTN_QUERY_HEAD_COUNT || tp_rank >= tp_degree || !LmGdnStageAttnDegreeOk(tp_degree) )
		return(cudaErrorInvalidValue);
	dim3 grid(SPARK_LLM_ATTN_QUERY_HEAD_COUNT / tp_degree,row_count,1u);
	LmGdnStageAttnPrepareKernel<<<grid,SPARK_LLM_ATTN_HEAD_DIMENSION,0,stream>>>(q_fused_bf16,k_bf16,v_bf16,query_norm_weight_bf16,key_norm_weight_bf16,kv_cache_bf16,slot_mapping,row_positions,row_count,attn_layer_ordinal,cache_layer_stride,cache_block_stride,epsilon,tp_degree,tp_rank);
	return(cudaGetLastError());
}
...
cudaError_t LmGdnStageLaunchAttnDecode(cudaStream_t stream, const void *q_fused_bf16, const void *kv_cache_bf16, const uint32_t *block_indices, const uint32_t *block_counts, uint32_t lane_stride, const uint32_t *row_lane_indices, const uint32_t *context_lengths, void *head_out_bf16, uint32_t row_count, uint32_t attn_layer_ordinal, uint64_t cache_layer_stride, uint64_t cache_block_stride, uint32_t tp_degree, uint32_t tp_rank)
{
	...
	dim3 grid(
		...
		1u);
	static_assert(
		SPARK_LLM_ATTN_QUERY_HEAD_COUNT %
			SPARK_LLM_ATTN_HEADS_PER_CTA == 0u,
		"query-head count must divide the grouped attention CTA width");
	static_assert(
		SPARK_GDN_STAGE_CUDA_ATTN_GROUP %
			SPARK_LLM_ATTN_HEADS_PER_CTA == 0u,
		"a grouped attention CTA may not cross KV-head ownership");
	LmGdnStageAttnDecodeKernel<<<grid, SPARK_LM_CTA_THREADS, 0u, stream>>>(
		q_fused_bf16,
		kv_cache_bf16,
		block_indices,
		block_counts,
...
cudaError_t LmGdnStageLaunchGdnChunk(cudaStream_t stream, const void *conv_out_bf16, const float *log_decay_f32, const float *beta_f32, float *workspace_qn, float *workspace_kn, float *workspace_cum_g, float *workspace_decay, float *workspace_attn, float *workspace_w, float *workspace_kg, float *state_f32, void *core_out_bf16, uint32_t lane_index, uint32_t token_count, uint32_t gdn_layer_ordinal, uint64_t state_lane_stride, uint64_t state_layer_stride, uint32_t tp_degree)
{
	...
	if ( status != cudaSuccess )
		return(status);
	LmGdnStageChunkTransformKernel<<<
		transform_grid,
		SPARK_GDN_STAGE_CUDA_DV,
		0u,
...
cudaError_t LmGdnStageLaunchResidualAdd(cudaStream_t stream, void *hidden_bf16, const void *delta_bf16, uint32_t row_count, uint32_t dimension)
{
	...
	LmGdnStageResidualAddKernel<<<(uint32_t)((pairs + SPARK_LM_CTA_THREADS - 1u) / SPARK_LM_CTA_THREADS),SPARK_LM_CTA_THREADS,0,stream>>>(hidden_bf16,delta_bf16,row_count,dimension);
	return(cudaGetLastError());
}
...
{
	return(SparkLmHostLaunchHeadScreenedArgmax(stream,hidden_bf16,head_weight_bf16,shadow_payload,shadow_scale,error_norm,logits_bf16,candidate_ids,candidate_counts,output_token_ids,row_count,candidate_count,SPARK_LLM_HIDDEN_DIMENSION));
}
...
{
	SparkLmHeadArgmaxKernel<<<row_count,SPARK_LM_CTA_THREADS,0,stream>>>(hidden_bf16,head_weight_bf16,token_ids,output_token_ids,row_count,SPARK_LLM_HIDDEN_DIMENSION,candidate_count);
	return(cudaGetLastError());
}
...
{
	LmGdnStageGateSelectKernel<<<row_count, SPARK_LM_CTA_THREADS, 0, stream>>>(scores_f32,bias_f32,row_count,expert_count,topk,route_scale,indices_u32,weights_f32);
	return(cudaGetLastError());
}
...
cudaError_t LmGdnStageLaunchSwiGlu(cudaStream_t stream, const void *gate_bf16, void *up_bf16, uint32_t row_count, uint32_t dimension)
{
	...
	LmGdnStageSwiGluKernel<<<(blocks == 0u ? 1u : blocks), threads, 0, stream>>>(gate_bf16,up_bf16,row_count,dimension);
	return(cudaGetLastError());
}
...
{
	LmGdnStageSharedGateKernel<<<row_count, SPARK_LM_CTA_THREADS, 0, stream>>>(accum_bf16,gate_weight_bf16,gate_input_bf16,row_count,dimension);
	return(cudaGetLastError());
}
...
cudaError_t LmGdnStageLaunchGateScores(cudaStream_t stream, const LmGdnStageLinearView *gate, const void *input_bf16, float *scores_f32, uint32_t row_count)
{
	...
	if ( gate == 0 || input_bf16 == 0 || scores_f32 == 0 || gate->weight_format != SPARK_LLM_WEIGHT_FORMAT_BF16 || gate->input_dimension == 0u )
		return(cudaErrorInvalidValue);
	LmGdnStageGateScoresKernel<<<dim3(row_count,expert_blocks),SPARK_LM_CTA_THREADS,gate->input_dimension * sizeof(float),stream>>>(gate->weight_payload,input_bf16,scores_f32,row_count,gate->input_dimension,gate->output_dimension);
	return(cudaGetLastError());
}
...
{
	int32_t launch_status = LmRouteBuild<SPARK_LM_CTA_THREADS,SPARK_LLM_ROUTED_EXPERT_COUNT>(route_expert,rows,rows * SPARK_LLM_EXPERTS_PER_TOKEN,SPARK_LLM_EXPERTS_PER_TOKEN,group_row_offset,route_packed_row,route_source_token,expert_width,SPARK_LLM_HIDDEN_DIMENSION,SPARK_LM_TILE_N,SPARK_LM_TILE_N,group_tile_prefix_w1,group_tile_prefix_w2,stream);
...
{
	return(SparkLmHostLaunchMoePairReduceOverwrite(stream,slot_out_bf16,inverse_map,pair_weights_f32,output_bf16,row_count,SPARK_LLM_EXPERTS_PER_TOKEN,hidden_dimension));
}
