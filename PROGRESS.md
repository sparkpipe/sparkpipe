...
| 5 | V3 real-weights block gate | **GATE REPAIRED, BLOCKED ON ANCHOR DATA** — r4 fixed the r3 refactor (unallocated scratch.segments garbage pointer; stray device_segments args breaking the nvcc compile) and the gate now runs: all 20 real-weight tensors load finite (max 1.3-8.4, stats printed per tensor), block forwards are deterministic on rerun and TP4-partial-vs-TP1 bit-identical. The ONLY failing check is the fixture compare: the anchor npz itself carries bf16-NaN in block0_out/block1_out (all 69888 elements, bits 7fc0) and packed_h (38.5% NaN, proven against batch-minimax-val's own dit_blocks01_real.npz) — the anchor lane's diffusers run diverged. The gate now fails closed naming this attribution; it binds the moment that lane regenerates the fixture |
| 6 | V4 VAE decode gates | **VIDEO: anchor-blocked (same NaN class). AUDIO: mismatch under bisect** — video expected fixture is 67584/67584 nonfinite from the anchor generator; gate fails closed with that attribution. Audio anchor is finite; our decode is finite and stage-magnitudes decay sanely (2.2 -> 0.1 through 7 stages, stats printed per stage) but the waveform is ~uncorrelated vs expected (rel 1.000, our max 0.033 vs expected 0.401): the r4 asymmetric-lowpass fix (5/6 pads) and per-stage stats are in; one open DSP discrepancy remains |
...
| 9 | cell E2E | **PENDING, generator verified** — deployment generator re-verified on the adopted tree (16 stage configs, frozen block 17408-18431, highest session port 18383; session matrices are parse-compat only under the mesh dataflow - weightd-owned QPs dial nothing). Live cell needs the 135GB pack set placed under the queue-window law + fleet weightd mesh; ABI seam instrumentation numbers to be captured at the live run |
