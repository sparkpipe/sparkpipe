...
TEST_NAMES := \
	test_status \
	test_adapter_common \
    test_adapter_common_integration \
...
PYTHON_TESTS := \
	...
	tests/test_dsv4_head_scale_value.py \
...
build/test_adapter_common: tests/test_adapter_common.c $(RUNTIME_LIBRARY) $(MODEL_COMMON_LIBRARY) $(CORE_LIBRARY)
	$(CC) $(CPPFLAGS) $(CFLAGS) tests/test_adapter_common.c $(RUNTIME_LIBRARY) $(MODEL_COMMON_LIBRARY) $(CORE_LIBRARY) $(LDFLAGS) $(LDLIBS) -o $@
...
TEST_ADAPTER_FAKE_A_DRIVER_SO := build/test_adapter_fake_a_driver.so
TEST_ADAPTER_FAKE_B_DRIVER_SO := build/test_adapter_fake_b_driver.so
TEST_ADAPTER_FAKE_C_DRIVER_SO := build/test_adapter_fake_c_driver.so
TEST_ADAPTER_FAKE_D_DRIVER_SO := build/test_adapter_fake_d_driver.so

# Fake family drivers for the shared-adapter integration suite: one source
# template compiled once per profile (-DPROF_*), dlopen'd through the REAL
# SparkAdapterLoadDriver path by test_adapter_common_integration.
$(TEST_ADAPTER_FAKE_A_DRIVER_SO): tests/fake_driver/adapter_fake_driver.c include/sparkpipe/spark_model_driver.h
	$(CC) $(CPPFLAGS) -Iinclude -O3 -g -fPIC -shared -DPROF_MODEL_ID='"integration.family.a"' -DPROF_MODEL_REVISION='"rev-a"' -DPROF_STAGE_NAME='"stage.a"' -DPROF_PROGRAM_NAME='"prog.a"' tests/fake_driver/adapter_fake_driver.c -o $@
$(TEST_ADAPTER_FAKE_B_DRIVER_SO): tests/fake_driver/adapter_fake_driver.c include/sparkpipe/spark_model_driver.h
	$(CC) $(CPPFLAGS) -Iinclude -O3 -g -fPIC -shared -DPROF_MODEL_ID='"integration.family.b"' -DPROF_MODEL_REVISION='"rev-b"' -DPROF_STAGE_NAME='"stage.b"' -DPROF_PROGRAM_NAME='"prog.b"' tests/fake_driver/adapter_fake_driver.c -o $@
$(TEST_ADAPTER_FAKE_C_DRIVER_SO): tests/fake_driver/adapter_fake_driver.c include/sparkpipe/spark_model_driver.h
	$(CC) $(CPPFLAGS) -Iinclude -O3 -g -fPIC -shared -DPROF_MODEL_ID='"integration.family.c"' -DPROF_MODEL_REVISION='"rev-c"' -DPROF_STAGE_NAME='"stage.c"' -DPROF_PROGRAM_NAME='"prog.c"' tests/fake_driver/adapter_fake_driver.c -o $@
$(TEST_ADAPTER_FAKE_D_DRIVER_SO): tests/fake_driver/adapter_fake_driver.c include/sparkpipe/spark_model_driver.h
	$(CC) $(CPPFLAGS) -Iinclude -O3 -g -fPIC -shared -DPROF_MODEL_ID='"integration.family.d"' -DPROF_MODEL_REVISION='"rev-d"' -DPROF_STAGE_NAME='"stage.d"' -DPROF_PROGRAM_NAME='"prog.d"' tests/fake_driver/adapter_fake_driver.c -o $@

build/test_adapter_common_integration: tests/test_adapter_common_integration.c runtime/adapter_common.h runtime/adapter_common.c tests/fake_driver/adapter_fake_driver.h $(RUNTIME_LIBRARY) $(MODEL_COMMON_LIBRARY) $(CORE_LIBRARY) $(TEST_ADAPTER_FAKE_A_DRIVER_SO) $(TEST_ADAPTER_FAKE_B_DRIVER_SO) $(TEST_ADAPTER_FAKE_C_DRIVER_SO) $(TEST_ADAPTER_FAKE_D_DRIVER_SO)
	$(CC) $(CPPFLAGS) -I. -Iruntime -Itests/fake_driver $(CFLAGS) tests/test_adapter_common_integration.c $(RUNTIME_LIBRARY) $(MODEL_COMMON_LIBRARY) $(CORE_LIBRARY) $(LDFLAGS) $(LDLIBS) -ldl -pthread -o $@
...
build/test_qwen38_math_kernels: tests/test_qwen38_math_kernels.cu modules/qwen38_max_resident_decode_stage/source/spark_qwen38_max_resident_decode_stage_cuda.cu
	$(NVCC) -std=c++17 $(NVCCFLAGS) -I. -Iinclude -Imodel-families/common/include -Imodel-families/qwen38_max/include -Imodules/qwen38_max_resident_decode_stage/include -Imodules/qwen38_max_resident_decode_stage/source $< -L$(CUDA_HOME)/lib64 -lcudart -o $@
...
# The host stagepack gate pins the deployed expert codec (fp8, the measured
# route): batch_tuning.h refuses to guess GLM52_EXPERT_CODEC_NAME.
build/test_glm52_stagepack: tests/test_glm52_stagepack.c modules/glm52_resident_decode_stage/source/spark_glm52_resident_decode_stage_internal.h modules/glm52_resident_decode_stage/include/sparkpipe/spark_glm52_resident_decode_stage_firmware.h
	$(CC) $(GLM52_INCLUDE_FLAGS) -Imodules/glm52_resident_decode_stage/source -Imodules/glm52_resident_decode_stage/include -DGLM52_EXPERT_CODEC_NAME=\"fp8\" \
...
# Shared paged-KV host core (runtime/paged_kv_common over runtime/prefix_cache):
# integration gate driving the production Initialize/Admit/Cover/checkpoint path.
# The two core sources compile in directly: linking the model-common archive
# would pull its duplicate prefix_cache.o basenames and trip two-level-namespace
# member ambiguity warnings.
build/test_paged_kv_core: tests/test_paged_kv_core.c runtime/paged_kv_common.c runtime/paged_kv_common.h runtime/prefix_cache.c runtime/prefix_cache.h
	$(CC) $(CPPFLAGS) -Itests $(CFLAGS) tests/test_paged_kv_core.c runtime/paged_kv_common.c runtime/prefix_cache.c $(LDFLAGS) $(LDLIBS) -o $@

# GLM-5.2 port of the shared paged-KV core, driven at the adapter's per-round
# call pattern with free-list conservation asserted after every step; the port
# source is compiled in directly (it is not part of any host archive).
build/test_glm52_prefix_cache: tests/test_glm52_prefix_cache.c modules/glm52_resident_decode_stage/source/spark_glm52_paged_kv.c modules/glm52_resident_decode_stage/source/spark_glm52_paged_kv.h model-families/glm52/include/sparkpipe/spark_glm52_kv_geometry.h runtime/paged_kv_common.c runtime/prefix_cache.c
	$(CC) $(CPPFLAGS) -Imodules/glm52_resident_decode_stage/source -Itests $(CFLAGS) tests/test_glm52_prefix_cache.c modules/glm52_resident_decode_stage/source/spark_glm52_paged_kv.c runtime/paged_kv_common.c runtime/prefix_cache.c $(LDFLAGS) $(LDLIBS) -o $@
