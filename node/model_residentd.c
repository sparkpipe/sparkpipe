	...
	SparkModelResidentdSequenceSlot *slot)
{
	...
	if ( route->submission.work_kind == SPARK_MODEL_SERVING_WORK_KIND_DECODE )
	{
		 ...
		 * to, PER LANE. completion.tokens_per_sequence is the only per-lane
		 * yield a DECODE completion carries (adapter contract: final-stage
		 * verify reports 1..chain width, plain decode reports 1).
		 * accepted_token_count is a batch-GLOBAL aggregate across all active
		 * sequences - feeding it into this per-lane position computation
		 * over-advances every slot of a multi-lane decode by
		 * (active_sequence_count-1) positions, and the client's legitimate
		 * next continuation then fails the lease position fence with
		 * SCHEMA_ERROR (a9343c1 units conflation; deterministic e2e
		 * continuation schema_error since). Guessing the admitted chain
		 * width instead is equally wrong on partial accept. When no per-lane
		 * count exists at this rank (transported status-only DECODE
		 * completion), defer the position fence: generation and step fences
		 * stay armed and the next continuation re-fences against the
		 * coordinator-authoritative position it carries (feeding a zero here
		 * rejects with INVALID_ARGUMENT and kills the run loop - the
		 * 1cfea8f break). */
	...
	const SparkModelServingCompletion *completion)
{
	...
	/* A deep-accept verify round may legitimately yield more than the
	 * frame's admitted ceiling (anchor + accepted drafts + bonus fold).
	 * Carry such a yield when the completion's OWN credited window
	 * certifies it: the window is already hard-capped by ABI validation
	 * (tokens_per_sequence <= MAX_TOKENS_PER_SEQUENCE, token payload shape-
	 * checked) and every lease consumer - this daemon's decode advance and
	 * the client's completion-derived advance alike - derives its step from
	 * that window, so a window-certified yield cannot desync the fence.
	 * Killing the route here converted every peak-acceptance round into a
	 * daemon exit (the BF16 deep-accept crash signature). Only a yield
	 * exceeding BOTH the ceiling and its own credited window remains a
	 * protocol violation. */
	if ( status == SPARK_STATUS_OK && completion->accepted_token_count > route->submission.new_token_count + runtime->adapter_library.adapter_interface.descriptor->max_speculative_token_count &&
		completion->accepted_token_count > completion->tokens_per_sequence )
	{
		status = SPARK_STATUS_SCHEMA_ERROR;
