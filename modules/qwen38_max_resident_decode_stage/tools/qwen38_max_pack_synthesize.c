...
#define SPARK_SYNTH_ALIGN_UNIT SPARK_QWEN38_MAX_STAGEPACK_PAYLOAD_ALIGNMENT
#define SPARK_SYNTH_ENTRY_T SparkQwen38MaxStagePackEntry
#define SPARK_SYNTH_HEADER_T SparkQwen38MaxStagePackHeaderCanonical
#define SPARK_SYNTH_SHAPE_T SparkQwen38MaxStagePackTensorShape
#define SPARK_SYNTH_HEADER_BYTES SPARK_QWEN38_MAX_STAGEPACK_HEADER_BYTES
#define SPARK_SYNTH_ENTRY_BYTES SPARK_QWEN38_MAX_STAGEPACK_ENTRY_BYTES
#define SPARK_SYNTH_RESOLVE_SHAPE(kind, layer, is_global, shape) \
	(SparkQwen38MaxStagePackResolvedShape((kind),(layer),(is_global),(shape)) < 0)
#define SPARK_SYNTH_SELECT_FORMAT(quantize, shape) \
	((quantize) != 0u ? (shape).natural_format : SPARK_QWEN38_MAX_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_BF16)
#define SPARK_SYNTH_SCALE_GROUP_SIZE(format) \
	((format) == SPARK_QWEN38_MAX_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_MXFP4_E2M1 ? 32u : ((format) == SPARK_QWEN38_MAX_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_FP8_E4M3_F32B128 ? 128u : 0u))
#define SPARK_SYNTH_PAYLOAD_KIND(entry) ((entry)->weight_format)
#define SPARK_SYNTH_PACKED_FORMAT SPARK_QWEN38_MAX_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_MXFP4_E2M1
#define SPARK_SYNTH_PACKED_NAN_MASK 0u
#define SPARK_SYNTH_F32_FORMAT SPARK_QWEN38_MAX_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_F32
#define SPARK_SYNTH_PAYLOAD_BYTES(format, rows, columns) \
	SparkQwen38MaxStagePackPayloadBytes((format),(rows),(columns))
...
#define SPARK_SYNTH_TENSOR_FINAL_NORM SPARK_QWEN38_MAX_STAGEPACK_TENSOR_FINAL_NORM
#define SPARK_SYNTH_TENSOR_LM_HEAD SPARK_QWEN38_MAX_STAGEPACK_TENSOR_LM_HEAD
#define SPARK_SYNTH_MODEL_LAYER_COUNT SPARK_QWEN38_MAX_MODEL_LAYER_COUNT
#define SPARK_SYNTH_MODEL_IS_GDN(layer) (SPARK_QWEN38_MAX_MODEL_LAYER_IS_GDN(layer) != 0u)
#define SPARK_SYNTH_EXPECTED_TENSOR_COUNT(first, count) \
	SparkQwen38MaxStagePackExpectedTensorCount((first),(count))
#define SPARK_SYNTH_EXPECTED_GEOMETRY(header, first, count) \
	SparkQwen38MaxStagePackExpectedGeometry((header),(first),(count))
#define SPARK_SYNTH_EVERY_LAYER_KINDS \
	SPARK_QWEN38_MAX_STAGEPACK_TENSOR_ATTENTION_NORM,SPARK_QWEN38_MAX_STAGEPACK_TENSOR_MLP_NORM, \
