...
cuda_validator="${script_directory}/spark_qwen38_max_resident_decode_stage_cuda_validation.cu"
...
require_configuration_value() {
    ...
    local actual="${!name:-}"
    if [[ "${actual}" != "${expected}" ]]; then
        echo "qwen38_max hardware validation requires ${name}=${expected}, got '${actual}'" >&2
        exit 2
    fi
}
...
if [[ -z "${SPARK_QWEN38_MAX_STAGE_PACK_PATH:-}" || ! -s "${SPARK_QWEN38_MAX_STAGE_PACK_PATH}" ]]; then
    echo "SPARK_QWEN38_MAX_STAGE_PACK_PATH must name a readable non-empty stage pack" >&2
    exit 2
fi
require_source_digest "${SPARK_QWEN38_MAX_CUDA_VALIDATOR_SHA256:-}" "${cuda_validator}" "Qwen38_max CUDA validator"
...
if [[ "${cuda_architecture}" != "sm_121a" ]]; then
    echo "Qwen38_max hardware validation admits only CUDA_ARCH=sm_121a" >&2
    exit 2
fi
if ! command -v "${nvcc_path}" >/dev/null 2>&1; then
    echo "nvcc unavailable for Qwen38_max hardware validation" >&2
    exit 2
fi
...
"${nvcc_path}" \
    ...
    -I"${repository_root}/model-families/qwen38_max/include" \
    ...
    -lm \
    -Xcompiler -pthread \
    -o "${validation_directory}/qwen38_max_resident_decode_stage_validator"

"${validation_directory}/qwen38_max_resident_decode_stage_validator" "${configuration_hash}"
