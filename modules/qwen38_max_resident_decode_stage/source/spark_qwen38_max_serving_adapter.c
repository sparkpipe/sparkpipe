 ...
 * the qwen38_max_resident_decode_stage firmware driver.
 *
...
#define SPARK_QWEN38_MAX_SERVING_HYBRID_GROUP_SIZE 16u
#define SPARK_QWEN38_MAX_SERVING_DESCRIPTOR_STAGE_LAYER_COUNTS \
...
#define SPARK_QWEN38_MAX_SERVING_PREFIX_CACHE_ENV "SPARK_QWEN38_MAX_SERVING_PREFIX_CACHE"

static uint32_t SparkQwen38MaxServingPrefixCacheEnabled(void)
{
	const char *value = getenv(SPARK_QWEN38_MAX_SERVING_PREFIX_CACHE_ENV);
...
static const SparkModelServingAdapterDescriptor SparkQwen38MaxServingDescriptor =
{
	...
	.parallel_group_size = SPARK_QWEN38_MAX_SERVING_HYBRID_GROUP_SIZE,
	...
	.artifact_sha256 = QWEN38_MAX_CONTRACT_SHA256,
	.stage_layer_counts = SPARK_QWEN38_MAX_SERVING_DESCRIPTOR_STAGE_LAYER_COUNTS,
	...
	uint32_t *max_sequence_positions)
{
	...
	if ( status == SPARK_STATUS_OK && (token < 0 || !SparkJsonStringEquals(&document,token,QWEN38_MAX_MODEL_REVISION)) )
		status = SPARK_STATUS_SCHEMA_ERROR;
	if ( status == SPARK_STATUS_OK )
 ...
 * uniqueness the qwen38_max paged KV table requires: two submission lanes
	...
	uint32_t slot)
{
	if ( state->paged_ready != 0u )
		SparkQwen38MaxPagedKvLaneReset(&state->paged,slot);
...
static uint32_t SparkQwen38MaxServingGatherLaneTokens(
	const SparkModelServingSubmission *submission,
	uint32_t lane,
...
static uint32_t SparkQwen38MaxServingMatchPrefix(
	SparkQwen38MaxServingState *state,
	const SparkModelServingSubmission *submission,
	uint32_t lane)
{
	...
	if ( state->prefix_enabled == 0u || state->stage_attn_layer_count == 0u ||
		...
		tokens == 0 )
		return(0u);
	count = SparkQwen38MaxServingGatherLaneTokens(submission,lane,0,tokens,state->max_input_row_count);
	slot = submission->lanes[lane].resident_sequence_slot;
	if ( count == 0u )
		return(0u);
	...
	if ( SparkQwen38MaxPagedKvAdmit(&state->paged,slot,tokens,count,&match) != SPARK_STATUS_OK || match.block_count == 0u )
		return(0u);
	fprintf(stderr,"qwen38_max_prefix_reuse lane=%u slot=%u matched_blocks=%u resumed_at=%u checkpoint_slot=%u\n",
		lane,slot,match.block_count,match.block_count * state->paged.configuration.block_token_count,match.checkpoint_slot);
	...
	uint64_t end_position)
{
	...
	committed = SparkQwen38MaxPagedKvCommittedTokens(&state->paged,slot);
	token_count = SparkQwen38MaxServingGatherLaneTokens(submission,lane,committed,state->match_tokens,state->max_input_row_count);
	return(SparkQwen38MaxPagedKvCover(&state->paged,slot,end_position,token_count != 0u ? state->match_tokens : 0,token_count));
}
	...
	const SparkModelServingSubmission *submission)
{
	...
	for (lane=0u; lane<submission->active_sequence_count; lane++)
	{
		...
		status = SparkQwen38MaxServingCoverLane(state,submission,lane,end_position);
		if ( status != SPARK_STATUS_OK )
		{
		...
		for (lane=0u; status == SPARK_STATUS_OK && lane<submission->active_sequence_count; lane++)
		{
			...
			if ( lane_first_position == 0u )
			{
				...
				if ( state->lane_context_tokens[slot] != 0u )
					SparkQwen38MaxServingReleaseLane(state,slot);
				(void)SparkQwen38MaxServingMatchPrefix(state,submission,lane);
				status = SparkQwen38MaxServingCoverLane(state,submission,lane,lane_end_position);
				if ( status != SPARK_STATUS_OK )
				{
			...
			for (wave=0u; status == SPARK_STATUS_OK && wave<lane_rows; wave+=chunk_rows)
			{
				...
				if ( status == SPARK_STATUS_OK && state->prefix_enabled != 0u && state->stage_attn_layer_count != 0u )
				{
					...
					boundary -= boundary % SPARK_QWEN38_MAX_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS;
					for (boundary += SPARK_QWEN38_MAX_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS;
						boundary <= chunk_end;
						boundary += SPARK_QWEN38_MAX_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS)
						if ( SparkQwen38MaxPagedKvCheckpointOffer(&state->paged,slot,boundary,&checkpoint_slot) != 0u )
							SparkQwen38MaxPagedKvCheckpointCommit(&state->paged,slot,checkpoint_slot,boundary);
				}
			}
		}
	}
...
static void SparkQwen38MaxServingDestroy(void *adapter_state)
{
	...
	if ( state->paged_ready != 0u )
	{
		...
		SparkQwen38MaxPagedKvDestroy(&state->paged);
...
static uint64_t SparkQwen38MaxServingBlockStrideBytes(
	const SparkQwen38MaxServingState *state)
{
	uint32_t tp_degree,local_heads;
	tp_degree = state->tp_degree != 0u ? state->tp_degree : SPARK_QWEN38_MAX_SERVING_DEFAULT_TP_DEGREE;
	local_heads = tp_degree <= SPARK_QWEN38_MAX_MODEL_ATTN_KV_HEAD_COUNT ?
		SPARK_QWEN38_MAX_MODEL_ATTN_KV_HEAD_COUNT / tp_degree : 1u;
	if ( local_heads == 0u )
		local_heads = 1u;
	return((uint64_t)state->stage_attn_layer_count * local_heads *
		SPARK_QWEN38_MAX_MODEL_ATTN_HEAD_DIMENSION * 2u *
		SPARK_QWEN38_MAX_MODEL_BF16_ELEMENT_BYTES *
...
static SparkStatus SparkQwen38MaxServingAllocatePools(
	SparkQwen38MaxServingState *state)
{
	SparkQwen38MaxPagedKvConfiguration paged_configuration;
	uint64_t indices;
	SparkStatus status;
	...
	paged_configuration.block_token_count = SPARK_QWEN38_MAX_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS;
	...
	paged_configuration.lane_count = state->resident_sequence_capacity;
	...
	paged_configuration.checkpoint_slot_count =
		...
		SPARK_QWEN38_MAX_RESIDENT_DECODE_STAGE_MAX_GDN_SNAPSHOT_SLOTS : 0u;
	paged_configuration.block_stride_bytes = SparkQwen38MaxServingBlockStrideBytes(state);
	status = SparkQwen38MaxPagedKvInitialize(&state->paged,&paged_configuration,
	...
	void **adapter_state)
{
	...
	*adapter_state = 0;
	...
	state->prefix_enabled = SparkQwen38MaxServingPrefixCacheEnabled();
