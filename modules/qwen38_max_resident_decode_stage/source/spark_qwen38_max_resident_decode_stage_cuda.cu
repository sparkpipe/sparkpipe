...
{
	return(LmGdnStageLaunchConvUpdate(stream,qkv_bf16,weights->conv_weight_bf16,conv_out_bf16,pool->conv_tail_bf16,row_lane_indices,pool->state_cold_by_row,row_count,gdn_layer_ordinal,pool->conv_tail_lane_stride_elements,pool->conv_tail_layer_stride_elements));
}
...
{
	return(LmGdnStageLaunchDecayBeta(stream,decay_pre_bf16,beta_pre_bf16,weights->a_log_f32,weights->dt_bias_f32,log_decay_f32,beta_f32,row_count));
}
...
{
	return(LmGdnStageLaunchGdnStep(stream,conv_out_bf16,log_decay_f32,beta_f32,pool->state_f32,core_out_bf16,row_lane_indices,pool->state_cold_by_row,row_count,gdn_layer_ordinal,pool->state_lane_stride_elements,pool->state_layer_stride_elements));
}
...
{
	return(LmGdnStageLaunchGatedNorm(stream,core_bf16,z_bf16,weights->gdn_norm_weight_bf16,output_bf16,row_count,epsilon));
}
...
{
	return(LmGdnStageLaunchChunkConv(stream,qkv_bf16,weights->conv_weight_bf16,conv_out_bf16,pool->conv_tail_bf16,lane_index,token_count,gdn_layer_ordinal,pool->conv_tail_lane_stride_elements,pool->conv_tail_layer_stride_elements));
}
