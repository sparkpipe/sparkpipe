...
{
	if ( tp_degree == 0u || tp_degree > SPARK_QWEN38_MAX_MODEL_ATTN_QUERY_HEAD_COUNT || tp_rank >= tp_degree || !SparkQwen38MaxAttnDegreeOk(tp_degree) )		return(cudaErrorInvalidValue);
