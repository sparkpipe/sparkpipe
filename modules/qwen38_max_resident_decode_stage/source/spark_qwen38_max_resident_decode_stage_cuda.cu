    ...
    uint32_t tp_rank)
{
    ...
    const uint32_t rank_kv_head_base =
        SPARK_QWEN38_MAX_MODEL_ATTN_RANK_KV_HEAD_BASE(tp_degree, tp_rank);
...
extern "C" cudaError_t SparkQwen38MaxLaunchChunkConv(cudaStream_t stream, const void *qkv_bf16, const SparkQwen38MaxGdnLayerWeights *weights, void *conv_out_bf16, const SparkQwen38MaxGdnStatePool *pool, uint32_t lane_index, uint32_t token_count, uint32_t gdn_layer_ordinal, uint32_t tp_degree)
{
	uint32_t local_conv_channels = SPARK_QWEN38_MAX_MODEL_GDN_CONV_CHANNELS / tp_degree;
	if ( token_count == 0u )
		return(cudaErrorInvalidValue);
...
extern "C" cudaError_t SparkQwen38MaxLaunchMoePairReduce(cudaStream_t stream, const void *slot_out_bf16, const uint32_t *inverse_map, const float *pair_weights_f32, void *accum_bf16, uint32_t row_count, uint32_t hidden_dimension)
{
 ...
 */
extern "C" cudaError_t SparkQwen38MaxLaunchGroupedExpertLinear(
	cudaStream_t stream,
 ...
 */
extern "C" cudaError_t SparkQwen38MaxLaunchGroupedExpertTileLinear(
	cudaStream_t stream,
	const SparkQwen38MaxLinearView *view,
	const void *input_bf16,
	const uint32_t *source_row_map,
