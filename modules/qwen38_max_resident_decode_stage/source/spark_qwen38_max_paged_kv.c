 ...
 * range (distinct from the qwen36 port range): one live residency per
 * lane, so id collisions cannot happen. */
#define SPARK_QWEN38_MAX_PAGED_KV_SEQUENCE_BASE UINT64_C(0x5133800000000000)

static uint64_t SparkQwen38MaxPagedKvSequenceId(uint32_t lane)
{
	return SPARK_QWEN38_MAX_PAGED_KV_SEQUENCE_BASE + (uint64_t)lane;
}

static uint32_t SparkQwen38MaxPagedKvRowBase(const SparkQwen38MaxPagedKv *cache,
	uint32_t lane)
{
...
static SparkStatus SparkQwen38MaxPagedKvScratchBorrow(
	SparkQwen38MaxPagedKv *cache, uint32_t *block_out)
{
	 ...
	 * in-use scratch as free, so SparkQwen38MaxPagedKvFreeBlocks() and
 ...
 * needs clearing). */
static void SparkQwen38MaxPagedKvScratchReturn(SparkQwen38MaxPagedKv *cache,
	uint32_t block)
{
...
static void SparkQwen38MaxPagedKvSyncRow(SparkQwen38MaxPagedKv *cache,
	uint32_t lane)
{
	uint32_t row_base = SparkQwen38MaxPagedKvRowBase(cache,lane);
	...
	if ( cache->lane_live[lane] != 0u )
	{
		...
		tokens = (uint32_t)SparkPrefixCacheCoreSequenceTokenCount(
			&cache->core,SparkQwen38MaxPagedKvSequenceId(lane));
		...
		if ( blocks != 0u )
			(void)SparkPrefixCacheCoreBuildBlockTable(&cache->core,
				SparkQwen38MaxPagedKvSequenceId(lane),tokens,
...
SparkStatus SparkQwen38MaxPagedKvInitialize(SparkQwen38MaxPagedKv *cache,
	...
	uint32_t *blocks_by_lane, uint32_t *counts_by_lane)
{
	...
	if ( cache == 0 || configuration == 0 || blocks_by_lane == 0 ||
		...
		configuration->blocks_per_lane >
			SPARK_QWEN38_MAX_PAGED_KV_MAX_BLOCKS_PER_LANE ||
	...
	cache->reserved_slot = SPARK_QWEN38_MAX_PAGED_KV_NO_SLOT;
	cache->reserved_lane = SPARK_QWEN38_MAX_PAGED_KV_NO_LANE;
	...
	{
		cache->checkpoints = (SparkQwen38MaxPagedKvCheckpoint *)calloc(
		...
		for (index=0u; index<configuration->checkpoint_slot_count; index++)
			cache->checkpoints[index].witness_block =
				SPARK_QWEN38_MAX_PAGED_KV_NO_BLOCK;
	}
...
void SparkQwen38MaxPagedKvLaneReset(SparkQwen38MaxPagedKv *cache, uint32_t lane)
{
	...
	if ( cache == 0 || lane >= cache->configuration.lane_count )
		return;
	row_base = SparkQwen38MaxPagedKvRowBase(cache,lane);
	if ( cache->lane_live[lane] != 0u )
	{
		(void)SparkPrefixCacheCoreReleaseSequence(&cache->core,
			SparkQwen38MaxPagedKvSequenceId(lane));
		...
		index<cache->counts_by_lane[lane]; index++)
	{
		...
		if ( block != SPARK_QWEN38_MAX_PAGED_KV_NO_BLOCK )
			SparkQwen38MaxPagedKvScratchReturn(cache,block);
	}
...
static uint32_t SparkQwen38MaxPagedKvWitnessedDepth(
	...
	uint32_t *slot_out)
{
	...
	for (boundary=limit_blocks; boundary>=1u; boundary--)
	{
		for (slot=0u; slot<cache->configuration.checkpoint_slot_count;
			slot++)
		{
			const SparkQwen38MaxPagedKvCheckpoint *checkpoint =
...
SparkStatus SparkQwen38MaxPagedKvAdmit(SparkQwen38MaxPagedKv *cache,
	...
	SparkQwen38MaxPagedKvMatch *match_out)
{
	...
	uint32_t attached[SPARK_QWEN38_MAX_PAGED_KV_MAX_BLOCKS_PER_LANE];
	...
	if ( match_out != 0 )
	{
		...
		match_out->checkpoint_slot = SPARK_QWEN38_MAX_PAGED_KV_NO_SLOT;
	}
	if ( cache == 0 || lane >= cache->configuration.lane_count ||
	...
	status = SparkPrefixCacheCoreAdmitSequence(&cache->core,
		SparkQwen38MaxPagedKvSequenceId(lane),tokens,token_count,
		&lcp_tokens);
	if ( status != SPARK_STATUS_OK )
	{
		SparkQwen38MaxPagedKvLaneReset(cache,lane);
	...
	SparkQwen38MaxPagedKvSyncRow(cache,lane);
	if ( match_out != 0 )
		*match_out = match;
	return(SPARK_STATUS_OK);
}

SparkStatus SparkQwen38MaxPagedKvCover(SparkQwen38MaxPagedKv *cache,
	...
	uint32_t token_count)
{
	...
	{
		status = SparkQwen38MaxPagedKvScratchBorrow(cache,&block);
		if ( status != SPARK_STATUS_OK )
		{
...
uint64_t SparkQwen38MaxPagedKvCommittedTokens(
	const SparkQwen38MaxPagedKv *cache, uint32_t lane)
{
	if ( cache == 0 || lane >= cache->configuration.lane_count ||
		cache->lane_live[lane] == 0u )
		return(0u);
...
uint32_t SparkQwen38MaxPagedKvCheckpointOffer(SparkQwen38MaxPagedKv *cache,
	uint32_t lane, uint64_t end_position, uint32_t *slot_out)
{
	...
	if ( cache == 0 || slot_out == 0 ||
		...
		return(0u);
	*slot_out = SPARK_QWEN38_MAX_PAGED_KV_NO_SLOT;
	if ( cache->reuse_enabled == 0u ||
		cache->reserved_slot != SPARK_QWEN38_MAX_PAGED_KV_NO_SLOT ||
		cache->lane_live[lane] == 0u )
		return(0u);
	...
	boundary = (uint32_t)(end_position /
		cache->configuration.block_token_count);
	victim_slot = SPARK_QWEN38_MAX_PAGED_KV_NO_SLOT;
	...
	if ( victim_slot == SPARK_QWEN38_MAX_PAGED_KV_NO_SLOT )
	{
...
void SparkQwen38MaxPagedKvCheckpointCommit(SparkQwen38MaxPagedKv *cache,
	uint32_t lane, uint32_t slot, uint64_t end_position)
{
	SparkQwen38MaxPagedKvCheckpoint *checkpoint;
	...
	if ( end_position >
		SparkQwen38MaxPagedKvCommittedTokens(cache,lane) )
		return;
	row_base = SparkQwen38MaxPagedKvRowBase(cache,lane);
	witness = cache->blocks_by_lane[row_base + boundary - 1u];
	if ( witness == SPARK_QWEN38_MAX_PAGED_KV_NO_BLOCK )
		return;
...
void SparkQwen38MaxPagedKvCheckpointAbort(SparkQwen38MaxPagedKv *cache,
