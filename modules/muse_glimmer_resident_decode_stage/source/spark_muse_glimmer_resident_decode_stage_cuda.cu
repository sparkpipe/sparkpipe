...
#define SPARK_MUSE_GLIMMER_CUDA_DK SPARK_MUSE_GLIMMER_MODEL_GDN_HEAD_KEY_DIMENSION
#define SPARK_MUSE_GLIMMER_CUDA_DV SPARK_MUSE_GLIMMER_MODEL_GDN_HEAD_VALUE_DIMENSION
#define SPARK_MUSE_GLIMMER_CUDA_GVA_GROUP (SPARK_MUSE_GLIMMER_MODEL_GDN_VALUE_HEAD_COUNT / SPARK_MUSE_GLIMMER_MODEL_GDN_KEY_HEAD_COUNT)
#define SPARK_MUSE_GLIMMER_CUDA_ATTN_GROUP (SPARK_MUSE_GLIMMER_MODEL_ATTN_QUERY_HEAD_COUNT / SPARK_MUSE_GLIMMER_MODEL_ATTN_KV_HEAD_COUNT)
#define SPARK_MUSE_GLIMMER_CUDA_ATTN_HEADS_PER_CTA 2u
#define SPARK_MUSE_GLIMMER_CUDA_ATTN_VALUE_PAIRS_PER_LANE \
    (SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION / (2u * SPARK_LM_WARP_LANES))
#define SPARK_MUSE_GLIMMER_CUDA_GDN_STATE_ELEMENTS \
    (SPARK_MUSE_GLIMMER_CUDA_DK * SPARK_MUSE_GLIMMER_CUDA_DV)
#define SPARK_MUSE_GLIMMER_CUDA_GDN_DECODE_SHARED_BYTES \
    (SPARK_MUSE_GLIMMER_CUDA_GDN_STATE_ELEMENTS * sizeof(float))

static __device__ __forceinline__ float SparkQwen38RopeFrequency(uint32_t pair)
{
	return(exp2f(-((float)(2u * pair) / (float)SPARK_MUSE_GLIMMER_MODEL_ATTN_ROPE_DIMENSION) * log2f((float)SPARK_MUSE_GLIMMER_MODEL_ATTN_ROPE_THETA)));
}
...
static __global__ void SparkQwen38ConvUpdateKernel(const void *qkv_bf16, const void *conv_weight_bf16, void *conv_out_bf16, void *conv_tail_bf16, const uint32_t *row_lane_indices, const uint32_t *state_cold_by_row, uint32_t row_count, uint32_t gdn_layer_ordinal, uint64_t tail_lane_stride, uint64_t tail_layer_stride)
{
	...
	uint32_t tap;
	if ( row >= row_count || channel >= SPARK_MUSE_GLIMMER_MODEL_GDN_CONV_CHANNELS )
		return;
	...
	window[3] = SparkLmBf16ToFloat(qkv_bf16,((uint64_t)row * SPARK_MUSE_GLIMMER_MODEL_GDN_CONV_CHANNELS) + channel);
	accumulator = 0.0f;
	for (tap = 0; tap < SPARK_MUSE_GLIMMER_MODEL_GDN_CONV_KERNEL; tap++)
		accumulator += (window[tap] * SparkLmBf16ToFloat(conv_weight_bf16,((uint64_t)channel * SPARK_MUSE_GLIMMER_MODEL_GDN_CONV_KERNEL) + tap));
	SparkLmFloatToBf16(conv_out_bf16,((uint64_t)row * SPARK_MUSE_GLIMMER_MODEL_GDN_CONV_CHANNELS) + channel,SparkLmSwish(accumulator));
...
static __global__ void SparkQwen38DecayBetaKernel(const void *decay_pre_bf16, const void *beta_pre_bf16, const float *a_log_f32, const float *dt_bias_f32, float *log_decay_f32, float *beta_f32, uint32_t row_count)
{
	...
	uint64_t index;
	if ( row >= row_count || head >= SPARK_MUSE_GLIMMER_MODEL_GDN_VALUE_HEAD_COUNT )
		return;
	index = ((uint64_t)row * SPARK_MUSE_GLIMMER_MODEL_GDN_VALUE_HEAD_COUNT) + head;
...
static __global__ void SparkQwen38GdnStepKernel(const void *conv_out_bf16, const float *log_decay_f32, const float *beta_f32, float *state_f32, void *core_out_bf16, const uint32_t *row_lane_indices, const uint32_t *state_cold_by_row, uint32_t row_count, uint32_t gdn_layer_ordinal, uint64_t state_lane_stride, uint64_t state_layer_stride)
{
    ...
    __shared__ float qn[SPARK_MUSE_GLIMMER_CUDA_DK];
    __shared__ float kn[SPARK_MUSE_GLIMMER_CUDA_DK];
    ...
    head = blockIdx.x;
    column = threadIdx.x;
    key_head = head / SPARK_MUSE_GLIMMER_CUDA_GVA_GROUP;
    if (row >= row_count)
    {
        return;
    }

    conv_row = (uint64_t)row * SPARK_MUSE_GLIMMER_MODEL_GDN_CONV_CHANNELS;
    value = SparkLmBf16ToFloat(
        conv_out_bf16,
        conv_row + ((uint64_t)key_head * SPARK_MUSE_GLIMMER_CUDA_DK) + column);
    q_norm = SparkLmBlockReduceSum(value * value, reduce_scratch);
    qn[column] = value * rsqrtf(q_norm + 1.0e-6f) *
        rsqrtf((float)SPARK_MUSE_GLIMMER_CUDA_DK);

    value = SparkLmBf16ToFloat(
        conv_out_bf16,
        conv_row + SPARK_MUSE_GLIMMER_MODEL_GDN_QK_DIMENSION +
            ((uint64_t)key_head * SPARK_MUSE_GLIMMER_CUDA_DK) + column);
    ...
    state_base =
        ...
        ((uint64_t)head * SPARK_MUSE_GLIMMER_CUDA_GDN_STATE_ELEMENTS);
    decay = state_cold_by_row[row] != 0u
        ? 0.0f
        : expf(log_decay_f32[
            ((uint64_t)row * SPARK_MUSE_GLIMMER_MODEL_GDN_VALUE_HEAD_COUNT) + head]);
    beta = beta_f32[
        ((uint64_t)row * SPARK_MUSE_GLIMMER_MODEL_GDN_VALUE_HEAD_COUNT) + head];

    kv_memory = 0.0f;
    for (element = 0u; element < SPARK_MUSE_GLIMMER_CUDA_DK; ++element)
    {
        state_index = (element * SPARK_MUSE_GLIMMER_CUDA_DV) + column;
    ...
    delta = (SparkLmBf16ToFloat(
        conv_out_bf16,
        conv_row + (2u * SPARK_MUSE_GLIMMER_MODEL_GDN_QK_DIMENSION) +
            ((uint64_t)head * SPARK_MUSE_GLIMMER_CUDA_DV) + column) - kv_memory) * beta;
    output = 0.0f;
    for (element = 0u; element < SPARK_MUSE_GLIMMER_CUDA_DK; ++element)
    {
        state_index = (element * SPARK_MUSE_GLIMMER_CUDA_DV) + column;
    ...
    SparkLmFloatToBf16(
        core_out_bf16,
        ((uint64_t)row * SPARK_MUSE_GLIMMER_MODEL_GDN_VALUE_DIMENSION) +
            ((uint64_t)head * SPARK_MUSE_GLIMMER_CUDA_DV) + column,
        output);
...
static __global__ void SparkQwen38GatedNormKernel(const void *core_bf16, const void *z_bf16, const void *norm_weight_bf16, void *output_bf16, uint32_t row_count, float epsilon)
{
	...
	uint64_t index = ((uint64_t)row * SPARK_MUSE_GLIMMER_MODEL_GDN_VALUE_DIMENSION) + ((uint64_t)head * SPARK_MUSE_GLIMMER_CUDA_DV) + column;
	float value,variance;
	if ( row >= row_count )
		return;
	value = SparkLmBf16ToFloat(core_bf16,index);
	variance = SparkLmBlockReduceSum(value * value,reduce_scratch) / (float)SPARK_MUSE_GLIMMER_CUDA_DV;
    ...
    uint32_t tp_rank)
{
    const uint32_t local_heads =
        SPARK_MUSE_GLIMMER_MODEL_ATTN_QUERY_HEAD_COUNT / tp_degree;
    const uint32_t local_kv_heads =
        SPARK_MUSE_GLIMMER_MODEL_ATTN_LOCAL_KV_HEAD_COUNT(tp_degree);
    const uint32_t rank_kv_head_base =
        SPARK_MUSE_GLIMMER_MODEL_ATTN_RANK_KV_HEAD_BASE(tp_degree, tp_rank);
    const uint32_t local_token_elements =
        2u * local_kv_heads * SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION;
    __shared__ float reduce_scratch[SPARK_LM_CTA_WARPS];
    __shared__ float query_shared[SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION];
    __shared__ float key_shared[SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION];
    __shared__ float rope_cosine[
        SPARK_MUSE_GLIMMER_MODEL_ATTN_ROPE_DIMENSION / 2u];
    ...
    column = threadIdx.x;
    kv_head = head / SPARK_MUSE_GLIMMER_CUDA_ATTN_GROUP;
    if (row >= row_count ||
        blockIdx.x >= local_heads ||
        column >= SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION)
    {
        return;
    }
    ...
    query_base =
        ...
        ((uint64_t)blockIdx.x * 2u * SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION);
    value = SparkLmBf16ToFloat(q_fused_bf16, query_base + column);
    sum_squares = SparkLmBlockReduceSum(
        value * value,
        reduce_scratch);
    inverse_rms = rsqrtf(
        sum_squares / (float)SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION +
        epsilon);
    query_shared[column] =
        value * inverse_rms *
        SparkLmBf16ToFloat(q_norm_weight_bf16, column);
    if (column < SPARK_MUSE_GLIMMER_MODEL_ATTN_ROPE_DIMENSION / 2u)
    {
        float angle;
    ...
    if (column < SPARK_MUSE_GLIMMER_MODEL_ATTN_ROPE_DIMENSION / 2u)
    {
        ...
        imaginary = query_shared[
            pair + SPARK_MUSE_GLIMMER_MODEL_ATTN_ROPE_DIMENSION / 2u];
        query_shared[pair] =
    ...
    if ((blockIdx.x % SPARK_MUSE_GLIMMER_CUDA_ATTN_GROUP) != 0u)
    {
        return;
    }
    ...
    key_base =
        ((uint64_t)row * (uint64_t)local_kv_heads *
            SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION) +
        ((uint64_t)(kv_head - rank_kv_head_base) *
            SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION);
    ...
    block = slot / SPARK_MUSE_GLIMMER_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS;
    offset = slot % SPARK_MUSE_GLIMMER_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS;
    cache_base =
    ...
    SparkLmFloatToBf16(
        ...
        cache_base + ((uint64_t)local_kv_heads *
            SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION) + column,
...
{
	uint32_t block = __ldg(block_indices + lane_base + (token / SPARK_MUSE_GLIMMER_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS));
	return(((uint64_t)block * cache_block_stride) + ((uint64_t)attn_layer_ordinal * cache_layer_stride) + ((uint64_t)(token % SPARK_MUSE_GLIMMER_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS) * local_token_elements) + ((uint64_t)local_kv_head * SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION));
}
...
static __global__ void SparkQwen38AttnDecodeKernel(const void *q_fused_bf16, const void *kv_cache_bf16, const uint32_t *block_indices, const uint32_t *block_counts, const uint32_t *row_lane_indices, const uint32_t *context_lengths, void *head_out_bf16, uint32_t row_count, uint32_t lane_stride, uint32_t attn_layer_ordinal, uint64_t cache_layer_stride, uint64_t cache_block_stride, uint32_t tp_degree, uint32_t tp_rank)
{
    ...
    const uint32_t local_token_elements =
        2u * local_kv_heads * SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION;
    const uint32_t heads_per_cta = SPARK_MUSE_GLIMMER_CUDA_ATTN_HEADS_PER_CTA;
    __shared__ float q_shared[
        SPARK_MUSE_GLIMMER_CUDA_ATTN_HEADS_PER_CTA *
        SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION];
    __shared__ float merge_max[
        SPARK_MUSE_GLIMMER_CUDA_ATTN_HEADS_PER_CTA * SPARK_LM_CTA_WARPS];
    ...
    uint64_t q_base;
    uint64_t out_base;
    float running_max[SPARK_MUSE_GLIMMER_CUDA_ATTN_HEADS_PER_CTA];
    float running_den[SPARK_MUSE_GLIMMER_CUDA_ATTN_HEADS_PER_CTA];
    float local_logit[SPARK_MUSE_GLIMMER_CUDA_ATTN_HEADS_PER_CTA];
    float rescale[SPARK_MUSE_GLIMMER_CUDA_ATTN_HEADS_PER_CTA];
    float weight[SPARK_MUSE_GLIMMER_CUDA_ATTN_HEADS_PER_CTA];
    float2 accumulator[
        SPARK_MUSE_GLIMMER_CUDA_ATTN_HEADS_PER_CTA]
        [SPARK_MUSE_GLIMMER_CUDA_ATTN_VALUE_PAIRS_PER_LANE];
    float2 key_pair;
    float2 value_pair;
    float head_max;
    float denominator;
    ...
    required_block_count =
        (context + SPARK_MUSE_GLIMMER_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS - 1u) /
        ...
        required_block_count > available_block_count)
    {
        ...
        for (element = threadIdx.x;
             element < heads_per_cta * SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION;
             element += blockDim.x)
        {
            local_head = element / SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION;
            partial = element % SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION;
            out_base =
    ...
    for (local_head = 0u; local_head < heads_per_cta; ++local_head)
    {
        ...
        for (element = threadIdx.x;
             element < SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION;
             element += blockDim.x)
        {
            q_shared[
                (local_head * SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION) +
        ...
        #pragma unroll
        for (pair = 0u;
             pair < SPARK_MUSE_GLIMMER_CUDA_ATTN_VALUE_PAIRS_PER_LANE;
             ++pair)
        {
    ...
    for (element = threadIdx.x;
         element < SPARK_MUSE_GLIMMER_CUDA_ATTN_HEADS_PER_CTA *
             SPARK_LM_CTA_WARPS *
             SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION;
         element += blockDim.x)
    {
             ...
             ++pair)
        {
            value_pair = SparkLmLoadBf16Pair(
                ...
                ((token_base + ((uint64_t)local_kv_heads *
                    SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION)) >> 1u) +
                    (pair * SPARK_LM_WARP_LANES) + lane);
            #pragma unroll
            for (local_head = 0u;
                 local_head < heads_per_cta;
    ...
    for (local_head = 0u; local_head < heads_per_cta; ++local_head)
    {
             ...
             element += blockDim.x)
        {
            ...
            gate = SparkLmSigmoid(SparkLmBf16ToFloat(
                q_fused_bf16,
                q_base + SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION + element));
            SparkLmFloatToBf16(
                head_out_bf16,
                out_base + element,
...
static __global__ void SparkQwen38EmbeddingGatherKernel(const uint32_t *token_ids, const void *embedding_bf16, void *hidden_bf16, uint32_t row_count)
{
	...
	if ( row >= row_count )
		return;
	SparkLmFloatToBf16(hidden_bf16,index,SparkLmBf16ToFloat(embedding_bf16,((uint64_t)token_ids[row] * SPARK_MUSE_GLIMMER_MODEL_HIDDEN_DIMENSION) + element));
}

#define SPARK_MUSE_GLIMMER_CUDA_CHUNK SPARK_MUSE_GLIMMER_MODEL_GDN_CHUNK_TOKENS
#define SPARK_MUSE_GLIMMER_CUDA_GDN_QK_SHARED_BYTES \
    (2u * SPARK_MUSE_GLIMMER_CUDA_CHUNK * SPARK_MUSE_GLIMMER_CUDA_DK * sizeof(float))
#define SPARK_MUSE_GLIMMER_CUDA_GDN_CHUNK_SHARED_BYTES \
    ((SPARK_MUSE_GLIMMER_CUDA_GDN_STATE_ELEMENTS + \
      (SPARK_MUSE_GLIMMER_CUDA_CHUNK * SPARK_MUSE_GLIMMER_CUDA_DV) + \
      (2u * SPARK_MUSE_GLIMMER_CUDA_CHUNK)) * sizeof(float))
static_assert(
    SPARK_MUSE_GLIMMER_CUDA_GDN_CHUNK_SHARED_BYTES == 98816u,
...
{
	uint32_t head = blockIdx.x,row = threadIdx.x,key_head = head / SPARK_MUSE_GLIMMER_CUDA_GVA_GROUP,element,column;
	uint64_t conv_row,qk_base = SparkQwen38ChunkHeadOffset(head,SPARK_MUSE_GLIMMER_CUDA_CHUNK * SPARK_MUSE_GLIMMER_CUDA_DK);
	uint64_t mat_base = SparkQwen38ChunkHeadOffset(head,SPARK_MUSE_GLIMMER_CUDA_CHUNK * SPARK_MUSE_GLIMMER_CUDA_CHUNK);
	float total,value,product;
	if ( row >= token_count )
		return;
	conv_row = (uint64_t)row * SPARK_MUSE_GLIMMER_MODEL_GDN_CONV_CHANNELS;
	total = 0.0f;
	for (element = 0; element < SPARK_MUSE_GLIMMER_CUDA_DK; element++)
	{
		value = SparkLmBf16ToFloat(conv_out_bf16,conv_row + ((uint64_t)key_head * SPARK_MUSE_GLIMMER_CUDA_DK) + element);
		total += (value * value);
	}
	total = rsqrtf(total + 1e-6f) * rsqrtf((float)SPARK_MUSE_GLIMMER_CUDA_DK);
	for (element = 0; element < SPARK_MUSE_GLIMMER_CUDA_DK; element++)
		views.qn[qk_base + ((uint64_t)row * SPARK_MUSE_GLIMMER_CUDA_DK) + element] = SparkLmBf16ToFloat(conv_out_bf16,conv_row + ((uint64_t)key_head * SPARK_MUSE_GLIMMER_CUDA_DK) + element) * total;
	total = 0.0f;
	for (element = 0; element < SPARK_MUSE_GLIMMER_CUDA_DK; element++)
	{
		value = SparkLmBf16ToFloat(conv_out_bf16,conv_row + SPARK_MUSE_GLIMMER_MODEL_GDN_QK_DIMENSION + ((uint64_t)key_head * SPARK_MUSE_GLIMMER_CUDA_DK) + element);
		total += (value * value);
	}
	total = rsqrtf(total + 1e-6f);
	for (element = 0; element < SPARK_MUSE_GLIMMER_CUDA_DK; element++)
		views.kn[qk_base + ((uint64_t)row * SPARK_MUSE_GLIMMER_CUDA_DK) + element] = SparkLmBf16ToFloat(conv_out_bf16,conv_row + SPARK_MUSE_GLIMMER_MODEL_GDN_QK_DIMENSION + ((uint64_t)key_head * SPARK_MUSE_GLIMMER_CUDA_DK) + element) * total;
	if ( row == 0u )
	{
		total = 0.0f;
		for (element = 0; element < token_count; element++)
		{
			total += log_decay_f32[((uint64_t)element * SPARK_MUSE_GLIMMER_MODEL_GDN_VALUE_HEAD_COUNT) + head];
			views.cum_g[SparkQwen38ChunkHeadOffset(head,SPARK_MUSE_GLIMMER_CUDA_CHUNK) + element] = total;
		}
	}
	__syncthreads();
	for (column = 0; column < token_count; column++)
	{
		value = column <= row ? __expf(views.cum_g[SparkQwen38ChunkHeadOffset(head,SPARK_MUSE_GLIMMER_CUDA_CHUNK) + row] - views.cum_g[SparkQwen38ChunkHeadOffset(head,SPARK_MUSE_GLIMMER_CUDA_CHUNK) + column]) : 0.0f;
		views.decay[mat_base + ((uint64_t)row * SPARK_MUSE_GLIMMER_CUDA_CHUNK) + column] = value;
		product = 0.0f;
		for (element = 0; element < SPARK_MUSE_GLIMMER_CUDA_DK && column < row; element++)
			product += (views.kn[qk_base + ((uint64_t)row * SPARK_MUSE_GLIMMER_CUDA_DK) + element] * beta_f32[((uint64_t)row * SPARK_MUSE_GLIMMER_MODEL_GDN_VALUE_HEAD_COUNT) + head] * views.kn[qk_base + ((uint64_t)column * SPARK_MUSE_GLIMMER_CUDA_DK) + element]);
		views.attn[mat_base + ((uint64_t)row * SPARK_MUSE_GLIMMER_CUDA_CHUNK) + column] = column < row ? -(product * value) : 0.0f;
	}
}

static __global__ void SparkQwen38ChunkSolveKernel(SparkQwen38ChunkWorkspaceView views, uint32_t token_count)
{
	__shared__ float solve_row[SPARK_MUSE_GLIMMER_CUDA_CHUNK];
	...
	{
		if ( column < row )
			solve_row[column] = views.attn[mat_base + ((uint64_t)row * SPARK_MUSE_GLIMMER_CUDA_CHUNK) + column];
		__syncthreads();
		if ( column < row )
		{
			accumulator = solve_row[column];
			for (element = 0; element < row; element++)
				accumulator += (solve_row[element] * views.attn[mat_base + ((uint64_t)element * SPARK_MUSE_GLIMMER_CUDA_CHUNK) + column]);
			views.attn[mat_base + ((uint64_t)row * SPARK_MUSE_GLIMMER_CUDA_CHUNK) + column] = accumulator;
		}
		__syncthreads();
	}
	if ( column < token_count )
		views.attn[mat_base + ((uint64_t)column * SPARK_MUSE_GLIMMER_CUDA_CHUNK) + column] += 1.0f;
}
...
{
	__shared__ float exp_cum_g[SPARK_MUSE_GLIMMER_CUDA_CHUNK];
	uint32_t head = blockIdx.x,row = blockIdx.y,column = threadIdx.x,element;
	uint64_t mat_base = SparkQwen38ChunkHeadOffset(head,SPARK_MUSE_GLIMMER_CUDA_CHUNK * SPARK_MUSE_GLIMMER_CUDA_CHUNK);
	uint64_t vec_base = SparkQwen38ChunkHeadOffset(head,SPARK_MUSE_GLIMMER_CUDA_CHUNK * SPARK_MUSE_GLIMMER_CUDA_DK);
		...
		exp_cum_g[threadIdx.x] = __expf(
			views.cum_g[
				SparkQwen38ChunkHeadOffset(
					head,
					SPARK_MUSE_GLIMMER_CUDA_CHUNK) +
				threadIdx.x]);
	__syncthreads();
	accumulator = 0.0f;
	for (element = 0; element < token_count; element++)
	{
		transform = views.attn[mat_base + ((uint64_t)row * SPARK_MUSE_GLIMMER_CUDA_CHUNK) + element] * beta_f32[((uint64_t)element * SPARK_MUSE_GLIMMER_MODEL_GDN_VALUE_HEAD_COUNT) + head];
		accumulator += (transform * SparkLmBf16ToFloat(conv_out_bf16,((uint64_t)element * SPARK_MUSE_GLIMMER_MODEL_GDN_CONV_CHANNELS) + (2u * SPARK_MUSE_GLIMMER_MODEL_GDN_QK_DIMENSION) + ((uint64_t)head * SPARK_MUSE_GLIMMER_CUDA_DV) + column));
	}
	views.w[vec_base + ((uint64_t)row * SPARK_MUSE_GLIMMER_CUDA_DV) + column] = accumulator;
	accumulator = 0.0f;
	for (element = 0; element < token_count; element++)
	{
		transform = views.attn[mat_base + ((uint64_t)row * SPARK_MUSE_GLIMMER_CUDA_CHUNK) + element] * beta_f32[((uint64_t)element * SPARK_MUSE_GLIMMER_MODEL_GDN_VALUE_HEAD_COUNT) + head] * exp_cum_g[element];
		accumulator += (transform * views.kn[vec_base + ((uint64_t)element * SPARK_MUSE_GLIMMER_CUDA_DK) + column]);
	}
	views.kg[vec_base + ((uint64_t)row * SPARK_MUSE_GLIMMER_CUDA_DK) + column] = accumulator;
}
...
static __global__ void SparkQwen38ChunkQkDecayKernel(SparkQwen38ChunkWorkspaceView views, uint32_t token_count)
{
    ...
    qn_shared = qk_shared;
    kn_shared = qn_shared +
        (SPARK_MUSE_GLIMMER_CUDA_CHUNK * SPARK_MUSE_GLIMMER_CUDA_DK);
    vector_base = SparkQwen38ChunkHeadOffset(
        head,
        SPARK_MUSE_GLIMMER_CUDA_CHUNK * SPARK_MUSE_GLIMMER_CUDA_DK);
    matrix_base = SparkQwen38ChunkHeadOffset(
        head,
        SPARK_MUSE_GLIMMER_CUDA_CHUNK * SPARK_MUSE_GLIMMER_CUDA_CHUNK);

    for (vector_element = threadIdx.x;
         vector_element < token_count * SPARK_MUSE_GLIMMER_CUDA_DK;
         ...
         matrix_element += blockDim.x)
    {
        ...
        if (column <= row)
        {
            ...
            {
                dot = fmaf(
                    qn_shared[(row * SPARK_MUSE_GLIMMER_CUDA_DK) + element],
                    kn_shared[(column * SPARK_MUSE_GLIMMER_CUDA_DK) + element],
                    dot);
            }
            views.decay[
                matrix_base +
                ((uint64_t)row * SPARK_MUSE_GLIMMER_CUDA_CHUNK) + column] *= dot;
        }
        else
        {
            views.decay[
                matrix_base +
                ((uint64_t)row * SPARK_MUSE_GLIMMER_CUDA_CHUNK) + column] = 0.0f;
        }
    }
}
...
static __global__ void SparkQwen38ChunkStepKernel(const float *log_decay_f32, SparkQwen38ChunkWorkspaceView views, float *state_f32, void *core_out_bf16, uint32_t lane_index, uint32_t token_count, uint32_t gdn_layer_ordinal, uint64_t state_lane_stride, uint64_t state_layer_stride)
{
    ...
    v_new_shared = state_shared + SPARK_MUSE_GLIMMER_CUDA_GDN_STATE_ELEMENTS;
    exp_cum_g_shared =
        v_new_shared + (SPARK_MUSE_GLIMMER_CUDA_CHUNK * SPARK_MUSE_GLIMMER_CUDA_DV);
    carry_decay_shared =
        exp_cum_g_shared + SPARK_MUSE_GLIMMER_CUDA_CHUNK;
    vector_base = SparkQwen38ChunkHeadOffset(
        head,
        SPARK_MUSE_GLIMMER_CUDA_CHUNK * SPARK_MUSE_GLIMMER_CUDA_DK);
    ...
    for (row = 0u; row < token_count; ++row)
    {
        ...
        for (element = 0u; element < SPARK_MUSE_GLIMMER_CUDA_DK; ++element)
        {
            accumulator = fmaf(
                views.kg[
                    vector_base +
                    ((uint64_t)row * SPARK_MUSE_GLIMMER_CUDA_DK) + element],
                state_shared[(element * SPARK_MUSE_GLIMMER_CUDA_DV) + column],
                accumulator);
        }
        v_new_shared[(row * SPARK_MUSE_GLIMMER_CUDA_DV) + column] =
            views.w[
                vector_base +
                ((uint64_t)row * SPARK_MUSE_GLIMMER_CUDA_DV) + column] - accumulator;
    }
    ...
    for (row = 0u; row < token_count; ++row)
    {
        ...
        for (element = 0u; element <= row; ++element)
        {
            accumulator = fmaf(
                views.decay[
                    matrix_base +
                    ((uint64_t)row * SPARK_MUSE_GLIMMER_CUDA_CHUNK) + element],
                v_new_shared[(element * SPARK_MUSE_GLIMMER_CUDA_DV) + column],
                accumulator);
        }
        SparkLmFloatToBf16(
            core_out_bf16,
...
static __global__ void SparkQwen38ChunkConvKernel(const void *qkv_bf16, const void *conv_weight_bf16, void *conv_out_bf16, void *conv_tail_bf16, uint32_t lane_index, uint32_t token_count, uint32_t gdn_layer_ordinal, uint64_t tail_lane_stride, uint64_t tail_layer_stride)
{
	...
	if ( channel >= SPARK_MUSE_GLIMMER_MODEL_GDN_CONV_CHANNELS )
		return;
	tail_base = ((uint64_t)lane_index * tail_lane_stride) + ((uint64_t)gdn_layer_ordinal * tail_layer_stride) + ((uint64_t)channel * SPARK_MUSE_GLIMMER_MODEL_GDN_CONV_TAIL_COLUMNS);
	window[0] = SparkLmBf16ToFloat(conv_tail_bf16,tail_base + 0u);
	window[1] = SparkLmBf16ToFloat(conv_tail_bf16,tail_base + 1u);
	window[2] = SparkLmBf16ToFloat(conv_tail_bf16,tail_base + 2u);
	for (tap = 0; tap < SPARK_MUSE_GLIMMER_MODEL_GDN_CONV_KERNEL; tap++)
		weight[tap] = SparkLmBf16ToFloat(conv_weight_bf16,((uint64_t)channel * SPARK_MUSE_GLIMMER_MODEL_GDN_CONV_KERNEL) + tap);
	for (token = 0; token < token_count; token++)
	{
		element = ((uint64_t)token * SPARK_MUSE_GLIMMER_MODEL_GDN_CONV_CHANNELS) + channel;
...
extern "C" cudaError_t SparkMuseGlimmerLaunchFusedResidualRmsNorm(cudaStream_t stream, void *hidden_bf16, const void *delta_bf16, const void *gain_bf16, void *output_bf16, uint32_t row_count, uint32_t dimension, float epsilon)
{
...
extern "C" cudaError_t SparkMuseGlimmerLaunchRmsNorm(cudaStream_t stream, const void *input_bf16, const void *gain_bf16, void *output_bf16, uint32_t row_count, uint32_t dimension, float epsilon)
{
    size_t shared_memory_bytes = (size_t)dimension * sizeof(float);
...
extern "C" cudaError_t SparkMuseGlimmerLaunchLinear(cudaStream_t stream, const SparkMuseGlimmerLinearView *view, const void *input_bf16, void *output_bf16, uint32_t row_count)
{
	if ( view->weight_format == SPARK_MUSE_GLIMMER_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_BF16 && row_count >= 2u * SPARK_LM_TILE && view->input_dimension > SPARK_LM_TILE_K && view->output_dimension != 0u )
...
extern "C" cudaError_t SparkMuseGlimmerLaunchConvUpdate(cudaStream_t stream, const void *qkv_bf16, const SparkMuseGlimmerGdnLayerWeights *weights, void *conv_out_bf16, const SparkMuseGlimmerGdnStatePool *pool, const uint32_t *row_lane_indices, uint32_t row_count, uint32_t gdn_layer_ordinal)
{
	dim3 grid((SPARK_MUSE_GLIMMER_MODEL_GDN_CONV_CHANNELS + SPARK_LM_CTA_THREADS - 1u) / SPARK_LM_CTA_THREADS,row_count,1u);
...
extern "C" cudaError_t SparkMuseGlimmerLaunchDecayBeta(cudaStream_t stream, const void *decay_pre_bf16, const void *beta_pre_bf16, const SparkMuseGlimmerGdnLayerWeights *weights, float *log_decay_f32, float *beta_f32, uint32_t row_count)
{
	SparkQwen38DecayBetaKernel<<<row_count,SPARK_MUSE_GLIMMER_MODEL_GDN_VALUE_HEAD_COUNT,0,stream>>>(decay_pre_bf16,beta_pre_bf16,weights->a_log_f32,weights->dt_bias_f32,log_decay_f32,beta_f32,row_count);
	return(cudaGetLastError());
}

extern "C" cudaError_t SparkMuseGlimmerLaunchGdnStep(cudaStream_t stream, const void *conv_out_bf16, const float *log_decay_f32, const float *beta_f32, const SparkMuseGlimmerGdnStatePool *pool, void *core_out_bf16, const uint32_t *row_lane_indices, uint32_t row_count, uint32_t gdn_layer_ordinal)
{
    dim3 grid(
        SPARK_MUSE_GLIMMER_MODEL_GDN_VALUE_HEAD_COUNT,
        row_count,
        1u);
    SparkQwen38GdnStepKernel<<<
        grid,
        SPARK_MUSE_GLIMMER_CUDA_DV,
        SPARK_MUSE_GLIMMER_CUDA_GDN_DECODE_SHARED_BYTES,
        stream>>>(
            conv_out_bf16,
            log_decay_f32,
            beta_f32,
...
extern "C" cudaError_t SparkMuseGlimmerLaunchGatedNorm(cudaStream_t stream, const void *core_bf16, const void *z_bf16, const SparkMuseGlimmerGdnLayerWeights *weights, void *output_bf16, uint32_t row_count, float epsilon)
{
	dim3 grid(SPARK_MUSE_GLIMMER_MODEL_GDN_VALUE_HEAD_COUNT,row_count,1u);
	SparkQwen38GatedNormKernel<<<grid,SPARK_MUSE_GLIMMER_CUDA_DV,0,stream>>>(core_bf16,z_bf16,weights->gdn_norm_weight_bf16,output_bf16,row_count,epsilon);
	return(cudaGetLastError());
}


static int SparkMuseGlimmerAttnDegreeOk(uint32_t tp_degree)
{
	uint32_t group = SPARK_MUSE_GLIMMER_MODEL_ATTN_QUERY_HEAD_COUNT
		/ SPARK_MUSE_GLIMMER_MODEL_ATTN_KV_HEAD_COUNT;
	if ( (SPARK_MUSE_GLIMMER_MODEL_ATTN_KV_HEAD_COUNT % tp_degree) == 0u )
		return 1;
	if ( (tp_degree % SPARK_MUSE_GLIMMER_MODEL_ATTN_KV_HEAD_COUNT) != 0u )
		return 0;
	return (group % (SPARK_MUSE_GLIMMER_MODEL_ATTN_QUERY_HEAD_COUNT / tp_degree)) == 0u;
}
...
{
	if ( tp_degree == 0u || tp_degree > SPARK_MUSE_GLIMMER_MODEL_ATTN_QUERY_HEAD_COUNT || tp_rank >= tp_degree || (SPARK_MUSE_GLIMMER_MODEL_ATTN_QUERY_HEAD_COUNT % tp_degree) != 0u || ((SPARK_MUSE_GLIMMER_MODEL_ATTN_QUERY_HEAD_COUNT / SPARK_MUSE_GLIMMER_MODEL_ATTN_KV_HEAD_COUNT) % (SPARK_MUSE_GLIMMER_MODEL_ATTN_QUERY_HEAD_COUNT / tp_degree)) != 0u )		return(cudaErrorInvalidValue);
	dim3 grid(SPARK_MUSE_GLIMMER_MODEL_ATTN_QUERY_HEAD_COUNT / tp_degree,row_count,1u);
	SparkQwen38AttnPrepareKernel<<<grid,SPARK_MUSE_GLIMMER_MODEL_ATTN_HEAD_DIMENSION,0,stream>>>(q_fused_bf16,k_bf16,v_bf16,weights->query_norm_weight_bf16,weights->key_norm_weight_bf16,kv_cache_bf16,slot_mapping,row_positions,row_count,attn_layer_ordinal,cache_layer_stride,cache_block_stride,epsilon,tp_degree,tp_rank);
	return(cudaGetLastError());
}
...
extern "C" cudaError_t SparkMuseGlimmerLaunchAttnDecode(cudaStream_t stream, const void *q_fused_bf16, const void *kv_cache_bf16, const SparkMuseGlimmerKvBlockTableView *table, const uint32_t *row_lane_indices, const uint32_t *context_lengths, void *head_out_bf16, uint32_t row_count, uint32_t attn_layer_ordinal, uint64_t cache_layer_stride, uint64_t cache_block_stride, uint32_t tp_degree, uint32_t tp_rank)
{
    ...
    dim3 grid(
        ...
        1u);
    static_assert(
        SPARK_MUSE_GLIMMER_MODEL_ATTN_QUERY_HEAD_COUNT %
            SPARK_MUSE_GLIMMER_CUDA_ATTN_HEADS_PER_CTA == 0u,
...
extern "C" cudaError_t SparkMuseGlimmerLaunchChunkConv(cudaStream_t stream, const void *qkv_bf16, const SparkMuseGlimmerGdnLayerWeights *weights, void *conv_out_bf16, const SparkMuseGlimmerGdnStatePool *pool, uint32_t lane_index, uint32_t token_count, uint32_t gdn_layer_ordinal)
{
	if ( token_count == 0u )
		return(cudaErrorInvalidValue);
	SparkQwen38ChunkConvKernel<<<(SPARK_MUSE_GLIMMER_MODEL_GDN_CONV_CHANNELS + SPARK_LM_CTA_THREADS - 1u) / SPARK_LM_CTA_THREADS,SPARK_LM_CTA_THREADS,0,stream>>>(qkv_bf16,weights->conv_weight_bf16,conv_out_bf16,pool->conv_tail_bf16,lane_index,token_count,gdn_layer_ordinal,pool->conv_tail_lane_stride_elements,pool->conv_tail_layer_stride_elements);
	return(cudaGetLastError());
}
...
extern "C" cudaError_t SparkMuseGlimmerLaunchGdnChunk(cudaStream_t stream, const void *conv_out_bf16, const float *log_decay_f32, const float *beta_f32, float *workspace_qn, float *workspace_kn, float *workspace_cum_g, float *workspace_decay, float *workspace_attn, float *workspace_w, float *workspace_kg, const SparkMuseGlimmerGdnStatePool *pool, void *core_out_bf16, uint32_t lane_index, uint32_t token_count, uint32_t gdn_layer_ordinal)
{
    ...
    if (token_count == 0u || token_count > SPARK_MUSE_GLIMMER_CUDA_CHUNK)
    {
        return cudaErrorInvalidValue;
    }
    ...
    SparkQwen38ChunkQkDecayKernel<<<
        ...
        SPARK_MUSE_GLIMMER_CUDA_GDN_QK_SHARED_BYTES,
        stream>>>(views, token_count);
    status = cudaGetLastError();
    if (status != cudaSuccess)
    {
    ...
    SparkQwen38ChunkStepKernel<<<
        SPARK_MUSE_GLIMMER_MODEL_GDN_VALUE_HEAD_COUNT,
        SPARK_MUSE_GLIMMER_CUDA_DV,
        SPARK_MUSE_GLIMMER_CUDA_GDN_CHUNK_SHARED_BYTES,
        stream>>>(
            log_decay_f32,
            views,
            pool->state_f32,
...
extern "C" cudaError_t SparkMuseGlimmerLaunchEmbeddingGather(cudaStream_t stream, const uint32_t *token_ids, const void *embedding_bf16, void *hidden_bf16, uint32_t row_count)
{
	uint64_t elements = (uint64_t)row_count * SPARK_MUSE_GLIMMER_MODEL_HIDDEN_DIMENSION;
...
extern "C" cudaError_t SparkMuseGlimmerLaunchHeadShadowQuantize(cudaStream_t stream, const void *head_bf16, uint8_t *shadow_payload, uint8_t *shadow_scale, float *error_norm, uint32_t candidate_count, uint32_t hidden_dimension)
{
...
extern "C" cudaError_t SparkMuseGlimmerLaunchHeadScreenedArgmax(cudaStream_t stream, const void *hidden_bf16, const void *head_weight_bf16, const uint8_t *shadow_payload, const uint8_t *shadow_scale, const float *error_norm, void *logits_bf16, uint32_t *candidate_ids, uint32_t *candidate_counts, uint32_t *output_token_ids, uint32_t row_count, uint32_t candidate_count)
{
	return(SparkLmHostLaunchHeadScreenedArgmax(stream,hidden_bf16,head_weight_bf16,shadow_payload,shadow_scale,error_norm,logits_bf16,candidate_ids,candidate_counts,output_token_ids,row_count,candidate_count,SPARK_MUSE_GLIMMER_MODEL_HIDDEN_DIMENSION));
}
    ...
    float *weights_f32)
{
    ...
    static_assert(
        SPARK_MUSE_GLIMMER_MODEL_ROUTED_EXPERT_COUNT <=
    ...
    if ( row >= row_count || expert_count == 0u ||
        expert_count > SPARK_MUSE_GLIMMER_MODEL_ROUTED_EXPERT_COUNT ||
...
extern "C" cudaError_t SparkMuseGlimmerLaunchMoeRoute(cudaStream_t stream, const uint32_t *route_expert, uint32_t rows, uint32_t expert_width, uint32_t *group_row_offset, uint32_t *route_packed_row, uint32_t *route_source_token, uint32_t *group_tile_prefix_w1, uint32_t *group_tile_prefix_w2)
{
	int32_t launch_status = LmRouteBuild<SPARK_LM_CTA_THREADS,SPARK_MUSE_GLIMMER_MODEL_ROUTED_EXPERT_COUNT>(route_expert,rows,rows * SPARK_MUSE_GLIMMER_MODEL_EXPERTS_PER_TOKEN,SPARK_MUSE_GLIMMER_MODEL_EXPERTS_PER_TOKEN,group_row_offset,route_packed_row,route_source_token,expert_width,SPARK_MUSE_GLIMMER_MODEL_HIDDEN_DIMENSION,SPARK_LM_TILE_N,SPARK_LM_TILE_N,group_tile_prefix_w1,group_tile_prefix_w2,stream);
...
{
	cudaError_t status;
	uint64_t required_rows = (uint64_t)SPARK_MUSE_GLIMMER_MODEL_ROUTED_EXPERT_COUNT * expert_width;
	if ( w1 == 0 || w3 == 0 || input_bf16 == 0 || route_source_token == 0 || group_row_offset == 0 || group_tile_prefix == 0 || activated_bf16 == 0 || w1->weight_format != SPARK_MUSE_GLIMMER_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_MXFP4_E2M1 || w3->weight_format != SPARK_MUSE_GLIMMER_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_MXFP4_E2M1 || w1->weight_payload == 0 || w3->weight_payload == 0 || w1->weight_scale_e8m0 == 0 || w3->weight_scale_e8m0 == 0 || w1->output_dimension != required_rows || w3->output_dimension != required_rows || w1->input_dimension != SPARK_MUSE_GLIMMER_MODEL_HIDDEN_DIMENSION || w3->input_dimension != SPARK_MUSE_GLIMMER_MODEL_HIDDEN_DIMENSION )
		return(cudaErrorInvalidValue);
	status = SparkLmHostLaunchSm121FusedExpertW13(stream,w1->weight_payload,w1->weight_scale_e8m0,w3->weight_payload,w3->weight_scale_e8m0,input_bf16,route_source_token,group_row_offset,group_tile_prefix,activated_bf16,rows,SPARK_MUSE_GLIMMER_MODEL_EXPERTS_PER_TOKEN,SPARK_MUSE_GLIMMER_MODEL_ROUTED_EXPERT_COUNT,SPARK_MUSE_GLIMMER_MODEL_HIDDEN_DIMENSION,expert_width,limit,SPARK_LM_WEIGHT_FORMAT_MXFP4_E2M1,multiprocessor_count);
	return(status);
}
...
{
	uint64_t required_rows = (uint64_t)SPARK_MUSE_GLIMMER_MODEL_ROUTED_EXPERT_COUNT * hidden_dimension;
	if ( stacked == 0 || input_bf16 == 0 || group_row_offset == 0 || group_tile_prefix == 0 || output_bf16 == 0 || stacked->weight_format != SPARK_MUSE_GLIMMER_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_MXFP4_E2M1 || stacked->weight_payload == 0 || stacked->weight_scale_e8m0 == 0 || stacked->output_dimension != required_rows || stacked->input_dimension != expert_width )
		return(cudaErrorInvalidValue);
	return(SparkLmHostLaunchSm121ExpertW2(stream,stacked->weight_payload,stacked->weight_scale_e8m0,input_bf16,group_row_offset,group_tile_prefix,output_bf16,rows,SPARK_MUSE_GLIMMER_MODEL_EXPERTS_PER_TOKEN,SPARK_MUSE_GLIMMER_MODEL_ROUTED_EXPERT_COUNT,expert_width,hidden_dimension,SPARK_LM_WEIGHT_FORMAT_MXFP4_E2M1,multiprocessor_count));
}

extern "C" cudaError_t SparkMuseGlimmerLaunchMoePairReduce(cudaStream_t stream, const void *slot_out_bf16, const uint32_t *inverse_map, const float *pair_weights_f32, void *accum_bf16, uint32_t row_count, uint32_t hidden_dimension)
{
	return(SparkLmHostLaunchMoePairReduce(stream,slot_out_bf16,inverse_map,pair_weights_f32,accum_bf16,row_count,SPARK_MUSE_GLIMMER_MODEL_EXPERTS_PER_TOKEN,hidden_dimension));
}
...
{
	return(SparkLmHostLaunchMoePairReduceOverwrite(stream,slot_out_bf16,inverse_map,pair_weights_f32,output_bf16,row_count,SPARK_MUSE_GLIMMER_MODEL_EXPERTS_PER_TOKEN,hidden_dimension));
}

extern "C" cudaError_t SparkMuseGlimmerLaunchSwiGlu(cudaStream_t stream, const void *gate_bf16, void *up_bf16, uint32_t row_count, uint32_t dimension)
{
...
extern "C" cudaError_t SparkMuseGlimmerLaunchSharedGate(cudaStream_t stream, void *accum_bf16, const void *gate_weight_bf16, const void *gate_input_bf16, uint32_t row_count, uint32_t dimension)
{
...
extern "C" cudaError_t SparkMuseGlimmerLaunchGateScores(cudaStream_t stream, const SparkMuseGlimmerLinearView *gate, const void *input_bf16, float *scores_f32, uint32_t row_count)
{
	...
	if ( gate == 0 || input_bf16 == 0 || scores_f32 == 0 || gate->weight_format != SPARK_MUSE_GLIMMER_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_BF16 || gate->input_dimension == 0u )
		return(cudaErrorInvalidValue);
...
extern "C" cudaError_t SparkMuseGlimmerConfigureCudaKernels(void)
{
    ...
    status = cudaFuncSetAttribute(
        ...
        (int)SPARK_MUSE_GLIMMER_CUDA_GDN_DECODE_SHARED_BYTES);
    if (status != cudaSuccess)
    {
        fprintf(stderr, "qwen38 configure gdn_step failed %d shared=%d\n", (int)status, (int)SPARK_MUSE_GLIMMER_CUDA_GDN_DECODE_SHARED_BYTES);
        return status;
    }
    status = cudaFuncSetAttribute(
        SparkQwen38ChunkQkDecayKernel,
        cudaFuncAttributeMaxDynamicSharedMemorySize,
        (int)SPARK_MUSE_GLIMMER_CUDA_GDN_QK_SHARED_BYTES);
    if (status != cudaSuccess)
    {
        return status;
    }
    status = cudaFuncSetAttribute(
        SparkQwen38ChunkStepKernel,
        cudaFuncAttributeMaxDynamicSharedMemorySize,
        (int)SPARK_MUSE_GLIMMER_CUDA_GDN_CHUNK_SHARED_BYTES);
    if (status != cudaSuccess)
    {
        return status;
    }
    return cudaFuncSetAttribute(
        ...
        cudaFuncAttributeMaxDynamicSharedMemorySize,
        (int)(SPARK_MUSE_GLIMMER_MODEL_ATTN_QUERY_DIMENSION * sizeof(float)));
}

extern "C" cudaError_t SparkMuseGlimmerLaunchGroupedExpertLinear(
	cudaStream_t stream,
	const SparkMuseGlimmerLinearView *view,
	...
	uint32_t tp_rank)
{
	...
	uint32_t experts_per_rank = SPARK_MUSE_GLIMMER_MODEL_ROUTED_EXPERT_COUNT / tp_degree;
	...
	if ( view == 0 || input_bf16 == 0 ||
		...
		view->weight_format != SPARK_MUSE_GLIMMER_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_FP8_E4M3_F32B128 ||
		view->output_dimension % SPARK_MUSE_GLIMMER_MODEL_ROUTED_EXPERT_COUNT != 0u ||
		view->weight_payload == 0 || view->weight_scale_e8m0 == 0 ||
		(source_row_map == 0 && source_row_count == 0u) ||
		tp_degree == 0u || tp_rank >= tp_degree ||
		(SPARK_MUSE_GLIMMER_MODEL_ROUTED_EXPERT_COUNT % tp_degree) != 0u )
		return(cudaErrorInvalidValue);
	rows_per_expert = (uint64_t)view->output_dimension / SPARK_MUSE_GLIMMER_MODEL_ROUTED_EXPERT_COUNT;
...
extern "C" cudaError_t SparkMuseGlimmerLaunchGroupedExpertTileLinear(
	cudaStream_t stream,
	const SparkMuseGlimmerLinearView *view,
	const void *input_bf16,
	const uint32_t *source_row_map,
	...
	uint32_t tp_rank)
{
	...
	(void)m_blocks;
	(void)n_tiles;
	experts_per_rank = SPARK_MUSE_GLIMMER_MODEL_ROUTED_EXPERT_COUNT / tp_degree;
	payload = (const uint8_t *)view->weight_payload + ((uint64_t)tp_rank * experts_per_rank * payload_stride);
	scale = (const uint8_t *)view->weight_scale_e8m0 + ((uint64_t)tp_rank * experts_per_rank * scale_stride);
	offsets = group_row_offset + ((uint64_t)tp_rank * experts_per_rank);
