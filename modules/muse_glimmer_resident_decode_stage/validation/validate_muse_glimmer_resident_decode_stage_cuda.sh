...
require_configuration_value SPARK_MUSE_GLIMMER_STAGE_MTP 0
...
if [[ "${SPARK_MUSE_GLIMMER_STAGE_COUNT:-0}" == "1" ]]; then
    require_configuration_value SPARK_MUSE_GLIMMER_STAGE_LAYER_COUNT 92
else
    if (( ${SPARK_MUSE_GLIMMER_STAGE_COUNT:-0} < 2 )); then
        echo "muse_glimmer hardware validation requires SPARK_MUSE_GLIMMER_STAGE_COUNT >= 2 (mid-pipeline stage 0) or 1 (whole stack)" >&2
        exit 2
    fi
    if (( ${SPARK_MUSE_GLIMMER_STAGE_LAYER_COUNT:-0} < 4 || ${SPARK_MUSE_GLIMMER_STAGE_LAYER_COUNT:-92} >= 92 )); then
        echo "muse_glimmer hardware validation requires 4 <= SPARK_MUSE_GLIMMER_STAGE_LAYER_COUNT < 92 (GDN and full attention, no final head)" >&2
        exit 2
    fi
fi
