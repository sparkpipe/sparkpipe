...
static int SparkQwen36ValCheckGdnStep(SparkQwen36ValDevice *device)
{
	...
	SparkQwen36ValFillBf16(host_conv,exact,2ull * SPARK_QWEN36_CONV,1.0f);
	{
		...
		for (index = 0u; index < state_elements; index++)
			state_host[state_elements + index] = SparkQwen36ValUniform(0.25f);  
...
static int SparkQwen36ValCheckGatedNorm(SparkQwen36ValDevice *device)
{
	...
	SparkQwen36ValFillBf16(packed,exact,elements,1.0f);               
	SparkQwen36ValFillBf16(packed + elements,exact + elements,elements,1.0f);  
...
static int SparkQwen36ValCheckDriftAccumulation(SparkQwen36ValDevice *device)
{
	...
	const uint32_t rows_per_round = 4u;       
