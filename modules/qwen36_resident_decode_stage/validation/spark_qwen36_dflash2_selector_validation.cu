 ...
 *
 * Runs the two new kernels through their PRODUCTION launchers on exactly the
 * inputs tools/qwen36_dflash2_selector_case.py generated, and diffs against
 ...
 * ports of vLLM PR #52816 - the contract).
 *
 ...
 * counter-based splitmix64. That keeps a 2.54 GB BF16 head out of the file
 * while leaving the comparison exact.
 *
 * Every generated value is a small integer (hidden in [-4,4], weights in
...
#define SPARK_DFLASH2_CASE_MAGIC "Q6DF2CS1"
...
/* Stream seeds, identical to the generator's. */
...
{
	uint64_t raw = SparkDflash2CaseValue(index,stream_seed,seed) >> 11u;
	return((float)((int64_t)(raw % (uint64_t)((2u * span) + 1u)) - (int64_t)span));
}

/* Numpy f32_to_bf16 in the reference: round to nearest even, no NaN games. */
...
static int SparkDflash2ReadCase(const char *path, SparkDflash2Case *output)
{
	...
	char magic[8];
	uint32_t header[8];
	uint64_t rows,lattice,gate_elements;
	...
	{
		fprintf(stderr,"case magic mismatch\n");
		fclose(file);
		return(-1);
	}
	if ( fread(header,sizeof(uint32_t),8u,file) != 8u || fread(&output->seed,sizeof(uint64_t),1u,file) != 1u )
	{
		fclose(file);
		return(-1);
	}
	...
	lattice = rows * output->top_k * output->top_k;
	gate_elements = rows * output->rank;
	output->anchor_token_ids = (uint32_t *)malloc(output->batch_count * sizeof(uint32_t));
	output->top_candidate_ids = (uint32_t *)malloc(rows * output->top_k * sizeof(uint32_t));
	output->unary_f32 = (float *)malloc(rows * output->top_k * sizeof(float));
	...
	if ( output->anchor_token_ids == 0 || output->top_candidate_ids == 0 || output->unary_f32 == 0 ||
	     output->context_gate_bf16 == 0 || output->edges_f32 == 0 || output->draft_token_ids == 0 ||
	     output->draft_candidate_slots == 0 )
	{
		fclose(file);
		return(-1);
	}
	if ( fread(output->anchor_token_ids,sizeof(uint32_t),output->batch_count,file) != output->batch_count ||
	     fread(output->top_candidate_ids,sizeof(uint32_t),rows * output->top_k,file) != rows * output->top_k ||
	     fread(output->unary_f32,sizeof(float),rows * output->top_k,file) != rows * output->top_k ||
 ...
 * a 248320 x 5120 head never needs a full host mirror. */
static int SparkDflash2FillDeviceBf16(void *device, uint64_t element_count, uint64_t stream_seed, uint64_t seed, uint32_t span, uint64_t chunk_elements, const char *label)
{
	uint16_t *staging;
	uint64_t first,index,count;
	cudaError_t error;
	...
	for (first = 0ull; first < element_count; first += chunk_elements)
	{
		...
		for (index = 0ull; index < count; index++)
			staging[index] = SparkDflash2FloatToBf16(SparkDflash2SmallInteger(first + index,stream_seed,seed,span));
...
int main(int argc, char **argv)
{
	...
	uint64_t rows,lattice,gate_elements,candidate_elements,chunk_keys_count;
	...
	lattice = rows * testcase.top_k * testcase.top_k;
	gate_elements = rows * testcase.rank;
	candidate_elements = rows * testcase.top_k;
	chunk_keys_count = SparkQwen36DsparkHeadTopKChunkKeyCount((uint32_t)rows,testcase.top_k);
	printf("case            = %s\n",case_path);
	...
	printf("head bytes      = %.3f GiB (BF16, dense, read through SparkQwen36LinearView)\n",
	...
	SPARK_DFLASH2_CUDA(cudaMalloc((void **)&top_ids_device,(size_t)candidate_elements * sizeof(uint32_t)));
	SPARK_DFLASH2_CUDA(cudaMalloc((void **)&oracle_ids_device,(size_t)candidate_elements * sizeof(uint32_t)));
	SPARK_DFLASH2_CUDA(cudaMalloc((void **)&top_scores_device,(size_t)candidate_elements * sizeof(float)));
	SPARK_DFLASH2_CUDA(cudaMalloc((void **)&oracle_unary_device,(size_t)candidate_elements * sizeof(float)));
	...
	if ( SparkDflash2FillDeviceBf16(hidden_device,rows * testcase.hidden_dimension,SPARK_DFLASH2_STREAM_HIDDEN,testcase.seed,testcase.hidden_span,1ull << 20u,"hidden") != 0 )
		return(3);
	if ( SparkDflash2FillDeviceBf16(head_device,(uint64_t)testcase.vocab_count * testcase.hidden_dimension,SPARK_DFLASH2_STREAM_HEAD,testcase.seed,testcase.weight_span,(uint64_t)SPARK_DFLASH2_HEAD_CHUNK_ROWS * testcase.hidden_dimension,"head") != 0 )
		return(3);
	if ( SparkDflash2FillDeviceBf16(projection_device,(uint64_t)testcase.rank * testcase.hidden_dimension,SPARK_DFLASH2_STREAM_PROJECTION,testcase.seed,testcase.weight_span,1ull << 20u,"projection") != 0 )
		return(3);
	if ( SparkDflash2FillDeviceBf16(predecessor_device,(uint64_t)testcase.vocab_count * testcase.rank,SPARK_DFLASH2_STREAM_PREDECESSOR,testcase.seed,testcase.weight_span,1ull << 22u,"predecessor") != 0 )
		return(3);
	if ( SparkDflash2FillDeviceBf16(successor_device,(uint64_t)testcase.vocab_count * testcase.rank,SPARK_DFLASH2_STREAM_SUCCESSOR,testcase.seed,testcase.weight_span,1ull << 22u,"successor") != 0 )
		return(3);
	...
	SPARK_DFLASH2_CUDA(cudaMemcpy(oracle_ids_device,testcase.top_candidate_ids,(size_t)candidate_elements * sizeof(uint32_t),cudaMemcpyHostToDevice));
	SPARK_DFLASH2_CUDA(cudaMemcpy(oracle_unary_device,testcase.unary_f32,(size_t)candidate_elements * sizeof(float),cudaMemcpyHostToDevice));
	...
	host_ids = (uint32_t *)malloc((size_t)candidate_elements * sizeof(uint32_t));
	host_scores = (float *)malloc((size_t)candidate_elements * sizeof(float));
	...
	SPARK_DFLASH2_CUDA(SparkQwen36LaunchDsparkHeadTopK(0,&head_view,hidden_device,chunk_keys_device,top_ids_device,top_scores_device,0,(uint32_t)rows,0u,testcase.top_k));
	SPARK_DFLASH2_CUDA(cudaDeviceSynchronize());
	SPARK_DFLASH2_CUDA(cudaMemcpy(host_ids,top_ids_device,(size_t)candidate_elements * sizeof(uint32_t),cudaMemcpyDeviceToHost));
	SPARK_DFLASH2_CUDA(cudaMemcpy(host_scores,top_scores_device,(size_t)candidate_elements * sizeof(float),cudaMemcpyDeviceToHost));
	printf("\n-- W4 top-%u over %u candidates, %llu slots\n",testcase.top_k,testcase.vocab_count,(unsigned long long)rows);
	SparkDflash2ReportU32("w4 top ids",host_ids,testcase.top_candidate_ids,candidate_elements);
	SparkDflash2ReportF32("w4 top scores (unary)",host_scores,testcase.unary_f32,candidate_elements);

	/* ---- W3 isolated: the oracle's candidates in, gate/lattice/walk out ---- */
	SPARK_DFLASH2_CUDA(SparkQwen36LaunchDsparkSelectorProject(0,hidden_device,projection_device,context_gate_device,(uint32_t)rows,testcase.rank,testcase.hidden_dimension));
	SPARK_DFLASH2_CUDA(SparkQwen36LaunchDsparkSelectorEdges(0,predecessor_device,successor_device,oracle_ids_device,anchor_device,oracle_unary_device,context_gate_device,edges_device,testcase.batch_count,testcase.slot_count,testcase.top_k,testcase.rank));
	SPARK_DFLASH2_CUDA(SparkQwen36LaunchDsparkSelectorWalk(0,edges_device,oracle_ids_device,draft_ids_device,draft_slots_device,testcase.batch_count,testcase.slot_count,testcase.top_k));
	SPARK_DFLASH2_CUDA(cudaDeviceSynchronize());
