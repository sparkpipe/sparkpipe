...
/*
 * Qwen36 resident-decode paged KV over the GENERAL prefix-cache core
 ...
 * CONTRACT plus the qwen36 hybrid wrinkle:
 *
...
#define SPARK_QWEN36_PAGED_KV_NO_BLOCK UINT32_MAX
#define SPARK_QWEN36_PAGED_KV_NO_SLOT UINT32_MAX
#define SPARK_QWEN36_PAGED_KV_NO_LANE UINT32_MAX
...
typedef struct SparkQwen36PagedKvConfiguration
{
	...
	/* Device KV blocks managed (the SPARK_QWEN36_STAGE_KV_BLOCKS pool);
	 ...
	 * consumes this for reuse accounting only (qwen36 keeps its KV row
...
SparkQwen36PagedKvConfiguration;
...
typedef struct SparkQwen36PagedKvMatch
{
	uint32_t block_count;
	uint32_t checkpoint_slot;
}
SparkQwen36PagedKvMatch;

typedef struct SparkQwen36PagedKvCheckpoint
{
	uint32_t live;
	uint32_t lane;
...
SparkQwen36PagedKvCheckpoint;

typedef struct SparkQwen36PagedKv
{
	...
	uint32_t *lane_live;
	SparkQwen36PagedKvCheckpoint *checkpoints;
