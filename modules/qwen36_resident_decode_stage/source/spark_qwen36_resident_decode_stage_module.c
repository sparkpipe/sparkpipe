...
static SparkStatus SparkQwen36ModuleRunGdnLayer(SparkQwen36ModuleState *state, SparkQwen36ModuleSlot *slot, const SparkQwen36PrefillFrameView *prefill, uint32_t layer, uint32_t rows)
{
	...
	if ( error == cudaSuccess )
	{
		...
		if ( step != 0u && prefill != 0 && slot->verify_frame != 0u &&
			...
			state->gdn_snapshot_slot_count >= SPARK_QWEN36_RESIDENT_DECODE_STAGE_VERIFY_CHECKPOINT_SLOT_BASE + 8u )
 ...
 *  1. SparkQwen36ModuleRunDsparkFullContextForward - unified HEAD's
 ...
 *  2. SparkQwen36ModuleRunDflash2CacheForward - origin/main's cache-based
 *     path verbatim (multi-block padding drafts, incremental context-KV
 *     cache, persistent block-KV history, device Select front-end).
 *
 * SPARK_QWEN36_DFLASH2_CACHE_PATH=1 selects main's path per process;
...
static SparkStatus SparkQwen36ModuleRunDsparkFullContextForward(
	SparkQwen36ModuleState *state,
	...
	uint32_t rows)
{
	...
	for (layer = 0u; status == SPARK_STATUS_OK && layer < SPARK_QWEN36_DSPARK_LAYER_COUNT; layer++)
	{
		...
		if ( error == cudaSuccess )
			error = SparkQwen36LaunchDsparkConv(stream,norm,slot->dspark_conv_delta,lw->conv_attn_base_bf16,slot->dspark_conv_out,B,SPARK_QWEN36_MODEL_HIDDEN_DIMENSION/SPARK_QWEN36_DSPARK_CONV_GROUP_SIZE,SPARK_QWEN36_DSPARK_CONV_GROUP_SIZE,0u);
		if ( error == cudaSuccess )
		...
		if ( error == cudaSuccess )
			error = SparkQwen36LaunchDsparkConv(stream,q,slot->dspark_conv_delta,(const void*)((const uint8_t*)lw->conv_attn_base_bf16 + (SPARK_QWEN36_DSPARK_CONV_KERNEL_SIZE * SPARK_QWEN36_MODEL_HIDDEN_DIMENSION) * SPARK_QWEN36_MODEL_BF16_ELEMENT_BYTES),slot->dspark_conv_out,B,SPARK_QWEN36_MODEL_HIDDEN_DIMENSION/SPARK_QWEN36_DSPARK_CONV_GROUP_SIZE,SPARK_QWEN36_DSPARK_CONV_GROUP_SIZE,1u);
		if ( error == cudaSuccess )
		...
		if ( error == cudaSuccess )
			error = SparkQwen36LaunchDsparkConv(stream,norm,slot->dspark_conv_delta,lw->conv_mlp_base_bf16,slot->dspark_conv_out,B,SPARK_QWEN36_MODEL_HIDDEN_DIMENSION/SPARK_QWEN36_DSPARK_CONV_GROUP_SIZE,SPARK_QWEN36_DSPARK_CONV_GROUP_SIZE,0u);
		if ( error == cudaSuccess )
...
static uint32_t SparkQwen36ModuleUseDflash2CachePath(void)
{
    const char *env = getenv("SPARK_QWEN36_DFLASH2_CACHE_PATH");
 ...
 * full-context path). */
static SparkStatus SparkQwen36ModuleRunDflash2CacheForward(SparkQwen36ModuleState *state,SparkQwen36ModuleSlot *slot,const SparkQwen36DsparkDraftView *view,const uint32_t *row_tokens_host,uint32_t rows);
static SparkStatus SparkQwen36ModuleRunDsparkBlockForward(
    ...
    uint32_t rows)
{
    if ( SparkQwen36ModuleUseDflash2CachePath() != 0u && state->dflash_taps_history != 0 )
        return(SparkQwen36ModuleRunDflash2CacheForward(state,slot,view,row_tokens_host,rows));
    (void)row_tokens_host;
    return(SparkQwen36ModuleRunDsparkFullContextForward(state,slot,view,rows));
}
...
static SparkStatus SparkQwen36ModuleRunDflash2CacheForward(
	SparkQwen36ModuleState *state,
	SparkQwen36ModuleSlot *slot,
	const SparkQwen36DsparkDraftView *view,
	const uint32_t *row_tokens_host,
	uint32_t rows)
{
	...
	if ( status == SPARK_STATUS_OK )
	{
		...
		if ( error == cudaSuccess )
			error = SparkQwen36LaunchDsparkSelect(stream,logits,norm,w->selector_hidden_projection.weight_payload,state->dspark_sel_out_dev,B,V,H,R,K);
		if ( error == cudaSuccess )
		...
		if ( error == cudaSuccess )
		{
			...
			if ( dflash2_dump_done == 0 )
			{
				...
				SparkQwen36ModuleDumpHostFile("/tmp/dflash2_taps.bin",taps_host,(uint64_t)5u * H * 2u);
				free(taps_host);
				SparkQwen36ModuleDumpHostFile("/tmp/dflash2_c0.bin",&prev,4u);
				SparkQwen36ModuleDumpHostFile("/tmp/dflash2_logits.bin",slot->dspark_logits_host,(uint64_t)B * V * 2u);
				SparkQwen36ModuleDumpHostFile("/tmp/dflash2_hidden.bin",slot->dspark_hidden_host,(uint64_t)B * H * 2u);
