	...
	for (lane=0u; status == SPARK_STATUS_OK && lane<submission->active_sequence_count; lane++)
	{
		...
		if ( status == SPARK_STATUS_OK )
		{
			...
			if ( SparkQwen36ServingBlockDraftMethod(spec_method) )
			{
				...
				/* Bonus convention: draft i predicts position base+i, so slots
				 * 1..6 map directly; the last verify row (base+7) has no draft -
				 * the mask id stands in and stops the accept loop cleanly. */
