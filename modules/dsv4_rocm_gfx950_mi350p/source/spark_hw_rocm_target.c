...
// capability must equal the pinned profile expectation (9.5) from
// sparkpipe/spark_hardware_topology.h - the same constant every other target
// consumer reads. Both must agree; any mismatch or query failure is a
...
static SparkHwStatus spark_hw_rocm_guard_probe(int device)
{
    ...
    /* Expectation comes from the compiled-for target profile
     * (rocm.gfx950.mi350p pins capability 9.5 in
     * sparkpipe/spark_hardware_topology.h); the queried values above stay
     * the measured side of the comparison. */
    if (major != (int)SPARK_HW_COMPUTE_CAPABILITY_MAJOR ||
        minor != (int)SPARK_HW_COMPUTE_CAPABILITY_MINOR)
    {
...
SparkHwStatus spark_hw_rocm_require_gfx950(int device)
{
    ...
    /* Only IDENTITY verdicts are cached. OK and UNSUPPORTED answer "is
     * this device a gfx950 part" definitively; anything else (INVALID,
     * EXHAUSTED, LOST, driver hiccups mapped by spark_hw_rocm_map_status)
     * says nothing about identity and is returned uncached: the call
     * still fails closed right now, but the next call re-probes instead
     * of inheriting a stale refusal - a recovered device must not stay
     * bricked by one bad probe. */
    if (status == SPARK_HW_OK)
    {
        atomic_store_explicit(&s_guard_cache[device],
                              (uint8_t)SPARK_HW_ROCM_GUARD_PASS,
                              memory_order_relaxed);
    }
    else if (status == SPARK_HW_UNSUPPORTED)
    {
        atomic_store_explicit(&s_guard_cache[device],
