...
/* Flag bits are an identity mapping onto HIP's values (the create call
 * forwards them verbatim); pinned so a HIP header value drift fails
 * compilation instead of silently changing queue semantics. */
_Static_assert(SPARK_HW_QUEUE_NONBLOCKING == (uint32_t)hipStreamNonBlocking,
               "SPARK_HW_QUEUE_NONBLOCKING must equal hipStreamNonBlocking");
