...
First on-hardware actions: \`tools/island_compile_check.sh hipcc\`, then
the synthetic sealed-route driver below (C2 halves bit-exact, C3 bounded)
before any performance work.

## Owed item delivered - synthetic sealed-route batch driver (2026-08-25)

The driver flagged above as the first on-hardware step is now landed under
validation/:

- l4_route_synthesis.h - ONE shared source of truth for both sides of the
  comparison: deterministic LCG case synthesis (route inputs, hidden rows,
  initial accumulator, per-view packed weight tensors incl. FP8 block-scale
  and MXFP4 nibble payloads) plus a scalar REFERENCE MODEL mirroring the
  device kernels' fixed trees exactly (per-lane strided partials +
  shuffle-down butterfly, RNE bf16 stores, clamp semantics, ascending-k
  accumulator-first weight fold, canonical route group).
- l4_route_driver.c - host C11 program (runs anywhere): selfcheck mode
  asserts reference invariants over all cases (offsets monotone + total,
  pairs land inside their expert window, inverse map is a perfect
  permutation, ascending within-expert packing); gen mode emits SLR4 v1
  binary fixtures + SHA-256 manifest. Nine edge cases: empty frame, single
  pair, uniform spread, heavy skew, K==E boundary, prime ragged experts,
  E=1024 wide ids, FP8 block-scale views, MXFP4 nibble views.
- l4_route_device_check.hip - hardware comparator: loads a fixture, drives
  the REAL island entry through the spark_hw_* queue/event primitives
  (stream-ordered callback completion; synchronize on failure path only),
  then compares C2 fields bit-exact and the C3 post-island accumulator
  under fixture-carried abs/rel tolerances (host libm vs device math ulps).
- run_l4_route_validation.sh - gen mode verified locally 2026-08-25:
  selfcheck OK over 9/9 cases; fixtures generated with stable hashes across
  runs (reproducibility proven). check mode (hipcc + gfx950) verifies every
  manifest hash before running each case and is ready for the MI350P node;
  NOT RUN on this box (plan section 2 box limitation).
