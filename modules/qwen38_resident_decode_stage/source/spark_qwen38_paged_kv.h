 ...
 * CONTRACT plus the qwen38 hybrid wrinkle (port 2 of 4; the structure
...
#define SPARK_QWEN38_PAGED_KV_NO_BLOCK UINT32_MAX
#define SPARK_QWEN38_PAGED_KV_NO_SLOT UINT32_MAX
#define SPARK_QWEN38_PAGED_KV_NO_LANE UINT32_MAX
...
typedef struct SparkQwen38PagedKvConfiguration
{
	...
	/* Device KV blocks managed (the SPARK_QWEN38_STAGE_KV_BLOCKS pool);
	 ...
	 * consumes this for reuse accounting only. qwen38 computes the stride
...
SparkQwen38PagedKvConfiguration;
...
typedef struct SparkQwen38PagedKvMatch
{
	uint32_t block_count;
	uint32_t checkpoint_slot;
}
SparkQwen38PagedKvMatch;

typedef struct SparkQwen38PagedKvCheckpoint
{
	uint32_t live;
	uint32_t lane;
...
SparkQwen38PagedKvCheckpoint;

typedef struct SparkQwen38PagedKv
{
	...
	uint32_t *lane_live;
	SparkQwen38PagedKvCheckpoint *checkpoints;
