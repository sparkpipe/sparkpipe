...
#define SPARK_K3_SERVING_STAGE_COUNT 16u
	...
	const SparkModelServingAdapterConfiguration *configuration)
{
...

	{
		...
		if ( SparkJsonGetUInt32Member(&doc, SparkJsonGetRootToken(&doc),
			"schema_version", &schema_version) == SPARK_STATUS_OK &&
			schema_version !=
			SPARK_K3_SERVING_ADAPTER_CONFIGURATION_SCHEMA_VERSION )
...
static void K3ServingFreeScratch(SparkK3ServingState *state)
{
	...
	free(state->positions_host64);
	free(state->tokens_host);
	...
	void **adapter_state)
{
	...
	status = SparkAdapterValidateConfiguration(&K3ServingDescriptor,
		configuration,"k3",SPARK_K3_SERVING_STAGE_COUNT);
	if ( status != SPARK_STATUS_OK )
		return status;
	...
	state->positions_host64 = (uint64_t *)malloc((uint64_t)state->max_rows * 8u);
	state->tokens_host = (uint32_t *)malloc((uint64_t)state->max_rows * 4u);
	if ( state->positions_host == 0 || state->context_host == 0 ||
		state->state_host == 0 || state->positions_host64 == 0 ||
		state->tokens_host == 0 )
	...
	const SparkModelServingSubmission *submission)
{
	...
	cudaMemcpy(state->positions_host64, submission->row_positions,
		(uint64_t)rows * 8u, cudaMemcpyDeviceToHost);
	for ( uint32_t i = 0u; i < rows; ++i )
	{
		state->positions_host[i] = (uint32_t)state->positions_host64[i];
		state->context_host[i] = (uint32_t)state->positions_host64[i] + 1u;
	...
	if ( state->common.sink.function != 0 )
	{
		...
		cudaMemcpy(state->tokens_host, state->output_tokens,
		...
		{
			completion.token_ids[i] = state->tokens_host[i];
			copied++;
		}
	...
	SparkModelServingAdapterSnapshot *snapshot)
{
	...
	snapshot->rejected_count = stats.failed_count +
...
static const SparkModelServingAdapterDescriptor K3ServingDescriptor =
{
	...
	.stage_layer_counts = { 6u, 6u, 6u, 6u, 6u, 6u, 6u, 6u, 6u, 6u, 6u, 6u, 6u, 6u, 6u, 3u },
