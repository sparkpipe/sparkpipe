...
MODULE_BATCH_VARIANT_BUCKETS ?= 1 2 4 6 8 9 11 16 32 64 128 256 512 1024
...
MTP_LAYER_COUNT ?= 1
CUDA_GRAPH_COUNT ?= 0
# The Pro full-stage reference fixture is not retained yet; the manifest
# stays EMPTY so the recipe's slice-independent oracle gate is dormant for
# Pro builds instead of pointing them at the Flash stage-0 fixture (whose
# layer range cannot match l61). Pin the Pro manifest in the same commit
# that retains the Pro fixture (audit F-3, TECHDEBT "DSV4-Pro").
override DSV4_GA_STAGE0_REFERENCE_MANIFEST_SHA256 :=
