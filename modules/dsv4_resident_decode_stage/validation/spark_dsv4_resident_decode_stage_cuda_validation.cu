...
typedef struct SparkDsv4ValidationMode
{
	...
	/* Boundary hidden entering the slice (fixture dump after the previous
	 * layer); mandatory exactly when the slice is not stage-0 layer 0. */
	const char *reference_hidden_path;
...
static void SparkDsv4ValidationBuildReferenceContext(SparkDsv4ValidationReferenceFrame *frame,uint64_t hidden_bytes)
{
	...
	/* Slices past layer 0 consume the fixture's previous boundary as their
	 * hidden input; the module sees exactly the production hand-off. */
	if ( frame->hidden_input_bf16 != 0 )
	{
		...
		frame->context.hidden_input_bytes = hidden_bytes;
	}
...
static int SparkDsv4ValidationBuildReferenceFrame(SparkDsv4ValidationReferenceFrame *frame,const char *token_path,const char *hidden_path)
{
	...
	const uint64_t fixture_hidden_bytes = (uint64_t)SPARK_DSV4_VALIDATION_REFERENCE_FIXTURE_ROW_COUNT * SPARK_DSV4_MODEL_BOUNDARY_STREAM_ELEMENTS * sizeof(uint16_t);
	cudaError_t error;
	...
	if ( error == cudaSuccess && hidden_path != 0 && hidden_path[0] != '\0' )
	{
		uint16_t *host_hidden = (uint16_t *)malloc((size_t)hidden_bytes);
		if ( host_hidden != 0 && SparkDsv4ValidationReadPrefix(hidden_path,host_hidden,hidden_bytes,fixture_hidden_bytes) == 0 )
		{
			error = cudaMalloc(&frame->hidden_input_bf16,hidden_bytes);
			if ( error == cudaSuccess )
				error = cudaMemcpyAsync(frame->hidden_input_bf16,host_hidden,hidden_bytes,cudaMemcpyHostToDevice,cudaStreamPerThread);
		}
		else
...
static int SparkDsv4ValidationRunReference(void *module_state,const SparkDsv4ResidentDecodeStageNodeContext *node_context,const char *token_path,const char *hidden_path,const char *output_path)
{
	const uint64_t elements = (uint64_t)SPARK_DSV4_VALIDATION_REFERENCE_ROW_COUNT * SPARK_DSV4_MODEL_BOUNDARY_STREAM_ELEMENTS;
	SparkDsv4ValidationReferenceFrame frame;
	uint16_t *actual;
	cudaError_t error;
	SparkStatus status;
	if ( node_context->first_layer_index >= SPARK_DSV4_MODEL_LAYER_COUNT ||
		node_context->layer_count == 0u ||
		node_context->first_layer_index + node_context->layer_count > SPARK_DSV4_MODEL_LAYER_COUNT )
		return(SparkDsv4ValidationRequire(0,"reference_slice_range"));
	actual = (uint16_t *)calloc((size_t)elements,sizeof(uint16_t));
	if ( actual == 0 || SparkDsv4ValidationBuildReferenceFrame(&frame,token_path,hidden_path) != 0 )
	{
		free(actual);
		return(1);
	}
...
static int SparkDsv4ValidationLoadMode(SparkDsv4ValidationMode *mode)
{
	...
	mode->reference_hidden_path = getenv("SPARK_DSV4_REFERENCE_INPUT_HIDDEN_PATH");
...
{
	int32_t exact_reference_slice,has_hidden,needs_hidden;
	/* The numerical oracle gate is slice-independent: every layer count
	 * compares against the fixture boundary for its own range. Stage 0
	 * layers 0..2 remains the mandated configuration; any other valid
	 * slice opts in with the same env pair. */
	if ( node_context->first_layer_index >= SPARK_DSV4_MODEL_LAYER_COUNT ||
		node_context->layer_count == 0u ||
		node_context->first_layer_index + node_context->layer_count > SPARK_DSV4_MODEL_LAYER_COUNT )
	{
		fprintf(stderr,"dsv4_validation reference_slice=invalid slice=%u+%u model_layers=%u\n",node_context->first_layer_index,node_context->layer_count,(unsigned)SPARK_DSV4_MODEL_LAYER_COUNT);
		return(1);
	}
	exact_reference_slice = node_context->stage_index == 0u && node_context->first_layer_index == 0u && node_context->layer_count == 3u;
	if ( exact_reference_slice != 0 && mode->use_reference == 0 )
	{
	...
	{
		has_hidden = mode->reference_hidden_path != 0 && mode->reference_hidden_path[0] != '\0' ? 1 : 0;
		needs_hidden = node_context->stage_index != 0u || node_context->first_layer_index != 0u ? 1 : 0;
		if ( has_hidden != needs_hidden )
		{
			fprintf(stderr,"dsv4_validation reference_hidden_configuration=invalid stage=%u slice=%u+%u provided=%d required=%d\n",node_context->stage_index,node_context->first_layer_index,node_context->layer_count,has_hidden,needs_hidden);
			return(1);
		}
	}
	return(0);
...
static int SparkDsv4ValidationRunMode(void *module_state,const SparkDsv4ResidentDecodeStageNodeContext *node_context,SparkDsv4ValidationCapture *capture,const SparkDsv4ValidationMode *mode)
{
	if ( SparkDsv4ValidationAdmit(module_state,mode->active_slot_count,mode->new_token_count,mode->frame_flags) != 0 )
		return(1);
	if ( mode->use_reference != 0 )
		return(SparkDsv4ValidationRunReference(module_state,node_context,mode->reference_token_path,mode->reference_hidden_path,mode->reference_path));
