...
struct SparkDsv4ModuleSlot
{
	...
	/* Updates enqueued to the device whose host-mirror write is deferred
	 * until the completion side confirms the device table is updated. */
	uint32_t page_table_mirror_pending;
	...
	uint32_t *dspark_state_row_limit;
	void *dspark_logits_bf16;
	float *dspark_logits_f32;
	/* Verify recurrence isolation: packed pre-frame image of the active
	 * lane's compressor/indexer state spans (see
	 * SparkDsv4ModuleStageVerifyStateSnapshot). */
	uint8_t *dspark_state_snapshot_b;
	uint64_t dspark_state_snapshot_bytes;
	uint32_t dspark_state_snapshot_page;
	uint32_t dspark_state_snapshot_valid;
	/* Verify window rollback: packed pre-frame images of the raw-KV ring
	 * slots and the compressed/index entry slots the staged rows touch
	 * (see SparkDsv4ModuleStageVerifyWindowSnapshot). */
	uint8_t *dspark_window_snapshot_b;
	uint64_t dspark_window_layer_stride;
	uint32_t dspark_window_base_slot;
	uint64_t dspark_window_positions[SPARK_DSV4_MODEL_DSPARK_SPEC_STEP + 1u];
	uint32_t dspark_window_valid;
};
...
struct SparkDsv4ModuleState
{
	...
	/* Host mirrors of the 1x1 pack scalars. The pack payloads live in
	 * device memory, so the value a kernel launch consumes is read back
	 * once at bind time and never dereferenced on the host afterwards. */
	float hc_head_scale_value;
	float mtp_hc_head_scale_value;
...
extern cudaError_t SparkDsv4LaunchCompressStep(cudaStream_t stream, const void *kv_bf16, const void *score_bf16, const float *ape_f32, float *kv_state_f32, float *score_state_f32, uint64_t state_lane_stride, const uint32_t *row_lane_indices, const uint64_t *row_positions, uint32_t row_count, uint32_t ratio, uint32_t overlapped, uint32_t width, void *emit_bf16, uint32_t *emitted, const uint32_t *state_row_limit);
...
/* The four transport combine callbacks differ only in the launcher they call
 * and the site tag they report (deep-audit D9: 66 lines of hand plumbing that
 * had already drifted from the other families' copies). One generator keeps
 * the plumbing single-sourced; the signatures are fixed by the collective ABI
 * (spark_tp_device_collective.h) and the tags are log-scraped contracts. The
 * varying parameter tail arrives through __VA_ARGS__ because a bare
 * comma-separated type list cannot be a single macro argument; the launch
 * expression precedes it so its parenthesised commas stay one argument. */
#define SPARK_DSV4_MODULE_COMBINE(name,site,launch,...) \
	static SparkStatus name(void *combine_context,__VA_ARGS__) \
	{ \
		cudaError_t error; \
		(void)combine_context; \
		error = launch; \
		return(SparkStageModuleCudaStatus(SPARK_DSV4_MODULE_TAG,error,site)); \
	}

SPARK_DSV4_MODULE_COMBINE(SparkDsv4ModuleCombineBf16,"tp_all_reduce_sum",
	SparkDsv4LaunchAccumAdd((cudaStream_t)cuda_stream,destination_device,
		source_device,active_sequence_count,hidden_dimension),
...
SPARK_DSV4_MODULE_COMBINE(SparkDsv4ModuleCombineRelayBf16,
	"tp_all_reduce_sum_relay",
	SparkDsv4LaunchAccumAddRelay((cudaStream_t)cuda_stream,destination_device,
		source_device,relay_device,active_sequence_count,hidden_dimension),
	void *destination_device,const void *source_device,void *relay_device,
	uint32_t active_sequence_count,uint32_t hidden_dimension,void *cuda_stream)

SPARK_DSV4_MODULE_COMBINE(SparkDsv4ModuleCombineTp4Bf16,
	"tp_all_reduce_sum_tp4_tree",
	SparkDsv4LaunchAccumAddTp4Tree((cudaStream_t)cuda_stream,destination_device,
		rank_devices,tp_rank,active_sequence_count,hidden_dimension),
	void *destination_device,const void *const rank_devices[4],
	uint32_t tp_rank,uint32_t active_sequence_count,uint32_t hidden_dimension,
	void *cuda_stream)

SPARK_DSV4_MODULE_COMBINE(SparkDsv4ModuleCombineU64Max,"tp_all_reduce_max_u64",
	SparkDsv4LaunchAccumU64Max((cudaStream_t)cuda_stream,destination_device,
		source_device,element_count),
	uint64_t *destination_device,const uint64_t *source_device,
	uint32_t element_count,void *cuda_stream)

#undef SPARK_DSV4_MODULE_COMBINE
	...
	const char **pack_path_out)
{
	...
	/* Serving kill switch: SPARK_DSV4_DSPARK_ENABLED=0 stages every decode
	 * frame through the duplicate-pad path (islands keep their bucket-width
	 * shapes) with the draft/verify cluster off. This is also the
	 * same-build no-spec control channel the losslessness gate compares
	 * against. */
	if ( getenv("SPARK_DSV4_DSPARK_ENABLED") != 0 &&
		strcmp(getenv("SPARK_DSV4_DSPARK_ENABLED"),"0") == 0 )
	{
		state->dspark_enabled = 0u;
		fprintf(stderr,"dspark disabled by SPARK_DSV4_DSPARK_ENABLED=0 tp_rank=%u\n",state->tp_rank);
	}
...
static SparkStatus SparkDsv4ModuleValidateSlice(SparkDsv4ModuleState *state)
{
	...
	if ( state->tp_degree > 1u )
	{
		...
		/* Same hash-router pin as the tp_degree==1 path below: a slice that
		 * starts inside the hash-routed range without owning the embedding
		 * has no token ids to hash and is refused at configuration, not
		 * discovered at runtime. */
		if ( state->owns_embedding == 0u &&
			state->first_layer_index < SPARK_DSV4_MODEL_HASH_ROUTED_LAYER_COUNT )
		{
			fprintf(stderr,"%s config_hash_layer_without_tokens tp slice=%u+%u\n",SPARK_DSV4_MODULE_TAG,state->first_layer_index,state->layer_count);
			return(SPARK_STATUS_INVALID_ARGUMENT);
		}
		return(SPARK_STATUS_OK);
	}
...
static SparkStatus SparkDsv4ModuleValidateEntry(SparkDsv4ModuleState *state, const SparkDsv4StagePackEntry *entry, uint64_t file_bytes, uint64_t minimum_offset, uint32_t *is_global)
{
	SparkDsv4StagePackTensorShape shape;
	...
	/* Subtraction form: hostile u64 offset/byte pairs cannot wrap the fit
	 * test the way the historical addition form allowed
	 * (offset+bytes overflowing past file_bytes and validating). */
	if ( entry->payload_offset < minimum_offset || entry->payload_offset > file_bytes || payload_bytes > file_bytes - entry->payload_offset )
		return(SPARK_STATUS_VALIDATION_FAILED);
	if ( scale_bytes != 0u )
	{
		/* Contiguity is safe to test with addition only AFTER the payload
		 * fit above pinned offset+payload inside the file window. */
		if ( entry->scale_offset != entry->payload_offset + payload_bytes || entry->scale_offset > file_bytes || scale_bytes > file_bytes - entry->scale_offset )
			return(SPARK_STATUS_VALIDATION_FAILED);
	}
	*is_global = global;
	return(SPARK_STATUS_OK);
...
/* Pack payloads are device allocations, so any scalar the host consumes
 * (a launcher argument or an initialize-time gate) is read back with one
 * synchronous copy at bind time; dereferencing a pack pointer on the host
 * is never valid. */
...
static SparkStatus SparkDsv4ModuleBindGlobal(SparkDsv4ModuleState *state, const SparkDsv4StagePackEntry *entry, void *payload, void *scale)
{
	...
	case SPARK_DSV4_STAGEPACK_TENSOR_HC_HEAD_SCALE:
		state->hc_head_scale_f32 = (const float *)payload;
		/* The 1x1 gate scale feeds HcHeadReduce by value; bind and seed in
		 * one step so the launch can never observe the calloc'd zero. */
		status = SparkDsv4ModuleSeedHeadScaleValue((const float *)payload,&state->hc_head_scale_value);
		if ( status != SPARK_STATUS_OK )
			return(status);
		break;
	...
	case SPARK_DSV4_STAGEPACK_TENSOR_MTP_HC_HEAD_SCALE:
		state->mtp.hc_head_scale_f32 = (const float *)payload;
		status = SparkDsv4ModuleSeedHeadScaleValue((const float *)payload,&state->mtp_hc_head_scale_value);
		if ( status != SPARK_STATUS_OK )
			return(status);
		break;
...
static SparkStatus SparkDsv4ModuleLoadEntry(SparkDsv4ModuleState *state, FILE *file, const SparkDsv4StagePackEntry *entry, uint64_t file_bytes, uint64_t minimum_offset)
{
	...
	SparkStatus status = SparkDsv4ModuleValidateEntry(state,entry,file_bytes,minimum_offset,&is_global);
	if ( status != SPARK_STATUS_OK )
	{
...
static SparkStatus SparkDsv4ModuleVerifyCoverage(SparkDsv4ModuleState *state)
{
	...
	if ( SPARK_DSV4_MODEL_MTP_LAYER_COUNT != 0u )
	{
		...
		if ( isfinite(state->mtp_hc_head_scale_value) == 0 ||
			state->mtp_hc_head_scale_value == 0.0f )
		{
			fprintf(stderr,"%s pack_mtp_head_scale_invalid value=%g\n",SPARK_DSV4_MODULE_TAG,state->mtp_hc_head_scale_value);
			return(SPARK_STATUS_VALIDATION_FAILED);
		}
	}
	...
	if ( state->hc_head_scale_f32 != 0 &&
		( isfinite(state->hc_head_scale_value) == 0 ||
		  state->hc_head_scale_value == 0.0f ) )
	{
		fprintf(stderr,"%s pack_head_scale_invalid value=%g\n",SPARK_DSV4_MODULE_TAG,state->hc_head_scale_value);
		return(SPARK_STATUS_VALIDATION_FAILED);
	}
	return(SPARK_STATUS_OK);
}

/* Directory sweep, the glm52 discipline applied to this family: after the
 * per-entry pin, no two entries' payload or scale spans may overlap (a
 * double-bound alias would let one hostile entry poison a neighbour) and
 * every span must start at or after the directory end. Per-entry fit was
 * already pinned; the sweep adds the cross-entry view. O(n^2) over a
 * directory the header has already bounded against the file size. */
static SparkStatus SparkDsv4ModuleSweepDirectory(const SparkDsv4StagePackEntry *directory,uint32_t tensor_count,uint64_t minimum_offset)
{
	uint32_t i,j;
	for ( i = 0u; i < tensor_count; i++ )
	{
		const SparkDsv4StagePackEntry *entry = &directory[i];
		uint64_t entry_payload = SparkDsv4StagePackPayloadBytes(entry->weight_format,entry->rows,entry->columns);
		uint64_t entry_scale = SparkDsv4StagePackScaleBytes(entry->weight_format,entry->rows,entry->columns);
		if ( entry->payload_offset < minimum_offset )
			return(SPARK_STATUS_VALIDATION_FAILED);
		for ( j = i + 1u; j < tensor_count; j++ )
		{
			const SparkDsv4StagePackEntry *other = &directory[j];
			uint64_t other_payload = SparkDsv4StagePackPayloadBytes(other->weight_format,other->rows,other->columns);
			uint64_t other_scale = SparkDsv4StagePackScaleBytes(other->weight_format,other->rows,other->columns);
			/* All four span-pair classes, matching the glm52 sweep: a scale
			 * plane aliasing another tensor's payload (or another scale
			 * plane) is exactly as toxic as a payload alias. */
			if ( SparkStagePackRangesOverlap(entry->payload_offset,entry_payload,other->payload_offset,other_payload) != 0u )
				return(SPARK_STATUS_VALIDATION_FAILED);
			if ( entry_scale != 0u && other_payload != 0u && SparkStagePackRangesOverlap(entry->scale_offset,entry_scale,other->payload_offset,other_payload) != 0u )
				return(SPARK_STATUS_VALIDATION_FAILED);
			if ( other_scale != 0u && entry_payload != 0u && SparkStagePackRangesOverlap(entry->payload_offset,entry_payload,other->scale_offset,other_scale) != 0u )
				return(SPARK_STATUS_VALIDATION_FAILED);
			if ( entry_scale != 0u && other_scale != 0u && SparkStagePackRangesOverlap(entry->scale_offset,entry_scale,other->scale_offset,other_scale) != 0u )
				return(SPARK_STATUS_VALIDATION_FAILED);
		}
	}
	return(SPARK_STATUS_OK);
...
static SparkStatus SparkDsv4ModuleLoadPack(SparkDsv4ModuleState *state, const char *path)
{
	...
	if ( status == SPARK_STATUS_OK )
	{
		/* Directory bound: the entry table itself must fit between the fixed
		 * header and end of file, or tensor_count is a hostile allocation. */
		if ( header.file_bytes < SPARK_DSV4_STAGEPACK_HEADER_BYTES ||
			(uint64_t)header.tensor_count > (header.file_bytes - SPARK_DSV4_STAGEPACK_HEADER_BYTES) / SPARK_DSV4_STAGEPACK_ENTRY_BYTES )
			status = SPARK_STATUS_VALIDATION_FAILED;
	}
	minimum_offset = SPARK_DSV4_STAGEPACK_HEADER_BYTES + (uint64_t)header.tensor_count * SPARK_DSV4_STAGEPACK_ENTRY_BYTES;
	...
	for (index = 0; status == SPARK_STATUS_OK && index < header.tensor_count; index++)
		status = SparkDsv4ModuleLoadEntry(state,file,&directory[index],header.file_bytes,minimum_offset);
	if ( status == SPARK_STATUS_OK )
		status = SparkDsv4ModuleSweepDirectory(directory,header.tensor_count,minimum_offset);
	if ( status == SPARK_STATUS_OK )
...
typedef struct SparkDsv4ModuleSlotAllocation
{
	void **target;
	uint64_t bytes;
} SparkDsv4ModuleSlotAllocation;

/* One sequential device allocation per entry; a failed entry stops the
 * walk with prior allocations released by the ledger at destroy. */
static SparkStatus SparkDsv4ModuleAllocateEntries(SparkDsv4ModuleState *state,const SparkDsv4ModuleSlotAllocation *allocations,uint32_t count)
{
	uint32_t index;
	SparkStatus status = SPARK_STATUS_OK;
	for (index = 0u; status == SPARK_STATUS_OK && index < count; index++)
		status = SparkStageModuleDeviceAllocate(&state->ledger,allocations[index].bytes,allocations[index].target);
	return(status);
}
...
static SparkStatus SparkDsv4ModuleAllocateSlotSmall(SparkDsv4ModuleState *state, SparkDsv4ModuleSlot *slot)
{
	...
	if ( status == SPARK_STATUS_OK )
	{
		const SparkDsv4ModuleSlotAllocation allocations[] =
		{
			{ (void **)&slot->output_token_ids, (uint64_t)head_rows * sizeof(uint32_t) },
			{ (void **)&slot->resident_token_ids, (uint64_t)SPARK_DSV4_RESIDENT_DECODE_STAGE_MAX_INPUT_ROW_COUNT * sizeof(uint32_t) },
			{ (void **)&slot->prefill_emit_rows_u32, (uint64_t)head_rows * sizeof(uint32_t) },
			{ (void **)&slot->slot_counts, (uint64_t)rows * sizeof(uint32_t) },
			{ (void **)&slot->attention_slot_counts, (uint64_t)rows * sizeof(uint32_t) },
			{ (void **)&slot->topk_idxs, (uint64_t)rows * state->topk_column_count * sizeof(int32_t) },
			{ (void **)&slot->mixes_f32, (uint64_t)rows * SPARK_DSV4_MODEL_HC_MIX_ROWS * sizeof(float) },
			{ (void **)&slot->hc_partials_f32, (uint64_t)rows * SPARK_DSV4_HC_SPLIT_K_COUNT * SPARK_DSV4_HC_SPLIT_K_PARTIALS * sizeof(float) },
			{ (void **)&slot->pre_f32, (uint64_t)rows * SPARK_DSV4_MODEL_HC_STREAM_COUNT * sizeof(float) },
			{ (void **)&slot->post_f32, (uint64_t)rows * SPARK_DSV4_MODEL_HC_STREAM_COUNT * sizeof(float) },
			{ (void **)&slot->comb_f32, (uint64_t)rows * SPARK_DSV4_MODEL_HC_STREAM_COUNT * SPARK_DSV4_MODEL_HC_STREAM_COUNT * sizeof(float) },
			{ (void **)&slot->moe_scores_f32, (uint64_t)rows * SPARK_DSV4_MODEL_ROUTED_EXPERT_COUNT * sizeof(float) },
			{ (void **)&slot->moe_weights_f32, (uint64_t)rows * SPARK_DSV4_MODEL_EXPERTS_PER_TOKEN * sizeof(float) }
		};
		status = SparkDsv4ModuleAllocateEntries(state,allocations,
			(uint32_t)(sizeof(allocations) / sizeof(allocations[0])));
	}
	return(status);
}
...
static SparkStatus SparkDsv4ModuleAllocateSlotWide(SparkDsv4ModuleState *state, SparkDsv4ModuleSlot *slot)
{
	...
	const SparkDsv4ModuleSlotAllocation allocations[] =
	{
		{ (void **)&slot->streams_bf16, stream_bytes },
		{ (void **)&slot->residual_bf16, stream_bytes },
		{ (void **)&slot->reduced_bf16, rows * dim * bf16 },
		{ (void **)&slot->normalized_bf16, rows * dim * bf16 },
		{ (void **)&slot->qr_bf16, rows * SPARK_DSV4_MODEL_QUERY_LORA_RANK * bf16 },
		{ (void **)&slot->q_bf16, rows * query_dimension * bf16 },
		{ (void **)&slot->kv_bf16, rows * SPARK_DSV4_MODEL_ATTN_HEAD_DIMENSION * bf16 },
		{ (void **)&slot->attn_out_bf16, rows * query_dimension * bf16 },
		{ (void **)&slot->sparse_attn_partials_f32, (uint64_t)state->sparse_attn_partial_capacity * SPARK_DSV4_SPARSE_ATTN_PARTIAL_SCALARS * sizeof(float) },
		{ (void **)&slot->o_ranks_bf16, rows * SparkDsv4ModuleTpOutputLora(state) * bf16 },
		{ (void **)&slot->delta_bf16, rows * dim * bf16 }
	};
	SparkStatus status;
	status = SparkDsv4ModuleAllocateEntries(state,allocations,
		(uint32_t)(sizeof(allocations) / sizeof(allocations[0])));
...
static SparkStatus SparkDsv4ModuleAllocateSlotTail(SparkDsv4ModuleState *state, SparkDsv4ModuleSlot *slot)
{
	...
	const uint64_t per_token_u32 = rows * SPARK_DSV4_MODEL_EXPERTS_PER_TOKEN * sizeof(uint32_t);
	const uint64_t group_prefix_u32 = (SPARK_DSV4_MODEL_ROUTED_EXPERT_COUNT + 1u) * sizeof(uint32_t);
	const uint64_t slot_width_bytes = rows * SPARK_DSV4_MODEL_EXPERTS_PER_TOKEN * expert_width * bf16;
	/* head_rows * vocab-shard logits and friends are appended only when
	 * this stage participates in the final head. */
#define SPARK_DSV4_MODULE_SLOT_TAIL_ALLOCATION_COUNT 17u
#define SPARK_DSV4_MODULE_SLOT_HEAD_ALLOCATION_COUNT 6u
	SparkDsv4ModuleSlotAllocation allocations[SPARK_DSV4_MODULE_SLOT_TAIL_ALLOCATION_COUNT + SPARK_DSV4_MODULE_SLOT_HEAD_ALLOCATION_COUNT] =
	{
		{ (void **)&slot->index_q_bf16, rows * SPARK_DSV4_MODEL_INDEX_DIMENSION * bf16 },
		{ (void **)&slot->index_weights_bf16, rows * SPARK_DSV4_MODEL_INDEX_HEAD_COUNT * bf16 },
		{ (void **)&slot->index_weights_f32, rows * SPARK_DSV4_MODEL_INDEX_HEAD_COUNT * sizeof(float) },
		{ (void **)&slot->index_scores_f32, rows * (uint64_t)state->index_slot_capacity * sizeof(float) },
		{ (void **)&slot->ffn_gate_bf16, rows * expert_width * bf16 },
		{ (void **)&slot->ffn_up_bf16, rows * expert_width * bf16 },
		{ (void **)&slot->ffn_delta_bf16, rows * dim * bf16 },
		{ (void **)&slot->ffn_accum_bf16, rows * dim * bf16 },
		{ (void **)&slot->moe_indices_u32, per_token_u32 },
		{ (void **)&slot->grouped_rows_u32, per_token_u32 },
		{ (void **)&slot->group_tile_prefix_w1_u32, group_prefix_u32 },
		{ (void **)&slot->group_tile_prefix_w2_u32, group_prefix_u32 },
		{ (void **)&slot->expert_offsets_u32, group_prefix_u32 },
		{ (void **)&slot->moe_inverse_u32, per_token_u32 },
		{ (void **)&slot->moe_slot_gate_bf16, slot_width_bytes },
		{ (void **)&slot->moe_slot_up_bf16, slot_width_bytes },
		{ (void **)&slot->moe_slot_out_bf16, rows * SPARK_DSV4_MODEL_EXPERTS_PER_TOKEN * dim * bf16 }
	};
	/* Start at the TAIL count: the array reserves TAIL+HEAD slots and only
	 * the first TAIL entries are initialized here; head appends must land in
	 * the reserved range, not past the end of the array. */
	uint32_t allocation_count = SPARK_DSV4_MODULE_SLOT_TAIL_ALLOCATION_COUNT;
	SparkStatus status;
	if ( state->participates_final_head != 0u )
	{
		const SparkDsv4ModuleSlotAllocation head_allocations[] =
		{
			{ (void **)&slot->head_logits_bf16, head_rows * state->vocabulary_rows_per_rank * bf16 },
			{ (void **)&slot->head_certified_scratch,
			  SparkHeadCertifiedFp8ScratchBytes(state->vocabulary_rows_per_rank,dim) },
			{ (void **)&slot->head_candidate_ids_u32,
			  head_rows * SPARK_DSV4_RESIDENT_DECODE_STAGE_HEAD_SCREEN_CAP * sizeof(uint32_t) >
			  SparkHeadCertifiedFp8CandidateBytes(state->vocabulary_rows_per_rank) ?
			  head_rows * SPARK_DSV4_RESIDENT_DECODE_STAGE_HEAD_SCREEN_CAP * sizeof(uint32_t) :
			  SparkHeadCertifiedFp8CandidateBytes(state->vocabulary_rows_per_rank) },
			{ (void **)&slot->head_candidate_counts_u32, head_rows * sizeof(uint32_t) },
			{ (void **)&slot->head_scores_f32, head_rows * sizeof(float) },
			{ (void **)&slot->head_maxloc_u64, head_rows * sizeof(uint64_t) }
		};
		for (uint32_t i = 0u; i < (uint32_t)(sizeof(head_allocations) / sizeof(head_allocations[0])); i++)
			allocations[allocation_count++] = head_allocations[i];
	}
	status = SparkDsv4ModuleAllocateEntries(state,allocations,allocation_count);
	return(status);
}
...
static SparkStatus SparkDsv4ModuleAllocateDspark(SparkDsv4ModuleState *state, SparkDsv4ModuleSlot *slot)
{
	...
	const uint64_t tap_bytes = SPARK_DSV4_MODEL_DSPARK_TARGET_LAYER_COUNT * dim * bf16;
	const SparkDsv4ModuleSlotAllocation allocations[] =
	{
		{ (void **)&slot->dspark_draft_token_ids, block * sizeof(uint32_t) },
		{ (void **)&slot->dspark_verify_token_ids, block * sizeof(uint32_t) },
		{ (void **)&slot->dspark_input_token_ids, block * sizeof(uint32_t) },
		{ (void **)&slot->dspark_row_positions, block * sizeof(uint64_t) },
		{ (void **)&slot->dspark_row_lane_indices, block * sizeof(uint32_t) },
		{ (void **)&slot->dspark_scores_f32, block * sizeof(float) },
		{ (void **)&slot->dspark_x_bf16, block * SPARK_DSV4_MODEL_HC_STREAM_COUNT * dim * bf16 },
		{ (void **)&slot->dspark_q_attn_bf16, block * qdim * bf16 },
		{ (void **)&slot->dspark_o_ranks_bf16, block * SPARK_DSV4_MODEL_OUTPUT_LORA_RANK * SPARK_DSV4_MODEL_OUTPUT_GROUP_COUNT * bf16 },
		{ (void **)&slot->dspark_main_cat_bf16, tap_bytes },
		{ (void **)&slot->dspark_main_x_bf16, dim * bf16 },
		{ (void **)&slot->dspark_tap_bf16, tap_bytes },
		{ (void **)&slot->dspark_tap_ring_bf16, block * tap_bytes },
		{ (void **)&slot->dspark_verify_tap_bf16, (SPARK_DSV4_MODEL_DSPARK_SPEC_STEP + 1u) * tap_bytes },
		{ (void **)&slot->dspark_logits_bf16, block * vocab * bf16 },
		{ (void **)&slot->dspark_logits_f32, block * vocab * sizeof(float) }
	};
	return(SparkDsv4ModuleAllocateEntries(state,allocations,
		(uint32_t)(sizeof(allocations) / sizeof(allocations[0]))));
	SparkStatus status;
	...
	if ( status == SPARK_STATUS_OK )
		...
		if ( status == SPARK_STATUS_OK )
			status = SparkStageModuleDeviceAllocate(&state->ledger,sizeof(uint32_t),(void **)&slot->dspark_state_row_limit);
	if ( status == SPARK_STATUS_OK )
	...
	if ( status == SPARK_STATUS_OK )
	{
		/* Verify recurrence isolation buffers: one packed per-lane mirror
		 * of every non-SWA layer's compressor (and, for CSA, indexer) state
		 * spans, plus per-layer images of the ring/entry slots the staged
		 * burst rows overwrite. Sizes derive from the same pool-layout
		 * arithmetic the paged cache was built with. */
		const SparkDsv4PagedPoolLayout *layout = &state->paged_cache.layout;
		uint64_t state_bytes = 0u,layer_stride;
		uint32_t layer;
		for (layer = layout->first_layer_index;
			layer < layout->first_layer_index + layout->layer_count; layer++)
		{
			uint32_t kind = SparkDsv4ModelLayerKind(layer);
			if ( kind == SPARK_DSV4_MODEL_LAYER_KIND_SWA )
				continue;
			state_bytes += 2u * SparkDsv4PoolCompressStateLaneElements(kind) *
				sizeof(float);
			if ( kind == SPARK_DSV4_MODEL_LAYER_KIND_CSA )
				state_bytes += 2u * SparkDsv4PoolIndexStateLaneElements() *
					sizeof(float);
		}
		slot->dspark_state_snapshot_bytes = state_bytes;
		status = SparkStageModuleDeviceAllocate(&state->ledger,state_bytes,
			(void **)&slot->dspark_state_snapshot_b);
		/* Per-layer window image, addressed by global layer index and by
		 * cache slot id inside three fixed sub-regions: [ring][compressed
		 * entries][index entries], each spanning the full 128-slot id space
		 * so capture and restore address bytes identically. */
		layer_stride = (uint64_t)SPARK_DSV4_MODEL_SLIDING_WINDOW_TOKENS *
			(2u * SPARK_DSV4_MODEL_ATTN_HEAD_DIMENSION +
			SPARK_DSV4_MODEL_INDEX_HEAD_DIMENSION) *
			SPARK_DSV4_MODEL_BF16_ELEMENT_BYTES;
		slot->dspark_window_layer_stride = layer_stride;
		if ( status == SPARK_STATUS_OK )
			status = SparkStageModuleDeviceAllocate(&state->ledger,
				layout->layer_count * layer_stride,
				(void **)&slot->dspark_window_snapshot_b);
	}
	return(status);
}
...
/* Single source of the TP bucket-width shape contract, shared verbatim by
 * frame validation and admission so the two can never drift again (the
 * pre-fix duplicated copies had already diverged on the spec-step arm). */
static uint32_t SparkDsv4ModuleBucketShapeAccepted(
	const SparkDsv4ModuleState *state,
	...
	uint32_t new_token_count)
{
	if ( state->tp_degree <= 1u )
		return(1u);
	if ( active_slot_count == SPARK_BATCH_BUCKET &&
		new_token_count == SPARK_BATCH_BUCKET )
		return(1u);
	if ( SPARK_BATCH_BUCKET == SPARK_DSV4_MODEL_DSPARK_SPEC_STEP + 1u &&
		state->dspark_enabled != 0u &&
		active_slot_count == 1u && new_token_count == 1u )
		return(1u);
	return(0u);
}

/* Frames that advance sequence state stage KV pages for every live lane;
 * a short or empty cache-lane vector would read stale host page-table rows
 * left behind by an earlier frame on this slot. Release frames
 * (new_token_count == 0) never stage pages and are exempt. */
static uint32_t SparkDsv4ModuleCacheLanesCoverFrame(
	uint32_t active_slot_count,
	uint32_t new_token_count,
	uint32_t cache_lane_count)
{
	if ( new_token_count == 0u )
		return(1u);
	return(cache_lane_count == active_slot_count ? 1u : 0u);
}

/* Base graph-island index of a stage-local layer: three islands per layer
 * (attn projection, FFN projection, MoE) plus the head island at the end. */
static uint32_t SparkDsv4ModuleLayerIslandBase(
	const SparkDsv4ModuleState *state,
	uint32_t layer_index)
{
	return(3u * (layer_index - state->first_layer_index));
}
	...
	uint32_t *is_prefill_out)
{
	...
	if ( (is_prefill != 0u && frame->tokens_per_sequence != 1u) ||
		...
		return(SPARK_STATUS_INVALID_ARGUMENT);
	if ( SparkDsv4ModuleBucketShapeAccepted(state,
			frame->active_slot_count,frame->new_token_count) == 0u ||
		SparkDsv4ModuleCacheLanesCoverFrame(frame->active_slot_count,
			frame->new_token_count,frame->cache_lane_count) == 0u )
		return(SPARK_STATUS_INVALID_ARGUMENT);
...
/* The six pinned staging writes shared by every staging path; the emit
 * position clamps mirror the CSA/HCA compressor window arithmetic and are
 * kept in exactly one place so the two staging loops cannot diverge. */
static void SparkDsv4ModuleStageRowSlotWrites(
	SparkDsv4ModuleSlot *slot,
	uint32_t row,
	uint32_t token_id,
	...
	uint64_t position)
{
	slot->host_input_token_ids[row] = token_id;
	...
	slot->host_row_emit_positions[row] = position + 1u >= SPARK_DSV4_MODEL_CSA_COMPRESS_RATIO ? position + 1u - SPARK_DSV4_MODEL_CSA_COMPRESS_RATIO : 0u;
	slot->host_row_emit_positions_hca[row] = position + 1u >= SPARK_DSV4_MODEL_HCA_COMPRESS_RATIO ? position + 1u - SPARK_DSV4_MODEL_HCA_COMPRESS_RATIO : 0u;
}
	...
	uint32_t row_count)
{
	...
	for (row = 0; row < row_count; row++)
	{
		...
		SparkDsv4ModuleStageRowSlotWrites(slot,row,token_ids[row],
			physical_page,lane,row_positions[row]);
	}
	return(SPARK_STATUS_OK);
}
...
/* ==== DSpark verify isolation: pre-frame snapshots + canonical replay ====
 * A verify/pad frame deliberately walks every staged row through the live
 * compressor, indexer and raw-KV ring so mid-burst rows see the progressive
 * state single-row decode would have at their positions. Rows that fail
 * acceptance must leave NONE of those writes behind: the completion side
 * restores these pre-frame images and re-applies only the accepted prefix
 * through rows=1 calls - the exact kernel shapes single-row decode uses -
 * which is bit-identical to never having staged rejected rows at all.
 * Image layout (per global layer index): [128 ring slots][128 compressed
 * entries][128 index entries], each slot one head-dimension bf16 vector. */

static cudaError_t SparkDsv4ModuleStageVerifyStateSnapshot(
	SparkDsv4ModuleState *state,SparkDsv4ModuleSlot *slot)
{
	const SparkDsv4PagedPoolLayout *layout = &state->paged_cache.layout;
	cudaStream_t stream = (cudaStream_t)slot->cuda_stream;
	uint64_t lane_stride = state->index_state_lane_stride;
	uint64_t offset = 0u;
	uint32_t layer,lane;
	cudaError_t error = cudaSuccess;
	if ( slot->dspark_state_snapshot_b == 0 ||
		slot->dspark_state_snapshot_bytes == 0u )
		return(cudaErrorInvalidValue);
	lane = slot->host_row_lane_indices[0];
	slot->dspark_state_snapshot_page = lane;
	for (layer = layout->first_layer_index; error == cudaSuccess &&
		layer < layout->first_layer_index + layout->layer_count; layer++)
	{
		uint32_t kind = SparkDsv4ModelLayerKind(layer);
		uint64_t elements;
		if ( kind == SPARK_DSV4_MODEL_LAYER_KIND_SWA )
			continue;
		elements = SparkDsv4PoolCompressStateLaneElements(kind);
		error = cudaMemcpyAsync(slot->dspark_state_snapshot_b + offset,
			state->compress_kv_state_f32 +
				state->compress_state_offset_by_layer[layer] +
				(uint64_t)lane * lane_stride,elements * sizeof(float),
		...
		if ( error == cudaSuccess )
			error = cudaMemcpyAsync(slot->dspark_state_snapshot_b + offset,
				...
				cudaMemcpyDeviceToDevice,stream);
		offset += elements * sizeof(float);
		if ( error == cudaSuccess && kind ==
			SPARK_DSV4_MODEL_LAYER_KIND_CSA )
		{
			elements = SparkDsv4PoolIndexStateLaneElements();
			error = cudaMemcpyAsync(slot->dspark_state_snapshot_b + offset,
				state->index_kv_state_f32 +
					state->index_kv_state_offset_by_layer[layer] +
					(uint64_t)lane * lane_stride,elements * sizeof(float),
				cudaMemcpyDeviceToDevice,stream);
			offset += elements * sizeof(float);
			if ( error == cudaSuccess )
				error = cudaMemcpyAsync(
					slot->dspark_state_snapshot_b + offset,
					state->index_score_state_f32 +
						state->index_score_state_offset_by_layer[layer] +
						(uint64_t)lane * lane_stride,elements * sizeof(float),
					cudaMemcpyDeviceToDevice,stream);
			offset += elements * sizeof(float);
	...
	slot->dspark_state_snapshot_valid = error == cudaSuccess ? 1u : 0u;
	return(error);
}

static cudaError_t SparkDsv4ModuleRestoreVerifyStateSnapshot(
	SparkDsv4ModuleState *state,SparkDsv4ModuleSlot *slot)
{
	const SparkDsv4PagedPoolLayout *layout = &state->paged_cache.layout;
	cudaStream_t stream = (cudaStream_t)slot->cuda_stream;
	...
	uint32_t layer,lane;
	cudaError_t error = cudaSuccess;
	if ( slot->dspark_state_snapshot_valid == 0u )
		return(cudaErrorInvalidValue);
	lane = slot->dspark_state_snapshot_page;
	for (layer = layout->first_layer_index; error == cudaSuccess &&
		layer < layout->first_layer_index + layout->layer_count; layer++)
	{
		uint32_t kind = SparkDsv4ModelLayerKind(layer);
		...
		if ( kind == SPARK_DSV4_MODEL_LAYER_KIND_SWA )
			continue;
		elements = SparkDsv4PoolCompressStateLaneElements(kind);
		error = cudaMemcpyAsync(state->compress_kv_state_f32 +
			state->compress_state_offset_by_layer[layer] +
			(uint64_t)lane * lane_stride,
			slot->dspark_state_snapshot_b + offset,elements * sizeof(float),
			cudaMemcpyDeviceToDevice,stream);
		offset += elements * sizeof(float);
		if ( error == cudaSuccess )
			error = cudaMemcpyAsync(state->compress_score_state_f32 +
				state->compress_score_state_offset_by_layer[layer] +
				(uint64_t)lane * lane_stride,
				slot->dspark_state_snapshot_b + offset,
				elements * sizeof(float),cudaMemcpyDeviceToDevice,stream);
		offset += elements * sizeof(float);
		if ( error == cudaSuccess && kind ==
			SPARK_DSV4_MODEL_LAYER_KIND_CSA )
		{
			elements = SparkDsv4PoolIndexStateLaneElements();
			error = cudaMemcpyAsync(state->index_kv_state_f32 +
				state->index_kv_state_offset_by_layer[layer] +
				(uint64_t)lane * lane_stride,
				slot->dspark_state_snapshot_b + offset,
				elements * sizeof(float),cudaMemcpyDeviceToDevice,stream);
			offset += elements * sizeof(float);
			if ( error == cudaSuccess )
				error = cudaMemcpyAsync(state->index_score_state_f32 +
					state->index_score_state_offset_by_layer[layer] +
					(uint64_t)lane * lane_stride,
					slot->dspark_state_snapshot_b + offset,
					elements * sizeof(float),cudaMemcpyDeviceToDevice,stream);
	...
	slot->dspark_state_snapshot_valid = 0u;
	return(error);
}

static cudaError_t SparkDsv4ModuleStageVerifyWindowSnapshot(
	SparkDsv4ModuleState *state,SparkDsv4ModuleSlot *slot)
{
	const SparkDsv4PagedPoolLayout *layout = &state->paged_cache.layout;
	cudaStream_t stream = (cudaStream_t)slot->cuda_stream;
	uint64_t layer_stride = slot->dspark_window_layer_stride;
	uint64_t entry_bytes = (uint64_t)SPARK_DSV4_MODEL_ATTN_HEAD_DIMENSION *
		SPARK_DSV4_MODEL_BF16_ELEMENT_BYTES;
	uint64_t index_entry_bytes = (uint64_t)
		SPARK_DSV4_MODEL_INDEX_HEAD_DIMENSION *
		SPARK_DSV4_MODEL_BF16_ELEMENT_BYTES;
	uint64_t window = SPARK_DSV4_MODEL_SLIDING_WINDOW_TOKENS;
	uint32_t layer,j;
	cudaError_t error = cudaSuccess;
	if ( slot->dspark_window_snapshot_b == 0 || window == 0u )
		return(cudaErrorInvalidValue);
	slot->dspark_window_base_slot =
		(uint32_t)(slot->host_row_positions[0] % window);
	for (j = 0u; j <= SPARK_DSV4_MODEL_DSPARK_SPEC_STEP; j++)
		slot->dspark_window_positions[j] = slot->host_row_positions[j];
	slot->dspark_window_valid = 1u;
	for (layer = layout->first_layer_index; error == cudaSuccess &&
		layer < layout->first_layer_index + layout->layer_count; layer++)
	{
		uint32_t kind = SparkDsv4ModelLayerKind(layer);
		uint8_t *image = slot->dspark_window_snapshot_b +
			(uint64_t)layer * layer_stride;
		const uint8_t *cache_region =
			(const uint8_t *)state->kv_cache_bf16 +
			state->cache_offset_by_layer[layer] *
			SPARK_DSV4_MODEL_BF16_ELEMENT_BYTES;
		const uint8_t *index_region =
			(const uint8_t *)state->index_cache_bf16 +
			state->index_cache_offset_by_layer[layer] *
			SPARK_DSV4_MODEL_BF16_ELEMENT_BYTES;
		if ( kind != SPARK_DSV4_MODEL_LAYER_KIND_CSA &&
			kind != SPARK_DSV4_MODEL_LAYER_KIND_HCA )
			continue;
		for (j = 0u; error == cudaSuccess &&
			j <= SPARK_DSV4_MODEL_DSPARK_SPEC_STEP; j++)
		{
			uint64_t position = slot->dspark_window_positions[j];
			uint32_t slot_id = (uint32_t)(position % window);
			uint32_t earlier;
			for (earlier = 0u; earlier < j; earlier++)
				if ( slot->dspark_window_positions[earlier] == position )
					break;
			if ( earlier != j )
				continue;
			error = cudaMemcpyAsync(image + (uint64_t)slot_id *
				entry_bytes,cache_region + (uint64_t)slot_id *
				entry_bytes,entry_bytes,cudaMemcpyDeviceToDevice,stream);
		}
		for (j = 0u; error == cudaSuccess &&
			j <= SPARK_DSV4_MODEL_DSPARK_SPEC_STEP; j++)
		{
			uint64_t position = slot->dspark_window_positions[j];
			uint64_t ratio = kind == SPARK_DSV4_MODEL_LAYER_KIND_CSA ?
				SPARK_DSV4_MODEL_CSA_COMPRESS_RATIO :
				SPARK_DSV4_MODEL_HCA_COMPRESS_RATIO;
			uint32_t entry_slot = (uint32_t)(window +
				((position % window) / ratio));
			uint32_t earlier;
			for (earlier = 0u; earlier < j; earlier++)
			{
				uint64_t earlier_position =
					slot->dspark_window_positions[earlier];
				uint64_t earlier_ratio = kind ==
					SPARK_DSV4_MODEL_LAYER_KIND_CSA ?
					SPARK_DSV4_MODEL_CSA_COMPRESS_RATIO :
					SPARK_DSV4_MODEL_HCA_COMPRESS_RATIO;
				if ((uint32_t)(window + ((earlier_position % window) /
					earlier_ratio)) == entry_slot)
					break;
			}
			if ( earlier != j )
				continue;
			error = cudaMemcpyAsync(image + window * entry_bytes +
				(uint64_t)entry_slot * entry_bytes,cache_region +
				(uint64_t)entry_slot * entry_bytes,entry_bytes,
				cudaMemcpyDeviceToDevice,stream);
			if ( error == cudaSuccess && kind ==
				SPARK_DSV4_MODEL_LAYER_KIND_CSA )
				error = cudaMemcpyAsync(image + 2u * window * entry_bytes +
					(uint64_t)(entry_slot - (uint32_t)window) *
					index_entry_bytes,index_region +
					(uint64_t)(entry_slot - (uint32_t)window) *
...
static cudaError_t SparkDsv4ModuleRestoreVerifyWindow(
	SparkDsv4ModuleState *state,SparkDsv4ModuleSlot *slot,
	uint32_t commit_last_row)
{
	const SparkDsv4PagedPoolLayout *layout = &state->paged_cache.layout;
	cudaStream_t stream = (cudaStream_t)slot->cuda_stream;
	uint64_t layer_stride = slot->dspark_window_layer_stride;
	...
	uint32_t layer,j;
	cudaError_t error = cudaSuccess;
	if ( slot->dspark_window_valid == 0u )
		return(cudaErrorInvalidValue);
	commit_position = slot->dspark_window_positions[commit_last_row];
	for (layer = layout->first_layer_index; error == cudaSuccess &&
		layer < layout->first_layer_index + layout->layer_count; layer++)
	{
		uint32_t kind = SparkDsv4ModelLayerKind(layer);
		uint8_t *image = slot->dspark_window_snapshot_b +
			(uint64_t)layer * layer_stride;
		uint8_t *cache_region = (uint8_t *)state->kv_cache_bf16 +
			state->cache_offset_by_layer[layer] *
			SPARK_DSV4_MODEL_BF16_ELEMENT_BYTES;
		uint8_t *index_region = (uint8_t *)state->index_cache_bf16 +
			state->index_cache_offset_by_layer[layer] *
			SPARK_DSV4_MODEL_BF16_ELEMENT_BYTES;
		if ( kind != SPARK_DSV4_MODEL_LAYER_KIND_CSA &&
			kind != SPARK_DSV4_MODEL_LAYER_KIND_HCA )
			continue;
		/* Raw-ring suffix only: accepted rows' writes ARE cell A's writes
		 * (the forward pass cannot be recomputed here), so keep them and
		 * revert strictly-future duplicate-free positions past the commit
		 * boundary. Duplicate-pad rows share row 0's slot and never
		 * qualify. */
		for (j = 0u; error == cudaSuccess &&
			j <= SPARK_DSV4_MODEL_DSPARK_SPEC_STEP; j++)
		{
			...
			{
				for (kept = 0u; kept <= commit_last_row; kept++)
					if ( slot->dspark_window_positions[kept] == position )
						break;
				if ( kept > commit_last_row )
					error = cudaMemcpyAsync(cache_region +
						(uint64_t)slot_id * entry_bytes,image +
						(uint64_t)slot_id * entry_bytes,entry_bytes,
						cudaMemcpyDeviceToDevice,stream);
			}
		}
		/* Compressed entries + index entries: restore every captured slot
		 * (superset is safe); the canonical replay below rewrites the
		 * accepted completions bit-exactly right after this. */
		for (j = 0u; error == cudaSuccess &&
			j <= SPARK_DSV4_MODEL_DSPARK_SPEC_STEP; j++)
		{
			uint64_t position = slot->dspark_window_positions[j];
			...
			if ( earlier != j )
				continue;
			error = cudaMemcpyAsync(cache_region +
				(uint64_t)entry_slot * entry_bytes,image + window *
				entry_bytes + (uint64_t)entry_slot * entry_bytes,
				entry_bytes,cudaMemcpyDeviceToDevice,stream);
			if ( error == cudaSuccess && kind ==
				SPARK_DSV4_MODEL_LAYER_KIND_CSA )
				error = cudaMemcpyAsync(index_region +
					(uint64_t)(entry_slot - (uint32_t)window) *
					index_entry_bytes,image + 2u * window * entry_bytes +
					(uint64_t)(entry_slot - (uint32_t)window) *
					index_entry_bytes,index_entry_bytes,
					cudaMemcpyDeviceToDevice,stream);
		}
	}
	slot->dspark_window_valid = 0u;
	return(error);
}
	...
	SparkDsv4ModuleSlot *slot,uint32_t rows)
{
	...
	uint64_t pitch64 = (uint64_t)SPARK_DSV4_RESIDENT_DECODE_STAGE_MAX_INPUT_ROW_COUNT * sizeof(uint64_t);
	cudaError_t error;
	/* Verify/pad frames must start with clean emission flags: rows a
	 * compress-step block skips (duplicate-lane rows) would otherwise read
	 * the previous frame's boundary verdicts and scatter garbage compressed
	 * entries for them. Fixed address + fixed width, so recording this in
	 * the captured decode graph is replay-safe. */
	error = cudaMemsetAsync(slot->compressor.emitted_u32,0,
		(uint64_t)rows * sizeof(uint32_t),stream);
	if ( error == cudaSuccess )
		error = cudaMemsetAsync(slot->index_compressor.emitted_u32,0,
			(uint64_t)rows * sizeof(uint32_t),stream);
	if ( error == cudaSuccess )
	...
	uint64_t anchor_position)
{
	...
	for (row = 0u; row < rows; row++)
	{
		...
		if ( physical_page >= state->physical_page_capacity )
			return(SPARK_STATUS_BUSY);
		SparkDsv4ModuleStageRowSlotWrites(slot,row,host_tokens[row],
			physical_page,lane_index,host_positions[row]);
	}
	...
	if ( status == SPARK_STATUS_OK )
		status = SparkStageModuleCudaStatus(SPARK_DSV4_MODULE_TAG,
			SparkDsv4ModuleStageVerifyStateSnapshot(state,slot),
			"dspark_state_snapshot");
	if ( status == SPARK_STATUS_OK )
		status = SparkStageModuleCudaStatus(SPARK_DSV4_MODULE_TAG,
			SparkDsv4ModuleStageVerifyWindowSnapshot(state,slot),
			"dspark_window_snapshot");
	if ( status == SPARK_STATUS_OK )
	{
...
static SparkStatus SparkDsv4ModuleStageFrameRows(SparkDsv4ModuleState *state, SparkDsv4ModuleSlot *slot, const SparkModelDriverFrame *frame, const SparkDsv4ResidentDecodeStageFrameContext *context)
{
		...
		if ( error == cudaSuccess && lane_index < state->resident_sequence_capacity )
		{
			...
			if ( prefill != 0u || slot->dspark_verify_rows == 0u )
			{
				...
				if ( stage_status == SPARK_STATUS_OK )
					stage_status = SparkDsv4ModulePadDuplicateRows(state,
						slot,SPARK_BATCH_BUCKET);
				if ( stage_status == SPARK_STATUS_OK )
					return(SPARK_STATUS_OK);
	...
	if ( SPARK_BATCH_BUCKET == SPARK_DSV4_MODEL_DSPARK_SPEC_STEP + 1u &&
		state->dspark_enabled == 0u && prefill == 0u && row_count == 1u )
	{
		/* Kill-switch control path (SPARK_DSV4_DSPARK_ENABLED=0): stage
		 * the one real row and pad duplicates to bucket width so the
		 * islands keep their recorded shapes. This is the same-build
		 * no-spec channel the losslessness gate compares against; the
		 * duplicate rows are idempotent and the completion's row-0-only
		 * commit keeps the recurrence exactly on the single-row path. */
		SparkStatus stage_status = SparkDsv4ModuleStageRows(state,slot,
			token_ids,row_lane_indices,row_positions,1u,
			frame->active_slot_count);
		if ( stage_status == SPARK_STATUS_OK )
			stage_status = SparkDsv4ModulePadDuplicateRows(state,slot,
				SPARK_BATCH_BUCKET);
		if ( stage_status == SPARK_STATUS_OK )
			return(SPARK_STATUS_OK);
		fprintf(stderr,"dspark_nospec_pad_failed status=%u tp_rank=%u\n",(uint32_t)stage_status,state->tp_rank);
	}
...
static cudaError_t SparkDsv4ModuleRunCausalAttention(SparkDsv4ModuleState *state, SparkDsv4ModuleSlot *slot, const SparkDsv4PrefillBatchView *prefill, void *cache, uint64_t lane_stride, const float *sink_f32, uint32_t layer_kind, uint32_t rows)
{
	...
	if ( prefill == 0 )
	{
		/* Verify/pad burst rows must not poison each other's window: the
		 * batched scatter writes ALL staged rows before any attention runs,
		 * so row r would read draft rows > r in ring slots that single-row
		 * decode still holds history in (slot = position % window). Walk
		 * the burst oldest-first, scattering+attending one row at a time:
		 * row r then sees exactly what a rows=1 frame would have at its
		 * position - untouched history slots (future rows' slots still
		 * hold position-128 entries, which ARE in r's window) plus its own
		 * and earlier rows' entries. Same kernels at rows=1 shapes as the
		 * no-spec path; fixed trip count and stable buffers keep it valid
		 * inside recorded/replayed decode graphs. */
		if ( slot->dspark_verify_rows != 0u && rows > 1u )
		{
			error = cudaSuccess;
			for (row=0u; error==cudaSuccess && row<rows; row++)
				error = SparkDsv4ModuleRunAttentionRows(state,slot,cache,
					lane_stride,sink_f32,layer_kind,row,1u);
			return(error);
		}
	...
	SparkDsv4TpFrameContinuation *continuation)
{
	...
	if ( state->tp_degree > 1u )
	{
		status = SparkDsv4ModuleReplayTpIsland(state,slot,
			SparkDsv4ModuleLayerIslandBase(state,continuation->layer_index),
			continuation->rows);
		if ( status != SPARK_STATUS_OK )
			return(status);
...
static void SparkDsv4ModuleContinueHeadMax(void *context,SparkStatus status)
{
		...
		state->dspark_enabled != 0u )
	{
		...
		if ( continuation->dspark_verify != 0u )
		{
			...
			/* Verify isolation commit: the frame walked the committed
			 * compressor/indexer states and the raw-KV ring through ALL
			 * staged rows (provisional, unaccepted drafts included) so the
			 * mid-burst rows would see single-row-decode state at their
			 * positions. Roll that back and re-apply ONLY the canonical
			 * prefix: window suffix + entry slots from the pre-frame
			 * images, state spans from the pre-frame image, then rows=1
			 * compressor/indexer replay for rows 0..commit_last - the
			 * exact kernel shapes single-row decode uses, so the next
			 * frame's boundary pool reads bit-identical state. Padded
			 * fallback frames commit row 0 only (their duplicate rows
			 * never legitimately advanced anything). */
			{
				uint32_t commit_rows = slot->dspark_verify_accept != 0u ?
					accepted + 1u : 1u;
				uint32_t row;
				error = SparkDsv4ModuleRestoreVerifyWindow(state,slot,
					commit_rows - 1u);
				if ( error == cudaSuccess )
					error = SparkDsv4ModuleRestoreVerifyStateSnapshot(
						state,slot);
				for (layer = state->first_layer_index;
					...
					state->layer_count && error == cudaSuccess; layer++)
				{
					...
					if ( kind == SPARK_DSV4_MODEL_LAYER_KIND_SWA )
						continue;
					for (row = 0u; row < commit_rows &&
						error == cudaSuccess; row++)
					{
						...
						if ( kind != SPARK_DSV4_MODEL_LAYER_KIND_CSA )
							continue;
						error = SparkDsv4ModuleRunCompressorPost(
							state,slot,&slot->index_compressor,
							(cudaStream_t)slot->cuda_stream,
							&state->layers[layer].indexer.compressor,
							state->index_kv_state_f32 +
							state->index_kv_state_offset_by_layer[layer],
							state->index_score_state_f32 +
							state->index_score_state_offset_by_layer[layer],
							state->index_state_lane_stride,
							(uint8_t *)state->index_cache_bf16 +
							state->index_cache_offset_by_layer[layer] *
							SPARK_DSV4_MODEL_BF16_ELEMENT_BYTES,
							state->index_lane_stride,0u,
							SPARK_DSV4_MODEL_INDEX_HEAD_DIMENSION,1u,
							row,1u);
					}
				}
			}
...
static void SparkDsv4ModuleContinueLayers(void *context,SparkStatus status)
{
	...
	if ( continuation->side == 0u )
	{
		status = state->tp_degree > 1u ?
			SparkDsv4ModuleReplayTpIsland(state,slot,
				SparkDsv4ModuleLayerIslandBase(state,continuation->layer_index) + 1u,
	...
	else if ( continuation->side == 1u )
	{
		if ( state->tp_degree > 1u )
			status = SparkDsv4ModuleReplayTpIsland(state,slot,
				SparkDsv4ModuleLayerIslandBase(state,continuation->layer_index) + 2u,
				continuation->rows);
		else
		{
			error = SparkDsv4LaunchHcPost(stream,slot->delta_bf16,
...
/* DSpark draft selection is a TP-visible result: the Flash lm_head is
 * vocabulary-sharded, so each rank's local argmax names a DIFFERENT token
 * and the stages would stage different verify bursts (measured iter-12:
 * four disjoint per-rank frozen draft vectors). Reduce the markov chain's
 * (ordered score, global token id) pair across the TP group with the same
 * U64 maxloc collective the final head uses, so every rank stages the SAME
 * burst. One element per call: the chain is sequential, each winner feeds
 * the next markov step. Synchronous bounded spin-wait: the drive runs on
 * the submission path where host syncs are already legal. */
typedef struct SparkDsv4DsparkReduceWait
{
	volatile uint32_t done;
	volatile uint32_t status;
} SparkDsv4DsparkReduceWait;

static void SparkDsv4ModuleDsparkReduceDone(void *context,const SparkTpDeviceCollectiveCompletion *completion)
{
	SparkDsv4DsparkReduceWait *wait = (SparkDsv4DsparkReduceWait *)context;
	wait->status = (uint32_t)completion->status;
	wait->done = 1u;
}

static cudaError_t SparkDsv4ModuleDsparkReduceDrafts(
	SparkDsv4ModuleState *state,SparkDsv4ModuleSlot *slot,uint32_t index)
{
	SparkTpDeviceCollectiveSubmission submission;
	SparkDsv4DsparkReduceWait wait = {0u,(uint32_t)SPARK_STATUS_OK};
	uint32_t spin = 0u;
	cudaError_t error;
	SparkStatus status;
	error = SparkDsv4LaunchHeadMaxlocPack((cudaStream_t)slot->cuda_stream,
		slot->dspark_scores_f32 + index,slot->dspark_draft_token_ids + index,
		slot->head_maxloc_u64,1u);
	if ( error != cudaSuccess )
		return(error);
	memset(&submission,0,sizeof(submission));
	submission.abi_version = SPARK_TP_DEVICE_COLLECTIVE_ABI_VERSION;
	...
	submission.ordinal = atomic_fetch_add_explicit(&state->tp_next_ordinal,
		1u,memory_order_relaxed);
	submission.local_device = slot->head_maxloc_u64;
	submission.full_device = slot->head_maxloc_u64;
	submission.cuda_stream = slot->cuda_stream;
	submission.completion_function = SparkDsv4ModuleDsparkReduceDone;
	submission.completion_context = &wait;
	status = SparkTpDeviceCollectiveSubmitU64Max(
		&state->tp_device_collective,&submission);
	if ( status != SPARK_STATUS_OK )
	{
		fprintf(stderr,"dspark_draft_reduce_failed status=%u tp_rank=%u\n",
			(uint32_t)status,state->tp_rank);
		return(cudaSuccess);
	}
	while ( wait.done == 0u && spin < 4000000u )
	{
		spin += 1u;
		sched_yield();
	}
	if ( wait.done == 0u || (SparkStatus)wait.status != SPARK_STATUS_OK )
		fprintf(stderr,"dspark_draft_reduce_wait done=%u status=%u tp_rank=%u\n",
			wait.done,wait.status,state->tp_rank);
	return(SparkDsv4LaunchHeadMaxlocUnpack((cudaStream_t)slot->cuda_stream,
		slot->head_maxloc_u64,slot->dspark_draft_token_ids + index,1u));
}
	...
	uint64_t anchor_position)
{
	...
	/* The markov chain submits back-to-back on the SAME stream as the
	 * draft forward: step i's argmax writes draft_token_ids[i+1] and step
	 * i+1's gather reads it straight from device memory. One anchor upload
	 * before the loop and ONE completion fence after it replace the former
	 * per-step H2D/D2H/sync round trips (identical kernels, identical
	 * values - only the host round trip is gone). The final sample has no
	 * successor, so it lands in the input_token_ids scratch cell instead
	 * of one past the ids array. */
	error = cudaMemcpyAsync(slot->dspark_draft_token_ids,&anchor_token_id,
		sizeof(uint32_t),cudaMemcpyHostToDevice,stream);
	for (index = 0u; error == cudaSuccess && index < block; index++)
	{
		uint32_t *next_ids = index + 1u < block ?
			slot->dspark_draft_token_ids + index + 1u :
			slot->input_token_ids;
		error = SparkDsv4LaunchEmbeddingGather(stream,
		...
		if ( error == cudaSuccess )
			error = SparkDsv4LaunchDsparkArgmax(stream,
				...
				rows_per_rank,state->vocabulary_row_start,next_ids,
				slot->dspark_scores_f32 + index);
		if ( error == cudaSuccess && state->tp_degree > 1u )
			error = SparkDsv4ModuleDsparkReduceDrafts(state,slot,index);
	}
	if ( error == cudaSuccess )
		error = cudaStreamSynchronize(stream);
	if ( error != cudaSuccess )
	...
	const SparkDsv4ResidentDecodeStageFrameContext *context)
{
	...
	/* Committed-recurrence gate: a DSpark verify/pad burst advances the
	 * CSA/HCA compressor state with the ANCHOR row only; accepted drafts
	 * are re-applied by the commit replay after the head returns. The
	 * scalar lives in device memory so graph replays see it without a
	 * host-side branch. Plain multi-lane waves keep the full row count.
	 * Written stream-ordered before any island replay; cudaMemsetAsync is
	 * capture-legal but this site runs outside capture anyway. */
	{
		uint32_t state_rows = slot->dspark_verify_rows != 0u ? 1u :
			(prefill != 0 ? prefill->row_count : context->decode_batch->row_count);
		cudaError_t limit_error = cudaMemcpyAsync(slot->dspark_state_row_limit,
			&state_rows,sizeof(uint32_t),cudaMemcpyHostToDevice,
			(cudaStream_t)slot->cuda_stream);
		if ( limit_error != cudaSuccess )
			return(SparkStageModuleCudaStatus(SPARK_DSV4_MODULE_TAG,
				limit_error,"dspark_state_row_limit"));
	}
	...
	if ( prefill != 0 && state->tp_degree > 1u )
	{
		if ( rows != SPARK_BATCH_BUCKET )
			return(SPARK_STATUS_UNSUPPORTED);
		/* A full-bucket prefill replays through the decode islands on TP,
		 * which cannot serve the per-row emission head; frames that actually
		 * emit are refused LOUDLY here instead of silently running as decode
		 * and losing their projected tokens. Non-emitting prefills (middle
		 * PP stages, no OUTPUT_TOKEN lanes) keep the historical path. */
		if ( context->prefill_batch != 0 &&
			context->prefill_batch->emit_count != 0u )
			return(SPARK_STATUS_UNSUPPORTED);
		prefill = 0;
	}
	...
	SparkDsv4AsyncCompletion *async)
{
	...
	/* Mirror publication is DEFERRED to the completion side
	 * (SparkDsv4ModuleFinishCachePages): the host view may advance only
	 * once the device update is known-good, otherwise an asynchronous
	 * kernel failure would leave the two page-table views diverged. */
	slot->page_table_mirror_pending = slot->page_table_update_count;
	...
	uint32_t completed)
{
	...
	{
		/* The stream milestone proves the device page-table kernel ran;
		 * publish the deferred host-mirror entries now. */
		for (index=0u; index<slot->page_table_mirror_pending; index++)
			state->paged_cache.host_device_page_table[
				slot->host_page_table_update_indices[index]] =
				slot->host_page_table_update_values[index];
		slot->page_table_mirror_pending = 0u;
	}
	if ( completed != 0u && result == SPARK_STATUS_OK )
	...
	if ( completed == 0u )
	{
		/* Aborted before completion: the device table never received these
		 * updates, so the host mirror must not receive them either. */
		slot->page_table_mirror_pending = 0u;
	...
	uint32_t is_prefill)
{
	...
	if ( SparkDsv4ModuleBucketShapeAccepted(state,
			request->active_slot_count,request->new_token_count) == 0u ||
		SparkDsv4ModuleCacheLanesCoverFrame(request->active_slot_count,
			request->new_token_count,request->cache_lane_count) == 0u )
		return(0u);
	...
	const SparkModelDriverAdmissionRequest *request)
{
	...
	for (lane=0u; status==SPARK_STATUS_OK && lane<request->cache_lane_count;
		lane++)
	{
		/* Single identity resolve for pages + mutable demand (profile P1-B):
		 * the former pair re-ran the hash probe and the O(depth) ancestor
		 * walk twice per lane per admission. */
		status = SparkKvPageCacheResolveLaneAdmission(
...
/* Shared failure teardown for AllocateState: releases the collective, the
 * pinned credit buffers, the ledger, the cache mutex and the instance
 * shell. Both early-failure paths previously duplicated this block. */
static SparkStatus SparkDsv4ModuleReleasePartialState(
	SparkDsv4ModuleState *state,
	SparkStatus status)
{
	if ( state->tp_device_collective_initialized != 0u )
	...
	const char **pack_path_out)
{
	...
	if ( status != SPARK_STATUS_OK )
		return(SparkDsv4ModuleReleasePartialState(state,status));
	...
	if ( state->tp_continuations == 0 )
		return(SparkDsv4ModuleReleasePartialState(state,
