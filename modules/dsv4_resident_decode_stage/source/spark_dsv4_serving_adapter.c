...
#if defined(SPARK_DSV4_PRO_BUILD)
/* Pro carries 61 backbone layers over the four pipeline stages; every
 * TP rank inside a pipeline stage runs the identical slice, so each rank
 * group is uniform and the group heads sum back to the layer count the
...
#else
/* Flash carries 43 backbone layers over the same four-stage split. */
#define SPARK_DSV4_SERVING_STAGE_LAYERS \
	...
	const SparkModelServingSubmission *submission)
{
	 ...
	 * The state check stays family-side: &state->common is a small non-NULL
	 * offset even when state is NULL, so the shared helper's common==0 gate
	 * cannot catch it. */
