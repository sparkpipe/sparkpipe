...
static __global__ void SparkDsv4CompressStepKernel(const void *kv_bf16,
	...
	void *emit_bf16,uint32_t *emitted,const uint32_t *state_row_limit)
{
	uint32_t row = blockIdx.x,coff = overlapped != 0u ? 2u : 1u,channels = coff * width,channel;
	uint32_t slot,boundary,lane,previous,state_limit;
	uint64_t ape_base,source,state_base;
	float *kv_state,*score_state;
	float pooled;
	state_limit = state_row_limit != 0u ? *state_row_limit : row_count;
	if ( row >= row_count || row >= state_limit )
		return;
...
// One accumulate walker for both combine paths: destination += source, with an
// optional relay mirror (NULL relay = plain accumulate). The relay variant was
// a byte-for-byte twin of this kernel except the extra store (deep-audit
// similarity 0.80); folding keeps both paths on one rounding/store body so
// they cannot drift. The packed store below is exactly what
// SparkLmStoreBf16Pair emits (__floats2bfloat162_rn + one uint32 store), so the
// plain path is bitwise unchanged and the relay path keeps its raw stores.
static __global__ void SparkDsv4AccumAddKernel(void *destination_bf16,
	...
	uint32_t width)
{
	...
	for (element = threadIdx.x; element < (width >> 1u); element += blockDim.x)
	{
		...
		((uint32_t *)destination_bf16)[offset + element] = *(const uint32_t *)&packed;
		if ( relay_bf16 != 0 )
			((uint32_t *)relay_bf16)[offset + element] = *(const uint32_t *)&packed;
	}
}
...
extern "C" cudaError_t SparkDsv4LaunchAccumU64Max(cudaStream_t stream, uint64_t *destination, const uint64_t *source, uint32_t element_count)
{
	if ( stream == 0 || destination == 0 || source == 0 || element_count == 0u )
		return(cudaErrorInvalidValue);
	/* Grid arithmetic is the exported launch truth; the literal ceil-div and
	 * bare block size it replaces were the third spelling of the same 256 in
	 * this file alone. */
	SparkDsv4AccumU64MaxKernel<<<LmLaunchAccumGridElements(element_count),
		LM_LAUNCH_ACCUM_THREADS,0u,stream>>>(destination,source,element_count);
	return(cudaGetLastError());
}
...
/* Defined at end of this file via spark_dsv4_dspark_kernels.cuh; the
 * prototype lets the configuration below pin the kernel's dynamic-smem
 * opt-in alongside its siblings. */
static __global__ void SparkDsv4DsparkAttentionKernel(
	const uint16_t *q_bf16,const uint16_t *kv_cache_bf16,
	uint64_t lane_stride_elements,uint32_t lane_index,
	const uint16_t *block_kv_bf16,const float *sink_f32,float scale,
	uint16_t *out_bf16,uint32_t block_size,uint32_t head_count,
...
extern "C" cudaError_t SparkDsv4ConfigureCudaKernels(uint32_t *multiprocessor_count)
{
    ...
    /* DSpark draft attention uses the same query-tile geometry as
     * SparseAttn; pin its dynamic-smem opt-in to the identical size so a
     * future head-dimension growth cannot silently cross the 48 KB
     * launch default again. */
    if ( error == cudaSuccess )
        error = cudaFuncSetAttribute(
            SparkDsv4DsparkAttentionKernel,
            cudaFuncAttributeMaxDynamicSharedMemorySize,
            (int)(SPARK_DSV4_SPARSE_ATTN_HEADS_PER_CTA *
...
extern "C" cudaError_t SparkDsv4LaunchCompressStep(cudaStream_t stream,
	...
	void *emit_bf16,uint32_t *emitted,const uint32_t *state_row_limit)
{
	if ( state_row_limit == 0 || kv_bf16 == 0 || score_bf16 == 0 || ape_f32 == 0 ||
	...
	SparkDsv4CompressStepKernel<<<row_count,SPARK_LM_CTA_THREADS,0,stream>>>(
		...
		overlapped,width,emit_bf16,emitted,state_row_limit);
	return(cudaGetLastError());
}
...
{
	/* Width policy routed through the exported launch truth (runtime/launch.h);
	 * this is the one deliberate tightening over the old unchecked launcher -
	 * the transport always passes the even compile-time hidden dimension, so
	 * valid calls are unaffected and invalid ones now fail closed like the
	 * other three launchers in this family. */
	if ( LmLaunchAccumWidthPolicy(width) != LM_LAUNCH_OK )
		return(cudaErrorInvalidValue);
	SparkDsv4AccumAddKernel<<<LmLaunchAccumGridRows(row_count),SPARK_LM_CTA_THREADS,0,stream>>>(destination_bf16,source_bf16,0,row_count,width);
	return(cudaGetLastError());
}
	...
	uint32_t row_count,uint32_t width)
{
	if ( stream == 0 || destination_bf16 == 0 || source_bf16 == 0 ||
		relay_bf16 == 0 || row_count == 0u ||
		LmLaunchAccumWidthPolicy(width) != LM_LAUNCH_OK )
		return(cudaErrorInvalidValue);
	SparkDsv4AccumAddKernel<<<LmLaunchAccumGridRows(row_count),SPARK_LM_CTA_THREADS,0,stream>>>(
	...
	uint32_t width)
{
	if ( stream == 0 || destination_bf16 == 0 || rank_devices == 0 ||
		...
		LmLaunchAccumWidthPolicy(width) != LM_LAUNCH_OK )
		return(cudaErrorInvalidValue);
	SparkDsv4AccumAddTp4TreeKernel<<<LmLaunchAccumGridRows(row_count),SPARK_LM_CTA_THREADS,0,stream>>>(
