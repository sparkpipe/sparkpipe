...
#error "LING_CONTRACT_SHA256 must identify the exact model package contract"
#endif

#define SPARK_LING_MODULE_TAG "ling_stage"
#define SPARK_LING_STAGEPACK_MAX_TENSOR_COUNT 2048u
#define SPARK_LING_HEAD_TILE 1024u
#define SPARK_LING_NO_KV_ORDINAL UINT32_MAX
#define SPARK_LING_NO_KDA_ORDINAL UINT32_MAX
#define SPARK_LING_KV_ACCESS_ERROR_WORD_COUNT 6u

#if SPARK_LING_MODEL_KV_SLOT_BYTES != \
	( SPARK_LING_KV_ARENA_KV_HEAD_COUNT * \
	  SPARK_LING_KV_ARENA_HEAD_DIM * \
...
typedef struct SparkLingAsyncCompletion
{
	...
	uint32_t lane_count;
	uint32_t row_count;
	uint32_t lane_indices[SPARK_LING_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT];
	uint8_t lane_bound[SPARK_LING_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT];
	uint64_t lane_sequence_ids[SPARK_LING_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT];
	uint64_t lane_next_positions[SPARK_LING_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT];
	uint32_t *output_token_destination;
	SparkModelDriverCompletion completion;
} SparkLingAsyncCompletion;

struct SparkLingModuleState
{
	...
	SparkLingLayerWeights layers[SPARK_LING_RESIDENT_DECODE_STAGE_LAYERS_PER_STAGE];
	uint32_t kv_ordinal_by_local_layer[SPARK_LING_RESIDENT_DECODE_STAGE_LAYERS_PER_STAGE];
	...
	uint64_t layer_seen[SPARK_LING_RESIDENT_DECODE_STAGE_LAYERS_PER_STAGE];
	...
	atomic_uint lane_states[SPARK_LING_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT];
	atomic_uchar lane_bound[SPARK_LING_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT];
	atomic_ullong lane_sequence_ids[SPARK_LING_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT];
	atomic_ullong lane_next_positions[SPARK_LING_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT];
	...
	SparkLingExecutionSlot *slot)
{
	...
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateRows(state,rows,SPARK_LING_MODEL_QUERY_DIMENSION,(void **)&slot->q_bf16);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateRows(state,rows,SPARK_LING_MODEL_HEAD_COUNT * SPARK_LING_MODEL_LATENT_DIMENSION,(void **)&slot->query_latent_bf16);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateRows(state,rows,SPARK_LING_MODEL_HEAD_COUNT * SPARK_LING_MODEL_ROPE_DIMENSION,(void **)&slot->query_rope_bf16);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateRows(state,rows,SPARK_LING_MODEL_HEAD_COUNT,(void **)&slot->attn_gate_bf16);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateRows(state,rows,2u * SPARK_LING_MODEL_KDA_QKV_DIMENSION + SPARK_LING_MODEL_KDA_VALUE_DIMENSION + SPARK_LING_MODEL_KDA_HEAD_COUNT,(void **)&slot->fused_qkvb_bf16);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateRows(state,rows,SPARK_LING_MODEL_KDA_HEAD_COUNT,(void **)&slot->kda_beta_logit);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateRows(state,rows,SPARK_LING_MODEL_KDA_VALUE_DIMENSION,(void **)&slot->kda_gate_bf16);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateRows(state,rows,SPARK_LING_MODEL_KDA_VALUE_DIMENSION,(void **)&slot->kda_decay_logit_bf16);
...
static SparkStatus SparkLingAllocateSlotMlp(
	SparkLingModuleState *state,
	SparkLingExecutionSlot *slot)
{
	...
	SparkStatus status;
	rows = state->execution_row_capacity;
	packed_rows = rows * SPARK_LING_MODEL_MOE_TOP_K;
	status = SparkLingAllocateRows(state,rows,SPARK_LING_MODEL_MOE_ROUTED_GATE_UP_DIMENSION,(void **)&slot->gate_up_bf16);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateRows(state,rows,SPARK_LING_MODEL_MOE_TOP_K * SPARK_LING_MODEL_MOE_INTERMEDIATE_DIMENSION,(void **)&slot->intermediate_bf16);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateRows(state,packed_rows,SPARK_LING_MODEL_HIDDEN_DIMENSION,(void **)&slot->expert_out_bf16);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateRows(state,rows,SPARK_LING_MODEL_HIDDEN_DIMENSION,(void **)&slot->shared_out_bf16);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateBytes(state,rows,SPARK_LING_MODEL_MOE_EXPERT_COUNT,sizeof(float),(void **)&slot->router_logits_f32);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateBytes(state,SPARK_LING_RESIDENT_DECODE_STAGE_ATTN_SPLIT_PARTIAL_BLOCKS(rows,SPARK_LING_MODEL_HEAD_COUNT / state->tp_degree),SPARK_LING_RESIDENT_DECODE_STAGE_ATTN_SPLIT_PARTIAL_FLOATS,sizeof(float),(void **)&slot->attention_split_partials_f32);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateBytes(state,packed_rows,1u,sizeof(uint32_t),(void **)&slot->route_expert);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateBytes(state,packed_rows,1u,sizeof(float),(void **)&slot->route_weight);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateBytes(state,packed_rows,1u,sizeof(uint32_t),(void **)&slot->route_source_token);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateBytes(state,packed_rows,1u,sizeof(uint32_t),(void **)&slot->route_packed_row);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateBytes(state,SPARK_LING_MODEL_MOE_EXPERT_COUNT + 1u,1u,sizeof(uint32_t),(void **)&slot->group_row_offset);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateBytes(state,SPARK_LING_MODEL_MOE_EXPERT_COUNT + 1u,1u,sizeof(uint32_t),(void **)&slot->group_tile_prefix_w1);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateBytes(state,SPARK_LING_MODEL_MOE_EXPERT_COUNT + 1u,1u,sizeof(uint32_t),(void **)&slot->group_tile_prefix_w2);
	return(status);
}

static SparkStatus SparkLingAllocateSlotHead(
	SparkLingModuleState *state,
	SparkLingExecutionSlot *slot)
{
	uint64_t rows,tiles;
	SparkStatus status;
	rows = state->execution_row_capacity;
	tiles = SparkCeilDivU64(SPARK_LING_MODEL_OUTPUT_VOCAB_COUNT,SPARK_LING_HEAD_TILE);
	status = SparkLingAllocateBytes(state,rows,tiles,sizeof(float),(void **)&slot->head_candidate_score);
	if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateBytes(state,rows,tiles,sizeof(uint32_t),(void **)&slot->head_candidate_token);
...
static SparkStatus SparkLingAllocateSlots(SparkLingModuleState *state)
{
	...
	for (index=0u; status==SPARK_STATUS_OK && index<state->pipeline_slot_count; index++)
	{
		...
		status = SparkLingAllocateSlotHost(&state->slots[index]);
		if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateSlotMetadata(state,&state->slots[index]);
		if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateSlotHidden(state,&state->slots[index]);
		if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateSlotMlp(state,&state->slots[index]);
		if ( status == SPARK_STATUS_OK ) status = SparkLingAllocateSlotHead(state,&state->slots[index]);
	}
	return(status);
}

static SparkStatus SparkLingBuildPageTable(SparkLingModuleState *state)
{
	uint32_t *host_table;
	...
	status = SparkLingAllocateBytes(state,entries,1u,sizeof(uint32_t),(void **)&state->page_table);
	if ( status == SPARK_STATUS_OK )
	{
		error = cudaMemcpy(state->page_table,host_table,entries * sizeof(uint32_t),cudaMemcpyHostToDevice);
		status = SparkStageModuleCudaStatus(SPARK_LING_MODULE_TAG,error,"page_table");
	}
	free(host_table);
	return(status);
}
...
static SparkStatus SparkLingAllocateCaches(SparkLingModuleState *state)
{
	...
	for (local=0u; local<state->layer_count; local++)
	{
		...
		state->kv_ordinal_by_local_layer[local] = SPARK_LING_NO_KV_ORDINAL;
		state->kda_ordinal_by_local_layer[local] = SPARK_LING_NO_KDA_ORDINAL;
		if ( layer < SPARK_LING_MODEL_LAYER_COUNT )
		{
			if ( SparkLingStagePackLayerIsMla(layer) != 0u )
				state->kv_ordinal_by_local_layer[local] = state->kv_layer_count++;
			else if ( SparkLingStagePackLayerIsKda(layer) != 0u )
				state->kda_ordinal_by_local_layer[local] = state->kda_layer_count++;
		}
	}
	status = SparkLingBuildPageTable(state);
	main_page_bytes = (uint64_t)64u * SPARK_LING_MODEL_KV_SLOT_BYTES;
	kda_window_stride = (uint64_t)state->resident_sequence_capacity *
		SPARK_LING_MODEL_KDA_CONV_WINDOW_BYTES_PER_LAYER;
	...
	state->kda_state_layer_stride_bytes = (uint64_t)state->resident_sequence_capacity *
		SPARK_LING_MODEL_KDA_STATE_BYTES_PER_LAYER;
	...
	if ( status == SPARK_STATUS_OK )
		status = SparkLingKvInitialize(state);
	return(status);
}

static SparkStatus SparkLingAdmissionPredicate(
	void *context,
...
static SparkStatus SparkLingValidateRoundMajor(
	const SparkLingModuleState *state,
	const SparkLingResidentDecodeStageBatchView *batch)
{
...
typedef enum SparkLingChainStage
{
	SPARK_LING_CHAIN_STAGE_BEGIN = 0,
	SPARK_LING_CHAIN_STAGE_ATTENTION,
	SPARK_LING_CHAIN_STAGE_REDUCE_ATTENTION,
	SPARK_LING_CHAIN_STAGE_MLP,
	SPARK_LING_CHAIN_STAGE_REDUCE_MLP,
	SPARK_LING_CHAIN_STAGE_HEAD,
	SPARK_LING_CHAIN_STAGE_REDUCE_HEAD,
	SPARK_LING_CHAIN_STAGE_FINISH
} SparkLingChainStage;

typedef struct SparkLingTpChain
{
	SparkLingModuleState *state;
	SparkLingExecutionSlot *slot;
	uint32_t slot_index;
...
static void SparkLingBuildWave(SparkLingTpChain *chain)
{
	SparkLingModuleState *state;
	SparkLingExecutionSlot *slot;
	const SparkLingResidentDecodeStageFrameContext *context;
	...
	wave->kv_ordinal_by_local_layer = state->kv_ordinal_by_local_layer;
	...
	wave->attention_split_partial_blocks = SPARK_LING_RESIDENT_DECODE_STAGE_ATTN_SPLIT_PARTIAL_BLOCKS(
	...
	const SparkLingResidentDecodeStageNodeContext *context)
{
	...
	for (route=0u; route<route_count; route++)
	{
		...
		credit_bytes = (uint64_t)configuration.max_active_sequence_count * hidden * SPARK_LING_MODEL_BF16_ELEMENT_BYTES + SPARK_TP_DEVICE_COLLECTIVE_NONCE_BYTES;
	...
	if ( total_bytes != 0u && memory_mode == SPARK_TP_DEVICE_COLLECTIVE_MEMORY_MODE_MAPPED_HOST )
	{
		...
		if ( error != cudaSuccess )
		{
			...
			return(SparkStageModuleCudaStatus(SPARK_LING_MODULE_TAG,error,"tp_credit_alloc"));
		}
...
static void SparkLingTpChainFail(SparkLingTpChain *chain,SparkStatus status)
{
	SparkLingModuleState *state;
...
static SparkStatus SparkLingStageHostBatch(
	const SparkLingModuleState *state,
	SparkLingExecutionSlot *slot,
	const SparkLingResidentDecodeStageBatchView *batch)
{
	uint32_t row;
	if ( state == 0 || slot == 0 || batch == 0 || batch->row_count > SPARK_LING_RESIDENT_DECODE_STAGE_MAX_INPUT_ROW_COUNT )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	for (row=0u; row<batch->row_count; row++)
	{
	...
	memset(slot->host_kv_access_error,0,SPARK_LING_KV_ACCESS_ERROR_WORD_COUNT * sizeof(uint32_t));
	return(SPARK_STATUS_OK);
}
...
static void CUDART_CB SparkLingCompleteAsync(void *context)
{
	...
	async = (SparkLingAsyncCompletion *)context;
...
{
	const SparkLingResidentDecodeStageBatchView *batch;
	SparkLingClaimedContinuityContext continuity;
	SparkLingExecutionSlot *slot;
	SparkLingTpChain *chain;
	uint8_t simulated_bound[SPARK_LING_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT];
	uint64_t simulated_sequence[SPARK_LING_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT];
	uint64_t simulated_next[SPARK_LING_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT];
	...
	status = SparkStageModuleIndexSetClaimAndPrepare(state->lane_states,state->resident_sequence_capacity,batch->row_resident_slots,batch->active_sequence_count,SparkLingPrepareClaimedContinuity,&continuity);
	if ( status != SPARK_STATUS_OK )
		return(status);
	...
	status = SparkLingStageHostBatch(state,slot,batch);
	if ( status != SPARK_STATUS_OK )
	{
	...
	if ( status == SPARK_STATUS_OK )
	{
		chain = (SparkLingTpChain *)calloc(1u,sizeof(*chain));
		if ( chain == 0 )
			status = SPARK_STATUS_CAPACITY_EXCEEDED;
	}
	if ( status != SPARK_STATUS_OK )
	{
		...
		SparkLingInvalidateClaimedLanes(state,batch->row_resident_slots,batch->active_sequence_count);
		atomic_fetch_add_explicit(&state->failed_count,1u,memory_order_relaxed);
		SparkStageModuleSlotRelease(state->slot_states,slot_index);
		SparkStageModuleIndexSetRelease(state->lane_states,state->resident_sequence_capacity,batch->row_resident_slots,batch->active_sequence_count);
		return(status);
	...
	SparkModelDriverAdmissionDecision *decision)
{
	...
	table.max_input_row_count = SPARK_LING_RESIDENT_DECODE_STAGE_MAX_INPUT_ROW_COUNT;
	...
	SparkLingModuleState **state_out)
{
	...
	SparkStatus status;
	uint32_t lane;
	state = (SparkLingModuleState *)calloc(1u,sizeof(*state));
	if ( state == 0 )
		return(SPARK_STATUS_CAPACITY_EXCEEDED);
	state->ledger.module_tag = SPARK_LING_MODULE_TAG;
	status = SparkLingModuleConfigure(state,configuration,host_services,&pack_path);
