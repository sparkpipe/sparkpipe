...
static const SparkModelServingAdapterDescriptor SparkLingServingDescriptor =
{
	...
	.capability_flags = SPARK_MODEL_SERVING_ADAPTER_CAPABILITY_PREFILL |
		...
		SPARK_LING_SERVING_TOPOLOGY_FLAG |
...
static SparkStatus SparkLingServingJsonUnsigned(
	const SparkJsonDocument *document,
	int32_t root,
	const char *name,
	uint32_t *value)
{
	int32_t token;
	token = SparkLingServingJsonMember(document,root,name);
	return(token < 0 ? SPARK_STATUS_SCHEMA_ERROR : SparkJsonGetUInt32(document,token,value));
}

static SparkStatus SparkLingServingLoadTpAlgorithms(
	const SparkJsonDocument *document,
	int32_t object,
	SparkTpDeviceCollectiveTopology *topology)
{
	...
	token = SparkLingServingJsonMember(document,object,"algorithms");
	if ( token < 0 ||
...
static SparkStatus SparkLingServingLoadTpStepRails(
	const SparkJsonDocument *document,
	int32_t object,
	uint32_t tp_degree,
	SparkTpDeviceCollectiveTopology *topology)
{
	...
	SparkStatus status;
	token = SparkLingServingJsonMember(document,object,"step_rail_indices");
	if ( token < 0 ||
		!SparkJsonTokenIsType(document,token,SPARK_JSON_TOKEN_ARRAY) )
		return(SPARK_STATUS_SCHEMA_ERROR);
	count = SparkJsonGetArrayElementCount(document,token);
...
static SparkStatus SparkLingServingLoadTpRailHosts(
	const SparkJsonDocument *document,
	int32_t object,
	SparkTpDeviceCollectiveTopology *topology,
	uint32_t tp_degree)
{
	...
	char *host;
	SparkStatus status;
	token = SparkLingServingJsonMember(document,object,"rail_peer_hosts");
	if ( token < 0 ||
		!SparkJsonTokenIsType(document,token,SPARK_JSON_TOKEN_ARRAY) ||
...
static SparkStatus SparkLingServingLoadSessionPorts(
	const SparkJsonDocument *document,
	int32_t object,
	const char *name,
	...
	uint32_t tp_degree)
{
	...
	SparkStatus status;
	token = SparkLingServingJsonMember(document,object,name);
	if ( token < 0 ||
		!SparkJsonTokenIsType(document,token,SPARK_JSON_TOKEN_ARRAY) )
		return(SPARK_STATUS_SCHEMA_ERROR);
	count = SparkJsonGetArrayElementCount(document,token);
...
static SparkStatus SparkLingServingValidateTpCollectiveMembers(
	const SparkJsonDocument *document,
	int32_t object,
	...
	uint32_t tp_degree)
{
	...
	if ( SparkJsonStringEquals(document,token,"nccl") )
		...
		return(SPARK_STATUS_SCHEMA_ERROR);
	status = SparkLingServingValidateTpCollectiveMembers(document,object,
		state->tp_collective_backend_kind);
	if ( status != SPARK_STATUS_OK )
		return(status);
	relative_backend_path = 0;
	token = SparkLingServingJsonMember(document,object,"backend_module_path");
	...
	status = SparkLingServingJsonUnsigned(document,object,"listen_port",&port);
	...
	status = SparkLingServingJsonUnsigned(document,object,"connect_timeout_milli",&state->tp_connect_timeout_milli);
	...
	token = SparkLingServingJsonMember(document,object,"peer_ports");
	if ( token < 0 || !SparkJsonTokenIsType(document,token,SPARK_JSON_TOKEN_ARRAY) )
		return(SPARK_STATUS_SCHEMA_ERROR);
	count = SparkJsonGetArrayElementCount(document,token);
	if ( count != tp_degree )
	...
	uint32_t *tp_rank)
{
	...
	if ( status == SPARK_STATUS_OK )
		status = SparkLingServingJsonUnsigned(&document,root,"schema_version",&schema_version);
	if ( status == SPARK_STATUS_OK && schema_version != SPARK_LING_SERVING_ADAPTER_CONFIGURATION_SCHEMA_VERSION )
		status = SPARK_STATUS_SCHEMA_ERROR;
	token = status == SPARK_STATUS_OK ? SparkLingServingJsonMember(&document,root,"model_revision") : -1;
	if ( status == SPARK_STATUS_OK && (token < 0 || !SparkJsonStringEquals(&document,token,LING_MODEL_REVISION)) )
		status = SPARK_STATUS_SCHEMA_ERROR;
	token = status == SPARK_STATUS_OK ? SparkLingServingJsonMember(&document,root,"expert_weight_codec") : -1;
	...
	token = status == SPARK_STATUS_OK ? SparkLingServingJsonMember(&document,root,"stage_pack_path") : -1;
	if ( status == SPARK_STATUS_OK )
		status = token < 0 ? SPARK_STATUS_SCHEMA_ERROR : SparkJsonCopyString(&document,token,&relative_stage_pack_path);
	if ( status == SPARK_STATUS_OK )
		status = SparkLingServingJsonUnsigned(&document,root,"max_sequence_positions",max_sequence_positions);
	if ( status == SPARK_STATUS_OK )
		status = SparkLingServingJsonUnsigned(&document,root,"execution_row_capacity",execution_row_capacity);
	if ( status == SPARK_STATUS_OK )
		status = SparkLingServingJsonUnsigned(&document,root,"decode_split_context_threshold",decode_split_context_threshold);
	if ( status == SPARK_STATUS_OK )
		status = SparkLingServingJsonUnsigned(&document,root,"tp_degree",tp_degree);
	if ( status == SPARK_STATUS_OK )
		status = SparkLingServingJsonUnsigned(&document,root,"tp_rank",tp_rank);
