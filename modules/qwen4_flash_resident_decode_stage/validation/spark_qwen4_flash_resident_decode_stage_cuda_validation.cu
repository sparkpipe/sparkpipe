...
#define SPARK_QWEN4_FLASH_DK SPARK_QWEN4_FLASH_MODEL_GDN_HEAD_KEY_DIMENSION
#define SPARK_QWEN4_FLASH_DV SPARK_QWEN4_FLASH_MODEL_GDN_HEAD_VALUE_DIMENSION
#define SPARK_QWEN4_FLASH_HEADS SPARK_QWEN4_FLASH_MODEL_GDN_VALUE_HEAD_COUNT
#define SPARK_QWEN4_FLASH_CONV SPARK_QWEN4_FLASH_MODEL_GDN_CONV_CHANNELS
...
static uint32_t SparkQwen4FlashValNext(void)
{
	SparkQwen4FlashValRandomState = SparkQwen4FlashValRandomState * 1664525u + 1013904223u;
	return(SparkQwen4FlashValRandomState >> 8u);
}

static float SparkQwen4FlashValUniform(float scale)
{
	return(((float)(int32_t)(SparkQwen4FlashValNext() & 0xffffu) - 32768.0f) * scale / 32768.0f);
}

static uint16_t SparkQwen4FlashValBf16(float value)
{
	uint32_t bits;
...
static float SparkQwen4FlashValFromBf16(uint16_t value)
{
...
static void SparkQwen4FlashValFillBf16(uint16_t *packed, float *exact, uint64_t count, float scale)
{
	...
	{
		packed[index] = SparkQwen4FlashValBf16(SparkQwen4FlashValUniform(scale));
		if (exact != 0)
			exact[index] = SparkQwen4FlashValFromBf16(packed[index]);
	}
}

static int SparkQwen4FlashValFail(const char *check, const char *detail)
{
	fprintf(stderr,"qwen4_flash_validation failure=%s detail=%s\n",check,detail);
	return(1);
}

static int SparkQwen4FlashValCuda(cudaError_t error, const char *check)
{
	if (error == cudaSuccess)
		return(0);
	fprintf(stderr,"qwen4_flash_validation failure=%s cuda=%s\n",check,cudaGetErrorString(error));
	return(1);
}

typedef struct SparkQwen4FlashValMetrics
{
	...
	uint64_t count;
} SparkQwen4FlashValMetrics;

static void SparkQwen4FlashValMeasure(SparkQwen4FlashValMetrics *metrics, const float *actual, const float *reference, uint64_t count)
{
	uint64_t index;
	double difference;
...
static int SparkQwen4FlashValReport(const char *check, const SparkQwen4FlashValMetrics *metrics, double max_relative_l2, double minimum_cosine)
{
	...
	printf("qwen4_flash_validation check=%s elements=%llu relative_l2=%.9g cosine=%.9g max_abs=%.9g\n",
	...
	if (isfinite(relative_l2) == 0 || relative_l2 > max_relative_l2)
		return(SparkQwen4FlashValFail(check,"relative_l2"));
	if (cosine < minimum_cosine)
		return(SparkQwen4FlashValFail(check,"cosine"));
	return(0);
}
...
static void SparkQwen4FlashValL2Norm(const float *input, float *output, uint32_t dimension)
{
	uint32_t element;
	float total = 0.0f;
...
static void SparkQwen4FlashValGdnRecurrence(const float *q, const float *k, const float *v, const float *g, const float *beta, float *state, float *output, uint32_t tokens)
{
	float qn[SPARK_QWEN4_FLASH_DK],kn[SPARK_QWEN4_FLASH_DK],delta[SPARK_QWEN4_FLASH_DV];
	float scale = 1.0f / sqrtf((float)SPARK_QWEN4_FLASH_DK),decay,kv_mem;
	uint32_t token,row,column;
	for (token = 0u; token < tokens; token++)
	{
		SparkQwen4FlashValL2Norm(q + ((uint64_t)token * SPARK_QWEN4_FLASH_DK),qn,SPARK_QWEN4_FLASH_DK);
		SparkQwen4FlashValL2Norm(k + ((uint64_t)token * SPARK_QWEN4_FLASH_DK),kn,SPARK_QWEN4_FLASH_DK);
		for (row = 0u; row < SPARK_QWEN4_FLASH_DK; row++)
			qn[row] *= scale;
		decay = expf(g[token]);
		for (row = 0u; row < SPARK_QWEN4_FLASH_DK; row++)
			for (column = 0u; column < SPARK_QWEN4_FLASH_DV; column++)
				state[(row * SPARK_QWEN4_FLASH_DV) + column] *= decay;
		for (column = 0u; column < SPARK_QWEN4_FLASH_DV; column++)
		{
			kv_mem = 0.0f;
			for (row = 0u; row < SPARK_QWEN4_FLASH_DK; row++)
				kv_mem += state[(row * SPARK_QWEN4_FLASH_DV) + column] * kn[row];
			delta[column] = (v[((uint64_t)token * SPARK_QWEN4_FLASH_DV) + column] - kv_mem) * beta[token];
		}
		for (row = 0u; row < SPARK_QWEN4_FLASH_DK; row++)
			for (column = 0u; column < SPARK_QWEN4_FLASH_DV; column++)
				state[(row * SPARK_QWEN4_FLASH_DV) + column] += kn[row] * delta[column];
		for (column = 0u; column < SPARK_QWEN4_FLASH_DV; column++)
		{
			kv_mem = 0.0f;
			for (row = 0u; row < SPARK_QWEN4_FLASH_DK; row++)
				kv_mem += state[(row * SPARK_QWEN4_FLASH_DV) + column] * qn[row];
			output[((uint64_t)token * SPARK_QWEN4_FLASH_DV) + column] = kv_mem;
		}
	}
}

static void SparkQwen4FlashValRope(float *vector, uint32_t rope_dim, uint32_t position, float theta)
{
...
static void SparkQwen4FlashValRmsNorm(const float *input, const float *weight, float *output, uint32_t dimension, float epsilon)
{
	uint32_t element;
...
static void SparkQwen4FlashValAttention(const float *q_fused, const float *k_cache, const float *v_cache, const float *q_norm_weight, float *output, uint32_t group, uint32_t tokens, float epsilon)
{
	...
	float scale = 1.0f / sqrtf((float)SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION),maximum,total;
	uint32_t head,element,token;
	for (head = 0u; head < group; head++)
	{
		const float *fused = q_fused + ((uint64_t)head * 2u * SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION);
		SparkQwen4FlashValRmsNorm(fused,q_norm_weight,qh,SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION,epsilon);
		SparkQwen4FlashValRope(qh,SPARK_QWEN4_FLASH_MODEL_ATTN_ROPE_DIMENSION,tokens - 1u,SPARK_QWEN4_FLASH_MODEL_ATTN_ROPE_THETA);
		maximum = -3.0e38f;
		for (token = 0u; token < tokens; token++)
		{
			probability = 0.0f;
			for (element = 0u; element < SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION; element++)
				probability += qh[element] * k_cache[((uint64_t)token * SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION) + element];
		...
		for (element = 0u; element < SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION; element++)
		{
			probability = 0.0f;
			for (token = 0u; token < tokens; token++)
				probability += (scores[token] / total) * v_cache[((uint64_t)token * SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION) + element];
			output[((uint64_t)head * SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION) + element] =
				probability * (1.0f / (1.0f + expf(-fused[SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION + element])));
		}
	}
}
...
typedef struct SparkQwen4FlashValDevice
{
	...
	float *chunk_w;
	float *chunk_kg;
} SparkQwen4FlashValDevice;

static int SparkQwen4FlashValDeviceSetup(SparkQwen4FlashValDevice *device)
{
	uint64_t state_elements = 2ull * SPARK_QWEN4_FLASH_HEADS * SPARK_QWEN4_FLASH_DK * SPARK_QWEN4_FLASH_DV;
	uint64_t vector_floats = (uint64_t)SPARK_QWEN4_FLASH_HEADS * SPARK_QWEN4_FLASH_MODEL_GDN_CHUNK_TOKENS * SPARK_QWEN4_FLASH_DK;
	uint64_t matrix_floats = (uint64_t)SPARK_QWEN4_FLASH_HEADS * SPARK_QWEN4_FLASH_MODEL_GDN_CHUNK_TOKENS * SPARK_QWEN4_FLASH_MODEL_GDN_CHUNK_TOKENS;
	uint32_t tokens = SPARK_QWEN4_FLASH_VALIDATION_CHUNK_TOKENS;
	cudaError_t error;
	memset(device,0,sizeof(*device));
	error = cudaMalloc((void **)&device->conv_weight,(uint64_t)SPARK_QWEN4_FLASH_CONV * SPARK_QWEN4_FLASH_MODEL_GDN_CONV_KERNEL * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->a_log,SPARK_QWEN4_FLASH_HEADS * sizeof(float));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->dt_bias,SPARK_QWEN4_FLASH_HEADS * sizeof(float));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->gdn_norm_weight,SPARK_QWEN4_FLASH_DV * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->q_norm_weight,SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->k_norm_weight,SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->state,state_elements * sizeof(float));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->conv_tail,2ull * SPARK_QWEN4_FLASH_CONV * (SPARK_QWEN4_FLASH_MODEL_GDN_CONV_KERNEL - 1u) * sizeof(uint16_t));
	...
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->qkv,(uint64_t)tokens * SPARK_QWEN4_FLASH_CONV * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->conv_out,(uint64_t)tokens * SPARK_QWEN4_FLASH_CONV * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->core_out,(uint64_t)tokens * SPARK_QWEN4_FLASH_MODEL_GDN_VALUE_DIMENSION * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->z_bf16,(uint64_t)tokens * SPARK_QWEN4_FLASH_MODEL_GDN_VALUE_DIMENSION * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->gated_out,(uint64_t)tokens * SPARK_QWEN4_FLASH_MODEL_GDN_VALUE_DIMENSION * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->ba_bf16,(uint64_t)tokens * SPARK_QWEN4_FLASH_HEADS * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->log_decay,(uint64_t)tokens * SPARK_QWEN4_FLASH_HEADS * sizeof(float));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->beta,(uint64_t)tokens * SPARK_QWEN4_FLASH_HEADS * sizeof(float));
	...
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->chunk_cum_g,(uint64_t)SPARK_QWEN4_FLASH_HEADS * SPARK_QWEN4_FLASH_MODEL_GDN_CHUNK_TOKENS * sizeof(float));
	...
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->chunk_w,(uint64_t)SPARK_QWEN4_FLASH_HEADS * SPARK_QWEN4_FLASH_MODEL_GDN_CHUNK_TOKENS * SPARK_QWEN4_FLASH_DV * sizeof(float));
	...
	device->pool.state_layer_stride_elements = (uint64_t)SPARK_QWEN4_FLASH_HEADS * SPARK_QWEN4_FLASH_DK * SPARK_QWEN4_FLASH_DV;
	...
	device->pool.conv_tail_layer_stride_elements = (uint64_t)SPARK_QWEN4_FLASH_CONV * (SPARK_QWEN4_FLASH_MODEL_GDN_CONV_KERNEL - 1u);
...
{
	uint16_t host_ba[SPARK_QWEN4_FLASH_VALIDATION_ROWS * SPARK_QWEN4_FLASH_HEADS * 2u];
	float host_a[SPARK_QWEN4_FLASH_HEADS],host_bias[SPARK_QWEN4_FLASH_HEADS];
	float host_decay[SPARK_QWEN4_FLASH_VALIDATION_ROWS * SPARK_QWEN4_FLASH_HEADS];
	float host_beta[SPARK_QWEN4_FLASH_VALIDATION_ROWS * SPARK_QWEN4_FLASH_HEADS];
	float ref_decay[SPARK_QWEN4_FLASH_VALIDATION_ROWS * SPARK_QWEN4_FLASH_HEADS];
	float ref_beta[SPARK_QWEN4_FLASH_VALIDATION_ROWS * SPARK_QWEN4_FLASH_HEADS];
	SparkQwen4FlashValMetrics metrics;
	uint32_t row,head;
	uint64_t index;
	float shifted;
	cudaError_t error;
	SparkQwen4FlashValRandomState = 11u;
	SparkQwen4FlashValFillBf16(host_ba,0,SPARK_QWEN4_FLASH_VALIDATION_ROWS * SPARK_QWEN4_FLASH_HEADS * 2u,2.0f);
	for (head = 0u; head < SPARK_QWEN4_FLASH_HEADS; head++)
	{
		host_a[head] = SparkQwen4FlashValUniform(1.0f);
		host_bias[head] = SparkQwen4FlashValUniform(1.0f);
	}
	...
	if (SparkQwen4FlashValCuda(error,"decay_beta") != 0)
		return(1);
	for (row = 0u; row < SPARK_QWEN4_FLASH_VALIDATION_ROWS; row++)
		for (head = 0u; head < SPARK_QWEN4_FLASH_HEADS; head++)
		{
			index = ((uint64_t)row * SPARK_QWEN4_FLASH_HEADS) + head;
			shifted = SparkQwen4FlashValFromBf16(host_ba[index]) + host_bias[head];
			ref_decay[index] = -expf(host_a[head]) * (shifted > 20.0f ? shifted : logf(1.0f + expf(shifted)));
			ref_beta[index] = 1.0f / (1.0f + expf(-SparkQwen4FlashValFromBf16(host_ba[SPARK_QWEN4_FLASH_VALIDATION_ROWS * SPARK_QWEN4_FLASH_HEADS + index])));
		}
	SparkQwen4FlashValMeasure(&metrics,host_decay,ref_decay,SPARK_QWEN4_FLASH_VALIDATION_ROWS * SPARK_QWEN4_FLASH_HEADS);
	if (SparkQwen4FlashValReport("decay_gate",&metrics,1e-5,0.99999999) != 0)
		return(1);
	SparkQwen4FlashValMeasure(&metrics,host_beta,ref_beta,SPARK_QWEN4_FLASH_VALIDATION_ROWS * SPARK_QWEN4_FLASH_HEADS);
...
static int SparkQwen4FlashValCheckConv(SparkQwen4FlashValDevice *device)
{
	...
	if (host_qkv == 0 || host_out == 0 || exact == 0 || expected == 0 || actual == 0 || tails == 0)
		return(SparkQwen4FlashValFail("conv_update","host_alloc"));
	SparkQwen4FlashValRandomState = 23u;
	SparkQwen4FlashValFillBf16(host_qkv,exact,frame_elements,1.0f);
	{
		uint16_t *weight_packed = (uint16_t *)calloc((uint64_t)SPARK_QWEN4_FLASH_CONV * SPARK_QWEN4_FLASH_MODEL_GDN_CONV_KERNEL,sizeof(uint16_t));
		float *weight_exact = (float *)calloc((uint64_t)SPARK_QWEN4_FLASH_CONV * SPARK_QWEN4_FLASH_MODEL_GDN_CONV_KERNEL,sizeof(float));
		if (weight_packed == 0 || weight_exact == 0)
			return(SparkQwen4FlashValFail("conv_update","weight_alloc"));
		SparkQwen4FlashValFillBf16(weight_packed,weight_exact,(uint64_t)SPARK_QWEN4_FLASH_CONV * SPARK_QWEN4_FLASH_MODEL_GDN_CONV_KERNEL,0.5f);
		error = cudaMemcpy(device->conv_weight,weight_packed,(uint64_t)SPARK_QWEN4_FLASH_CONV * SPARK_QWEN4_FLASH_MODEL_GDN_CONV_KERNEL * sizeof(uint16_t),cudaMemcpyHostToDevice);
		...
		if (SparkQwen4FlashValCuda(error,"conv_update") != 0)
			return(1);
		for (row = 0u; row < 2u; row++)
			for (channel = 0u; channel < SPARK_QWEN4_FLASH_CONV; channel++)
			{
				...
				input[0] = exact[base];
				input[1] = exact[2ull * SPARK_QWEN4_FLASH_CONV + base];
				tail[0] = tail[1] = tail[2] = 0.0f;
				{
					...
					for (token_index = 0u; token_index < 2u; token_index++)
					{
						...
						output[token_index] = SparkQwen4FlashValSilu(accumulator);
					}
					tail[2] = input[1];
					tail[1] = input[0];
					tail[0] = 0.0f;
				...
				expected[(uint64_t)0u * 2u * SPARK_QWEN4_FLASH_CONV + base] = output[0];
				expected[(uint64_t)1u * 2u * SPARK_QWEN4_FLASH_CONV + base] = output[1];
			}
		for (token = 0u; token < 2u; token++)
			for (uint64_t i = 0u; i < 2ull * SPARK_QWEN4_FLASH_CONV; i++)
				actual[(uint64_t)token * 2ull * SPARK_QWEN4_FLASH_CONV + i] =
					SparkQwen4FlashValFromBf16(host_out[(uint64_t)token * 2ull * SPARK_QWEN4_FLASH_CONV + i]);
		free(weight_packed);
		free(weight_exact);
	}
	SparkQwen4FlashValMeasure(&metrics,actual,expected,frame_elements);
...
static int SparkQwen4FlashValCheckGdnStep(SparkQwen4FlashValDevice *device)
{
	...
	uint16_t *host_conv = (uint16_t *)calloc(2ull * SPARK_QWEN4_FLASH_CONV,sizeof(uint16_t));
	float *exact = (float *)calloc(2ull * SPARK_QWEN4_FLASH_CONV,sizeof(float));
	...
	float *oracle_out = (float *)calloc(2ull * SPARK_QWEN4_FLASH_HEADS * SPARK_QWEN4_FLASH_DV,sizeof(float));
	float *actual = (float *)calloc(2ull * SPARK_QWEN4_FLASH_HEADS * SPARK_QWEN4_FLASH_DV,sizeof(float));
	uint16_t *core_packed = (uint16_t *)calloc(2ull * SPARK_QWEN4_FLASH_HEADS * SPARK_QWEN4_FLASH_DV,sizeof(uint16_t));
	uint32_t lanes[2] = {0u,1u};
	...
	if (host_conv == 0 || exact == 0 || state_host == 0 || state_reference == 0 || oracle_out == 0 || actual == 0 || core_packed == 0)
		return(SparkQwen4FlashValFail("gdn_step","host_alloc"));
	SparkQwen4FlashValRandomState = 37u;
	SparkQwen4FlashValFillBf16(host_conv,exact,2ull * SPARK_QWEN4_FLASH_CONV,1.0f);
	{
		uint64_t index;
	...
	{
		float host_log_decay[2 * SPARK_QWEN4_FLASH_HEADS],host_beta[2 * SPARK_QWEN4_FLASH_HEADS];
		uint32_t head_index;
		for (head_index = 0u; head_index < 2u * SPARK_QWEN4_FLASH_HEADS; head_index++)
		{
			host_log_decay[head_index] = SparkQwen4FlashValUniform(0.5f) - 0.5f;
			host_beta[head_index] = 0.25f + fabsf(SparkQwen4FlashValUniform(0.5f));
		}
		error = cudaMemcpy(device->state,state_host,2ull * state_elements * sizeof(float),cudaMemcpyHostToDevice);
		if (error == cudaSuccess) error = cudaMemcpy(device->qkv,host_conv,2ull * SPARK_QWEN4_FLASH_CONV * sizeof(uint16_t),cudaMemcpyHostToDevice);
		...
		for (row = 0u; row < 2u; row++)
			for (head = 0u; head < SPARK_QWEN4_FLASH_HEADS; head++)
			{
				uint32_t key_head = head / SPARK_QWEN4_FLASH_MODEL_GDN_VALUE_HEADS_PER_KEY_HEAD;
				const float *q = exact + ((uint64_t)row * SPARK_QWEN4_FLASH_CONV) + ((uint64_t)key_head * SPARK_QWEN4_FLASH_DK);
				const float *k = q + SPARK_QWEN4_FLASH_MODEL_GDN_QK_DIMENSION;
				const float *v = exact + ((uint64_t)row * SPARK_QWEN4_FLASH_CONV) + (2ull * SPARK_QWEN4_FLASH_MODEL_GDN_QK_DIMENSION) + ((uint64_t)head * SPARK_QWEN4_FLASH_DV);
				float g = cold[row] != 0u ? -30.0f : host_log_decay[(row * SPARK_QWEN4_FLASH_HEADS) + head];
				float beta = host_beta[(row * SPARK_QWEN4_FLASH_HEADS) + head];
				SparkQwen4FlashValGdnRecurrence(q,k,v,&g,&beta,
					state_reference + ((uint64_t)row * state_elements) + ((uint64_t)head * SPARK_QWEN4_FLASH_DK * SPARK_QWEN4_FLASH_DV),
					oracle_out + ((uint64_t)row * SPARK_QWEN4_FLASH_HEADS * SPARK_QWEN4_FLASH_DV) + ((uint64_t)head * SPARK_QWEN4_FLASH_DV),1u);
			}
	}
	{
		uint64_t index;
		for (index = 0u; index < 2ull * SPARK_QWEN4_FLASH_HEADS * SPARK_QWEN4_FLASH_DV; index++)
			actual[index] = SparkQwen4FlashValFromBf16(core_packed[index]);
	}
	SparkQwen4FlashValMeasure(&metrics,actual,oracle_out,2ull * SPARK_QWEN4_FLASH_HEADS * SPARK_QWEN4_FLASH_DV);
	if (SparkQwen4FlashValReport("gdn_step_output",&metrics,5e-3,0.99999) != 0)
		return(1);
	SparkQwen4FlashValMeasure(&metrics,state_host,state_reference,2ull * state_elements);
	free(host_conv); free(exact); free(state_host); free(state_reference);
	free(oracle_out); free(actual); free(core_packed);
	return(SparkQwen4FlashValReport("gdn_step_state",&metrics,1e-3,0.999999));
}

static int SparkQwen4FlashValCheckGatedNorm(SparkQwen4FlashValDevice *device)
{
	uint64_t elements = (uint64_t)SPARK_QWEN4_FLASH_VALIDATION_ROWS * SPARK_QWEN4_FLASH_MODEL_GDN_VALUE_DIMENSION;
	...
	uint16_t *norm_packed = (uint16_t *)calloc(SPARK_QWEN4_FLASH_DV,sizeof(uint16_t));
	float *norm_exact = (float *)calloc(SPARK_QWEN4_FLASH_DV,sizeof(float));
	SparkQwen4FlashValMetrics metrics;
	...
	for (row = 0u; row < SPARK_QWEN4_FLASH_VALIDATION_ROWS; row++)
		for (head = 0u; head < SPARK_QWEN4_FLASH_HEADS; head++)
		{
			uint64_t base = ((uint64_t)row * SPARK_QWEN4_FLASH_MODEL_GDN_VALUE_DIMENSION) + ((uint64_t)head * SPARK_QWEN4_FLASH_DV);
			float variance = 0.0f,inverse;
			for (element = 0u; element < SPARK_QWEN4_FLASH_DV; element++)
				variance += exact[base + element] * exact[base + element];
			inverse = 1.0f / sqrtf((variance / (float)SPARK_QWEN4_FLASH_DV) + SPARK_QWEN4_FLASH_MODEL_RMS_NORM_EPSILON);
			for (element = 0u; element < SPARK_QWEN4_FLASH_DV; element++)
				expected[base + element] = exact[base + element] * inverse * norm_exact[element] *
					SparkQwen4FlashValSilu(exact[elements + base + element]);
		}
	for (row = 0u; row < elements; row++)
		actual[row] = SparkQwen4FlashValFromBf16(packed[row]);
	SparkQwen4FlashValMeasure(&metrics,actual,expected,elements);
...
static int SparkQwen4FlashValCheckAttention(SparkQwen4FlashValDevice *device)
{
	...
	const uint64_t fused_elements = (uint64_t)tokens * 2u * SPARK_QWEN4_FLASH_MODEL_ATTN_QUERY_DIMENSION;
	const uint64_t kv_elements = (uint64_t)tokens * SPARK_QWEN4_FLASH_MODEL_ATTN_KV_DIMENSION;
	const uint64_t cache_elements = (uint64_t)SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS * SPARK_QWEN4_FLASH_MODEL_ATTN_CACHE_TOKEN_ELEMENTS;
	...
	float *expected = (float *)calloc(SPARK_QWEN4_FLASH_MODEL_ATTN_QUERY_DIMENSION,sizeof(float));
	float *actual = (float *)calloc(SPARK_QWEN4_FLASH_MODEL_ATTN_QUERY_DIMENSION,sizeof(float));
	uint16_t *out_packed = (uint16_t *)calloc(SPARK_QWEN4_FLASH_MODEL_ATTN_QUERY_DIMENSION,sizeof(uint16_t));
	uint16_t *norm_packed = (uint16_t *)calloc(2u * SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION,sizeof(uint16_t));
	float *q_norm_exact = (float *)calloc(SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION,sizeof(float));
	float *k_norm_exact = (float *)calloc(SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION,sizeof(float));
	uint32_t slot_mapping[SPARK_QWEN4_FLASH_VALIDATION_ATTN_TOKENS];
	uint64_t positions[SPARK_QWEN4_FLASH_VALIDATION_ATTN_TOKENS];
	uint32_t block_indices[1] = {0u};
	uint32_t block_counts[1] = {1u};
	uint32_t lane_zero[1] = {0u};
	uint32_t context[1] = {SPARK_QWEN4_FLASH_VALIDATION_ATTN_TOKENS};
	uint32_t token;
	cudaError_t error;
	SparkQwen4FlashValMetrics metrics;
	...
	if (q_packed == 0 || k_packed == 0 || v_packed == 0 || q_exact == 0 || k_exact == 0 ||
		...
		return(SparkQwen4FlashValFail("attn_decode","host_alloc"));
	SparkQwen4FlashValRandomState = 67u;
	SparkQwen4FlashValFillBf16(q_packed,q_exact,fused_elements,0.5f);
	SparkQwen4FlashValFillBf16(k_packed,k_exact,kv_elements,0.5f);
	SparkQwen4FlashValFillBf16(v_packed,v_exact,kv_elements,0.5f);
	SparkQwen4FlashValFillBf16(norm_packed,q_norm_exact,SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION,0.5f);
	...
	if (error == cudaSuccess) error = cudaMalloc((void **)&device_out,SPARK_QWEN4_FLASH_MODEL_ATTN_QUERY_DIMENSION * sizeof(uint16_t));
	 ...
	 * RefAttention call per kv head over the newest fused query row. */
	{
		...
		if (k_cache == 0 || v_cache == 0)
			return(SparkQwen4FlashValFail("attn_decode","oracle_alloc"));
		for (kv_head = 0u; kv_head < SPARK_QWEN4_FLASH_MODEL_ATTN_KV_HEAD_COUNT; kv_head++)
		{
			for (token = 0u; token < tokens; token++)
			{
				const float *k_row = k_exact + (((uint64_t)token * SPARK_QWEN4_FLASH_MODEL_ATTN_KV_DIMENSION) + ((uint64_t)kv_head * SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION));
				float *cache_row = k_cache + ((uint64_t)token * SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION);
				SparkQwen4FlashValRmsNorm(k_row,k_norm_exact,cache_row,SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION,SPARK_QWEN4_FLASH_MODEL_RMS_NORM_EPSILON);
				SparkQwen4FlashValRope(cache_row,SPARK_QWEN4_FLASH_MODEL_ATTN_ROPE_DIMENSION,token,SPARK_QWEN4_FLASH_MODEL_ATTN_ROPE_THETA);
				for (element = 0u; element < SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION; element++)
					v_cache[((uint64_t)token * SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION) + element] =
						v_exact[((uint64_t)token * SPARK_QWEN4_FLASH_MODEL_ATTN_KV_DIMENSION) + ((uint64_t)kv_head * SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION) + element];
			}
			SparkQwen4FlashValAttention(
				q_exact + (((uint64_t)(tokens - 1u) * 2u * SPARK_QWEN4_FLASH_MODEL_ATTN_QUERY_DIMENSION) + ((uint64_t)kv_head * SPARK_QWEN4_FLASH_VALIDATION_ATTN_GROUP * 2u * SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION)),
				k_cache,v_cache,q_norm_exact,
				expected + ((uint64_t)kv_head * SPARK_QWEN4_FLASH_VALIDATION_ATTN_GROUP * SPARK_QWEN4_FLASH_MODEL_ATTN_HEAD_DIMENSION),
				SPARK_QWEN4_FLASH_VALIDATION_ATTN_GROUP,tokens,SPARK_QWEN4_FLASH_MODEL_RMS_NORM_EPSILON);
		}
		free(k_cache);
		free(v_cache);
	}
	for (token = 0u; token < SPARK_QWEN4_FLASH_MODEL_ATTN_QUERY_DIMENSION; token++)
		actual[token] = SparkQwen4FlashValFromBf16(out_packed[token]);
	SparkQwen4FlashValMeasure(&metrics,actual,expected,SPARK_QWEN4_FLASH_MODEL_ATTN_QUERY_DIMENSION);
...
static int SparkQwen4FlashValCheckGdnChunk(SparkQwen4FlashValDevice *device)
{
	...
	const uint64_t conv_elements = (uint64_t)tokens * SPARK_QWEN4_FLASH_CONV;
	const uint64_t out_elements = (uint64_t)tokens * SPARK_QWEN4_FLASH_MODEL_GDN_VALUE_DIMENSION;
	const uint64_t state_elements = (uint64_t)SPARK_QWEN4_FLASH_HEADS * SPARK_QWEN4_FLASH_DK * SPARK_QWEN4_FLASH_DV;
	...
	float *log_decay = (float *)calloc((uint64_t)tokens * SPARK_QWEN4_FLASH_HEADS,sizeof(float));
	float *beta = (float *)calloc((uint64_t)tokens * SPARK_QWEN4_FLASH_HEADS,sizeof(float));
	uint32_t head,token;
	uint64_t index;
	cudaError_t error;
	SparkQwen4FlashValMetrics metrics;
	if (conv_packed == 0 || exact == 0 || out_packed == 0 || actual == 0 || expected == 0 ||
		...
		return(SparkQwen4FlashValFail("gdn_chunk","host_alloc"));
	SparkQwen4FlashValRandomState = 83u;
	SparkQwen4FlashValFillBf16(conv_packed,exact,conv_elements,1.0f);
	for (index = 0u; index < (uint64_t)tokens * SPARK_QWEN4_FLASH_HEADS; index++)
	{
		log_decay[index] = SparkQwen4FlashValUniform(0.25f) - 0.25f;
		beta[index] = 0.25f + fabsf(SparkQwen4FlashValUniform(0.5f));
	}
	...
	for (uint32_t base = 0u; base < tokens && error == cudaSuccess; base += SPARK_QWEN4_FLASH_MODEL_GDN_CHUNK_TOKENS)
		error = SparkQwen4FlashLaunchGdnChunk(cudaStreamPerThread,
			...
			device->log_decay + ((uint64_t)base * SPARK_QWEN4_FLASH_HEADS),
			device->beta + ((uint64_t)base * SPARK_QWEN4_FLASH_HEADS),
	...
	if (SparkQwen4FlashValCuda(error,"gdn_chunk") != 0)
		return(1);
	for (head = 0u; head < SPARK_QWEN4_FLASH_HEADS; head++)
	{
		uint32_t key_head = head / SPARK_QWEN4_FLASH_MODEL_GDN_VALUE_HEADS_PER_KEY_HEAD;
		float *q_head = (float *)calloc((uint64_t)tokens * SPARK_QWEN4_FLASH_DK,sizeof(float));
		float *k_head = (float *)calloc((uint64_t)tokens * SPARK_QWEN4_FLASH_DK,sizeof(float));
		float *v_head = (float *)calloc((uint64_t)tokens * SPARK_QWEN4_FLASH_DV,sizeof(float));
		...
		if (q_head == 0 || k_head == 0 || v_head == 0 || g_head == 0 || b_head == 0)
			return(SparkQwen4FlashValFail("gdn_chunk","oracle_alloc"));
		for (token = 0u; token < tokens; token++)
		{
			uint64_t base = (uint64_t)token * SPARK_QWEN4_FLASH_CONV;
			memcpy(q_head + ((uint64_t)token * SPARK_QWEN4_FLASH_DK),exact + base + ((uint64_t)key_head * SPARK_QWEN4_FLASH_DK),SPARK_QWEN4_FLASH_DK * sizeof(float));
			memcpy(k_head + ((uint64_t)token * SPARK_QWEN4_FLASH_DK),exact + base + SPARK_QWEN4_FLASH_MODEL_GDN_QK_DIMENSION + ((uint64_t)key_head * SPARK_QWEN4_FLASH_DK),SPARK_QWEN4_FLASH_DK * sizeof(float));
			memcpy(v_head + ((uint64_t)token * SPARK_QWEN4_FLASH_DV),exact + base + (2ull * SPARK_QWEN4_FLASH_MODEL_GDN_QK_DIMENSION) + ((uint64_t)head * SPARK_QWEN4_FLASH_DV),SPARK_QWEN4_FLASH_DV * sizeof(float));
			g_head[token] = log_decay[((uint64_t)token * SPARK_QWEN4_FLASH_HEADS) + head];
			b_head[token] = beta[((uint64_t)token * SPARK_QWEN4_FLASH_HEADS) + head];
		}
		SparkQwen4FlashValGdnRecurrence(q_head,k_head,v_head,g_head,b_head,
			state_oracle + ((uint64_t)head * SPARK_QWEN4_FLASH_DK * SPARK_QWEN4_FLASH_DV),
	 ...
	 * re-map before comparing. */
	{
		...
		if (expected_ordered == 0)
			return(SparkQwen4FlashValFail("gdn_chunk","order_alloc"));
		for (token = 0u; token < tokens; token++)
			for (head = 0u; head < SPARK_QWEN4_FLASH_HEADS; head++)
				memcpy(expected_ordered + ((uint64_t)token * SPARK_QWEN4_FLASH_MODEL_GDN_VALUE_DIMENSION) + ((uint64_t)head * SPARK_QWEN4_FLASH_DV),
					expected + ((uint64_t)head * tokens * SPARK_QWEN4_FLASH_DV) + ((uint64_t)token * SPARK_QWEN4_FLASH_DV),
					SPARK_QWEN4_FLASH_DV * sizeof(float));
		for (index = 0u; index < out_elements; index++)
			actual[index] = SparkQwen4FlashValFromBf16(out_packed[index]);
		SparkQwen4FlashValMeasure(&metrics,actual,expected_ordered,out_elements);
		free(expected_ordered);
	}
	if (SparkQwen4FlashValReport("gdn_chunk_output",&metrics,2e-2,0.999) != 0)
		return(1);
	SparkQwen4FlashValMeasure(&metrics,state_device,state_oracle,state_elements);
...
typedef struct SparkQwen4FlashValModule
{
	...
	uint32_t head_stage;
	uint32_t lanes[SPARK_QWEN4_FLASH_VALIDATION_PREFILL_TOKENS];
	uint64_t positions[SPARK_QWEN4_FLASH_VALIDATION_PREFILL_TOKENS];
	uint64_t sequence_ids[SPARK_QWEN4_FLASH_VALIDATION_PREFILL_TOKENS];
	uint32_t host_blocks[SPARK_QWEN4_FLASH_VALIDATION_KV_LANES];
	uint32_t host_counts[SPARK_QWEN4_FLASH_VALIDATION_KV_LANES];
	uint32_t *device_blocks;
	uint32_t *device_counts;
	SparkQwen4FlashKvBlockTableView table;
	SparkQwen4FlashDecodeBatchView decode_batch;
	SparkQwen4FlashPrefillFrameView prefill_view;
	SparkQwen4FlashResidentDecodeStageFrameContext context;
	SparkModelDriverBuffer buffers[2];
	SparkModelDriverFrame frame;
	SparkQwen4FlashValCapture capture;
} SparkQwen4FlashValModule;

static int SparkQwen4FlashValModuleInitialize(SparkQwen4FlashValModule *module)
{
	...
	if (SparkQwen4FlashValCuda(error,"module_table_alloc") != 0)
		return(1);
	module->table.abi_version = SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_KV_BLOCK_TABLE_ABI_VERSION;
	module->table.descriptor_bytes = sizeof(module->table);
	module->table.block_token_count = SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS;
	module->table.lane_count = SPARK_QWEN4_FLASH_VALIDATION_KV_LANES;
	module->table.lane_stride = 1u;
	module->table.lane_capacity = SPARK_QWEN4_FLASH_VALIDATION_KV_LANES;
	...
	configuration.stage_name = "qwen4_flash_resident_decode_stage";
	configuration.program_name = "resident_decode";
	configuration.operation_name = "qwen4_flash_resident_decode_stage";
	...
	host_services.node_id = "spark-qwen4_flash-validator";
	host_services.node_target = "cuda.sm121.qwen4_flash.resident_decode_stage.fp8";
	host_services.execution_stream = (void *)cudaStreamPerThread;
	status = SparkQwen4FlashResidentDecodeStageInitialize(&configuration,&host_services,&module->state);
	if (status != SPARK_STATUS_OK)
	{
		fprintf(stderr,"qwen4_flash_validation failure=module_initialize status=%d\n",(int)status);
		return(1);
	}
	return(0);
}
...
static int SparkQwen4FlashValModuleExecute(SparkQwen4FlashValModule *module, uint32_t prefill, uint32_t rows, uint32_t lane, uint32_t draft_count, const SparkQwen4FlashMtpDraftView *draft_view)
{
	...
	module->context.abi_version = SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_ABI_VERSION;
	module->context.descriptor_bytes = sizeof(module->context);
	module->context.flags =
		SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_KV_BLOCK_TABLE |
		(module->head_stage == 0u
			? SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_HIDDEN_OUTPUT_TRANSPORT
			: 0u) |
		(prefill != 0u
			? SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_PREFILL_FRAME_VIEW
			: SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_DECODE_BATCH_VIEW) |
		(draft_count != 0u ? SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_MTP_DRAFT_AFTER : 0u);
	module->context.mtp_draft = draft_view;
	module->context.kv_block_table = &module->table;
	if (prefill != 0u)
	{
		module->prefill_view.abi_version = SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_PREFILL_FRAME_VIEW_ABI_VERSION;
	...
	{
		module->decode_batch.abi_version = SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_DECODE_BATCH_VIEW_ABI_VERSION;
	...
	module->context.hidden_output_send_function = module->head_stage != 0u ? 0 : SparkQwen4FlashValCaptureSend;
	...
	status = SparkQwen4FlashResidentDecodeStageExecute(module->state,&module->frame);
	if (status != SPARK_STATUS_OK)
	{
		fprintf(stderr,"qwen4_flash_validation failure=module_execute prefill=%u rows=%u status=%d\n",prefill,rows,(int)status);
		return(1);
	}
	return(0);
}

static int SparkQwen4FlashValCheckFinite(const char *check, const uint16_t *hidden, uint64_t rows)
{
	uint64_t index,count = rows * SPARK_QWEN4_FLASH_MODEL_HIDDEN_DIMENSION;
	for (index = 0u; index < count; index++)
		if (isfinite(SparkQwen4FlashValFromBf16(hidden[index])) == 0)
			return(SparkQwen4FlashValFail(check,"nonfinite"));
	return(0);
}
...
static int SparkQwen4FlashValCheckMtpDraft(SparkQwen4FlashValModule *module)
{
	...
	module->lanes[0] = 0u;
	module->positions[0] = SPARK_QWEN4_FLASH_VALIDATION_PREFILL_TOKENS + 1u;
	module->sequence_ids[0] = 1u;
	memset(&draft_view,0,sizeof(draft_view));
	draft_view.abi_version = SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_MTP_DRAFT_VIEW_ABI_VERSION;
	draft_view.descriptor_bytes = sizeof(draft_view);
	draft_view.lane_index = 0u;
	draft_view.draft_token_count = 2u;
	draft_view.base_position = (uint64_t)SPARK_QWEN4_FLASH_VALIDATION_PREFILL_TOKENS + 2u;
	draft_view.sequence_id = 1u;
	draft_view.row_token_ids = module->token_ids;
	if (SparkQwen4FlashValModuleExecute(module,0u,1u,0u,2u,&draft_view) != 0)
		return(1);
	for (draft = 0u; draft < 2u; draft++)
	{
		if (module->output_token_ids[1u + draft] >= SPARK_QWEN4_FLASH_MODEL_VOCAB_COUNT)
			return(SparkQwen4FlashValFail("module_mtp_draft","out_of_vocab"));
	}
	printf("qwen4_flash_validation check=module_mtp_draft in_vocab=1 drafts=[%u,%u]\n",module->output_token_ids[1],module->output_token_ids[2]);
	return(0);
}
