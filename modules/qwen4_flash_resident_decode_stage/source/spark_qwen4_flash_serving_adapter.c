...
static const char *const SparkQwen4FlashServingConfigurationMembers[] =
{
	"schema_version",
	"model_revision",
	"stage_pack_path",
...
static uint32_t SparkQwen4FlashServingPpStageIndex(const SparkQwen4FlashServingState *state, uint32_t world_rank)
{
	return(state->tp_degree != 0u ? world_rank / state->tp_degree : world_rank / SPARK_QWEN4_FLASH_SERVING_DEFAULT_TP_DEGREE);
}
...
static const SparkModelServingAdapterDescriptor SparkQwen4FlashServingDescriptor =
{
	...
	.adapter_id = SPARK_QWEN4_FLASH_SERVING_ADAPTER_ID,
	.model_id = SPARK_QWEN4_FLASH_SERVING_MODEL_ID,
...
static int32_t SparkQwen4FlashServingJsonMember(
...
static SparkStatus SparkQwen4FlashServingJsonUnsigned(
	const SparkJsonDocument *document,
	int32_t root,
	const char *name,
	uint32_t *value)
{
	int32_t token;
	token = SparkQwen4FlashServingJsonMember(document,root,name);
	return(token < 0 ? SPARK_STATUS_SCHEMA_ERROR : SparkJsonGetUInt32(document,token,value));
}

static SparkStatus SparkQwen4FlashServingLoadConfiguration(
	const char *path,
	...
	uint32_t *max_sequence_positions)
{
	...
	if ( status == SPARK_STATUS_OK )
		status = SparkJsonValidateObjectMembersExact(&document,root,SparkQwen4FlashServingConfigurationMembers,(uint32_t)(sizeof(SparkQwen4FlashServingConfigurationMembers) / sizeof(SparkQwen4FlashServingConfigurationMembers[0])));
	if ( status == SPARK_STATUS_OK )
		status = SparkQwen4FlashServingJsonUnsigned(&document,root,"schema_version",&schema_version);
	if ( status == SPARK_STATUS_OK && schema_version != SPARK_QWEN4_FLASH_SERVING_ADAPTER_CONFIGURATION_SCHEMA_VERSION )
		status = SPARK_STATUS_SCHEMA_ERROR;
	token = status == SPARK_STATUS_OK ? SparkQwen4FlashServingJsonMember(&document,root,"model_revision") : -1;
	if ( status == SPARK_STATUS_OK && (token < 0 || !SparkJsonStringEquals(&document,token,QWEN4_FLASH_MODEL_REVISION)) )
		status = SPARK_STATUS_SCHEMA_ERROR;
	if ( status == SPARK_STATUS_OK )
	...
	if ( status == SPARK_STATUS_OK )
		status = SparkQwen4FlashServingJsonUnsigned(&document,root,"max_sequence_positions",max_sequence_positions);
...
static uint32_t SparkQwen4FlashServingFirstLayer(const SparkQwen4FlashServingState *state, uint32_t stage_index)
{
	uint32_t index,first_layer,pp_stage;
	first_layer = 0u;
	pp_stage = SparkQwen4FlashServingPpStageIndex(state,stage_index);
	for (index=0u; index<pp_stage; index++)
		first_layer += state->stage_layer_counts[index];
	return(first_layer);
}

static uint32_t SparkQwen4FlashServingStageAttentionLayers(uint32_t first_layer, uint32_t layer_count)
{
	uint32_t layer,count;
	count = 0u;
	for (layer=first_layer; layer<first_layer+layer_count; layer++)
		count += SPARK_QWEN4_FLASH_MODEL_LAYER_IS_GDN(layer) == 0u ? 1u : 0u;
	return(count);
}

static SparkStatus SparkQwen4FlashServingSetEnvironment(
	const SparkQwen4FlashServingState *state)
{
	char value[32];
#define SPARK_QWEN4_FLASH_SERVING_SET_TEXT(name,text) \
	do { if ( setenv(name,text,1) != 0 ) return(SPARK_STATUS_INTERNAL_ERROR); } while (0)
#define SPARK_QWEN4_FLASH_SERVING_SET_UNSIGNED(name,number) \
	...
	SPARK_QWEN4_FLASH_SERVING_SET_UNSIGNED("SPARK_QWEN4_FLASH_STAGE_PIPELINE_SLOTS",state->pipeline_slot_count);
	SPARK_QWEN4_FLASH_SERVING_SET_UNSIGNED("SPARK_QWEN4_FLASH_STAGE_KV_BLOCKS",state->kv_block_count);
	SPARK_QWEN4_FLASH_SERVING_SET_TEXT("SPARK_QWEN4_FLASH_STAGE_MTP","0");
	SPARK_QWEN4_FLASH_SERVING_SET_TEXT("SPARK_QWEN4_FLASH_STAGE_GDN_SNAPSHOT_SLOTS","0");
	SPARK_QWEN4_FLASH_SERVING_SET_TEXT("SPARK_QWEN4_FLASH_STAGE_KV_STORE","none");
	SPARK_QWEN4_FLASH_SERVING_SET_TEXT("SPARK_QWEN4_FLASH_STAGE_KV_SERVICE","none");
	SPARK_QWEN4_FLASH_SERVING_SET_TEXT("SPARK_QWEN4_FLASH_STAGE_KV_SOCKET","none");
	SPARK_QWEN4_FLASH_SERVING_SET_TEXT("SPARK_QWEN4_FLASH_STAGE_KV_POOL_BYTES","0");
	SPARK_QWEN4_FLASH_SERVING_SET_TEXT("SPARK_QWEN4_FLASH_STAGE_KV_WORKERS","0");
#undef SPARK_QWEN4_FLASH_SERVING_SET_TEXT
#undef SPARK_QWEN4_FLASH_SERVING_SET_UNSIGNED
...
static SparkStatus SparkQwen4FlashServingValidateRowOrder(
	const SparkQwen4FlashServingState *state,
	const SparkModelServingSubmission *submission)
{
	uint8_t seen[SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT] = {0u};
	uint8_t slot_seen[SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT] = {0u};
	uint64_t last_position[SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT] = {0u};
	uint32_t lane,row,wave,maximum;
	uint32_t counts[SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT] = {0u};
...
static SparkStatus SparkQwen4FlashServingValidateBoundaries(
	const SparkQwen4FlashServingState *state,
	const SparkModelServingSubmission *submission)
{
	uint64_t boundary_bytes;
	uint32_t pp_stage;
	boundary_bytes = (uint64_t)submission->row_count * SPARK_QWEN4_FLASH_MODEL_HIDDEN_BF16_BYTES;
	pp_stage = SparkQwen4FlashServingPpStageIndex(state,state->stage_index);
	if ( (pp_stage != 0u && (submission->hidden_input_address == 0 || submission->hidden_input_bytes < boundary_bytes)) || (pp_stage == 0u && (submission->hidden_input_address != 0 || submission->hidden_input_bytes != 0u)) || (pp_stage + 1u < SparkQwen4FlashServingPpStageCount(state) && (submission->hidden_output_address == 0 || submission->hidden_output_bytes < boundary_bytes)) || (pp_stage + 1u == SparkQwen4FlashServingPpStageCount(state) && (submission->hidden_output_address != 0 || submission->hidden_output_bytes != 0u)) )
		return(SPARK_STATUS_CAPACITY_EXCEEDED);
	return(SPARK_STATUS_OK);
}

static SparkStatus SparkQwen4FlashServingValidateSubmissionBase(
	SparkQwen4FlashServingState *state,
	const SparkModelServingSubmission *submission)
{
	SparkStatus status;
	...
	status = SparkModelServingAdapterValidateRuntimeSubmission(&SparkQwen4FlashServingDescriptor,&state->runtime_limits,submission);
	if ( status != SPARK_STATUS_OK )
		return(status);
	if ( submission->boundary_sideband_input_address != 0 || submission->boundary_sideband_input_bytes != 0u || submission->boundary_sideband_output_address != 0 || submission->boundary_sideband_output_bytes != 0u )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	status = SparkQwen4FlashServingValidateRowOrder(state,submission);
	if ( status != SPARK_STATUS_OK )
		return(status);
...
static SparkStatus SparkQwen4FlashServingValidateSubmission(
	void *adapter_state,
	const SparkModelServingSubmission *submission)
{
	SparkQwen4FlashServingState *state;
	uint32_t emit_count;
	SparkStatus status;
	state = (SparkQwen4FlashServingState *)adapter_state;
	status = SparkQwen4FlashServingValidateSubmissionBase(state,submission);
	if ( status != SPARK_STATUS_OK )
		return(status);
	return(SparkModelServingAdapterSelectEmitRows(submission,0,0,0u,&emit_count));
}

static SparkQwen4FlashServingPending *SparkQwen4FlashServingReservePending(
	SparkQwen4FlashServingState *state,
	const SparkModelServingSubmission *submission)
{
	SparkQwen4FlashServingPending *pending;
...
static void SparkQwen4FlashServingOrphanDriverCompletion(
	void *completion_context,
	const SparkModelDriverCompletion *driver_completion)
{
	SparkQwen4FlashServingState *state;
	(void)driver_completion;
	state = (SparkQwen4FlashServingState *)completion_context;
	if ( state != 0 )
		state->orphan_completion_count++;
}

static void SparkQwen4FlashServingDriverCompletion(
	void *completion_context,
	const SparkModelDriverCompletion *driver_completion)
{
	SparkQwen4FlashServingPending *pending;
	SparkQwen4FlashServingState *state;
	uint32_t matches;
	pending = (SparkQwen4FlashServingPending *)completion_context;
...
static void SparkQwen4FlashServingDriverWake(void *wake_context)
{
	SparkQwen4FlashServingState *state;
	state = (SparkQwen4FlashServingState *)wake_context;
	if ( state != 0 && state->wake_function != 0 )
		state->wake_function(state->wake_context);
}

static uint32_t SparkQwen4FlashServingAvailableSubmissionCount(
	const SparkQwen4FlashServingState *state)
{
...
static void SparkQwen4FlashServingReleaseLane(
	SparkQwen4FlashServingState *state,
	uint32_t slot)
{
	uint32_t ordinal;
...
static SparkStatus SparkQwen4FlashServingCoverLane(
	SparkQwen4FlashServingState *state,
	uint32_t slot,
	uint64_t end_position)
{
	...
	if ( state->stage_attn_layer_count == 0u )
		return(SPARK_STATUS_OK);
	required = (uint32_t)((end_position + SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS - 1u) / SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS);
...
static void SparkQwen4FlashServingDropSubmission(
	SparkQwen4FlashServingState *state,
	const SparkModelServingSubmission *submission)
{
	uint32_t lane;
	for (lane=0u; lane<submission->active_sequence_count; lane++)
		SparkQwen4FlashServingReleaseLane(state,submission->lanes[lane].resident_sequence_slot);
}

static SparkStatus SparkQwen4FlashServingCoverSubmission(
	SparkQwen4FlashServingState *state,
	const SparkModelServingSubmission *submission)
{
	uint32_t lane,row;
	...
	for (lane=0u; lane<submission->active_sequence_count; lane++)
	{
		...
		if ( first_position == 0u && state->lane_context_tokens[slot] != 0u )
			SparkQwen4FlashServingReleaseLane(state,slot);
		status = SparkQwen4FlashServingCoverLane(state,slot,end_position);
		if ( status != SPARK_STATUS_OK )
		{
			/* Coverage failure is KV exhaustion: drop every lane the
			 * submission touches so a partial allocation cannot linger. */
			SparkQwen4FlashServingDropSubmission(state,submission);
			return(status);
		}
	}
	return(SPARK_STATUS_OK);
...
static void SparkQwen4FlashServingCommitSubmission(
	SparkQwen4FlashServingState *state,
	const SparkModelServingSubmission *submission)
{
	uint32_t lane,row;
...
static SparkStatus SparkQwen4FlashServingUploadBlockTable(
	const SparkQwen4FlashServingState *state,
	const SparkModelServingSubmission *submission)
{
...
static SparkStatus SparkQwen4FlashServingPostReceive(
	...
	SparkHiddenTransportPacket *packet)
{
	...
	const void *source;
	uint32_t row;
	shim = (SparkQwen4FlashServingTransportShim *)transport_session;
	...
	if ( shim->input_row_map != 0 )
	{
		for (row=0u; row<shim->input_rows; row++)
			if ( cudaMemcpyAsync((uint8_t *)shim->input_scratch + ((uint64_t)row * SPARK_QWEN4_FLASH_MODEL_HIDDEN_BF16_BYTES),(const uint8_t *)shim->input_base + ((uint64_t)shim->input_row_map[row] * SPARK_QWEN4_FLASH_MODEL_HIDDEN_BF16_BYTES),SPARK_QWEN4_FLASH_MODEL_HIDDEN_BF16_BYTES,cudaMemcpyDeviceToDevice,(cudaStream_t)shim->execution_stream) != cudaSuccess )
				return(SPARK_STATUS_IO_ERROR);
	...
	packet->bytes_per_sequence = SPARK_QWEN4_FLASH_MODEL_HIDDEN_BF16_BYTES;
	packet->hidden_bf16 = source;
	packet->cuda_stream = shim->execution_stream;
	return(SPARK_STATUS_OK);
}

static SparkStatus SparkQwen4FlashServingSend(
	SparkHiddenTransportSession *transport_session,
	const SparkHiddenTransportPacket *packet)
{
	...
	uint32_t row;
	shim = (SparkQwen4FlashServingTransportShim *)transport_session;
	if ( shim == 0 || packet == 0 || packet->hidden_bf16 == 0 || shim->output_base == 0 || packet->active_sequence_count == 0u || packet->hidden_dimension != SPARK_QWEN4_FLASH_MODEL_HIDDEN_DIMENSION )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	if ( shim->output_row_map != 0 )
	{
		for (row=0u; row<packet->active_sequence_count; row++)
			if ( cudaMemcpyAsync((uint8_t *)shim->output_base + ((uint64_t)shim->output_row_map[row] * SPARK_QWEN4_FLASH_MODEL_HIDDEN_BF16_BYTES),(const uint8_t *)packet->hidden_bf16 + ((uint64_t)row * SPARK_QWEN4_FLASH_MODEL_HIDDEN_BF16_BYTES),SPARK_QWEN4_FLASH_MODEL_HIDDEN_BF16_BYTES,cudaMemcpyDeviceToDevice,(cudaStream_t)packet->cuda_stream) != cudaSuccess )
				return(SPARK_STATUS_IO_ERROR);
	}
	else if ( cudaMemcpyAsync(shim->output_base,packet->hidden_bf16,(uint64_t)packet->active_sequence_count * SPARK_QWEN4_FLASH_MODEL_HIDDEN_BF16_BYTES,cudaMemcpyDeviceToDevice,(cudaStream_t)packet->cuda_stream) != cudaSuccess )
		return(SPARK_STATUS_IO_ERROR);
	return(cudaStreamSynchronize((cudaStream_t)packet->cuda_stream) == cudaSuccess ? SPARK_STATUS_OK : SPARK_STATUS_IO_ERROR);
}

static void SparkQwen4FlashServingBuildFrame(
	SparkQwen4FlashServingState *state,
	const SparkModelServingSubmission *submission,
	SparkQwen4FlashServingPending *pending,
	uint32_t prefill,
	...
	SparkModelDriverFrame *frame)
{
	...
	context->abi_version = SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_ABI_VERSION;
	context->descriptor_bytes = sizeof(*context);
	if ( state->stage_attn_layer_count != 0u )
	{
		context->flags |= SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_KV_BLOCK_TABLE;
		context->kv_block_table = &state->block_table;
	}
	if ( SparkQwen4FlashServingPpStageIndex(state,state->stage_index) != 0u )
	{
		context->flags |= SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_HIDDEN_INPUT_TRANSPORT;
		context->hidden_input_transport_session = (SparkHiddenTransportSession *)&state->shim;
		context->hidden_input_post_receive_function = SparkQwen4FlashServingPostReceive;
	}
	if ( SparkQwen4FlashServingPpStageIndex(state,state->stage_index) + 1u < SparkQwen4FlashServingPpStageCount(state) )
	{
		context->flags |= SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_HIDDEN_OUTPUT_TRANSPORT;
		context->hidden_output_transport_session = (SparkHiddenTransportSession *)&state->shim;
		context->hidden_output_send_function = SparkQwen4FlashServingSend;
	}
	...
	if ( prefill != 0u )
	{
		...
		prefill_view->abi_version = SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_PREFILL_FRAME_VIEW_ABI_VERSION;
		...
		context->flags |= SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_PREFILL_FRAME_VIEW;
	...
	else
	{
		...
		decode_batch->abi_version = SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_DECODE_BATCH_VIEW_ABI_VERSION;
		...
		context->flags |= SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_DECODE_BATCH_VIEW;
	...
	{
		uint32_t out_index;
		out_index = SparkQwen4FlashServingPpStageIndex(state,state->stage_index) == 0u ? 1u : 0u;
	...
	frame->buffers = SparkQwen4FlashServingPpStageIndex(state,state->stage_index) == 0u || SparkQwen4FlashServingPpStageIndex(state,state->stage_index) + 1u == SparkQwen4FlashServingPpStageCount(state) ? buffers : 0;
	frame->buffer_count = (SparkQwen4FlashServingPpStageIndex(state,state->stage_index) == 0u ? 1u : 0u) + (SparkQwen4FlashServingPpStageIndex(state,state->stage_index) + 1u == SparkQwen4FlashServingPpStageCount(state) ? 1u : 0u);
	frame->residency = submission->residency;
	frame->user_context = context;
	frame->completion_function = SparkQwen4FlashServingDriverCompletion;
...
static SparkStatus SparkQwen4FlashServingAdmit(
	SparkQwen4FlashServingState *state,
	const SparkModelServingSubmission *submission,
	SparkModelDriverFrame *frame)
{
...
static SparkStatus SparkQwen4FlashServingRunFrame(
	SparkQwen4FlashServingState *state,
	const SparkModelServingSubmission *submission,
	SparkQwen4FlashServingPending *pending,
	uint32_t prefill,
	...
	uint32_t frame_rows)
{
	...
	SparkModelDriverFrame frame;
	SparkStatus status;
	SparkQwen4FlashServingBuildFrame(state,submission,pending,prefill,lane,wave_base,frame_rows,&decode_batch,&prefill_view,&context,buffers,&frame);
	status = SparkQwen4FlashServingAdmit(state,submission,&frame);
	if ( status == SPARK_STATUS_OK )
		status = state->program->submit(state->driver_instance,&frame);
	if ( status == SPARK_STATUS_OK )
		status = pending->frame_status;
	if ( status == SPARK_STATUS_OK && SparkQwen4FlashServingPpStageIndex(state,state->stage_index) + 1u == SparkQwen4FlashServingPpStageCount(state) )
	{
		if ( prefill != 0u )
...
static void SparkQwen4FlashServingComplete(
	SparkQwen4FlashServingState *state,
	SparkQwen4FlashServingPending *pending,
	SparkStatus status)
{
	...
	if ( SparkQwen4FlashServingPpStageIndex(state,state->stage_index) + 1u == SparkQwen4FlashServingPpStageCount(state) && status == SPARK_STATUS_OK )
	{
...
static SparkStatus SparkQwen4FlashServingSubmit(
	void *adapter_state,
	const SparkModelServingSubmission *submission)
{
	SparkQwen4FlashServingState *state;
	...
	if ( status == SPARK_STATUS_OK && submission->work_kind != SPARK_MODEL_SERVING_WORK_KIND_RELEASE )
		status = SparkQwen4FlashServingValidateBoundaries(state,submission);
	if ( status != SPARK_STATUS_OK )
		return(status);
	pending = SparkQwen4FlashServingReservePending(state,submission);
	if ( pending == 0 )
		return(SPARK_STATUS_BUSY);
	status = SparkQwen4FlashServingCoverSubmission(state,submission);
	if ( status == SPARK_STATUS_OK )
		status = SparkQwen4FlashServingUploadBlockTable(state,submission);
	if ( status == SPARK_STATUS_OK && submission->work_kind == SPARK_MODEL_SERVING_WORK_KIND_DECODE )
		status = SparkQwen4FlashServingRunFrame(state,submission,pending,0u,0u,0u,submission->row_count);
	else if ( status == SPARK_STATUS_OK && submission->work_kind == SPARK_MODEL_SERVING_WORK_KIND_PREFILL )
	{
		...
		for (lane=0u; status == SPARK_STATUS_OK && lane<submission->active_sequence_count; lane++)
		{
			...
			for (wave=0u; status == SPARK_STATUS_OK && wave<lane_rows; wave+=chunk_rows)
			{
				...
				status = SparkQwen4FlashServingRunFrame(state,submission,pending,1u,lane,wave,chunk_rows);
			}
		}
	}
	...
	SparkQwen4FlashServingCommitSubmission(state,submission);
	SparkQwen4FlashServingComplete(state,pending,SPARK_STATUS_OK);
	return(SPARK_STATUS_OK);
}

static SparkStatus SparkQwen4FlashServingProgress(
	void *adapter_state,
...
static SparkStatus SparkQwen4FlashServingQuiesce(
	void *adapter_state,
	uint64_t deadline_time_ns)
{
	...
	if ( state == 0 || deadline_time_ns == 0u )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	state->quiescing = 1u;
	if ( SparkQwen4FlashServingAvailableSubmissionCount(state) != state->pipeline_slot_count )
		return(SPARK_STATUS_BUSY);
...
static SparkStatus SparkQwen4FlashServingSnapshot(
	void *adapter_state,
	SparkModelServingAdapterSnapshot *snapshot)
{
	...
	available = SparkQwen4FlashServingAvailableSubmissionCount(state);
	...
	snapshot->active_submission_count = state->pipeline_slot_count - SparkQwen4FlashServingAvailableSubmissionCount(state);
...
static void SparkQwen4FlashServingDestroy(void *adapter_state)
{
	SparkQwen4FlashServingState *state;
	SparkModelDriverRuntimeSnapshot snapshot;
	state = (SparkQwen4FlashServingState *)adapter_state;
...
static SparkStatus SparkQwen4FlashServingLoadDriver(
	SparkQwen4FlashServingState *state,
	const SparkModelServingAdapterConfiguration *configuration)
{
	...
	if ( descriptor == 0 || strcmp(descriptor->model_id,SPARK_QWEN4_FLASH_SERVING_DRIVER_MODEL_ID) != 0 || strcmp(descriptor->model_revision,QWEN4_FLASH_MODEL_REVISION) != 0 || strcmp(descriptor->stage_name,SPARK_QWEN4_FLASH_SERVING_STAGE_NAME) != 0 || strcmp(descriptor->target,SPARK_QWEN4_FLASH_SERVING_TARGET) != 0 || strcmp(descriptor->model_description_sha256,QWEN4_FLASH_CONTRACT_SHA256) != 0 )
	...
	request.completion_function = SparkQwen4FlashServingOrphanDriverCompletion;
	request.completion_context = state;
	request.wake_function = SparkQwen4FlashServingDriverWake;
...
static SparkStatus SparkQwen4FlashServingAllocatePools(
	SparkQwen4FlashServingState *state)
{
	uint32_t block;
	uint64_t indices;
	...
	if ( cudaMalloc(&state->gather_scratch,(size_t)((uint64_t)state->max_active_sequence_count * SPARK_QWEN4_FLASH_MODEL_HIDDEN_BF16_BYTES)) != cudaSuccess )
		return(SPARK_STATUS_CAPACITY_EXCEEDED);
	if ( state->stage_attn_layer_count != 0u )
	{
	...
	state->block_table.abi_version = SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_KV_BLOCK_TABLE_ABI_VERSION;
	state->block_table.descriptor_bytes = sizeof(state->block_table);
	state->block_table.block_token_count = SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS;
...
static SparkStatus SparkQwen4FlashServingValidateConfiguration(
	const SparkModelServingAdapterConfiguration *configuration)
{
	SparkStatus status;
	if ( configuration == 0 )
	...
	void **adapter_state)
{
	...
	if ( adapter_state == 0 )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	*adapter_state = 0;
	status = SparkQwen4FlashServingValidateConfiguration(configuration);
	if ( status != SPARK_STATUS_OK )
		return(status);
	state = (SparkQwen4FlashServingState *)calloc(1u,sizeof(*state));
	if ( state == 0 )
		return(SPARK_STATUS_CAPACITY_EXCEEDED);
	state->stage_index = configuration->stage_index;
	state->tp_degree = SPARK_QWEN4_FLASH_SERVING_DEFAULT_TP_DEGREE;
	...
	status = SparkQwen4FlashServingLoadConfiguration(configuration->adapter_configuration_path,configuration->runtime_root,state,&max_sequence_positions);
	if ( status == SPARK_STATUS_OK && (max_sequence_positions == 0u || max_sequence_positions > SPARK_QWEN4_FLASH_SERVING_MAX_SEQUENCE_POSITIONS_CAP) )
		status = SPARK_STATUS_SCHEMA_ERROR;
	if ( status == SPARK_STATUS_OK )
	{
		...
		state->first_layer_index = SparkQwen4FlashServingFirstLayer(state,configuration->stage_index);
		state->stage_layer_count = state->stage_layer_counts[SparkQwen4FlashServingPpStageIndex(state,configuration->stage_index)];
		state->stage_attn_layer_count = SparkQwen4FlashServingStageAttentionLayers(state->first_layer_index,state->stage_layer_count);
		state->max_sequence_positions = max_sequence_positions;
		state->blocks_per_lane = (max_sequence_positions + SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS - 1u) / SPARK_QWEN4_FLASH_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS;
		state->kv_block_count = state->resident_sequence_capacity * state->blocks_per_lane;
		status = SparkQwen4FlashServingAllocatePools(state);
		state->shim.input_scratch = state->gather_scratch;
	}
	if ( status == SPARK_STATUS_OK )
		status = SparkQwen4FlashServingSetEnvironment(state);
	if ( status == SPARK_STATUS_OK )
		status = SparkQwen4FlashServingLoadDriver(state,configuration);
	if ( status != SPARK_STATUS_OK )
	{
		SparkQwen4FlashServingDestroy(state);
		return(status);
	}
	*adapter_state = state;
	return(SPARK_STATUS_OK);
...
static const SparkModelServingAdapterInterface SparkQwen4FlashServingInterface =
{
	.abi_version = SPARK_MODEL_SERVING_ADAPTER_ABI_VERSION,
	.interface_bytes = SPARK_MODEL_SERVING_ADAPTER_INTERFACE_BYTES,
	.descriptor = &SparkQwen4FlashServingDescriptor,
	.initialize = SparkQwen4FlashServingInitialize,
	.destroy = SparkQwen4FlashServingDestroy,
	.validate_submission = SparkQwen4FlashServingValidateSubmission,
	.submit = SparkQwen4FlashServingSubmit,
	.progress = SparkQwen4FlashServingProgress,
	.quiesce = SparkQwen4FlashServingQuiesce,
	.snapshot = SparkQwen4FlashServingSnapshot
};

__attribute__((visibility("default")))
const SparkModelServingAdapterInterface *SparkModelServingAdapterGetInterface(void)
{
	return(&SparkQwen4FlashServingInterface);
}
