...
typedef struct SparkLagunaSynthesizeContext
{
	SparkLagunaStagePackEntry entries[SPARK_LAGUNA_SYNTHESIZE_MAX_TENSORS];
...
#define SPARK_SYNTH_CONTEXT_T SparkLagunaSynthesizeContext
#define SPARK_SYNTH_CHUNK_BYTES SPARK_LAGUNA_SYNTHESIZE_CHUNK_BYTES
#define SPARK_SYNTH_ALIGN_UNIT SPARK_LAGUNA_STAGEPACK_ALIGNMENT_BYTES
#define SPARK_SYNTH_ENTRY_T SparkLagunaStagePackEntry
#define SPARK_SYNTH_PAYLOAD_KIND(entry) ((entry)->payload_type)
#define SPARK_SYNTH_PACKED_FORMAT SPARK_LAGUNA_STAGEPACK_PAYLOAD_PACKED_WEIGHT
#define SPARK_SYNTH_PACKED_NAN_MASK 0x7Fu
#define SPARK_SYNTH_F32_FORMAT SPARK_LAGUNA_STAGEPACK_PAYLOAD_F32
...
static int32_t SparkLagunaSynthesizeBuild(SparkLagunaSynthesizeContext *context)
{
	uint32_t layer;
	...
	{
		if ( SparkLagunaSynthesizeAppend(context,SPARK_LAGUNA_STAGEPACK_TENSOR_FINAL_NORM,SPARK_LAGUNA_STAGEPACK_GLOBAL_LAYER) < 0 )
			return(-4);
...
int main(int argc, char **argv)
{
	...
	status = SparkLagunaSynthesizeBuild(&context);
	if ( status != 0 )
	{
	...
	chunk = (uint8_t *)malloc(SPARK_LAGUNA_SYNTHESIZE_CHUNK_BYTES);
	if ( chunk == 0 )
	{
		fclose(file);
		return(1);
	...
	header.magic = SPARK_LAGUNA_STAGEPACK_MAGIC;
	header.format_version = SPARK_LAGUNA_STAGEPACK_FORMAT_VERSION;
	...
	header.total_layer_count = SPARK_LAGUNA_MODEL_LAYER_COUNT;
	header.hidden_dimension = SPARK_LAGUNA_MODEL_HIDDEN_DIMENSION;
	header.vocab_count = SPARK_LAGUNA_MODEL_OUTPUT_VOCAB_COUNT;
	header.routed_expert_count = SPARK_LAGUNA_MODEL_MOE_EXPERT_COUNT;
	...
	if ( contract != 0 )
		SparkLagunaSynthesizeHexParse(contract,header.contract_sha256,SPARK_LAGUNA_STAGEPACK_SHA256_BYTES);
	directory_offset = SparkSynthAlign(SPARK_LAGUNA_STAGEPACK_HEADER_BYTES);
