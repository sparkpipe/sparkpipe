...
typedef struct SparkLagunaCudaWave
{
	...
	const SparkLagunaLayerWeights *mtp_layer_weights;
...
typedef struct SparkLagunaMtpDraftOps
{
	void *context;
...
int32_t SparkLagunaLaunchCudaMtpDraft(const SparkLagunaCudaWave *wave,const SparkLagunaMtpDraftOps *ops,uint16_t *committed_hidden_bf16,uint32_t first_token,uint32_t *host_draft_tokens);
int32_t SparkLagunaLaunchCudaMtpCommit(const SparkLagunaCudaWave *wave,uint32_t committed_steps);
