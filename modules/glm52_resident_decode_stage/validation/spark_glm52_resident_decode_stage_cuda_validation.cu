...
/* Full routed population (group_row_offset has EXPERTS+1 entries; the
 * validator reads them all to invert a packed row to its owning expert). */
#define SPARK_GLM52_VEXPERTS SPARK_GLM52_MODEL_MOE_EXPERT_COUNT
	...
	uint32_t expert,uint32_t row,uint32_t rows,uint32_t columns,uint32_t column)
{
	...
	/* The payload buffer is expert-major: one rows*columns slab per expert.
	 * The expert argument MUST select the slab before any byte is decoded -
	 * ReadCode addresses within one slab only. This offset was missing, so
	 * every oracle expert silently read expert 0's codes (tier-2a bug #8:
	 * device grouped-GEMM rows were compared against the wrong weights). */
	const uint8_t *expert_payload = payload +
		(uint64_t)expert * SparkGlm52ValPayloadBytesPerExpert(codec,rows,columns);
	float raw,scale;
	if ( codec == SPARK_GLM52_VAL_CODEC_NVFP4 )
	{
		...
		raw = SparkGlm52ValE2m1Decode((uint8_t)SparkGlm52ValReadCode(expert_payload,codec,row,columns,column));
	}
	...
	else
	{
		...
		if ( codec == SPARK_GLM52_VAL_CODEC_FP8 )
			raw = SparkGlm52ValE4m3Decode((uint8_t)SparkGlm52ValReadCode(expert_payload,codec,row,columns,column));
		else
			raw = (float)SparkGlm52ValReadCode(expert_payload,codec,row,columns,column);
	}
...
static int SparkGlm52ValCompareStreams(SparkGlm52ValFixture *fixture,SparkGlm52ValOracle *oracle,
	const char *label,int *result,double max_relative_l2)
{
	...
	local_result = SparkGlm52ValReport(label,&metrics,max_relative_l2,0.999);
	for (index = 0u; index < SPARK_GLM52_VHIDDEN; index++)
	{
	...
	if ( local_result == 0 )
	{
		...
		local_result = SparkGlm52ValReport(residual_label,&metrics,max_relative_l2,0.999);
	}
...
static int SparkGlm52ValRunRoutedTier(SparkGlm52ValFixture *fixture,int *result)
{
	...
	uint32_t slot_owner[SPARK_GLM52_VEXPERTS + 1u];
	...
	for (token = 0u; token < 2u; token++)
	{
		...
		if ( token == 0u )
		{
			...
			if ( cudaMemcpy(slot_owner,fixture->group_row_offset,(uint64_t)(SPARK_GLM52_VEXPERTS + 1u) * sizeof(uint32_t),
				cudaMemcpyDeviceToHost) != cudaSuccess )
			{
				free(expert_row_zero);
				return(SparkGlm52ValFail("routed_expert","readback"));
			}
			for (owner_expert = SPARK_GLM52_VEXPERTS; owner_expert > 0u && slot_owner[owner_expert] > packed_row_zero; owner_expert--)
				;
	 ...
	 * expert's forward recomputes from it directly. */
	{
		...
		SparkGlm52ValOracleRoutedMlp(&oracle,fixture,selected,weights);
		SparkGlm52ValExpertGemmRow(fixture->experts.w1_payload,fixture->experts.w1_scales,owner_expert,
			oracle.normed,oracle.gate_up,SPARK_GLM52_VW1_ROWS,SPARK_GLM52_VEXPERT_COLUMNS,SPARK_GLM52_VW1_ROWS);
		for (index = 0u; index < SPARK_GLM52_VEXPERT_INTER; index++)
		{
		...
		printf("glm52_validation check=routed_expert_owner slot=%u owner=%u\n",
		...
		SparkGlm52ValMeasure(&metrics,actual,reference,SPARK_GLM52_VHIDDEN);
		/* 2e-2, not tighter: with correct pairing the forward measures
		 * rel_l2 4.97e-3 / cosine 0.999996 (fp32 accumulation-order
		 * freedom against one bf16 store rounding). The threshold stays
		 * well above that noise floor while still gating functional
		 * divergence - the expert mispairing this tier used to report
		 * measured 0.336. */
	...
	/* The routed-layer stream tolerance sits at 3e-1, not the dense tier's
	 * 2e-2: the mixture hidden is an fp32 weighted sum of eight ~7e5-scale
	 * bf16 rows whose realization (summation order, FMA contraction in the
	 * oracle restatement itself) moves rel_l2 between 0.036 and 0.244 with
	 * cosine 0.985..1.000 - measured by recompiling this oracle with an
	 * order-only change (reversed route accumulation). The gate keeps its
	 * teeth through the cosine floor and the tier-2a element-wise check:
	 * the expert-mispairing defect this tier used to mask measures
	 * rel_l2=0.336 with cosine 0.9426, which fails both. */
	status = SparkGlm52ValCompareStreams(fixture,&oracle,"routed_layer_forward",&local_result,3e-1);
	if ( status != 0 )
		return(status);
...
static int SparkGlm52ValSelftestCodecRoundTrip(void)
{
	...
	/* Dequant of a written slab must reproduce the encoded grid exactly. */
	{
		...
		for (codec = 2u; codec <= 7u; codec++)
		{
			...
			/* Two slabs: expert 1 is decoded, so its slab must live at the
			 * expert-major offset the dequant applies. */
			...
			if ( payload == 0 || scales == 0 )
				return(1);
			SparkGlm52ValFillExpertSlab(codec,payload + payload_bytes,
