...
static int32_t SparkBenchInitRoutes(int32_t *slots_device, float *weights_device, uint32_t tokens, uint32_t groups)
{
    ...
    for (i=0; i<routes; i++)
    {
        slots[i] = (int32_t)(i % groups);
        weights[i] = 1.0f / (float)SPARK_GLM52_SOTA_TOP_K;
    }
    if (SparkBenchCuda(cudaMemcpy(slots_device, slots.data(), routes * sizeof(int32_t), cudaMemcpyHostToDevice), "cudaMemcpy slots") < 0)
        return -1;
    if (SparkBenchCuda(cudaMemcpy(weights_device, weights.data(), routes * sizeof(float), cudaMemcpyHostToDevice), "cudaMemcpy weights") < 0)
        return -2;
    return 0;
}

static int32_t SparkBenchInitCutlassGemm(SparkGlm52SotaNvfp4GroupedGemmPlanSm121 *gemm, uint32_t kind, uint32_t m, uint32_t n_capacity, uint32_t k, uint32_t groups, uint64_t cap, const uint8_t *a, const uint8_t *sfa, const uint8_t *b, const uint8_t *sfb, const void *c, void *d, int32_t *tokens_device, int32_t *tokens_host, void *workspace, uint64_t workspace_bytes, void *state, uint64_t state_bytes, cudaStream_t stream)
{
    ...
    gemm->capability_flags = cap | SPARK_GLM52_SOTA_FAST_CAP_SM121_NVFP4_SCALE_LAYOUT;
...
static int32_t SparkBenchRun(SparkGlm52SotaProductionMoePlanSm121 *plan, SparkGlm52SotaMoeArguments *arguments, uint32_t warmup, uint32_t iterations, cudaStream_t stream)
{
    cudaEvent_t start_event;
    cudaEvent_t stop_event;
    float elapsed_ms;
    double gate_flops,down_flops,total_flops,tflops;
    uint32_t i;
    ...
    if (SparkBenchCuda(cudaMemcpy(&overflow, plan->overflow_flag, sizeof(overflow), cudaMemcpyDeviceToHost), "cudaMemcpy overflow") < 0)
        return -10;
    gate_flops = (2.0 * (double)plan->maximum_bound_experts * (double)plan->gate_up_gemm.cutlass_m * (double)plan->gate_up_gemm.cutlass_n_capacity * (double)plan->gate_up_gemm.cutlass_k);
    down_flops = (2.0 * (double)plan->maximum_bound_experts * (double)plan->down_gemm.cutlass_m * (double)plan->down_gemm.cutlass_n_capacity * (double)plan->down_gemm.cutlass_k);
    total_flops = (gate_flops + down_flops);
    tflops = (total_flops * (double)iterations) / ((double)elapsed_ms * 1.0e9);
    printf("cutlass_fixed_moe_ready=1 active_tokens=%u active_routes=%u bound_experts=%u route_capacity_per_expert=%u fixed_capacity_routes=%u iterations=%u total_ms=%.3f avg_us=%.3f estimated_tflops=%.3f overflow=%u\n", arguments->active_token_count, arguments->active_route_count, plan->maximum_bound_experts, plan->gate_up_gemm.cutlass_n_capacity, plan->maximum_bound_experts * plan->gate_up_gemm.cutlass_n_capacity, iterations, elapsed_ms, (elapsed_ms * 1000.0f) / (float)iterations, tflops, overflow);
...
int main(int argc, char **argv)
{
    ...
    uint32_t route_capacity = 8u;
    uint32_t warmup = 5u;
    uint32_t iterations = 20u;
    uint32_t workspace_mb = 256u;
    uint32_t routes;
    uint64_t route_rows;
    ...
    uint8_t *hidden_scales = 0;
    int32_t *topk_slots = 0;
    float *topk_weights = 0;
    uint32_t *grouped_row = 0;
    uint32_t *overflow = 0;
    uint8_t *gate_a = 0,*gate_sfa = 0,*grouped_hidden = 0,*grouped_hidden_sfb = 0;
    __nv_bfloat16 *gate_c = 0,*gate_d = 0;
    uint8_t *down_a = 0,*down_sfa = 0,*intermediate = 0,*intermediate_sfb = 0;
    __nv_bfloat16 *down_c = 0,*down_d = 0,*combined = 0;
    ...
    tokens_host.assign(groups, (int32_t)route_capacity);
    ...
    if (SparkBenchCuda(cudaMalloc((void **)&tokens_device, groups * sizeof(int32_t)), "cudaMalloc tokens") < 0)
        return 8;
    if (SparkBenchDeviceAlloc((void **)&hidden, (uint64_t)tokens * SPARK_GLM52_SOTA_HIDDEN_DIMENSION * sizeof(__nv_bfloat16), 0, "cudaMalloc hidden") < 0)
        return 9;
    if (SparkBenchDeviceAlloc((void **)&hidden_payload, (uint64_t)tokens * (SPARK_GLM52_SOTA_HIDDEN_DIMENSION / 2u), 0, "cudaMalloc hidden payload") < 0)
        return 10;
    if (SparkBenchDeviceAlloc((void **)&hidden_scales, (uint64_t)tokens * SPARK_GLM52_SOTA_NVFP4_SCALE_GROUPS_HIDDEN, 0x38, "cudaMalloc hidden scales") < 0)
        return 11;
    ...
    if (SparkBenchInitRoutes(topk_slots, topk_weights, tokens, groups) < 0)
        return 14;
    if (SparkBenchDeviceAlloc((void **)&grouped_row, (uint64_t)routes * sizeof(uint32_t), 0, "cudaMalloc grouped row") < 0)
        return 15;
    ...
    if (SparkBenchDeviceAlloc((void **)&gate_sfa, SparkBenchSm1xxScaleBytes(groups, SPARK_GLM52_SOTA_MOE_INTERMEDIATE_DIMENSION * 2u, SPARK_GLM52_SOTA_HIDDEN_DIMENSION), 0x38, "cudaMalloc gate SFA") < 0)
        return 18;
    if (SparkBenchDeviceAlloc((void **)&grouped_hidden, route_rows * (SPARK_GLM52_SOTA_HIDDEN_DIMENSION / 2u), 0, "cudaMalloc grouped hidden") < 0)
        return 19;
    if (SparkBenchDeviceAlloc((void **)&grouped_hidden_sfb, SparkBenchSm1xxScaleBytes(groups, route_capacity, SPARK_GLM52_SOTA_HIDDEN_DIMENSION), 0x38, "cudaMalloc grouped hidden SFB") < 0)
        return 20;
    ...
    if (SparkBenchDeviceAlloc((void **)&intermediate, route_rows * (SPARK_GLM52_SOTA_MOE_INTERMEDIATE_DIMENSION / 2u), 0, "cudaMalloc intermediate") < 0)
        return 25;
    if (SparkBenchDeviceAlloc((void **)&intermediate_sfb, SparkBenchSm1xxScaleBytes(groups, route_capacity, SPARK_GLM52_SOTA_MOE_INTERMEDIATE_DIMENSION), 0x38, "cudaMalloc intermediate SFB") < 0)
        return 26;
    ...
    plan.maximum_routes = routes;
    plan.maximum_bound_experts = groups;
    plan.capability_flags = SPARK_GLM52_SOTA_FAST_CAP_TOKEN_QUANT_ONCE | SPARK_GLM52_SOTA_FAST_CAP_CUTLASS_NVFP4_GATE_UP | SPARK_GLM52_SOTA_FAST_CAP_FUSED_SILU_REQUANT | SPARK_GLM52_SOTA_FAST_CAP_CUTLASS_NVFP4_DOWN | SPARK_GLM52_SOTA_FAST_CAP_WEIGHTED_COMBINE | SPARK_GLM52_SOTA_FAST_CAP_NO_HOST_STAGING | SPARK_GLM52_SOTA_FAST_CAP_NO_DEVICE_MEMCPY | SPARK_GLM52_SOTA_FAST_CAP_SM121_NVFP4_SCALE_LAYOUT | SPARK_GLM52_SOTA_FAST_CAP_FIXED_GLM52_SHAPES;
    plan.grouped_row_by_route = grouped_row;
    plan.overflow_flag = overflow;
    plan.grouped_hidden_nvfp4_by_route.payload_u8 = grouped_hidden;
    plan.grouped_hidden_nvfp4_by_route.scale_e4m3_u8 = grouped_hidden_sfb;
    plan.grouped_hidden_nvfp4_by_route.row_count = (uint32_t)route_rows;
    plan.grouped_hidden_nvfp4_by_route.column_count = SPARK_GLM52_SOTA_HIDDEN_DIMENSION;
    plan.grouped_hidden_nvfp4_by_route.packed_row_stride_bytes = SPARK_GLM52_SOTA_HIDDEN_DIMENSION / 2u;
    plan.gate_up_bf16_by_grouped_route = gate_d;
    plan.intermediate_nvfp4_by_grouped_route.payload_u8 = intermediate;
    plan.intermediate_nvfp4_by_grouped_route.scale_e4m3_u8 = intermediate_sfb;
    plan.intermediate_nvfp4_by_grouped_route.row_count = (uint32_t)route_rows;
    plan.intermediate_nvfp4_by_grouped_route.column_count = SPARK_GLM52_SOTA_MOE_INTERMEDIATE_DIMENSION;
    plan.intermediate_nvfp4_by_grouped_route.packed_row_stride_bytes = SPARK_GLM52_SOTA_MOE_INTERMEDIATE_DIMENSION / 2u;
    plan.down_bf16_by_grouped_route = down_d;
    if (SparkBenchInitCutlassGemm(&plan.gate_up_gemm, SPARK_GLM52_SOTA_GEMM_KIND_GATE_UP, SPARK_GLM52_SOTA_MOE_INTERMEDIATE_DIMENSION * 2u, route_capacity, SPARK_GLM52_SOTA_HIDDEN_DIMENSION, groups, SPARK_GLM52_SOTA_FAST_CAP_CUTLASS_NVFP4_GATE_UP, gate_a, gate_sfa, grouped_hidden, grouped_hidden_sfb, gate_c, gate_d, tokens_device, tokens_host.data(), workspace, workspace_bytes, gate_state, state_bytes, stream) < 0)
        return 30;
    if (SparkBenchInitCutlassGemm(&plan.down_gemm, SPARK_GLM52_SOTA_GEMM_KIND_DOWN, SPARK_GLM52_SOTA_HIDDEN_DIMENSION, route_capacity, SPARK_GLM52_SOTA_MOE_INTERMEDIATE_DIMENSION, groups, SPARK_GLM52_SOTA_FAST_CAP_CUTLASS_NVFP4_DOWN, down_a, down_sfa, intermediate, intermediate_sfb, down_c, down_d, tokens_device, tokens_host.data(), workspace, workspace_bytes, down_state, state_bytes, stream) < 0)
        return 31;
    ...
    arguments.hidden_nvfp4_by_token.scale_e4m3_u8 = hidden_scales;
    arguments.hidden_nvfp4_by_token.row_count = tokens;
    ...
    return SparkBenchRun(&plan, &arguments, warmup, iterations, stream) == 0 ? 0 : 32;
}
