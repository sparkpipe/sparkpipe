...
#define SPARK_QWEN38_27B_CUDA_GVA_GROUP (SPARK_QWEN38_27B_MODEL_GDN_VALUE_HEAD_COUNT / SPARK_QWEN38_27B_MODEL_GDN_KEY_HEAD_COUNT)
#define SPARK_QWEN38_27B_CUDA_ATTN_GROUP (SPARK_QWEN38_27B_MODEL_ATTN_QUERY_HEAD_COUNT / SPARK_QWEN38_27B_MODEL_ATTN_KV_HEAD_COUNT)
#define SPARK_QWEN38_27B_CUDA_ATTN_HEADS_PER_CTA 2u
#define SPARK_QWEN38_27B_CUDA_ATTN_VALUE_PAIRS_PER_LANE \
    (SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION / (2u * SPARK_LM_WARP_LANES))
#define SPARK_QWEN38_27B_CUDA_GDN_STATE_ELEMENTS \
    (SPARK_QWEN38_27B_CUDA_DK * SPARK_QWEN38_27B_CUDA_DV)
...
enum SparkQwen38_27bTpDim
{
	...
	SPARK_QWEN38_27B_TPD_GDN_VALUE_CHANNELS,
	SPARK_QWEN38_27B_TPD_GDN_CONV_CHANNELS,
	SPARK_QWEN38_27B_TPD_GDN_KEY_HEADS,
	SPARK_QWEN38_27B_TPD_GDN_VALUE_HEADS,
	SPARK_QWEN38_27B_TPD_ATTN_QUERY_HEADS,
	SPARK_QWEN38_27B_TPD_ATTN_KV_HEADS,
	SPARK_QWEN38_27B_TPD_GDN_QK_CHANNEL_BASE,
	SPARK_QWEN38_27B_TPD_GDN_VALUE_CHANNEL_BASE,
	SPARK_QWEN38_27B_TPD_GDN_KEY_HEAD_BASE,
	SPARK_QWEN38_27B_TPD_GDN_VALUE_HEAD_BASE,
	SPARK_QWEN38_27B_TPD_COUNT
};

__device__ uint32_t spark_qwen38_27b_tp_dim[SPARK_QWEN38_27B_TPD_COUNT] =
{
	SPARK_QWEN38_27B_MODEL_GDN_QK_DIMENSION,
	SPARK_QWEN38_27B_MODEL_GDN_VALUE_DIMENSION,
	SPARK_QWEN38_27B_MODEL_GDN_CONV_CHANNELS,
	SPARK_QWEN38_27B_MODEL_GDN_KEY_HEAD_COUNT,
	SPARK_QWEN38_27B_MODEL_GDN_VALUE_HEAD_COUNT,
	SPARK_QWEN38_27B_MODEL_ATTN_QUERY_HEAD_COUNT,
	SPARK_QWEN38_27B_MODEL_ATTN_KV_HEAD_COUNT,
	0u, 0u, 0u, 0u
};

static uint32_t spark_qwen38_27b_tp_host_dim[SPARK_QWEN38_27B_TPD_COUNT] =
{
	SPARK_QWEN38_27B_MODEL_GDN_QK_DIMENSION,
	SPARK_QWEN38_27B_MODEL_GDN_VALUE_DIMENSION,
	SPARK_QWEN38_27B_MODEL_GDN_CONV_CHANNELS,
...
static __host__ __device__ __forceinline__ uint32_t SparkQwen38_27bTpDim(uint32_t index)
{
#ifdef __CUDA_ARCH__
	return(spark_qwen38_27b_tp_dim[index]);
#else
	return(spark_qwen38_27b_tp_host_dim[index]);
#endif
}
...
static __host__ __device__ __forceinline__ uint32_t SparkQwen38_27bTpConvChannel(uint32_t channel)
{
	...
	if ( channel < qk )
		return(SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_QK_CHANNEL_BASE) + channel);
	if ( channel < 2u * qk )
		return(SPARK_QWEN38_27B_MODEL_GDN_QK_DIMENSION +
			SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_QK_CHANNEL_BASE) +
			(channel - qk));
	return((2u * SPARK_QWEN38_27B_MODEL_GDN_QK_DIMENSION) +
		SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_VALUE_CHANNEL_BASE) +
		(channel - (2u * qk)));
}
extern "C" cudaError_t SparkQwen38_27bTpSetGeometry(
...
{
	uint32_t values[SPARK_QWEN38_27B_TPD_COUNT] =
	{
	...
	uint32_t index;
	cudaError_t error;
	for (index = 0u; index < SPARK_QWEN38_27B_TPD_COUNT; index++)
		spark_qwen38_27b_tp_host_dim[index] = values[index];
	error = cudaMemcpyToSymbol(spark_qwen38_27b_tp_dim,values,sizeof(values),0u,
		cudaMemcpyHostToDevice);
	return(error);
}

static __device__ __forceinline__ float SparkQwen38_27bRopeFrequency(uint32_t pair)
{
...
static __global__ void SparkQwen38_27bConvUpdateKernel(const void *qkv_bf16, const void *conv_weight_bf16, void *conv_out_bf16, void *conv_tail_bf16, const uint32_t *row_lane_indices, const uint32_t *state_cold_by_row, uint32_t row_count, uint32_t gdn_layer_ordinal, uint64_t tail_lane_stride, uint64_t tail_layer_stride, uint8_t *row_snap_tails, uint64_t snap_tail_lane_stride, uint64_t snap_tail_layer_stride)
{
	 ...
	 * GDN step race; fixed in the DSV4 session). A sequential loop with a
	 * sync between rows makes a k-row frame bit-match k sequential
	 * single-row frames. */
	...
	window[3] = SparkLmBf16ToFloat(qkv_bf16,((uint64_t)row * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_CONV_CHANNELS)) + channel);
	accumulator = 0.0f;
	for (tap = 0; tap < SPARK_QWEN38_27B_MODEL_GDN_CONV_KERNEL; tap++)
		accumulator += (window[tap] * SparkLmBf16ToFloat(conv_weight_bf16,((uint64_t)SparkQwen38_27bTpConvChannel(channel) * SPARK_QWEN38_27B_MODEL_GDN_CONV_KERNEL) + tap));
	SparkLmFloatToBf16(conv_out_bf16,((uint64_t)row * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_CONV_CHANNELS)) + channel,SparkLmSwish(accumulator));
	...
	if ( row_snap_tails != 0 )
	{
		 ...
		 * channel), indexed [row][layer][channel*3]. This is origin/main's
		 * verify per-row checkpoint write (snapshot slots 8-15); unified's
		 * harness snapshots keep using the whole-lane snapshot path. */
...
static __global__ void SparkQwen38_27bGdnStepKernel(const void *conv_out_bf16, const float *log_decay_f32, const float *beta_f32, float *state_f32, void *core_out_bf16, const uint32_t *row_lane_indices, const uint32_t *state_cold_by_row, uint32_t row_count, uint32_t gdn_layer_ordinal, uint64_t state_lane_stride, uint64_t state_layer_stride, float *row_snap_states, uint64_t snap_lane_stride, uint64_t snap_layer_stride)
{
    ...
    __shared__ float qn[SPARK_QWEN38_27B_CUDA_DK];
    __shared__ float kn[SPARK_QWEN38_27B_CUDA_DK];
     ...
     * multi-row frame) - the silent divergence source found in the DSV4
     * session. A sequential loop with a sync between rows makes a k-row
     * frame bit-match k sequential single-row frames. */
    for (row = 0u; row < row_count; row++)
    {

    conv_row = (uint64_t)row * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_CONV_CHANNELS);
    value = SparkLmBf16ToFloat(
        conv_out_bf16,
        conv_row + ((uint64_t)key_head * SPARK_QWEN38_27B_CUDA_DK) + column);
    q_norm = SparkLmBlockReduceSum(value * value, reduce_scratch);
    qn[column] = value * rsqrtf(q_norm + 1.0e-6f) *
        rsqrtf((float)SPARK_QWEN38_27B_CUDA_DK);

    value = SparkLmBf16ToFloat(
        conv_out_bf16,
        conv_row + SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_QK_CHANNELS) +
            ((uint64_t)key_head * SPARK_QWEN38_27B_CUDA_DK) + column);
    ...
    state_base =
        ...
        ((uint64_t)head * SPARK_QWEN38_27B_CUDA_GDN_STATE_ELEMENTS);
    decay = state_cold_by_row[row] != 0u
        ? 0.0f
        : expf(log_decay_f32[
            ((uint64_t)row * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_VALUE_HEADS)) + head]);
    beta = beta_f32[
        ((uint64_t)row * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_VALUE_HEADS)) + head];

    kv_memory = 0.0f;
    for (element = 0u; element < SPARK_QWEN38_27B_CUDA_DK; ++element)
    {
    ...
    delta = (SparkLmBf16ToFloat(
        conv_out_bf16,
        conv_row + (2u * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_QK_CHANNELS)) +
            ((uint64_t)head * SPARK_QWEN38_27B_CUDA_DV) + column) - kv_memory) * beta;
    output = 0.0f;
    for (element = 0u; element < SPARK_QWEN38_27B_CUDA_DK; ++element)
    {
        state_index = (element * SPARK_QWEN38_27B_CUDA_DV) + column;
    ...
    SparkLmFloatToBf16(
        core_out_bf16,
        ((uint64_t)row * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_VALUE_CHANNELS)) +
            ((uint64_t)head * SPARK_QWEN38_27B_CUDA_DV) + column,
        output);
    ...
    for (element = 0u; element < SPARK_QWEN38_27B_CUDA_DK; ++element)
    {
        ...
        if ( row_snap_states != 0 )
            row_snap_states[
                ...
                ((uint64_t)head * SPARK_QWEN38_27B_CUDA_GDN_STATE_ELEMENTS) + state_index] = state_shared[state_index];
    }
    __syncthreads();
    }
}
...
static __global__ void SparkQwen38_27bGatedNormKernel(const void *core_bf16, const void *z_bf16, const void *norm_weight_bf16, void *output_bf16, uint32_t row_count, float epsilon)
{
	...
	uint64_t index = ((uint64_t)row * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_VALUE_CHANNELS)) + ((uint64_t)head * SPARK_QWEN38_27B_CUDA_DV) + column;
	uint64_t z_index = ((uint64_t)row * SPARK_QWEN38_27B_MODEL_GDN_VALUE_DIMENSION) + ((uint64_t)(head + SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_VALUE_HEAD_BASE)) * SPARK_QWEN38_27B_CUDA_DV) + column;
	float value,variance;
	if ( row >= row_count )
		return;
	value = SparkLmBf16ToFloat(core_bf16,index);
	variance = SparkLmBlockReduceSum(value * value,reduce_scratch) / (float)SPARK_QWEN38_27B_CUDA_DV;
...
{
    __shared__ float reduce_scratch[SPARK_LM_CTA_WARPS];
    __shared__ float query_shared[SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION];
    __shared__ float key_shared[SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION];
    __shared__ float rope_cosine[
        SPARK_QWEN38_27B_MODEL_ATTN_ROPE_DIMENSION / 2u];
    ...
    head = blockIdx.x;
    column = threadIdx.x;
    kv_head = head / SPARK_QWEN38_27B_CUDA_ATTN_GROUP;
    if (row >= row_count ||
        head >= SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_ATTN_QUERY_HEADS) ||
        column >= SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION)
    {
        return;
    }

    query_base =
        ((uint64_t)row * 2u * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_ATTN_QUERY_HEADS) * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION) +
        ((uint64_t)head * 2u * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION);
    value = SparkLmBf16ToFloat(q_fused_bf16, query_base + column);
    sum_squares = SparkLmBlockReduceSum(
        value * value,
        reduce_scratch);
    inverse_rms = rsqrtf(
        sum_squares / (float)SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION +
        epsilon);
    query_shared[column] =
        value * inverse_rms *
        SparkLmBf16ToFloat(q_norm_weight_bf16, column);
    if (column < SPARK_QWEN38_27B_MODEL_ATTN_ROPE_DIMENSION / 2u)
    {
        float angle;

        pair = column;
        angle =
            (float)row_positions[row] * SparkQwen38_27bRopeFrequency(pair);
    ...
    if (column < SPARK_QWEN38_27B_MODEL_ATTN_ROPE_DIMENSION / 2u)
    {
        ...
        imaginary = query_shared[
            pair + SPARK_QWEN38_27B_MODEL_ATTN_ROPE_DIMENSION / 2u];
        query_shared[pair] =
    ...
    if ((head % SPARK_QWEN38_27B_CUDA_ATTN_GROUP) != 0u)
    {
        return;
    }

    key_base =
        ((uint64_t)row * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_ATTN_KV_HEADS) * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION) +
        ((uint64_t)kv_head * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION);
    ...
    block = slot / SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS;
    offset = slot % SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS;
    cache_base =
        ((uint64_t)block * cache_block_stride) +
        ((uint64_t)attn_layer_ordinal * cache_layer_stride) +
        ((uint64_t)offset * 2u * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_ATTN_KV_HEADS) * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION) +
        ((uint64_t)kv_head * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION);
    SparkLmFloatToBf16(
        kv_cache_bf16,
        cache_base + column,
    ...
    SparkLmFloatToBf16(
        kv_cache_bf16,
        cache_base + (SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_ATTN_KV_HEADS) * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION) + column,
...
{
    const uint32_t heads_per_cta = SPARK_QWEN38_27B_CUDA_ATTN_HEADS_PER_CTA;
    __shared__ float q_shared[
        SPARK_QWEN38_27B_CUDA_ATTN_HEADS_PER_CTA *
        SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION];
    __shared__ float merge_max[
        SPARK_QWEN38_27B_CUDA_ATTN_HEADS_PER_CTA * SPARK_LM_CTA_WARPS];
    ...
    uint64_t q_base;
    uint64_t out_base;
    float running_max[SPARK_QWEN38_27B_CUDA_ATTN_HEADS_PER_CTA];
    float running_den[SPARK_QWEN38_27B_CUDA_ATTN_HEADS_PER_CTA];
    float local_logit[SPARK_QWEN38_27B_CUDA_ATTN_HEADS_PER_CTA];
    float rescale[SPARK_QWEN38_27B_CUDA_ATTN_HEADS_PER_CTA];
    float weight[SPARK_QWEN38_27B_CUDA_ATTN_HEADS_PER_CTA];
    float2 accumulator[
        SPARK_QWEN38_27B_CUDA_ATTN_HEADS_PER_CTA]
        [SPARK_QWEN38_27B_CUDA_ATTN_VALUE_PAIRS_PER_LANE];
    float2 key_pair;
    float2 value_pair;
    float head_max;
    float denominator;
    ...
    required_block_count =
        (context + SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS - 1u) /
        ...
        required_block_count > available_block_count)
    {
        ...
        for (element = threadIdx.x;
             element < heads_per_cta * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION;
             element += blockDim.x)
        {
            local_head = element / SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION;
            partial = element % SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION;
            out_base =
                ((uint64_t)row * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_ATTN_QUERY_HEADS) * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION) +
    ...
    for (local_head = 0u; local_head < heads_per_cta; ++local_head)
    {
        ...
        q_base =
            ...
            ((uint64_t)head * 2u * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION);
        for (element = threadIdx.x;
             element < SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION;
             element += blockDim.x)
        {
            q_shared[
                (local_head * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION) +
        ...
        #pragma unroll
        for (pair = 0u;
             pair < SPARK_QWEN38_27B_CUDA_ATTN_VALUE_PAIRS_PER_LANE;
             ++pair)
        {
    ...
    for (element = threadIdx.x;
         element < SPARK_QWEN38_27B_CUDA_ATTN_HEADS_PER_CTA *
             SPARK_LM_CTA_WARPS *
             SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION;
         element += blockDim.x)
    {
    ...
    {
        token_base = SparkQwen38_27bAttnTokenBase(
            block_indices,
            lane_base,
            token,
            attn_layer_ordinal,
             ...
             ++pair)
        {
            value_pair = SparkLmLoadBf16Pair(
                kv_cache_bf16,
                ((token_base + (SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_ATTN_KV_HEADS) * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION)) >> 1u) +
                    (pair * SPARK_LM_WARP_LANES) + lane);
            #pragma unroll
            for (local_head = 0u;
                 local_head < heads_per_cta;
    ...
    for (local_head = 0u; local_head < heads_per_cta; ++local_head)
    {
        ...
        out_base =
            ((uint64_t)row * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_ATTN_QUERY_HEADS) * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION) +
            ((uint64_t)head * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION);
        for (element = threadIdx.x;
             element < SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION;
             element += blockDim.x)
        {
            ...
            gate = SparkLmSigmoid(SparkLmBf16ToFloat(
                q_fused_bf16,
                q_base + SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION + element));
            SparkLmFloatToBf16(
                head_out_bf16,
                out_base + element,
...
#define SPARK_QWEN38_27B_CUDA_GDN_CHUNK_SHARED_BYTES \
    ((SPARK_QWEN38_27B_CUDA_GDN_STATE_ELEMENTS + \
      (SPARK_QWEN38_27B_CUDA_CHUNK * SPARK_QWEN38_27B_CUDA_DV) + \
      (2u * SPARK_QWEN38_27B_CUDA_CHUNK)) * sizeof(float))
static_assert(
    SPARK_QWEN38_27B_CUDA_GDN_CHUNK_SHARED_BYTES == 98816u,
    "Qwen GDN chunk shared layout must fit the SM 12.x 99-KB block limit");

typedef struct SparkQwen38_27bChunkWorkspaceView
{
	float *qn;
	float *kn;
	float *cum_g;
	...
	float *w;
	float *kg;
} SparkQwen38_27bChunkWorkspaceView;

static __device__ __forceinline__ uint64_t SparkQwen38_27bChunkHeadOffset(uint32_t head, uint32_t per_head_elements)
{
...
static __global__ void SparkQwen38_27bChunkPrepareKernel(const void *conv_out_bf16, const float *log_decay_f32, const float *beta_f32, SparkQwen38_27bChunkWorkspaceView views, uint32_t token_count)
{
	...
	uint64_t conv_row,qk_base = SparkQwen38_27bChunkHeadOffset(head,SPARK_QWEN38_27B_CUDA_CHUNK * SPARK_QWEN38_27B_CUDA_DK);
	uint64_t mat_base = SparkQwen38_27bChunkHeadOffset(head,SPARK_QWEN38_27B_CUDA_CHUNK * SPARK_QWEN38_27B_CUDA_CHUNK);
	float total,value,product;
	if ( row >= token_count )
		return;
	conv_row = (uint64_t)row * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_CONV_CHANNELS);
	total = 0.0f;
	for (element = 0; element < SPARK_QWEN38_27B_CUDA_DK; element++)
	{
		value = SparkLmBf16ToFloat(conv_out_bf16,conv_row + ((uint64_t)key_head * SPARK_QWEN38_27B_CUDA_DK) + element);
		total += (value * value);
	}
	total = rsqrtf(total + 1e-6f) * rsqrtf((float)SPARK_QWEN38_27B_CUDA_DK);
	for (element = 0; element < SPARK_QWEN38_27B_CUDA_DK; element++)
		views.qn[qk_base + ((uint64_t)row * SPARK_QWEN38_27B_CUDA_DK) + element] = SparkLmBf16ToFloat(conv_out_bf16,conv_row + ((uint64_t)key_head * SPARK_QWEN38_27B_CUDA_DK) + element) * total;
	total = 0.0f;
	for (element = 0; element < SPARK_QWEN38_27B_CUDA_DK; element++)
	{
		value = SparkLmBf16ToFloat(conv_out_bf16,conv_row + SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_QK_CHANNELS) + ((uint64_t)key_head * SPARK_QWEN38_27B_CUDA_DK) + element);
		total += (value * value);
	}
	total = rsqrtf(total + 1e-6f);
	for (element = 0; element < SPARK_QWEN38_27B_CUDA_DK; element++)
		views.kn[qk_base + ((uint64_t)row * SPARK_QWEN38_27B_CUDA_DK) + element] = SparkLmBf16ToFloat(conv_out_bf16,conv_row + SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_QK_CHANNELS) + ((uint64_t)key_head * SPARK_QWEN38_27B_CUDA_DK) + element) * total;
	if ( row == 0u )
	{
		total = 0.0f;
		for (element = 0; element < token_count; element++)
		{
			total += log_decay_f32[((uint64_t)element * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_VALUE_HEADS)) + head];
			views.cum_g[SparkQwen38_27bChunkHeadOffset(head,SPARK_QWEN38_27B_CUDA_CHUNK) + element] = total;
		}
	}
	__syncthreads();
	for (column = 0; column < token_count; column++)
	{
		value = column <= row ? __expf(views.cum_g[SparkQwen38_27bChunkHeadOffset(head,SPARK_QWEN38_27B_CUDA_CHUNK) + row] - views.cum_g[SparkQwen38_27bChunkHeadOffset(head,SPARK_QWEN38_27B_CUDA_CHUNK) + column]) : 0.0f;
		views.decay[mat_base + ((uint64_t)row * SPARK_QWEN38_27B_CUDA_CHUNK) + column] = value;
		product = 0.0f;
		for (element = 0; element < SPARK_QWEN38_27B_CUDA_DK && column < row; element++)
			product += (views.kn[qk_base + ((uint64_t)row * SPARK_QWEN38_27B_CUDA_DK) + element] * beta_f32[((uint64_t)row * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_VALUE_HEADS)) + head] * views.kn[qk_base + ((uint64_t)column * SPARK_QWEN38_27B_CUDA_DK) + element]);
		views.attn[mat_base + ((uint64_t)row * SPARK_QWEN38_27B_CUDA_CHUNK) + column] = column < row ? -(product * value) : 0.0f;
	}
}
...
static __global__ void SparkQwen38_27bChunkSolveKernel(SparkQwen38_27bChunkWorkspaceView views, uint32_t token_count)
{
	...
	for (row = 1; row < token_count; row++)
	{
		if ( column < row )
		{
			accumulator = views.attn[mat_base + ((uint64_t)row * SPARK_QWEN38_27B_CUDA_CHUNK) + column];
			for (element = 0; element < row; element++)
				accumulator += (views.attn[mat_base + ((uint64_t)row * SPARK_QWEN38_27B_CUDA_CHUNK) + element] * views.attn[mat_base + ((uint64_t)element * SPARK_QWEN38_27B_CUDA_CHUNK) + column]);
			views.attn[mat_base + ((uint64_t)row * SPARK_QWEN38_27B_CUDA_CHUNK) + column] = accumulator;
		}
		__syncthreads();
	}
...
static __global__ void SparkQwen38_27bChunkTransformKernel(const void *conv_out_bf16, const float *beta_f32, SparkQwen38_27bChunkWorkspaceView views, uint32_t token_count)
{
	...
	uint64_t mat_base = SparkQwen38_27bChunkHeadOffset(head,SPARK_QWEN38_27B_CUDA_CHUNK * SPARK_QWEN38_27B_CUDA_CHUNK);
	uint64_t vec_base = SparkQwen38_27bChunkHeadOffset(head,SPARK_QWEN38_27B_CUDA_CHUNK * SPARK_QWEN38_27B_CUDA_DK);
	...
	if ( threadIdx.x < token_count )
		exp_cum_g[threadIdx.x] = __expf(
			views.cum_g[
				SparkQwen38_27bChunkHeadOffset(
					head,
					SPARK_QWEN38_27B_CUDA_CHUNK) +
				threadIdx.x]);
	__syncthreads();
	accumulator = 0.0f;
	for (element = 0; element < token_count; element++)
	{
		transform = views.attn[mat_base + ((uint64_t)row * SPARK_QWEN38_27B_CUDA_CHUNK) + element] * beta_f32[((uint64_t)element * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_VALUE_HEADS)) + head];
		accumulator += (transform * SparkLmBf16ToFloat(conv_out_bf16,((uint64_t)element * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_CONV_CHANNELS)) + (2u * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_QK_CHANNELS)) + ((uint64_t)head * SPARK_QWEN38_27B_CUDA_DV) + column));
	}
	views.w[vec_base + ((uint64_t)row * SPARK_QWEN38_27B_CUDA_DV) + column] = accumulator;
	accumulator = 0.0f;
	for (element = 0; element < token_count; element++)
	{
		transform = views.attn[mat_base + ((uint64_t)row * SPARK_QWEN38_27B_CUDA_CHUNK) + element] * beta_f32[((uint64_t)element * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_VALUE_HEADS)) + head] * exp_cum_g[element];
		accumulator += (transform * views.kn[vec_base + ((uint64_t)element * SPARK_QWEN38_27B_CUDA_DK) + column]);
	}
	views.kg[vec_base + ((uint64_t)row * SPARK_QWEN38_27B_CUDA_DK) + column] = accumulator;
}


static __global__ void SparkQwen38_27bChunkQkDecayKernel(SparkQwen38_27bChunkWorkspaceView views, uint32_t token_count)
{
    ...
    qn_shared = qk_shared;
    kn_shared = qn_shared +
        (SPARK_QWEN38_27B_CUDA_CHUNK * SPARK_QWEN38_27B_CUDA_DK);
    vector_base = SparkQwen38_27bChunkHeadOffset(
        head,
        SPARK_QWEN38_27B_CUDA_CHUNK * SPARK_QWEN38_27B_CUDA_DK);
    matrix_base = SparkQwen38_27bChunkHeadOffset(
        head,
        SPARK_QWEN38_27B_CUDA_CHUNK * SPARK_QWEN38_27B_CUDA_CHUNK);

    for (vector_element = threadIdx.x;
         vector_element < token_count * SPARK_QWEN38_27B_CUDA_DK;
         ...
         matrix_element += blockDim.x)
    {
        ...
        if (column <= row)
        {
            ...
            {
                dot = fmaf(
                    qn_shared[(row * SPARK_QWEN38_27B_CUDA_DK) + element],
                    kn_shared[(column * SPARK_QWEN38_27B_CUDA_DK) + element],
                    dot);
            }
            views.decay[
                matrix_base +
                ((uint64_t)row * SPARK_QWEN38_27B_CUDA_CHUNK) + column] *= dot;
        }
        else
        {
            views.decay[
                matrix_base +
                ((uint64_t)row * SPARK_QWEN38_27B_CUDA_CHUNK) + column] = 0.0f;
        }
    }
}
...
static __global__ void SparkQwen38_27bChunkStepKernel(const float *log_decay_f32, SparkQwen38_27bChunkWorkspaceView views, float *state_f32, void *core_out_bf16, uint32_t lane_index, uint32_t token_count, uint32_t gdn_layer_ordinal, uint64_t state_lane_stride, uint64_t state_layer_stride)
{
    ...
    v_new_shared = state_shared + SPARK_QWEN38_27B_CUDA_GDN_STATE_ELEMENTS;
    exp_cum_g_shared =
        v_new_shared + (SPARK_QWEN38_27B_CUDA_CHUNK * SPARK_QWEN38_27B_CUDA_DV);
    carry_decay_shared =
        exp_cum_g_shared + SPARK_QWEN38_27B_CUDA_CHUNK;
    vector_base = SparkQwen38_27bChunkHeadOffset(
        head,
        SPARK_QWEN38_27B_CUDA_CHUNK * SPARK_QWEN38_27B_CUDA_DK);
    g_base = SparkQwen38_27bChunkHeadOffset(
        head,
        SPARK_QWEN38_27B_CUDA_CHUNK);
    matrix_base = SparkQwen38_27bChunkHeadOffset(
        head,
    ...
    for (row = 0u; row < token_count; ++row)
    {
        ...
        for (element = 0u; element < SPARK_QWEN38_27B_CUDA_DK; ++element)
        {
            accumulator = fmaf(
                views.kg[
                    vector_base +
                    ((uint64_t)row * SPARK_QWEN38_27B_CUDA_DK) + element],
                state_shared[(element * SPARK_QWEN38_27B_CUDA_DV) + column],
                accumulator);
        }
        v_new_shared[(row * SPARK_QWEN38_27B_CUDA_DV) + column] =
            views.w[
                vector_base +
                ((uint64_t)row * SPARK_QWEN38_27B_CUDA_DV) + column] - accumulator;
    }
    ...
    for (row = 0u; row < token_count; ++row)
    {
        ...
        for (element = 0u; element <= row; ++element)
        {
            accumulator = fmaf(
                views.decay[
                    matrix_base +
                    ((uint64_t)row * SPARK_QWEN38_27B_CUDA_CHUNK) + element],
                v_new_shared[(element * SPARK_QWEN38_27B_CUDA_DV) + column],
                accumulator);
        }
        SparkLmFloatToBf16(
            core_out_bf16,
...
static __global__ void SparkQwen38_27bChunkConvKernel(const void *qkv_bf16, const void *conv_weight_bf16, void *conv_out_bf16, void *conv_tail_bf16, uint32_t lane_index, uint32_t token_count, uint32_t gdn_layer_ordinal, uint64_t tail_lane_stride, uint64_t tail_layer_stride)
{
	...
	if ( channel >= SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_CONV_CHANNELS) )
		return;
	tail_base = ((uint64_t)lane_index * tail_lane_stride) + ((uint64_t)gdn_layer_ordinal * tail_layer_stride) + ((uint64_t)channel * SPARK_QWEN38_27B_MODEL_GDN_CONV_TAIL_COLUMNS);
	window[0] = SparkLmBf16ToFloat(conv_tail_bf16,tail_base + 0u);
	window[1] = SparkLmBf16ToFloat(conv_tail_bf16,tail_base + 1u);
	window[2] = SparkLmBf16ToFloat(conv_tail_bf16,tail_base + 2u);
	for (tap = 0; tap < SPARK_QWEN38_27B_MODEL_GDN_CONV_KERNEL; tap++)
		weight[tap] = SparkLmBf16ToFloat(conv_weight_bf16,((uint64_t)SparkQwen38_27bTpConvChannel(channel) * SPARK_QWEN38_27B_MODEL_GDN_CONV_KERNEL) + tap);
	for (token = 0; token < token_count; token++)
	{
		element = ((uint64_t)token * SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_CONV_CHANNELS)) + channel;
...
extern "C" cudaError_t SparkQwen38_27bConfigureCudaKernels(void)
{
    ...
    status = cudaFuncSetAttribute(
        ...
        (int)SPARK_QWEN38_27B_CUDA_GDN_DECODE_SHARED_BYTES);
    if (status != cudaSuccess)
    {
        return status;
    }
    status = cudaFuncSetAttribute(
        SparkQwen38_27bChunkQkDecayKernel,
        cudaFuncAttributeMaxDynamicSharedMemorySize,
        (int)SPARK_QWEN38_27B_CUDA_GDN_QK_SHARED_BYTES);
    if (status != cudaSuccess)
    {
        return status;
    }
    status = cudaFuncSetAttribute(
        SparkQwen38_27bChunkStepKernel,
        cudaFuncAttributeMaxDynamicSharedMemorySize,
        (int)SPARK_QWEN38_27B_CUDA_GDN_CHUNK_SHARED_BYTES);
    if (status != cudaSuccess)
    {
        return status;
    }
    ...
    return cudaFuncSetAttribute(
        ...
        cudaFuncAttributeMaxDynamicSharedMemorySize,
        (int)(SPARK_QWEN38_27B_MODEL_FFN_INTERMEDIATE_DIMENSION * sizeof(float)));
}
...
#define SPARK_QWEN38_27B_RANS_SUBSTREAMS 128u
...
static __device__ void SparkQwen38_27bRansDecodeTileHalf(const uint32_t *s_sf, const uint16_t *s_c, const uint8_t *stage, const uint16_t *id_table, uint32_t half, __nv_bfloat16 *tile)
{
	...
	const uint8_t *sd = stage + SPARK_QWEN38_27B_RANS_HEADER_BYTES + half * 32u;
	uint32_t j = 0u;
	#pragma unroll 4
	for ( uint32_t it = 0u; it < SPARK_QWEN38_27B_RANS_SUB_LEN; it++ )
	{
		const uint32_t slot = x & (SPARK_QWEN38_27B_RANS_SLOTS - 1u);
		...
		uint32_t nx = (uint32_t)fs * (x >> SPARK_QWEN38_27B_RANS_L) + slot - cs;
		while ( nx < SPARK_QWEN38_27B_RANS_BOUND ) { nx = (nx << 8) | sd[j * SPARK_QWEN38_27B_RANS_SUBSTREAMS + lane]; j++; }
		uint16_t value;
		if ( sym == 0xFFFFu )
		{
			const uint32_t rid = (uint32_t)sd[j * SPARK_QWEN38_27B_RANS_SUBSTREAMS + lane] | ((uint32_t)sd[(j + 1u) * SPARK_QWEN38_27B_RANS_SUBSTREAMS + lane] << 8);
			j += 2u;
			value = id_table[rid];
		}
		else
			value = sym;
		((uint16_t *)tile)[it * SPARK_QWEN38_27B_RANS_SUBSTREAMS + glane] = value;
		x = nx;
	}
}
...
static __global__ void __launch_bounds__(1024u, 1u) SparkQwen38_27bSmallBatchTiledRansKernel(const void *weight_rans, const void *input_bf16, void *output_bf16, uint32_t row_count, uint32_t input_dimension, uint32_t output_dimension, uint64_t payload_bytes)
{
	...
	const uint32_t PER_GROUP = SPARK_QWEN38_27B_SMALL_BATCH_TILE_N / GROUPS;
	const uint32_t neuron_base = blockIdx.x * SPARK_QWEN38_27B_SMALL_BATCH_TILE_N;
	...
	SparkQwen38_27bRansBuildTable(s_sf, s_c, entries3, ndirect);
	__syncthreads();
	if ( warp >= 28u )
	{
	...
	else
	{
		for ( k_base = 0u; k_base < input_dimension; k_base += SPARK_QWEN38_27B_SMALL_BATCH_K_CHUNK )
		{
			...
			asm volatile("bar.sync 1, 896;");
			const __nv_bfloat16 *wt = weight_tile + (ki % 3u) * SPARK_QWEN38_27B_SMALL_BATCH_TILE_N * SPARK_QWEN38_27B_SMALL_BATCH_K_CHUNK;
			#pragma unroll
...
extern "C" cudaError_t SparkQwen38_27bLaunchSmallBatchLinearRans(cudaStream_t stream, const void *weight_rans, uint64_t payload_bytes, const void *input_bf16, void *output_bf16, uint32_t row_count, uint32_t input_dimension, uint32_t output_dimension)
{
	uint32_t blocks = (output_dimension + SPARK_QWEN38_27B_SMALL_BATCH_TILE_N - 1u) / SPARK_QWEN38_27B_SMALL_BATCH_TILE_N;
	size_t shared_bytes = 44576u + (size_t)(3u * SPARK_QWEN38_27B_SMALL_BATCH_TILE_N + SPARK_QWEN38_27B_SMALL_BATCH_MAX_ROWS) * SPARK_QWEN38_27B_SMALL_BATCH_K_CHUNK * sizeof(__nv_bfloat16) + 64u;
	cudaError_t ea = cudaFuncSetAttribute((const void *)SparkQwen38_27bSmallBatchTiledRansKernel<8u>, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)shared_bytes);
	if (ea != cudaSuccess) return ea;
	ea = cudaFuncSetAttribute((const void *)SparkQwen38_27bSmallBatchTiledRansKernel<4u>, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)shared_bytes);
	if (ea != cudaSuccess) return ea;
	ea = cudaFuncSetAttribute((const void *)SparkQwen38_27bSmallBatchTiledRansKernel<2u>, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)shared_bytes);
	if (ea != cudaSuccess) return ea;
	if ( row_count <= 2u )
		SparkQwen38_27bSmallBatchTiledRansKernel<2u><<<blocks, 1024u, shared_bytes, stream>>>(weight_rans, input_bf16, output_bf16, row_count, input_dimension, output_dimension, payload_bytes);
	else if ( row_count <= 4u )
		SparkQwen38_27bSmallBatchTiledRansKernel<4u><<<blocks, 1024u, shared_bytes, stream>>>(weight_rans, input_bf16, output_bf16, row_count, input_dimension, output_dimension, payload_bytes);
	else
		SparkQwen38_27bSmallBatchTiledRansKernel<8u><<<blocks, 1024u, shared_bytes, stream>>>(weight_rans, input_bf16, output_bf16, row_count, input_dimension, output_dimension, payload_bytes);
	{
...
static __global__ void SparkQwen38_27bSmallBatchLean1Kernel(const void *weight_bf16, const void *input_bf16, void *output_bf16, uint32_t input_dimension, uint32_t output_dimension)
{
	const uint32_t neuron_base = blockIdx.x * SPARK_QWEN38_27B_SMALL_BATCH_TILE_N;
...
template <uint32_t ROWS>
static __global__ void SparkQwen38_27bSmallBatchTiledKernel(const void *weight_bf16, const void *input_bf16, void *output_bf16, uint32_t row_count, uint32_t input_dimension, uint32_t output_dimension)
{
	extern __shared__ __nv_bfloat16 tile[];
	__nv_bfloat16 *weight_tile = tile;
	__nv_bfloat16 *input_tile = tile + (SPARK_QWEN38_27B_SMALL_BATCH_TILE_N * SPARK_QWEN38_27B_SMALL_BATCH_K_CHUNK);
	const uint32_t GROUP_SHIFT = (ROWS == 2u) ? 1u : ((ROWS == 4u) ? 2u : 3u);
	const uint32_t GROUPS = 32u >> GROUP_SHIFT;
	const uint32_t PER_GROUP = SPARK_QWEN38_27B_SMALL_BATCH_TILE_N / GROUPS;
	const uint32_t neuron_base = blockIdx.x * SPARK_QWEN38_27B_SMALL_BATCH_TILE_N;
...
static __global__ void SparkQwen38_27bSmallBatchFfnGateUpKernel(const void *gate_weight_bf16, const void *up_weight_bf16, const void *input_bf16, void *gated_up_bf16, uint32_t row_count, uint32_t input_dimension, uint32_t output_dimension)
{
	...
	__nv_bfloat16 *up_tile = tile + (SPARK_QWEN38_27B_SMALL_BATCH_FFN_TILE_N * SPARK_QWEN38_27B_SMALL_BATCH_K_CHUNK);
	__nv_bfloat16 *input_tile = up_tile + (SPARK_QWEN38_27B_SMALL_BATCH_FFN_TILE_N * SPARK_QWEN38_27B_SMALL_BATCH_K_CHUNK);
	const uint32_t neuron_base = blockIdx.x * SPARK_QWEN38_27B_SMALL_BATCH_FFN_TILE_N;
	const uint32_t thread = threadIdx.x;
	const uint32_t warp = thread >> 5u;
	const uint32_t lane = thread & 31u;
...
extern "C" cudaError_t SparkQwen38_27bLaunchFfnGateUp(cudaStream_t stream, const void *gate_weight_bf16, const void *up_weight_bf16, const void *input_bf16, void *gated_up_bf16, uint32_t row_count, uint32_t input_dimension, uint32_t output_dimension)
{
	uint32_t blocks = (output_dimension + SPARK_QWEN38_27B_SMALL_BATCH_FFN_TILE_N - 1u) / SPARK_QWEN38_27B_SMALL_BATCH_FFN_TILE_N;
	size_t shared_bytes = (size_t)(2u * SPARK_QWEN38_27B_SMALL_BATCH_FFN_TILE_N + SPARK_QWEN38_27B_SMALL_BATCH_MAX_ROWS) * SPARK_QWEN38_27B_SMALL_BATCH_K_CHUNK * sizeof(__nv_bfloat16);
	SparkQwen38_27bSmallBatchFfnGateUpKernel<<<blocks, 1024u, shared_bytes, stream>>>(gate_weight_bf16, up_weight_bf16, input_bf16, gated_up_bf16, row_count, input_dimension, output_dimension);
	return(cudaGetLastError());
}

extern "C" cudaError_t SparkQwen38_27bLaunchSmallBatchLinear(cudaStream_t stream, const void *weight_bf16, const void *input_bf16, void *output_bf16, uint32_t row_count, uint32_t input_dimension, uint32_t output_dimension)
{
	uint32_t blocks = (output_dimension + SPARK_QWEN38_27B_SMALL_BATCH_TILE_N - 1u) / SPARK_QWEN38_27B_SMALL_BATCH_TILE_N;
	size_t shared_bytes = (size_t)(SPARK_QWEN38_27B_SMALL_BATCH_TILE_N + SPARK_QWEN38_27B_SMALL_BATCH_MAX_ROWS) * SPARK_QWEN38_27B_SMALL_BATCH_K_CHUNK * sizeof(__nv_bfloat16);
	if ( row_count == 1u )
		SparkQwen38_27bSmallBatchLean1Kernel<<<blocks, 512u, 0u, stream>>>(weight_bf16, input_bf16, output_bf16, input_dimension, output_dimension);
	else if ( row_count <= 2u )
		SparkQwen38_27bSmallBatchTiledKernel<2u><<<blocks, 1024u, shared_bytes, stream>>>(weight_bf16, input_bf16, output_bf16, row_count, input_dimension, output_dimension);
	else if ( row_count <= 4u )
		SparkQwen38_27bSmallBatchTiledKernel<4u><<<blocks, 1024u, shared_bytes, stream>>>(weight_bf16, input_bf16, output_bf16, row_count, input_dimension, output_dimension);
	else
		SparkQwen38_27bSmallBatchTiledKernel<8u><<<blocks, 1024u, shared_bytes, stream>>>(weight_bf16, input_bf16, output_bf16, row_count, input_dimension, output_dimension);
	return(cudaGetLastError());
}
 ...
 * and the store is round-to-nearest BF16, matching SparkQwen38_27bLaunchLinear.
 */
#define SPARK_QWEN38_27B_DSPARK_PROJECTOR_TILE_N 64u
#define SPARK_QWEN38_27B_DSPARK_PROJECTOR_TILE_ROWS 8u

static __global__ void __launch_bounds__(1024u, 1u) SparkQwen38_27bDsparkProjectorKernel(
	...
	uint32_t row_count, uint32_t input_dimension, uint32_t output_dimension)
{
	extern __shared__ __nv_bfloat16 tile[];
	__nv_bfloat16 *weight_tile = tile;
	__nv_bfloat16 *input_tile = tile + (SPARK_QWEN38_27B_DSPARK_PROJECTOR_TILE_N * SPARK_QWEN38_27B_SMALL_BATCH_K_CHUNK);
	const uint32_t neuron_base = blockIdx.y * SPARK_QWEN38_27B_DSPARK_PROJECTOR_TILE_N;
	const uint32_t row_base = blockIdx.x * SPARK_QWEN38_27B_DSPARK_PROJECTOR_TILE_ROWS;
	const uint32_t thread = threadIdx.x;
	const uint32_t warp = thread >> 5u;
	const uint32_t lane = thread & 31u;
	const uint32_t row = warp & (SPARK_QWEN38_27B_DSPARK_PROJECTOR_TILE_ROWS - 1u);
	const uint32_t neuron_group = warp >> 3u;
	const uint32_t PER_GROUP = SPARK_QWEN38_27B_DSPARK_PROJECTOR_TILE_N >> 2u;
	uint32_t k_base, neuron, p;
	float value;
	float acc[16];
	#pragma unroll
...
extern "C" cudaError_t SparkQwen38_27bLaunchDsparkProjector(cudaStream_t stream, const void *weight_bf16, const void *input_bf16, void *output_bf16, uint32_t row_count, uint32_t input_dimension, uint32_t output_dimension)
{
	dim3 grid((row_count + SPARK_QWEN38_27B_DSPARK_PROJECTOR_TILE_ROWS - 1u) / SPARK_QWEN38_27B_DSPARK_PROJECTOR_TILE_ROWS,
		(output_dimension + SPARK_QWEN38_27B_DSPARK_PROJECTOR_TILE_N - 1u) / SPARK_QWEN38_27B_DSPARK_PROJECTOR_TILE_N);
	size_t shared_bytes = (size_t)(SPARK_QWEN38_27B_DSPARK_PROJECTOR_TILE_N + SPARK_QWEN38_27B_DSPARK_PROJECTOR_TILE_ROWS) * SPARK_QWEN38_27B_SMALL_BATCH_K_CHUNK * sizeof(__nv_bfloat16);
	SparkQwen38_27bDsparkProjectorKernel<<<grid, 1024u, shared_bytes, stream>>>(weight_bf16, input_bf16, output_bf16, row_count, input_dimension, output_dimension);
	return(cudaGetLastError());
}

extern "C" cudaError_t SparkQwen38_27bLaunchLinear(cudaStream_t stream, const SparkQwen38_27bLinearView *view, const void *input_bf16, void *output_bf16, uint32_t row_count)
{
	const char *gate = getenv("SPARK_QWEN38_27B_SMALL_BATCH_GEMM");
	if ( view->weight_format == SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_BF16_RANS &&
		row_count <= SPARK_QWEN38_27B_SMALL_BATCH_MAX_ROWS &&
	 ...
	 * lossless controls prove is correct. Re-enable with SPARK_QWEN38_27B_SMALL_BATCH_GEMM=1. */
...
{
	SparkQwen38_27bDecayBetaKernel<<<row_count,SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_VALUE_HEADS),0,stream>>>(decay_pre_bf16,beta_pre_bf16,weights->a_log_f32,weights->dt_bias_f32,log_decay_f32,beta_f32,row_count);
	return(cudaGetLastError());
}
...
{
    dim3 grid(
        SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_VALUE_HEADS),
        1u,
        1u);
    SparkQwen38_27bGdnStepKernel<<<
        grid,
        SPARK_QWEN38_27B_CUDA_DV,
        SPARK_QWEN38_27B_CUDA_GDN_DECODE_SHARED_BYTES,
        stream>>>(
            conv_out_bf16,
            log_decay_f32,
            beta_f32,
...
{
	dim3 grid(SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_VALUE_HEADS),row_count,1u);
	SparkQwen38_27bGatedNormKernel<<<grid,SPARK_QWEN38_27B_CUDA_DV,0,stream>>>(core_bf16,z_bf16,weights->gdn_norm_weight_bf16,output_bf16,row_count,epsilon);
	return(cudaGetLastError());
}
...
{
	dim3 grid(SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_ATTN_QUERY_HEADS),row_count,1u);
	SparkQwen38_27bAttnPrepareKernel<<<grid,SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION,0,stream>>>(q_fused_bf16,k_bf16,v_bf16,weights->query_norm_weight_bf16,weights->key_norm_weight_bf16,kv_cache_bf16,slot_mapping,row_positions,row_count,attn_layer_ordinal,cache_layer_stride,cache_block_stride,epsilon);
	return(cudaGetLastError());
}
...
extern "C" cudaError_t SparkQwen38_27bLaunchAttnDecode(cudaStream_t stream, const void *q_fused_bf16, const void *kv_cache_bf16, const SparkQwen38_27bKvBlockTableView *table, const uint32_t *row_lane_indices, const uint32_t *context_lengths, void *head_out_bf16, uint32_t row_count, uint32_t attn_layer_ordinal, uint64_t cache_layer_stride, uint64_t cache_block_stride)
{
    ...
    static_assert(
        SPARK_QWEN38_27B_MODEL_ATTN_QUERY_HEAD_COUNT %
            SPARK_QWEN38_27B_CUDA_ATTN_HEADS_PER_CTA == 0u,
        "query-head count must divide the grouped attention CTA width");
    static_assert(
        SPARK_QWEN38_27B_CUDA_ATTN_GROUP %
            SPARK_QWEN38_27B_CUDA_ATTN_HEADS_PER_CTA == 0u,
        "a grouped attention CTA may not cross KV-head ownership");
    if ( (SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_ATTN_QUERY_HEADS) %
            SPARK_QWEN38_27B_CUDA_ATTN_HEADS_PER_CTA) != 0u )
        return(cudaErrorInvalidValue);
    SparkQwen38_27bAttnDecodeKernel<<<grid, SPARK_LM_CTA_THREADS, 0u, stream>>>(
        q_fused_bf16,
        kv_cache_bf16,
...
extern "C" cudaError_t SparkQwen38_27bLaunchGdnChunk(cudaStream_t stream, const void *conv_out_bf16, const float *log_decay_f32, const float *beta_f32, float *workspace_qn, float *workspace_kn, float *workspace_cum_g, float *workspace_decay, float *workspace_attn, float *workspace_w, float *workspace_kg, const SparkQwen38_27bGdnStatePool *pool, void *core_out_bf16, uint32_t lane_index, uint32_t token_count, uint32_t gdn_layer_ordinal)
{
    ...
    if (token_count == 0u || token_count > SPARK_QWEN38_27B_CUDA_CHUNK)
    {
        return cudaErrorInvalidValue;
    }

    SparkQwen38_27bChunkPrepareKernel<<<
        SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_VALUE_HEADS),
        SPARK_QWEN38_27B_CUDA_CHUNK,
        0u,
    ...
    SparkQwen38_27bChunkSolveKernel<<<
        SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_VALUE_HEADS),
        SPARK_QWEN38_27B_CUDA_CHUNK,
        0u,
    ...
    SparkQwen38_27bChunkTransformKernel<<<
        transform_grid,
        SPARK_QWEN38_27B_CUDA_DV,
        0u,
    ...
    SparkQwen38_27bChunkQkDecayKernel<<<
        ...
        SPARK_QWEN38_27B_CUDA_GDN_QK_SHARED_BYTES,
        stream>>>(views, token_count);
    status = cudaGetLastError();
    if (status != cudaSuccess)
    {
    ...
    SparkQwen38_27bChunkStepKernel<<<
        SparkQwen38_27bTpDim(SPARK_QWEN38_27B_TPD_GDN_VALUE_HEADS),
        SPARK_QWEN38_27B_CUDA_DV,
        SPARK_QWEN38_27B_CUDA_GDN_CHUNK_SHARED_BYTES,
        stream>>>(
            log_decay_f32,
            views,
            pool->state_f32,
...
extern "C" cudaError_t SparkQwen38_27bLaunchHeadShadowQuantize(cudaStream_t stream, const void *head_bf16, uint8_t *shadow_payload, uint8_t *shadow_scale, float *error_norm, uint32_t candidate_count, uint32_t hidden_dimension)
{
...
extern "C" cudaError_t SparkQwen38_27bLaunchHeadArgmax(cudaStream_t stream, const void *hidden_bf16, const void *head_weight_bf16, const uint32_t *token_ids, uint32_t *output_token_ids, uint32_t row_count, uint32_t candidate_count)
{
	SparkLmHeadArgmaxKernel<<<row_count,SPARK_LM_CTA_THREADS,0,stream>>>(hidden_bf16,head_weight_bf16,token_ids,output_token_ids,row_count,SPARK_QWEN38_27B_MODEL_HIDDEN_DIMENSION,candidate_count);
	return(cudaGetLastError());
}
...
static __global__ void SparkQwen38_27bHeadMaxLocUnpackKernel(const uint64_t *keys_u64, uint32_t *token_ids_u32, uint32_t row_count)
{
	uint32_t row = blockIdx.x;
	if ( row >= row_count )
		return;
...
extern "C" cudaError_t SparkQwen38_27bLaunchHeadMaxLocUnpack(cudaStream_t stream, const uint64_t *keys_u64, uint32_t *token_ids_u32, uint32_t row_count)
{
...
/* Merge consolidation: BOTH drafter attention paths keep their launchers.
 * LaunchDsparkAttn + TapCapture serve unified's full-sequence-context path
 * (tap ring + tiled-K projector); TapStore/KPrep/QPrep/CacheAttn/Select
 * serve origin/main's cache path (block-KV history, per-position taps). */
extern "C" cudaError_t SparkQwen38_27bLaunchDsparkAttn(cudaStream_t stream, const void *q_bf16, const void *k_bf16, const void *v_bf16, const void *q_norm_bf16, const void *k_norm_bf16, void *attn_out_bf16, uint32_t block_size, uint64_t base_position, uint32_t context_length)
{
	dim3 grid(block_size, 8u);
	SparkQwen38_27bDsparkAttnKernel<<<grid, SPARK_QWEN38_27B_DSPARK_ATTN_QUERY_HEADS / SPARK_QWEN38_27B_DSPARK_ATTN_KV_HEADS, 0u, stream>>>((const __nv_bfloat16 *)q_bf16, (const __nv_bfloat16 *)k_bf16, (const __nv_bfloat16 *)v_bf16, (const __nv_bfloat16 *)q_norm_bf16, (const __nv_bfloat16 *)k_norm_bf16, (__nv_bfloat16 *)attn_out_bf16, block_size, base_position, context_length);
	return(cudaGetLastError());
}

extern "C" cudaError_t SparkQwen38_27bLaunchDsparkTapCapture(cudaStream_t stream, const void *hidden_bf16, const uint64_t *row_positions, void *ring_bf16, uint32_t tap_index, uint32_t row_count, uint32_t hidden_dimension, uint32_t capacity)
{
	dim3 grid(row_count, (hidden_dimension + 255u) / 256u);
	SparkQwen38_27bDsparkTapCaptureKernel<<<grid, 256u, 0u, stream>>>((const __nv_bfloat16 *)hidden_bf16, row_positions, (__nv_bfloat16 *)ring_bf16, tap_index, row_count, hidden_dimension, capacity);
	return(cudaGetLastError());
}
...
extern "C" cudaError_t SparkQwen38_27bLaunchDsparkTapStore(cudaStream_t stream, const void *hidden_bf16, const uint64_t *row_positions, void *taps_bf16, uint32_t rows, uint32_t tap_index, uint32_t hidden_dim, uint32_t tap_layers)
{
	...
	SparkQwen38_27bDsparkTapStoreKernel<<<grid, 256u, 0u, stream>>>((const __nv_bfloat16 *)hidden_bf16, row_positions, (__nv_bfloat16 *)taps_bf16, rows, tap_index, hidden_dim, tap_layers);
	return(cudaGetLastError());
}

extern "C" cudaError_t SparkQwen38_27bLaunchDsparkKPrep(cudaStream_t stream, void *k_bf16, const void *k_norm_bf16, const uint64_t *positions, uint32_t rows)
{
	dim3 grid(rows, 8u);
	SparkQwen38_27bDsparkKPrepKernel<<<grid, SPARK_QWEN38_27B_DSPARK_ATTN_HEAD_DIM, 0u, stream>>>((__nv_bfloat16 *)k_bf16, (const __nv_bfloat16 *)k_norm_bf16, positions, rows);
	return(cudaGetLastError());
}

extern "C" cudaError_t SparkQwen38_27bLaunchDsparkQPrep(cudaStream_t stream, void *q_bf16, const void *q_norm_bf16, const uint64_t *positions, uint32_t rows)
{
	dim3 grid(rows, 32u);
	SparkQwen38_27bDsparkQPrepKernel<<<grid, SPARK_QWEN38_27B_DSPARK_ATTN_HEAD_DIM, 0u, stream>>>((__nv_bfloat16 *)q_bf16, (const __nv_bfloat16 *)q_norm_bf16, positions, rows);
	return(cudaGetLastError());
}
...
extern "C" cudaError_t SparkQwen38_27bLaunchDsparkConv(cudaStream_t stream, const void *x_bf16, const void *delta_bf16, const void *base_bf16, void *out_bf16, uint32_t block_size, uint32_t num_groups, uint32_t group_size, uint32_t side)
{
	dim3 grid(block_size, num_groups);
	SparkQwen38_27bDsparkConvKernel<<<grid, group_size, 0u, stream>>>((const __nv_bfloat16 *)x_bf16, (const __nv_bfloat16 *)delta_bf16, (const __nv_bfloat16 *)base_bf16, (__nv_bfloat16 *)out_bf16, block_size, num_groups, group_size, side);
	return(cudaGetLastError());
}

extern "C" cudaError_t SparkQwen38_27bLaunchDsparkMarkov(cudaStream_t stream, const void *markov_w1_bf16, const void *markov_w2_bf16, const uint32_t *prev_token_ids, uint32_t draft_count, uint32_t rank, void *bias_out, uint32_t vocab)
{
	dim3 grid(draft_count, (vocab + 255u) / 256u);
	SparkQwen38_27bDsparkMarkovKernel<<<grid, 256u, 0u, stream>>>((const __nv_bfloat16 *)markov_w1_bf16, (const __nv_bfloat16 *)markov_w2_bf16, prev_token_ids, draft_count, rank, (float *)bias_out, vocab);
	return(cudaGetLastError());
}
 ...
 *
 * The head is read through the module's OWN head view (SparkQwen38_27bLinearView,
 * the same descriptor SparkQwen38_27bLaunchLinear consumes and the drafter host
...
{
	return((uint64_t)row_count * SPARK_QWEN38_27B_DSPARK_TOPK_CHUNK_COUNT * top_k);
}
...
extern "C" cudaError_t SparkQwen38_27bLaunchDsparkHeadTopK(cudaStream_t stream, const SparkQwen38_27bLinearView *head, const void *hidden_bf16, uint64_t *chunk_keys, uint32_t *top_candidate_ids, float *top_scores_f32, void *top_scores_bf16, uint32_t row_count, uint32_t candidate_offset, uint32_t top_k)
{
	...
	if ( head == 0 || hidden_bf16 == 0 || chunk_keys == 0 || top_candidate_ids == 0 || row_count == 0u )
		return(cudaErrorInvalidValue);
	if ( head->abi_version != SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_LINEAR_VIEW_ABI_VERSION || head->weight_payload == 0 )
		return(cudaErrorInvalidValue);
	if ( head->weight_format != SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_BF16 )
		return(cudaErrorInvalidValue);
	if ( top_k == 0u || top_k > SPARK_QWEN38_27B_DSPARK_TOPK_WIDTH || head->output_dimension < top_k )
		return(cudaErrorInvalidValue);
	if ( head->input_dimension == 0u || (head->input_dimension & 1u) != 0u )
		return(cudaErrorInvalidValue);
	shared_bytes = head->input_dimension * (uint32_t)sizeof(float);
	grid = dim3(row_count,SPARK_QWEN38_27B_DSPARK_TOPK_CHUNK_COUNT);
	SparkQwen38_27bDsparkHeadTopKChunkKernel<<<grid,SPARK_LM_CTA_THREADS,shared_bytes,stream>>>(hidden_bf16,head->weight_payload,chunk_keys,row_count,head->output_dimension,head->input_dimension,top_k);
	error = cudaGetLastError();
	if ( error != cudaSuccess )
		return(error);
	SparkQwen38_27bDsparkHeadTopKMergeKernel<<<row_count,SPARK_LM_CTA_THREADS,0,stream>>>(chunk_keys,top_candidate_ids,top_scores_f32,top_scores_bf16,row_count,candidate_offset,top_k);
	return(cudaGetLastError());
}
...
extern "C" cudaError_t SparkQwen38_27bLaunchDsparkSelectorProject(cudaStream_t stream, const void *hidden_bf16, const void *hidden_projection_bf16, void *context_gate_bf16, uint32_t row_count, uint32_t rank, uint32_t hidden_dimension)
{
	...
	if ( hidden_bf16 == 0 || hidden_projection_bf16 == 0 || context_gate_bf16 == 0 || row_count == 0u )
		return(cudaErrorInvalidValue);
	if ( rank == 0u || rank > SPARK_QWEN38_27B_DSPARK_SELECTOR_RANK )
		return(cudaErrorInvalidValue);
	if ( hidden_dimension == 0u || (hidden_dimension & 1u) != 0u )
		return(cudaErrorInvalidValue);
	shared_bytes = hidden_dimension * (uint32_t)sizeof(float);
	SparkQwen38_27bDsparkSelectorProjectKernel<<<row_count,SPARK_LM_CTA_THREADS,shared_bytes,stream>>>(hidden_bf16,hidden_projection_bf16,context_gate_bf16,row_count,rank,hidden_dimension);
	return(cudaGetLastError());
}
...
extern "C" cudaError_t SparkQwen38_27bLaunchDsparkSelectorEdges(cudaStream_t stream, const void *predecessor_bf16, const void *successor_bf16, const uint32_t *candidate_ids, const uint32_t *anchor_token_ids, const float *unary_f32, const void *context_gate_bf16, float *edges_f32, uint32_t batch_count, uint32_t slot_count, uint32_t top_k, uint32_t rank)
{
	...
	if ( batch_count == 0u || slot_count == 0u )
		return(cudaErrorInvalidValue);
	if ( top_k == 0u || top_k > SPARK_QWEN38_27B_DSPARK_SELECTOR_TOP_K || (top_k * top_k) > SPARK_LM_CTA_THREADS )
		return(cudaErrorInvalidValue);
	if ( rank == 0u || rank > SPARK_QWEN38_27B_DSPARK_SELECTOR_RANK )
		return(cudaErrorInvalidValue);
	SparkQwen38_27bDsparkSelectorEdgeKernel<<<batch_count * slot_count,SPARK_LM_CTA_THREADS,0,stream>>>(predecessor_bf16,successor_bf16,candidate_ids,anchor_token_ids,unary_f32,context_gate_bf16,edges_f32,batch_count,slot_count,top_k,rank);
	return(cudaGetLastError());
}
...
extern "C" cudaError_t SparkQwen38_27bLaunchDsparkSelectorWalk(cudaStream_t stream, const float *edges_f32, const uint32_t *candidate_ids, uint32_t *draft_token_ids, uint32_t *draft_candidate_slots, uint32_t batch_count, uint32_t slot_count, uint32_t top_k)
{
	if ( edges_f32 == 0 || candidate_ids == 0 || draft_token_ids == 0 )
		return(cudaErrorInvalidValue);
	if ( batch_count == 0u || slot_count == 0u || top_k == 0u || top_k > SPARK_QWEN38_27B_DSPARK_SELECTOR_TOP_K )
		return(cudaErrorInvalidValue);
	SparkQwen38_27bDsparkSelectorWalkKernel<<<(batch_count + SPARK_LM_CTA_THREADS - 1u) / SPARK_LM_CTA_THREADS,SPARK_LM_CTA_THREADS,0,stream>>>(edges_f32,candidate_ids,draft_token_ids,draft_candidate_slots,batch_count,slot_count,top_k);
	return(cudaGetLastError());
}
...
{
	cudaError_t error;
	error = SparkQwen38_27bLaunchDsparkSelectorProject(stream,hidden_bf16,hidden_projection_bf16,context_gate_bf16,batch_count * slot_count,rank,hidden_dimension);
	if ( error == cudaSuccess )
		error = SparkQwen38_27bLaunchDsparkSelectorEdges(stream,predecessor_bf16,successor_bf16,candidate_ids,anchor_token_ids,unary_f32,context_gate_bf16,edges_f32,batch_count,slot_count,top_k,rank);
	if ( error == cudaSuccess )
		error = SparkQwen38_27bLaunchDsparkSelectorWalk(stream,edges_f32,candidate_ids,draft_token_ids,draft_candidate_slots,batch_count,slot_count,top_k);
	return(error);
}

/* origin/main's device-side selector front-end: fused top-16 + hidden
 * projection per draft slot, kept alongside the W4/W3 suite above. */
