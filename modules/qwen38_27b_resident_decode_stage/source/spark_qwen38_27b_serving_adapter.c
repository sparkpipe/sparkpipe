...
#include "spark_qwen38_27b_paged_kv.h"
...
#define SPARK_QWEN38_27B_SERVING_MAX_SEQUENCE_POSITIONS_CAP 8192u
...
#define SPARK_QWEN38_27B_SERVING_PREFIX_CACHE_ENV "SPARK_QWEN38_27B_SERVING_PREFIX_CACHE"
static uint32_t SparkQwen38_27bServingPrefixCacheEnabled(void)
{
	const char *value = getenv(SPARK_QWEN38_27B_SERVING_PREFIX_CACHE_ENV);
...
/* Round audit. With SPARK_QWEN38_27B_SPEC_AUDIT set (and not "0") the replay frame
...
static uint32_t SparkQwen38_27bServingSpecAudit(void)
{
	const char *text = getenv("SPARK_QWEN38_27B_SPEC_AUDIT");
...
static uint32_t SparkQwen38_27bServingCreditCeiling(uint32_t draft_count)
{
 ...
 * zero SPARK_QWEN38_27B_SERVING_SPECULATIVE_DRAFT_COUNT means the FULL block,
...
typedef struct SparkQwen38_27bServingPending
{
	...
	uint32_t resume_tokens[SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT];
	uint32_t resume_module_slot[SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_MAX_ACTIVE_SEQUENCE_COUNT];
...
typedef struct SparkQwen38_27bServingState
{
	 ...
	 * SPARK_QWEN38_27B_STAGE_GDN_SNAPSHOT_SLOTS: speculation keeps slot 0. */
	uint32_t checkpoint_slot_base;
	uint32_t checkpoint_slot_count;
	SparkQwen38_27bPagedKv paged;
...
#define SPARK_QWEN38_27B_SERVING_SET_UNSIGNED(name,number) \
		...
		(state->stage_layer_count - state->stage_attn_layer_count) != 0u )
	{
		...
		SPARK_QWEN38_27B_SERVING_SET_TEXT("SPARK_QWEN38_27B_STAGE_MTP","0");
		SPARK_QWEN38_27B_SERVING_SET_UNSIGNED("SPARK_QWEN38_27B_STAGE_GDN_SNAPSHOT_SLOTS",SPARK_QWEN38_27B_SERVING_GDN_SNAPSHOT_SLOTS);
	}
	else
	{
		SPARK_QWEN38_27B_SERVING_SET_TEXT("SPARK_QWEN38_27B_STAGE_MTP","0");
 ...
 * prefix-entry protocol (SPARK_QWEN38_27B_SERVING_KV_FLAT=1 selects that whole
 * backend); unified's paged pool remains the default ledger. */
static uint32_t SparkQwen38_27bServingKvFlatMode(void)
{
	const char *env = getenv("SPARK_QWEN38_27B_SERVING_KV_FLAT");
...
static void SparkQwen38_27bServingReleaseLane(
	...
	uint32_t slot)
{
	if ( SparkQwen38_27bServingKvFlatMode() != 0u )
	{
		uint32_t ordinal;
		...
		SparkQwen38_27bPagedKvLaneReset(&state->paged,slot);
...
static uint32_t SparkQwen38_27bServingGatherLaneTokens(
	const SparkModelServingSubmission *submission,
	uint32_t lane,
	...
	uint64_t end_position)
{
	...
	committed = SparkQwen38_27bPagedKvCommittedTokens(&state->paged,slot);
	token_count = SparkQwen38_27bServingGatherLaneTokens(submission,lane,
		committed,state->match_tokens,state->max_input_row_count);
	return(SparkQwen38_27bPagedKvCover(&state->paged,slot,end_position,
...
static uint32_t SparkQwen38_27bServingMatchPrefix(
	SparkQwen38_27bServingState *state,
	const SparkModelServingSubmission *submission,
	uint32_t lane,
	uint32_t *module_slot_out)
{
	SparkQwen38_27bPagedKvMatch match;
	...
	if ( state->prefix_enabled == 0u || state->stage_attn_layer_count == 0u ||
		...
		return(0u);
	count = SparkQwen38_27bServingGatherLaneTokens(submission,lane,0,tokens,
		state->max_input_row_count);
	slot = submission->lanes[lane].resident_sequence_slot;
	*module_slot_out = SPARK_QWEN38_27B_PAGED_KV_NO_SLOT;
	if ( count == 0u )
		return(0u);
	...
	if ( SparkQwen38_27bPagedKvAdmit(&state->paged,slot,tokens,count,
		&match) != SPARK_STATUS_OK || match.block_count == 0u )
		return(0u);
	*module_slot_out = state->checkpoint_slot_base + match.checkpoint_slot;
	fprintf(stderr,"qwen38_27b_prefix_reuse lane=%u slot=%u matched_blocks=%u resumed_at=%u checkpoint_slot=%u\n",
	...
	SparkQwen38_27bServingPending *pending)
{
	...
	for (lane=0u; lane<submission->active_sequence_count; lane++)
	{
		...
		pending->resume_module_slot[lane] = SPARK_QWEN38_27B_PAGED_KV_NO_SLOT;
		...
		if ( first_position == 0u && end_position != 0u )
			pending->resume_tokens[lane] =
				SparkQwen38_27bServingMatchPrefix(state,submission,lane,
					&pending->resume_module_slot[lane]);
		status = SparkQwen38_27bServingCoverLane(state,submission,lane,
			end_position);
		if ( status != SPARK_STATUS_OK )
		{
	...
	const SparkModelServingSubmission *submission)
{
	...
	if ( total_needed > SparkQwen38_27bPagedKvFreeBlocks(&state->paged) )
		return(SPARK_STATUS_CAPACITY_EXCEEDED);
	for (lane=0u; lane<submission->active_sequence_count; lane++)
	{
...
static uint64_t SparkQwen38_27bServingLaneRowPosition(
	const SparkModelServingSubmission *submission,
	uint32_t lane,
...
static void SparkQwen38_27bServingFillContextCommon(
	SparkQwen38_27bServingState *state,
	const SparkModelServingSubmission *submission,
...
static void SparkQwen38_27bServingFillFrameCommon(SparkQwen38_27bServingState *state, const SparkModelServingSubmission *submission,
	SparkQwen38_27bServingPending *pending, uint32_t prefill, uint32_t frame_rows, uint64_t frame_sequence_id,
	uint64_t frame_sequence_position, uint32_t active_slot_count, SparkQwen38_27bResidentDecodeStageFrameContext *context,
...
static void SparkQwen38_27bServingFillTokenBuffers(
	SparkQwen38_27bServingState *state,
	SparkQwen38_27bServingPending *pending,
	uint32_t frame_rows,
	...
	SparkModelDriverFrame *frame)
{
	...
	if ( snapshot_index != SPARK_QWEN38_27B_PAGED_KV_NO_SLOT )
	{
	...
	SparkQwen38_27bServingFillContextCommon(state,submission,context,frame_rows);
	if ( prefill != 0u )
	{
		...
		else if ( state->lane_publish_armed[slot] != 0u && base_position + frame_rows == state->lane_publish_tokens[slot] )
		{
			...
			if ( publish_status == SPARK_STATUS_OK && state->lane_prefix_entry[slot] != 0xFFu )
			{
				...
				if ( (extra_flags & SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_GDN_CHECKPOINT) != 0u )
				{
	...
	SparkQwen38_27bServingFillTokenBuffers(state,pending,frame_rows,prefill != 0u ? 1u : frame_rows,buffers);
	SparkQwen38_27bServingFillFrameCommon(state,submission,pending,prefill,frame_rows,
	...
	uint32_t snapshot_index)
{
	...
	SparkModelDriverFrame frame;
	SparkStatus status;
	SparkQwen38_27bServingBuildFrame(state,submission,pending,prefill,lane,wave_base,frame_rows,extra_flags,snapshot_index,&gdn_snapshot,&decode_batch,&prefill_view,&context,buffers,&frame);
	...
	SparkModelDriverFrame *frame)
{
	...
	if ( gdn_snapshot != 0 )
		context->gdn_snapshot = gdn_snapshot;
	SparkQwen38_27bServingFillTokenBuffers(state,pending,frame_rows,output_id_count,buffers);
	SparkQwen38_27bServingFillFrameCommon(state,submission,pending,prefill,frame_rows,frame_sequence_id,frame_sequence_position,1u,context,buffers,frame);
}
	...
	SparkQwen38_27bServingPending *pending)
{
	...
	for (lane=0u; status == SPARK_STATUS_OK && lane<submission->active_sequence_count; lane++)
	{
		...
		if ( SparkQwen38_27bServingBlockDraftMethod(spec_method) )
		{
			...
			dspark_draft.block_size = SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_DSPARK_BLOCK_SIZE;
			dspark_draft.draft_token_count = SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_DSPARK_BLOCK_SIZE - 1u;
	 ...
	 * round shape (SPARK_QWEN38_27B_DFLASH2_BONUS_FOLD, dflash2_draft_matrix)
 ...
 * dflash2 tool battery still exercises them there. */
		...
		if ( status == SPARK_STATUS_OK )
		{
			 ...
			 * CHANGE. */
			{
				...
				uint64_t diag_committed = state->paged_ready != 0u ?
					SparkQwen38_27bPagedKvCommittedTokens(&state->paged,diag_slot) : 0ull;
				uint64_t diag_committed_blocks = (diag_committed +
					SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS - 1u) /
				...
				fprintf(stderr, "qwen38_27b_spec_diag lane=%u base_position=%llu t=%.6f C0=%u accepted=%u drafts=[%u,%u,%u,%u,%u,%u,%u,%u] emitted=[%u,%u,%u,%u,%u,%u,%u,%u] matched_blocks=%u borrowed=%u\n",
	...
	if ( status == SPARK_STATUS_OK )
	{
		 ...
		 * deployed log shows: six "qwen38_27b_spec accepted=6" lines and six
		 ...
		 * consistent if enabled; the clamp diag line survives from unified. */
		{
			...
			{
				fprintf(stderr,"qwen38_27b_spec accept_clamped accepted=%u credited=%u cap=%u\n",
		...
		for (lane=0u; lane<submission->active_sequence_count; lane++)
			fprintf(stderr,"qwen38_27b_spec round_commit lane=%u base_position=%llu accepted=%u min_accepted=%u credited=%u ceiling=%u positions=[%llu..%llu]\n",
				...
				SparkQwen38_27bServingCreditCeiling(draft_count),
	...
	for (lane=0u; status == SPARK_STATUS_OK && pending->spec_chain_dead == 0u && lane<submission->active_sequence_count; lane++)
	{
		 ...
		 * same frame under SPARK_QWEN38_27B_DFLASH2_STATE_SELECT - the module then
		 ...
		 * bonus-fold deferral above. */
		{
			...
			status = SparkQwen38_27bServingRunSpeculativeFrame(state,submission,pending,slot,1u,replay_tokens,0,0,replay_rows,replay_base,spec->sequence_id,replay_base,
				...
				(spec_audit != 0u ? SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_SPEC_AUDIT_EMIT_ALL : 0u) |
				(sel_on != 0u ? SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_GDN_RESTORE_VERIFY_ROW : 0u),
			...
			if ( spec_audit != 0u )
			{
				...
				for (row=0u; row<=min_accepted; row++)
					if ( pending->frame_output_ids[row] != spec->emitted_ids[row] )
						fprintf(stderr,"qwen38_27b_spec_audit replay_row_mismatch lane=%u base_position=%llu row=%u position=%llu verify=%u replay=%u accepted=%u min_accepted=%u\n",
				 ...
				 * block credits ten. */
				fprintf(stderr,"qwen38_27b_spec_audit round_ids lane=%u positions=[%llu..%llu] committed=[%u,%u,%u,%u,%u,%u,%u,%u,%u,%u] credited=%u\n",
					lane,(unsigned long long)spec->base_position,
	...
	for (lane=0u; lane<submission->active_sequence_count; lane++)
	{
		SparkQwen38_27bServingSpecState *spec = &pending->spec[lane];
		SparkQwen38_27bGdnSnapshotView repair_snapshot;
		...
		repair_snapshot.abi_version = SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_GDN_SNAPSHOT_VIEW_ABI_VERSION;
		...
		fprintf(stderr,"qwen38_27b_spec rollback_repair lane=%u base_position=%llu token=%u (verify walked, no replay)\n",
			lane,(unsigned long long)spec->base_position,repair_token);
		repair_status = SparkQwen38_27bServingRunSpeculativeFrame(state,submission,pending,spec->resident_slot,1u,&repair_token,0,0,1u,spec->base_position,spec->sequence_id,spec->base_position,SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_GDN_RESTORE_FIRST,0,0,&repair_snapshot,1u);
		if ( repair_status != SPARK_STATUS_OK )
			fprintf(stderr,"qwen38_27b_spec rollback_repair_failed lane=%u status=%d\n",lane,(int)repair_status);
		if ( status == SPARK_STATUS_OK )
	...
	SparkStatus status)
{
	...
	}
	{
		...
		fprintf(stderr,"qwen38_27b_residency sid=%llu pending_sid=%llu pending_origin=%llu hex="
	...
	const SparkModelServingSubmission *submission)
{
	...
	speculate = 0u;
	status = SparkQwen38_27bServingCoverSubmission(state,submission,pending);
	...
	else if ( status == SPARK_STATUS_OK && submission->work_kind == SPARK_MODEL_SERVING_WORK_KIND_DECODE )
	{
		...
		state->dflash2_fold_armed = 0u;
		status = SparkQwen38_27bServingRunFrame(state,submission,pending,0u,0u,0u,submission->row_count,0u,SPARK_QWEN38_27B_PAGED_KV_NO_SLOT);
	}
	else if ( status == SPARK_STATUS_OK && submission->work_kind == SPARK_MODEL_SERVING_WORK_KIND_PREFILL )
	{
		...
		for (lane=0u; status == SPARK_STATUS_OK && lane<submission->active_sequence_count; lane++)
		{
			...
			for (wave=wave_start; status == SPARK_STATUS_OK && wave<lane_rows; wave+=chunk_rows)
			{
				...
				extra_flags = 0u;
				snapshot_index = SPARK_QWEN38_27B_PAGED_KV_NO_SLOT;
				offered_slot = SPARK_QWEN38_27B_PAGED_KV_NO_SLOT;
				base_position = SparkQwen38_27bServingLaneRowPosition(submission,lane,wave);
				end_position = base_position + chunk_rows;
				boundary_distance =
					(((base_position + SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS - 1u) /
					SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS) *
					SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS) - base_position;
				if ( boundary_distance == 0u )
					boundary_distance = SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS;
				...
				{
					extra_flags = SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_PREFIX_RESUME;
					snapshot_index = pending->resume_module_slot[lane];
					pending->resume_tokens[lane] = 0u;
				}
				else if ( state->checkpoint_slot_count != 0u &&
					extra_flags == 0u &&
					end_position % SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS == 0u )
				{
					uint32_t offer_slot;
					if ( SparkQwen38_27bPagedKvCheckpointOffer(&state->paged,
						submission->lanes[lane].resident_sequence_slot,
						end_position,&offer_slot) != 0u )
					{
						extra_flags = SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_GDN_CHECKPOINT;
						snapshot_index = state->checkpoint_slot_base + offer_slot;
						offered_slot = offer_slot;
					}
				}
				status = SparkQwen38_27bServingRunFrame(state,submission,pending,1u,lane,wave,chunk_rows,extra_flags,snapshot_index);
				...
				if ( status == SPARK_STATUS_OK )
				{
					if ( offered_slot != SPARK_QWEN38_27B_PAGED_KV_NO_SLOT )
						SparkQwen38_27bPagedKvCheckpointCommit(&state->paged,
							submission->lanes[lane].resident_sequence_slot,
							offered_slot,end_position);
				}
				else if ( offered_slot != SPARK_QWEN38_27B_PAGED_KV_NO_SLOT )
					SparkQwen38_27bPagedKvCheckpointAbort(&state->paged,
						submission->lanes[lane].resident_sequence_slot,
						offered_slot);
			}
		}
	...
	uint32_t prepare)
{
	...
	if ( adapter_state == 0 || submission == 0 )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	return(((SparkQwen38_27bServingState *)adapter_state)->quiescing != 0u ?
...
static void SparkQwen38_27bServingDestroy(void *adapter_state)
{
	...
	if ( state->paged_ready != 0u )
		SparkQwen38_27bPagedKvDestroy(&state->paged);
...
{
	uint64_t indices;
	SparkQwen38_27bPagedKvConfiguration paged_configuration;
	SparkStatus status;
	 ...
	 * runs under SPARK_QWEN38_27B_SERVING_KV_FLAT=1) alongside unified's
	...
	paged_configuration.block_token_count = SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS;
	...
	/* qwen38_27b keeps its KV row layout firmware-side, so no host byte
	...
	paged_configuration.block_stride_bytes =
		...
		SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS;
	status = SparkQwen38_27bPagedKvInitialize(&state->paged,&paged_configuration,
	...
	void **adapter_state)
{
	...
	*adapter_state = 0;
	...
	state->spec_draft_count = SparkQwen38_27bServingActiveDraftCount(state->spec_method);
	state->spec_first_draft_policy = SparkQwen38_27bServingSpecFirstDraftPolicy();
	state->spec_audit = SparkQwen38_27bServingSpecAudit();
	...
	fprintf(stderr,"qwen38_27b_serving spec_method=%s (%s=%s)\n",
		state->spec_method == SPARK_QWEN38_27B_SERVING_SPEC_METHOD_DSPARK ? "dspark" : "mtp",
		SPARK_QWEN38_27B_SERVING_SPEC_METHOD_ENV,
		getenv(SPARK_QWEN38_27B_SERVING_SPEC_METHOD_ENV) != 0 ? getenv(SPARK_QWEN38_27B_SERVING_SPEC_METHOD_ENV) : "(unset)");
	...
	if ( status == SPARK_STATUS_OK )
	{
		...
		uint32_t derived_pool = state->resident_sequence_capacity * ((max_sequence_positions + SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS - 1u) / SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS);
		...
		state->prefix_enabled = SparkQwen38_27bServingPrefixCacheEnabled();
	...
	if ( status == SPARK_STATUS_OK )
	{
		...
		uint32_t snapshots_armed = SparkQwen38_27bServingSpeculationEnabled() != 0u &&
			SparkQwen38_27bServingOwnsFinalHead(state) != 0u ? 1u :
			(state->prefix_enabled != 0u && gdn_layers != 0u ? 1u : 0u);
		(void)snapshots_armed;
		state->checkpoint_slot_base = SparkQwen38_27bServingSpeculationEnabled() != 0u &&
			SparkQwen38_27bServingOwnsFinalHead(state) != 0u ? 1u : 0u;
		state->checkpoint_slot_count = SPARK_QWEN38_27B_SERVING_GDN_SNAPSHOT_SLOTS -
