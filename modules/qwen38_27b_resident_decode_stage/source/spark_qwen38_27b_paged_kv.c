...
#define SPARK_QWEN38_27B_PAGED_KV_SEQUENCE_BASE UINT64_C(0x5133600000000000)

static uint64_t SparkQwen38_27bPagedKvSequenceId(uint32_t lane)
{
	return SPARK_QWEN38_27B_PAGED_KV_SEQUENCE_BASE + (uint64_t)lane;
}

static uint32_t SparkQwen38_27bPagedKvRowBase(const SparkQwen38_27bPagedKv *cache,
	uint32_t lane)
{
...
static SparkStatus SparkQwen38_27bPagedKvScratchBorrow(
	SparkQwen38_27bPagedKv *cache, uint32_t *block_out)
{
	 ...
	 * in-use scratch as free, so SparkQwen38_27bPagedKvFreeBlocks() and
 ...
 * needs clearing). */
static void SparkQwen38_27bPagedKvScratchReturn(SparkQwen38_27bPagedKv *cache,
	uint32_t block)
{
...
static void SparkQwen38_27bPagedKvSyncRow(SparkQwen38_27bPagedKv *cache,
	uint32_t lane)
{
	uint32_t row_base = SparkQwen38_27bPagedKvRowBase(cache,lane);
	...
	if ( cache->lane_live[lane] != 0u )
	{
		...
		tokens = (uint32_t)SparkPrefixCacheCoreSequenceTokenCount(
			&cache->core,SparkQwen38_27bPagedKvSequenceId(lane));
		...
		if ( blocks != 0u )
			(void)SparkPrefixCacheCoreBuildBlockTable(&cache->core,
				SparkQwen38_27bPagedKvSequenceId(lane),tokens,
...
SparkStatus SparkQwen38_27bPagedKvInitialize(SparkQwen38_27bPagedKv *cache,
	...
	uint32_t *blocks_by_lane, uint32_t *counts_by_lane)
{
	...
	if ( cache == 0 || configuration == 0 || blocks_by_lane == 0 ||
		...
		configuration->blocks_per_lane >
			SPARK_QWEN38_27B_PAGED_KV_MAX_BLOCKS_PER_LANE ||
	...
	cache->reserved_slot = SPARK_QWEN38_27B_PAGED_KV_NO_SLOT;
	cache->reserved_lane = SPARK_QWEN38_27B_PAGED_KV_NO_LANE;
	...
	{
		cache->checkpoints = (SparkQwen38_27bPagedKvCheckpoint *)calloc(
		...
		for (index=0u; index<configuration->checkpoint_slot_count; index++)
			cache->checkpoints[index].witness_block =
				SPARK_QWEN38_27B_PAGED_KV_NO_BLOCK;
	}
...
void SparkQwen38_27bPagedKvLaneReset(SparkQwen38_27bPagedKv *cache, uint32_t lane)
{
	...
	if ( cache == 0 || lane >= cache->configuration.lane_count )
		return;
	row_base = SparkQwen38_27bPagedKvRowBase(cache,lane);
	if ( cache->lane_live[lane] != 0u )
	{
		(void)SparkPrefixCacheCoreReleaseSequence(&cache->core,
			SparkQwen38_27bPagedKvSequenceId(lane));
		...
		index<cache->counts_by_lane[lane]; index++)
	{
		...
		if ( block != SPARK_QWEN38_27B_PAGED_KV_NO_BLOCK )
			SparkQwen38_27bPagedKvScratchReturn(cache,block);
	}
...
static uint32_t SparkQwen38_27bPagedKvWitnessedDepth(
	...
	uint32_t *slot_out)
{
	...
	for (boundary=limit_blocks; boundary>=1u; boundary--)
	{
		for (slot=0u; slot<cache->configuration.checkpoint_slot_count;
			slot++)
		{
			const SparkQwen38_27bPagedKvCheckpoint *checkpoint =
...
SparkStatus SparkQwen38_27bPagedKvAdmit(SparkQwen38_27bPagedKv *cache,
	...
	SparkQwen38_27bPagedKvMatch *match_out)
{
	...
	uint32_t attached[SPARK_QWEN38_27B_PAGED_KV_MAX_BLOCKS_PER_LANE];
	uint32_t attached_count,lcp_tokens,resumed_blocks,readmit_tokens,slot;
	uint32_t block_tokens;
	SparkStatus status;
	match.block_count = 0u;
	match.checkpoint_slot = SPARK_QWEN38_27B_PAGED_KV_NO_SLOT;
	if ( match_out != 0 )
	{
		match_out->block_count = 0u;
		match_out->checkpoint_slot = SPARK_QWEN38_27B_PAGED_KV_NO_SLOT;
	}
	if ( cache == 0 || lane >= cache->configuration.lane_count ||
	...
	SparkQwen38_27bPagedKvLaneReset(cache,lane);
	if ( cache->reuse_enabled == 0u )
		return(SPARK_STATUS_OK);
	status = SparkPrefixCacheCoreAdmitSequence(&cache->core,
		SparkQwen38_27bPagedKvSequenceId(lane),tokens,token_count,
		&lcp_tokens);
	if ( status != SPARK_STATUS_OK )
	{
		SparkQwen38_27bPagedKvLaneReset(cache,lane);
	...
	SparkQwen38_27bPagedKvSyncRow(cache,lane);
	if ( match_out != 0 )
		*match_out = match;
	return(SPARK_STATUS_OK);
}

SparkStatus SparkQwen38_27bPagedKvCover(SparkQwen38_27bPagedKv *cache,
	...
	uint32_t token_count)
{
	...
	{
		status = SparkQwen38_27bPagedKvScratchBorrow(cache,&block);
		if ( status != SPARK_STATUS_OK )
		{
...
uint64_t SparkQwen38_27bPagedKvCommittedTokens(
	const SparkQwen38_27bPagedKv *cache, uint32_t lane)
{
	if ( cache == 0 || lane >= cache->configuration.lane_count ||
		cache->lane_live[lane] == 0u )
		return(0u);
...
uint32_t SparkQwen38_27bPagedKvCheckpointOffer(SparkQwen38_27bPagedKv *cache,
	uint32_t lane, uint64_t end_position, uint32_t *slot_out)
{
	...
	if ( cache == 0 || slot_out == 0 ||
		...
		return(0u);
	*slot_out = SPARK_QWEN38_27B_PAGED_KV_NO_SLOT;
	if ( cache->reuse_enabled == 0u ||
		cache->reserved_slot != SPARK_QWEN38_27B_PAGED_KV_NO_SLOT ||
		cache->lane_live[lane] == 0u )
		return(0u);
	...
	boundary = (uint32_t)(end_position /
		cache->configuration.block_token_count);
	victim_slot = SPARK_QWEN38_27B_PAGED_KV_NO_SLOT;
	...
	if ( victim_slot == SPARK_QWEN38_27B_PAGED_KV_NO_SLOT )
	{
...
void SparkQwen38_27bPagedKvCheckpointCommit(SparkQwen38_27bPagedKv *cache,
	uint32_t lane, uint32_t slot, uint64_t end_position)
{
	SparkQwen38_27bPagedKvCheckpoint *checkpoint;
	...
	if ( end_position >
		SparkQwen38_27bPagedKvCommittedTokens(cache,lane) )
		return;
	row_base = SparkQwen38_27bPagedKvRowBase(cache,lane);
	witness = cache->blocks_by_lane[row_base + boundary - 1u];
	if ( witness == SPARK_QWEN38_27B_PAGED_KV_NO_BLOCK )
		return;
...
void SparkQwen38_27bPagedKvCheckpointAbort(SparkQwen38_27bPagedKv *cache,
