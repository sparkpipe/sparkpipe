...
/* Geometry lives in spark_qwen38_27b_dspark_format.h (QUERY_HEADS, KV_HEADS,
 ...
 *  - unified HEAD's full-sequence context path: SparkQwen38_27bDsparkAttnKernel
 ...
 *    SparkQwen38_27bDsparkTapCaptureKernel feeding that ring, plus the W4/W3
 *    candidate-selector kernels.
 *  - origin/main's cache path: SparkQwen38_27bDsparkTapStoreKernel (per-position
 *    tap history), the K/Q prep kernels, SparkQwen38_27bDsparkCacheAttnKernel
 *    over the draft-side block-KV history (HF DynamicCache semantics, NeoX
 *    full-head rope) and the SparkQwen38_27bDsparkSelectKernel front-end.
 ...
 */
static __global__ void SparkQwen38_27bDsparkAttnKernel(
	...
	__nv_bfloat16 *attn_out_bf16, uint32_t block_size, uint64_t base_position, uint32_t context_length)
{
	...
	const uint32_t q_heads_per_group = SPARK_QWEN38_27B_DSPARK_ATTN_QUERY_HEADS / kv_heads;
	...
	/* Q: weighted head RMSNorm (single [128] weight shared across heads) + rope. */
	{
		...
		for (d = 0u; d < head_dim; d++)
			qn[d] = __bfloat162float(q_bf16[((uint64_t)q_pos * SPARK_QWEN38_27B_DSPARK_ATTN_QUERY_HEADS + q_head) * head_dim + d]);
		#pragma unroll
		for (d = 0u; d < head_dim; d++)
	...
	#pragma unroll
	for (d = 0u; d < SPARK_QWEN38_27B_DSPARK_ATTN_ROPE_DIM; d += 2u)
	{
		float f = pos * SparkQwen38_27bDsparkRopeFrequency(d >> 1u);
	...
	#pragma unroll
	for (d = 0u; d < head_dim; d++)
		attn_out_bf16[((uint64_t)q_pos * SPARK_QWEN38_27B_DSPARK_ATTN_QUERY_HEADS + q_head) * head_dim + d] = __float2bfloat16(acc[d] / sum_exp);
}
 ...
 */
static __global__ void SparkQwen38_27bDsparkTapCaptureKernel(
	...
	uint32_t hidden_dimension, uint32_t capacity)
{
	...
	slot = (row_positions[row] & (uint64_t)(capacity - 1u)) *
		(uint64_t)SPARK_QWEN38_27B_DSPARK_TAP_ROW_DIMENSION + (uint64_t)tap_index * hidden_dimension;
...
#define SPARK_QWEN38_27B_DSPARK_TOPK_CHUNK_COUNT SPARK_LM_HEAD_FALLBACK_CHUNK_COUNT
/* The lattice width IS the pack's selector_top_k; the kernels accept any
 * top_k <= this and the launchers reject anything wider. */
#define SPARK_QWEN38_27B_DSPARK_TOPK_WIDTH SPARK_QWEN38_27B_DSPARK_SELECTOR_TOP_K
/* Per-CTA candidate pool in stage one: one 16-deep list per warp. */
#define SPARK_QWEN38_27B_DSPARK_TOPK_WARP_POOL (SPARK_LM_CTA_WARPS * SPARK_QWEN38_27B_DSPARK_TOPK_WIDTH)
...
#define SPARK_QWEN38_27B_DSPARK_TOPK_MERGE_POOL (SPARK_QWEN38_27B_DSPARK_TOPK_CHUNK_COUNT * SPARK_QWEN38_27B_DSPARK_TOPK_WIDTH)
...
static __device__ __forceinline__ uint64_t SparkQwen38_27bDsparkTopKKey(float score, uint32_t candidate)
{
	if ( score <= -3.0e38f )
		return(0ull);
...
static __device__ __forceinline__ uint32_t SparkQwen38_27bDsparkTopKKeyCandidate(uint64_t key)
{
	return(0xffffffffu - (uint32_t)key);
}

/* Inverse of SparkLmOrderedTopKKey's monotone float mapping. */
static __device__ __forceinline__ float SparkQwen38_27bDsparkTopKKeyScore(uint64_t key)
{
...
static __device__ __forceinline__ void SparkQwen38_27bDsparkTopKInsert(uint64_t *ordered, uint32_t width, uint64_t key)
{
	uint32_t slot;
 ...
 */
static __global__ void SparkQwen38_27bDsparkHeadTopKChunkKernel(
	...
	uint32_t row_count, uint32_t candidate_count, uint32_t hidden_dimension, uint32_t top_k)
{
	...
	__shared__ uint64_t candidate_pool[SPARK_QWEN38_27B_DSPARK_TOPK_WARP_POOL];
	...
	if ( row >= row_count )
		return;
	for (slot = threadIdx.x; slot < SPARK_QWEN38_27B_DSPARK_TOPK_WARP_POOL; slot += blockDim.x)
	...
	first_candidate = (uint32_t)(((uint64_t)candidate_count * chunk) / SPARK_QWEN38_27B_DSPARK_TOPK_CHUNK_COUNT);
	end_candidate = (uint32_t)(((uint64_t)candidate_count * (chunk + 1u)) / SPARK_QWEN38_27B_DSPARK_TOPK_CHUNK_COUNT);
	for (candidate = first_candidate + warp; candidate < end_candidate; candidate += SPARK_LM_CTA_WARPS)
	{
		 ...
		 * on the way out. */
		if ( lane == 0u )
			SparkQwen38_27bDsparkTopKInsert(candidate_pool + ((uint64_t)warp * SPARK_QWEN38_27B_DSPARK_TOPK_WIDTH),
				SPARK_QWEN38_27B_DSPARK_TOPK_WIDTH,
				SparkQwen38_27bDsparkTopKKey(__bfloat162float(__float2bfloat16(score)),candidate));
	}
	__syncthreads();
	SparkLmBitonicSortKeysAscending<SPARK_QWEN38_27B_DSPARK_TOPK_WARP_POOL>(candidate_pool);
	for (slot = threadIdx.x; slot < top_k; slot += blockDim.x)
		chunk_keys[((((uint64_t)row * SPARK_QWEN38_27B_DSPARK_TOPK_CHUNK_COUNT) + chunk) * top_k) + slot] =
			candidate_pool[(SPARK_QWEN38_27B_DSPARK_TOPK_WARP_POOL - 1u) - slot];
}
 ...
 */
static __global__ void SparkQwen38_27bDsparkHeadTopKMergeKernel(
...
{
	__shared__ uint64_t candidate_pool[SPARK_QWEN38_27B_DSPARK_TOPK_MERGE_POOL];
	uint32_t row = blockIdx.x,slot,pool_count;
	uint64_t key;
	float score;
	if ( row >= row_count )
		return;
	pool_count = SPARK_QWEN38_27B_DSPARK_TOPK_CHUNK_COUNT * top_k;
	for (slot = threadIdx.x; slot < SPARK_QWEN38_27B_DSPARK_TOPK_MERGE_POOL; slot += blockDim.x)
		candidate_pool[slot] = slot < pool_count ? chunk_keys[((uint64_t)row * pool_count) + slot] : 0ull;
	__syncthreads();
	SparkLmBitonicSortKeysAscending<SPARK_QWEN38_27B_DSPARK_TOPK_MERGE_POOL>(candidate_pool);
	for (slot = threadIdx.x; slot < top_k; slot += blockDim.x)
	{
		key = candidate_pool[(SPARK_QWEN38_27B_DSPARK_TOPK_MERGE_POOL - 1u) - slot];
		score = key != 0ull ? SparkQwen38_27bDsparkTopKKeyScore(key) : -3.4028235e38f;
		top_candidate_ids[((uint64_t)row * top_k) + slot] =
			key != 0ull ? SparkQwen38_27bDsparkTopKKeyCandidate(key) + candidate_offset : UINT32_MAX;
		if ( top_scores_f32 != 0 )
 ...
 */
static __global__ void SparkQwen38_27bDsparkSelectorProjectKernel(
 ...
 */
static __global__ void SparkQwen38_27bDsparkSelectorEdgeKernel(
...
{
	__shared__ float gate_shared[SPARK_QWEN38_27B_DSPARK_SELECTOR_TOP_K * SPARK_QWEN38_27B_DSPARK_SELECTOR_RANK];
	__shared__ float successor_shared[SPARK_QWEN38_27B_DSPARK_SELECTOR_RANK * SPARK_QWEN38_27B_DSPARK_SELECTOR_TOP_K];
	__shared__ float context_shared[SPARK_QWEN38_27B_DSPARK_SELECTOR_RANK];
	__shared__ uint32_t predecessor_ids[SPARK_QWEN38_27B_DSPARK_SELECTOR_TOP_K];
	__shared__ uint32_t successor_ids[SPARK_QWEN38_27B_DSPARK_SELECTOR_TOP_K];
 ...
 */
static __global__ void SparkQwen38_27bDsparkSelectorWalkKernel(
