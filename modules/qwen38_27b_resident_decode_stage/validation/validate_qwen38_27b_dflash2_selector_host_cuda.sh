...
#
# usage: validate_qwen38_27b_dflash2_selector_host_cuda.sh [smoke|mid|full|tie] [OUTPUT_DIRECTORY]
set -euo pipefail
...
binary="${output_directory}/spark_qwen38_27b_dflash2_selector_host_validation"
...
"${nvcc_binary}" -std=c++17 -gencode "arch=compute_${cuda_architecture#sm_},code=${cuda_architecture}" \
    ...
    -I"${repository_root}/model-families/common/include" -I"${repository_root}/model-families/qwen38_27b/include" \
    -I"${module_directory}/include" -I"${module_directory}/source" \
    "${script_directory}/spark_qwen38_27b_dflash2_selector_host_validation.cu" \
    "${module_directory}/source/spark_qwen38_27b_resident_decode_stage_cuda.cu" \
    -o "${binary}"
