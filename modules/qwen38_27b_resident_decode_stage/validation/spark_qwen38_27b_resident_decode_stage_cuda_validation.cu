...
#define SPARK_QWEN38_27B_VALIDATION_KV_LANES 8u

#define SPARK_QWEN38_27B_DK SPARK_QWEN38_27B_MODEL_GDN_HEAD_KEY_DIMENSION
#define SPARK_QWEN38_27B_DV SPARK_QWEN38_27B_MODEL_GDN_HEAD_VALUE_DIMENSION
#define SPARK_QWEN38_27B_HEADS SPARK_QWEN38_27B_MODEL_GDN_VALUE_HEAD_COUNT
#define SPARK_QWEN38_27B_CONV SPARK_QWEN38_27B_MODEL_GDN_CONV_CHANNELS

extern "C" cudaError_t SparkQwen38_27bConfigureCudaKernels(void);
extern "C" cudaError_t SparkQwen38_27bLaunchConvUpdate(cudaStream_t stream, const void *qkv_bf16, const SparkQwen38_27bGdnLayerWeights *weights, void *conv_out_bf16, const SparkQwen38_27bGdnStatePool *pool, const uint32_t *row_lane_indices, uint32_t row_count, uint32_t gdn_layer_ordinal, uint8_t *row_snap_tails, uint64_t snap_tail_lane_stride, uint64_t snap_tail_layer_stride);
...
extern "C" cudaError_t SparkQwen38_27bLaunchChunkConv(cudaStream_t stream, const void *qkv_bf16, const SparkQwen38_27bGdnLayerWeights *weights, void *conv_out_bf16, const SparkQwen38_27bGdnStatePool *pool, uint32_t lane_index, uint32_t token_count, uint32_t gdn_layer_ordinal);
...
{
	return(((float)(int32_t)(SparkQwen38_27bValNext() & 0xffffu) - 32768.0f) * scale / 32768.0f);
}

static uint16_t SparkQwen38_27bValBf16(float value)
{
	uint32_t bits;
...
static void SparkQwen38_27bValFillBf16(uint16_t *packed, float *exact, uint64_t count, float scale)
{
	...
	{
		packed[index] = SparkQwen38_27bValBf16(SparkQwen38_27bValUniform(scale));
		if (exact != 0)
			exact[index] = SparkQwen38_27bValFromBf16(packed[index]);
	}
}
...
{
	float qn[SPARK_QWEN38_27B_DK],kn[SPARK_QWEN38_27B_DK],delta[SPARK_QWEN38_27B_DV];
	float scale = 1.0f / sqrtf((float)SPARK_QWEN38_27B_DK),decay,kv_mem;
	uint32_t token,row,column;
	for (token = 0u; token < tokens; token++)
	{
		SparkQwen38_27bValL2Norm(q + ((uint64_t)token * SPARK_QWEN38_27B_DK),qn,SPARK_QWEN38_27B_DK);
		SparkQwen38_27bValL2Norm(k + ((uint64_t)token * SPARK_QWEN38_27B_DK),kn,SPARK_QWEN38_27B_DK);
		for (row = 0u; row < SPARK_QWEN38_27B_DK; row++)
			qn[row] *= scale;
		decay = expf(g[token]);
		for (row = 0u; row < SPARK_QWEN38_27B_DK; row++)
			for (column = 0u; column < SPARK_QWEN38_27B_DV; column++)
				state[(row * SPARK_QWEN38_27B_DV) + column] *= decay;
		for (column = 0u; column < SPARK_QWEN38_27B_DV; column++)
		{
			kv_mem = 0.0f;
			for (row = 0u; row < SPARK_QWEN38_27B_DK; row++)
				kv_mem += state[(row * SPARK_QWEN38_27B_DV) + column] * kn[row];
			delta[column] = (v[((uint64_t)token * SPARK_QWEN38_27B_DV) + column] - kv_mem) * beta[token];
		}
		for (row = 0u; row < SPARK_QWEN38_27B_DK; row++)
			for (column = 0u; column < SPARK_QWEN38_27B_DV; column++)
				state[(row * SPARK_QWEN38_27B_DV) + column] += kn[row] * delta[column];
		for (column = 0u; column < SPARK_QWEN38_27B_DV; column++)
		{
			kv_mem = 0.0f;
			for (row = 0u; row < SPARK_QWEN38_27B_DK; row++)
				kv_mem += state[(row * SPARK_QWEN38_27B_DV) + column] * qn[row];
			output[((uint64_t)token * SPARK_QWEN38_27B_DV) + column] = kv_mem;
		}
	}
}
...
{
	uint64_t state_elements = 2ull * SPARK_QWEN38_27B_HEADS * SPARK_QWEN38_27B_DK * SPARK_QWEN38_27B_DV;
	uint64_t vector_floats = (uint64_t)SPARK_QWEN38_27B_HEADS * SPARK_QWEN38_27B_MODEL_GDN_CHUNK_TOKENS * SPARK_QWEN38_27B_DK;
	uint64_t matrix_floats = (uint64_t)SPARK_QWEN38_27B_HEADS * SPARK_QWEN38_27B_MODEL_GDN_CHUNK_TOKENS * SPARK_QWEN38_27B_MODEL_GDN_CHUNK_TOKENS;
	uint32_t tokens = SPARK_QWEN38_27B_VALIDATION_CHUNK_TOKENS;
	cudaError_t error;
	memset(device,0,sizeof(*device));
	error = cudaMalloc((void **)&device->conv_weight,(uint64_t)SPARK_QWEN38_27B_CONV * SPARK_QWEN38_27B_MODEL_GDN_CONV_KERNEL * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->a_log,SPARK_QWEN38_27B_HEADS * sizeof(float));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->dt_bias,SPARK_QWEN38_27B_HEADS * sizeof(float));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->gdn_norm_weight,SPARK_QWEN38_27B_DV * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->q_norm_weight,SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->k_norm_weight,SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->state,state_elements * sizeof(float));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->conv_tail,2ull * SPARK_QWEN38_27B_CONV * (SPARK_QWEN38_27B_MODEL_GDN_CONV_KERNEL - 1u) * sizeof(uint16_t));
	...
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->qkv,(uint64_t)tokens * SPARK_QWEN38_27B_CONV * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->conv_out,(uint64_t)tokens * SPARK_QWEN38_27B_CONV * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->core_out,(uint64_t)tokens * SPARK_QWEN38_27B_MODEL_GDN_VALUE_DIMENSION * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->z_bf16,(uint64_t)tokens * SPARK_QWEN38_27B_MODEL_GDN_VALUE_DIMENSION * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->gated_out,(uint64_t)tokens * SPARK_QWEN38_27B_MODEL_GDN_VALUE_DIMENSION * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->ba_bf16,(uint64_t)tokens * SPARK_QWEN38_27B_HEADS * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->log_decay,(uint64_t)tokens * SPARK_QWEN38_27B_HEADS * sizeof(float));
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->beta,(uint64_t)tokens * SPARK_QWEN38_27B_HEADS * sizeof(float));
	...
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->chunk_cum_g,(uint64_t)SPARK_QWEN38_27B_HEADS * SPARK_QWEN38_27B_MODEL_GDN_CHUNK_TOKENS * sizeof(float));
	...
	if (error == cudaSuccess) error = cudaMalloc((void **)&device->chunk_w,(uint64_t)SPARK_QWEN38_27B_HEADS * SPARK_QWEN38_27B_MODEL_GDN_CHUNK_TOKENS * SPARK_QWEN38_27B_DV * sizeof(float));
	...
	device->pool.state_layer_stride_elements = (uint64_t)SPARK_QWEN38_27B_HEADS * SPARK_QWEN38_27B_DK * SPARK_QWEN38_27B_DV;
	...
	device->pool.conv_tail_layer_stride_elements = (uint64_t)SPARK_QWEN38_27B_CONV * (SPARK_QWEN38_27B_MODEL_GDN_CONV_KERNEL - 1u);
...
{
	uint16_t host_ba[SPARK_QWEN38_27B_VALIDATION_ROWS * SPARK_QWEN38_27B_HEADS * 2u];
	float host_a[SPARK_QWEN38_27B_HEADS],host_bias[SPARK_QWEN38_27B_HEADS];
	float host_decay[SPARK_QWEN38_27B_VALIDATION_ROWS * SPARK_QWEN38_27B_HEADS];
	float host_beta[SPARK_QWEN38_27B_VALIDATION_ROWS * SPARK_QWEN38_27B_HEADS];
	float ref_decay[SPARK_QWEN38_27B_VALIDATION_ROWS * SPARK_QWEN38_27B_HEADS];
	float ref_beta[SPARK_QWEN38_27B_VALIDATION_ROWS * SPARK_QWEN38_27B_HEADS];
	SparkQwen38_27bValMetrics metrics;
	uint32_t row,head;
	uint64_t index;
	float shifted;
	cudaError_t error;
	SparkQwen38_27bValRandomState = 11u;
	SparkQwen38_27bValFillBf16(host_ba,0,SPARK_QWEN38_27B_VALIDATION_ROWS * SPARK_QWEN38_27B_HEADS * 2u,2.0f);
	for (head = 0u; head < SPARK_QWEN38_27B_HEADS; head++)
	{
		host_a[head] = SparkQwen38_27bValUniform(1.0f);
		host_bias[head] = SparkQwen38_27bValUniform(1.0f);
	}
	...
	if (SparkQwen38_27bValCuda(error,"decay_beta") != 0)
		return(1);
	for (row = 0u; row < SPARK_QWEN38_27B_VALIDATION_ROWS; row++)
		for (head = 0u; head < SPARK_QWEN38_27B_HEADS; head++)
		{
			index = ((uint64_t)row * SPARK_QWEN38_27B_HEADS) + head;
			shifted = SparkQwen38_27bValFromBf16(host_ba[index]) + host_bias[head];
			ref_decay[index] = -expf(host_a[head]) * (shifted > 20.0f ? shifted : logf(1.0f + expf(shifted)));
			ref_beta[index] = 1.0f / (1.0f + expf(-SparkQwen38_27bValFromBf16(host_ba[SPARK_QWEN38_27B_VALIDATION_ROWS * SPARK_QWEN38_27B_HEADS + index])));
		}
	SparkQwen38_27bValMeasure(&metrics,host_decay,ref_decay,SPARK_QWEN38_27B_VALIDATION_ROWS * SPARK_QWEN38_27B_HEADS);
	if (SparkQwen38_27bValReport("decay_gate",&metrics,1e-5,0.99999999) != 0)
		return(1);
	SparkQwen38_27bValMeasure(&metrics,host_beta,ref_beta,SPARK_QWEN38_27B_VALIDATION_ROWS * SPARK_QWEN38_27B_HEADS);
...
static int SparkQwen38_27bValCheckConv(SparkQwen38_27bValDevice *device)
{
	...
	if (host_qkv == 0 || host_out == 0 || exact == 0 || expected == 0 || actual == 0 || tails == 0)
		return(SparkQwen38_27bValFail("conv_update","host_alloc"));
	SparkQwen38_27bValRandomState = 23u;
	SparkQwen38_27bValFillBf16(host_qkv,exact,frame_elements,1.0f);
	{
		uint16_t *weight_packed = (uint16_t *)calloc((uint64_t)SPARK_QWEN38_27B_CONV * SPARK_QWEN38_27B_MODEL_GDN_CONV_KERNEL,sizeof(uint16_t));
		float *weight_exact = (float *)calloc((uint64_t)SPARK_QWEN38_27B_CONV * SPARK_QWEN38_27B_MODEL_GDN_CONV_KERNEL,sizeof(float));
		if (weight_packed == 0 || weight_exact == 0)
			return(SparkQwen38_27bValFail("conv_update","weight_alloc"));
		SparkQwen38_27bValFillBf16(weight_packed,weight_exact,(uint64_t)SPARK_QWEN38_27B_CONV * SPARK_QWEN38_27B_MODEL_GDN_CONV_KERNEL,0.5f);
		error = cudaMemcpy(device->conv_weight,weight_packed,(uint64_t)SPARK_QWEN38_27B_CONV * SPARK_QWEN38_27B_MODEL_GDN_CONV_KERNEL * sizeof(uint16_t),cudaMemcpyHostToDevice);
		...
		for (token = 0u; token < 2u && error == cudaSuccess; token++)
		{
			...
			if (error == cudaSuccess)
				error = SparkQwen38_27bLaunchConvUpdate(cudaStreamPerThread,device->qkv + (uint64_t)token * 2u * SPARK_QWEN38_27B_CONV,&device->gdn_weights,device->conv_out + (uint64_t)token * 2u * SPARK_QWEN38_27B_CONV,&device->pool,device->lane_indices,2u,0u,0,0,0);
			if (error == cudaSuccess) error = cudaStreamSynchronize(cudaStreamPerThread);
		}
		if (error == cudaSuccess)
			error = cudaMemcpy(host_out,device->conv_out,frame_elements * sizeof(uint16_t),cudaMemcpyDeviceToHost);
		if (SparkQwen38_27bValCuda(error,"conv_update") != 0)
			return(1);
		for (row = 0u; row < 2u; row++)
			for (channel = 0u; channel < SPARK_QWEN38_27B_CONV; channel++)
			{
				...
				input[0] = exact[base];
				input[1] = exact[2ull * SPARK_QWEN38_27B_CONV + base];
				tail[0] = tail[1] = tail[2] = 0.0f;
				{
					...
					for (token_index = 0u; token_index < 2u; token_index++)
					{
						...
						output[token_index] = SparkQwen38_27bValSilu(accumulator);
					}
					tail[2] = input[1];
					tail[1] = input[0];
					tail[0] = 0.0f;
				...
				expected[(uint64_t)0u * 2u * SPARK_QWEN38_27B_CONV + base] = output[0];
				expected[(uint64_t)1u * 2u * SPARK_QWEN38_27B_CONV + base] = output[1];
			}
		for (token = 0u; token < 2u; token++)
			for (uint64_t i = 0u; i < 2ull * SPARK_QWEN38_27B_CONV; i++)
				actual[(uint64_t)token * 2ull * SPARK_QWEN38_27B_CONV + i] =
					SparkQwen38_27bValFromBf16(host_out[(uint64_t)token * 2ull * SPARK_QWEN38_27B_CONV + i]);
		free(weight_packed);
		free(weight_exact);
	}
	SparkQwen38_27bValMeasure(&metrics,actual,expected,frame_elements);
...
static int SparkQwen38_27bValCheckGdnStep(SparkQwen38_27bValDevice *device)
{
	...
	uint16_t *host_conv = (uint16_t *)calloc(2ull * SPARK_QWEN38_27B_CONV,sizeof(uint16_t));
	float *exact = (float *)calloc(2ull * SPARK_QWEN38_27B_CONV,sizeof(float));
	...
	float *oracle_out = (float *)calloc(2ull * SPARK_QWEN38_27B_HEADS * SPARK_QWEN38_27B_DV,sizeof(float));
	float *actual = (float *)calloc(2ull * SPARK_QWEN38_27B_HEADS * SPARK_QWEN38_27B_DV,sizeof(float));
	uint16_t *core_packed = (uint16_t *)calloc(2ull * SPARK_QWEN38_27B_HEADS * SPARK_QWEN38_27B_DV,sizeof(uint16_t));
	uint32_t lanes[2] = {0u,1u};
	...
	if (host_conv == 0 || exact == 0 || state_host == 0 || state_reference == 0 || oracle_out == 0 || actual == 0 || core_packed == 0)
		return(SparkQwen38_27bValFail("gdn_step","host_alloc"));
	SparkQwen38_27bValRandomState = 37u;
	SparkQwen38_27bValFillBf16(host_conv,exact,2ull * SPARK_QWEN38_27B_CONV,1.0f);
	{
		uint64_t index;
	...
	{
		float host_log_decay[2 * SPARK_QWEN38_27B_HEADS],host_beta[2 * SPARK_QWEN38_27B_HEADS];
		uint32_t head_index;
		for (head_index = 0u; head_index < 2u * SPARK_QWEN38_27B_HEADS; head_index++)
		{
			host_log_decay[head_index] = SparkQwen38_27bValUniform(0.5f) - 0.5f;
			host_beta[head_index] = 0.25f + fabsf(SparkQwen38_27bValUniform(0.5f));
		}
		error = cudaMemcpy(device->state,state_host,2ull * state_elements * sizeof(float),cudaMemcpyHostToDevice);
		if (error == cudaSuccess) error = cudaMemcpy(device->qkv,host_conv,2ull * SPARK_QWEN38_27B_CONV * sizeof(uint16_t),cudaMemcpyHostToDevice);
		...
		if (error == cudaSuccess) error = cudaMemcpy(device->lane_indices,lanes,sizeof(lanes),cudaMemcpyHostToDevice);
		if (error == cudaSuccess)
			error = SparkQwen38_27bLaunchGdnStep(cudaStreamPerThread,device->qkv,device->log_decay,device->beta,&device->pool,device->core_out,device->lane_indices,2u,0u,0,0,0);
		if (error == cudaSuccess) error = cudaStreamSynchronize(cudaStreamPerThread);
		if (error == cudaSuccess) error = cudaMemcpy(core_packed,device->core_out,2ull * SPARK_QWEN38_27B_HEADS * SPARK_QWEN38_27B_DV * sizeof(uint16_t),cudaMemcpyDeviceToHost);
		...
		for (row = 0u; row < 2u; row++)
			for (head = 0u; head < SPARK_QWEN38_27B_HEADS; head++)
			{
				...
				const float *q = exact + ((uint64_t)row * SPARK_QWEN38_27B_CONV) + ((uint64_t)key_head * SPARK_QWEN38_27B_DK);
				const float *k = q + SPARK_QWEN38_27B_MODEL_GDN_QK_DIMENSION;
				const float *v = exact + ((uint64_t)row * SPARK_QWEN38_27B_CONV) + (2ull * SPARK_QWEN38_27B_MODEL_GDN_QK_DIMENSION) + ((uint64_t)head * SPARK_QWEN38_27B_DV);
				float g = cold[row] != 0u ? -30.0f : host_log_decay[(row * SPARK_QWEN38_27B_HEADS) + head];
				float beta = host_beta[(row * SPARK_QWEN38_27B_HEADS) + head];
				SparkQwen38_27bValGdnRecurrence(q,k,v,&g,&beta,
					state_reference + ((uint64_t)row * state_elements) + ((uint64_t)head * SPARK_QWEN38_27B_DK * SPARK_QWEN38_27B_DV),
					oracle_out + ((uint64_t)row * SPARK_QWEN38_27B_HEADS * SPARK_QWEN38_27B_DV) + ((uint64_t)head * SPARK_QWEN38_27B_DV),1u);
			}
	}
	{
		uint64_t index;
		for (index = 0u; index < 2ull * SPARK_QWEN38_27B_HEADS * SPARK_QWEN38_27B_DV; index++)
			actual[index] = SparkQwen38_27bValFromBf16(core_packed[index]);
	}
	SparkQwen38_27bValMeasure(&metrics,actual,oracle_out,2ull * SPARK_QWEN38_27B_HEADS * SPARK_QWEN38_27B_DV);
	if (SparkQwen38_27bValReport("gdn_step_output",&metrics,5e-3,0.99999) != 0)
		return(1);
	SparkQwen38_27bValMeasure(&metrics,state_host,state_reference,2ull * state_elements);
	free(host_conv); free(exact); free(state_host); free(state_reference);
	free(oracle_out); free(actual); free(core_packed);
	return(SparkQwen38_27bValReport("gdn_step_state",&metrics,1e-3,0.999999));
}

static int SparkQwen38_27bValCheckGatedNorm(SparkQwen38_27bValDevice *device)
{
	uint64_t elements = (uint64_t)SPARK_QWEN38_27B_VALIDATION_ROWS * SPARK_QWEN38_27B_MODEL_GDN_VALUE_DIMENSION;
	...
	uint16_t *norm_packed = (uint16_t *)calloc(SPARK_QWEN38_27B_DV,sizeof(uint16_t));
	float *norm_exact = (float *)calloc(SPARK_QWEN38_27B_DV,sizeof(float));
	SparkQwen38_27bValMetrics metrics;
	...
	if (error == cudaSuccess) error = cudaMemcpy(device->gdn_norm_weight,norm_packed,SPARK_QWEN38_27B_DV * sizeof(uint16_t),cudaMemcpyHostToDevice);
	if (error == cudaSuccess)
		error = SparkQwen38_27bLaunchGatedNorm(cudaStreamPerThread,device->core_out,device->z_bf16,&device->gdn_weights,device->gated_out,SPARK_QWEN38_27B_VALIDATION_ROWS,SPARK_QWEN38_27B_MODEL_RMS_NORM_EPSILON);
	if (error == cudaSuccess) error = cudaStreamSynchronize(cudaStreamPerThread);
	if (error == cudaSuccess) error = cudaMemcpy(packed,device->gated_out,elements * sizeof(uint16_t),cudaMemcpyDeviceToHost);
	if (SparkQwen38_27bValCuda(error,"gated_norm") != 0)
		return(1);
	for (row = 0u; row < SPARK_QWEN38_27B_VALIDATION_ROWS; row++)
		for (head = 0u; head < SPARK_QWEN38_27B_HEADS; head++)
		{
			uint64_t base = ((uint64_t)row * SPARK_QWEN38_27B_MODEL_GDN_VALUE_DIMENSION) + ((uint64_t)head * SPARK_QWEN38_27B_DV);
			float variance = 0.0f,inverse;
			for (element = 0u; element < SPARK_QWEN38_27B_DV; element++)
				variance += exact[base + element] * exact[base + element];
			inverse = 1.0f / sqrtf((variance / (float)SPARK_QWEN38_27B_DV) + SPARK_QWEN38_27B_MODEL_RMS_NORM_EPSILON);
			for (element = 0u; element < SPARK_QWEN38_27B_DV; element++)
				expected[base + element] = exact[base + element] * inverse * norm_exact[element] *
					SparkQwen38_27bValSilu(exact[elements + base + element]);
		}
	for (row = 0u; row < elements; row++)
		actual[row] = SparkQwen38_27bValFromBf16(packed[row]);
	SparkQwen38_27bValMeasure(&metrics,actual,expected,elements);
...
static int SparkQwen38_27bValCheckAttention(SparkQwen38_27bValDevice *device)
{
	...
	const uint64_t fused_elements = (uint64_t)tokens * 2u * SPARK_QWEN38_27B_MODEL_ATTN_QUERY_DIMENSION;
	const uint64_t kv_elements = (uint64_t)tokens * SPARK_QWEN38_27B_MODEL_ATTN_KV_DIMENSION;
	const uint64_t cache_elements = (uint64_t)SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS * SPARK_QWEN38_27B_MODEL_ATTN_CACHE_TOKEN_ELEMENTS;
	...
	float *expected = (float *)calloc(SPARK_QWEN38_27B_MODEL_ATTN_QUERY_DIMENSION,sizeof(float));
	float *actual = (float *)calloc(SPARK_QWEN38_27B_MODEL_ATTN_QUERY_DIMENSION,sizeof(float));
	uint16_t *out_packed = (uint16_t *)calloc(SPARK_QWEN38_27B_MODEL_ATTN_QUERY_DIMENSION,sizeof(uint16_t));
	uint16_t *norm_packed = (uint16_t *)calloc(2u * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION,sizeof(uint16_t));
	float *q_norm_exact = (float *)calloc(SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION,sizeof(float));
	float *k_norm_exact = (float *)calloc(SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION,sizeof(float));
	uint32_t slot_mapping[SPARK_QWEN38_27B_VALIDATION_ATTN_TOKENS];
	uint64_t positions[SPARK_QWEN38_27B_VALIDATION_ATTN_TOKENS];
	uint32_t block_indices[1] = {0u};
	uint32_t block_counts[1] = {1u};
	uint32_t lane_zero[1] = {0u};
	uint32_t context[1] = {SPARK_QWEN38_27B_VALIDATION_ATTN_TOKENS};
	uint32_t token;
	cudaError_t error;
	SparkQwen38_27bValMetrics metrics;
	SparkQwen38_27bKvBlockTableView table;
	void *kv_cache = 0;
	...
	if (q_packed == 0 || k_packed == 0 || v_packed == 0 || q_exact == 0 || k_exact == 0 ||
		...
		return(SparkQwen38_27bValFail("attn_decode","host_alloc"));
	SparkQwen38_27bValRandomState = 67u;
	SparkQwen38_27bValFillBf16(q_packed,q_exact,fused_elements,0.5f);
	SparkQwen38_27bValFillBf16(k_packed,k_exact,kv_elements,0.5f);
	SparkQwen38_27bValFillBf16(v_packed,v_exact,kv_elements,0.5f);
	SparkQwen38_27bValFillBf16(norm_packed,q_norm_exact,SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION,0.5f);
	...
	if (error == cudaSuccess) error = cudaMalloc((void **)&device_out,SPARK_QWEN38_27B_MODEL_ATTN_QUERY_DIMENSION * sizeof(uint16_t));
	...
	table.abi_version = SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TABLE_ABI_VERSION;
	table.descriptor_bytes = sizeof(table);
	table.block_token_count = SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS;
	table.lane_count = 1u;
	...
	if (error == cudaSuccess)
		error = SparkQwen38_27bLaunchAttnDecode(cudaStreamPerThread,device_q + ((uint64_t)(tokens - 1u) * 2u * SPARK_QWEN38_27B_MODEL_ATTN_QUERY_DIMENSION),kv_cache,&table,device_lane,device_context,device_out,1u,0u,cache_elements,cache_elements);
	if (error == cudaSuccess) error = cudaStreamSynchronize(cudaStreamPerThread);
	 ...
	 * RefAttention call per kv head over the newest fused query row. */
	{
		...
		if (k_cache == 0 || v_cache == 0)
			return(SparkQwen38_27bValFail("attn_decode","oracle_alloc"));
		for (kv_head = 0u; kv_head < SPARK_QWEN38_27B_MODEL_ATTN_KV_HEAD_COUNT; kv_head++)
		{
			for (token = 0u; token < tokens; token++)
			{
				const float *k_row = k_exact + (((uint64_t)token * SPARK_QWEN38_27B_MODEL_ATTN_KV_DIMENSION) + ((uint64_t)kv_head * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION));
				float *cache_row = k_cache + ((uint64_t)token * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION);
				SparkQwen38_27bValRmsNorm(k_row,k_norm_exact,cache_row,SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION,SPARK_QWEN38_27B_MODEL_RMS_NORM_EPSILON);
				SparkQwen38_27bValRope(cache_row,SPARK_QWEN38_27B_MODEL_ATTN_ROPE_DIMENSION,token,SPARK_QWEN38_27B_MODEL_ATTN_ROPE_THETA);
				for (element = 0u; element < SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION; element++)
					v_cache[((uint64_t)token * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION) + element] =
						v_exact[((uint64_t)token * SPARK_QWEN38_27B_MODEL_ATTN_KV_DIMENSION) + ((uint64_t)kv_head * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION) + element];
			}
			SparkQwen38_27bValAttention(
				q_exact + (((uint64_t)(tokens - 1u) * 2u * SPARK_QWEN38_27B_MODEL_ATTN_QUERY_DIMENSION) + ((uint64_t)kv_head * 6u * 2u * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION)),
				k_cache,v_cache,q_norm_exact,
				expected + ((uint64_t)kv_head * 6u * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION),
				6u,tokens,SPARK_QWEN38_27B_MODEL_RMS_NORM_EPSILON);
		}
		free(k_cache);
		free(v_cache);
	}
	for (token = 0u; token < SPARK_QWEN38_27B_MODEL_ATTN_QUERY_DIMENSION; token++)
		actual[token] = SparkQwen38_27bValFromBf16(out_packed[token]);
	SparkQwen38_27bValMeasure(&metrics,actual,expected,SPARK_QWEN38_27B_MODEL_ATTN_QUERY_DIMENSION);
...
static int SparkQwen38_27bValCheckGdnChunk(SparkQwen38_27bValDevice *device)
{
	...
	const uint64_t conv_elements = (uint64_t)tokens * SPARK_QWEN38_27B_CONV;
	const uint64_t out_elements = (uint64_t)tokens * SPARK_QWEN38_27B_MODEL_GDN_VALUE_DIMENSION;
	const uint64_t state_elements = (uint64_t)SPARK_QWEN38_27B_HEADS * SPARK_QWEN38_27B_DK * SPARK_QWEN38_27B_DV;
	...
	float *log_decay = (float *)calloc((uint64_t)tokens * SPARK_QWEN38_27B_HEADS,sizeof(float));
	float *beta = (float *)calloc((uint64_t)tokens * SPARK_QWEN38_27B_HEADS,sizeof(float));
	uint32_t head,token;
	uint64_t index;
	cudaError_t error;
	SparkQwen38_27bValMetrics metrics;
	if (conv_packed == 0 || exact == 0 || out_packed == 0 || actual == 0 || expected == 0 ||
		...
		return(SparkQwen38_27bValFail("gdn_chunk","host_alloc"));
	SparkQwen38_27bValRandomState = 83u;
	SparkQwen38_27bValFillBf16(conv_packed,exact,conv_elements,1.0f);
	for (index = 0u; index < (uint64_t)tokens * SPARK_QWEN38_27B_HEADS; index++)
	{
		log_decay[index] = SparkQwen38_27bValUniform(0.25f) - 0.25f;
		beta[index] = 0.25f + fabsf(SparkQwen38_27bValUniform(0.5f));
	}
	...
	for (uint32_t base = 0u; base < tokens && error == cudaSuccess; base += SPARK_QWEN38_27B_MODEL_GDN_CHUNK_TOKENS)
		error = SparkQwen38_27bLaunchGdnChunk(cudaStreamPerThread,
			...
			device->log_decay + ((uint64_t)base * SPARK_QWEN38_27B_HEADS),
			device->beta + ((uint64_t)base * SPARK_QWEN38_27B_HEADS),
			device->chunk_qn,device->chunk_kn,device->chunk_cum_g,device->chunk_decay,
			device->chunk_attn,device->chunk_w,device->chunk_kg,
			&device->pool,device->core_out + ((uint64_t)base * SPARK_QWEN38_27B_MODEL_GDN_VALUE_DIMENSION),
			0u,SPARK_QWEN38_27B_MODEL_GDN_CHUNK_TOKENS,0u);
	if (error == cudaSuccess) error = cudaStreamSynchronize(cudaStreamPerThread);
	...
	if (SparkQwen38_27bValCuda(error,"gdn_chunk") != 0)
		return(1);
	for (head = 0u; head < SPARK_QWEN38_27B_HEADS; head++)
	{
		uint32_t key_head = head / 3u;
		float *q_head = (float *)calloc((uint64_t)tokens * SPARK_QWEN38_27B_DK,sizeof(float));
		float *k_head = (float *)calloc((uint64_t)tokens * SPARK_QWEN38_27B_DK,sizeof(float));
		float *v_head = (float *)calloc((uint64_t)tokens * SPARK_QWEN38_27B_DV,sizeof(float));
		...
		if (q_head == 0 || k_head == 0 || v_head == 0 || g_head == 0 || b_head == 0)
			return(SparkQwen38_27bValFail("gdn_chunk","oracle_alloc"));
		for (token = 0u; token < tokens; token++)
		{
			uint64_t base = (uint64_t)token * SPARK_QWEN38_27B_CONV;
			memcpy(q_head + ((uint64_t)token * SPARK_QWEN38_27B_DK),exact + base + ((uint64_t)key_head * SPARK_QWEN38_27B_DK),SPARK_QWEN38_27B_DK * sizeof(float));
			memcpy(k_head + ((uint64_t)token * SPARK_QWEN38_27B_DK),exact + base + SPARK_QWEN38_27B_MODEL_GDN_QK_DIMENSION + ((uint64_t)key_head * SPARK_QWEN38_27B_DK),SPARK_QWEN38_27B_DK * sizeof(float));
			memcpy(v_head + ((uint64_t)token * SPARK_QWEN38_27B_DV),exact + base + (2ull * SPARK_QWEN38_27B_MODEL_GDN_QK_DIMENSION) + ((uint64_t)head * SPARK_QWEN38_27B_DV),SPARK_QWEN38_27B_DV * sizeof(float));
			g_head[token] = log_decay[((uint64_t)token * SPARK_QWEN38_27B_HEADS) + head];
			b_head[token] = beta[((uint64_t)token * SPARK_QWEN38_27B_HEADS) + head];
		}
		SparkQwen38_27bValGdnRecurrence(q_head,k_head,v_head,g_head,b_head,
			state_oracle + ((uint64_t)head * SPARK_QWEN38_27B_DK * SPARK_QWEN38_27B_DV),
	 ...
	 * re-map before comparing. */
	{
		...
		if (expected_ordered == 0)
			return(SparkQwen38_27bValFail("gdn_chunk","order_alloc"));
		for (token = 0u; token < tokens; token++)
			for (head = 0u; head < SPARK_QWEN38_27B_HEADS; head++)
				memcpy(expected_ordered + ((uint64_t)token * SPARK_QWEN38_27B_MODEL_GDN_VALUE_DIMENSION) + ((uint64_t)head * SPARK_QWEN38_27B_DV),
					expected + ((uint64_t)head * tokens * SPARK_QWEN38_27B_DV) + ((uint64_t)token * SPARK_QWEN38_27B_DV),
					SPARK_QWEN38_27B_DV * sizeof(float));
		for (index = 0u; index < out_elements; index++)
			actual[index] = SparkQwen38_27bValFromBf16(out_packed[index]);
		SparkQwen38_27bValMeasure(&metrics,actual,expected_ordered,out_elements);
		free(expected_ordered);
	}
	if (SparkQwen38_27bValReport("gdn_chunk_output",&metrics,2e-2,0.999) != 0)
		return(1);
	SparkQwen38_27bValMeasure(&metrics,state_device,state_oracle,state_elements);
...
static SparkStatus SparkQwen38_27bValCaptureSend(SparkHiddenTransportSession *session, const SparkHiddenTransportPacket *packet)
{
	SparkQwen38_27bValCapture *capture = (SparkQwen38_27bValCapture *)session;
	uint64_t bytes = (uint64_t)packet->active_sequence_count * SPARK_QWEN38_27B_MODEL_HIDDEN_DIMENSION * 2u;
	if (packet->hidden_bf16 == 0 || packet->active_sequence_count > SPARK_QWEN38_27B_VALIDATION_PREFILL_TOKENS ||
...
typedef struct SparkQwen38_27bValModule
{
	...
	uint32_t head_stage;
	uint32_t lanes[SPARK_QWEN38_27B_VALIDATION_PREFILL_TOKENS];
	uint64_t positions[SPARK_QWEN38_27B_VALIDATION_PREFILL_TOKENS];
	uint64_t sequence_ids[SPARK_QWEN38_27B_VALIDATION_PREFILL_TOKENS];
	uint32_t host_blocks[SPARK_QWEN38_27B_VALIDATION_KV_LANES];
	uint32_t host_counts[SPARK_QWEN38_27B_VALIDATION_KV_LANES];
...
static int SparkQwen38_27bValModuleExecute(SparkQwen38_27bValModule *module, uint32_t prefill, uint32_t rows, uint32_t lane, uint32_t draft_count, const SparkQwen38_27bMtpDraftView *draft_view)
{
	...
	module->context.abi_version = SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_ABI_VERSION;
	...
	module->context.flags =
		...
		(draft_count != 0u ? SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_MTP_DRAFT_AFTER : 0u);
	module->context.mtp_draft = draft_view;
	module->context.kv_block_table = &module->table;
	if (prefill != 0u)
	{
		module->prefill_view.abi_version = SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_PREFILL_FRAME_VIEW_ABI_VERSION;
	...
	{
		module->decode_batch.abi_version = SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_DECODE_BATCH_VIEW_ABI_VERSION;
	...
	module->context.hidden_output_send_function = module->head_stage != 0u ? 0 : SparkQwen38_27bValCaptureSend;
	...
	status = SparkQwen38_27bResidentDecodeStageExecute(module->state,&module->frame);
	if (status != SPARK_STATUS_OK)
	{
		fprintf(stderr,"qwen38_27b_validation failure=module_execute prefill=%u rows=%u status=%d\n",prefill,rows,(int)status);
		return(1);
	}
	return(0);
}
...
static int SparkQwen38_27bValCheckMtpDraft(SparkQwen38_27bValModule *module)
{
	...
	module->lanes[0] = 0u;
	module->positions[0] = SPARK_QWEN38_27B_VALIDATION_PREFILL_TOKENS + 1u;
	module->sequence_ids[0] = 1u;
	memset(&draft_view,0,sizeof(draft_view));
	draft_view.abi_version = SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_MTP_DRAFT_VIEW_ABI_VERSION;
	draft_view.descriptor_bytes = sizeof(draft_view);
	draft_view.lane_index = 0u;
	draft_view.draft_token_count = 2u;
	draft_view.base_position = (uint64_t)SPARK_QWEN38_27B_VALIDATION_PREFILL_TOKENS + 2u;
	draft_view.sequence_id = 1u;
	draft_view.row_token_ids = module->token_ids;
	if (SparkQwen38_27bValModuleExecute(module,0u,1u,0u,2u,&draft_view) != 0)
		return(1);
	for (draft = 0u; draft < 2u; draft++)
	{
		if (module->output_token_ids[1u + draft] >= SPARK_QWEN38_27B_MODEL_VOCAB_COUNT)
			return(SparkQwen38_27bValFail("module_mtp_draft","out_of_vocab"));
	}
 ...
 */
static int SparkQwen38_27bValCheckSpecPathEquivalence(SparkQwen38_27bValDevice *device)
{
	...
	if (qkv_packed == 0 || qkv_exact == 0 || log_decay == 0 || beta == 0 || state_chunk == 0 ||
		...
		return(SparkQwen38_27bValFail("spec_path_equivalence","host_alloc"));
	SparkQwen38_27bValRandomState = 991u;
	SparkQwen38_27bValFillBf16(qkv_packed,qkv_exact,conv_elements,1.0f);
	for (index = 0u; index < (uint64_t)tokens * SPARK_QWEN38_27B_HEADS; index++)
	{
		log_decay[index] = SparkQwen38_27bValUniform(0.25f) - 0.25f;
		beta[index] = 0.25f + fabsf(SparkQwen38_27bValUniform(0.5f));
	...
	if (error == cudaSuccess)
		error = SparkQwen38_27bLaunchChunkConv(cudaStreamPerThread,device->qkv,&device->gdn_weights,device->conv_out,&device->pool,0u,tokens,0u);
	if (error == cudaSuccess)
		error = SparkQwen38_27bLaunchGdnChunk(cudaStreamPerThread,device->conv_out,device->log_decay,device->beta,
	...
	{
		error = SparkQwen38_27bLaunchConvUpdate(cudaStreamPerThread,device->qkv + ((uint64_t)token * SPARK_QWEN38_27B_CONV),
			&device->gdn_weights,device->conv_out + ((uint64_t)token * SPARK_QWEN38_27B_CONV),&device->pool,
			device->lane_indices,1u,0u,0,0,0);
		if (error == cudaSuccess)
			error = SparkQwen38_27bLaunchGdnStep(cudaStreamPerThread,device->conv_out + ((uint64_t)token * SPARK_QWEN38_27B_CONV),
				device->log_decay + ((uint64_t)token * SPARK_QWEN38_27B_HEADS),device->beta + ((uint64_t)token * SPARK_QWEN38_27B_HEADS),
				&device->pool,device->core_out + ((uint64_t)token * SPARK_QWEN38_27B_MODEL_GDN_VALUE_DIMENSION),
				device->lane_indices,1u,0u,0,0,0);
		if (error == cudaSuccess) error = cudaStreamSynchronize(cudaStreamPerThread);
	}
	...
	if (SparkQwen38_27bValCuda(error,"spec_path_equivalence") != 0)
		return(1);
	for (index = 0u; index < state_elements; index++)
	...
	for (index = 0u; index < tail_elements; index++)
		if (tail_chunk[index] != tail_step[index])
			tail_bad++;
	printf("qwen38_27b_validation check=spec_path_equivalence tokens=%u state_elements=%llu "
	...
	if (worst > 1e-4)
		return(SparkQwen38_27bValFail("spec_path_equivalence","chunk and step disagree beyond fp32 rounding - one path is wrong"));
	if (state_bad == 0u && tail_bad == 0u)
		printf("qwen38_27b_validation note=spec_path_equivalence paths are bit-identical on this build\n");
	return(0);
}
 ...
 */
static int SparkQwen38_27bValCheckStepRowBatch(SparkQwen38_27bValDevice *device)
{
	...
	if (qkv_packed == 0 || qkv_exact == 0 || log_decay == 0 || beta == 0 || state_batch == 0 ||
		...
		return(SparkQwen38_27bValFail("step_row_batch","host_alloc"));
	SparkQwen38_27bValRandomState = 1777u;
	SparkQwen38_27bValFillBf16(qkv_packed,qkv_exact,conv_elements,1.0f);
	for (index = 0u; index < (uint64_t)tokens * SPARK_QWEN38_27B_HEADS; index++)
	{
		log_decay[index] = SparkQwen38_27bValUniform(0.25f) - 0.25f;
	...
	if (error == cudaSuccess)
		error = SparkQwen38_27bLaunchConvUpdate(cudaStreamPerThread,device->qkv,&device->gdn_weights,device->conv_out,&device->pool,device->lane_indices,tokens,0u,0,0,0);
	if (error == cudaSuccess)
		error = SparkQwen38_27bLaunchGdnStep(cudaStreamPerThread,device->conv_out,device->log_decay,device->beta,&device->pool,device->core_out,device->lane_indices,tokens,0u,0,0,0);
	if (error == cudaSuccess) error = cudaStreamSynchronize(cudaStreamPerThread);
	...
	{
		error = SparkQwen38_27bLaunchConvUpdate(cudaStreamPerThread,device->qkv + ((uint64_t)token * SPARK_QWEN38_27B_CONV),
			&device->gdn_weights,device->conv_out + ((uint64_t)token * SPARK_QWEN38_27B_CONV),&device->pool,device->lane_indices,1u,0u,0,0,0);
		if (error == cudaSuccess)
			error = SparkQwen38_27bLaunchGdnStep(cudaStreamPerThread,device->conv_out + ((uint64_t)token * SPARK_QWEN38_27B_CONV),
				device->log_decay + ((uint64_t)token * SPARK_QWEN38_27B_HEADS),device->beta + ((uint64_t)token * SPARK_QWEN38_27B_HEADS),
				&device->pool,device->core_out + ((uint64_t)token * SPARK_QWEN38_27B_MODEL_GDN_VALUE_DIMENSION),device->lane_indices,1u,0u,0,0,0);
		if (error == cudaSuccess) error = cudaStreamSynchronize(cudaStreamPerThread);
	}
	...
	if (SparkQwen38_27bValCuda(error,"step_row_batch") != 0)
		return(1);
	for (index = 0u; index < state_elements; index++)
	...
	for (index = 0u; index < tail_elements; index++)
		if (tail_batch[index] != tail_serial[index])
			tail_bad++;
	printf("qwen38_27b_validation check=step_row_batch tokens=%u state_mismatch=%llu tail_mismatch=%llu worst_abs=%.9g\n",
		tokens,(unsigned long long)state_bad,(unsigned long long)tail_bad,worst);
	if (state_bad != 0u || tail_bad != 0u)
		return(SparkQwen38_27bValFail("step_row_batch","a k-row same-lane step launch is not k sequential one-row launches - the replay cannot stand in for the decodes it replaces"));
	return(0);
}

/*
 * DRIFT ACCUMULATION - does the path substitution actually reach the measured drift?
 *
 * The claim under test: a speculative round rebuilds its committed positions with
 * the CHUNK path where a no-spec run built them with the STEP path, and that
 * substitution compounds. The claim is only worth anything if the compounded size
 * matches what the box's dumps show: at position 235 the spec lane's state differs
 * from the no-spec lane's by absmax 0.016 / absmean 1.6e-05 on a state of scale
 * +-30, which is 0.1% of ONE token of legitimate change.
 *
 * So simulate exactly that, round for round, on two lanes of the real pool:
 *
 *   lane 0 (no-spec): every row walked one at a time through the STEP path
 *   lane 1 (spec)   : the round's first row through the STEP path (the decode),
 *                     then the remaining rows through the CHUNK path (the replay)
 *
 * Same tokens, same order, same weights. After each round the two lanes' states are
 * compared, so the output is the drift CURVE - and its magnitude after ~20 rounds is
 * a prediction that either matches the measured 0.016 or does not.
 */
static int SparkQwen38_27bValCheckDriftAccumulation(SparkQwen38_27bValDevice *device)
{
	const uint32_t rounds = 20u;
	const uint32_t rows_per_round = 4u;      /* a typical round: ma=2 -> replay 4 rows */
	const uint64_t conv_elements = (uint64_t)rows_per_round * SPARK_QWEN38_27B_CONV;
	const uint64_t state_elements = device->pool.state_lane_stride_elements;
	uint16_t *qkv_packed = (uint16_t *)calloc(conv_elements,sizeof(uint16_t));
	float *qkv_exact = (float *)calloc(conv_elements,sizeof(float));
	float *log_decay = (float *)calloc((uint64_t)rows_per_round * SPARK_QWEN38_27B_HEADS,sizeof(float));
	float *beta = (float *)calloc((uint64_t)rows_per_round * SPARK_QWEN38_27B_HEADS,sizeof(float));
	float *state_a = (float *)calloc(state_elements,sizeof(float));
	float *state_b = (float *)calloc(state_elements,sizeof(float));
	uint32_t warm[2] = {0u,0u};
	uint32_t lane0[1] = {0u},lane1[1] = {1u};
	uint32_t round,row;
	uint64_t index;
	double absmax = 0.0,abssum = 0.0,scale = 0.0;
	cudaError_t error;
	if (qkv_packed == 0 || qkv_exact == 0 || log_decay == 0 || beta == 0 || state_a == 0 || state_b == 0)
		return(SparkQwen38_27bValFail("drift_accumulation","host_alloc"));
	error = cudaMemset(device->state,0,2u * state_elements * sizeof(float));
	if (error == cudaSuccess) error = cudaMemset(device->conv_tail,0,2u * device->pool.conv_tail_lane_stride_elements * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMemcpy(device->cold,warm,sizeof(warm),cudaMemcpyHostToDevice);
	SparkQwen38_27bValRandomState = 4242u;
	for (round = 0u; round < rounds && error == cudaSuccess; round++)
	{
		/* Fresh tokens for the round, identical for both lanes. */
		SparkQwen38_27bValFillBf16(qkv_packed,qkv_exact,conv_elements,1.0f);
		for (index = 0u; index < (uint64_t)rows_per_round * SPARK_QWEN38_27B_HEADS; index++)
		{
			log_decay[index] = SparkQwen38_27bValUniform(0.25f) - 0.25f;
			beta[index] = 0.25f + fabsf(SparkQwen38_27bValUniform(0.5f));
		}
		error = cudaMemcpy(device->qkv,qkv_packed,conv_elements * sizeof(uint16_t),cudaMemcpyHostToDevice);
		if (error == cudaSuccess) error = cudaMemcpy(device->log_decay,log_decay,(uint64_t)rows_per_round * SPARK_QWEN38_27B_HEADS * sizeof(float),cudaMemcpyHostToDevice);
		if (error == cudaSuccess) error = cudaMemcpy(device->beta,beta,(uint64_t)rows_per_round * SPARK_QWEN38_27B_HEADS * sizeof(float),cudaMemcpyHostToDevice);
		/* lane 0: the no-spec trajectory - every row through the step path. */
		if (error == cudaSuccess) error = cudaMemcpy(device->lane_indices,lane0,sizeof(lane0),cudaMemcpyHostToDevice);
		for (row = 0u; row < rows_per_round && error == cudaSuccess; row++)
		{
			error = SparkQwen38_27bLaunchConvUpdate(cudaStreamPerThread,device->qkv + ((uint64_t)row * SPARK_QWEN38_27B_CONV),
				&device->gdn_weights,device->conv_out + ((uint64_t)row * SPARK_QWEN38_27B_CONV),&device->pool,device->lane_indices,1u,0u,0,0,0);
			if (error == cudaSuccess)
				error = SparkQwen38_27bLaunchGdnStep(cudaStreamPerThread,device->conv_out + ((uint64_t)row * SPARK_QWEN38_27B_CONV),
					device->log_decay + ((uint64_t)row * SPARK_QWEN38_27B_HEADS),device->beta + ((uint64_t)row * SPARK_QWEN38_27B_HEADS),
					&device->pool,device->core_out + ((uint64_t)row * SPARK_QWEN38_27B_MODEL_GDN_VALUE_DIMENSION),device->lane_indices,1u,0u,0,0,0);
		}
		...
		if (error == cudaSuccess)
			error = SparkQwen38_27bLaunchConvUpdate(cudaStreamPerThread,device->qkv,&device->gdn_weights,device->conv_out,&device->pool,device->lane_indices,1u,0u,0,0,0);
		if (error == cudaSuccess)
			error = SparkQwen38_27bLaunchGdnStep(cudaStreamPerThread,device->conv_out,device->log_decay,device->beta,&device->pool,device->core_out,device->lane_indices,1u,0u,0,0,0);
		if (error == cudaSuccess)
			error = SparkQwen38_27bLaunchChunkConv(cudaStreamPerThread,device->qkv + SPARK_QWEN38_27B_CONV,&device->gdn_weights,
				device->conv_out + SPARK_QWEN38_27B_CONV,&device->pool,1u,rows_per_round - 1u,0u);
		if (error == cudaSuccess)
			error = SparkQwen38_27bLaunchGdnChunk(cudaStreamPerThread,device->conv_out + SPARK_QWEN38_27B_CONV,
				device->log_decay + SPARK_QWEN38_27B_HEADS,device->beta + SPARK_QWEN38_27B_HEADS,
				device->chunk_qn,device->chunk_kn,device->chunk_cum_g,device->chunk_decay,device->chunk_attn,
				device->chunk_w,device->chunk_kg,&device->pool,device->core_out + SPARK_QWEN38_27B_MODEL_GDN_VALUE_DIMENSION,
				1u,rows_per_round - 1u,0u);
		if (error == cudaSuccess) error = cudaStreamSynchronize(cudaStreamPerThread);
		if (error == cudaSuccess) error = cudaMemcpy(state_a,device->state,state_elements * sizeof(float),cudaMemcpyDeviceToHost);
		if (error == cudaSuccess) error = cudaMemcpy(state_b,device->state + state_elements,state_elements * sizeof(float),cudaMemcpyDeviceToHost);
		if (error != cudaSuccess)
			break;
		absmax = 0.0; abssum = 0.0; scale = 0.0;
		for (index = 0u; index < state_elements; index++)
		{
			double delta = fabs((double)state_a[index] - (double)state_b[index]);
			double magnitude = fabs((double)state_a[index]);
			abssum += delta;
			if (delta > absmax) absmax = delta;
			if (magnitude > scale) scale = magnitude;
		}
		if (round == 0u || round == 4u || round == 9u || round == rounds - 1u)
			printf("qwen38_27b_validation check=drift_accumulation round=%u absmax=%.6g absmean=%.6g state_scale=%.6g\n",
				round + 1u,absmax,abssum / (double)state_elements,scale);
	}
	if (SparkQwen38_27bValCuda(error,"drift_accumulation") != 0)
		return(1);
	/* The box measured absmax 0.016 / absmean 1.6e-05 at position 235 on a state of
	 * scale ~30. This simulation is not the same tokens or the same number of rounds,
	 * so the test is ORDER OF MAGNITUDE: the substitution must produce a drift that
	 * is small against the state scale yet far above a single fp32 rounding, which is
	 * what makes it invisible per round and fatal after twenty. */
	printf("qwen38_27b_validation check=drift_accumulation rounds=%u rows_per_round=%u final_absmax=%.6g relative_to_scale=%.3g\n",
		rounds,rows_per_round,absmax,scale > 0.0 ? absmax / scale : 0.0);
	if (absmax <= 0.0)
		return(SparkQwen38_27bValFail("drift_accumulation","the substitution produced NO drift - the paths would be interchangeable"));
	return(0);
}

/*
 * FORTY-EIGHT-LAYER CHUNK vs STEP: the box's replay, end to end.
 *
 * The box replay is ONE chunk launch per GDN layer over its rows; the no-spec
 * reference it replaces is one STEP launch per row per layer. The single-layer
 * gate (spec_path_equivalence) measured 4.47e-08 for two rows; this walks the same
 * two rows through all 48 layers, on the REAL pool layout (lane stride = 48 *
 * layer stride), so the output is the magnitude a single round actually injects -
 * which must sit in the 1e-6..1e-5 range to match the box's measured 1.6e-05 at
 * the first replay (position 235), where exactly one round has run.
 */
/*
 * FORTY-EIGHT-LAYER CHUNK vs STEP: the box's replay walk, end to end.
 *
 * A replay walks min_accepted + 2 rows through EVERY GDN layer in one chunk launch
 * per layer; the no-spec reference it replaces walks each row through each layer in
 * a one-row step launch. The single-layer gate measured 4.47e-08 for two rows; the
 * box's first replay injects a state difference of absmax 0.016 / absmean 1.6e-05.
 * The only place that extra three orders can come from is the 48-layer compounding
 * WITHIN one walk - each layer transforms the previous layer's slightly-different
 * state. This case runs that exactly: two lanes, two rows, all 48 layers, and the
 * difference after the LAST layer is the prediction.
 */
static int SparkQwen38_27bValCheckLayerAmplification(SparkQwen38_27bValDevice *device)
{
	const uint32_t rows = 2u;
	const uint32_t layers = SPARK_QWEN38_27B_MODEL_GDN_LAYER_COUNT;
	const uint64_t conv_elements = (uint64_t)rows * SPARK_QWEN38_27B_CONV;
	uint16_t *qkv_packed = (uint16_t *)calloc(conv_elements,sizeof(uint16_t));
	float *qkv_exact = (float *)calloc(conv_elements,sizeof(float));
	float *log_decay = (float *)calloc((uint64_t)rows * SPARK_QWEN38_27B_HEADS,sizeof(float));
	float *beta = (float *)calloc((uint64_t)rows * SPARK_QWEN38_27B_HEADS,sizeof(float));
	uint32_t warm[2] = {0u,0u};
	uint32_t lane0[1] = {0u},lane1[1] = {1u};
	float *state_pool = 0, *state_a = 0, *state_b = 0;
	uint16_t *tail_pool = 0;
	SparkQwen38_27bGdnStatePool pool;
	uint64_t layer_state, layer_tail, lane_state, lane_tail, state_elements, tail_elements, index;
	uint64_t state_bad = 0u;
	double absmax = 0.0, abssum = 0.0, scale = 0.0;
	uint32_t layer,row;
	cudaError_t error;
	if (qkv_packed == 0 || qkv_exact == 0 || log_decay == 0 || beta == 0)
		return(SparkQwen38_27bValFail("layer_amplification","host_alloc"));
	layer_state = (uint64_t)SPARK_QWEN38_27B_HEADS * SPARK_QWEN38_27B_DK * SPARK_QWEN38_27B_DV;
	layer_tail = (uint64_t)SPARK_QWEN38_27B_CONV * (SPARK_QWEN38_27B_MODEL_GDN_CONV_KERNEL - 1u);
	lane_state = layer_state * (uint64_t)layers;
	lane_tail = layer_tail * (uint64_t)layers;
	state_elements = lane_state * 2u;
	tail_elements = lane_tail * 2u;
	error = cudaMalloc((void **)&state_pool, state_elements * sizeof(float));
	if (error == cudaSuccess) error = cudaMalloc((void **)&tail_pool, tail_elements * sizeof(uint16_t));
	state_a = (float *)malloc(lane_state * sizeof(float));
	state_b = (float *)malloc(lane_state * sizeof(float));
	if (error != cudaSuccess || state_pool == 0 || tail_pool == 0 || state_a == 0 || state_b == 0)
		return(SparkQwen38_27bValFail("layer_amplification","pool_alloc"));
	/* A 48-layer pool over the freshly allocated region, lane_capacity 2. */
	memset(&pool,0,sizeof(pool));
	pool.abi_version = SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_GDN_STATE_POOL_ABI_VERSION;
	pool.lane_capacity = 2u;
	pool.gdn_layer_count = layers;
	pool.state_f32 = state_pool;
	pool.state_layer_stride_elements = layer_state;
	pool.state_lane_stride_elements = lane_state;
	pool.conv_tail_bf16 = tail_pool;
	pool.conv_tail_layer_stride_elements = layer_tail;
	pool.conv_tail_lane_stride_elements = lane_tail;
	pool.state_cold_by_row = device->cold;
	error = cudaMemset(state_pool,0,state_elements * sizeof(float));
	if (error == cudaSuccess) error = cudaMemset(tail_pool,0,tail_elements * sizeof(uint16_t));
	if (error == cudaSuccess) error = cudaMemcpy(device->cold,warm,sizeof(warm),cudaMemcpyHostToDevice);
	SparkQwen38_27bValRandomState = 99u;
	SparkQwen38_27bValFillBf16(qkv_packed,qkv_exact,conv_elements,1.0f);
	for (index = 0u; index < (uint64_t)rows * SPARK_QWEN38_27B_HEADS; index++)
	{
		log_decay[index] = SparkQwen38_27bValUniform(0.25f) - 0.25f;
		beta[index] = 0.25f + fabsf(SparkQwen38_27bValUniform(0.5f));
	}
	error = cudaMemcpy(device->qkv,qkv_packed,conv_elements * sizeof(uint16_t),cudaMemcpyHostToDevice);
	if (error == cudaSuccess) error = cudaMemcpy(device->log_decay,log_decay,(uint64_t)rows * SPARK_QWEN38_27B_HEADS * sizeof(float),cudaMemcpyHostToDevice);
	if (error == cudaSuccess) error = cudaMemcpy(device->beta,beta,(uint64_t)rows * SPARK_QWEN38_27B_HEADS * sizeof(float),cudaMemcpyHostToDevice);
	/* lane 0: the no-spec trajectory - one row at a time through the step path. */
	if (error == cudaSuccess) error = cudaMemcpy(device->lane_indices,lane0,sizeof(lane0),cudaMemcpyHostToDevice);
	for (layer = 0u; layer < layers && error == cudaSuccess; layer++)
		for (row = 0u; row < rows && error == cudaSuccess; row++)
		{
			error = SparkQwen38_27bLaunchConvUpdate(cudaStreamPerThread,device->qkv + ((uint64_t)row * SPARK_QWEN38_27B_CONV),
				&device->gdn_weights,device->conv_out + ((uint64_t)row * SPARK_QWEN38_27B_CONV),&pool,device->lane_indices,1u,layer,0,0,0);
			if (error == cudaSuccess)
				error = SparkQwen38_27bLaunchGdnStep(cudaStreamPerThread,device->conv_out + ((uint64_t)row * SPARK_QWEN38_27B_CONV),
					device->log_decay + ((uint64_t)row * SPARK_QWEN38_27B_HEADS),device->beta + ((uint64_t)row * SPARK_QWEN38_27B_HEADS),
					&pool,device->core_out + ((uint64_t)row * SPARK_QWEN38_27B_MODEL_GDN_VALUE_DIMENSION),device->lane_indices,1u,layer,0,0,0);
		}
	/* lane 1: the replay trajectory - both rows in one chunk launch per layer. */
	if (error == cudaSuccess) error = cudaMemcpy(device->lane_indices,lane1,sizeof(lane1),cudaMemcpyHostToDevice);
	for (layer = 0u; layer < layers && error == cudaSuccess; layer++)
	{
		error = SparkQwen38_27bLaunchChunkConv(cudaStreamPerThread,device->qkv,&device->gdn_weights,device->conv_out,&pool,1u,rows,layer);
		if (error == cudaSuccess)
			error = SparkQwen38_27bLaunchGdnChunk(cudaStreamPerThread,device->conv_out,device->log_decay,device->beta,
				device->chunk_qn,device->chunk_kn,device->chunk_cum_g,device->chunk_decay,device->chunk_attn,
				device->chunk_w,device->chunk_kg,&pool,device->core_out,1u,rows,layer);
	}
	if (error == cudaSuccess) error = cudaStreamSynchronize(cudaStreamPerThread);
	if (error == cudaSuccess) error = cudaMemcpy(state_a,state_pool,lane_state * sizeof(float),cudaMemcpyDeviceToHost);
	if (error == cudaSuccess) error = cudaMemcpy(state_b,state_pool + lane_state,lane_state * sizeof(float),cudaMemcpyDeviceToHost);
	if (SparkQwen38_27bValCuda(error,"layer_amplification") != 0)
		return(1);
	for (index = 0u; index < lane_state; index++)
	{
		double delta = fabs((double)state_a[index] - (double)state_b[index]);
		double magnitude = fabs((double)state_a[index]);
		if (state_a[index] != state_b[index]) state_bad++;
		abssum += delta;
		if (delta > absmax) absmax = delta;
		if (magnitude > scale) scale = magnitude;
	}
	printf("qwen38_27b_validation check=layer_amplification layers=%u rows=%u differing=%llu/%llu absmax=%.6g absmean=%.6g scale=%.6g\n",
	 ...
	 * SPARK_QWEN38_27B_SMALL_BATCH_GEMM=1. The gate therefore asserts the retraction:
	...
	printf("qwen38_27b_validation note=layer_amplification single_layer_was_4.47e-08 box_measured_absmean_1.6e-05\n");
	if (abssum / (double)lane_state >= 1e-7)
		return(SparkQwen38_27bValFail("layer_amplification","path substitution AMPLIFIED - chunk-vs-step retraction violated"));
 ...
 */
static int SparkQwen38_27bValCheckMultiRowAttention(SparkQwen38_27bValDevice *device)
{
	const uint32_t tokens = 8u;
	const uint64_t fused = (uint64_t)tokens * 2u * SPARK_QWEN38_27B_MODEL_ATTN_QUERY_DIMENSION;
	const uint64_t kv_elements = (uint64_t)tokens * SPARK_QWEN38_27B_MODEL_ATTN_KV_DIMENSION;
	const uint64_t out_elements = (uint64_t)tokens * SPARK_QWEN38_27B_MODEL_ATTN_QUERY_DIMENSION;
	const uint64_t cache_elements = (uint64_t)SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS * SPARK_QWEN38_27B_MODEL_ATTN_CACHE_TOKEN_ELEMENTS;
	...
	uint16_t *norm = (uint16_t *)calloc(2u * SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION,sizeof(uint16_t));
	...
	if (q == 0 || k == 0 || v == 0 || norm == 0 || out_multi == 0 || out_single == 0 ||
		...
		return(SparkQwen38_27bValFail("multi_row_attn","host_alloc"));
	SparkQwen38_27bValRandomState = 313u;
	SparkQwen38_27bValFillBf16(q,0,fused,0.5f);
	SparkQwen38_27bValFillBf16(k,0,kv_elements,0.5f);
	SparkQwen38_27bValFillBf16(v,0,kv_elements,0.5f);
	SparkQwen38_27bValFillBf16(norm,0,SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION,0.5f);
	SparkQwen38_27bValFillBf16(norm + SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION,0,SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION,0.5f);
	...
	if (error == cudaSuccess) error = cudaMemcpy(device->q_norm_weight,norm,SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION * sizeof(uint16_t),cudaMemcpyHostToDevice);
	if (error == cudaSuccess) error = cudaMemcpy(device->k_norm_weight,norm + SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION,SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION * sizeof(uint16_t),cudaMemcpyHostToDevice);
	memset(&table,0,sizeof(table));
	table.abi_version = SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TABLE_ABI_VERSION;
	table.descriptor_bytes = sizeof(table);
	table.block_token_count = SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS;
	...
	if (error == cudaSuccess)
		error = SparkQwen38_27bLaunchAttnPrepare(cudaStreamPerThread,dq_multi,dk_multi,dv_multi,&device->attn_weights,kv_multi,dslots_multi,dpos_multi,tokens,0u,cache_elements,cache_elements,SPARK_QWEN38_27B_MODEL_RMS_NORM_EPSILON);
	if (error == cudaSuccess) error = cudaStreamSynchronize(cudaStreamPerThread);
	if (error == cudaSuccess) error = cudaMemcpy(kv_multi_after,kv_multi,cache_elements * sizeof(uint16_t),cudaMemcpyDeviceToHost);
	if (error == cudaSuccess)
		error = SparkQwen38_27bLaunchAttnDecode(cudaStreamPerThread,dq_multi,kv_multi,&table,dlane_multi,dctx_multi,dout_multi,tokens,0u,cache_elements,cache_elements);
	if (error == cudaSuccess) error = cudaStreamSynchronize(cudaStreamPerThread);
	...
	for (row = 0u; row < tokens; row++)
	{
		...
		if (error == cudaSuccess)
			error = SparkQwen38_27bLaunchAttnPrepare(cudaStreamPerThread,(uint8_t *)dq_single + row * 2u * SPARK_QWEN38_27B_MODEL_ATTN_QUERY_DIMENSION * sizeof(uint16_t),(uint8_t *)dk_single + row * SPARK_QWEN38_27B_MODEL_ATTN_KV_DIMENSION * sizeof(uint16_t),(uint8_t *)dv_single + row * SPARK_QWEN38_27B_MODEL_ATTN_KV_DIMENSION * sizeof(uint16_t),&device->attn_weights,kv_single,dslots_single + row,dpos_single + row,1u,0u,cache_elements,cache_elements,SPARK_QWEN38_27B_MODEL_RMS_NORM_EPSILON);
		if (error == cudaSuccess)
			error = SparkQwen38_27bLaunchAttnDecode(cudaStreamPerThread,(uint8_t *)dq_single + row * 2u * SPARK_QWEN38_27B_MODEL_ATTN_QUERY_DIMENSION * sizeof(uint16_t),kv_single,&table,dlane_single,dctx_single,(uint8_t *)dout_single + row * SPARK_QWEN38_27B_MODEL_ATTN_QUERY_DIMENSION * sizeof(uint16_t),1u,0u,cache_elements,cache_elements);
	}
	if (error == cudaSuccess) error = cudaStreamSynchronize(cudaStreamPerThread);
	...
	if (SparkQwen38_27bValCuda(error,"multi_row_attn") != 0)
		return(1);
	...
	printf("qwen38_27b_validation check=multi_row_attn tokens=%u kv_bad=%llu/%llu first_kv=%llu out_bad=%llu/%llu first_out=%llu\n",
	...
	if (bad != 0u || kv_bad != 0u)
		return(SparkQwen38_27bValFail("multi_row_attn","the multi-row frame differs from per-row frames"));
	return(0);
}
...
int main(int argc, char **argv)
{
	...
	if (result == 0) result = SparkQwen38_27bValCheckSpecPathEquivalence(&device);
	if (result == 0) result = SparkQwen38_27bValCheckStepRowBatch(&device);
	if (result == 0) result = SparkQwen38_27bValCheckDriftAccumulation(&device);
	if (result == 0) result = SparkQwen38_27bValCheckLayerAmplification(&device);
	if (result == 0) result = SparkQwen38_27bValCheckMultiRowAttention(&device);
