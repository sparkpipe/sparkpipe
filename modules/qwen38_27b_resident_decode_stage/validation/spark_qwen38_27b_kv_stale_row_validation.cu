...
#define SPARK_KVTEST_QUERY_HEADS SPARK_QWEN38_27B_MODEL_ATTN_QUERY_HEAD_COUNT
#define SPARK_KVTEST_KV_HEADS SPARK_QWEN38_27B_MODEL_ATTN_KV_HEAD_COUNT
#define SPARK_KVTEST_HEAD_DIM SPARK_QWEN38_27B_MODEL_ATTN_HEAD_DIMENSION
#define SPARK_KVTEST_BLOCK_TOKENS SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_KV_BLOCK_TOKENS
...
extern "C" cudaError_t SparkQwen38_27bTpSetGeometry(uint32_t gdn_qk_channels,uint32_t gdn_value_channels,uint32_t gdn_conv_channels,uint32_t gdn_key_heads,uint32_t gdn_value_heads,uint32_t attn_query_heads,uint32_t attn_kv_heads,uint32_t gdn_qk_channel_base,uint32_t gdn_value_channel_base,uint32_t gdn_key_head_base,uint32_t gdn_value_head_base);
...
int main(void)
{
	...
	SparkQwen38_27bAttnLayerWeights weights;
	...
	SPARK_KVTEST_CUDA(SparkQwen38_27bTpSetGeometry(0u,0u,0u,0u,0u,SPARK_KVTEST_QUERY_HEADS,SPARK_KVTEST_KV_HEADS,0u,0u,0u,0u));
	...
	for (run = 0u; run < 5u; run++)
	{
		...
		SPARK_KVTEST_CUDA(SparkQwen38_27bLaunchAttnPrepare(0,query,key,value,&weights,cache,slot_mapping,row_positions,SPARK_KVTEST_ROWS,0u,layer_stride,block_stride,SPARK_QWEN38_27B_MODEL_RMS_NORM_EPSILON));
		SPARK_KVTEST_CUDA(SparkQwen38_27bLaunchAttnDecode(0,query,cache,&table,row_lanes,context_lengths,out,SPARK_KVTEST_ROWS,0u,layer_stride,block_stride));
