 ...
 * calls SparkQwen38_27bDsparkSelectorEmit - the exact inline sequence the module's
 ...
 *      SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_DSPARK_BLOCK_SIZE is what the serving
...
int main(int argc, char **argv)
{
	...
	printf("workspace bytes = %llu at slots=%u K=%u rank=%u\n",
		(unsigned long long)SparkQwen38_27bDsparkSelectorWorkspaceBytes(slot_count,top_k,rank),slot_count,top_k,rank);
	...
	SPARK_DFLASH2_CUDA(cudaMalloc((void **)&workspace.chunk_keys,(size_t)SparkQwen38_27bDsparkHeadTopKChunkKeyCount(slot_count,top_k) * sizeof(uint64_t)));
	...
	for (batch = 0u; batch < batch_count; batch++)
	{
		...
		SPARK_DFLASH2_CUDA(SparkQwen38_27bDsparkSelectorEmit(0,&head_view,
	...
	SparkDflash2Check("emitted width == adapter's count",
		slot_count == SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_DSPARK_BLOCK_SIZE ? 1u : 0u,
		slot_count == SPARK_QWEN38_27B_RESIDENT_DECODE_STAGE_DSPARK_BLOCK_SIZE ? "(7 = block 8 minus the anchor)" : "case slots != the adapter's draft count");
