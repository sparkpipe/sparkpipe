# qwen38_27b driver -> hwiface v1 island mapping (qwen27b-dev)
...
it prepares the qwen38_27b port so step 7 is a mechanical application of frozen
decisions, and it flags every place where the qwen38_27b architecture needs an
island-set EXTENSION (a coordinator/freeze act), not an interpretation.

## 1. Function -> island table

qwen38_27b = 48 GDN (gated delta net) layers + 16 full-attention layers, dense FFN,
PP-Nx stage slices, MTP chain drafter + DSpark block drafter (device selector).
Launchers: 33 SparkQwen38_27bLaunch* externs in module.c against the .cu archive.

| Island (freeze) | qwen38_27b functions today | File | Notes / class |
|---|---|---|---|
...
The freeze list was written from DSV4's module.c; qwen38_27b uses the same shape:
...
- collective: SparkQwen38_27bTpState over tp_device_collective (NCCL backend or
...
## 3. Extensions the qwen38_27b port needs from the interface (coordinator acts)

1. Dense-FFN island row. The frozen set only names routed/shared MoE. qwen38_27b
...
3. Drafter packs. MTP weights ride the main pack; the DSpark drafter loads a
   SECOND pack (SPARK_QWEN38_27B_DSPARK_PACK_PATH). Per freeze Q3 both heads stay
...
- keep this mapping current as the DSV4 S1/S2 splits land (they will name the
  exact spark_hw_iface.h content qwen38_27b then consumes);
- pre-stage the mechanical part: qwen38_27b's GB10 constants audit (same sweep-list
...
### qwen38_27b GB10 constant audit (F4 method)

- SPARK_QWEN38_27B_SMALL_BATCH_MAX_ROWS/TILE_N/K_CHUNK (module.c 372-374, mirrored
...
- GDN chunk length 64 (SPARK_QWEN38_27B_MODEL_GDN_CHUNK_TOKENS) and tap ring
