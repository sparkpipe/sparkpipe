...
# below are gemma4's own tier policy.

validation_label="Gemma4_27b"
validation_digest_label="Gemma4_27b"
...
validation_oracle_file="spark_gemma4_reference.c"
...
validation_nvcc_extra_args() {
    printf '%s\n' "-DSPARK_GEMMA4_STAGE_MAX_ACTIVE_SEQUENCES=${SPARK_GEMMA4_STAGE_MAX_ACTIVE_SEQUENCES:-8}"
}
...
require_configuration_value SPARK_GEMMA4_ALLOW_UNQUALIFIED_EXECUTION 1
require_configuration_value SPARK_GEMMA4_STAGE_INDEX 0
require_configuration_value SPARK_GEMMA4_STAGE_FIRST_LAYER 0
...
case "${SPARK_GEMMA4_STAGE_MAX_ACTIVE_SEQUENCES:-8}" in
    8|16|64) ;;
    *)
        echo "gemma4 hardware validation requires SPARK_GEMMA4_STAGE_MAX_ACTIVE_SEQUENCES in {8,16,64}, got '${SPARK_GEMMA4_STAGE_MAX_ACTIVE_SEQUENCES:-}'" >&2
        exit 2
        ;;
esac
...
if [[ "${SPARK_GEMMA4_TP_DEGREE:-1}" != "1" ]] || [[ "${SPARK_GEMMA4_TP_STANDALONE:-0}" == "1" ]]; then
    require_configuration_value SPARK_GEMMA4_TP_RANK 0
    require_configuration_value SPARK_GEMMA4_TP_STANDALONE 1
    require_configuration_value SPARK_GEMMA4_STAGE_COUNT 1
    require_configuration_value SPARK_GEMMA4_STAGE_LAYER_COUNT 48
    require_configuration_value SPARK_GEMMA4_STAGE_MTP 1
else
    if (( ${SPARK_GEMMA4_STAGE_COUNT:-0} < 2 )); then
        echo "gemma4 hardware validation requires SPARK_GEMMA4_STAGE_COUNT >= 2 (mid-pipeline stage 0)" >&2
        exit 2
    fi
    if (( ${SPARK_GEMMA4_STAGE_LAYER_COUNT:-0} < 4 || ${SPARK_GEMMA4_STAGE_LAYER_COUNT:-48} >= 48 )); then
        echo "gemma4 hardware validation requires 4 <= SPARK_GEMMA4_STAGE_LAYER_COUNT < 48 (GDN and full attention, no final head)" >&2
        exit 2
    fi
fi
if (( ${SPARK_GEMMA4_STAGE_KV_BLOCKS:-0} < 8 )); then
    echo "gemma4 hardware validation requires SPARK_GEMMA4_STAGE_KV_BLOCKS >= 8" >&2
    exit 2
fi
