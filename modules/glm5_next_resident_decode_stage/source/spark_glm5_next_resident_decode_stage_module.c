...
typedef struct SparkGlm5NextTpChain
{
	...
	uint32_t hill_covered;
	uint32_t hill_lazy;
	uint32_t hill_sweeps;
	uint64_t hill_start_ns;
...
static void SparkGlm5NextSweepWork(void *context)
{
	...
	fprintf(stderr,
	    "SWP st=%d rr=%u uni=%u all=%u\n",
	    (int32_t)status,chain->slot->route_recorded,
	    (unsigned)chain->state->decode_union_count,
    ...
    SparkGlm5NextTpChain *chain)
{
		...
		return(SPARK_STATUS_BUSY);
	{
		...
		uint32_t miss_index;
		for ( miss_index = 0u;
		      miss_index < recorded && miss_index <
...
static void SparkGlm5NextTpChainAdvance(void *chain_context,SparkStatus status)
{
	...
	case SPARK_GLM5_NEXT_CHAIN_STAGE_REDUCE_MLP:
		...
		if ( chain->next_layer >= 3u && chain->next_layer <= 31u )
		{
