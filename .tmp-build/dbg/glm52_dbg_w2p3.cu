...
static int SparkGlm52ValFixtureSetup(SparkGlm52ValFixture *fixture)
{
	...
	/* DBG: payload row R of lane L is one distinct E4M3 value, constant along
	 * K: tag = (R*7 + L*11) mod 119. 120 is coprime to every tile width the
	 * GEMM uses, so any row-freeze or tile-local freeze shows as structure.
	 * The oracle decodes the same host master, so comparisons stay honest. */
	{
		uint32_t row;
		payload_bytes = SparkGlm52ValPayloadBytesPerExpert(SPARK_GLM52_VAL_CODEC,SPARK_GLM52_VW2_ROWS,SPARK_GLM52_VW2_COLUMNS);
		for (lane = 0u; lane < SPARK_GLM52_VALIDATION_EXPERT_SLOTS; lane++)
		{
			uint8_t *slab = fixture->experts.w2_payload + (uint64_t)lane * payload_bytes;
			for (row = 0u; row < SPARK_GLM52_VW2_ROWS; row++)
			{
				int t = (int)(((row * 7u) + (lane * 11u)) % 119u);
				memset(slab + (uint64_t)row * SPARK_GLM52_VW2_COLUMNS,(uint8_t)(0x08 + (((row * 7u) + (lane * 11u)) % 119u)),SPARK_GLM52_VW2_COLUMNS);
			}
		}
		if ( cudaMemcpy(fixture->experts.w2_device_payload,fixture->experts.w2_payload,
			(uint64_t)SPARK_GLM52_VALIDATION_EXPERT_SLOTS * payload_bytes,cudaMemcpyHostToDevice) != cudaSuccess )
			return(SparkGlm52ValFail("fixture","dbg_upload"));
...
#define DBG_TAG_VALUES 119
static float DBG_tag_value(int index)
{
	/* E4M3 codes 0x08+i, i in 0..119: finite positive strictly increasing. */
...
static int DBG_decode(float v)
{
	...
	best = 1.0e30f;
	for (index = 0; index < DBG_TAG_VALUES; index++)
	{
		float d = fabsf(v - DBG_tag_value(index));
...
static int SparkGlm52ValRunRoutedTier(SparkGlm52ValFixture *fixture,int *result)
{
	 ...
	 * expert's forward recomputes from it directly. */
	{
		...
		SparkGlm52ValMeasure(&metrics,actual,reference,SPARK_GLM52_VHIDDEN);
		{
			uint32_t qi; double s = 0.0,hit = 0.0;
			...
			fprintf(stderr,"DBG packed_row=%u g0=%.6g sum_inter=%.6g\n",packed_row_zero,g0,sum_inter);
			fprintf(stderr,"DBG w2 got/pred j=0..15:");
			for (qi = 0u; qi < 16u; qi++)
			{
				got = DBG_decode(actual[qi] / sum_inter);
				pred = (int)(((qi * 7u) + (packed_row_zero * 11u)) % 119u);
				fprintf(stderr," %d/%d",got,pred);
			}
			fprintf(stderr,"\nDBG w2 got j@128-steps:");
			for (qi = 0u; qi < SPARK_GLM52_VHIDDEN; qi += 128u)
				fprintf(stderr," %d",DBG_decode(actual[qi] / sum_inter));
			fprintf(stderr,"\n");
			for (qi = 0u; qi < SPARK_GLM52_VHIDDEN; qi++)
			{
				got = DBG_decode(actual[qi] / sum_inter);
				t = (int)(((qi * 7u) + (packed_row_zero * 11u)) % 119u);
				if ( got == t || got == ((t + 1) % 119) || got == ((t + 119) % 119) )
					hit += 1.0;
			}
			fprintf(stderr,"DBG w2 identity-ish hits=%.0f/%d\n",hit,SPARK_GLM52_VHIDDEN);
				{
					uint32_t wi; double si = 0.0, sa = 0.0, worst;
					int wj = -1;
					fprintf(stderr,"DBG raw act0..3=%.6g,%.6g,%.6g,%.6g ref0..3=%.6g,%.6g,%.6g,%.6g\n",
						actual[0],actual[1],actual[2],actual[3],
						reference[0],reference[1],reference[2],reference[3]);
					fprintf(stderr,"DBG oracle.inter0..3=%.6g,%.6g,%.6g,%.6g\n",
						oracle.intermediate[0],oracle.intermediate[1],oracle.intermediate[2],oracle.intermediate[3]);
					for (wi = 0u; wi < SPARK_GLM52_VEXPERT_INTER; wi++)
						si += oracle.intermediate[wi];
					worst = 0.0; wj = -1;
					for (wi = 0u; wi < SPARK_GLM52_VHIDDEN; wi++)
					{
						double d = fabs((double)actual[wi] - reference[wi]);
						sa += (double)actual[wi] * actual[wi];
						if ( d > worst ) { worst = d; wj = (int)wi; }
					}
					fprintf(stderr,"DBG |act|=%.6g sum_inter_oracle=%.6g worst_j=%d worst_d=%.6g act_wj=%.6g ref_wj=%.6g\n",
						sqrt(sa),si,wj,worst,actual[wj],reference[wj]);
				}
		}
