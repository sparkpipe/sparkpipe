...
static int SparkGlm52ValFixtureSetup(SparkGlm52ValFixture *fixture)
{
	...
	if ( cudaMemcpy(fixture->experts.w2_device_scale,fixture->experts.w2_scales,scale_bytes,cudaMemcpyHostToDevice) != cudaSuccess )
		return(SparkGlm52ValFail("fixture","expert_upload"));
#ifdef SPARK_GLM52_DBG_CONST_SLABS
	/* DBG: make every expert slab byte-identical (E4M3 code 0x08, scales
	 * already all-1.0). Any K/neuron/expert permutation of the B fetch is
	 * transparent in this mode; only A staging, scale reads and output
	 * addressing can still break the numerics. */
	{
		payload_bytes = SparkGlm52ValPayloadBytesPerExpert(SPARK_GLM52_VAL_CODEC,SPARK_GLM52_VW1_ROWS,SPARK_GLM52_VEXPERT_COLUMNS);
		for (lane = 0u; lane < SPARK_GLM52_VALIDATION_EXPERT_SLOTS; lane++)
		...
		payload_bytes = SparkGlm52ValPayloadBytesPerExpert(SPARK_GLM52_VAL_CODEC,SPARK_GLM52_VW2_ROWS,SPARK_GLM52_VW2_COLUMNS);
		for (lane = 0u; lane < SPARK_GLM52_VALIDATION_EXPERT_SLOTS; lane++)
			memset(fixture->experts.w2_payload + (uint64_t)lane * payload_bytes,0x08,payload_bytes);
		if ( cudaMemcpy(fixture->experts.w2_device_payload,fixture->experts.w2_payload,
			(uint64_t)SPARK_GLM52_VALIDATION_EXPERT_SLOTS * payload_bytes,cudaMemcpyHostToDevice) != cudaSuccess )
			return(SparkGlm52ValFail("fixture","dbg_upload"));
		printf("glm52_validation dbg=const_slabs code=0x08\n");
	}
#endif
...
static int SparkGlm52ValRunRoutedTier(SparkGlm52ValFixture *fixture,int *result)
{
	 ...
	 * expert's forward recomputes from it directly. */
	{
		...
		SparkGlm52ValMeasure(&metrics,actual,reference,SPARK_GLM52_VHIDDEN);
		{
			uint32_t qi; double s = 0.0,mn,mx,p = 0.0,nn = 0.0;
			float c,g0;
			...
			for (qi = 0u; qi < SPARK_GLM52_VHIDDEN; qi++)
				s += oracle.normed[qi];
			g0 = SparkGlm52ValFromBf16(DBG_grow[0]); mn = mx = g0;
			for (qi = 0u; qi < 2u * SPARK_GLM52_VEXPERT_INTER; qi++)
			{
				float v = SparkGlm52ValFromBf16(DBG_grow[qi]);
				if ( v < mn ) mn = v;
				if ( v > mx ) mx = v;
			}
			for (qi = 0u; qi < SPARK_GLM52_VHIDDEN; qi++)
			{
				double d = (double)actual[qi] - reference[qi];
				p += d * d;
				nn += (double)reference[qi] * reference[qi];
			}
			fprintf(stderr,"DBG const: c=%.6g sum_normed=%.6g expect_g0=%.6g got_g0=%.6g implied_sum=%.6g gate_min=%.6g gate_max=%.6g inter0..3=%.6g,%.6g,%.6g,%.6g out0..3=%.6g/%.6g,%.6g/%.6g,%.6g/%.6g,%.6g/%.6g rel_l2=%.6g\n",
				c,s,(float)(c * s),g0,(float)(mn > 0.0f || mn < 0.0f ? g0 / c : 0.0),mn,mx,
				SparkGlm52ValFromBf16(DBG_irow[0]),SparkGlm52ValFromBf16(DBG_irow[1]),
				SparkGlm52ValFromBf16(DBG_irow[2]),SparkGlm52ValFromBf16(DBG_irow[3]),
				actual[0],reference[0],actual[1],reference[1],
				actual[2],reference[2],actual[3],reference[3],sqrt(p / (nn + 1e-30)));
		}
