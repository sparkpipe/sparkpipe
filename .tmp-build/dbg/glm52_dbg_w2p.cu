...
static int SparkGlm52ValFixtureSetup(SparkGlm52ValFixture *fixture)
{
	 ...
	 * K: tag = (R*7 + L*11) mod 120. 120 is coprime to every tile width the
	 ...
	 * The oracle decodes the same host master, so comparisons stay honest. */
	{
		...
		for (lane = 0u; lane < SPARK_GLM52_VALIDATION_EXPERT_SLOTS; lane++)
		{
			...
			{
				int t = (int)(((row * 7u) + (lane * 11u)) % 120u);
				float v = DBG_tag_value(t);
				uint8_t code = (uint8_t)(0x08 + ((v == 0.0f) ? 0 : t));
				(void)code;
				memset(slab + (uint64_t)row * SPARK_GLM52_VW2_COLUMNS,(uint8_t)(0x08 + (((row * 7u) + (lane * 11u)) % 120u)),SPARK_GLM52_VW2_COLUMNS);
			}
		}
	 ...
	 * expert's forward recomputes from it directly. */
	{
		...
		SparkGlm52ValMeasure(&metrics,actual,reference,SPARK_GLM52_VHIDDEN);
		{
			...
			for (qi = 0u; qi < 16u; qi++)
			{
				...
				pred = (int)(((qi * 7u) + (packed_row_zero * 11u)) % 120u);
			...
			{
				got = DBG_decode(actual[qi] / sum_inter);
				t = (int)(((qi * 7u) + (packed_row_zero * 11u)) % 120u);
				if ( got == t || got == ((t + 1) % 120) || got == ((t + 119) % 120) )
					hit += 1.0;
			}
