...
static int SparkGlm52ValFixtureSetup(SparkGlm52ValFixture *fixture)
{
	...
	if ( SparkGlm52ValAllocMatrix(&fixture->embedding,SPARK_GLM52_VALIDATION_EMBED_ROWS,SPARK_GLM52_VHIDDEN,0,0.05f) != 0 ||
		...
		/* The bias matrix MUST be allocated before the zero-fill below:
		 * this was missing and memset'd the zeroed struct's NULL host
		 * pointer - the fixture-setup SIGSEGV that blocked publication. */
	...
	if ( cudaMemcpy(fixture->experts.w2_device_scale,fixture->experts.w2_scales,scale_bytes,cudaMemcpyHostToDevice) != cudaSuccess )
		return(SparkGlm52ValFail("fixture","expert_upload"));
#ifdef SPARK_GLM52_DBG_CONST_W1
	/* DBG: uniform W1 codes so the routed intermediate is an exact known
	 * constant and every downstream signature becomes readable. */
	{
		payload_bytes = SparkGlm52ValPayloadBytesPerExpert(SPARK_GLM52_VAL_CODEC,SPARK_GLM52_VW1_ROWS,SPARK_GLM52_VEXPERT_COLUMNS);
		for (lane = 0u; lane < SPARK_GLM52_VALIDATION_EXPERT_SLOTS; lane++)
			memset(fixture->experts.w1_payload + (uint64_t)lane * payload_bytes,0x08,payload_bytes);
		if ( cudaMemcpy(fixture->experts.w1_device_payload,fixture->experts.w1_payload,
			(uint64_t)SPARK_GLM52_VALIDATION_EXPERT_SLOTS * payload_bytes,cudaMemcpyHostToDevice) != cudaSuccess )
			return(SparkGlm52ValFail("fixture","dbg_upload"));
		printf("glm52_validation dbg=const_w1\n");
	}
#endif
#ifdef SPARK_GLM52_DBG_ROWTAG_W2
	/* DBG: every payload ROW carries one distinct E4M3 tag value, constant
	 * along K, lane-shifted: tag_index = (row + 11*lane) & 31. Whatever
	 * (expert,row) the B fetch actually touches becomes readable back from
	 * the output magnitudes; a K-side permutation stays invisible here. */
	{
		static const uint8_t tag[32] = {0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f,
		...
		for (lane = 0u; lane < SPARK_GLM52_VALIDATION_EXPERT_SLOTS; lane++)
		{
			uint8_t *slab = fixture->experts.w1_payload + (uint64_t)lane * payload_bytes;
			for (row = 0u; row < SPARK_GLM52_VW1_ROWS; row++)
				memset(slab + (uint64_t)row * SPARK_GLM52_VEXPERT_COLUMNS,tag[(row + lane * 11u) & 31u],SPARK_GLM52_VEXPERT_COLUMNS);
		}
		if ( cudaMemcpy(fixture->experts.w1_device_payload,fixture->experts.w1_payload,
			(uint64_t)SPARK_GLM52_VALIDATION_EXPERT_SLOTS * payload_bytes,cudaMemcpyHostToDevice) != cudaSuccess )
			return(SparkGlm52ValFail("fixture","dbg_upload"));
		printf("glm52_validation dbg=rowtag_w2\n");
	}
#endif
...
static void SparkGlm52ValBuildWave(SparkGlm52ValFixture *fixture,uint32_t first_layer_index,uint32_t token,uint32_t position)
{
	...
	/* Remote field tree requires the attention-output and post-attention
	 * norm pointers wired too (sparke build's layer contract consumes
	 * them); synced from the sparke fix so both trees stay identical. */
...
static int DBD_decode_j(const float *values,float scale,uint32_t j)
{
	return(DBG_decode(values[j] / scale));
}
static uint16_t DBG_grow[2u * SPARK_GLM52_VEXPERT_INTER];
static uint16_t DBG_irow[SPARK_GLM52_VEXPERT_INTER];
static int DBG_decode(float v)
{
	float best;
	int index,index_best = 0;
	best = 1.0e30f;
	for (index = 0; index < 32; index++)
	{
		float value = ldexpf(1.0f,((index >> 3) + 1) - 7) * (1.0f + (float)(index & 7) / 8.0f);
		float d = fabsf(v - value);
		if ( d < best ) { best = d; index_best = index; }
	}
	return(index_best);
}
static float DBG_tag_value(int index)
{
	return(ldexpf(1.0f,((index >> 3) + 1) - 7) * (1.0f + (float)(index & 7) / 8.0f));
}
...
static int SparkGlm52ValRunRoutedTier(SparkGlm52ValFixture *fixture,int *result)
{
	...
	for (token = 0u; token < 2u; token++)
	{
		...
		if ( token == 0u )
		{
			...
			cudaMemcpy(DBG_grow,fixture->gate_up + (uint64_t)packed_row_zero * (2u * SPARK_GLM52_VEXPERT_INTER),
				(uint64_t)2u * SPARK_GLM52_VEXPERT_INTER * sizeof(uint16_t),cudaMemcpyDeviceToHost);
			cudaMemcpy(DBG_irow,fixture->intermediate + (uint64_t)packed_row_zero * SPARK_GLM52_VEXPERT_INTER,
				(uint64_t)SPARK_GLM52_VEXPERT_INTER * sizeof(uint16_t),cudaMemcpyDeviceToHost);
		}
	}
	if ( SparkGlm52ValCheckAccessError(fixture) != 0 )
	{
	 ...
	 * expert's forward recomputes from it directly. */
	{
		...
		SparkGlm52ValMeasure(&metrics,actual,reference,SPARK_GLM52_VHIDDEN);
		{
			uint32_t qi; double s = 0.0;
			float c,g0,ic,sum_inter;
			int fit_w1 = -1,fit_w2 = -1;
			double hit_w1,hit_w2;
			c = SparkGlm52ValDequantWeight(fixture->experts.w1_payload,fixture->experts.w1_scales,
				SPARK_GLM52_VAL_CODEC,256u,selected[0],0u,SPARK_GLM52_VW1_ROWS,SPARK_GLM52_VEXPERT_COLUMNS,0u);
			for (qi = 0u; qi < SPARK_GLM52_VHIDDEN; qi++)
				s += oracle.normed[qi];
			g0 = SparkGlm52ValFromBf16(DBG_grow[0]);
			ic = SparkGlm52ValFromBf16(SparkGlm52ValBf16((g0 / (1.0f + expf(-g0))) * g0));
			sum_inter = ic * (float)SPARK_GLM52_VEXPERT_INTER;
			fprintf(stderr,"DBG packed_row=%u c=%.6g sum_normed=%.6g g0=%.6g ic=%.6g\n",
				packed_row_zero,c,s,g0,ic);
			fprintf(stderr,"DBG w1 idx j=0..47:");
			for (qi = 0u; qi < 48u; qi++)
				fprintf(stderr," %d",DBG_decode(SparkGlm52ValFromBf16(DBG_grow[qi]) / (float)s));
			fprintf(stderr,"\n");
			hit_w1 = 0.0;
			for (qi = 0u; qi < 2u * SPARK_GLM52_VEXPERT_INTER; qi++)
			{
				int got = DBG_decode(SparkGlm52ValFromBf16(DBG_grow[qi]) / (float)s);
				if ( got == (int)((qi + (unsigned)packed_row_zero * 11u) & 31u) )
					hit_w1 += 1.0;
			}
			fprintf(stderr,"DBG w1 affine(row+11*packed_row) hits=%.0f/%d\n",hit_w1,2 * SPARK_GLM52_VEXPERT_INTER);
			fprintf(stderr,"DBG w2 idx @tiles:");
			for (qi = 0u; qi < SPARK_GLM52_VHIDDEN; qi += 64u)
				fprintf(stderr," %d:%d",qi,DBD_decode_j(actual,sum_inter,qi));
			fprintf(stderr,"\n");
			fprintf(stderr,"DBG w2 raw j=0..7:");
			for (qi = 0u; qi < 8u; qi++)
				fprintf(stderr," %.6g",actual[qi]);
			fprintf(stderr," predict_v31=%.6g sum_inter=%.6g\n",
				DBG_tag_value(31) * sum_inter,sum_inter);
			hit_w2 = 0.0;
			for (qi = 0u; qi < SPARK_GLM52_VHIDDEN; qi++)
			{
				int got = DBG_decode(actual[qi] / sum_inter);
				if ( got == (int)((qi + (unsigned)packed_row_zero * 11u) & 31u) )
					hit_w2 += 1.0;
			}
			fprintf(stderr,"DBG w2 affine(row+11*packed_row) hits=%.0f/%d\n",hit_w2,SPARK_GLM52_VHIDDEN);
			(void)fit_w1; (void)fit_w2;
		}
