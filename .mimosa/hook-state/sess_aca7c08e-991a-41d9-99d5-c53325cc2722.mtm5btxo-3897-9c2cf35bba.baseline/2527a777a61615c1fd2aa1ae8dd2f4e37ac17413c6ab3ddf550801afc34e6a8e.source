...
while :; do
  ...
  for s in 0 1 2 3; do
    for t in 0 1 2 3; do
      ...
      info=$(ssh "${STAGE_HOST[$s]}" "stat -c %s $f 2>/dev/null || echo 0; pgrep -f 'k3_shard.py' >/dev/null && echo RUNNING || echo DONE" 2>/dev/null)
      size=$(echo "$info" | head -1 | tr -dc '0-9')
      state=$(echo "$info" | tail -1)
      [ "${size:-0}" -gt 90000000000 ] 2>/dev/null && [ "$state" = "DONE" ] || ready=0
    done
  done
