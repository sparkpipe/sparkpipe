	...
	uint32_t second_output_dimension)
{
	...
	uint32_t neuron = blockIdx.x * SPARK_LM_CTA_WARPS + warp;
	uint32_t element,pair_count = input_dimension >> 1u;
	float first,second;
	float2 input_pair;
	...
	if ( neuron < first_output_dimension && neuron < second_output_dimension )
	{
		SparkLmDotRowFp8Pair<GROUP_SIZE>(shared_input,first_payload,first_scale,
			second_payload,second_scale,neuron,input_dimension,lane,&first,&second);
		first = SparkLmWarpReduceSum(first);
		second = SparkLmWarpReduceSum(second);
		if ( lane == 0u )
		{
			SparkLmFloatToBf16(first_output_bf16,neuron,first);
			SparkLmFloatToBf16(second_output_bf16,neuron,second);
		}
	}
	else if ( neuron < first_output_dimension )
	{
		first = SparkLmDotRowFp8<GROUP_SIZE>(shared_input,first_payload,
			first_scale,neuron,input_dimension,lane);
		first = SparkLmWarpReduceSum(first);
		if ( lane == 0u )
			SparkLmFloatToBf16(first_output_bf16,neuron,first);
	}
	else if ( neuron < second_output_dimension )
	{
		second = SparkLmDotRowFp8<GROUP_SIZE>(shared_input,second_payload,
			second_scale,neuron,input_dimension,lane);
		second = SparkLmWarpReduceSum(second);
		if ( lane == 0u )
			SparkLmFloatToBf16(second_output_bf16,neuron,second);
	}
	...
	uint32_t input_dimension,uint32_t output_dimension)
{
	...
	uint32_t warp = threadIdx.x / SPARK_LM_WARP_LANES;
	uint32_t lane = threadIdx.x % SPARK_LM_WARP_LANES;
	uint32_t neuron = blockIdx.y * SPARK_LM_CTA_WARPS + warp;
	...
	if ( neuron >= output_dimension )
		return;
	SparkLmDotRowBf16Pair(shared_input,first_weight,second_weight,neuron,
		input_dimension,lane,&first,&second);
	first = SparkLmWarpReduceSum(first);
	second = SparkLmWarpReduceSum(second);
	if ( lane == 0u )
	{
		SparkLmFloatToBf16(first_output_bf16,
			((uint64_t)row * output_dimension) + neuron,first);
		SparkLmFloatToBf16(second_output_bf16,
			((uint64_t)row * output_dimension) + neuron,second);
	}
}
	...
	float limit)
{
	...
	for (task=blockIdx.x; task<total_tasks; task+=gridDim.x)
	{
		...
		if ( row >= __ldg(group_row_offset + group + 1u) )
			asm volatile("trap;\n");
	...
	uint32_t output_dimension)
{
	...
	for (task=blockIdx.x; task<total_tasks; task+=gridDim.x)
	{
		...
		if ( row >= __ldg(group_row_offset + group + 1u) )
			asm volatile("trap;\n");
		SparkLmSm121B1ExpertW2Task<TILE_N>(weight_payload,weight_scale_e8m0,
			input_bf16,output_bf16,group,row,neuron_base,input_dimension,
			output_dimension,shared_input);
	}
}

template<uint32_t WEIGHT_BITS>
static __global__ void SparkLmSm121FusedDenseW13GemvKernel(
	const uint8_t *w1_payload,
	const uint8_t *w1_scale_e8m0,
	const uint8_t *w3_payload,
	const uint8_t *w3_scale_e8m0,
	...
	float limit)
{
	...
	uint32_t warp = threadIdx.x / SPARK_LM_WARP_LANES;
	uint32_t lane = threadIdx.x % SPARK_LM_WARP_LANES;
	uint32_t neuron = blockIdx.x * SPARK_LM_CTA_WARPS + warp,element;
	float gate,up;
	for (element=threadIdx.x; element<input_dimension; element+=blockDim.x)
		shared_input[element] = SparkLmBf16ToFloat(input_bf16,element);
	__syncthreads();
	...
	float limit)
{
	...
	if ( row_count == 1u )
	{
		SparkLmSm121FusedDenseW13GemvKernel<WEIGHT_BITS>
			<<<((output_dimension + SPARK_LM_CTA_WARPS - 1u) / SPARK_LM_CTA_WARPS),SPARK_LM_CTA_THREADS,input_dimension * sizeof(float),stream>>>(
				(const uint8_t *)w1_payload,w1_scale_e8m0,
				(const uint8_t *)w3_payload,w3_scale_e8m0,input_bf16,
				activated_bf16,input_dimension,output_dimension,limit);
		return(cudaGetLastError());
	}
	dim3 grid((row_count + SPARK_LM_SM121_NATIVE_TILE_M - 1u) /
		SPARK_LM_SM121_NATIVE_TILE_M,
		output_dimension / SPARK_LM_SM121_NATIVE_TILE_N);
	SparkLmSm121FusedDenseW13Kernel<SPARK_LM_SM121_NATIVE_TILE_M,WEIGHT_BITS>
		<<<grid,SPARK_LM_CTA_THREADS,0,stream>>>(
			(const uint8_t *)w1_payload,w1_scale_e8m0,
			(const uint8_t *)w3_payload,w3_scale_e8m0,input_bf16,
			activated_bf16,row_count,input_dimension,output_dimension,limit);
	return(cudaGetLastError());
}
	...
	uint32_t multiprocessor_count)
{
	...
	if ( rows == 1024u )
		SparkLmSm121FusedExpertW13Kernel<SPARK_LM_SM121_NATIVE_WIDE_TILE_M>
			<<<multiprocessor_count,SPARK_LM_CTA_THREADS,0,stream>>>(
				(const uint8_t *)w1_payload,w1_scale_e8m0,
				(const uint8_t *)w3_payload,w3_scale_e8m0,input_bf16,
				source_row_map,group_row_offset,group_tile_prefix,activated_bf16,
				rows,group_count,input_dimension,output_dimension,limit);
	else
		SparkLmSm121FusedExpertW13Kernel<SPARK_LM_SM121_NATIVE_TILE_M>
			<<<multiprocessor_count,SPARK_LM_CTA_THREADS,0,stream>>>(
				(const uint8_t *)w1_payload,w1_scale_e8m0,
				(const uint8_t *)w3_payload,w3_scale_e8m0,input_bf16,
				source_row_map,group_row_offset,group_tile_prefix,activated_bf16,
	...
	uint32_t multiprocessor_count)
{
	...
	if ( rows == 1024u )
		SparkLmSm121ExpertW2Kernel<SPARK_LM_SM121_NATIVE_WIDE_TILE_M>
			<<<multiprocessor_count,SPARK_LM_CTA_THREADS,0,stream>>>(
				(const uint8_t *)weight_payload,weight_scale_e8m0,input_bf16,
				group_row_offset,group_tile_prefix,output_bf16,packed_rows,
				group_count,input_dimension,output_dimension);
	else
		SparkLmSm121ExpertW2Kernel<SPARK_LM_SM121_NATIVE_TILE_M>
			<<<multiprocessor_count,SPARK_LM_CTA_THREADS,0,stream>>>(
				(const uint8_t *)weight_payload,weight_scale_e8m0,input_bf16,
				group_row_offset,group_tile_prefix,output_bf16,packed_rows,
				group_count,input_dimension,output_dimension);
	...
	uint32_t row_count,uint32_t input_dimension,uint32_t output_dimension)
{
	...
	if ( row_count < SPARK_LM_TILE )
	{
		grid = dim3(row_count,(output_dimension + SPARK_LM_CTA_WARPS - 1u) /
			SPARK_LM_CTA_WARPS);
		SparkLmBf16LinearPairKernel<<<grid,SPARK_LM_CTA_THREADS,
...
{
	uint32_t maximum_output_dimension;
	cudaError_t error;
	...
	maximum_output_dimension = first_output_dimension > second_output_dimension ?
		first_output_dimension : second_output_dimension;
	SparkLmFp8LinearPairKernel<GROUP_SIZE,ACTIVATION_CODEC><<<
		(maximum_output_dimension + SPARK_LM_CTA_WARPS - 1u) /
