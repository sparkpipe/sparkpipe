...
static SparkStatus SparkQwen4FlashWorkControlValidatePlanConfiguration(
...
static SparkStatus SparkQwen4FlashWorkControlPrepareBlock(
	const SparkQwen4FlashWorkControlKvPlanConfig *configuration,
...
static SparkStatus SparkQwen4FlashWorkControlProgressBatch(
	SparkStageKvClient *client,
	SparkQwen4FlashWorkControlKvBatchState *batch_state)
{
	SparkKvStoreCompletion completion;
	SparkStatus status;
	if ( batch_state->state != SPARK_QWEN4_FLASH_WORK_CONTROL_BATCH_SUBMITTED )
		return(SPARK_STATUS_OK);
	...
	batch_state->state = SPARK_QWEN4_FLASH_WORK_CONTROL_BATCH_READY;
	return(SPARK_STATUS_OK);
}
	...
	uint32_t *cumulative_nonresident_block_counts)
{
	...
	for ( packet_index = 0u; packet_index < packet_count; ++packet_index )
	{
		...
		for ( packet_lane_index = 0u;
			...
			++packet_lane_index )
		{
			const SparkQwen4FlashWorkControlPendingLane *lane;
			uint64_t lane_count;
	...
	uint32_t *lanes_built)
{
	...
	SparkStatus status;
	status = SparkQwen4FlashWorkControlValidatePlanConfiguration(configuration);
	...
	*lanes_built = 0u;
	...
	for ( lane_index = 0u; lane_index < pending_lane_count; ++lane_index )
	{
		...
		if ( lane->gdn_nonresident != 0u )
		{
			status = SparkQwen4FlashWorkControlPrepareBlock(
	...
	uint32_t priority)
{
	...
	if ( client == 0 || batch_state == 0 || blocks == 0
		...
		|| priority > SPARK_QWEN4_FLASH_WORK_CONTROL_RESTORE_PRIORITY_SPECULATIVE )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	if ( batch_state->state != SPARK_QWEN4_FLASH_WORK_CONTROL_BATCH_IDLE )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	...
	batch_state->state = SPARK_QWEN4_FLASH_WORK_CONTROL_BATCH_SUBMITTED;
	...
	SparkQwen4FlashWorkControlKvState *state)
{
	...
	if ( client == 0 || state == 0 )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	status = SparkQwen4FlashWorkControlProgressBatch(client,&state->restore);
	if ( status != SPARK_STATUS_OK )
		return(status);
	return(SparkQwen4FlashWorkControlProgressBatch(client,&state->evict));
}
