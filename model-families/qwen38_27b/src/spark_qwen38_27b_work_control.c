...
static SparkStatus SparkQwen38_27bWorkControlValidatePlanConfiguration(
...
static SparkStatus SparkQwen38_27bWorkControlPrepareBlock(
	const SparkQwen38_27bWorkControlKvPlanConfig *configuration,
...
static SparkStatus SparkQwen38_27bWorkControlProgressBatch(
	SparkStageKvClient *client,
	SparkQwen38_27bWorkControlKvBatchState *batch_state)
{
	SparkKvStoreCompletion completion;
	SparkStatus status;
	if ( batch_state->state != SPARK_QWEN38_27B_WORK_CONTROL_BATCH_SUBMITTED )
		return(SPARK_STATUS_OK);
	...
	batch_state->state = SPARK_QWEN38_27B_WORK_CONTROL_BATCH_READY;
	return(SPARK_STATUS_OK);
}
	...
	uint32_t *cumulative_nonresident_block_counts)
{
	...
	for ( packet_index = 0u; packet_index < packet_count; ++packet_index )
	{
		...
		for ( packet_lane_index = 0u;
			...
			++packet_lane_index )
		{
			const SparkQwen38_27bWorkControlPendingLane *lane;
			uint64_t lane_count;
	...
	uint32_t *lanes_built)
{
	...
	SparkStatus status;
	status = SparkQwen38_27bWorkControlValidatePlanConfiguration(configuration);
	...
	*lanes_built = 0u;
	...
	for ( lane_index = 0u; lane_index < pending_lane_count; ++lane_index )
	{
		...
		if ( lane->gdn_nonresident != 0u )
		{
			status = SparkQwen38_27bWorkControlPrepareBlock(
	...
	uint32_t priority)
{
	...
	if ( client == 0 || batch_state == 0 || blocks == 0
		...
		|| priority > SPARK_QWEN38_27B_WORK_CONTROL_RESTORE_PRIORITY_SPECULATIVE )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	if ( batch_state->state != SPARK_QWEN38_27B_WORK_CONTROL_BATCH_IDLE )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	...
	batch_state->state = SPARK_QWEN38_27B_WORK_CONTROL_BATCH_SUBMITTED;
	...
	SparkQwen38_27bWorkControlKvState *state)
{
	...
	if ( client == 0 || state == 0 )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	status = SparkQwen38_27bWorkControlProgressBatch(client,&state->restore);
	if ( status != SPARK_STATUS_OK )
		return(status);
	return(SparkQwen38_27bWorkControlProgressBatch(client,&state->evict));
}
